...
public class MessageAuthCodeTest {
	...
	private static final byte[] hmacMessage =
	...
	private static final byte[][] falseMacs = {
		Hex.decode(
			"4bb5e21dd13001ed5faccfcfdaf8a854881dc200c9833da726e9376c2e32cff7"
		),
		Hex.decode(
	...
	public void testVerifyData()
		...
		for (int i = 0; i < types.length; i++) {
			...
			assertTrue(
				"MACType: " + types[i].name(),
				mac.verifyData(trueMacs[i], messages[i])
			);
		}
	}
	...
	public void testVerifyDataFalse()
		...
		for (int i = 0; i < types.length; i++) {
			...
			assertFalse(
				"MACType: " + types[i].name(),
				mac.verifyData(falseMacs[i], messages[i])
			);
		}
	}
