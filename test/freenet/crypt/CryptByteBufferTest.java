...
public class CryptByteBufferTest {
	...
	private static final CryptByteBufferType[] cipherTypes =
		CryptByteBufferType.values();
	...
	private static final byte[][] keys = {
		Hex.decode(
			"deadbeefcafebabe0123456789abcdefcafebabedeadbeefcafebabe01234567"
		),
		Hex.decode(
			"deadbeefcafebabe0123456789abcdefcafebabedeadbeefcafebabe01234567"
		),
		...
		Hex.decode(
			"a63add96a3d5975e2dad2f904ff584a32920e8aa54263254161362d1fb785790"
		),
	};
	...
	public void testSuccessfulRoundTripByteArray()
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			byte[] decipheredtext = crypt.decryptCopy(
				crypt.encryptCopy(Hex.decode(plainText[i]))
			);
			assertArrayEquals(
				"CryptByteBufferType: " + type.name(),
				Hex.decode(plainText[i]),
				decipheredtext
			);
		}
	}
	...
	@Test
	public void testSuccessfulRoundTripInPlace()
		throws GeneralSecurityException {
		for (int i = 0; i < cipherTypes.length; i++) {
			CryptByteBufferType type = cipherTypes[i];
			CryptByteBuffer crypt;
	...
	@Test
	public void testSuccessfulRoundTripInPlaceOffset()
		throws GeneralSecurityException {
		int header = 5;
		int footer = 5;
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			assertArrayEquals(
				...
				Arrays.copyOfRange(
					buffer,
					footer,
					footer + originalPlaintext.length
				)
			);
		}
	}
	...
	@Test
	public void testSuccessfulRoundTripOutOfPlaceOffset()
		throws GeneralSecurityException {
		int inHeader = 5;
		int inFooter = 5;
		int outHeader = 33;
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			byte[] outBuffer = new byte[outHeader +
			originalPlaintext.length +
			outFooter];
			crypt.encrypt(
				buffer,
			...
			assertArrayEquals(
				...
				Arrays.copyOfRange(
					buffer,
					inFooter,
					inFooter + originalPlaintext.length
				)
			);
		}
	}
	...
	public void testSuccessfulRoundTripByteArrayReset()
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			assertTrue(
				"CryptByteBufferType: " + type.name(),
				plain.equals(decipheredtext1)
			);
			assertTrue(
				"CryptByteBufferType: " + type.name(),
				plain.equals(decipheredtext2)
			);
			assertTrue(
				"CryptByteBufferType: " + type.name(),
				plain.equals(decipheredtext3)
			);
		}
	}
	...
	public void testEncryptWrapByteBuffer() throws GeneralSecurityException {
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			ByteBuffer plaintext = ByteBuffer.wrap(
				buf,
				header,
				origPlaintext.length
			);
	...
	public void testEncryptByteBufferToByteBuffer()
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			ByteBuffer ciphertext = ByteBuffer.wrap(
				ciphertextBuf,
				header,
				origPlaintext.length
			);
	...
	@Test
	public void testEncryptByteBufferToByteBufferDirect()
		throws GeneralSecurityException {
		for (int i = 0; i < cipherTypes.length; i++) {
			CryptByteBufferType type = cipherTypes[i];
			CryptByteBuffer crypt;
			...
			ByteBuffer plaintext = ByteBuffer.allocateDirect(
				origPlaintext.length
			);
			plaintext.put(origPlaintext);
			plaintext.position(0);
			ByteBuffer ciphertext = ByteBuffer.allocateDirect(
				origPlaintext.length
			);
			crypt.encrypt(plaintext, ciphertext);
	...
	public void testSuccessfulRoundTripByteArrayNewInstance()
		...
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			assertArrayEquals(
				"CryptByteBufferType2: " + type.name(),
				plain,
				decipheredtext
			);
			decipheredtext = crypt.decryptCopy(ciphertext3);
			assertArrayEquals(
				"CryptByteBufferType3: " + type.name(),
				plain,
				decipheredtext
			);
		}
	...
	public void testGenIVLength()
		...
		assertEquals(
			crypt.genIV().getIV().length,
			cipherTypes[i].ivSize.intValue()
		);
	}
	...
	public void testOverlappingEncode() throws GeneralSecurityException {
		for (int i = 0; i < cipherTypes.length; i++) {
			...
			assertArrayEquals(
				originalCiphertext,
				Arrays.copyOfRange(buf, 1, buf.length)
			);
		}
	}
