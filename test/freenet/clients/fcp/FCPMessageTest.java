...
public class FCPMessageTest {
	...
	private static final String LIST_REQUEST_IDENTIFIER =
	...
	public void wrappingNullReturnsNull() {
		assertThat(
			FCPMessage.withListRequestIdentifier(null, LIST_REQUEST_IDENTIFIER),
			nullValue()
		);
	}
	...
	public void wrappingMessageAddsIdentifier() {
		when(originalMessage.getFieldSet()).thenReturn(
			new SimpleFieldSet(true)
		);
		FCPMessage wrappedMessage = FCPMessage.withListRequestIdentifier(
			originalMessage,
			IDENTIFIER
		);
		assertThat(
			wrappedMessage.getFieldSet().get(LIST_REQUEST_IDENTIFIER),
			is(IDENTIFIER)
		);
	}
	...
	public void messageIsNotWrappedIfListRequestIdentifierIsNull() {
		assertThat(
			FCPMessage.withListRequestIdentifier(originalMessage, null),
	...
	public void wrappedMessageDelegatesRun() throws MessageInvalidException {
		...
		FCPConnectionHandler connectionHandler = mock(
			FCPConnectionHandler.class
		);
