...
public final class FCPPluginMessageEncodeDecodeTest {
	...
	public final void testEncodeDecode()
		...
		messages.add(
			FCPPluginMessage.constructSuccessReply(FCPPluginMessage.construct())
		);
	...
	private final void testEncodeDecode(FCPPluginMessage message)
		throws MessageInvalidException, IOException, FSParseException {
		SimpleFieldSet encodedMessage = new FCPPluginServerMessage(
			"testPlugin",
			message
		).getFieldSet();
		...
		SimpleFieldSet params = encodedMessage.subset(
			FCPPluginServerMessage.PARAM_PREFIX
		);
		if (params != null) {
		...
		FCPPluginMessage decodedMessage = new FCPPluginClientMessage(
			encodedMessage
		...
		if (message.params == null) {
			...
			assertEquals(
				...
				decodedMessage.params.toOrderedString()
			);
		}
