...
public class GetFailedMessageTest {
	...
	public void getFailedMessageWithoutGlobalFlagDoesNotContainGlobalFlagInFieldSet() {
		GetFailedMessage message = new GetFailedMessage(
			fetchException,
			"TestMessage",
			false
		);
		assertThat(
			message.getFieldSet().get("Global"),
			anyOf(nullValue(), equalTo("false"))
		);
	}
	...
	private final FetchException fetchException = new FetchException(
		INTERNAL_ERROR
	);
}
