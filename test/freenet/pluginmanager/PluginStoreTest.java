...
public class PluginStoreTest {
	...
	private void check(PluginStore store)
		...
		assertEquals(
			fs.toOrderedString(),
			copyStore.exportStoreAsSFS().toOrderedString()
		);
	}
	...
	@Test
	public void testStringsWithInvalidChars()
		throws IllegalBase64Exception, FSParseException {
	...
	private char randomChar(Random r) {
		...
		if (mode == 0) return (char) (32 + r.nextInt(95));
		else if (mode == 1) return invalidCharsForSFS.charAt(
			r.nextInt(invalidCharsForSFS.length())
		);
		else {
			while (true) {
	...
	@Test
	public void testWriteStringArrays()
		throws IllegalBase64Exception, FSParseException {
		String key = "test";
