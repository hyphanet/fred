...
public class ContentFilterTest {
	...
	private static final String EXTERNAL_LINK_CHECK1 =
	...
	private static final String INTERNAL_RELATIVE_LINK =
		"<a href=\"/KSK@gpl.txt\" />";
	private static final String INTERNAL_ABSOLUTE_LINK =
		"<a href=\"" + BASE_URI + "KSK@gpl.txt\" />";

	private static final String INTERNAL_RELATIVE_LINK1 =
		"<a href=\"test.html\" />";
	...
	// @see bug #2496
	private static final String ANCHOR_RELATIVE1 =
		"<a href=\"/KSK@test/test.html#C2\">";
	private static final String ANCHOR_RELATIVE2 =
		"<a href=\"/KSK@test/path/test.html#C2\">";
	...
	private static final String ANCHOR_MIXED =
		"<a href=\"/KSK@test/path/music #1.ogg\">";
	private static final String ANCHOR_MIXED_RESULT =
		"<a href=\"/KSK@test/path/music%20%231.ogg\">";
	...
	// @see bug #2297
	private static final String PREVENT_FPROXY_ACCESS =
		"<a href=\"" + BASE_URI + "\"/>";
	// @see bug #2921
	...
	private static final String FRAME_SRC_CHARSET_BAD1C =
		"<frame src=\"test.html?type=text/html\">";
	...
	public void testHTMLFilter() throws Exception {
		if (TestProperty.VERBOSE) {
			Logger.setupStdoutLogging(
				LogLevel.MINOR,
				"freenet.client.filter.Generic:DEBUG"
			);
		}
		...
		assertEquals(
			INTERNAL_RELATIVE_LINK,
			htmlFilter(INTERNAL_RELATIVE_LINK, true)
		);
		assertEquals(
			INTERNAL_RELATIVE_LINK1,
			htmlFilter(INTERNAL_RELATIVE_LINK1, true)
		);
		assertEquals(
			INTERNAL_RELATIVE_LINK,
		...
		assertTrue(
			htmlFilter(EXTERNAL_LINK_CHECK1).startsWith(EXTERNAL_LINK_OK)
		);
		assertTrue(
			htmlFilter(EXTERNAL_LINK_CHECK2).contains(ExternalLinkToadlet.PATH)
		);
		assertTrue(
			htmlFilter(EXTERNAL_LINK_CHECK3).startsWith(EXTERNAL_LINK_OK)
		);
		...
		// bug #2451
		assertEquals(
			POUNT_CHARACTER_ENCODING_TEST_RESULT,
			htmlFilter(POUNT_CHARACTER_ENCODING_TEST)
		);
		// bug #2297
		assertTrue(
			htmlFilter(PREVENT_FPROXY_ACCESS).contains(ExternalLinkToadlet.PATH)
		);
		// bug #2921
		assertTrue(
			htmlFilter(PREVENT_EXTERNAL_ACCESS_CSS_SIMPLE).contains("div { }")
		);
		assertTrue(
			htmlFilter(PREVENT_EXTERNAL_ACCESS_CSS_ESCAPE).contains("div { }")
		);
		assertTrue(
			htmlFilter(PREVENT_EXTERNAL_ACCESS_CSS_CASE).contains("div { }")
		);
		assertEquals(
			WHITELIST_STATIC_CONTENT,
			htmlFilter(WHITELIST_STATIC_CONTENT)
		);
		assertEquals(XHTML_VOIDELEMENTC, htmlFilter(XHTML_VOIDELEMENT));
		assertEquals(
			XHTML_INCOMPLETEDOCUMENTC,
			htmlFilter(XHTML_INCOMPLETEDOCUMENT)
		);
		...
		assertEquals(
			HTML_STYLESHEET_MAYBECHARSETC,
			htmlFilter(HTML_STYLESHEET_MAYBECHARSET, true)
		);
		assertEquals(
			HTML_STYLESHEET_CHARSETC,
			htmlFilter(HTML_STYLESHEET_CHARSET, true)
		);
		assertEquals(
			HTML_STYLESHEET_CHARSET_BADC,
			htmlFilter(HTML_STYLESHEET_CHARSET_BAD, true)
		);
		assertEquals(
			HTML_STYLESHEET_CHARSET_BAD1C,
			htmlFilter(HTML_STYLESHEET_CHARSET_BAD1, true)
		);
		assertEquals(
			HTML_STYLESHEET_WITH_MEDIAC,
			htmlFilter(HTML_STYLESHEET_WITH_MEDIA, true)
		);

		assertEquals(FRAME_SRC_CHARSETC, htmlFilter(FRAME_SRC_CHARSET, true));
		assertEquals(
			FRAME_SRC_CHARSET_BADC,
			htmlFilter(FRAME_SRC_CHARSET_BAD, true)
		);
		assertEquals(
			FRAME_SRC_CHARSET_BAD1C,
			htmlFilter(FRAME_SRC_CHARSET_BAD1, true)
		);
	...
	private static final String META_TIME_ONLY =
	...
	private static final String META_TIME_ONLY_BADNUM2 =
	...
	public void testMetaRefresh() throws Exception {
		...
		assertEquals(
			META_BOGUS_REDIRECT3_OUT,
			headFilter(META_BOGUS_REDIRECT3)
		);
		assertEquals(
			META_BOGUS_REDIRECT4_OUT,
	...
	public void testEvilCharset() throws IOException {
		...
		try (FileOutputStream fos = new FileOutputStream("output.filtered")) {
			...
			failures.add(
				new RuntimeException(
					"Filter accepted dangerous UTF8 text with BOM as UTF16! (ContentFilter) - Detected charset: " +
					fo.charset
				)
			);
		} catch (DataFilterException e) {
