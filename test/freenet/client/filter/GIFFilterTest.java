...
public class GIFFilterTest {
	...
	public void testReject() throws IOException {
		for (String reject : REJECT) {
			try (
				InputStream inStream = resourceToBucket(
					reject
				).getInputStream();
	 ...
	 */
	private static void assertEqualAfterFilter(
		String fileUnfiltered,
		String fileExpected
	) throws IOException {
	...
	private static boolean equalBuckets(Bucket a, Bucket b) throws IOException {
		return Arrays.equals(
			BucketTools.toByteArray(a),
			BucketTools.toByteArray(b)
		);
	}
