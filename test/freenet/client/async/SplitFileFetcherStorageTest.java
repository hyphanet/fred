...
public class SplitFileFetcherStorageTest {
	...
	static LockableRandomAccessBufferFactory rafFactory =
		new ByteArrayRandomAccessBufferFactory();
	static final WaitableExecutor exec = new WaitableExecutor(
		new PooledExecutor()
	);
	...
	private static final List<COMPRESSOR_TYPE> NO_DECOMPRESSORS =
	...
	private static class TestSplitfile {
		...
		static TestSplitfile constructSingleSegment(
			long size,
			int checkBlocks,
			boolean persistent
		)
			...
			ClientCHK[] dataKeys = makeKeys(
				originalDataBlocks,
				cryptoKey,
				cryptoAlgorithm
			);
			ClientCHK[] checkKeys = makeKeys(
				originalCheckBlocks,
				cryptoKey,
				cryptoAlgorithm
			);
		...
		static TestSplitfile constructMultipleSegments(
			...
			for (int seg = 0; seg < segments; seg++) {
				byte[][] segmentDataBlocks = Arrays.copyOfRange(
					originalDataBlocks,
					startDataBlock,
					startDataBlock + segmentDataBlockCount[seg]
				);
				byte[][] segmentCheckBlocks = Arrays.copyOfRange(
					originalCheckBlocks,
					startCheckBlock,
			...
			LockableRandomAccessBufferFactory f =
				new LockableRandomAccessBufferFactory() {
					...
					public LockableRandomAccessBuffer makeRAF(
						...
						LockableRandomAccessBuffer t = rafFactory.makeRAF(
							initialContents,
							offset,
							size,
							readOnly
		...
		public void verifyOutput(SplitFileFetcherStorage storage)
			throws IOException {
	...
	public static ClientCHK[] makeKeys(
		byte[][] blocks,
		byte[] cryptoKey,
		byte cryptoAlgorithm
	) throws CHKEncodeException {
		...
		for (int i = 0; i < blocks.length; i++) {
			keys[i] = ClientCHKBlock.encodeSplitfileBlock(
				blocks[i],
				cryptoKey,
				cryptoAlgorithm
			).getClientKey();
	...
	private static class StorageCallback
		...
		public StorageCallback(TestSplitfile splitfile) {
			...
			encodedBlocks = new boolean[splitfile.dataBlocks.length +
			splitfile.checkBlocks.length];
		}
		...
		@Override
		public void setSplitfileBlocks(
			int requiredBlocks,
			int remainingBlocks
		) {
	...
	public static byte[][] splitAndPadBlocks(Bucket data, long size)
		throws IOException {
	...
	public void testSingleSegment()
		throws CHKEncodeException, IOException, FetchException, MetadataParseException, MetadataUnresolvedException {
		// 2 data blocks.
	...
	private void testSingleSegment(int dataBlocks, int checkBlocks, long size)
		...
		TestSplitfile test = TestSplitfile.constructSingleSegment(
			size,
			checkBlocks,
			false
		);
	...
	private void testDataBlocksOnly(TestSplitfile test)
		...
		for (int i = 0; i < test.dataBlocks.length; i++) {
			...
			assertTrue(
				segment.onGotKey(
					test.dataKeys[i].getNodeCHK(),
					test.encodeDataBlock(i)
				)
			);
	...
	private void testCheckBlocksOnly(TestSplitfile test)
		...
		for (
			int i = 0;
			i < test.dataBlocks.length/* only need that many to decode */;
			i++
		) {
			assertFalse(segment.hasStartedDecode());
			assertTrue(
				segment.onGotKey(
					test.checkKeys[i].getNodeCHK(),
					test.encodeCheckBlock(i)
				)
			);
	...
	private void testRandomMixture(TestSplitfile test)
		...
		for (int i = 0; i < total; i++) segment.onNonFatalFailure(i); // We want healing on all blocks that aren't found.
		...
		for (int i = 0; i < test.dataBlocks.length; i++) {
			...
			assertFalse(segment.hasStartedDecode());
			assertTrue(
				segment.onGotKey(test.getCHK(block), test.encodeBlock(block))
			);
	...
	public void testMultiSegment()
		throws CHKEncodeException, IOException, MetadataUnresolvedException, MetadataParseException, FetchException {
	...
	private void testMultiSegment(
		...
		throws CHKEncodeException, IOException, MetadataUnresolvedException, MetadataParseException, FetchException {
		TestSplitfile test = TestSplitfile.constructMultipleSegments(
			size,
			segmentDataBlockCount,
			segmentCheckBlockCount,
			segmentSize,
	...
	private void testRandomMixtureMultiSegment(TestSplitfile test)
		...
		for (int i = 0; i < test.dataBlocks.length; i++) {
			...
			SplitFileFetcherSegmentStorage segment =
				storage.segments[test.segmentFor(block)];
	...
	private void innerChooseKeyTest(
		...
		assertTrue(
			storage.getOverallCooldownTime() != Long.MAX_VALUE ||
			storage.hasFailed()
		);
	...
	public void testChooseKeyCooldown()
		throws CHKEncodeException, IOException, MetadataUnresolvedException, MetadataParseException, FetchException, InterruptedException {
		int dataBlocks = 3, checkBlocks = 3;
		...
		assertNotEquals(
			Long.MAX_VALUE,
			storage.segments[0].getOverallCooldownTime()
		);
		// Now in cooldown.
	...
	public void testWriteReadSegmentKeys()
		throws FetchException, MetadataParseException, IOException, CHKEncodeException, MetadataUnresolvedException, ChecksumFailedException {
		int dataBlocks = 3, checkBlocks = 3;
		TestSplitfile test = TestSplitfile.constructSingleSegment(
			dataBlocks * BLOCK_SIZE,
			checkBlocks,
	...
	public void testPersistenceReload()
		throws CHKEncodeException, IOException, MetadataUnresolvedException, MetadataParseException, FetchException, StorageFormatException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
		assertTrue(dataBlocks * (long) BLOCK_SIZE >= size);
		...
		StorageCallback cb = test.createStorageCallback();
		SplitFileFetcherStorage storage = createSplitFileFetcherStorageTwice(
			test,
			cb
		);
		storage.close();
	...
	public void testPersistenceReloadThenFetch()
		throws IOException, StorageFormatException, CHKEncodeException, MetadataUnresolvedException, MetadataParseException, FetchException {
		int dataBlocks = 2;
		int checkBlocks = 3;
		long size = 32768 * 2 - 1;
		assertTrue(dataBlocks * (long) BLOCK_SIZE >= size);
	...
	public void testPersistenceReloadThenChooseKey()
		...
		SplitFileFetcherStorage storage = test.createStorage(
			cb,
			ctx,
			cb.getRAF()
		);
	...
	public void testPersistenceReloadBetweenFetches()
		...
		for (int i = 0; i < test.dataBlocks.length; i++) {
			...
			if (i != test.dataBlocks.length - 1) {
				// Reload.
				exec.waitForIdle();
				storage = test.createStorage(
					cb,
					test.makeFetchContext(),
					cb.getRAF()
				);
