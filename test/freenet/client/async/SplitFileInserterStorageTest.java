...
public class SplitFileInserterStorageTest {
	...
	public SplitFileInserterStorageTest() throws IOException {
		...
		bigBucketFactory = new TempBucketFactory(
			executor,
			fg,
			0,
			0,
		...
		baseContext = HighLevelSimpleClientImpl.makeDefaultInsertContext(
			bigBucketFactory,
	...
	private static class MyCallback
		implements SplitFileInserterStorageCallback {
	...
	public void testSmallSplitfileNoLastBlock() throws Exception {
		SplitFileInserterStorage storage = createSplitFileInserterStorage(
			data,
			size,
			cb,
			false,
	...
	public void testSmallSplitfileWithLastBlock() throws Exception {
		...
		byte[] originalLastBlock = Arrays.copyOfRange(
			originalData,
			(int) offsetLastBlock,
			originalData.length
		);
	...
	public void testSmallSplitfileConsecutiveRNFsHackFailure()
		...
		// Should count as success at this point.
		InsertException e = assertThrows(
		...
		assertEquals(
			2,
			e.errorCodes.getErrorCount(InsertExceptionMode.ROUTE_NOT_FOUND)
		);
		assertEquals(
			1,
			e.errorCodes.getErrorCount(InsertExceptionMode.REJECTED_OVERLOAD)
		);
	...
	public void testSmallSplitfileFailureFatalError() throws Exception {
		...
		assertEquals(
			1,
			e.errorCodes.getErrorCount(InsertExceptionMode.INTERNAL_ERROR)
		);
		assertEquals(1, e.errorCodes.totalCount());
		assertEquals(Status.FAILED, storage.getStatus());
	}

	private HashResult[] getHashes(LockableRandomAccessBuffer data)
		throws IOException {
		try (
			InputStream is = new RAFInputStream(data, 0, data.size());
			MultiHashInputStream hashStream = new MultiHashInputStream(
				is,
	...
	private LockableRandomAccessBuffer generateData(Random random, long size)
		...
		LockableRandomAccessBufferFactory f = size > 262144
			? bigRAFFactory
			: smallRAFFactory;
	...
	public void testRoundTripSimple() throws Exception {
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 2,
			CompatibilityMode.COMPAT_CURRENT
		);
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 2 - 1,
			CompatibilityMode.COMPAT_CURRENT
		);
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 128,
			CompatibilityMode.COMPAT_CURRENT
		);
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 128 + 1,
			CompatibilityMode.COMPAT_CURRENT
		);
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 192,
			CompatibilityMode.COMPAT_CURRENT
		);
		testRoundTripSimpleRandom(
			CHKBlock.DATA_LENGTH * 192 + 1,
			CompatibilityMode.COMPAT_CURRENT
		);
	}
	...
	public void testRepeatedEncodeAfterShutdown() throws Exception {
		testRepeatedEncodeAfterShutdownCrossSegment(
			CHKBlock.DATA_LENGTH * 128 * 5
		); // Not cross-segment.
		if (!TestProperty.EXTENSIVE) return;
		testRepeatedEncodeAfterShutdownCrossSegment(
			CHKBlock.DATA_LENGTH * 128 * 21
		); // Cross-segment.
	}
	...
	private void testRoundTripSimpleRandom(long size, CompatibilityMode cmode)
		throws Exception {
		...
		FetchCallbackForTestingSplitFileInserter fcb =
			new FetchCallbackForTestingSplitFileInserter();

		FetchContext fctx = HighLevelSimpleClientImpl.makeDefaultFetchContext(
			size * 2,
			size * 2,
			smallBucketFactory,
			new SimpleEventProducer()
		...
		SplitFileFetcherStorage fetcherStorage = createFetcherStorage(
			m1,
			fcb,
			new ArrayList<>(),
			cmode.code,
		...
		for (int i = 0; i < storage.segments.length; i++) {
			SplitFileFetcherSegmentStorage fetcherSegment =
				fetcherStorage.segments[i];
			SplitFileInserterSegmentStorage inserterSegment =
				storage.segments[i];
			int minBlocks =
				inserterSegment.dataBlockCount +
				inserterSegment.crossCheckBlockCount;
			int totalBlocks = inserterSegment.totalBlockCount;
			boolean[] fetched = new boolean[totalBlocks];
			if (
				i == storage.segments.length - 1 &&
			...
			for (int j = 0; j < minBlocks; j++) {
				...
				boolean success = fetcherSegment.onGotKey(
					block.getClientKey().getNodeCHK(),
					block.getBlock()
				);
				assertTrue(success);
				fcb.checkFailed();
	...
	private void testResumeCrossSegment(long size) throws Exception {
		...
		for (int i = 0; i < storage.segments.length; i++) {
			...
			assertEquals(
				storage.segments[i].totalBlockCount,
				resumed.segments[i].totalBlockCount
			);
		}
	...
	private void testRepeatedEncodeAfterShutdownCrossSegment(long size)
		...
		if (storage.crossSegments != null) {
			for (int i = 0; i < storage.crossSegments.length; i++) {
				...
				resumed = createSplitFileInserterStorage(
					storage,
					data,
					cb,
					memoryLimitedJobRunner,
		...
		for (int i = 0; i < storage.segments.length; i++) {
			...
			if (storage.crossSegments != null) {
				assertEquals(
					resumed.crossSegments.length,
					countEncodedCrossSegments(resumed)
				);
				assertEquals(
					Status.ENCODED_CROSS_SEGMENTS,
					resumed.getStatus()
				);
			}
			resumed.start();
	...
	private void testRoundTripCrossSegmentRandom(long size) throws Exception {
		...
		int dataBlocks = (int) ((size + CHKBlock.DATA_LENGTH - 1) /
		...
		assertTrue(
			"Downloaded more blocks than data+cross check",
			i < expectedBlocks
		);
	...
	private void testRoundTripCrossSegmentDataBlocks(long size)
		throws Exception {
		RandomSource r = new DummyRandomSource(12123);
		LockableRandomAccessBuffer data = generateData(r, size);
		Bucket dataBucket = new RAFBucket(data);
		...
		for (int segNo = 0; segNo < storage.segments.length; segNo++) {
			SplitFileInserterSegmentStorage inserterSegment =
				storage.segments[segNo];
			SplitFileFetcherSegmentStorage fetcherSegment =
				fetcherStorage.segments[segNo];
			for (
				int blockNo = 0;
				blockNo < inserterSegment.dataBlockCount;
				blockNo++
			) {
				ClientCHKBlock block = inserterSegment.encodeBlock(blockNo);
				boolean success = fetcherSegment.onGotKey(
			...
			while (true) {
				...
				while (true) {
					...
					ClientCHKBlock block =
						storage.segments[segNo].encodeBlock(blockNo);
					boolean success = segment.onGotKey(
						block.getClientKey().getNodeCHK(),
						block.getBlock()
					);
					assertTrue(success);
	...
	private class FetchCallbackForTestingSplitFileInserter
		...
		@Override
		public synchronized void setSplitfileBlocks(
			int requiredBlocks,
			int remainingBlocks
		) {
		...
		@Override
		public synchronized void queueHeal(
			byte[] data,
			byte[] cryptoKey,
			byte cryptoAlgorithm
		) {}
	...
	public void testCancel() throws Exception {
		this.size = 32768 * 6;
		BarrierRandomAccessBuffer data = new BarrierRandomAccessBuffer(
			generateData(random, size)
		);
		HashResult[] hashes = getHashes(data);
		data.pause();
		context.earlyEncode = true;
	...
	@Test
	public void testPersistentSmallSplitfileNoLastBlockCompletion()
		throws Exception {
		LockableRandomAccessBuffer data = generateData(
			random,
			size,
			bigRAFFactory
		);
		...
		SplitFileInserterSegmentStorage segment =
			assertProperSegmentStateAndGet(resumed);
		for (int i = 0; i < segment.totalBlockCount; i++) {
			segment.onInsertedBlock(i, segment.encodeBlock(i).getClientKey());
		}
		cb.waitForSucceededInsert();
	...
	@Test
	public void testPersistentSmallSplitfileNoLastBlockCompletionAfterResume()
		throws Exception {
	...
	@Test
	public void testPersistentSmallSplitfileWithLastBlockCompletionAfterResume()
		throws Exception {
		data = generateData(random, size, bigRAFFactory);
		hashes = getHashes(data);
		SplitFileInserterStorage storage = createSplitFileInserterStorage(
	...
	@Test
	public void testPersistentSmallSplitfileNoLastBlockFailAfterResume()
		throws Exception {
		data = generateData(random, size, bigRAFFactory);
		hashes = getHashes(data);
		context.consecutiveRNFsCountAsSuccess = 0;
	...
	@Test
	public void testPersistentSmallSplitfileNoLastBlockChooseAfterResume()
		throws Exception {
		LockableRandomAccessBuffer data = generateData(
			random,
			size,
