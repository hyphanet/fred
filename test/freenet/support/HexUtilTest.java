...
public class HexUtilTest {
	...
	public void testHexToBytes_String() {
		...
		for (int i = 255; i >= 0; i--) {
			...
			assertTrue(
				Arrays.equals(
					expectedByteArray,
					HexUtil.hexToBytes(methodHexString)
				)
			);
		}
	}
	...
	public void testHexToBytes_StringInt() {
		...
		for (int i = 255; i >= 0; i--) {
			...
			assertTrue(
				Arrays.equals(
					expectedByteArray,
					HexUtil.hexToBytes(methodHexString, 0)
				)
			);
		}
	}
	...
	public void testCountBytesForBits_int() {
		...
		for (
			int expectedBytesCount = 1;
			expectedBytesCount < 256;
			expectedBytesCount++
		) for (
			int bits = (expectedBytesCount - 1) * 8 + 1;
			bits <= (expectedBytesCount) * 8;
			bits++
	...
	public void testBytesToBits_byteBitSetInt() {
		...
		for (int i = 0; i < 255; i++) {
			...
			assertTrue(
				Arrays.equals(
					methodByteArray,
					HexUtil.bitsToBytes(methodBitSet, 8)
				)
			);
		}
	}
	...
	public void testWriteAndReadBigInteger() {
		BigInteger methodBigInteger = new BigInteger("999999999999999");
		ByteArrayOutputStream methodByteArrayOutStream =
			new ByteArrayOutputStream();
		DataOutputStream methodDataOutStream = new DataOutputStream(
			methodByteArrayOutStream
		);
		try {
			...
			DataInputStream methodDataInStream = new DataInputStream(
				methodByteArrayInStream
			);
			assertTrue(
				methodBigInteger.compareTo(
