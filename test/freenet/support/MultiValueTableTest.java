...
public class MultiValueTableTest {
	 ...
	 */
	private Object[][] createSampleKeyMultiVal(
		int keysNumber,
		int valuesNumber,
		boolean isRandom
	) {
	...
	private MultiValueTable<Object, Object> createSampleMultiValueTable(
		...
		Object[][] sampleObjects = createSampleKeyMultiVal(
			keyNumber,
			maxValueNumber,
			isRandom
		);
	...
	private MultiValueTable<Object, Object> fillMultiValueTable(
		...
		for (int i = 0; i < sampleKeyNumber; i++) {
			...
			while (itr.hasNext()) methodMVTable.put(
				sampleObjects[i][0],
				itr.next()
			);
		}
	...
	public void testPut() {
		assertNotNull(
			createSampleMultiValueTable(
				sampleKeyNumber,
				sampleMaxValueNumber,
				sampleIsRandom
			)
		);
	}
	...
	public void testGet() {
		...
		for (int i = 0; i < sampleObjects.length; i++) assertEquals(
			methodMVTable.get(sampleObjects[i][0]),
			((List<Object>) sampleObjects[i][1]).get(0)
		);
	}
	...
	public void testContainsKey() {
		...
		for (int i = 0; i < sampleObjects.length; i++) assertTrue(
			methodMVTable.containsKey(sampleObjects[i][0])
		);
		assertFalse(methodMVTable.containsKey(new Object()));
	}
	...
	public void testContainsElement() {
		...
		for (int i = 0; i < sampleObjects.length; i++) {
			...
			assertFalse(
				methodMVTable.containsElement(sampleObjects[i][0], new Object())
			);
			while (iter.hasNext()) assertTrue(
				methodMVTable.containsElement(sampleObjects[i][0], iter.next())
			);
		}
	}
	...
	public void testGetAll() {
		...
		for (int i = 0; i < sampleObjects.length; i++) {
			...
			while (iter.hasNext()) assertEquals(
	...
	public void testGetSync() {
		...
		methodMVTable = fillMultiValueTable(sampleObjects);
		for (int i = 0; i < sampleObjects.length; i++) assertEquals(
			methodMVTable.getSync(sampleObjects[i][0]),
			((List<Object>) sampleObjects[i][1])
		);
	}
	...
	public void testRemove() {
		...
		methodMVTable = fillMultiValueTable(sampleObjects);
		for (int i = 0; i < sampleObjects.length; i++) methodMVTable.remove(
			sampleObjects[i][0]
		);
	...
	public void testClear() {
		...
		MultiValueTable<Object, Object> methodMVTable = fillMultiValueTable(
			sampleObjects
		);
		methodMVTable.clear();
		for (int i = 0; i < sampleObjects.length; i++) assertFalse(
			methodMVTable.containsKey(sampleObjects[i][0])
		);
		assertTrue(methodMVTable.isEmpty());
	}
	...
	public void testRemoveElement() {
		...
		for (int i = 0; i < sampleObjects.length; i++) {
			iter = ((List<Object>) (sampleObjects[i][1])).iterator();
			assertFalse(
				methodMVTable.removeElement(sampleObjects[i][0], new Object())
			);
			while (iter.hasNext()) {
				methodValue = iter.next();
				assertTrue(
					methodMVTable.removeElement(
						sampleObjects[i][0],
						methodValue
					)
				);
	...
	public void testDifferentKeysSameElement() {
		...
		for (int i = 0; i < keysNumber; i++) methodMVTable.put(
			new Object(),
			sampleValue
		);
