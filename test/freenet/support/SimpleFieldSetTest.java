...
public class SimpleFieldSetTest {
  ...
  public void testPut_StringBoolean() {
    SimpleFieldSet methodSFS = new SimpleFieldSet(true);
    int length = 15;
    for (int i = 0; i < length; i++) methodSFS.put(Integer.toString(i), true);
  ...
  public void testDirectSubsetNameIterator() {
    ...
    while (methodIter.hasNext()) assertEquals(methodIter.next(), expectedSubset);
    methodSFS = new SimpleFieldSet(true);
  ...
  public void testPutAppend() {
    ...
    SimpleFieldSet methodSFS = new SimpleFieldSet(true);
    for (int i = 0; i < methodValues.length; i++) methodSFS.putAppend(methodKey, methodValues[i]);
    assertEquals(methodSFS.get(methodKey), expectedResult);
  }
  ...
  public void testGetIntArray() {
    ...
    for (int i = 0; i < 15; i++) methodSFS.putAppend(keyPrefix, String.valueOf(i));
  ...
  public void testGetDoubleArray() {
    SimpleFieldSet methodSFS = new SimpleFieldSet(true);
    String keyPrefix = "foo";
    for (int i = 0; i < 15; i++) methodSFS.putAppend(keyPrefix, String.valueOf((double) i));
  ...
  public void testRemoveSubset() {
    ...
    for (int i = 1; i < 4; i++) assertNull(methodSFS.get(SAMPLE_STRING_PAIRS[i][0]));
  ...
  private boolean isAKey(String[][] aStringPairsArray, String aPrefix, String aKey) {
    for (int i = 0; i < aStringPairsArray.length; i++)
      if (aKey.equals(aPrefix + aStringPairsArray[i][0])) return true;
    return false;
  }
