...
public class SimpleFieldSetTest {
	...
	public void testSimpleFieldSetPutSingle_StringString_WithTwoPairedMultiLevelChars() {
		...
		assertEquals(
			methodSFS.subset("foo").subset("").subset("bar").get(""),
			methodValue
		);
	...
	private boolean checkPutAndGetPairs(String[][] aPairsArray) {
		...
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		//putting values
		for (int i = 0; i < aPairsArray.length; i++) methodSFS.putSingle(
			aPairsArray[i][0],
			aPairsArray[i][1]
		);
		for (
	...
	public void testSimpleFieldSetSubset_String() {
		...
		for (
			int i = 0;
			i < methodPairsArray_MultiLevel.length;
			i++
		) methodSFS.putSingle(
		...
		assertTrue(
			checkSimpleFieldSetSize(
				methodSFS,
				methodPairsArray_MultiLevel.length
			)
		);
	}
	...
	public void testPutAllOverwrite() {
		...
		for (int i = 0; i < SAMPLE_STRING_PAIRS.length; i++) assertEquals(
		...
		for (int i = 0; i < SAMPLE_STRING_PAIRS.length; i++) assertEquals(
			nullSFS.get(SAMPLE_STRING_PAIRS[i][0]),
			SAMPLE_STRING_PAIRS[i][1] + methodAppendedString
		);
	}
	...
	public void testPut_StringBoolean() {
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		int length = 15;
		for (int i = 0; i < length; i++) methodSFS.put(
			Integer.toString(i),
			true
		);
		for (int i = 0; i < length; i++) assertEquals(
			methodSFS.getBoolean(Integer.toString(i), false),
			true
		);
	 ...
	 */
	private boolean checkSimpleFieldSetSize(
		SimpleFieldSet aSimpleFieldSet,
		int expectedSize
	) {
		int actualSize = 0;
	...
	public void testPut_StringInt() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) methodSFS.put(
			Integer.toString(methodPairsArray[i][0]),
		...
		for (int i = 0; i < methodPairsArray.length; i++) {
			try {
				assertEquals(
					methodSFS.getInt(Integer.toString(methodPairsArray[i][0])),
					methodPairsArray[i][1]
				);
				assertEquals(
					methodSFS.getInt(
				...
				fail(
					"Not expected exception thrown : " + aException.getMessage()
				);
			}
		}
	}
	...
	public void testPut_StringLong() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) methodSFS.put(
			Long.toString(methodPairsArray[i][0]),
			methodPairsArray[i][1]
		);
		...
		for (int i = 0; i < methodPairsArray.length; i++) {
			try {
				assertEquals(
					methodSFS.getLong(Long.toString(methodPairsArray[i][0])),
					methodPairsArray[i][1]
				);
				assertEquals(
					methodSFS.getLong(Long.toString(methodPairsArray[i][0]), 5),
					methodPairsArray[i][1]
				);
			} catch (FSParseException aException) {
				fail(
	...
	public void testPut_StringChar() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) methodSFS.put(
			String.valueOf(methodPairsArray[i][0]),
			methodPairsArray[i][1]
		);
		...
		for (int i = 0; i < methodPairsArray.length; i++) {
			try {
				assertEquals(
					methodSFS.getChar(String.valueOf(methodPairsArray[i][0])),
					methodPairsArray[i][1]
				);
				assertEquals(
					methodSFS.getChar(
	...
	public void testPut_StringShort() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) methodSFS.put(
			Short.toString(methodPairsArray[i][0]),
			methodPairsArray[i][1]
		);
		...
		for (int i = 0; i < methodPairsArray.length; i++) {
			try {
				assertEquals(
					methodSFS.getShort(Short.toString(methodPairsArray[i][0])),
					methodPairsArray[i][1]
				);
				assertEquals(
					methodSFS.getShort(
	...
	public void testPut_StringDouble() {
		...
		for (int i = 0; i < methodPairsArray.length; i++) methodSFS.put(
			Double.toString(methodPairsArray[i][0]),
			methodPairsArray[i][1]
		);
	...
	private String sfsReadyString(String[][] aStringPairsArray) {
		...
		for (int i = 0; i < aStringPairsArray.length; i++) methodStringToReturn
	...
	public void testSimpleFieldSet_StringBooleanBoolean() {
		...
		try {
			SimpleFieldSet methodSFS = new SimpleFieldSet(
				methodStringToParse,
				false,
				false,
				false
			);
			for (int i = 0; i < methodStringPairs.length; i++) assertEquals(
				methodSFS.get(methodStringPairs[i][0]),
	...
	public void testSimpleFieldSet_BufferedReaderBooleanBoolean() {
		...
		BufferedReader methodBufferedReader = new BufferedReader(
			new StringReader(sfsReadyString(methodStringPairs))
		);
		try {
			SimpleFieldSet methodSFS = new SimpleFieldSet(
				methodBufferedReader,
				false,
				false
			);
			for (int i = 0; i < methodStringPairs.length; i++) assertEquals(
	...
	private SimpleFieldSet sfsFromSampleStringPairs() {
		...
		assertTrue(
			checkSimpleFieldSetSize(methodSFS, SAMPLE_STRING_PAIRS.length)
		);
		return methodSFS;
	}
	...
	public void testDirectSubsetNameIterator() {
		...
		while (methodIter.hasNext()) assertEquals(
			methodIter.next(),
			expectedSubset
		);
		methodSFS = new SimpleFieldSet(true);
	...
	public void testNamesOfDirectSubsets() {
		...
		SimpleFieldSet methodSFS = sfsFromSampleStringPairs();
		assertTrue(
			Arrays.equals(methodSFS.namesOfDirectSubsets(), expectedResult)
		);

		methodSFS = new SimpleFieldSet(true);
		assertTrue(
			Arrays.equals(methodSFS.namesOfDirectSubsets(), new String[0])
		);
	}
	...
	public void testPutOverwrite_String() {
		...
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		for (int i = 0; i < methodValues.length; i++) methodSFS.putOverwrite(
			methodKey,
			methodValues[i]
		);
	...
	public void testPutAppend() {
		...
		String expectedResult =
			...
			"zoo";
		SimpleFieldSet methodSFS = new SimpleFieldSet(true);
		for (int i = 0; i < methodValues.length; i++) methodSFS.putAppend(
			methodKey,
			methodValues[i]
		);
		assertEquals(methodSFS.get(methodKey), expectedResult);
	...
	public void testGetIntArray() {
		...
		for (int i = 0; i < 15; i++) methodSFS.putAppend(
			keyPrefix,
			String.valueOf(i)
		);
		int[] result = methodSFS.getIntArray(keyPrefix);
		for (int i = 0; i < 15; i++) assertTrue(result[i] == i);
	}
	...
	public void testRemoveValue() {
		...
		for (int i = 1; i < SAMPLE_STRING_PAIRS.length; i++) assertEquals(
			methodSFS.get(SAMPLE_STRING_PAIRS[i][0]),
			SAMPLE_STRING_PAIRS[i][1]
		);
	}
	...
	public void testRemoveSubset() {
		...
		for (int i = 1; i < 4; i++) assertNull(
			methodSFS.get(SAMPLE_STRING_PAIRS[i][0])
		);
		assertEquals(
			methodSFS.get(SAMPLE_STRING_PAIRS[0][0]),
			SAMPLE_STRING_PAIRS[0][1]
		);
		for (int i = 4; i < 6; i++) assertEquals(
			methodSFS.get(SAMPLE_STRING_PAIRS[i][0]),
			SAMPLE_STRING_PAIRS[i][1]
		);
	}
	...
	private boolean isAKey(
		String[][] aStringPairsArray,
		String aPrefix,
		String aKey
	) {
		for (int i = 0; i < aStringPairsArray.length; i++) if (
			aKey.equals(aPrefix + aStringPairsArray[i][0])
		) return true;
		return false;
	}
	...
	private void checkBase64(String key, String value, String base64Value)
		throws IOException {
		SimpleFieldSet sfs = new SimpleFieldSet(true);
		...
		LineReader r = Readers.fromBufferedReader(
			new BufferedReader(new StringReader(written))
		);
		SimpleFieldSet sfsCheck = new SimpleFieldSet(
			r,
			1024,
			1024,
			true,
	...
	public void testSplit() {
		assertTrue(
			Arrays.equals(SimpleFieldSet.split("blah"), new String[] { "blah" })
		);
		assertTrue(
			Arrays.equals(
				SimpleFieldSet.split("blah; blah"),
