...
public class JarClassLoaderTest {
  ...
  public void serviceLoaderCanLocateTestImplementation() throws Exception {
    ...
    assertThat(
        implementations,
        containsInAnyOrder(
            instanceOf(TestImplementation1.class), instanceOf(TestImplementation2.class)));
  }

  /**
   * Create a temporary JAR file that contains two files that will be used by {@link ServiceLoader}
   * in order to provide two different implementations of {@link TestInterface}.
   *
  ...
  /**
   * Clears the names of already written entries of the given {@link ZipOutputStream}. This is
   * necessary to prevent exceptions because of duplicate file entries because we absolutely
   * <em>do</em> want multiple files with the same name here.
   *
