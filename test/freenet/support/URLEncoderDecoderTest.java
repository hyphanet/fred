...
public class URLEncoderDecoderTest {
	...
	public static final String stressedUTF_8Chars = new String(
		UTFUtil.STRESSED_UTF
	);
	public static final String allCharsExceptNull = new String(
		UTFUtil.ALL_CHARACTERS
	).replace("\u0000", "");
	 ...
	 */
	@Test
	public void testEncodeDecodeString_allChars()
		throws URLEncodedFormatException {
		assertTrue(
			areCorrectlyEncodedDecoded(
				new String[] { allCharsExceptNull },
				true
			)
		);
		assertTrue(
	 ...
	 * @throws URLEncodedFormatException
	 */
	private boolean areCorrectlyEncodedDecoded(
		String[] toEncode,
		boolean withLetters
	) throws URLEncodedFormatException {
		String[] encoded = new String[toEncode.length];
		//encoding
		for (int i = 0; i < encoded.length; i++) encoded[i] = URLEncoder.encode(
			toEncode[i],
			withLetters
		);
		//decoding
		for (int i = 0; i < encoded.length; i++) {
			...
			if (orig.equals(decoded) == false) {
				for (int c = 0; c < orig.length(); ++c) {
					...
					if (
						c > decoded.length() || origChar != decodedChar
	...
	public void testEncodeForced() {
		for (int i = 0; i < URLEncoder.safeURLCharacters.length(); i++) {
			...
			assertEquals(
				expectedResult,
				URLEncoder.encode(toEncode, toEncode, false)
			);
			assertEquals(
				expectedResult,
				URLEncoder.encode(toEncode, toEncode, true)
			);
		}
	}
	 ...
	 * @return
	 */
	private boolean isDecodeRaisingEncodedException(
		String toDecode,
		boolean tolerant
	) {
	...
	public void testDecodeWrongHex() {
		...
		for (int i = 0; i < toDecode.length(); i++) assertTrue(
			isDecodeRaisingEncodedException(
				"%" + toDecode.substring(i, i + 1),
				false
			)
		);
	}
