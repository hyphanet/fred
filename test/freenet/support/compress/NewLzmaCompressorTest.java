...
public class NewLzmaCompressorTest {
	...
	public void testNewLzmaCompressor() throws IOException {
		Compressor.COMPRESSOR_TYPE lzcompressor =
			Compressor.COMPRESSOR_TYPE.LZMA_NEW;
		Compressor compressorZero =
			Compressor.COMPRESSOR_TYPE.getCompressorByMetadataID((short) 3);
	...
	public void testByteArrayDecompress() throws IOException {
		...
		writtenBytes = Compressor.COMPRESSOR_TYPE.LZMA_NEW.decompress(
			compressedData,
			0,
		...
		assertEquals(
			originalUncompressedData.length,
	...
	private byte[] doCompress(byte[] uncompressedData) throws IOException {
		...
		outBucket = Compressor.COMPRESSOR_TYPE.LZMA_NEW.compress(
			inBucket,
			factory,
