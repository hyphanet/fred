...
public class MemoryLimitedJobRunnerTest {
	...
	private void innerTestQueueingSmallDelayed(
		...
		for (int i = 0; i < jobs.length; i++) jobs[i] = new SynchronousJob(
			JOB_SIZE,
			startLive,
			completion
		);
		...
		MemoryLimitedJobRunner runner = new MemoryLimitedJobRunner(
			JOB_LIMIT,
			maxThreads,
			executor,
	...
	@Test
	public void testAsyncQueueingManySmallDelayed()
		throws InterruptedException {
