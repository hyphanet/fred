...
public abstract class TempBucketFactoryRAFBase
	...
	private static final int[] TEST_LIST_NOT_MIGRATED = new int[] {
		1,
		32,
		64,
		1024,
	...
	public void testArrayMigration() throws IOException {
		...
		for (int size : TEST_LIST_NOT_MIGRATED) innerTestArrayMigration(
			size,
			r
		);
	}
	...
	protected void innerTestArrayMigration(int len, Random r)
		throws IOException {
		if (len == 0) return;
	...
	public void testBucketToRAFWhileArray() throws IOException {
		...
		TempRandomAccessBuffer raf =
			(TempRandomAccessBuffer) bucket.toRandomAccessBuffer();
	...
	private File getFile(TempBucket bucket) {
		if (!this.enableCrypto()) return (
			(TempFileBucket) (((TempBucket) bucket).getUnderlying())
		).getFile();
		else {
			EncryptedRandomAccessBucket erab =
				(EncryptedRandomAccessBucket) bucket.getUnderlying();
	...
	public void testBucketToRAFWhileFile() throws IOException {
		...
		File f = getFile(bucket);
		assertTrue(f.exists());
		if (enableCrypto()) assertEquals(f.length(), 8192);
		else assertEquals(f.length(), 4095);
		TempRandomAccessBuffer raf =
			(TempRandomAccessBuffer) bucket.toRandomAccessBuffer();
		assertTrue(f.exists());
		if (enableCrypto()) assertEquals(f.length(), 8192);
