...
public abstract class RandomAccessBufferTestBase {
	 ...
	 * @throws IOException */
	protected abstract RandomAccessBuffer construct(long size)
		throws IOException;
	...
	public void testSize() throws IOException {
		for (long size : fullSizeList) innerTestSize(size);
	}
	...
	protected void innerTestFormula(long sz, Random r, Formula f)
		throws IOException {
		RandomAccessBuffer raf = construct(sz);
		assertEquals(raf.size(), sz);
		int x = 0;
		...
		while (x < sz) {
			...
			for (int i = 0; i < buf.length; i++) assertEquals(
				buf[i],
				f.getByte(i + x)
			);
			x += toRead;
	...
	private void innerTestWriteOverLimit(long sz, int choppedBytes)
		throws IOException {
		RandomAccessBuffer raf = construct(sz);
		assertEquals(raf.size(), sz);
		...
		if (sz != 0 && choppedBytes < sz) {
			if (startAt >= 0) readWriteMustSucceed(
				raf,
				startAt,
				buf,
				0,
				buf.length
			); // Read, write up to the end work.
			else try {
		...
		if (startAt + 1 >= 0) readWriteMustFail(
			raf,
			startAt + 1,
			buf,
			0,
			buf.length
		); // Read, write over the end fail.
		else try {
	...
	public void testClose() throws IOException {
		...
		for (long size : fullSizeList) innerTestClose(size);
	}
	...
	public void testArray() throws IOException {
		...
		for (int size : sizeList) innerTestArray(size, r, false);
	}
	...
	protected void innerTestArray(int len, Random r, boolean readOnly)
		throws IOException {
		if (len == 0) return;
		...
		if (len > 1) checkArraySectionEqualsReadData(
			buf,
			raf,
			1,
			len - 1,
	...
	public static void checkArraySectionEqualsReadData(
		...
		for (int i = 0; i < len; i++) assertEquals(tmp[i], buf[start + i]);
		if (!readOnly) {
