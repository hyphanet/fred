...
public class TempBucketTest {
	...
	public static class TempBucketMigrationTest {
		...
		public void testRamLimitCreate() throws IOException {
			TempBucketFactory tbf = new TempBucketFactory(
				exec,
				fg,
				16,
				128,
		...
		public void testWriteExcessConversionFactor() throws IOException {
			...
			try {
				...
				for (
					int i = 0;
					i < TempBucketFactory.RAMBUCKET_CONVERSION_FACTOR - 1;
					i++
				) {
					os.write(new byte[16]);
				}
		...
		public void testBigConversionWhileReading() throws IOException {
			...
			for (int i = 0; i < readTo.length; i++) assertTrue(
				readTo[i] == data[i]
			);
			is.close();
			os.close();
	...
	private static class RealTempBucketTest_ extends BucketTestBase {
		...
		public RealTempBucketTest_(
			int maxRamSize,
			int maxTotalRamSize,
			boolean encrypted
		) throws IOException {
			fg = new FilenameGenerator(weakPRNG, false, null, "junit");
			tbf = new TempBucketFactory(
				exec,
				fg,
				maxRamSize,
				maxTotalRamSize,
	...
	public static class RealTempBucketTest_64_128_F
		extends RealTempBucketTest_ {
	...
	public static class RealTempBucketTest_64k_128k_F
		extends RealTempBucketTest_ {
	...
	public static class RealTempBucketTest_64k_128k_T
		extends RealTempBucketTest_ {
