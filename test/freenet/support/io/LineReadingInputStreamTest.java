...
public class LineReadingInputStreamTest {
	...
	public void testReadLineWithoutMarking() throws Exception {
		...
		InputStream is = new ByteArrayInputStream(
			STRESSED_LINE.getBytes(StandardCharsets.UTF_8)
		);
		...
		is = new ByteArrayInputStream(
			BLOCK.getBytes(StandardCharsets.ISO_8859_1)
		);
		...
		instance = new LineReadingInputStream(is);
		try {
			instance.readLineWithoutMarking(
				LENGTH_CHECKING_LINE_LF - 1,
				BUFFER_SIZE,
				true
			);
			fail();
	...
	public void testReadLine() throws Exception {
		...
		assertEquals(
			"\u0114",
			instance.readLine(MAX_LENGTH, BUFFER_SIZE, true)
		);
	...
	public void testBothImplementation() throws Exception {
		...
		while (bis1.available() > 0 || bis2.available() > 0) {
			String stringWithoutMark = lris2.readLineWithoutMarking(
				MAX_LENGTH * 10,
				BUFFER_SIZE,
				true
			);
			String stringWithMark = lris1.readLine(
				MAX_LENGTH * 10,
				BUFFER_SIZE,
				true
			);
