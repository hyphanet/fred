...
public class BitInputStreamTest {
	...
	public void readAlignedBytesTest() throws IOException {
		...
		try (
			BitInputStream in = new BitInputStream(new ByteArrayInputStream(ba))
		) {
	...
	public void unalignedBytesTest() throws IOException {
		String bitData =
			"0101 00001 00001 00010 00011 00101 01000 01101 10101" +
	...
	private static int[] readNmbrs(BitInputStream in, int n)
		throws IOException {
		int[] nmbrs = new int[n];
		int nBits = in.readInt(4);
		System.out.println(
			"nBits: " + nBits + " = 0b" + BigInteger.valueOf(nBits).toString(2)
		);
		for (int i = 0; i < n; i++) {
		...
		try (
			BitInputStream in = new BitInputStream(
				new ByteArrayInputStream(
					new BigInteger("1111111100000001", 2).toByteArray()
				),
				ByteOrder.LITTLE_ENDIAN
			)
		) {
