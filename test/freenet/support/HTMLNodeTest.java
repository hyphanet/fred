...
public class HTMLNodeTest {
	...
	public void testHTMLNode_AttributesArray() {
		...
		//checks presence
		for (int i = 0; i < size; i++) assertEquals(
			methodAttributesValue[i],
	...
	public void testSameAttributeManyTimes() {
		...
		for (int i = 0; i < times; i++) {
			exampleNodeNonEmpty.addAttribute(
				methodAttributeName,
	...
	private void testSingleChildAttributes(
		...
		assertEquals(
			attibutesNames.length,
			childHTMLNode.getAttributes().size()
		);
		for (int i = 0; i < attibutesNames.length; i++) assertEquals(
			attributesValues[i],
	...
	public void testGetAttribute() {
		...
		assertEquals(
			SAMPLE_ATTRIBUTE_VALUE,
			methodHTMLNode.getAttribute(SAMPLE_OKAY_ATTRIBUTE_NAME)
		);
		methodHTMLNode = new HTMLNode(
			"#",
			SAMPLE_OKAY_ATTRIBUTE_NAME,
	...
	public void testAddGetAttributes() {
		...
		for (int i = 0; i < attributesNumber; i++) {
			...
			assertEquals(
				counterString,
				methodHTMLNode.getAttribute(methodAttributeName)
			);
			assertEquals(
				counterString,
				methodHTMLNode.getAttributes().get(methodAttributeName)
			);
		}
	}
	...
	public void testHTMLNodeArray_nullAttributeName() {
		...
		testHTMLNodeArray_null(
			methodAttributesNameArray,
	 ...
	 */
	private void testHTMLNodeArray_null(
	...
	public void testGenerate_fromHTMLNodeWithChild_SpecialNames() {
		...
		String[] nodeNamesArray = {
			"div",
			"form", // input is an empty element!
			"script",
			"table",
			"tr",
			"td",
		};
		HTMLNode methodChildNode = new HTMLNode(
			SAMPLE_OKAY_NODE_NAME_NON_EMPTY,
			SAMPLE_OKAY_ATTRIBUTE_NAME,
			SAMPLE_ATTRIBUTE_VALUE,
			SAMPLE_NODE_CONTENT
	...
	public void testGenerate_fromHTMLNode_String() {
		HTMLNode methodHTMLNode = new HTMLNode(SAMPLE_OKAY_NODE_NAME_EMPTY);
		assertEquals(
			("<" + SAMPLE_OKAY_NODE_NAME_EMPTY + " />").toLowerCase(),
			methodHTMLNode.generate()
		);
	}
	...
	private String generateFullNodeOutput(
		...
		if (!aContent.equals("")) sb.append(indenting + aContent);
	...
	public void testGenerate_HTMLNode_withChild() {
		...
		HTMLNode methodHTMLNodeChild = new HTMLNode(
			SAMPLE_OKAY_NODE_NAME_NON_EMPTY,
			SAMPLE_OKAY_ATTRIBUTE_NAME,
			SAMPLE_ATTRIBUTE_VALUE,
			SAMPLE_NODE_CONTENT
	...
	public void testHTMLDoctype_generate() {
		...
		HTMLNode methodHTMLNodeDoc = new HTMLNode.HTMLDoctype(
			sampleDocType,
			sampleSystemUri
		);
