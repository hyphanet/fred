...
public class ProbeTest {
	...
	public void probeLocationIsRandomized() {
		...
		when(node.getRandom().nextDouble()).thenReturn(0.6);

		probe.start((byte) 1, 12345L, Type.LOCATION, listener);
		verify(listener).onLocation(0.4f + (0.6f - 0.5f) * 0.01f / 2.0f);
	}

	@Test
	public void overflowOnNodeLocationRandomizationIsHandledCorrectly() {
		when(node.getLocation()).thenReturn(0.999);
		when(node.getRandom().nextDouble()).thenReturn(1.0);

		probe.start((byte) 1, 12345L, Type.LOCATION, listener);
		verify(listener).onLocation((float) ((0.999 + (1.0 - 0.5) * 0.01 / 2.0) - 1.0));
	}
    ...
    // the fake nextDouble is also used for wait, so we cannot go below Probe.getRandomizedWaitStepTime()
		...
		probe.start((byte) 1, 12345L, Type.LOCATION, listener);
		verify(listener).onLocation((float) ((0.001 + (0.1 - 0.5) * 0.01 / 2.0) + 1.0));
	}
