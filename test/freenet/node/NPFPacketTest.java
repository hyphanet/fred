...
public class NPFPacketTest {
	...
	public void testEncodeDecodeLossyPerPacketMessages() {
		...
		p.addMessageFragment(
			new MessageFragment(true, false, true, 0, 8, 8, 0, fragData, null)
		);
		byte[] lossyFragment = new byte[] {
			(byte) 0xFF,
			(byte) 0xEE,
			(byte) 0xDD,
			(byte) 0xCC,
	...
	public void testEncodeDecodeLossyPerPacketMessages2Padded() {
		...
		assertEquals(
			"Seed was " + randomSeed,
			2,
			received.getLossyMessages().size()
		);
