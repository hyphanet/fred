...
public class NewPacketFormatTest {
	...
	public void testLostLastAck()
		...
		SessionKey senderKey = new SessionKey(
			null,
			null,
			null,
			null,
		...
		SessionKey receiverKey = new SessionKey(
			null,
			null,
			null,
			null,
		...
		NPFPacket fragment1 = sender.createPacket(
			512,
			senderQueue,
			senderKey,
			false
		...
		NPFPacket fragment2 = sender.createPacket(
			512,
			senderQueue,
			senderKey,
			false
		...
		Thread.sleep(NewPacketFormatKeyContext.MAX_ACK_DELAY * 2);
		NPFPacket ack1 = receiver.createPacket(
			512,
			receiverQueue,
			receiverKey,
			false
		...
		NPFPacket fragment3 = sender.createPacket(
			512,
			senderQueue,
			senderKey,
			false
		...
		NPFPacket resend1 = sender.createPacket(
			512,
			senderQueue,
			senderKey,
			false
		...
		assertEquals(
			0,
			receiver.handleDecryptedPacket(resend1, receiverKey).size()
		);

		//Make sure an ack is sent
		Thread.sleep(NewPacketFormatKeyContext.MAX_ACK_DELAY * 2);
		NPFPacket ack2 = receiver.createPacket(
			512,
			receiverQueue,
			receiverKey,
			false
	...
	public void testOutOfOrderDelivery() throws BlockedTooLongException {
		...
		assertEquals(
			1,
			receiver.handleDecryptedPacket(fragment2, receiverKey).size()
		);
	}
	...
	public void testReceiveUnknownMessageLength()
		...
		assertEquals(
			1,
			receiver.handleDecryptedPacket(fragment1, receiverKey).size()
		);
	}
	...
	public void testResendAlreadyCompleted()
		...
		NPFPacket packet1 = sender.createPacket(
			512,
			senderQueue,
			senderKey,
			false
		...
		assertEquals(
			1,
			receiver.handleDecryptedPacket(packet1, receiverKey).size()
		);
	...
	public void testLoadStatsSendWhenPeerWants()
		...
		NullBasePeerNode senderNode = new NullBasePeerNode() {
			...
			@Override
			public synchronized boolean grabSendLoadStatsASAP(
				boolean realtime
			) {
		...
		List<byte[]> finished = receiver.handleDecryptedPacket(
			packet1,
			receiverKey
		);
		assertEquals(2, finished.size());
		DecodingMessageGroup decoder =
			receiverNode.startProcessingDecryptedMessages(finished.size());
	...
	@Test
	public void testLoadStatsLowLevel()
		throws BlockedTooLongException, InterruptedException {
		final byte[] loadMessage = new byte[] {
			(byte) 0xFF,
			(byte) 0xEE,
			(byte) 0xDD,
			(byte) 0xCC,
		...
		NPFPacketTest.checkEquals(
			loadMessage,
			packet1.getLossyMessages().get(0)
		);
	...
	@Test
	public void testLoadStatsHighLevel()
		throws BlockedTooLongException, InterruptedException {
		final Message loadMessage = DMT.createFNPVoid();
		final MutableBoolean gotMessage = new MutableBoolean();
		NullBasePeerNode senderNode = new NullBasePeerNode() {
	...
	public void testEncryption()
		...
		NewPacketFormatKeyContext senderContext = new NewPacketFormatKeyContext(
			senderStartSeq,
			receiverStartSeq
		);
		...
		SessionKey senderSessionKey = new SessionKey(
			null,
			outgoingCipher,
			outgoingKey,
			incomingCipher,
		...
		SessionKey receiverSessionKey = new SessionKey(
			null,
			incomingCipher,
			incomingKey,
			outgoingCipher,
		...
		NewPacketFormat senderNPF = new NewPacketFormat(
			senderNode,
			senderStartSeq,
			receiverStartSeq
		);
		NewPacketFormat receiverNPF = new NewPacketFormat(
			receiverNode,
			receiverStartSeq,
			senderStartSeq
		);
		...
		FreenetInetAddress LOCALHOST = new FreenetInetAddress(
			"127.0.0.1",
			true
		);
		...
		receiverNPF.handleReceivedPacket(
			data,
			0,
			data.length,
		...
		assertTrue(
			Arrays.equals(message, receiverNode.decryptedMessages.get(0))
		);
	}
}
