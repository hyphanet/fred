...
public class NodeAndClientLayerBlobTest extends NodeAndClientLayerTestBase {
	...
	private static final File dir = new File(
		"test-fetch-pull-blob-single-node"
	);

	@Test
	public void testFetchPullBlobSingleNode()
		throws InvalidThresholdException, NodeInitException, InsertException, FetchException, IOException, BinaryBlobFormatException {
		...
		assertTrue(
			BucketTools.equalBuckets(result.asBucket(), block.getData())
		);
		// Now fetch the blob...
		fw = new FetchWaiter(rc);
		Bucket blobBucket = node.clientCore.tempBucketFactory.makeBucket(
			FILE_SIZE * 3
		);
		BinaryBlobWriter bbw = new BinaryBlobWriter(blobBucket);
		ClientGetter getter = new ClientGetter(
			fw,
			uri,
			ctx,
			(short) 0,
		...
		node2.start(false);
		HighLevelSimpleClient client2 = node.clientCore.makeClient(
			(short) 0,
			false,
			false
		);
