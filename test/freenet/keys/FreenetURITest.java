...
public class FreenetURITest {
	...
	@Test
	public void testDeriveRequestURIFromInsertURI()
		...
		assertEquals(
			requestUriUSK,
			insertUriUSK.deriveRequestURIFromInsertURI()
		);

		try {
			requestUriUSK.deriveRequestURIFromInsertURI();
			fail(
				"requestUriUSK.deriveRequestURIFromInsertURI() should fail because it IS a request URI already!"
			);
		...
		assertEquals(
			requestUriSSK,
			insertUriSSK.deriveRequestURIFromInsertURI()
		);

		try {
			requestUriSSK.deriveRequestURIFromInsertURI();
			fail(
				"requestUriSSK.deriveRequestURIFromInsertURI() should fail because it IS a request URI already!"
			);
		} catch (MalformedURLException e) {
			// Success
		}
		...
		assertEquals(
			requestUriSSKPlain,
			insertUriSSKPlain.deriveRequestURIFromInsertURI()
		);

		try {
			requestUriSSKPlain.deriveRequestURIFromInsertURI();
			fail(
				"requestUriSSKPlain.deriveRequestURIFromInsertURI() should fail because it IS a request URI already!"
			);
		} catch (MalformedURLException e) {
			// Success
		}
	...
	@Test
	public void sskCanBeCreatedWithoutRoutingKey()
		throws MalformedURLException {
		new FreenetURI("SSK@");
	}
	...
	@Test
	public void addedValidSchemaPrefixesAreIgnored()
		throws MalformedURLException {
