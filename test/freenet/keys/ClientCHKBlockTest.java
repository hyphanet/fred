...
public class ClientCHKBlockTest {
	...
	private void checkBlock(byte[] data, boolean newAlgo) throws Exception {
		...
		ClientCHKBlock encodedBlock = ClientCHKBlock.encode(
		...
		if (newAlgo) {
			// Check with no JCA.
			ClientCHKBlock otherEncodedBlock = ClientCHKBlock.encode(
				new ArrayBucket(data),
				false,
				false,
				(short) -1,
			...
			assertTrue(
				Arrays.equals(
					otherEncodedBlock.getBlock().data,
					encodedBlock.getBlock().data
				)
			);
			assertTrue(
				Arrays.equals(
					otherEncodedBlock.getBlock().headers,
					encodedBlock.getBlock().headers
				)
			);
		}
		// Verify it.
		CHKBlock block = CHKBlock.construct(
			encodedBlock.getBlock().data,
			encodedBlock.getBlock().headers,
			cryptoAlgorithm
		);
		ClientCHKBlock checkBlock = new ClientCHKBlock(block, key);
		ArrayBucket checkData = (ArrayBucket) checkBlock.decode(
		...
		if (newAlgo) {
			checkData = (ArrayBucket) checkBlock.decode(
				new ArrayBucketFactory(),
				data.length,
				false,
				true
