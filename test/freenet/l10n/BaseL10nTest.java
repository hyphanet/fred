...
public class BaseL10nTest {
	...
	public void testAddL10nSubstitutionUnclosed() {
		...
		l10n.addL10nSubstitution(
			node,
			"test.unclosedSubstitution",
			new String[] { "image" },
			new HTMLNode[] { imgNode }
		);
		assertEquals(
			"Text with <img /> unclosed substitution",
	...
	public void testAddL10nSubstitutionUnclosedMissing() {
		...
		assertEquals(
			"Text with  unclosed substitution",
			node.generateChildren()
		);
	}
	...
	public void testAddL10nSubstitutionMultiple() {
		...
		assertEquals(
			"<r1>Rep 1</r1><r2>Rep 2</r2> and <r3>Rep 3</r3>",
			node.generateChildren()
		);
	}
	...
	public void testAddL10nSubstitutionMissing() {
		...
		assertEquals(
			"Rep 1<r2>Rep 2</r2> and <r3>Rep 3</r3>",
			node.generateChildren()
		);
	}
	...
	public void testAddL10nSubstitutionNested() {
		...
		assertEquals(
			"<out>Text and <in>replacement</in></out>",
			node.generateChildren()
		);
	}
	...
	public void testAddL10nSubstitutionSelfNestedEmpty() {
		...
		l10n.addL10nSubstitution(
			node,
			"test.emptySelfNestedSubstitution",
			new String[] { "tag" },
			new HTMLNode[] { tagNode }
		);
		assertEquals(
			"<tag>content <tag></tag>nested</tag>",
			node.generateChildren()
		);
	}
	...
	public void testAddL10nSubstitutionUnmatchedClose() {
		...
		l10n.addL10nSubstitution(
			node,
			"test.unmatchedCloseSubstitution",
			new String[] { "ok" },
			new HTMLNode[] { okNode }
		);
		assertEquals(
	...
	public void testStrings() throws Exception {
		for (LANGUAGE lang : LANGUAGE.values()) {
			...
			if (fields != null) {
				for (
					Iterator<String> itr = fields.keyIterator();
					itr.hasNext();
				) {
					String key = itr.next();
