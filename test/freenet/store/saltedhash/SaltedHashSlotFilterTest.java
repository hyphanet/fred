...
public class SaltedHashSlotFilterTest {
	...
	private static final int ACCEPTABLE_FALSE_POSITIVES = TestProperty.EXTENSIVE
		? 5
		: 2;
	private static final int STORE_SIZE = TEST_COUNT * 5;
	private static final File TEMP_DIR = new File(
		"tmp-SaltedHashSlotFilterTest"
	);
	...
	private int populateStore(
		...
		for (int i = 0; i < numKeys; i++) {
			...
			CHKBlock verifyBlock = store.fetch(
				key.getNodeCHK(),
				false,
				false,
				null
	...
	public void testCHKPresentWithClose_writeImmediately()
		throws IOException, CHKEncodeException, CHKVerifyException, CHKDecodeException {
		checkCHKPresentWithClose(
			-1,
			"testCHKPresentWithClose_writeImmediately"
		);
	}
	...
	public void testCHKPresentWithClose_veryLongPersistanceTime()
		...
		checkCHKPresentWithClose(
			600 * 1000,
			"testCHKPresentWithClose_veryLongPersistanceTime"
		);
	}
