...
public abstract class Version {
	...
	public static String altProtocolVersion = "2.0";

	/** The build number of the current revision */
	public static final int buildNumber = 70000;

	/** Oldest build of Fred we will talk to */
	public static final int firstGoodBuild = 70000;
	// Rate limiting fixes
...
//	// 5077: bidi routing
	...
	static {
		StringTokenizer cvsId =
			new StringTokenizer("$Id: Version.java,v 1.1 2005/03/16 22:10:57 amphibian Exp $");
		cvsId.nextToken();
		cvsId.nextToken();
...
//		}
		if(logDEBUG)
			Logger.debug(Version.class, "Accepting: " + version);
		return true;
	}
...
//						+ " or " + stableProtocolVersion
		...
		if (sameVersion(v)) {
			try {
				int build = Integer.parseInt(v[3]);
				if (build < firstGoodBuild)
					return "Build older than last good build " + firstGoodBuild;
			} catch (NumberFormatException e) {
