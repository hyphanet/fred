...
public class PersistentPutDir extends FCPMessage {
	...
	private SimpleFieldSet generateFieldSet() {
		...
		ManifestElement[] elements = BaseManifestPutter.flatten(
			manifestElements
		);
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (tempURI != null) {
				...
				if (data instanceof DelayedFreeBucket) {
					...
					data =
						((DelayedFreeRandomAccessBucket) data).getUnderlying();
				}
				subset.put("DataLength", e.getSize());
				if (mimeOverride != null) subset.putSingle(
				...
				if (data == null) {
					...
					data instanceof PaddedEphemerallyEncryptedBucket ||
					...
					throw new IllegalStateException(
						"Don't know what to do with bucket: " + data
					);
				}
			}
