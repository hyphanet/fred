		...
		while (true) {
			...
			try {
				if (logDEBUG) Logger.debug(
					this,
					"Incoming FCP message:\n" +
					messageType +
					'\n' +
					fs.toString()
				);
				msg = FCPMessage.create(
					messageType,
					fs,
					handler.bf,
			...
			if (msg instanceof BaseDataCarryingMessage) {
				...
				try {
					((BaseDataCarryingMessage) msg).readFrom(
							lis,
							handler.bf,
							handler.server
						);
			...
			if ((!firstMessage) && (msg instanceof ClientHelloMessage)) {
				FCPMessage err = new ProtocolErrorMessage(
					ProtocolErrorMessage.NO_LATE_CLIENT_HELLOS,
					false,
					null,
					null,
					false
			...
			try {
				if (logDEBUG) Logger.debug(
					this,
					"Parsed message: " + msg + " for " + handler
				);
