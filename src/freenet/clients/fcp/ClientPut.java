...
public class ClientPut extends ClientPutBase {
	...
	public ClientPut(
		...
		if (uploadFromType == UploadFrom.DISK) {
			if (
				!core.allowUploadFrom(origFilename)
			) throw new NotAllowedException();
			if (
				!(origFilename.exists() && origFilename.canRead())
		...
		if (logMINOR) Logger.minor(
			this,
			"data = " + tempData + ", uploadFrom = " + uploadFrom
		);
		if (uploadFrom == UploadFrom.REDIRECT) {
			...
			tempData = m.toBucket(
				core.clientContext.getBucketFactory(isPersistentForever())
			);
			isMetadata = true;
		} else targetURI = null;
	...
	public ClientPut(
		...
		if (message.uploadFromType == UploadFrom.DISK) {
			if (
				!handler.server.core.allowUploadFrom(message.origFilename)
			...
			if (message.fileHash != null) {
				...
				!handler.allowDDAFrom(message.origFilename, false)
			) throw new MessageInvalidException(
		...
		if (mimeType == null && origFilename != null) {
			mimeType = DefaultMIMETypes.guessMIMEType(
		...
		if (
			mimeType != null && !DefaultMIMETypes.isPlausibleMIMEType(mimeType)
		) {
			throw new MessageInvalidException(
		...
		if (uploadFrom == UploadFrom.REDIRECT) {
			...
			try {
				tempData = m.toBucket(
					server.core.clientContext.getBucketFactory(
						isPersistentForever()
					)
				);
		...
		if (salt != null) {
			...
			if (
				!Arrays.equals(saltedHash, foundHash)
			) throw new MessageInvalidException(
				ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
		...
		if (logMINOR) Logger.minor(
			this,
			"data = " + data + ", uploadFrom = " + uploadFrom
		);
		putter = new ClientPutter(
			this,
			data,
