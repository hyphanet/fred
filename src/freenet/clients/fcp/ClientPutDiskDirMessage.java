...
public class ClientPutDiskDirMessage extends ClientPutDirMessage {
	...
	public ClientPutDiskDirMessage(SimpleFieldSet fs)
		...
		if (fnam == null) throw new MessageInvalidException(
	...
	public void run(FCPConnectionHandler handler, Node node)
		throws MessageInvalidException {
		if (
			!handler.server.core.allowUploadFrom(dirname)
	 ...
	 * @throws MessageInvalidException
	 */
	private HashMap<String, Object> makeBucketsByName(
		File thisdir,
		String prefix
	) throws MessageInvalidException {
		...
		if (filelist == null) throw new MessageInvalidException(
			...
			if (filelist[i].canRead() && filelist[i].exists()) {
				if (filelist[i].isFile()) {
					...
					HashMap<String, Object> subdir = makeBucketsByName(
						new File(thisdir, filelist[i].getName()),
				...
				if (!allowUnreadableFiles) throw new MessageInvalidException(
					ProtocolErrorMessage.FILE_NOT_FOUND,
