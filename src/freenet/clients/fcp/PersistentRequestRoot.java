...
public class PersistentRequestRoot {
	...
	public PersistentRequestRoot() {
		globalForeverClient = new PersistentRequestClient(
			"Global Queue",
			null,
			true,
			null,
	...
	public PersistentRequestClient registerForeverClient(
		...
		synchronized (this) {
			...
			if (client == null) client = new PersistentRequestClient(
				name,
				handler,
				false,
				null,
	...
	public ClientRequest[] getPersistentRequests() {
		...
		for (PersistentRequestClient client : clients.values()) client.addPersistentRequests(
			requests,
			true
		);
	...
	PersistentRequestClient resume(
