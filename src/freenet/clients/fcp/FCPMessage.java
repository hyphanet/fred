...
public abstract class FCPMessage {
	...
	public static final String BULK_CHK_REQUEST_REJECTS =
		"Rejects.Bulk.Request.CHK";
	public static final String BULK_SSK_REQUEST_REJECTS =
		"Rejects.Bulk.Request.SSK";
	public static final String BULK_CHK_INSERT_REJECTS =
		"Rejects.Bulk.Insert.CHK";
	public static final String BULK_SSK_INSERT_REJECTS =
		"Rejects.Bulk.Insert.SSK";
	public static final String OUTPUT_BANDWIDTH_CLASS = "OutputBandwidthClass";
	public static final String OVERALL_BULK_OUTPUT_CAPACITY_USAGE =
		"OverallBulkOutputCapacityUsage";
	...
	public static FCPMessage create(
		...
		if (name.equals(ClientHelloMessage.NAME)) return new ClientHelloMessage(
			fs
		);
		if (
			name.equals(ClientPutComplexDirMessage.NAME)
		) return new ClientPutComplexDirMessage(fs, bfTemp, bfPersistent);
		if (
			name.equals(ClientPutDiskDirMessage.NAME)
		) return new ClientPutDiskDirMessage(fs);
		if (name.equals(ClientPutMessage.NAME)) return new ClientPutMessage(fs);
		if (
			name.equals(SendBookmarkMessage.NAME)
		...
		if (name.equals(DisconnectMessage.NAME)) return new DisconnectMessage(
			fs
		);
		if (
			name.equals(FCPPluginClientMessage.NAME)
		) return new FCPPluginClientMessage(fs);
		if (name.equals(GenerateSSKMessage.NAME)) return new GenerateSSKMessage(
			fs
		);
		...
		if (
			name.equals(GetRequestStatusMessage.NAME)
		...
		if (
			name.equals(ListPeerNotesMessage.NAME)
		) return new ListPeerNotesMessage(fs);
		if (
			name.equals(ListPersistentRequestsMessage.NAME)
		...
		if (
			name.equals(ModifyPersistentRequest.NAME)
		...
		if (
			name.equals(RemovePersistentRequest.NAME) ||
		...
		if (name.equals(WatchFeedsMessage.NAME)) return new WatchFeedsMessage(
			fs
		);
		if (
			name.equals(SubscribeUSKMessage.NAME)
		) return new SubscribeUSKMessage(fs);
		if (name.equals(WatchFeedsMessage.NAME)) return new WatchFeedsMessage(
			fs
		);
		if (
			name.equals(UnsubscribeUSKMessage.NAME)
		) return new UnsubscribeUSKMessage(fs);
		if (
			name.equals(TestDDARequestMessage.NAME)
		) return new TestDDARequestMessage(fs);
		if (
			name.equals(TestDDAResponseMessage.NAME)
		...
		if (name.equals(FilterMessage.NAME)) return new FilterMessage(
			fs,
			bfTemp
		);
