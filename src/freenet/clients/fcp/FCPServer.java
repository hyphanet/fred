...
public class FCPServer implements Runnable, DownloadCache {
	...
	public FCPServer(
		...
		globalRebootClient = new PersistentRequestClient(
			"Global Queue",
			null,
			true,
			null,
	...
	private void maybeGetNetworkInterface() {
		...
		try {
			if (ssl) {
				tempNetworkInterface = SSLNetworkInterface.create(
					port,
					bindTo,
					allowedHosts,
					node.executor,
				...
				tempNetworkInterface = NetworkInterface.create(
					port,
					bindTo,
					allowedHosts,
					node.executor,
			...
			Logger.error(
				this,
				"Couldn't bind to FCP Port " +
				bindTo +
				':' +
				port +
				". FCP Server not started.",
				be
			);
			System.out.println(
				"Couldn't bind to FCP Port " +
	...
	public void maybeStart() {
		if (this.enabled) {
			...
			Logger.normal(
				this,
				"Starting FCP server on " + bindTo + ':' + port + '.'
			);
			System.out.println(
				"Starting FCP server on " + bindTo + ':' + port + '.'
			);
		...
		public void set(Integer val) throws InvalidConfigValueException {
			if (!get().equals(val)) {
				throw new InvalidConfigValueException(
					"Cannot change FCP port number on the fly"
				);
			}
		}
	...
	static class FCPEnabledCallback extends BooleanCallback {
		...
		public void set(Boolean val) throws InvalidConfigValueException {
			if (!get().equals(val)) {
				throw new InvalidConfigValueException(
					l10n("cannotStartOrStopOnTheFly")
				);
			}
		}
	...
	static class FCPSSLCallback extends BooleanCallback {
		...
		public void set(Boolean val) throws InvalidConfigValueException {
			...
			if (!SSL.available()) {
				throw new InvalidConfigValueException(
					"Enable SSL support before use ssl with FCP"
				);
			}
			ssl = val;
			throw new InvalidConfigValueException(
	...
	static class FCPBindtoCallback extends StringCallback {
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			if (!val.equals(oldValue)) {
				...
				String[] failedAddresses = server.networkInterface.setBindTo(
					val,
					true
				);
		...
		public void set(String val) throws InvalidConfigValueException {
			if (!val.equals(get())) {
				try {
					node
						.getFCPServer()
						.allowedHostsFullAccess.setAllowedHosts(val);
				} catch (IllegalArgumentException e) {
					throw new InvalidConfigValueException(e);
				}
			}
	...
	public final FCPPluginConnection createFCPPluginConnectionForIntraNodeFCP(
		...
		// The constructor function already did this for us
		/* pluginConnectionTracker.registerConnection(connection); */
		return connection.getDefaultSendDirectionAdapter(
	 ...
	 */
	public final FCPPluginConnection getPluginConnectionByID(UUID connectionID)
		throws IOException {
	...
	public PersistentRequestClient registerRebootClient(
		...
		synchronized (this) {
			...
			if (oldClient == null) {
				// Create new client
				PersistentRequestClient client = new PersistentRequestClient(
					name,
					handler,
					false,
					null,
	...
	public RequestStatus[] getGlobalRequests()
	...
	public boolean removeGlobalRequestBlocking(final String identifier)
		...
		if (
			!globalRebootClient.removeByIdentifier(
				identifier,
				true,
				this,
				core.clientContext
			...
			core.clientContext.jobRunner.queue(
				new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						...
						try {
							succeeded = globalForeverClient.removeByIdentifier(
								identifier,
								true,
								FCPServer.this,
								core.clientContext
			...
			new PersistentJob() {
				...
				public boolean run(ClientContext context) {
					...
					try {
						...
						Logger.error(
							this,
							"Caught while processing panic: " + t,
							t
						);
						System.err.println("PANIC INCOMPLETE: CAUGHT " + t);
						t.printStackTrace();
						System.err.println(
							"Your requests have not been deleted!"
						);
					} finally {
						success.set(succeeded);
						done.countDown();
			...
			new PersistentJob() {
				...
				public boolean run(ClientContext context) {
					...
					try {
						...
						Logger.error(
							this,
							"Failed to make persistent request: " + t,
							t
						);
						return false;
					} finally {
			...
			core.clientContext.jobRunner.queue(
				new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						...
						try {
							ClientRequest req = globalForeverClient.getRequest(
								identifier
							);
							if (req != null) req.modifyRequest(
								newToken,
								newPriority,
								FCPServer.this
							);
	...
	public void makePersistentGlobalRequest(
		...
		ReturnType returnType = ReturnType.valueOf(
			returnTypeString.toUpperCase()
		);
		File returnFilename = null;
		if (returnType == ReturnType.DISK) {
			returnFilename = makeReturnFilename(
				fetchURI,
				expectedMimeType,
				downloadsDir
			);
	...
	private File makeReturnFilename(
		FreenetURI uri,
		String expectedMimeType,
		File downloadsDir
	) {
		...
		if (
			!(ext != null && preferredWithExt.endsWith(ext))
	...
	private void innerMakePersistentGlobalRequest(
		...
		FetchContext defaultFetchContext =
			core.clientContext.getDefaultPersistentFetchContext();
		final ClientGet cg = new ClientGet(
	...
	public boolean restartBlocking(
		final String identifier,
		...
		if (req != null) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Queueing restart of " + identifier
			);
			core.clientContext.jobRunner.queue(
				new PersistentJob() {
					@Override
	...
	public FetchResult getCompletedRequestBlocking(final FreenetURI key)
		...
		if (get != null) {
			...
			return new FetchResult(
				new ClientMetadata(get.getMIMEType()),
				new NoFreeBucket(get.getBucket())
			);
		}

		FetchResult result = globalForeverClient
			.getRequestStatusCache()
			.getShadowBucket(key, false);
		if (result != null) {
			return result;
		}
	...
	public CacheFetchResult lookupInstant(
		...
		if (origData == null && globalForeverClient != null) {
			CacheFetchResult result = globalForeverClient
				.getRequestStatusCache()
				.getShadowBucket(key, noFilter);
			if (result != null) {
		...
		if (!mustCopy) return new CacheFetchResult(
			new ClientMetadata(mime),
			origData,
			filtered
		);
	...
	public CacheFetchResult lookup(
		...
		if (get != null) {
			...
			if (newData == null) {
				try {
					if (preferred != null) newData = preferred;
					else newData = core.tempBucketFactory.makeBucket(
						origData.size()
					);
					BucketTools.copy(origData, newData);
				} catch (IOException e) {
