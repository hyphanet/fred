...
public class TestDDACompleteMessage extends FCPMessage {
	...
	public SimpleFieldSet getFieldSet() {
		...
		sfs.putSingle(
			TestDDARequestMessage.DIRECTORY,
		...
		if (checkJob.writeFilename != null) {
			...
			if (
				maybeWrittenFile.exists() &&
				maybeWrittenFile.isFile() &&
				maybeWrittenFile.canRead()
			) {
				try {
					String existingContent = FileUtil.readUTF(maybeWrittenFile)
						.toString()
						.trim();
					isWriteAllowed = checkJob.writeContent.equals(
						existingContent
					);
				} catch (IOException e) {
					Logger.error(
