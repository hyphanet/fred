...
public class ClientPutComplexDirMessage extends ClientPutDirMessage {
	...
	public ClientPutComplexDirMessage(
		...
		if (files == null) throw new MessageInvalidException(
		...
		for (int i = 0;; i++) {
			...
			DirPutFile f = DirPutFile.create(
				subset,
				identifier,
				global,
	...
	private void convertFilesByNameToManifestElements(
		HashMap<String, Object> filesByName,
		...
		for (Map.Entry<String, Object> entry : filesByName.entrySet()) {
			...
			if (val instanceof HashMap) {
				...
				if (
					f instanceof DiskDirPutFile &&
					!node.clientCore.allowUploadFrom(
						((DiskDirPutFile) f).getFile()
					)
				) throw new MessageInvalidException(
