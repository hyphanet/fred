...
final class FCPPluginConnectionImpl implements FCPPluginConnection {
	...
	> defaultSendDirectionAdapters = new EnumMap<
		SendDirection,
		DefaultSendDirectionAdapter
	>(SendDirection.class);
	...
	@Override
	public void send(final SendDirection direction, FCPPluginMessage message)
		throws IOException {
		// We first have to compute the message.permissions field ourselves - we shall ignore what
		// caller said for security.
		ClientPermissions currentClientPermissions = (direction ==
				SendDirection.ToClient)
		...
		message = FCPPluginMessage.constructRawMessage(
		...
		assert (direction == SendDirection.ToServer
				? server != null
				: client !=
				null) : "We already decided that the message handler exists locally. " +
		...
		// handle case 1.
		if (
			dispatchMessageLocallyToSendSynchronousThreadIfExisting(
				direction,
				message
			)
		) return;
		...
		final FredPluginFCPMessageHandler messageHandler = (direction ==
				SendDirection.ToServer)
			? server.get()
			: client;
		...
		dispatchMessageLocallyToMessageHandler(
			messageHandler,
			direction,
			message
		);
	...
	private void dispatchMessageByNetwork(
		...
		assert (direction ==
			SendDirection.ToClient) : "By design, this class always shall execute in the same VM as the server plugin. " +
		"So for networked messages, we should always be sending to the client.";

		assert (clientConnection !=
			null) : "Trying to send a message over the network to the client. " +
		"So the network connection to it should not be null.";

		if (clientConnection.isClosed()) throw new IOException(
			"Connection to client closed for " + this
		);

		clientConnection.send(
			new FCPPluginServerMessage(serverPluginName, message)
		);
	}
	...
	private boolean dispatchMessageLocallyToSendSynchronousThreadIfExisting(
		...
		try {
			SynchronousSend synchronousSend = synchronousSends.get(
				message.identifier
			);
			...
			assert (synchronousSend.reply ==
				null) : "One identifier should not be used for multiple messages or replies";
				...
				try {
					try {
						reply = messageHandler.handlePluginFCPMessage(
							getDefaultSendDirectionAdapter(direction.invert()),
							message
						);
					} catch (Error e) {
			...
			public int getPriority() {
				...
				if (messageHandler instanceof PrioritizedMessageHandler) {
					try {
						...
						Logger.error(
							messageHandler,
							"Message handler's getPriority() threw!",
							t
						);
					}
				}
	...
	public FCPPluginMessage sendSynchronous(
		...
		synchronousSendsLock.writeLock().lock();
		try {
			final Condition completionSignal = synchronousSendsLock
				.writeLock()
				.newCondition();
			final SynchronousSend synchronousSend = new SynchronousSend(
				completionSignal
			);
			...
			assert (!synchronousSends.containsKey(
					message.identifier
			...
			do {
				...
				timeoutNanoSeconds = completionSignal.awaitNanos(
					timeoutNanoSeconds
				);
				...
				if (synchronousSend.reply != null) {
					assert (synchronousSend.reply.identifier.equals(
							message.identifier
						));
	 ...
	 */
	private abstract static class DefaultSendDirectionAdapter
		 ...
		 */
		protected abstract FCPPluginConnection getConnection()
			throws IOException;
		...
		public FCPPluginMessage sendSynchronous(
			...
			return getConnection()
				.sendSynchronous(direction, message, timeoutNanoSeconds);
		}
	}
	...
	private static final class SendToClientAdapter
		...
		SendToClientAdapter(
			...
			try {
				this.connectionRef = tracker.getConnectionWeakReference(
					connectionID
				);
			} catch (IOException e) {
		...
		public String toString() {
			...
			try {
				...
				return (
					prefix + " FCPPluginConnectionImpl (" + e.getMessage() + ")"
				);
			}
		}
	}
	 ...
	 * - {@link FCPPluginConnection#sendSynchronous(FCPPluginMessage, long)}<br><br>
	 */
	public FCPPluginConnection getDefaultSendDirectionAdapter(
		SendDirection direction
	) {
