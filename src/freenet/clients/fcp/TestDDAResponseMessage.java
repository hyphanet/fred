...
public class TestDDAResponseMessage extends FCPMessage {
	...
	public TestDDAResponseMessage(SimpleFieldSet sfs)
		...
		if (identifier.length() == 0) throw new MessageInvalidException(
			ProtocolErrorMessage.MISSING_FIELD,
	...
	public void run(FCPConnectionHandler handler, Node node)
		...
		if (job == null) throw new MessageInvalidException(
			ProtocolErrorMessage.INVALID_MESSAGE,
			"The node doesn't know that testDDA identifier! double check it! (" +
			identifier +
			").",
			identifier,
			false
			...
			(job.readFilename != null) && (readContent == null)
		) throw new MessageInvalidException(
			ProtocolErrorMessage.MISSING_FIELD,
			"You need to send " +
			READ_CONTENT +
		...
		TestDDACompleteMessage reply = new TestDDACompleteMessage(
			handler,
			job,
			readContent
		);
