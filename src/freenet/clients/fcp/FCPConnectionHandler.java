...
public class FCPConnectionHandler implements Closeable {
	...
	> pluginConnectionsByServerName = new TreeMap<
		String,
		FCPPluginConnectionImpl
	>();
	...
	public void close() {
		...
		synchronized (this) {
			...
			uskSubscriptions2 = uskSubscriptions
				.values()
				.toArray(new SubscribeUSK[uskSubscriptions.size()]);
			dupe = killedDupe;
		}
		for (ClientRequest req : requests) req.onLostConnection(
	...
	public void setClientName(final String name) {
		...
		PersistentRequestClient client = server.getForeverClient(
			name,
			server.core,
			this
		);
	...
	protected PersistentRequestClient createForeverClient(String name) {
		...
		PersistentRequestClient client = server.registerForeverClient(
			name,
			server.core,
	...
	public void startClientGet(final ClientGetMessage message) {
								...
								new PersistentJob() {
									...
									public boolean run(ClientContext context) {
										...
										try {
											...
											Logger.normal(
												this,
												"Identifier collision on " +
												this
											);
											FCPMessage msg =
												new IdentifierCollisionMessage(
													id,
													message.global
												);
		...
		if (message.persistence == Persistence.REBOOT) try {
			cg.register(false);
		} catch (IdentifierCollisionException e) {
			success = false;
		}
	...
	public void startClientPut(final ClientPutMessage message) {
		if (logMINOR) Logger.minor(
			this,
			"Starting insert ID=\"" + message.identifier + '"'
		);
		final String id = message.identifier;
		final boolean global = message.global;
		ClientPut cp = null;
		...
		synchronized (this) {
			...
			if (success) {
				if (!persistent) {
					try {
						...
						failedMessage = new ProtocolErrorMessage(
			...
			if (!success) {
				Logger.normal(this, "Identifier collision on " + this);
				failedMessage = new IdentifierCollisionMessage(
					id,
					message.global
				);
			}
		}
		if (message.persistence == Persistence.REBOOT && cp != null) try {
			cp.register(false);
		} catch (IdentifierCollisionException e) {
	...
	FCPPluginConnection getFCPPluginConnection(String serverPluginName)
		...
		try {
			...
			if (
				peekOldConnection != null && !peekOldConnection.isServerDead()
			) {
		...
		try {
			...
			FCPPluginConnectionImpl newConnection =
				server.createFCPPluginConnectionForNetworkedFCP(
					serverPluginName,
					this
				);
	...
	protected boolean allowDDAFrom(File filename, boolean writeRequest) {
		String parentDirectory = FileUtil.getCanonicalFile(
			filename
		).getParent();
		...
		if (logMINOR) Logger.minor(
			this,
			"Checking DDA: " + da + " for " + parentDirectory
		);

		if (writeRequest) return (
			da == null ? server.isDownloadDDAAlwaysAllowed() : da.canWrite
		);
		else return (
			da == null ? server.isUploadDDAAlwaysAllowed() : da.canRead
		);
	}
	 ...
	 * @param write
	 */
	protected void registerTestDDAResult(
		String path,
		boolean read,
		boolean write
	) {
		...
		if (logMINOR) Logger.minor(
			this,
			"DDA: read=" + read + " write=" + write + " for " + path
		);
	}
	 ...
	 */
	protected DDACheckJob enqueueDDACheck(
		String path,
		boolean read,
		boolean write
	) throws IllegalArgumentException {
		...
		if (job != null) throw new IllegalArgumentException(
			"There is already a TestDDA going on for that directory!"
		);
		...
		File writeFile =
			(write
					? new File(
						path,
						"DDACheck-" +
						server.node.fastWeakRandom.nextInt() +
						".tmp"
					)
					: null);
		File readFile = null;
		...
		DDACheckJob result = new DDACheckJob(
			server.node.fastWeakRandom,
			directory,
			readFile,
			writeFile
		);
		...
		if (read && (readFile != null) && readFile.canWrite()) {
			...
			try {
				...
				Logger.error(
					this,
					"Got a IOE while creating the file (" +
					readFile.toString() +
					" ! " +
					e.getMessage()
				);
	 ...
	 * @throws IllegalArgumentException
	 */
	protected DDACheckJob popDDACheck(String path)
		throws IllegalArgumentException {
		File directory = FileUtil.getCanonicalFile(new File(path));
		if (
			!directory.exists() || !directory.isDirectory()
	...
	public ClientRequest removeRequestByIdentifier(
		String identifier,
		boolean kill
	) {
		ClientRequest req;
	...
	ClientRequest getRebootRequest(
		boolean global,
		FCPConnectionHandler handler,
		String identifier
	) {
		if (global) return handler.server.globalRebootClient.getRequest(
			identifier
		);
		else return handler.getRebootClient().getRequest(identifier);
	}

	ClientRequest getForeverRequest(
		boolean global,
		FCPConnectionHandler handler,
		String identifier
	) {
		if (global) return handler.server.globalForeverClient.getRequest(
			identifier
		);
		else return handler.getForeverClient().getRequest(identifier);
	}

	ClientRequest removePersistentRebootRequest(
		boolean global,
		String identifier
	) throws MessageInvalidException {
		PersistentRequestClient client = global
			? server.globalRebootClient
			: getRebootClient();
		ClientRequest req = client.getRequest(identifier);
		if (req != null) {
			client.removeByIdentifier(
				identifier,
				true,
				server,
				server.core.clientContext
	...
	public synchronized void addUSKSubscription(
		String identifier,
		SubscribeUSK subscribeUSK
	) throws IdentifierCollisionException {
		if (
			uskSubscriptions.containsKey(identifier)
	...
	public void unsubscribeUSK(String identifier)
		...
		synchronized (this) {
			...
			if (sub == null) throw new MessageInvalidException(
