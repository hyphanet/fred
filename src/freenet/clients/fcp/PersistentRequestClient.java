...
public class PersistentRequestClient {
	...
	public PersistentRequestClient(
		...
		assert (persistence == Persistence.FOREVER ||
			persistence == Persistence.REBOOT);
	...
	public void queuePendingMessagesOnConnectionRestartAsync(
		...
		if (persistence == Persistence.FOREVER) {
			PersistentListJob job = new PersistentListJob(
				this,
				outputHandler,
				context,
				null
			...
			TransientListJob job = new TransientListJob(
				this,
				outputHandler,
				context,
				null
	...
	public void register(ClientRequest cg) throws IdentifierCollisionException {
		...
		if (statusCache != null) {
			if (cg instanceof ClientGet) {
				statusCache.addDownload(
					(DownloadRequestStatus) (cg.getStatus())
				);
	...
	public boolean removeByIdentifier(
		...
		synchronized (this) {
			if (completionCallbacks != null) callbacks =
				completionCallbacks.toArray(
					new RequestCompletionCallback[completionCallbacks.size()]
				);
		}
		if (callbacks != null) {
	...
	public boolean hasPersistentRequests() {
		return !(
			runningPersistentRequests.isEmpty() &&
			completedUnackedRequests.isEmpty()
		);
	}

	public void addPersistentRequests(
		List<ClientRequest> v,
		boolean onlyForever
	) {
		synchronized (this) {
	...
	/** From database */
	private void addPersistentRequestStatus(
		...
		for (ClientRequest req : reqs) {
			try {
				...
				Logger.error(
					this,
					"BROKEN REQUEST LOADING PERSISTENT REQUEST STATUS: " + t,
					t
				);
	...
	public boolean setWatchGlobal(
		...
		if (isGlobalQueue) {
			Logger.error(
				this,
				"Set watch global on global queue!: " + this,
				new Exception("debug")
			);
			return false;
		}
		...
		if (watchGlobal && !enabled) {
			...
			if (connHandler != null) {
				if (
					...
					connHandler.outputHandler,
					server.core.clientContext
				);
				else server.globalForeverClient.queuePendingMessagesOnConnectionRestartAsync(
					connHandler.outputHandler,
					server.core.clientContext
				);
			}
	...
	public void queueClientRequestMessage(
		FCPMessage msg,
		int verbosityLevel,
		...
		if (
			useGlobalMask &&
			(verbosityLevel & watchGlobalVerbosityMask) != verbosityLevel
		) return;
		...
		if (isGlobalQueue) {
			synchronized (clientsWatchingLock) {
				if (clientsWatching != null) clients = clientsWatching.toArray(
					new PersistentRequestClient[clientsWatching.size()]
				);
				else clients = null;
			}
			if (
	...
	public void notifySuccess(ClientRequest req) {
		...
		if (callbacks != null) {
			for (RequestCompletionCallback cb : callbacks) cb.notifySuccess(
				req
			);
		}
	}
	...
	public void notifyFailure(ClientRequest req) {
		...
		if (callbacks != null) {
			for (RequestCompletionCallback cb : callbacks) cb.notifyFailure(
				req
			);
		}
	}

	public synchronized void addRequestCompletionCallback(
		RequestCompletionCallback cb
	) {
		if (completionCallbacks == null) completionCallbacks = new ArrayList<
			RequestCompletionCallback
		>(); // it is transient so it might be null
		completionCallbacks.add(cb);
	}

	public synchronized void removeRequestCompletionCallback(
		RequestCompletionCallback cb
	) {
	...
	private void updateRequestStatusCache(RequestStatusCache cache) {
		if (persistence == Persistence.FOREVER) {
			...
			for (RequestStatus status : statuses) {
				if (status instanceof DownloadRequestStatus) cache.addDownload(
	...
	public void addPersistentRequesters(List<ClientRequester> requesters) {
		for (ClientRequest req : runningPersistentRequests) requesters.add(
			req.getClientRequest()
		);
		for (ClientRequest req : completedUnackedRequests) requesters.add(
			req.getClientRequest()
		);
	}

	public void resume(ClientRequest clientRequest) {
		if (clientRequest.hasFinished()) completedUnackedRequests.add(
			clientRequest
		);
		...
		if (clientRequestsByIdentifier.get(identifier) != null) {
			if (
				clientRequest != clientRequestsByIdentifier.get(identifier)
				...
				Logger.error(
					this,
					"Adding the same identifier twice: " + identifier
				);
				return;
			}
		} else {
