...
public class ConfigData extends FCPMessage {
	...
	public SimpleFieldSet getFieldSet() {
		...
		if (withCurrent) {
			SimpleFieldSet current = node.config.exportFieldSet(
		...
		if (withDefaults) {
			SimpleFieldSet defaultSettings = node.config.exportFieldSet(
		...
		if (withSortOrder) {
			SimpleFieldSet sortOrder = node.config.exportFieldSet(
		...
		if (withExpertFlag) {
			SimpleFieldSet expertFlag = node.config.exportFieldSet(
		...
		if (withForceWriteFlag) {
			SimpleFieldSet forceWriteFlag = node.config.exportFieldSet(
		...
		if (withShortDescription) {
			SimpleFieldSet shortDescription = node.config.exportFieldSet(
		...
		if (withLongDescription) {
			SimpleFieldSet longDescription = node.config.exportFieldSet(
		...
		if (withDataTypes) {
			SimpleFieldSet type = node.config.exportFieldSet(
