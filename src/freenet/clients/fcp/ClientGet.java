...
public class ClientGet
	...
	implements
		ClientGetCallback, ClientEventListener, PersistentClientCallback {
	...
	private static Map<Short, ReturnType> returnTypeByCode = new HashMap<
		Short,
		ReturnType
	>();
	...
	public enum ReturnType {
		...
		ReturnType(short code) {
			if (returnTypeByCode.containsKey(code)) throw new Error(
				"Duplicate"
			);
	...
	public ClientGet(
		...
		if (returnType == ReturnType.DISK) {
			...
			if (
				!(core.allowDownloadTo(returnFilename))
			...
			if (targetFile.exists()) {
				if (targetFile.length() == 0) {
					...
					Logger.error(
						this,
						"Target file already exists but is zero length, deleting..."
					);
				}
				if (targetFile.exists()) throw new IOException(
					"Target filename exists already: " + targetFile
				);
			}
			...
			if (filterData) {
				...
				if (idx != -1) {
					idx++;
					if (idx != name.length()) extensionCheck = name.substring(
						idx
					);
				}
			}
	...
	public ClientGet(
		...
		if (message.allowedMIMETypes != null) {
			...
			for (String mime : message.allowedMIMETypes) fctx.allowedMIMETypes.add(
				mime
			);
		}
		...
		if (returnType == ReturnType.DISK) {
			...
			if (
				!core.allowDownloadTo(targetFile)
	...
	private ClientGetter makeGetter(NodeClientCore core, Bucket ret)
		throws IOException {
		if (binaryBlob && ret == null) {
			ret = core.clientContext
	...
	public void setSuccessForMigration(
		...
		synchronized (this) {
			...
			if (returnType == ReturnType.NONE) {
				...
				if (
					!(targetFile.exists() &&
						targetFile.length() == foundDataLength)
				) throw new ResumeFailedException(
					"Success but target file doesn't exist or isn't valid"
				);
			} else if (returnType == ReturnType.DIRECT) {
				returnBucketDirect = data;
				if (
					returnBucketDirect.size() != foundDataLength
				) throw new ResumeFailedException(
	...
	private void trySendDataFoundOrGetFailed(
		...
		if (handler == null && persistence == Persistence.CONNECTION) handler =
			origHandler.outputHandler;
		if (handler != null) handler.queue(
			FCPMessage.withListRequestIdentifier(msg, listRequestIdentifier)
		);
		else client.queueClientRequestMessage(
			FCPMessage.withListRequestIdentifier(msg, listRequestIdentifier),
			0
		);
	}
	...
	private synchronized AllDataMessage getAllDataMessage() {
		...
		AllDataMessage msg = new AllDataMessage(
			returnBucketDirect,
			identifier,
			global,
			startupTime,
	...
	private void trySendAllDataMessage(
		...
		if (handler != null) {
			FCPMessage allData = FCPMessage.withListRequestIdentifier(
				getAllDataMessage(),
	...
	private void queueProgressMessageInner(
		...
		if (persistence == Persistence.CONNECTION && handler == null) handler =
			origHandler.outputHandler;
	...
	public void sendPendingMessages(
		...
		if (!onlyData) {
			...
			if (sentToNetwork) handler.queue(
				FCPMessage.withListRequestIdentifier(
					new SendingToNetworkMessage(identifier, global),
					listRequestIdentifier
				)
			);
			if (finished) trySendDataFoundOrGetFailed(
				handler,
				listRequestIdentifier
			);
		} else if (returnType != ReturnType.DIRECT) {
		...
		synchronized (this) {
			...
			if (foundDataMimeType != null) mimeMsg = new ExpectedMIME(
				identifier,
				global,
				foundDataMimeType
			);
			if (foundDataLength > 0) lengthMsg = new ExpectedDataLength(
				identifier,
				global,
				foundDataLength
			);
		...
		handler.queue(
			FCPMessage.withListRequestIdentifier(cmsg, listRequestIdentifier)
		);
	...
	public void onFailure(FetchException e, ClientGetter state) {
		...
		synchronized (this) {
			...
			if (e.getExpectedMimeType() != null) this.foundDataMimeType =
	...
	public void requestWasRemoved(ClientContext context) {
		...
		if (!finished) {
			synchronized (this) {
				succeeded = false;
				finished = true;
				FetchException cancelled = new FetchException(
					FetchExceptionMode.CANCELLED
				);
				getFailedMessage = new GetFailedMessage(
					cancelled,
					identifier,
					global
				);
		...
		FCPMessage msg = new PersistentRequestRemovedMessage(
			getIdentifier(),
			global
		);
		if (persistence != Persistence.CONNECTION) {
	...
	public void receive(ClientEvent ce, ClientContext context) {
		...
		if (ce instanceof SplitfileProgressEvent) {
			...
			synchronized (this) {
				progress = progressPending = new SimpleProgressMessage(
					identifier,
					global,
			...
			handleCompatibilityMode(
				(SplitfileCompatibilityModeEvent) ce,
				context
			);
			return;
			...
			synchronized (this) {
				if (expectedHashes != null) {
					Logger.error(
						this,
						"Got a new ExpectedHashes",
						new Exception("debug")
					);
					return;
				} else {
					progress = this.expectedHashes = new ExpectedHashes(
						event,
						identifier,
						global
					);
			...
			progress = new ExpectedMIME(
				identifier,
				global,
				event.expectedMIMEType
			);
			if ((verbosity & verbosityMask) == 0) return;
			...
			if ((verbosity & verbosityMask) == 0) return;
			progress = new ExpectedDataLength(
				identifier,
				global,
				event.expectedSize
			);
			...
			progress = new EnterFiniteCooldown(
				identifier,
				global,
				event.wakeupTime
			);
	...
	private void handleCompatibilityMode(
		final SplitfileCompatibilityModeEvent ce,
		ClientContext context
	) {
		if (
			persistence == Persistence.FOREVER && context.jobRunner.hasLoaded()
		) {
			try {
	...
	private void innerHandleCompatibilityMode(
		...
		if (
			(verbosity & VERBOSITY_COMPATIBILITY_MODE) != 0
	...
	public String getFailureReason(boolean longDescription) {
		...
		if (longDescription && getFailedMessage.extraDescription != null) s +=
		": " + getFailedMessage.extraDescription;
		return s;
	}
	...
	public boolean canRestart() {
		if (!finished) {
			Logger.minor(
				this,
				"Cannot restart because not finished for " + identifier
			);
			return false;
		}
		if (succeeded) {
			Logger.minor(
				this,
				"Cannot restart because succeeded for " + identifier
			);
			return false;
		}
	...
	public boolean restart(
		...
		synchronized (this) {
			finished = false;
			if (
				persistence == Persistence.FOREVER && getFailedMessage != null
			) {
				if (getFailedMessage.redirectURI != null) {
					redirect = getFailedMessage.redirectURI;
				}
			} else if (getFailedMessage != null) redirect =
				...
				if (succeeded) {
					if (returnType == ReturnType.DIRECT) {
						innerDOS = new DataOutputStream(
							checker.checksumWriterWithLength(
								dos,
	...
	private ClientGet(
		...
		if (magic != CLIENT_DETAIL_MAGIC) throw new StorageFormatException(
			"Bad magic for request"
		);
		int version = dis.readInt();
		if (version != CLIENT_DETAIL_VERSION) throw new StorageFormatException(
			"Bad version " + version
		);
		...
		if (dis.readBoolean()) {
			initialMetadata = BucketTools.restoreFrom(
				dis,
		...
		if (finished) {
			...
			if (succeeded) {
				if (returnType == ReturnType.DIRECT) {
					try {
						...
						try {
							returnBucketDirect = BucketTools.restoreFrom(
								innerDIS,
								context.persistentFG,
								context.persistentFileTracker,
								context.getPersistentMasterSecret()
				...
				try {
					...
					try {
						...
						Logger.error(
							this,
							"Unable to restore reason for failure, restarting request : " +
							e,
							e
						);
						finished = false;
			...
			try {
				...
				try {
					...
					Logger.error(
						this,
						"Unable to restore splitfile, restarting: " + e
					);
				} finally {
					innerDIS.close();
				}
				...
				Logger.error(
					this,
					"Unable to restore splitfile, restarting (checksum failed)"
				);
			}
		}
	...
	private synchronized void writeTransientProgressFields(
		...
		HashResult.write(
			expectedHashes == null ? null : expectedHashes.hashes,
			dos
		);
	}
	...
	protected void innerResume(ClientContext context)
		...
		if (foundDataMimeType == null) this.foundDataMimeType =
			getter.expectedMIME();
	}
