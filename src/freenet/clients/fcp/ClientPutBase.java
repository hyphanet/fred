...
public abstract class ClientPutBase
	...
	private static Map<Integer, UploadFrom> uploadFromByCode = new HashMap<
		Integer,
		UploadFrom
	>();
	...
	public enum UploadFrom { // Codes must be constant at least for migration
		...
		UploadFrom(int code) {
			if (uploadFromByCode.containsKey(code)) throw new Error(
				"Duplicate"
			);
	...
	static FreenetURI checkEmptySSK(
		...
		if (
			"SSK".equals(uri.getKeyType()) &&
			uri.getDocName() == null &&
			uri.getRoutingKey() == null
		) {
			if (filename == null || filename.isEmpty()) filename = "key";
			// SSK@ = use a random SSK.
			InsertableClientSSK key = InsertableClientSSK.createRandom(
				context.random,
				""
			);
	...
	public void onSuccess(BaseClientPutter state) {
		synchronized (this) {
			...
			if (generatedURI == null) Logger.error(
				this,
				"No generated URI in onSuccess() for " + this + " from " + state
			);
		}
	...
	public void onGeneratedURI(FreenetURI uri, BaseClientPutter state) {
		synchronized (this) {
			if (generatedURI != null) {
				if (!uri.equals(generatedURI)) Logger.error(
					this,
					"onGeneratedURI(" +
					uri +
					',' +
					...
					this,
					"onGeneratedURI() twice with same value: " +
					generatedURI +
					" -> " +
					uri
				);
	...
	public void onGeneratedMetadata(Bucket metadata, BaseClientPutter state) {
		...
		synchronized (this) {
			if (generatedURI != null) Logger.error(
				this,
				"Got generated metadata but already have URI on " +
				this +
				" from " +
				state
			);
			if (generatedMetadata != null) {
				Logger.error(
					this,
					"Already got generated metadata from " +
					state +
					" on " +
					this
				);
	...
	public void requestWasRemoved(ClientContext context) {
		...
		if (!finished) {
			synchronized (this) {
				finished = true;
				InsertException cancelled = new InsertException(
					InsertExceptionMode.CANCELLED
				);
				putFailedMessage = new PutFailedMessage(
					cancelled,
					identifier,
					global
				);
	...
	public void receive(final ClientEvent ce, ClientContext context) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Receiving event " + ce + " on " + this
		);
		if (ce instanceof SplitfileProgressEvent) {
			if (
				(verbosity & VERBOSITY_SPLITFILE_PROGRESS) ==
				VERBOSITY_SPLITFILE_PROGRESS
			) {
				SimpleProgressMessage progress = new SimpleProgressMessage(
					identifier,
					global,
					(SplitfileProgressEvent) ce
				);
				trySendProgressMessage(
					progress,
					VERBOSITY_SPLITFILE_PROGRESS,
					null,
					context
			...
			if (
				(verbosity & VERBOSITY_COMPRESSION_START_END) ==
				VERBOSITY_COMPRESSION_START_END
			) {
				StartedCompressionMessage msg = new StartedCompressionMessage(
					identifier,
					global,
			...
			if (
				...
				FinishedCompressionMessage msg = new FinishedCompressionMessage(
					identifier,
					global,
			...
			if (
				(verbosity & VERBOSITY_EXPECTED_HASHES) ==
				VERBOSITY_EXPECTED_HASHES
			) {
				ExpectedHashes msg = new ExpectedHashes(
					(ExpectedHashesEvent) ce,
					identifier,
					global
				);
				trySendProgressMessage(
	...
	public void onFetchable(BaseClientPutter putter) {
		...
		if ((verbosity & VERBOSITY_PUT_FETCHABLE) == VERBOSITY_PUT_FETCHABLE) {
			...
			PutFetchableMessage msg = new PutFetchableMessage(
				identifier,
				global,
				temp
			);
	...
	private void trySendFinalMessage(
		...
		if (msg == null) {
			Logger.error(
				this,
				"Trying to send null message on " + this,
				new Exception("error")
			);
		} else {
			if (
				persistence == Persistence.CONNECTION && handler == null
	...
	private void trySendGeneratedMetadataMessage(
		...
		FCPMessage msg = FCPMessage.withListRequestIdentifier(
			new GeneratedMetadataMessage(identifier, global, metadata),
			listRequestIdentifier
		);
		if (persistence == Persistence.CONNECTION && handler == null) handler =
			origHandler.outputHandler;
	...
	public void sendPendingMessages(
		...
		synchronized (this) {
			...
			msg = FCPMessage.withListRequestIdentifier(
				progressMessage,
				listRequestIdentifier
			);
			fin = finished;
		...
		if (generated) trySendGeneratedURIMessage(
			handler,
			listRequestIdentifier
		);
		if (meta != null) trySendGeneratedMetadataMessage(
			meta,
			handler,
			listRequestIdentifier
		);
	...
	public synchronized String getFailureReason(boolean longDescription) {
		...
		if (longDescription && putFailedMessage.extraDescription != null) s +=
		": " + putFailedMessage.extraDescription;
		return s;
	}
