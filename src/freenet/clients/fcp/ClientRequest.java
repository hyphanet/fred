...
public abstract class ClientRequest implements Serializable {
	...
	public ClientRequest(
		...
		if (persistence == Persistence.CONNECTION) {
			this.origHandler = handler;
			client = null;
			lowLevelClient = new RequestClientBuilder()
				.realTime(realTime)
				.build();
			this.clientName = null;
			this.verbosity = verbosity2;
			...
			origHandler = null;
			if (global) {
				client = persistence == Persistence.FOREVER
			...
			lowLevelClient = client.lowLevelClient(realTime);
			if (lowLevelClient == null) throw new NullPointerException(
	...
	public enum Persistence {
		...
		public static Persistence parseOrThrow(
			...
			try {
				...
				else return Persistence.valueOf(
					persistenceString.toUpperCase()
				);
		...
		public static Persistence getByCode(short persistenceType) {
			if (
				persistenceType < 0 || persistenceType > values().length
	...
	public void modifyRequest(
		...
		if (clientTokenChanged && priorityClassChanged) {
			modifiedMsg = new PersistentRequestModifiedMessage(
				identifier,
				global,
				priorityClass,
				clientToken
	...
	protected ClientRequest(
		...
		if (!copyReq.equals(reqID)) throw new StorageFormatException(
			"Request identifier has changed"
		);
	...
	protected abstract void innerResume(ClientContext context)
		throws ResumeFailedException;
