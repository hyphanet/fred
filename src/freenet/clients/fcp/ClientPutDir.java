...
public class ClientPutDir extends ClientPutBase {
	...
	public ClientPutDir(
		...
		if (logMINOR) Logger.minor(
			this,
			"Putting dir " + identifier + " : " + priorityClass
		);
	}
	...
	public ClientPutDir(
		...
		throws FileNotFoundException, IdentifierCollisionException, MalformedURLException, TooManyFilesInsertException {
		super(
		...
		logMINOR = Logger.shouldLog(LogLevel.MINOR, this);
		this.manifestElements = makeDiskDirManifest(
			dir,
			"",
	...
	public ClientPutDir(
		...
		if (logMINOR) Logger.minor(
			this,
			"Putting data from custom buckets " +
			identifier +
			" : " +
			priorityClass
		);
		...
		for (File f : files) {
			...
			if (f.exists() && f.canRead()) {
				if (f.isFile()) {
					...
					if (!allowUnreadableFiles) throw new FileNotFoundException(
						"Not a file and not a directory : " + f
					);
				}
			} else if (!allowUnreadableFiles) throw new FileNotFoundException(
				"The file does not exist or is unreadable : " + f
			);
		}
	...
	private void makePutter(ClientContext context)
		throws TooManyFilesInsertException {
		putter = new DefaultManifestPutter(
			this,
			manifestElements,
			priorityClass,
			uri,
	...
	protected void freeData() {
		if (logMINOR) Logger.minor(
			this,
			"freeData() on " + this + " persistence type = " + persistence
		);
		synchronized (this) {
		...
		if (logMINOR) Logger.minor(
			this,
			"freeData() more on " + this + " persistence type = " + persistence
		);
	...
	protected FCPMessage persistentTagMessage() {
		...
		if (lowLevelClient == null) Logger.error(
			this,
			"lowLevelClient == null",
			new Exception("error")
		);
		if (putter == null) Logger.error(
			this,
			"putter == null",
			new Exception("error")
		);
	...
	@Override
	public void innerResume(ClientContext context)
