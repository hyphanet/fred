...
public class HTTPRequestImpl implements HTTPRequest {
	...
	private Map<String, HTTPUploadedFileImpl> uploadedFiles = new HashMap<
		String,
		HTTPUploadedFileImpl
	>();
	...
	public HTTPRequestImpl(
		...
		if (data != null) {
			try {
				...
				Logger.error(
					this,
					"Temporary files error ? Could not parse: " + ioe,
					ioe
				);
			}
		}
	...
	private void parseRequestParameters(
		...
		Map<String, List<String>> parameters = parseUriParameters(
			queryString,
			doUrlDecoding
		);
		...
		if (asParts) {
			for (Entry<
				String,
				List<String>
			> parameterValues : parameters.entrySet()) {
	...
	public static String createQueryString(
		...
		for (Entry<
			String,
			List<String>
		> parameter : parameterValues.entrySet()) {
			for (String value : parameter.getValue()) {
				if (queryString.length() > 0) {
					queryString.append('&');
				}
				queryString.append(
					URLEncoder.encode(parameter.getKey(), doUrlEncoding)
				);
				queryString.append('=');
	...
	private void parseMultiPartData() throws IOException {
		...
		try {
			...
			if (boundary.charAt(boundary.length() - 1) == '"') boundary =
			...
			while (is.available() > 0) {
				...
				// chomp headers
				while (
					(line = lis.readLine(200, 200, true)) !=
					/* should be UTF-8 as we told the browser to send UTF-8 */null
				) {
					...
					if (hdrname.equalsIgnoreCase("Content-Disposition")) {
						...
						for (int i = 0; i < valueparts.length; i++) {
							...
							if (fieldname.equalsIgnoreCase("name")) name =
								value;
							else if (
				...
				if (filename != null) uploadedFiles.put(
					name,
					new HTTPUploadedFileImpl(filename, contentType, filedata)
				);
			}
		} finally {
	...
	private String getPartAsLimitedString(Bucket part, int maxLength) {
		return new String(
			getPartAsLimitedBytes(part, maxLength),
			StandardCharsets.UTF_8
		);
	}
	...
	public boolean isIncognito() {
		if (isParameterSet("incognito")) return Boolean.valueOf(
			getParam("incognito")
		);
		return false;
	}
