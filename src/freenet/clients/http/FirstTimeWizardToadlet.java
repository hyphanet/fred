...
public class FirstTimeWizardToadlet extends Toadlet {
    ...
    public enum WIZARD_PRESET { LOW, HIGH }

    public enum WIZARD_STEP {
        WELCOME, BROWSER_WARNING, MISC, OPENNET, SECURITY_NETWORK, SECURITY_PHYSICAL, NAME_SELECTION, DATASTORE_SIZE,
        BANDWIDTH, BANDWIDTH_MONTHLY, BANDWIDTH_RATE, COMPLETE    // Redirects to front page
    }
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (currentStep.equals(WIZARD_STEP.WELCOME)
            ...
            try {
                ...
                StringBuilder msg = new StringBuilder("<html><head><title>").append(title).append(
                                        "</title></head><body><h1>").append(title).append("</h1><pre>");
                ...
                if (internal != null) {
                    msg.append("<h1>").append(NodeL10n.getBase().getString("Toadlet.internalErrorPleaseReport")).append(
                        "</h1>").append("<pre>");
                    sw = new StringWriter();
