...
public class FirstTimeWizardToadlet extends Toadlet {
	...
	public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx) throws ToadletContextClosedException, IOException {
		...
		if(noPassword) {
			if(Logger.shouldLog(LoggerPriority.MINOR, this)) Logger.minor(this, "No password ("+passwd+" should be "+core.formPassword+ ')');
		...
		if(request.isPartSet("security-levels.networkThreatLevel")) {
			...
			if(Logger.shouldLog(LoggerPriority.MINOR, this)) Logger.minor(this, "Old threat level: "+oldThreatLevel+" new threat level: "+newThreatLevel);
			if(newThreatLevel == null) {
				super.writeTemporaryRedirect(ctx, "step1", TOADLET_URL+"?step="+WIZARD_STEP.SECURITY_PHYSICAL);
				return;
			}
