...
public class FirstTimeWizardToadlet extends Toadlet {
	...
	FirstTimeWizardToadlet(
		...
		steps.put(
			WIZARD_STEP.BANDWIDTH_MONTHLY,
			new BANDWIDTH_MONTHLY(core, config)
		);
	...
	private void addWizardConfiguration(Config configuration) {
		SubConfig wizardConfiguration = new SubConfig(
			"firstTimeWizard",
			configuration
		);
	...
	public void handleMethodGET(
		...
		try {
			currentStep = WIZARD_STEP.valueOf(
				request.getParam("step", WIZARD_STEP.WELCOME.toString())
			);
		...
		if (
			currentStep == WIZARD_STEP.BROWSER_WARNING &&
			request.isChrome() &&
			request.isIncognito()
		) {
			super.writeTemporaryRedirect(
				ctx,
				"Skipping unneeded warning",
				persistFields.appendTo(TOADLET_URL + "?step=MISC")
			);
			return;
		} else if (
			currentStep == WIZARD_STEP.MISC && persistFields.isUsingPreset()
		) {
			...
			StringBuilder redirectBase = new StringBuilder(
				TOADLET_URL + "?step="
			);
			...
			super.writeTemporaryRedirect(
				ctx,
				"Skipping to next necessary step",
				redirectBase.toString()
			);
			return;
		} else if (
			currentStep == WIZARD_STEP.SECURITY_NETWORK &&
			!request.isParameterSet("opennet")
		) {
			...
			currentStep == WIZARD_STEP.NAME_SELECTION &&
			core.node.isOpennetEnabled()
		) {
			//Skip node name selection if not in darknet mode.
			super.writeTemporaryRedirect(
				ctx,
				"Skip name selection",
				persistFields.appendTo(
					stepURL(WIZARD_STEP.DATASTORE_SIZE.name())
				)
			);
			return;
	...
	public void handleMethodPOST(
		...
		try {
			...
			currentStep = currentValue.isEmpty()
				? WIZARD_STEP.WELCOME
				: WIZARD_STEP.valueOf(currentValue);
		} catch (IllegalArgumentException e) {
		...
		if (
			currentStep.equals(WIZARD_STEP.WELCOME) &&
			...
			StringBuilder redirectTo = new StringBuilder(
				TOADLET_URL + "?step=BROWSER_WARNING&incognito="
			);
			...
			if (request.isPartSet("presetLow")) {
				...
				stepSECURITY_NETWORK.setThreatLevel(
					SecurityLevels.NETWORK_THREAT_LEVEL.LOW
				);
			...
			if (request.isPartSet("singlestep")) {
				...
				redirectTarget = getPreviousStep(
					currentStep,
					persistFields.preset
				).name();
			}
		} else {
			try {
				...
				if (
					...
					!redirectTarget.startsWith(currentStep.name())
				) {
					redirectTarget = WIZARD_STEP.COMPLETE.name();
				}
				...
				if (currentStep == WIZARD_STEP.OPENNET) {
					try {
						HTTPRequest newRequest = new HTTPRequestImpl(
							new URI(stepURL(redirectTarget)),
							"GET"
						);
						//Only continue if a value for opennet has been selected.
						if (newRequest.isPartSet("opennet")) {
							redirectTarget =
								WIZARD_STEP.SECURITY_NETWORK.name();
							persistFields = new PersistFields(
								persistFields.preset,
								newRequest
							);
						}
						...
						Logger.error(
							this,
							"Unexpected invalid query string from OPENNET step! " +
							e,
							e
						);
				...
				if (e.getMessage().equals("cantWriteNewMasterKeysFile")) {
					...
					title = NodeL10n.getBase()
						.getString(
							"SecurityLevels.cantWriteNewMasterKeysFileTitle"
						);
				} else {
					//Some other error.
					title = NodeL10n.getBase()
						.getString("Toadlet.internalErrorPleaseReport");
				}

				//Very loud error message, with descriptive title and header if possible.
				StringBuilder msg = new StringBuilder("<html><head><title>")
					.append(title)
	...
	public static WIZARD_STEP getPreviousStep(
		WIZARD_STEP currentStep,
		WIZARD_PRESET preset
	) {
