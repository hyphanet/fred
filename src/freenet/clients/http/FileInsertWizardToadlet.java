...
public class FileInsertWizardToadlet
	...
	protected FileInsertWizardToadlet(
	...
	public void handleMethodGET(
		...
		contentNode.addChild(
			createInsertBox(pageMaker, ctx, ctx.isAdvancedModeEnabled())
		);
		if (ctx.isAdvancedModeEnabled()) contentNode.addChild(
			createFilterBox(pageMaker, ctx)
		);
	...
	private HTMLNode createInsertBox(
		...
		HTMLNode insertForm = ctx.addFormChild(
			insertContent,
		...
		HTMLNode input = insertForm.addChild(
			"input",
		...
		if (isAdvancedModeEnabled) insertForm.addChild(
			"#",
			" " + l10n("insertCanonicalAdvanced")
		);
		insertForm.addChild("br");
		input = insertForm.addChild(
			"input",
			new String[] { "type", "name", "value", "id" },
		...
		if (isAdvancedModeEnabled) {
			...
			insertForm.addChild(
				...
				' ' +
				NodeL10n.getBase()
					.getString("QueueToadlet.insertFileCompressLabel")
			);
		} else {
			insertForm.addChild(
				"input",
		...
		if (isAdvancedModeEnabled) {
			...
			insertForm.addChild(
				"#",
				NodeL10n.getBase().getString("QueueToadlet.compatModeLabel") +
				": "
			);
			HTMLNode select = insertForm.addChild(
				"select",
				"name",
				"compatibilityMode"
			);
			for (CompatibilityMode mode : InsertContext.CompatibilityMode.values()) {
				...
				// FIXME l10n???
				HTMLNode option = select.addChild(
					"option",
					"value",
					mode.name(),
					NodeL10n.getBase()
						.getString(
							"InsertContext.CompatibilityMode." + mode.name()
						)
				);
				if (
					mode == CompatibilityMode.COMPAT_DEFAULT
		...
		if (ctx.isAllowedFullAccess()) {
			insertForm.addChild(
				"#",
				NodeL10n.getBase()
					.getString("QueueToadlet.insertFileBrowseLabel") +
				": "
			);
			insertForm.addChild(
				"input",
				...
				new String[] {
					...
					"insert-local",
					NodeL10n.getBase()
						.getString("QueueToadlet.insertFileBrowseButton") +
					"...",
				}
			);
			insertForm.addChild("br");
		...
		insertForm.addChild(
			...
			new String[] {
				...
				"insert",
				NodeL10n.getBase()
					.getString("QueueToadlet.insertFileInsertFileLabel"),
			}
		);
		insertForm.addChild("#", " \u00a0 ");
	...
	private HTMLNode createFilterBox(PageMaker pageMaker, ToadletContext ctx) {
		...
		insertForm.addChild(
			"#",
			ContentFilterToadlet.l10n("mimeTypeLabel") + ": "
		);
		insertForm.addChild(
			"input",
			new String[] { "type", "name", "value" },
	...
	String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("FileInsertWizardToadlet." + key, pattern, value);
	}
