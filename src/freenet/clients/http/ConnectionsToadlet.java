...
public abstract class ConnectionsToadlet extends Toadlet {
	...
	protected class ComparatorByStatus implements Comparator<PeerNodeStatus> {
		...
		public int compare(
			...
			if (!isSet) {
				...
				if (statusDifference != 0) result = (statusDifference < 0
						? -1
						: 1);
		...
		protected int lastResortCompare(
			PeerNodeStatus firstNode,
			PeerNodeStatus secondNode
		) {
			return compareLocations(firstNode, secondNode);
	...
	abstract class SimpleColumn {
		...
		protected abstract void drawColumn(
			HTMLNode peerRow,
	...
	public void handleMethodGET(
		...
		if (path.endsWith("myref.fref")) {
			...
			MultiValueTable<String, String> extraHeaders = new MultiValueTable<
				String,
				String
			>();
			...
			writeReply(
				ctx,
				200,
				"application/x-freenet-reference",
				"OK",
				extraHeaders,
				noderefString
			);
		...
		int numberOfClockProblem = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_CLOCK_PROBLEM
		);
		int numberOfConnError = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
		...
		int numberOfRoutingDisabled = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(getPageTitle(titleCountString), ctx);
		...
		if (peerNodeStatuses.length > 0) {
			if (advancedMode) {
				/* node status values */
				long nodeUptimeSeconds = SECONDS.convert(
				...
				int networkSizeEstimateSession = stats.getDarknetSizeEstimate(
					-1
				);
				int networkSizeEstimateRecent = 0;
				if (nodeUptimeSeconds > HOURS.toSeconds(48)) {
					networkSizeEstimateRecent = stats.getDarknetSizeEstimate(
				...
				overviewList.addChild(
					"li",
					"nodeUptime:\u00a0" + nodeUptimeString
				);
				overviewList.addChild(
					"li",
					"routingMissDistanceLocal:\u00a0" +
					fix4.format(routingMissDistanceLocal)
				);
				overviewList.addChild(
					"li",
					"routingMissDistanceRemote:\u00a0" +
					fix4.format(routingMissDistanceRemote)
				);
				overviewList.addChild(
					"li",
					"routingMissDistanceOverall:\u00a0" +
					fix4.format(routingMissDistanceOverall)
				);
				overviewList.addChild(
					"li",
					"routingMissDistanceBulk:\u00a0" +
					fix4.format(routingMissDistanceBulk)
				);
				overviewList.addChild(
					"li",
					"routingMissDistanceRT:\u00a0" +
					fix4.format(routingMissDistanceRT)
				);
				overviewList.addChild(
					"li",
				...
				overviewList.addChild(
					"li",
					"pInstantReject:\u00a0" +
					fix1.format(stats.pRejectIncomingInstantly())
				);
				...
				HTMLNode activityList = StatisticsToadlet.drawActivity(
					activityInfoboxContent,
					node
				);
				if (advancedMode && (activityList != null)) {
					...
					StatisticsToadlet.drawBandwidth(
						activityList,
						node,
						nodeUptimeSeconds,
						advancedMode
				...
				if (advancedMode) {
					...
					HTMLNode title = backoffReasonInfobox.addChild(
						"div",
						"class",
						"infobox-header",
					...
					if (routingBackoffReasons.length == 0) {
						backoffReasonContent.addChild(
							"#",
							NodeL10n.getBase()
								.getString("StatisticsToadlet.notBackedOff")
						);
					} else {
						HTMLNode reasonList = backoffReasonContent.addChild(
							"ul"
						);
					...
					backoffReasonInfobox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					title = backoffReasonInfobox.addChild(
						"div",
						"class",
						"infobox-header",
						"Peer backoff reasons (bulk)"
					);
					backoffReasonContent = backoffReasonInfobox.addChild(
						"div",
						"class",
						"infobox-content"
					);
			...
			HTMLNode peerTableInfobox = contentNode.addChild(
				"div",
				"class",
				"infobox infobox-normal"
			);
			HTMLNode peerTableInfoboxHeader = peerTableInfobox.addChild(
				"div",
				"class",
				"infobox-header"
			);
			...
			if (advancedMode) {
				if (!path.endsWith("displaymessagetypes.html")) {
					...
					peerTableInfoboxHeader.addChild(
						"a",
						"href",
						"displaymessagetypes.html",
						l10n("bracketedMoreDetailed")
					);
				}
			}
			HTMLNode peerTableInfoboxContent = peerTableInfobox.addChild(
				"div",
				"class",
				"infobox-content"
			);
			...
			if (!isOpennet()) {
				...
				myName.addChild(
					...
					NodeL10n.getBase()
						.getString(
							"DarknetConnectionsToadlet.myName",
							"name",
							node.getMyName()
						)
				);
				...
				myName
					...
					.addChild(
						...
						"/config/node#name",
						NodeL10n.getBase()
							.getString("DarknetConnectionsToadlet.changeMyName")
					);
				myName.addChild("span", "]");
			}
			...
			if (peerNodeStatuses.length == 0) {
				...
				if (enablePeerActions) {
					peerForm = ctx.addFormChild(
						peerTableInfoboxContent,
						".",
						"peersForm"
					);
					peerTable = peerForm.addChild(
						"table",
						"class",
						"darknet_connections"
					);
				} else {
					peerTable = peerTableInfoboxContent.addChild(
						"table",
						"class",
						"darknet_connections"
					);
				...
				if (hasNameColumn()) peerTableHeaderRow
					.addChild("th")
					...
					.addChild(
						...
						new String[] {
							l10n("nameClickToMessage"),
							"border-bottom: 1px dotted; cursor: help;",
						},
						l10n("nameTitle")
					);
				if (hasTrustColumn()) peerTableHeaderRow
					.addChild("th")
				...
				if (hasVisibilityColumn()) peerTableHeaderRow
					.addChild("th")
					.addChild("a", "href", sortString(isReversed, "trust"))
					.addChild(
						"span",
				...
				if (advancedMode) {
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "location")
						)
					...
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "backoffRT")
						)
						.addChild(
							"span",
							new String[] { "title", "style" },
					...
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "backoffBulk")
						)
						.addChild(
							"span",
							new String[] { "title", "style" },
					...
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "overload_p")
						)
						.addChild(
							"span",
							new String[] { "title", "style" },
				...
				if (hasPrivateNoteColumn()) peerTableHeaderRow
					.addChild("th")
				...
				if (advancedMode) {
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "time_routable")
						)
					...
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "selection_percentage")
						)
						.addChild("#", "%\u00a0Selection");
					peerTableHeaderRow
						.addChild("th")
						.addChild(
							"a",
							"href",
							sortString(isReversed, "total_traffic")
						)
						.addChild(
							"#",
					...
					peerTableHeaderRow
						...
						.addChild(
							"a",
							"href",
							sortString(isReversed, "time_delta")
						)
						.addChild("#", "Time\u00a0Delta");
					peerTableHeaderRow
						.addChild("th")
				...
				if (endCols != null) {
					for (SimpleColumn col : endCols) {
						...
						if (sortString != null) header = header.addChild(
							"a",
							"href",
							sortString(isReversed, sortString)
						);
						header.addChild(
							"span",
							new String[] { "title", "style" },
							new String[] {
								NodeL10n.getBase()
									.getString(col.getExplanationKey()),
								"border-bottom: 1px dotted; cursor: help;",
							},
				...
				PeerNodeStatus[] allPeerNodeStatuses =
					node.peers.getPeerNodeStatuses(true);
			...
			if (advancedMode) {
					...
					for (PeerNodeStatus peerNodeStatus : peerNodeStatuses) {
						...
						if (peersLoc != null) {
							for (double location : peersLoc) {
								...
								for (
									i = 0;
									i < max && locations.get(i) < location;
									i++
								);
								...
								if (
									i < max &&
									locations.get(i).doubleValue() == location
								) {
				...
				peerTableInfoboxContent.addChild(
					"b",
					l10n(
						"secondDegreeConnectionsCountTitle",
						"count",
				...
				if (!showTrivialFoafConnections) {
					peerTableInfoboxContent.addChild(
						"i",
						l10n(
							"secondDegreeTrivialHiddenCount",
							"count",
				...
				HTMLNode foafTable = peerTableInfoboxContent.addChild(
					"table",
					"class",
					"darknet_connections"
				...
				for (int i = 0; i < max; i++) {
					...
					if (
						peersWithFriend.size() == 1 &&
						!showTrivialFoafConnections &&
						!isTransitivePeer
					) continue;
					foafRow = foafTable.addChild("tr");
					{
						if (isTransitivePeer) {
							foafRow
								.addChild("td")
								.addChild("b", String.valueOf(location));
						} else {
							foafRow.addChild("td", String.valueOf(location));
						}
						foafRow.addChild(
							"td",
							String.valueOf(peersWithFriend.size())
						);
						HTMLNode locationCell = foafRow.addChild(
							"td",
							"class",
							"peer-location"
						);
						for (PeerNodeStatus peerNodeStatus : peersWithFriend) {
							String address =
								((peerNodeStatus.getPeerAddress() != null)
										? (peerNodeStatus.getPeerAddress() +
											':' +
											peerNodeStatus.getPeerPort())
										: (l10n("unknownAddress")));
	...
	public void handleMethodPOST(
		...
		if (request.isPartSet("add")) {
			...
			String reftext = request.getPartAsStringFailsafe(
				"ref",
				Integer.MAX_VALUE
			);
			reftext = reftext.trim();
			if (reftext.length() < 200) {
				reftext = request.getPartAsStringFailsafe(
					"reffile",
					Integer.MAX_VALUE
				);
				reftext = reftext.trim();
			...
			if (!isOpennet()) privateComment = request
				.getPartAsStringFailsafe("peerPrivateNote", 250)
				.trim();

			if (
				Boolean.parseBoolean(
					request.getPartAsStringFailsafe("peers-offers-files", 5)
				)
			) {
				File[] files = core.node
					.runDir()
					.file("peers-offers")
					.listFiles();
				if (files != null && files.length > 0) {
					...
					for (final File file : files) {
						if (file.isFile()) {
							...
							if (
								filename.endsWith(".fref")
							) peersOffersFilesContent.append(
								FileUtil.readUTF(file)
							);
						}
					}
			...
			if (trustS != null && !trustS.isEmpty()) trust =
				FRIEND_TRUST.valueOf(trustS);

			String visibilityS = request.getPartAsStringFailsafe(
				"visibility",
				10
			);
			FRIEND_VISIBILITY visibility = null;
			if (visibilityS != null && !visibilityS.isEmpty()) visibility =
				FRIEND_VISIBILITY.valueOf(visibilityS);
			...
			if (urltext.length() > 0) {
				...
				try {
					try {
						...
						ref = AddPeer.getReferenceFromFreenetURI(
							refUri,
							client
						);
						...
						Logger.warning(
							this,
							"Url cannot be used as Freenet URI, trying to fetch as URL: " +
							urltext
						);
			...
			for (int i = 0; i < nodesToAdd.length; i++) {
				...
				PeerAdditionReturnCodes result = addNewNode(
					nodesToAdd[i].trim().concat("\nEnd"),
					privateComment,
					trust,
					visibility
				);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("reportOfNodeAddition"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			...
			detailedStatusBox
				...
				.addChildren(
					new HTMLNode[] {
						new HTMLNode("th", l10n("resultName")),
						new HTMLNode("th", l10n("numOfResults")),
					}
				);
			HTMLNode statusBoxTable = detailedStatusBox.addChild(
				new HTMLNode("tbody")
			);
					...
					statusBoxTable
						.addChild(
							new HTMLNode(
								...
								"style",
								"color:" +
								(returnCode == PeerAdditionReturnCodes.OK
										? "green"
										: "red")
							)
						)
			...
			HTMLNode infoboxContent = ctx
				...
				.getInfobox(
					"infobox",
					l10n("reportOfNodeAddition"),
					contentNode,
					"node-added",
					true
				);
			infoboxContent.addChild(detailedStatusBox);
			if (!isOpennet()) infoboxContent
				.addChild("p")
				.addChild("a", "href", "/addfriend/", l10n("addAnotherFriend"));
			infoboxContent
				.addChild("p")
				.addChild(
					"a",
	...
	private PeerAdditionReturnCodes addNewNode(
		...
		try {
			...
			if (!fs.getEndMarker().endsWith("End")) {
				Logger.error(
					this,
					"Trying to add noderef with end marker \"" +
					fs.getEndMarker() +
					"\""
				);
			...
			Logger.error(
				this,
				"IOException adding reference :" + e.getMessage(),
				e
			);
			...
			Logger.error(
				this,
				"Internal error adding reference :" + t.getMessage(),
				t
			);
		...
		try {
			if (isOpennet()) {
				...
				((DarknetPeerNode) pn).setPrivateDarknetCommentNote(
						privateComment
					);
			}
		...
		if (
			Arrays.equals(pn.peerECDSAPubKeyHash, node.getDarknetPubKeyHash())
		) {
	...
	private static SimpleFieldSet parseNoderefLiberally(String nodeReference)
		throws IOException {
	 ...
	 */
	protected abstract void drawPeerActionSelectBox(
		HTMLNode peerForm,
		boolean advancedModeEnabled
	);

	protected abstract boolean shouldDrawNoderefBox(
		boolean advancedModeEnabled
	);
	...
	void drawNoderefBox(HTMLNode contentNode, SimpleFieldSet fs) {
		HTMLNode referenceInfobox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		HTMLNode headerReferenceInfobox = referenceInfobox.addChild(
			"div",
			"class",
			"infobox-header"
		);
		...
		HTMLNode referenceInfoboxContent = referenceInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (!isOpennet()) {
			...
			myIps.addChild(
				...
				NodeL10n.getBase()
					.getString(
						"DarknetConnectionsToadlet.myIps",
						"ips",
						fs.get("physical.udp")
					)
			);
	...
	protected static void drawAddPeerBox(
		...
		HTMLNode peerAdditionInfobox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		...
		HTMLNode peerAdditionContent = peerAdditionInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode peerAdditionForm = ctx.addFormChild(
			peerAdditionContent,
			formTarget,
			"addPeerForm"
		);
		...
		if (!isOpennet) {
			...
			peerAdditionForm.addChild(
				new PeerVisibilityInputForAddPeerBoxNode()
			);
		}
	...
	private void drawRow(
		...
		if (totalSelectionRate > 0) peerSelectionPercentage =
			(int) ((selectionRate * 100) / totalSelectionRate);
		 ...
		 */
		final String key =
			"ConnectionsToadlet.nodeStatus." + statusString.replace(' ', '_');
		peerRow
		...
		IPConverter ipc = IPConverter.getInstance(
			NodeFile.IPv4ToCountry.getFile(node)
		);
		...
		) { // Don't draw attention to a version problem if NEVER CONNECTED
			peerRow
		...
		if (advancedModeEnabled) {
			HTMLNode locationNode = peerRow.addChild(
				"td",
				"class",
				"peer-location"
			);
			locationNode.addChild(
				"b",
		...
		if (advancedModeEnabled) {
			// backoff column
			HTMLNode backoffCell = peerRow.addChild(
				"td",
				"class",
				"peer-backoff"
			);
			backoffCell.addChild(
				"#",
				fix1.format(peerNodeStatus.getBackedOffPercent(true))
			);
			int backoff = (int) (Math.max(
					peerNodeStatus.getRoutingBackedOffUntil(true) - now,
					0
				));
			...
			backoffCell.addChild(
				"#",
				fix1.format(peerNodeStatus.getBackedOffPercent(false))
			);
			backoff = (int) (Math.max(
					peerNodeStatus.getRoutingBackedOffUntil(false) - now,
					0
				));
			// Don't list the backoff as zero before it's actually zero
			if ((backoff > 0) && (backoff < 1000)) {
			...
			HTMLNode pRejectCell = peerRow.addChild(
				"td",
				"class",
				"peer-backoff"
			); // FIXME
		...
		if (
			...
			(now - idle) > (2 * 7 * 24 * 60 * 60 * (long) 1000)
		) { // 2 weeks
			peerRow
		...
		if (hasPrivateNoteColumn()) drawPrivateNoteColumn(
			peerRow,
			peerNodeStatus,
			fProxyJavascriptEnabled
		);
		...
		if (advancedModeEnabled) {
			...
			peerRow
				...
				.addChild(
					"#",
					fix1.format(
						peerNodeStatus.getPercentTimeRoutableConnection()
					)
				);
			// selection stats
			peerRow
			...
			peerRow
				...
				.addChild(
					"#",
					SizeUtil.formatSize(
						peerNodeStatus.getMessageQueueLengthBytes()
					) +
					":" +
					TimeUtil.formatTime(
			...
			IncomingLoadSummaryStats loadStatsBulk =
				peerNodeStatus.incomingLoadStatsBulk;
			if (loadStatsBulk == null) peerRow.addChild(
				"td",
				"class",
				"peer-idle"
				/* FIXME */
				...
				.addChild(
					...
					"/" +
					SizeUtil.formatSize(
						loadStatsBulk.othersUsedCapacityOutputBytes
					) +
					"/" +
					SizeUtil.formatSize(loadStatsBulk.peerCapacityOutputBytes) +
					"/" +
					SizeUtil.formatSize(
						loadStatsBulk.totalCapacityOutputBytes
					) +
					":in:" +
					SizeUtil.formatSize(loadStatsBulk.usedCapacityInputBytes) +
					"/" +
					SizeUtil.formatSize(
						loadStatsBulk.othersUsedCapacityInputBytes
					) +
					"/" +
			...
			IncomingLoadSummaryStats loadStatsRT =
				peerNodeStatus.incomingLoadStatsRealTime;
			if (loadStatsRT == null) peerRow.addChild(
				"td",
				"class",
				"peer-idle"
				/* FIXME */
				...
				.addChild(
					...
					"/" +
					SizeUtil.formatSize(
						loadStatsRT.othersUsedCapacityOutputBytes
					) +
					"/" +
					...
					"/" +
					SizeUtil.formatSize(
						loadStatsRT.othersUsedCapacityInputBytes
					) +
					"/" +
	...
	protected void drawTrustColumn(
		HTMLNode peerRow,
		PeerNodeStatus peerNodeStatus
	) {
		// Do nothing
	...
	private void drawMessageTypes(
		HTMLNode peerTable,
		PeerNodeStatus peerNodeStatus
	) {
		HTMLNode messageCountRow = peerTable.addChild(
			"tr",
			"class",
			"message-status"
		);
		messageCountRow.addChild("td", "colspan", "2");
		HTMLNode messageCountCell = messageCountRow.addChild(
			"td",
			"colspan",
			"9"
		); // = total table row width - 2 from above colspan
		HTMLNode messageCountTable = messageCountCell.addChild(
			"table",
			"class",
			"message-count"
		);
		...
		for (Map.Entry<String, Long> entry : peerNodeStatus
			.getLocalMessagesReceived()
			.entrySet()) {
			...
			messageCounts.put(
				messageName,
				new Long[] { messageCount, Long.valueOf(0) }
			);
		}
		for (Map.Entry<String, Long> entry : peerNodeStatus
			.getLocalMessagesSent()
			.entrySet()) {
			String messageName = entry.getKey();
			Long messageCount = entry.getValue();
			...
			if (existingCounts == null) {
				messageCounts.put(
					messageName,
					new Long[] { Long.valueOf(0), messageCount }
				);
			} else {
	...
	private static String l10n(String string) {
		return NodeL10n.getBase()
			.getString("DarknetConnectionsToadlet." + string);
	}

	private static String l10n(String string, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("DarknetConnectionsToadlet." + string, pattern, value);
	}

	private String sortString(boolean isReversed, String type) {
		return (
			isReversed ? ("?sortBy=" + type) : ("?sortBy=" + type + "&reversed")
		);
	}
	...
	protected void sendErrorPage(
		...
		HTMLNode infoboxContent = ctx
			.getPageMaker()
			.getInfobox("infobox-error", desc, contentNode, null, true);
