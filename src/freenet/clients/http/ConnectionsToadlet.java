...
public abstract class ConnectionsToadlet extends Toadlet {
		...
		if(peerNodeStatuses.length>0){
			...
			if (peerNodeStatuses.length == 0) {
				...
				if(advancedMode) {
					...
					peerTableHeaderRow.addChild("th").addChild("span", new String[] { "title", "style" }, new String[] { "How much of our per-peer capacity the peer is using", "border-bottom: 1px dotted; cursor: help;" }, "Local\u00a0Capacity");
					peerTableHeaderRow.addChild("th", "Transmit\u00a0Queue");
					peerTableHeaderRow.addChild("th").addChild("span", new String[] { "title", "style" }, new String[] { "How much of the peer's per-peer capacity we are using (for bulk requests)", "border-bottom: 1px dotted; cursor: help;" }, "Peer\u00a0Capacity\u00a0Bulk");
					peerTableHeaderRow.addChild("th").addChild("span", new String[] { "title", "style" }, new String[] { "How much of the peer's per-peer capacity we are using (for realtime requests)", "border-bottom: 1px dotted; cursor: help;" }, "Peer\u00a0Capacity\u00a0Realtime");
	...
	private void drawRow(HTMLNode peerTable, PeerNodeStatus peerNodeStatus, boolean advancedModeEnabled, boolean fProxyJavascriptEnabled, long now, String path, boolean enablePeerActions, SimpleColumn[] endCols, boolean drawMessageTypes, double totalSelectionRate, DecimalFormat fix1) {
		...
		if(advancedModeEnabled) {
			...
			// our capacity used by the peer
			peerRow.addChild("td", "class", "peer-idle" /* FIXME */).addChild("#", fix3p1pct.format(peerNodeStatus.localCapacityUsedFraction())+" : "+SizeUtil.formatSize(peerNodeStatus.localCapacityUsed())+" / "+SizeUtil.formatSize(peerNodeStatus.localCapacityPeerLimit()));
			...
			if(loadStatsBulk == null)
				...
				peerRow.addChild("td", "class", "peer-idle" /* FIXME */).addChild("#", this.fix3p1pct.format(loadStatsBulk.outputUsageFraction())+" : "+loadStatsBulk.runningRequestsTotal+"reqs:out:"+SizeUtil.formatSize(loadStatsBulk.usedCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.othersUsedCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.peerCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.totalCapacityOutputBytes)+":in:"+SizeUtil.formatSize(loadStatsBulk.usedCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.othersUsedCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.peerCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsBulk.totalCapacityInputBytes)+"("+fix3p1pct.format(loadStatsBulk.outputUsageFractionOverall())+")");
			IncomingLoadSummaryStats loadStatsRT = peerNodeStatus.incomingLoadStatsRealTime;
			if(loadStatsRT == null)
				peerRow.addChild("td", "class", "peer-idle" /* FIXME */);
			else
				peerRow.addChild("td", "class", "peer-idle" /* FIXME */).addChild("#", this.fix3p1pct.format(loadStatsRT.outputUsageFraction())+" : "+loadStatsRT.runningRequestsTotal+"reqs:out:"+SizeUtil.formatSize(loadStatsRT.usedCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.othersUsedCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.peerCapacityOutputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.totalCapacityOutputBytes)+":in:"+SizeUtil.formatSize(loadStatsRT.usedCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.othersUsedCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.peerCapacityInputBytes)+"/"+SizeUtil.formatSize(loadStatsRT.totalCapacityInputBytes)+"("+fix3p1pct.format(loadStatsRT.outputUsageFractionOverall())+")");
		}
