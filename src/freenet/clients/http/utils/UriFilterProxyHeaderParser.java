...
public class UriFilterProxyHeaderParser {
	...
	public static SchemeAndHostWithPort parse(
		...
		Set<String> safeProtocols = new HashSet<>(
			Arrays.asList("http", "https")
		);

		List<String> bindToHosts = Arrays.stream(
			fProxyBindToConfig.getValueString().split(",")
		)
		...
		String protocol = headers.containsKey("x-forwarded-proto")
			...
			: uriScheme != null && !uriScheme.trim().isEmpty()
				? uriScheme
				: "http";
		String host = headers.containsKey("x-forwarded-host")
			? headers.get("x-forwarded-host")
			: uriHost != null && !uriHost.trim().isEmpty()
				? uriHost
				: headers.get("host");
		// check allow list
