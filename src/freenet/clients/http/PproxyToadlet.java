...
public class PproxyToadlet extends Toadlet {
	...
	private static final long MAX_THREADED_UNLOAD_WAIT_TIME = SECONDS.toMillis(
		60
	);
	...
	public void handleMethodPOST(
		...
		if (path.startsWith("plugins/")) path = path.substring(
			"plugins/".length()
		);
		...
		if (path.length() > 0) {
			...
			try {
				...
				sendErrorPage(
					ctx,
					NotFoundPluginHTTPException.code,
					e.message,
					e.location
				);
				...
				sendErrorPage(
					ctx,
					AccessDeniedPluginHTTPException.code,
					e.message,
					e.location
				);
				...
				MultiValueTable<String, String> head = new MultiValueTable<
					String,
					String
				>();
				head.put(
				...
				ctx.sendReplyHeaders(
					DownloadPluginHTTPException.CODE,
					"Found",
					head,
					e.mimeType,
					e.data.length
			...
			if (request.isPartSet("submit-official")) {
				final String pluginName = request.getPartAsStringFailsafe(
					"plugin-name",
					40
				);
			...
			if (request.isPartSet("submit-other")) {
				...
				final boolean fileonly =
					"on".equalsIgnoreCase(
							request.getPartAsStringFailsafe("fileonly", 20)
						);
			...
			if (
				...
				String pluginThreadName = request.getPartAsStringFailsafe(
					"unloadconfirm",
					MAX_PLUGIN_NAME_LENGTH
				);
				String pluginSpecification = getPluginSpecification(
					pm,
					pluginThreadName
				);
				pm.killPlugin(
			...
			if (
				request
					.getPartAsStringFailsafe("unload", MAX_PLUGIN_NAME_LENGTH)
					.length() >
				0
			) {
				PageNode page = pageMaker.getPageNode(l10n("plugins"), ctx);
				...
				HTMLNode unloadForm = ctx.addFormChild(
					infoboxContent,
					"/plugins/",
				...
				request
					.getPartAsStringFailsafe("reload", MAX_PLUGIN_NAME_LENGTH)
					.length() >
				0
			) {
				PageNode page = pageMaker.getPageNode(l10n("plugins"), ctx);
				HTMLNode pageNode = page.outer;
				HTMLNode contentNode = page.content;
				HTMLNode reloadContent = pageMaker.getInfobox(
					"infobox infobox-query",
				...
				HTMLNode reloadForm = ctx.addFormChild(
					reloadContent,
					"/plugins/",
					"reloadPluginConfirmForm"
				);
				String pluginIdentifier = request.getPartAsStringFailsafe(
					"reload",
					MAX_PLUGIN_NAME_LENGTH
				);
				reloadForm.addChild(
				...
				request
					.getPartAsStringFailsafe("update", MAX_PLUGIN_NAME_LENGTH)
					.length() >
				0
			) {
				// Deploy the plugin update
				final String pluginFilename = request.getPartAsStringFailsafe(
					"update",
					MAX_PLUGIN_NAME_LENGTH
				);
	 ...
	 *         no plugin was found
	 */
	private String getPluginSpecification(
		PluginManager pluginManager,
	...
	public void handleMethodGET(
		...
		try {
			if (path.isEmpty()) {
				...
				Iterator<PluginProgress> loadingPlugins = pm
					.getStartingPlugins()
					.iterator();

				PageNode page = ctx
					.getPageMaker()
					.getPageNode(l10n("plugins"), ctx);
				...
				for (PluginInfoWrapper pluginInfoWrapper : pm.getPlugins()) {
					...
					String shortPluginName = pluginName.substring(
						pluginName.lastIndexOf('.') + 1
					);
					...
					if (
						shortPluginName.equals("FreemailPlugin")
					) shortPluginName = "Freemail"; // DOH!

					availablePlugins.remove(
						pm.isOfficialPlugin(shortPluginName)
					);
				}
				...
				SortedMap<
					String,
					List<OfficialPluginDescription>
				> groupedAvailablePlugins = new TreeMap<
					String,
					List<OfficialPluginDescription>
				>();
				for (OfficialPluginDescription pluginDescription : availablePlugins) {
					...
					String translatedGroup = l10n(
						"pluginGroup." + pluginDescription.group
					);
					if (!groupedAvailablePlugins.containsKey(translatedGroup)) {
						groupedAvailablePlugins.put(
							translatedGroup,
							new ArrayList<OfficialPluginDescription>()
						);
					}
				...
				> pluginDescriptions : groupedAvailablePlugins.values()) {
					Collections.sort(
						pluginDescriptions,
						new Comparator<OfficialPluginDescription>() {
							...
							@Override
							public int compare(
								OfficialPluginDescription o1,
								OfficialPluginDescription o2
							) {
	 ...
	 */
	private void showStartingPlugins(
		PluginManager pluginManager,
		...
		if (!startingPlugins.isEmpty()) {
			HTMLNode startingPluginsBox = contentNode.addChild(
				"div",
				"class",
				"infobox infobox-normal"
			);
			startingPluginsBox.addChild(
				"div",
				"class",
				"infobox-header",
				l10n("startingPluginsTitle")
			);
			HTMLNode startingPluginsContent = startingPluginsBox.addChild(
				"div",
				"class",
				"infobox-content"
			);
			HTMLNode startingPluginsTable = startingPluginsContent.addChild(
				"table"
			);
			HTMLNode startingPluginsHeader = startingPluginsTable.addChild(
				"tr"
			);
			...
			while (startingPluginsIterator.hasNext()) {
				...
				HTMLNode startingPluginsRow = startingPluginsTable.addChild(
					"tr"
				);
				startingPluginsRow.addChild(
					"td",
					pluginProgress.getLocalisedPluginName()
				);
	...
	private void showPluginList(
		...
		if (pm.getPlugins().isEmpty()) {
			...
			HTMLNode pluginTable = infoboxContent.addChild(
				"table",
				"class",
				"plugins"
			);
			...
			while (it.hasNext()) {
				...
				if (advancedMode) pluginRow.addChild(
					"td",
					pi.getPluginClassName()
				);
				long ver = pi.getPluginLongVersion();
				if (ver != -1) pluginRow.addChild(
					"td",
				...
				if (advancedMode) {
					...
					pluginRow.addChild(
						"td",
						new Date(pi.getStarted()).toString()
					);
				}
				if (pi.isStopping()) {
	...
	private void showOfficialPluginLoader(
		...
		HTMLNode addOfficialPluginBox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		addOfficialPluginBox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("loadOfficialPlugin")
		);
		HTMLNode addOfficialPluginContent = addOfficialPluginBox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode addOfficialForm = toadletContext.addFormChild(
			addOfficialPluginContent,
			".",
		...
		for (Entry<
			String,
			List<OfficialPluginDescription>
		> groupPlugins : availablePlugins.entrySet()) {
	...
	private void addPluginGroupForLoading(
		...
		HTMLNode pluginGroupNode = addOfficialForm.addChild(
			"div",
			"class",
			"plugin-group"
		);
		...
		for (OfficialPluginDescription pluginDescription : notLoadedPlugins) {
			...
			HTMLNode pluginNode = pluginGroupNode.addChild(
				"div",
				"class",
				"plugin"
			);
			HTMLNode option = pluginNode.addChild(
				"input",
			...
			if (pluginDescription.deprecated) option.addChild(
				"b",
				" (" + l10n("loadLabelDeprecated") + ")"
			);
			if (pluginDescription.experimental) option.addChild(
				"b",
				" (" + l10n("loadLabelExperimental") + ")"
			);
			if (advancedModeEnabled && pluginDescription.minimumVersion >= 0) {
				option.addChild(
					...
					l10n("pluginVersion") +
					" " +
					pluginDescription.recommendedVersion +
					")"
				);
			}
			option.addChild(
				"#",
				" - " + pluginDescription.getLocalisedPluginDescription()
			);
		}
	}
	...
	private List<OfficialPluginDescription> getNotLoadedPlugins(
		...
		List<OfficialPluginDescription> notLoadedPlugins = new ArrayList<
			OfficialPluginDescription
		>();
	...
	private void showUnofficialPluginLoader(
		ToadletContext toadletContext,
		HTMLNode contentNode
	) {
		/* box for unofficial plugins. */
		HTMLNode addOtherPluginBox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		addOtherPluginBox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("loadOtherPlugin")
		);
		HTMLNode addOtherPluginContent = addOtherPluginBox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode addOtherForm = toadletContext.addFormChild(
			addOtherPluginContent,
			".",
			"addOtherPluginForm"
		);
	...
	private void showFreenetPluginLoader(
		ToadletContext toadletContext,
		HTMLNode contentNode
	) {
		/* box for freenet plugins. */
		HTMLNode addFreenetPluginBox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		addFreenetPluginBox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("loadFreenetPlugin")
		);
		HTMLNode addFreenetPluginContent = addFreenetPluginBox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode addFreenetForm = toadletContext.addFormChild(
			addFreenetPluginContent,
			".",
