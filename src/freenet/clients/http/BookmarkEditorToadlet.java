...
public class BookmarkEditorToadlet extends Toadlet {
	...
	private void addCategoryToList(
		...
		final String edit = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.edit");
		final String delete = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.delete");
		final String cut = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.cut");
		final String moveUp = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.moveUp");
		final String moveDown = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.moveDown");
		final String paste = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.paste");
		final String addBookmark = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.addBookmark");
		final String addCategory = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.addCategory");
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (i != items.size() - 1) actions
				.addChild("a", "href", "?action=down&bookmark=" + itemPath)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
					new String[] {
						"/static/icon/go-down.png",
						moveDown,
						moveDown,
					}
				);

			if (hasFriends) actions.addChild(
				"a",
				"href",
		...
		for (int i = 0; i < cats.size(); i++) {
			...
			HTMLNode subCat = list.addChild(
				"li",
				"class",
				"cat",
				cats.get(i).getVisibleName()
			);
			...
			actions
				.addChild(
					"a",
					"href",
					"?action=edit&bookmark=" + catPathEncoded
				)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
			...
			actions
				.addChild(
					"a",
					"href",
					"?action=addItem&bookmark=" + catPathEncoded
				)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
					new String[] {
						"/static/icon/bookmark-new.png",
						addBookmark,
						addBookmark,
					}
				);
			...
			actions
				.addChild(
					"a",
					"href",
					"?action=addCat&bookmark=" + catPathEncoded
				)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
					new String[] {
						"/static/icon/folder-new.png",
						addCategory,
						addCategory,
					}
				);
			...
			if (i != cats.size() - 1) actions
				.addChild(
					"a",
					"href",
					"?action=down&bookmark=" + catPathEncoded
				)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
			...
			if (
				...
				.addChild(
					"a",
					"href",
					"?action=paste&bookmark=" + catPathEncoded
				)
				.addChild(
					"img",
					new String[] { "src", "alt", "title" },
			...
			if (cats.get(i).size() != 0) addCategoryToList(
				cats.get(i),
				catPath,
	...
	private void sendBookmarkFeeds(
		HTTPRequest req,
		BookmarkItem item,
		String publicDescription
	) {
		for (DarknetPeerNode peer : core.node.getDarknetConnections()) if (
			req.isPartSet("node_" + peer.hashCode())
	...
	private HTMLNode getBookmarksList(BookmarkManager bookmarkManager) {
		...
		HTMLNode actions = new HTMLNode("span", "class", "actions");
		String addBookmark = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.addBookmark");
		String addCategory = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.addCategory");
		String paste = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.paste");
		actions
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		String editorTitle = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.title");
		String error = NodeL10n.getBase()
			.getString("BookmarkEditorToadlet.error");
		...
		if (
			req.getParam("action").length() > 0 && originalBookmark.length() > 0
		) {
			...
			try {
				...
				pageMaker
					.getInfobox(
						...
						error,
						content,
						"bookmark-url-decode-error",
						false
					)
					.addChild(
						"#",
						NodeL10n.getBase()
							.getString("BookmarkEditorToadlet.urlDecodeError")
					);
			...
			if (bookmarkPath.endsWith("/")) bookmark =
			...
			if (bookmark == null) {
				...
				HTMLNode confirmForm = ctx.addFormChild(
					infoBoxContent,
					"",
					"confirmDeleteForm"
				);
				...
				confirmForm.addChild(
					...
					new String[] {
						...
						"confirmdelete",
						NodeL10n.getBase()
							.getString("BookmarkEditorToadlet.confirmDelete"),
					}
				);
				...
				String header;
				if ("edit".equals(action)) header = NodeL10n.getBase()
					.getString(
				...
				else if ("addItem".equals(action)) header = NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.addNewBookmark");
				else if ("share".equals(action)) header = NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.share");
				else header = NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.addNewCategory");

				HTMLNode actionBoxContent = pageMaker.getInfobox(
					"infobox-query",
					header,
					content,
					"bookmark-action",
				...
				form.addChild(
					...
					"name",
					(NodeL10n.getBase()
							.getString("BookmarkEditorToadlet.nameLabel") +
						' ')
				);
				form.addChild(
					"input",
				...
				if (
					...
					form.addChild(
						...
						"key",
						(NodeL10n.getBase()
								.getString("BookmarkEditorToadlet.keyLabel") +
							' ')
					);
					form.addChild(
						"input",
					...
					if ("edit".equals(action) || "addItem".equals(action)) {
						...
						form.addChild(
							...
							(NodeL10n.getBase()
									.getString(
										"BookmarkEditorToadlet.explainLabel"
									) +
								' ')
						);
						form.addChild("br");
					...
					form.addChild(
						...
						(NodeL10n.getBase()
								.getString(
									"BookmarkEditorToadlet.hasAnActivelinkLabel"
								) +
							' ')
					);
					if (!isNew && item.hasAnActivelink()) form.addChild(
						"input",
					...
					if (
						core.node.getDarknetConnections().length > 0 &&
						...
						if (core.node.isFProxyJavascriptEnabled()) {
							...
							headerRow.addChild(
								...
								NodeL10n.getBase()
									.getString(
										"QueueToadlet.recommendToFriends"
									)
							);
						} else {
							peerTable
						...
						form.addChild(
							...
							(NodeL10n.getBase()
									.getString(
										"BookmarkEditorToadlet.publicDescLabel"
									) +
								' ')
						);
						form.addChild("br");
					...
					new String[] {
						...
						"share".equals(action)
							? NodeL10n.getBase()
								.getString("BookmarkEditorToadlet.share")
							: NodeL10n.getBase()
								.getString("BookmarkEditorToadlet.save"),
					}
				);
			} else if ("up".equals(action)) bookmarkManager.moveBookmarkUp(
				bookmarkPath,
				true
			);
			else if ("down".equals(action)) bookmarkManager.moveBookmarkDown(
				bookmarkPath,
				true
			);
		}
		...
		if (cutedPath != null) {
			HTMLNode infoBoxContent = pageMaker.getInfobox(
				"infobox-normal",
				NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.pasteTitle"),
				content,
				null,
				false
			);
			infoBoxContent.addChild(
				"#",
				NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.pasteOrCancel")
			);
			HTMLNode cancelForm = ctx.addFormChild(
				infoBoxContent,
				"/bookmarkEditor/",
				"cancelCutForm"
			);
			cancelForm.addChild(
				...
				new String[] {
					...
					"cancelCut",
					NodeL10n.getBase()
						.getString("BookmarkEditorToadlet.cancelCut"),
				}
			);
			cancelForm.addChild(
				"input",
		...
		pageMaker
			.getInfobox(
				"infobox-normal",
				NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.myBookmarksTitle"),
				content,
				"bookmark-title",
				false
			)
		...
		HTMLNode addDefaultBookmarksForm = ctx.addFormChild(
			content,
			"",
			"AddDefaultBookmarks"
		);
		addDefaultBookmarksForm.addChild(
			...
			new String[] {
				...
				"AddDefaultBookmarks",
				NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.addDefaultBookmarks"),
			}
		);
	...
	public void handleMethodPOST(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		PageNode page = pageMaker.getPageNode(
			NodeL10n.getBase().getString("BookmarkEditorToadlet.title"),
			ctx
		);
		HTMLNode pageNode = page.outer;
		HTMLNode content = page.content;
		...
		String bookmarkPath = req.getPartAsStringFailsafe(
			"bookmark",
		...
		try {
			...
			String action = req.getPartAsStringFailsafe(
				"action",
				MAX_ACTION_LENGTH
			);
			...
			if (req.isPartSet("confirmdelete")) {
				...
				pageMaker
					...
					.addChild(
						"p",
						NodeL10n.getBase()
							.getString("BookmarkEditorToadlet.deleteSucceeded")
					);
				...
				String name = "unnamed";
				if (req.isPartSet("name")) name = req.getPartAsStringFailsafe(
					"name",
					MAX_NAME_LENGTH
				);
				...
				if ("edit".equals(action)) {
					...
					pageMaker
						.getInfobox(
							...
							NodeL10n.getBase()
								.getString(
									"BookmarkEditorToadlet.changesSavedTitle"
								),
							content,
							"bookmark-error",
							false
					...
					if ("addItem".equals(action)) {
						 ...
						 */
						boolean hasAnActivelink = req.isPartSet(
							"hasAnActivelink"
						);
						if (!isValidName(name)) {
							addNameError(pageMaker, content);
						} else newBookmark = new BookmarkItem(
							key,
							name,
							req.getPartAsStringFailsafe(
								"descB",
					...
					if (newBookmark != null) {
						...
						bookmarkManager.storeBookmarks();
						if (
							newBookmark instanceof BookmarkItem
						) sendBookmarkFeeds(
							req,
			...
			} else if ("share".equals(action)) sendBookmarkFeeds(
				req,
			...
			pageMaker
				.getInfobox(
					"infobox-error",
					NodeL10n.getBase()
						.getString("BookmarkEditorToadlet.invalidKeyTitle"),
					content,
					"bookmark-error",
					false
				)
				.addChild(
					"#",
					NodeL10n.getBase()
						.getString("BookmarkEditorToadlet.invalidKey")
				);
		}
		pageMaker
			.getInfobox(
	...
	private void addNameError(PageMaker pageMaker, HTMLNode parent) {
		...
		errorBox.addChild(
			"#",
			NodeL10n.getBase().getString("BookmarkEditorToadlet.invalidName")
		);
	}
}
