...
public class ProgressInfoElement extends BaseUpdateableElement {
	...
	public void updateState(boolean initial) {
		...
		FProxyFetchWaiter waiter = tracker.makeWaiterForFetchInProgress(
			key,
			maxSize,
			fctx
		);
		...
		addChild(
			"a",
			"href",
			"/" + key.toString(false, false),
			key.getPreferredFilename()
		);
		if (fr.mimeType != null) addChild(
			"br",
			FProxyToadlet.l10n("contentTypeLabel") + " " + fr.mimeType
		);
		if (fr.size > 0) addChild(
			"br",
		...
		addChild("br");
		addChild(
			new SecondCounterNode(
				elapsed,
				true,
				FProxyToadlet.l10n("timeElapsedLabel") + " "
			)
		);
		...
		if (ctx.getContainer().isFProxyJavascriptEnabled()) {
			...
			lastRefreshNode.addChild(
				new SecondCounterNode(
					0,
					true,
					FProxyToadlet.l10n("lastRefresh")
				)
			);
			addChild(lastRefreshNode);
		}
		if (fr.goneToNetwork) addChild(
			"p",
			FProxyToadlet.l10n("progressDownloading")
		);
		else addChild("p", FProxyToadlet.l10n("progressCheckingStore"));
		if (!fr.finalizedBlocks) addChild(
			"p",
			FProxyToadlet.l10n("progressNotFinalized")
		);
