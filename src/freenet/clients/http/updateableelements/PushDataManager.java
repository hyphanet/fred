...
public class PushDataManager {
	...
	public synchronized void updateElement(String id) {
		...
		if (elements.containsKey(id)) for (String reqId : elements.get(id)) {
			...
			for (Map.Entry<
				String,
				List<UpdateEvent>
			> entry : awaitingNotifications.entrySet()) {
	 ...
	 */
	public synchronized void elementRendered(
		...
		if (awaitingNotifications.containsKey(requestUniqueId) == false) {
			awaitingNotifications.put(
				requestUniqueId,
				new ArrayList<UpdateEvent>()
			);
		}
		...
		if (isScheduled == false) {
			if (logMINOR) {
				Logger.minor(
					this,
					"Cleaner is queued(1) time:" + System.currentTimeMillis()
				);
			}
			cleaner.queueTimedJob(
				cleanerTask,
				"cleanerTask",
				getDelayInMs(),
				false,
	 ...
	 *            - The element's id
	 */
	public synchronized BaseUpdateableElement getRenderedElement(
		String requestId,
		String id
	) {
		if (logMINOR) {
			Logger.minor(
				this,
				"Getting element data for element:" +
				id +
				" in page:" +
				requestId
			);
	 ...
	 */
	public synchronized boolean failover(
		...
		if (awaitingNotifications.containsKey(originalRequestId)) {
			awaitingNotifications.put(
				newRequestId,
				awaitingNotifications.remove(originalRequestId)
			);
			if (logMINOR) {
				Logger.minor(
					this,
	...
	private synchronized boolean deleteRequest(String requestId) {
		...
		for (BaseUpdateableElement element : new ArrayList<
		...
		>(pages.get(requestId))) {
			...
			for (String events : awaitingNotifications.keySet()) {
				for (UpdateEvent updateEvent : new ArrayList<UpdateEvent>(
					awaitingNotifications.get(events)
				)) {
	...
	public class UpdateEvent {
		...
		public boolean equals(Object obj) {
			...
			if (obj instanceof UpdateEvent) {
				...
				if (
					o.getRequestId().compareTo(requestId) == 0 &&
		...
		public void run() {
			synchronized (PushDataManager.this) {
				if (logMINOR) {
					Logger.minor(
						this,
						"Cleaner running:" + isKeepaliveReceived
					);
				}
				isScheduled = false;
				for (Entry<String, Boolean> entry : new HashMap<
					String,
					Boolean
				>(isKeepaliveReceived).entrySet()) {
					if (entry.getValue() == false) {
						if (logMINOR) {
							Logger.minor(
								this,
								"Cleaner cleaned request:" + entry.getKey()
							);
						}
						...
						if (logMINOR) {
							Logger.minor(
								this,
								"Cleaner reseted request:" + entry.getKey()
							);
						}
