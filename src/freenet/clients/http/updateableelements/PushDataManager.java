...
public class PushDataManager {
    ...
    public synchronized void updateElement(String id) {
        ...
        if (elements.containsKey(id) == false) {
            if (logMINOR) {
                Logger.minor(this,
                             "Element is updating, but not present on elements! elements:" + elements + " pages:"
        ...
        if (elements.containsKey(id)) {
            for (String reqId : elements.get(id)) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Element is present on page:" + reqId
...
//                  for (List<UpdateEvent> notificationList : awaitingNotifications.values()) {
                    ...
                    if (notificationList.contains(updateEvent) == false) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Notification(" + updateEvent + ") added to a notification list for "
                                         + entry.getKey());
                        }
                    } else {
                        if (logMINOR) {
    ...
    public synchronized BaseUpdateableElement getRenderedElement(String requestId, String id) {
        ...
        Logger.error(this,
                     "Could not find data for the element requested. requestId:" + requestId + " id:" + id + " pages:"
                     + pages + " keepaliveReceived:" + isKeepaliveReceived);
    ...
    public synchronized boolean failover(String originalRequestId, String newRequestId) {
        ...
        if (awaitingNotifications.containsKey(originalRequestId)) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "copied " + awaitingNotifications.get(newRequestId).size() + " notification:"
    ...
    public synchronized UpdateEvent getNextNotification(String requestId) {
        ...
        while (((awaitingNotifications.get(requestId) != null) && (awaitingNotifications.get(requestId).size() == 0)) ||    // No notifications
                ((awaitingNotifications.get(requestId) != null) && (awaitingNotifications.get(requestId).size() != 0)
                 && (isFirstKeepaliveReceived.containsKey(awaitingNotifications.get(requestId).get(0).requestId)
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Getting notification, notification:" + awaitingNotifications.get(requestId).get(0)
