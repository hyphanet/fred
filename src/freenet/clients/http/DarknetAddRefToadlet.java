...
public class DarknetAddRefToadlet extends Toadlet {
	...
	public void handleMethodGET(
		...
		HTMLNode boxContent = pageMaker.getInfobox(
			"infobox-information",
		...
		if (installer != null) NodeL10n.getBase()
			.addL10nSubstitution(
				p,
			...
			.addL10nSubstitution(
				...
				new HTMLNode[] {
					HTMLNode.link(
						"/" +
						node.nodeUpdater.getInstallerWindowsURI().toString()
					),
				}
			);
		...
		if (installer != null) NodeL10n.getBase()
			...
			.addL10nSubstitution(
				...
				new HTMLNode[] {
					HTMLNode.link(
						"/" +
						node.nodeUpdater.getInstallerNonWindowsURI().toString()
					),
					HTMLNode.text(shortFilename),
				}
			);

		ConnectionsToadlet.drawAddPeerBox(
			contentNode,
			ctx,
			false,
			friendsToadlet.path()
