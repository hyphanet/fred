...
public final class SimpleToadletServer
		...
		public void set(Boolean val) throws InvalidConfigValueException {
			...
			if (!SSL.available()) {
				throw new InvalidConfigValueException(
					"Enable SSL support before use ssl with Fproxy"
				);
			}
			ssl = val;
			throw new InvalidConfigValueException(
				"Cannot change SSL on the fly, please restart freenet"
			);
		}
	...
	private class FProxyBindtoCallback extends StringCallback {
		...
		public void set(String bindTo) throws InvalidConfigValueException {
			...
			if (!bindTo.equals(oldValue)) {
				String[] failedAddresses = networkInterface.setBindTo(
					bindTo,
					false
				);
	...
	private class FProxyCSSNameCallback
		...
		public void set(String CSSName) throws InvalidConfigValueException {
			if (
				(CSSName.indexOf(':') != -1) || (CSSName.indexOf('/') != -1)
	...
	private class FProxyCSSOverrideCallback extends StringCallback {
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			if (cssOverride == null) pageMaker.setOverride(null);
				...
				pageMaker.setOverride(
					StaticToadlet.OVERRIDE_URL + cssOverride.getName()
				);
			}
		}
	}
	...
	private class ReFilterCallback
		...
		public String[] getPossibleValues() {
			...
			for (int i = 0; i < possible.length; i++) ret[i] =
				possible[i].name();
			return ret;
		}
	...
	public void createFproxy() {
		...
		intervalPushManager = new IntervalPusherManager(
			getTicker(),
			pushDataManager
		);
	...
	public SimpleToadletServer(
		...
		fproxyHasCompletedWizard = fproxyConfig.getBoolean(
			"hasCompletedWizard"
		);
		...
		enableExtendedMethodHandling = fproxyConfig.getBoolean(
			"enableExtendedMethodHandling"
		);
		...
		fproxyConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean value) {
					if (
						value == SimpleToadletServer.isPanicButtonToBeShown
					) return;
		...
		fproxyConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean val)
					...
					throw new NodeNeedRestartException(
						l10n("publicGatewayModeNeedsRestart")
					);
				}
			}
		);
		wasPublicGatewayMode = publicGatewayMode = fproxyConfig.getBoolean(
			"publicGatewayMode"
		);
		...
		enablePersistentConnections = fproxyConfig.getBoolean(
			"enablePersistentConnections"
		);
		...
		FProxyToadlet.MAX_LENGTH_NO_PROGRESS = fproxyConfig.getLong(
			"passthroughMaxSize"
		);
		fproxyConfig.register(
		...
		FProxyToadlet.MAX_LENGTH_WITH_PROGRESS = fproxyConfig.getLong(
			"passthroughMaxSizeProgress"
		);
		System.out.println(
		...
		enableCachingForChkAndSskKeys = fproxyConfig.getBoolean(
			"enableCachingForChkAndSskKeys"
		);
		fproxyConfig.register(
			"allowedHosts",
		...
		allowedFullAccess = new AllowedHosts(
			fproxyConfig.getString("allowedHostsFullAccess")
		);
		fproxyConfig.register(
			"doRobots",
			false,
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					if (val < -1) throw new InvalidConfigValueException(
						"-1 = disabled, 0+ = set a minimum interval"
					); // FIXME l10n
		...
		HTMLFilter.metaRefreshSamePageMinInterval = Math.max(
			-1,
		...
		HTMLFilter.metaRefreshRedirectMinInterval = Math.max(
			-1,
		...
		HTMLFilter.embedM3uPlayer = fproxyConfig.getBoolean(
			"embedM3uPlayerInFreesites"
		);
		...
		this.refilterPolicy = REFILTER_POLICY.valueOf(
			fproxyConfig.getString("refilterPolicy")
		);

		// Network seclevel not physical seclevel because bad filtering can cause network level anonymity breaches.
		SimpleToadletServer.isPanicButtonToBeShown = fproxyConfig.getBoolean(
			"showPanicButton"
		);
		SimpleToadletServer.noConfirmPanic = fproxyConfig.getBoolean(
			"noConfirmPanic"
		);
		...
		if (
			...
			"CSS name must not contain slashes or colons!"
		);
		cssTheme = THEME.themeFromName(cssName);
		...
		fetchKeyBoxAboveBookmarks = fproxyConfig.getBoolean(
			"fetchKeyBoxAboveBookmarks"
		);

		this.advancedModeEnabled = fproxyConfig.getBoolean(
			"advancedModeEnabled"
		);
	...
	private void maybeGetNetworkInterface() throws IOException {
		...
		if (ssl) {
			this.networkInterface = SSLNetworkInterface.create(
				port,
				this.bindTo,
				allowedHosts,
				executor,
			...
			this.networkInterface = NetworkInterface.create(
				port,
				this.bindTo,
				allowedHosts,
				executor,
			...
			new SecurityLevelListener<NETWORK_THREAT_LEVEL>() {
				...
				public void onChange(
					...
					if (
						...
						oldLevel == NETWORK_THREAT_LEVEL.LOW &&
						newLevel != oldLevel
					) {
	...
	public Toadlet findToadlet(URI uri) throws PermanentRedirectException {
		...
		if (core != null && core.node != null && !fproxyHasCompletedWizard) {
			...
			if (
				!(path.startsWith(FirstTimeWizardToadlet.TOADLET_URL) ||
		...
		synchronized (toadlets) {
			for (ToadletElement te : toadlets) {
				...
				if (
					...
					if (
						path.equals(
							te.prefix.substring(0, te.prefix.length() - 1)
						)
					) {
						URI newURI;
						try {
	...
	public void run() {
		...
		while (true) {
			synchronized (this) {
				...
				if (
					(!finishedStartup) && this.finishedStartup
				) finishedStartup = true;
	...
	public class SocketHandler implements PrioRunnable {
		...
		void start() {
			if (finishedStartup) executor.execute(
				this,
				"HTTP socket handler@" + hashCode()
			);
	...
	private static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("SimpleToadletServer." + key, pattern, value);
	}
