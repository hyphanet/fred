...
public class DiagnosticToadlet extends Toadlet {
	...
	public void handleMethodGET(
		...
		synchronized (this) {
			...
			textBuilder
				...
				.append("\n");
			textBuilder
				.append(l10n("cpus", "count", Integer.toString(availableCpus)))
				.append("\n");
			textBuilder
				.append(
					l10n(
			...
			textBuilder
				...
				.append("\n");
			textBuilder
				.append(l10n("osName", "name", System.getProperty("os.name")))
				.append("\n");
			textBuilder
				.append(
					l10n(
				...
				.append("\n");
			textBuilder
				.append(l10n("osArch", "arch", System.getProperty("os.arch")))
				.append("\n");
			textBuilder.append("\n");
			...
			> entry : storeStats.entrySet()) {
				...
				try {
					textBuilder
						.append(fix1p4.format(sessionAccess.successRate()))
						.append("%");
					if (totalAccess != null) {
						try {
							textBuilder
			...
			if (total[0] == 0 || total[1] == 0) textBuilder.append(
				"bandwidth error\n"
			);
			else {
				...
				if (delta > 0) {
					...
					int inputBandwidthLimit = nodeConfig.getInt(
						"inputBandwidthLimit"
					);
				...
				textBuilder
					.append(
						l10n(
							...
							new String[] { "total" },
							new String[] {
								SizeUtil.formatSize(overall_total_in, true),
							}
						)
					)
					.append("\n");
				textBuilder
					.append(
						l10n(
							...
							new String[] { "total" },
							new String[] {
								SizeUtil.formatSize(overall_total_out, true),
							}
						)
					)
					.append("\n");
				...
				textBuilder
					.append(
						l10n(
							...
							new String[] {
								SizeUtil.formatSize(
									totalBytesSentRemaining,
									true
								),
								Integer.toString(
									(int) ((totalBytesSentRemaining * 100) /
										total[0])
								),
							}
						)
			...
			if (!pm.getPlugins().isEmpty()) {
				textBuilder
					.append(baseL10n.getString("PluginToadlet.pluginListTitle"))
					.append("\n");
				for (PluginInfoWrapper pi : pm.getPlugins()) {
					long ver = pi.getPluginLongVersion();
					if (ver != -1) textBuilder
			...
			try {
				...
				if (reqs.length < 1) textBuilder
					.append(
						baseL10n.getString("QueueToadlet.globalQueueIsEmpty")
					)
					.append("\n");
				else {
	...
	private StringBuilder threadsStats() {
		...
		NodeThreadSnapshot threadSnapshot =
			threadDiagnostics.getThreadSnapshot();
		...
		threads.sort(
			Comparator.comparing(NodeThreadInfo::getCpuTime).reversed()
		);
		...
		for (NodeThreadInfo thread : threads) {
			String line = String.format(
				...
				thread
					.getName()
					.substring(0, Math.min(90, thread.getName().length())),
				thread.getPrio(),
				thread
					.getGroupName()
					.substring(0, Math.min(10, thread.getGroupName().length())),
				thread.getState(),
