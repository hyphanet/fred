...
public class DiagnosticToadlet extends Toadlet {
    ...
    public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        synchronized (this) {
            ...
            text += l10n("threads", new String[] { "running", "max" }, new String[] { thousandPoint.format(threadCount),
            ...
            if ((numTransferringRequests == 0) && (numCHKRequests == 0) && (numSSKRequests == 0)
                ...
                if ((numCHKInserts > 0) || (numSSKInserts > 0)) {
                    text += l10n("activityInserts", new String[] { "CHKhandlers", "SSKhandlers", "local" },
                ...
                if ((numCHKRequests > 0) || (numSSKRequests > 0)) {
                    text += l10n("activityRequests", new String[] { "CHKhandlers", "SSKhandlers", "local" },
                ...
                if ((numTransferringRequests > 0) || (numTransferringRequestHandlers > 0)) {
                    text += l10n("transferringRequests", new String[] { "senders", "receivers", "turtles" },
                ...
                if ((numCHKOfferReplys > 0) || (numSSKOfferReplys > 0)) {
                    text += l10n("offerReplys", new String[] { "chk", "ssk" },
                                 new String[] { Integer.toString(numCHKOfferReplys),
                                                Integer.toString(numSSKOfferReplys) }) + "\n";
                }

                text += l10n("runningBlockTransfers", new String[] { "sends", "receives" },
            ...
            if ((total[0] == 0) || (total[1] == 0)) {
                ...
                long totalBytesSentRemaining = total[0]
                                               - (totalPayload + totalBytesSentCHKRequests + totalBytesSentSSKRequests
                                                  ...
                                                  + totalBytesSentSwapOutput + totalBytesSentAuth
                                                  + totalBytesSentAckOnly + totalBytesSentResends + totalBytesSentUOM
                                                  + totalBytesSentAnnounce + totalBytesSentRoutingStatus
                                                  + totalBytesSentNetworkColoring + totalBytesSentPing
                                                  + totalBytesSentProbeRequest + totalBytesSentRouted
                                                  + totalBytesSentDisconn + totalBytesSentInitial
                                                  + totalBytesSentChangedIP + totalBytesSentNodeToNode
                                                  + totalBytesSentAllocationNotices + totalBytesSentFOAF);

                text += l10n("requestOutput", new String[] { "chk", "ssk" },
                             new String[] { SizeUtil.formatSize(totalBytesSentCHKRequests, true),
                                            SizeUtil.formatSize(totalBytesSentSSKRequests, true) }) + "\n";
                text += l10n("insertOutput", new String[] { "chk", "ssk" },
                             new String[] { SizeUtil.formatSize(totalBytesSentCHKInserts, true),
                                            SizeUtil.formatSize(totalBytesSentSSKInserts, true) }) + "\n";
                text += l10n("offeredKeyOutput", new String[] { "total", "offered" },
                ...
                text += l10n("debuggingBytes", new String[] { "netColoring", "ping", "probe", "routed" },
                ...
                text += l10n("loadAllocationNoticesBytes", "total",
                ...
                text += l10n("unaccountedBytes", new String[] { "total", "percent" },
                             new String[] { SizeUtil.formatSize(totalBytesSentRemaining, true),
                                            Integer.toString((int) (totalBytesSentRemaining * 100
                                            / total[0])) }) + "\n";
            ...
            if (!pm.getPlugins().isEmpty()) {
                ...
                for (PluginInfoWrapper pi : pm.getPlugins()) {
                    ...
                    if (ver != -1) {
                        text += pi.getFilename() + " (" + pi.getPluginClassName() + ") - " + pi.getPluginVersion()
                                + " (" + ver + ")" + " " + pi.getThreadName() + "\n";
                    } else {
                        text += pi.getFilename() + " (" + pi.getPluginClassName() + ") - " + pi.getPluginVersion()
                                + " " + pi.getThreadName() + "\n";
                    }
            ...
            for (int i = 0; i < activeThreadsByPriority.length; i++) {
                text += l10n("running") + ": " + String.valueOf(activeThreadsByPriority[i]) + " ("
                        + String.valueOf(i + 1) + ")\n";
                text += l10n("waiting") + ": " + String.valueOf(waitingThreadsByPriority[i]) + " ("
                        + String.valueOf(i + 1) + ")\n";
            }
