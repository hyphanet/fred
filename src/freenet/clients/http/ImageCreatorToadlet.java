	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		if (ctx.getHeaders().containsKey("if-modified-since")) {
			try {
				...
				if (
					ToadletContextImpl.parseHTTPDate(
						ctx.getHeaders().get("if-modified-since")
		...
		if (needsGeneration) {
			...
			int requiredWidth = req.getParam("width").compareTo("") != 0
				? Integer.parseInt(
					req.getParam("width").endsWith("px")
						? req
							.getParam("width")
							.substring(0, req.getParam("width").length() - 2)
						: req.getParam("width")
				)
				: DEFAULT_WIDTH;
			int requiredHeight = req.getParam("height").compareTo("") != 0
				? Integer.parseInt(
					req.getParam("height").endsWith("px")
						? req
							.getParam("height")
							.substring(0, req.getParam("height").length() - 2)
						: req.getParam("height")
				)
				: DEFAULT_HEIGHT;
			// Validate image size
			...
			if (
				requiredWidth > WIDTH_AND_HEIGHT_LIMIT ||
				requiredHeight > WIDTH_AND_HEIGHT_LIMIT
			) {
				writeHTMLReply(
					ctx,
					400,
			...
			specifyMaximumFontSizeThatFitsInImage(
				g2,
				fc,
				requiredWidth,
				requiredHeight,
