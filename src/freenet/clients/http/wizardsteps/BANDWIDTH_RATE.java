...
public class BANDWIDTH_RATE extends BandwidthManipulator implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		HTMLNode infoBox = helper.getInfobox(
			"infobox-normal",
		...
		NodeL10n.getBase()
			.addL10nSubstitution(
				...
				new HTMLNode[] {
					...
					new HTMLNode(
						"#",
						NodeL10n.getBase().getString("ConfigToadlet.node")
					),
				}
			);
		...
		try {
			...
			BandwidthLimit usable = new BandwidthLimit(
				detected.downBytes / 2,
				detected.upBytes / 2,
				"bandwidthDetected",
				true
			);
	...
	public String postStep(HTTPRequest request) {
		String limitSelected = request.getPartAsStringFailsafe(
			"bandwidth",
			100
		);
		...
		if (!limitSelected.isEmpty()) {
			...
			if (x != -1) {
				...
				if (!preset.isEmpty()) {
					...
					Logger.error(
						this,
						"Failed to parse pre-defined limit! Please report."
					);
					return (
	...
	private void addLimitRow(
		...
		String downColumn =
			SizeUtil.formatSize(limit.downBytes) +
			WizardL10n.l10n("bandwidthPerSecond");
		if (limit.downBytes >= 32 * 1024) {
			downColumn += " (= ";
			if (limit.downBytes < 256 * 1024) downColumn +=
			new DecimalFormat("0.0").format(
				((double) ((limit.downBytes * 8))) / (1024 * 1024)
			);
		...
		row.addChild(
			"td",
			SizeUtil.formatSize(limit.upBytes) +
			WizardL10n.l10n("bandwidthPerSecond")
		);

		HTMLNode buttonCell = row.addChild("td");

		HTMLNode radio = buttonCell.addChild(
			"input",
			new String[] { "type", "name", "value" },
			new String[] {
				"radio",
				"bandwidth",
				limit.downBytes + "/" + limit.upBytes,
			}
		);
		if (
			recommended || (useMaybeDefault && limit.maybeDefault)
		) radio.addAttribute("checked", "checked");
		if (recommended) {
			buttonCell.addChild(
				"#",
				WizardL10n.l10n("autodetectedSuggestedLimit")
			);
		}
	}
}
