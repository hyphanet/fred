...
public class SECURITY_NETWORK implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		HTMLNode contentNode = helper.getPageContent(
			WizardL10n.l10n("networkSecurityPageTitle")
		);
		...
		if (request.isParameterSet("confirm")) {
			String networkThreatLevel = request.getParam(
			...
			HTMLNode infoboxContent = helper.getInfobox(
				"infobox-information",
				WizardL10n.l10n(
					"networkThreatLevelConfirmTitle." + newThreatLevel
				),
				contentNode,
				null,
				false
			...
			HTMLNode formNode = helper.addFormChild(
				infoboxContent,
				".",
				"configFormSecLevels"
			);
		...
		if (opennet) {
			...
			infoboxContent.addChild(
				"p",
				WizardL10n.l10n("networkThreatLevelIntroOpennet")
			);

			form = helper.addFormChild(
				infoboxContent,
				".",
				"networkSecurityForm"
			);
			...
			infoboxContent.addChild(
				"p",
				WizardL10n.l10n("networkThreatLevelIntroDarknet")
			);
			...
			form
				...
				.addChild(
					"b",
					WizardL10n.l10nSec(
						"networkThreatLevel.opennetFriendsWarning"
					)
				);
		}
		form.addChild(
	 ...
	 */
	private void securityLevelChoice(
		HTMLNode parent,
		SecurityLevels.NETWORK_THREAT_LEVEL level
	) {
		HTMLNode input = parent
			.addChild("p")
			.addChild(
				"input",
		...
		input
			...
			.addChild(
				"b",
				WizardL10n.l10nSec("networkThreatLevel.name." + level)
			);
		input.addChild("#", ": ");
		NodeL10n.getBase()
			.addL10nSubstitution(
	...
	public String postStep(HTTPRequest request) {
		...
		if (
			newThreatLevel == null ||
			!request.isPartSet("security-levels.networkThreatLevel")
		) {
		...
		if (
			(newThreatLevel == SecurityLevels.NETWORK_THREAT_LEVEL.MAXIMUM ||
