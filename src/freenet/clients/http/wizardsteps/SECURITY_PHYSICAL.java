...
public class SECURITY_PHYSICAL implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		HTMLNode contentNode = helper.getPageContent(
			WizardL10n.l10n("physicalSecurityPageTitle")
		);
			...
			.addL10nSubstitution(
				...
				new HTMLNode[] {
					...
					HTMLNode.linkInNewWindow(
						ExternalLinkToadlet.escape("http://www.truecrypt.org/")
					),
				}
			);
		...
		for (SecurityLevels.PHYSICAL_THREAT_LEVEL level : SecurityLevels.PHYSICAL_THREAT_LEVEL.values()) {
			...
			if (
				level == SecurityLevels.PHYSICAL_THREAT_LEVEL.HIGH &&
				...
				p.addChild(
					...
					"for",
					"passwordBox",
					WizardL10n.l10nSec("setPasswordLabel") + ":"
				);
				p.addChild(
					"input",
	...
	private boolean errorHandler(HTTPRequest request, PageHelper helper) {
		...
		if (error.equals("pass")) {
			...
			HTMLNode contentNode = helper.getPageContent(
				WizardL10n.l10nSec(pageTitleKey)
			);

			HTMLNode content = helper.getInfobox(
				"infobox-error",
			...
			if (
				type == PASSWORD_PROMPT.SET_BLANK ||
				type == PASSWORD_PROMPT.DECRYPT_BLANK
			) {
				content.addChild(
					"p",
					WizardL10n.l10nSec("passwordNotZeroLength")
				);
			} else if (type == PASSWORD_PROMPT.SET_NO_MATCH) {
				content.addChild(
					"p",
					WizardL10n.l10nSec("passwordsDoNotMatch")
				);
			}
	...
	public String postStep(HTTPRequest request) throws IOException {
		final String errorCorrupt =
			...
			"&error=corrupt";
		String pass = request.getPartAsStringFailsafe(
			"masterPassword",
			SecurityLevelsToadlet.MAX_PASSWORD_LENGTH
		);
		String confirmPass = request.getPartAsStringFailsafe(
			"confirmMasterPassword",
			SecurityLevelsToadlet.MAX_PASSWORD_LENGTH
		);
		...
		if (
			newThreatLevel == SecurityLevels.PHYSICAL_THREAT_LEVEL.HIGH &&
			...
			if (passwordIsBlank) {
				...
				try {
					...
					throw new IOException(
						"Incorrect password when changing from another level to high",
						e
					);
