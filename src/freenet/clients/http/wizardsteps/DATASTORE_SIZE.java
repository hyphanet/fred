...
public class DATASTORE_SIZE implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		HTMLNode contentNode = helper.getPageContent(
			WizardL10n.l10n("step4Title")
		);
		HTMLNode bandwidthInfoboxContent = helper.getInfobox(
			"infobox-header",
		...
		bandwidthInfoboxContent.addChild(
			"#",
			WizardL10n.l10n("datastoreSizeLong")
		);
		HTMLNode bandwidthForm = helper.addFormChild(
			bandwidthInfoboxContent,
			".",
			"dsForm"
		);
		...
		if (maxSize >= 3L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"3G",
			"3 GiB"
		);
		if (maxSize >= 5L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"5G",
			"5 GiB"
		);
		if (maxSize >= 10L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"10G",
			"10 GiB"
		);
		if (maxSize >= 20L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"20G",
			"20 GiB"
		);
		if (maxSize >= 50L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"50G",
			"50 GiB"
		);
		if (maxSize >= 200L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"200G",
			"200GiB"
		);
		if (maxSize >= 500L * 1024 * 1024 * 1024) result.addChild(
			"option",
			"value",
			"500G",
			"500GiB"
	...
	private static void _setDatastoreSize(
		...
		try {
			...
			int upstreamLimit = config
				.get("node")
				.getInt("outputBandwidthLimit");
			int downstreamLimit = config
				.get("node")
				.getInt("inputBandwidthLimit");
			...
			System.out.println(
				"Setting datastore size to " +
				Fields.longToString(storeSize, true)
			);
			config
				.get("node")
				.set("storeSize", Fields.longToString(storeSize, true));
			if (firsttime) config.get("node").set("storeType", "salt-hash");
			System.out.println(
				"Setting client cache size to " +
				Fields.longToString(clientCacheSize, true)
			);
			config
				.get("node")
				.set(
			...
			Logger.normal(
				callback,
				"The storeSize has been set to " + selectedStoreSize
			);
