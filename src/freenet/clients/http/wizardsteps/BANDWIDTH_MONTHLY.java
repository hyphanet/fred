...
public class BANDWIDTH_MONTHLY extends BandwidthManipulator implements Step {
    ...
    public void getStep(HTTPRequest request, PageHelper helper) {
        ...
        if (request.isParameterSet("parseError")) {
            ...
            HTMLNode errorBox = parseErrorBox(contentNode, helper,
                                              WizardL10n.l10n("bandwidthMonthlyLow", new String[] { "requested",
                    "minimum", "useMinimum" }, new String[] { parseTarget, String.valueOf(Math.round(minCap)),
                    WizardL10n.l10n("bandwidthMonthlyUseMinimum") }));
            HTMLNode minimumForm = helper.addFormChild(errorBox, ".", "use-minimum");

            minimumForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "capTo",
                    String.valueOf(minCap) });
            minimumForm.addChild("input", new String[] { "type", "value" }, new String[] { "submit",
                    WizardL10n.l10n("bandwidthMonthlyUseMinimum") });
        }
        ...
        NodeL10n.getBase().addL10nSubstitution(infoBox, "FirstTimeWizardToadlet.bandwidthLimitMonthly",
                new String[] { "bold",
                               "coreSettings" }, new HTMLNode[] { HTMLNode.STRONG,
        ...
        for (long cap : caps) {
            ...
            selectForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "capTo",
                    String.valueOf(cap) });
            selectForm.addChild("input", new String[] { "type", "value" }, new String[] { "submit",
                    WizardL10n.l10n("bandwidthSelect") });
        }
        ...
        customForm.addChild("td").addChild("input", new String[] { "type", "value" }, new String[] { "submit",
                WizardL10n.l10n("bandwidthSelect") });

        // Back / next buttons
        HTMLNode backForm = helper.addFormChild(infoBox, ".", "backForm");

        backForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "back",
    ...
    public String postStep(HTTPRequest request) {
        ...
        StringBuilder target =
            new StringBuilder(FirstTimeWizardToadlet.WIZARD_STEP.BANDWIDTH_MONTHLY.name()).append("&parseTarget=");
        ...
        double downloadFraction = ((maxFraction - minFraction) / (maxScaleCap - 2 * minCap)) * (GBPerMonth - minCap)
                                  + minFraction;

        // No minimum fraction clamp: caps less than the minimum get redirected to a warning instead.
        if (downloadFraction > maxFraction) {
            downloadFraction = maxFraction;
        }
