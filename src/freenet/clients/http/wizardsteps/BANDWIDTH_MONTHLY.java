...
public class BANDWIDTH_MONTHLY extends BandwidthManipulator implements Step {
	...
	private static final long[] caps = {
		(long) Math.ceil(BandwidthLimit.minMonthlyLimit),
		100,
		150,
		250,
		500,
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		if (request.isParameterSet("parseError")) {
			parseErrorBox(
				contentNode,
				helper,
				WizardL10n.l10n("bandwidthCouldNotParse", "limit", parseTarget)
			);
		} else if (request.isParameterSet("tooLow")) {
			HTMLNode errorBox = parseErrorBox(
				contentNode,
				helper,
				WizardL10n.l10n(
					"bandwidthMonthlyLow",
					...
					new String[] {
						parseTarget,
						String.valueOf(
							Math.round(BandwidthLimit.minMonthlyLimit)
						),
			...
			HTMLNode minimumForm = helper.addFormChild(
				errorBox,
				".",
				"use-minimum"
			);
			minimumForm.addChild(
				...
				new String[] {
					"hidden",
					"capTo",
					String.valueOf(BandwidthLimit.minMonthlyLimit),
				}
			);
			minimumForm.addChild(
				"input",
		...
		for (long cap : caps) {
			...
			HTMLNode selectForm = helper.addFormChild(
				row.addChild("td"),
				".",
				"limit"
			);
		...
		HTMLNode customForm = helper.addFormChild(
			table.addChild("tr"),
			".",
			"custom-form"
		);
