...
public abstract class BandwidthManipulator {
	...
	protected void setBandwidthLimit(String limit, boolean setOutputLimit)
		...
		String limitType = setOutputLimit
		...
		try {
			...
			Logger.normal(
				this,
				"The " + limitType + " has been set to " + limit
			);
	 ...
	 */
	protected HTMLNode parseErrorBox(
		HTMLNode parent,
		PageHelper helper,
		String message
	) {
	...
	protected BandwidthLimit getCurrentBandwidthLimitsOrNull() {
		if (!config.get("node").getOption("outputBandwidthLimit").isDefault()) {
			return new BandwidthLimit(
				core.node.getInputBandwidthLimit(),
				core.node.getOutputBandwidthLimit(),
				"bandwidthCurrent",
				false
			);
		}
	 ...
	 */
	public static BandwidthLimit detectBandwidthLimits(
