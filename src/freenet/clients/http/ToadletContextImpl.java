...
public class ToadletContextImpl implements ToadletContext {
	...
	private void sendMethodNotAllowed(String method, boolean shouldDisconnect)
		...
		MultiValueTable<String, String> mvt = new MultiValueTable<
			String,
			String
		>();
	...
	private static void sendURIParseError(
		OutputStream os,
		boolean shouldDisconnect,
		Throwable e
	) throws IOException {
	...
	public void sendReplyHeaders(
		...
		boolean enableJavascript =
			(!forceDisableJavascript) && container.isFProxyJavascriptEnabled();
		sendReplyHeaders(
			code,
			desc,
			mvt,
	...
	public void sendReplyHeaders(
		...
		if (mTime != null) sendReplyHeadersStatic(
			code,
			desc,
			mvt,
			mimeType,
	...
	public void sendReplyHeadersFProxy(
		...
		if (
			container.isFProxyWebPushingEnabled() &&
	...
	public boolean hasFormPassword(HTTPRequest request) throws IOException {
		...
		byte[] compareBytes = getFormPassword()
	...
	private void parseCookies() throws ParseException {
		...
		for (String cookieHeader : headers.iterateAll("cookie")) {
			ArrayList<ReceivedCookie> parsedCookies =
				ReceivedCookie.parseHeader(cookieHeader);
	...
	static void sendReplyHeaders(
		...
		// Construct headers
		if (mvt == null) mvt = new MultiValueTable<String, String>();
		if (mimeType != null) if (mimeType.equalsIgnoreCase("text/html")) {
			mvt.put("content-type", mimeType + "; charset=UTF-8");
		} else {
			mvt.put("content-type", mimeType);
		}
		if (contentLength >= 0) mvt.put(
			"content-length",
		...
		if (allowCaching) {
			...
			expiresTime = TimeUtil.makeHTTPDate(
				System.currentTimeMillis() + DAYS.toMillis(30)
			);
	...
	private static String generateCSP(
		boolean allowScripts,
		boolean allowFrames
	) {
	...
	public static void handle(
		...
		try {
			...
			while (true) {
				...
				if (split.length != 3) throw new ParseException(
					"Could not parse request line (split.length=" +
					split.length +
					"): " +
					firstLine,
					-1
				...
				if (!split[2].startsWith("HTTP/1.")) throw new ParseException(
					"Unrecognized protocol " + split[2],
					-1
				);
				...
				while (true) {
					...
					if (index < 0) {
						throw new ParseException(
							"Missing ':' in request header field",
							-1
						);
					}
				...
				boolean disconnect =
					shouldDisconnectAfterHandled(
						split[2].equals("HTTP/1.0"),
						headers
					) ||
				...
				ToadletContextImpl ctx = new ToadletContextImpl(
					sock,
					headers,
					bf,
					pageMaker,
			...
			Logger.error(
				ToadletContextImpl.class,
				"Caught error: " + t + " handling socket",
				t
			);
			try {
				String msg =
