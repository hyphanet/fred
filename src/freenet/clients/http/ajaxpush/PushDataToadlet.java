...
public class PushDataToadlet extends Toadlet {
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		BaseUpdateableElement node =
			((SimpleToadletServer) ctx.getContainer()).pushDataManager.getRenderedElement(
					requestId,
					elementId
				);
		if (logMINOR) {
		...
		writeHTMLReply(
			...
			":" +
			Base64.encodeStandard(
				node.getUpdaterType().getBytes(StandardCharsets.UTF_8)
			) +
			":" +
			Base64.encodeStandard(
				node.generateChildren().getBytes(StandardCharsets.UTF_8)
			)
		);
	}
