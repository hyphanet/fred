...
public class StatisticsToadlet extends Toadlet {
	...
	private final DecimalFormat fix3p1US = new DecimalFormat(
		"##0.0",
		new DecimalFormatSymbols(Locale.US)
	);
	private final DecimalFormat fix3pctUS = new DecimalFormat(
		"##0%",
		new DecimalFormatSymbols(Locale.US)
	);
	private final DecimalFormat fix6p6 = new DecimalFormat("#####0.0#####");

	protected StatisticsToadlet(
		Node n,
		NodeClientCore core,
	 ...
	 */
	private int getPeerStatusCount(
	...
	public void handleMethodGET(
		...
		if (requestPath.length() > 0) {
			if (
				requestPath.equals("requesters.html") ||
				requestPath.equals("/requesters.html")
			) {
		...
		synchronized (this) {
			...
			int numberOfConnected = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_CONNECTED
			);
			int numberOfRoutingBackedOff = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_ROUTING_BACKED_OFF
			);
			int numberOfTooNew = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_TOO_NEW
			);
			int numberOfTooOld = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_TOO_OLD
			);
			int numberOfDisconnected = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_DISCONNECTED
			);
			int numberOfNeverConnected = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED
			);
			int numberOfDisabled = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_DISABLED
			);
			int numberOfBursting = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_BURSTING
			);
			int numberOfListening = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_LISTENING
			);
			int numberOfListenOnly = getPeerStatusCount(
				peerNodeStatuses,
			...
			int numberOfRoutingDisabled = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_ROUTING_DISABLED
			);
			int numberOfClockProblem = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_CLOCK_PROBLEM
			);
			int numberOfConnError = getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_CONN_ERROR
			);
			int numberOfDisconnecting = PeerNodeStatus.getPeerStatusCount(
				peerNodeStatuses,
				PeerManager.PEER_NODE_STATUS_DISCONNECTING
			);
			int numberOfNoLoadStats = PeerNodeStatus.getPeerStatusCount(
				peerNodeStatuses,
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("fullTitle"), ctx);
			...
			HTMLNode overviewTable = contentNode.addChild(
				"table",
				"class",
				"column"
			);
			...
			HTMLNode nextTableCell = overviewTableRow.addChild(
				"td",
				"class",
				"first"
			);
			...
			HTMLNode versionInfobox = nextTableCell.addChild(
				"div",
				"class",
				"infobox"
			);
			...
			// jvm stats box
			HTMLNode jvmStatsInfobox = nextTableCell.addChild(
				"div",
				"class",
				"infobox"
			);
			...
			HTMLNode statGatheringContent = ctx
				.getPageMaker()
				.getInfobox(
					"#",
			...
			if (node.isUsingWrapper()) {
				HTMLNode threadDumpForm = ctx.addFormChild(
					statGatheringContent,
					"/",
					"threadDumpForm"
				);
			...
			if (nodeConfig.config.get("logger").getBoolean("enabled")) logsList
				.addChild("li")
				.addChild(
					"a",
			...
			if (advancedMode) {
				...
				HTMLNode storeSizeInfobox = contentNode.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				if (numberOfConnected + numberOfRoutingBackedOff > 0) {
					HTMLNode loadStatsInfobox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					...
					loadStatsInfobox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					...
					HTMLNode newLoadManagementBox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					...
					// Psuccess box
					HTMLNode successRateBox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					successRateBox.addChild(
						"div",
						"class",
						"infobox-header",
						l10n("successRate")
					);
					HTMLNode successRateContent = successRateBox.addChild(
						"div",
						"class",
						"infobox-content"
					);
					...
					HTMLNode timeDetailBox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					timeDetailBox.addChild(
						"div",
						"class",
						"infobox-header",
						l10n("chkDetailTiming")
					);
					HTMLNode timingsContent = timeDetailBox.addChild(
						"div",
						"class",
						"infobox-content"
					);
					...
					HTMLNode byHTLBox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					byHTLBox.addChild(
						...
						"class",
						"infobox-header",
						l10n("successByHTLBulk")
					);
					HTMLNode byHTLContent = byHTLBox.addChild(
						"div",
						"class",
						"infobox-content"
					);
					...
					byHTLBox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					...
					byHTLContent = byHTLBox.addChild(
						"div",
						"class",
						"infobox-content"
					);
			...
			if (
				advancedMode || numberOfConnected + numberOfRoutingBackedOff > 0
			) {
				// Activity box
				nextTableCell = overviewTableRow.addChild(
					"td",
					"class",
					"last"
				);
				HTMLNode activityInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				if (advancedMode) {
					HTMLNode overviewInfobox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
					drawOverviewBox(
						overviewInfobox,
						nodeUptimeSeconds,
						node.clientCore.bandwidthStatsPutter.getLatestUptimeData()
							.totalUptime,
						now,
						swaps,
						noSwaps
				...
				// Peer statistics box
				HTMLNode peerStatsInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				// Bandwidth box
				HTMLNode bandwidthInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
			...
			if (advancedMode) {
				...
				HTMLNode backoffReasonInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				HTMLNode backoffReasonContent = backoffReasonInfobox.addChild(
					"div",
					"class",
					"infobox-content"
				);

				HTMLNode curBackoffReasonInfobox =
					backoffReasonContent.addChild("div", "class", "infobox");
				...
				if (routingBackoffReasons.length == 0) {
					...
					HTMLNode reasonList = curBackoffReasonContent.addChild(
						"ul"
					);
					for (String routingBackoffReason : routingBackoffReasons) {
						...
						if (reasonCount > 0) {
							reasonList.addChild(
								"li",
								routingBackoffReason + '\u00a0' + reasonCount
							);
						}
					}
				}

				curBackoffReasonInfobox = backoffReasonContent.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				curBackoffReasonContent = curBackoffReasonInfobox.addChild(
					"div",
					"class",
					"infobox-content"
				);

				routingBackoffReasons = peers.getPeerNodeRoutingBackoffReasons(
					true
				);
				if (routingBackoffReasons.length == 0) {
					curBackoffReasonContent.addChild("#", l10n("notBackedOff"));
				...
				HTMLNode row = mandatoryBackoffStatisticsTableBulk.addChild(
					"tr"
				);
				...
				for (NodeStats.TimedStats entry : stats.getMandatoryBackoffStatistics(
					false
				)) {
					...
					row.addChild(
						"td",
						TimeUtil.formatTime(entry.avgTime, 2, true)
					);
					row.addChild(
						"td",
						TimeUtil.formatTime(entry.totalTime, 2, true)
					);
				}
				...
				row.addChild(
					"th",
					l10n("mandatoryBackoffReason") + " (realtime)"
				);
				row.addChild("th", l10n("count"));
				row.addChild("th", l10n("avgTime"));
				row.addChild("th", l10n("totalTime"));
				...
				row.addChild(
					"th",
					l10n("routingBackoffReason") + " (realtime)"
				);
				row.addChild("th", l10n("count"));
				row.addChild("th", l10n("avgTime"));
				row.addChild("th", l10n("totalTime"));
				...
				row.addChild(
					"th",
					l10n("transferBackoffReason") + " (realtime)"
				);
				row.addChild("th", l10n("count"));
				row.addChild("th", l10n("avgTime"));
				row.addChild("th", l10n("totalTime"));
				...
				//Swap statistics box
				HTMLNode locationSwapInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				drawUnclaimedFIFOMessageCountsBox(
					unclaimedFIFOMessageCountsInfobox
				);

				HTMLNode threadsPriorityInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				// thread usage box
				HTMLNode threadUsageInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				threadUsageInfobox.addChild(
					"div",
					"class",
					"infobox-header",
					"Thread usage"
				);
				HTMLNode threadUsageContent = threadUsageInfobox.addChild(
					"div",
					"class",
					"infobox-content"
				);
				...
				if (om != null) {
					...
					if (node.isSeednode()) drawSeedStatsBox(
						nextTableCell.addChild("div", "class", "infobox"),
						om
					);
				}
				...
				HTMLNode peerCircleInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				HTMLNode peerCircleTable = peerCircleInfobox
				...
				HTMLNode nodeCircleInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				HTMLNode nodeCircleTable = nodeCircleInfobox
					.addChild("div", "class", "infobox-content")
					.addChild("table");
				...
				if (incomingRequestsCount > 0) {
					HTMLNode nodeSpecialisationInfobox = nextTableCell.addChild(
						"div",
						"class",
						"infobox"
					);
				...
				int[] outgoingLocalRequestLocation =
					stats.getOutgoingLocalRequestLocation(
				...
				if (
					outgoingLocalRequestsCount > 0 && outgoingRequestsCount > 0
				) {
					HTMLNode nodeSpecialisationInfobox = nextTableCell.addChild(
						"div",
						"class",
				...
				int[] locationSuccessRatesArray =
					stats.chkSuccessRatesByLocation.getPercentageArray(1000);
				HTMLNode nodeSpecialisationInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				...
				addSpecialisation(
					nodeSpecialisationTable,
					myLocation,
					locationSuccessRatesArray
				);
				nextTableCell = overviewTableRow.addChild("td");

				// FOAF link-length distribution box
				HTMLNode foafLinkInfobox = nextTableCell.addChild(
					"div",
					"class",
					"infobox"
				);
				...
				HTMLNode foafLinkTable = foafLinkInfobox
					.addChild("div", "class", "infobox-content")
					.addChild("table");
	...
	private void drawLoadBalancingBox(
		...
		HTMLNode loadStatsContent = loadStatsInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		loadStatsList.addChild(
			"li",
			l10n("realGlobalWindow") + ": " + realWindow
		);
		loadStatsList.addChild(
			"li",
			starters.statsPageLine(false, false, realTime)
		);
		loadStatsList.addChild(
			"li",
			starters.statsPageLine(true, false, realTime)
		);
		loadStatsList.addChild(
			"li",
			starters.statsPageLine(false, true, realTime)
		);
		loadStatsList.addChild(
			"li",
			starters.statsPageLine(true, true, realTime)
		);
	...
	private void drawRejectReasonsBox(HTMLNode nextTableCell, boolean local) {
		...
		if (!success) return;
		HTMLNode rejectReasonsInfobox = nextTableCell.addChild(
			"div",
			"class",
			"infobox"
		);
	...
	private void drawNodeVersionBox(HTMLNode versionInfobox) {
		versionInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("versionTitle")
		);
		HTMLNode versionInfoboxContent = versionInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		node
			.getNodeUpdater()
			.addChangelogLinks(Version.buildNumber(), versionInfobox);
	}

	private void drawJVMStatsBox(
		HTMLNode jvmStatsInfobox,
		boolean advancedModeEnabled
	) {
		jvmStatsInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("jvmInfoTitle")
		);
		HTMLNode jvmStatsInfoboxContent = jvmStatsInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		jvmStatsList.addChild(
			"li",
			l10n("usedMemory", "memory", SizeUtil.formatSize(usedJavaMem, true))
		);
		jvmStatsList.addChild(
			"li",
			l10n(
		...
		jvmStatsList.addChild(
			"li",
			l10n("maxMemory", "memory", SizeUtil.formatSize(maxJavaMem, true))
		);
		jvmStatsList.addChild(
			"li",
			l10n(
				...
				new String[] {
					thousandPoint.format(threadCount),
		...
		jvmStatsList.addChild(
			"li",
			l10n("cpus", "count", Integer.toString(availableCpus))
		);
		jvmStatsList.addChild(
			"li",
		...
		if (advancedModeEnabled) {
			if (Rijndael.AesCtrProvider == null) jvmStatsList.addChild(
				"li",
				l10n("cryptoUsingBuiltin")
			);
			else jvmStatsList.addChild(
				"li",
				l10n("cryptoUsingJCA", "provider", Rijndael.getProviderName())
			);
		}
	}
	...
	private void drawThreadPriorityStatsBox(HTMLNode node) {
		...
		HTMLNode threadsInfoboxContent = node.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		HTMLNode threadsByPriorityTable = threadsInfoboxContent.addChild(
			"table",
			"border",
			"0"
		);
	...
	private void drawOpennetStatsBox(HTMLNode box, OpennetManager om) {
		...
		HTMLNode opennetStatsContent = box.addChild(
			"div",
			"class",
			"infobox-content"
		);
	...
	private void drawClientRequestersBox(HTMLNode box) {
		box.addChild(
			...
			"class",
			"infobox-header",
			l10n("clientRequesterObjects")
		);
		HTMLNode masterContent = box.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		for (ClientRequester request : requests) {
			...
			row.addChild(
				"td",
				client == null ? "?" : Boolean.toString(client.realTimeFlag())
			);
	...
	private void drawStoreSizeBox(
		HTMLNode storeSizeInfobox,
		double loc,
		long nodeUptimeSeconds
	) {
		storeSizeInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("datastore")
		);
		HTMLNode storeSizeInfoboxContent = storeSizeInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);

		HTMLNode scrollDiv = storeSizeInfoboxContent.addChild(
			"div",
			"style",
			"overflow:scr"
		);
		...
		Map<DataStoreInstanceType, DataStoreStats> storeStats =
			node.getDataStoreStats();
		for (Map.Entry<
			DataStoreInstanceType,
			DataStoreStats
		> entry : storeStats.entrySet()) {
			...
			try {
				...
				if (totalAccess != null) {
					try {
						rate +=
						" (" + fix1p4.format(totalAccess.successRate()) + "%)";
					} catch (StatsNotAvailableException e) {
						// Ignore
					}
				}
			...
			String access =
				fix1p2.format(sessionAccess.accessRate(nodeUptimeSeconds)) +
				" /s";
			if (totalAccess != null) access +=
			" (" +
			fix1p2.format(totalAccess.accessRate(totalUptimeSeconds)) +
			" /s)";
			row.addChild("td", access);
			access = fix1p2.format(sessionAccess.writeRate(nodeUptimeSeconds)) +
			" /s";
			if (totalAccess != null) access +=
			" (" +
			fix1p2.format(totalAccess.writeRate(totalUptimeSeconds)) +
			" /s)";
			row.addChild("td", access);
			row.addChild(
				"td",
	...
	private void drawUnclaimedFIFOMessageCountsBox(
		HTMLNode unclaimedFIFOMessageCountsInfobox
	) {
		...
		Map<String, Integer> unclaimedFIFOMessageCountsMap = node
			.getUSM()
		...
		for (Map.Entry<
			String,
			Integer
		> e : unclaimedFIFOMessageCountsMap.entrySet()) {
			String messageName = e.getKey();
			int messageCount = e.getValue();
			totalCount = totalCount + messageCount;
			unclaimedFIFOMessageCountsArray[i++] = new STMessageCount(
				messageName,
				messageCount
			);
		}
	...
	private void drawSwapStatsBox(
		...
		HTMLNode locationSwapInfoboxContent = locationSwapInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (swaps > 0.0) {
			...
			locationSwapList.addChild(
				"li",
				"locChangePerSwap:\u00a0" +
				fix1p6sci.format(locChangeSession / swaps)
			);
		}
	...
	protected static void drawPeerStatsBox(
		...
		HTMLNode peerStatsContent = peerStatsInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (numberOfConnected > 0) {
			HTMLNode peerStatsConnectedListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfRoutingBackedOff > 0) {
			HTMLNode peerStatsRoutingBackedOffListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfTooNew > 0) {
			HTMLNode peerStatsTooNewListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfTooOld > 0) {
			HTMLNode peerStatsTooOldListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfDisconnected > 0) {
			HTMLNode peerStatsDisconnectedListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfNeverConnected > 0) {
			HTMLNode peerStatsNeverConnectedListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfDisabled > 0) {
			HTMLNode peerStatsDisabledListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfBursting > 0) {
			HTMLNode peerStatsBurstingListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfListening > 0) {
			HTMLNode peerStatsListeningListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfListenOnly > 0) {
			HTMLNode peerStatsListenOnlyListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfClockProblem > 0) {
			HTMLNode peerStatsRoutingDisabledListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfSeedServers > 0) {
			HTMLNode peerStatsSeedServersListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfSeedClients > 0) {
			HTMLNode peerStatsSeedClientsListItem = peerStatsList
				.addChild("li")
				.addChild("span");
		...
		if (numberOfNoLoadStats > 0) {
			HTMLNode peerStatsNoLoadStatsListItem = peerStatsList
				.addChild("li")
				.addChild("span");
	...
	private static String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("StatisticsToadlet." + key, patterns, values);
	}
	...
	private void drawActivityBox(
		...
		HTMLNode activityInfoboxContent = activityInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (advancedModeEnabled && activityList != null) {
			if (numARKFetchers > 0) activityList.addChild(
				"li",
	...
	static void drawBandwidth(
		...
		BandwidthStatsContainer stats =
			node.clientCore.bandwidthStatsPutter.getLatestBWData();
		...
		if (delta > 0) {
			...
			int outputBandwidthLimit = nodeConfig.getInt(
		...
		if (isAdvancedModeEnabled) {
			long totalBytesSentCHKRequests =
				node.nodeStats.getCHKRequestTotalBytesSent();
			long totalBytesSentSSKRequests =
				node.nodeStats.getSSKRequestTotalBytesSent();
			...
			long totalBytesSentAnnouncePayload =
				node.nodeStats.getAnnounceBytesPayloadSent();
			...
			long totalBytesSentAllocationNotices =
				node.nodeStats.getAllocationNoticesBytesSent();
			...
			activityList.addChild(
				...
				l10n(
					"ackOnlyBytes",
					"total",
					SizeUtil.formatSize(totalBytesSentAckOnly, true)
				)
			);
			activityList.addChild(
				"li",
			...
			activityList.addChild(
				...
				l10n(
					"foafBytes",
					"total",
					SizeUtil.formatSize(totalBytesSentFOAF, true)
				)
			);
			activityList.addChild(
				"li",
	...
	static HTMLNode drawActivity(HTMLNode activityInfoboxContent, Node node) {
		...
		int numTransferringRequests =
			tracker.getNumTransferringRequestSenders();
		int numTransferringRequestHandlers =
			tracker.getNumTransferringRequestHandlers();
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] {
								...
								Integer.toString(
									numTransferringRequestHandlers
								),
								"0",
							}
						)
				...
				activityList.addChild(
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] { "chk", "ssk" },
							new String[] {
								Integer.toString(numCHKOfferReplys),
			...
			activityList.addChild(
				...
				NodeL10n.getBase()
					.getString(
						...
						new String[] {
							Integer.toString(
								BlockTransmitter.getRunningSends()
							),
							Integer.toString(
								BlockReceiver.getRunningReceives()
							),
						}
					)
			);
	...
	private void drawOverviewBox(
		...
		overviewInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			"Node status overview"
		);
		HTMLNode overviewInfoboxContent = overviewInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (nodeUptimeSeconds > HOURS.toSeconds(24)) {
			darknetSizeEstimate24h = stats.getDarknetSizeEstimate(
				now - HOURS.toMillis(24)
			);
		}
		if (nodeUptimeSeconds > HOURS.toSeconds(48)) {
			darknetSizeEstimate48h = stats.getDarknetSizeEstimate(
				now - HOURS.toMillis(48)
			);
		}
		// Opennet
		...
		if (nodeUptimeSeconds > HOURS.toSeconds(24)) {
			opennetSizeEstimate24h = stats.getOpennetSizeEstimate(
				now - HOURS.toMillis(24)
			);
		}
		if (nodeUptimeSeconds > HOURS.toSeconds(48)) {
			opennetSizeEstimate48h = stats.getOpennetSizeEstimate(
				now - HOURS.toMillis(48)
			);
		}

		double routingMissDistanceLocal =
			stats.routingMissDistanceLocal.currentValue();
		double routingMissDistanceRemote =
			stats.routingMissDistanceRemote.currentValue();
		double routingMissDistanceOverall =
			stats.routingMissDistanceOverall.currentValue();
		double routingMissDistanceBulk =
			stats.routingMissDistanceBulk.currentValue();
		double routingMissDistanceRT =
			stats.routingMissDistanceRT.currentValue();
		...
		overviewList.addChild(
			"li",
			"bwlimitDelayTimeBulk:\u00a0" + bwlimitDelayTimeBulk + "ms"
		);
		overviewList.addChild(
			"li",
			"bwlimitDelayTimeRT:\u00a0" + bwlimitDelayTimeRT + "ms"
		);
		overviewList.addChild(
			"li",
		...
		overviewList.addChild(
			...
			"nodeUptimeSession:\u00a0" +
			TimeUtil.formatTime(
				MILLISECONDS.convert(nodeUptimeSeconds, SECONDS)
			)
		);
		overviewList.addChild(
			"li",
			"nodeUptimeTotal:\u00a0" + TimeUtil.formatTime(nodeUptimeTotal)
		);
		overviewList.addChild(
			"li",
			"routingMissDistanceLocal:\u00a0" +
			fix1p4.format(routingMissDistanceLocal)
		);
		overviewList.addChild(
			"li",
			"routingMissDistanceRemote:\u00a0" +
			fix1p4.format(routingMissDistanceRemote)
		);
		overviewList.addChild(
			"li",
			"routingMissDistanceOverall:\u00a0" +
			fix1p4.format(routingMissDistanceOverall)
		);
		overviewList.addChild(
			"li",
			"routingMissDistanceBulk:\u00a0" +
			fix1p4.format(routingMissDistanceBulk)
		);
		overviewList.addChild(
			"li",
			"routingMissDistanceRT:\u00a0" +
			fix1p4.format(routingMissDistanceRT)
		);
		overviewList.addChild(
			"li",
		...
		overviewList.addChild(
			"li",
			"uptimeAverage:\u00a0" + fix3p1pct.format(node.uptime.getUptime())
		);
		...
		if (decoded != null) {
			overviewList.addChild(
				...
				"packetsDecoded:\u00a0" +
				fix3p1pct.format(
					((double) decoded[0]) / ((double) decoded[1])
				) +
				"\u00a0(" +
				decoded[1] +
				")"
	...
	private void drawBandwidthBox(
		...
		bandwidthInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			l10n("bandwidthTitle")
		);
		HTMLNode bandwidthInfoboxContent = bandwidthInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
	...
	private void getThreadNames(HTMLNode threadUsageList) {
		...
		ThreadBunch[] bunches = map
			.values()
			.toArray(new ThreadBunch[map.size()]);
		Arrays.sort(
			bunches,
		...
		for (ThreadBunch bunch : bunches) {
			thisThreadPercentOfTotal = ((double) bunch.count) /
			((double) totalCount);
	...
	private void addNodeCircle(HTMLNode circleTable, double myLocation) {
		...
		HTMLNode nodeCircleTableCell = nodeCircleTableRow.addChild(
			"td",
		...
		HTMLNode nodeCircleInfoboxContent = nodeCircleTableCell.addChild(
			"div",
		...
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0, false, 1.0),
				"mark",
			},
			"|"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.125, false, 1.0),
				"mark",
			},
			"+"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.25, false, 1.0),
				"mark",
			},
			"--"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.375, false, 1.0),
				"mark",
			},
			"+"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.5, false, 1.0),
				"mark",
			},
			"|"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.625, false, 1.0),
				"mark",
			},
			"+"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.75, false, 1.0),
				"mark",
			},
			"--"
		);
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(0.875, false, 1.0),
				"mark",
			},
			"+"
		);
		...
		for (int i = 0; i < locations.length; i++) {
			...
			histogramIndex = (int) (Math.floor(
					location.doubleValue() * HISTOGRAM_LENGTH
				));
		...
		nodeCircleInfoboxContent.addChild(
			...
			new String[] { "style", "class" },
			new String[] {
				generatePeerCircleStyleString(myLocation, true, 1.0),
				"me",
			},
			"x"
		);
		...
		for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
			nodeHistogramLegendCell = nodeHistogramLegendTableRow.addChild(
				"td"
			);
			nodeHistogramGraphCell = nodeHistogramGraphTableRow.addChild(
				"td",
				"style",
				"height: 100px;"
			);
	...
	private void addSpecialisation(
		...
		for (int i = 0; i < incomingRequestLocation.length; i++) {
			...
			HTMLNode nodeHistogramGraphCell =
				nodeHistogramGraphTableRow.addChild(
					"td",
					"style",
					"height: 100px;"
				);
			HTMLNode nodeHistogramGraphCell2 = nodeHistogramLegendCell.addChild(
				"div",
				"class",
				"histogramLabel"
			);
			if (i == myIndex) nodeHistogramGraphCell2 =
	...
	private void addCombinedSpecialisation(
		...
		int histogramDiv = combinedHistogramDivisor(
			locallyOriginatingRequests,
	...
	private void addFOAFLinkLengthHistogram(
		HTMLNode circleTable,
		PeerNodeStatus[] peerNodeStatuses
	) {
		...
		for (PeerNodeStatus pns : peerNodeStatuses) {
			...
			for (double foafLoc : foafLocs) {
				...
				int idx = (int) Math.floor(
					(Location.distance(peerLoc, foafLoc) * HISTOGRAM_LENGTH) /
					0.5
				);
		...
		for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
			peerHistogramLegendCell = peerHistogramLegendTableRow.addChild(
				"td"
			);
			peerHistogramGraphCell = peerHistogramGraphTableRow.addChild(
				"td",
				"style",
				"height: 100px;"
			);
			...
			double histogramFraction =
				((double) peersLinkHistogram[i]) / peersLinkCount;
			peerHistogramGraphCell.addChild(
				...
				new String[] {
					"histogramConnected",
					"height: " +
					fix3pctUS.format(histogramFraction) +
					"; width: 100%;",
				},
				"\u00a0"
			);
			peerHistogramGraphCell.addChild(
				...
				new String[] {
					"histogramDisconnected",
					"height: " +
					fix3pctUS.format(cumulativeFraction) +
					"; width: 100%;",
				},
				"\u00a0"
			);
	...
	private void addPeerCircle(
		...
		HTMLNode peerHistogramLegendTableRow = circleTable.addChild("tr");
		HTMLNode peerHistogramGraphTableRow = circleTable.addChild("tr");
		HTMLNode peerCircleTableCell = peerCircleTableRow.addChild(
			"td",
			new String[] { "class", "colspan" },
			new String[] { "first", "10" }
		);
		HTMLNode peerHistogramLegendCell;
		HTMLNode peerHistogramGraphCell;
		HTMLNode peerCircleInfoboxContent = peerCircleTableCell.addChild(
			"div",
			new String[] { "style", "class" },
			new String[] {
				"position: relative; height: " +
	...
	private String generatePeerCircleStyleString(
		...
		double y =
			PEER_CIRCLE_RADIUS -
			Math.cos(peerLocation) * (PEER_CIRCLE_RADIUS - offset); // no PEER_CIRCLE_ADDITIONAL_FREE_SPACE for y-disposition
		//
		return (
			"position: absolute; top: " +
			fix3p1US.format(y) +
