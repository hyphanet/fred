...
public class ContentFilterToadlet
		...
		try {
			...
			if (request.isPartSet("filter-local")) {
				try {
					...
					String mimeType = request.getPartAsStringFailsafe(
						"mime-type",
						100
					);
	...
	private HTMLNode createContent(PageMaker pageMaker, ToadletContext ctx) {
		...
		HTMLNode filterForm = ctx.addFormChild(
			filterContent,
			PATH,
			"filterForm"
		);
	...
	private void handleFilterRequest(
		...
		try {
			...
			Bucket bucket;
			if (localFile) {
				filename = request.getPartAsStringFailsafe(
					"filename",
	...
	private String makeResultFilename(
		...
		if (p > 0) {
			filteredFilename = originalFilename.substring(0, p) +
			".filtered" +
			originalFilename.substring(p);
		} else {
	...
	private void handleFilter(
		...
		try {
			FilterStatus status = applyFilter(
				data,
				resultBucket,
				mimeType,
				operation,
		...
		if (unsafe) {
			sendErrorPage(
				ctx,
				200,
				l10n("errorUnsafeContentTitle"),
				l10n("errorUnsafeContent")
			);
		} else {
	...
	private FilterStatus applyFilter(
		...
		try {
			...
			Logger.error(
				ContentFilterToadlet.class,
				"Inexplicable URI error",
				e
			);
			return null;
		}
