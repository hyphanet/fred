...
public class ExternalLinkToadlet extends Toadlet {
	...
	public void handleMethodPOST(
		...
		String url = request.getPartAsStringFailsafe(
		...
		if (
			request.getPartAsStringFailsafe("Go", 32).isEmpty() || url.isEmpty()
		) {
	...
	public void handleMethodGET(
		...
		boolean renderBars = node.clientCore
			.getToadletContainer()
		...
		PageNode page = ctx
			...
			.getPageNode(
				...
				new RenderParameters()
					.renderNavigationLinks(renderBars)
					.renderStatus(renderBars)
			);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
		HTMLNode warnboxContent = ctx
			.getPageMaker()
			.getInfobox(
				"infobox-warning",
		...
		HTMLNode externalLinkForm = ctx.addFormChild(
			warnboxContent,
			PATH,
	...
	public static String escape(String uri) {
		return (
			ExternalLinkToadlet.PATH + "?" + magicHTTPEscapeString + '=' + uri
		);
	}
