...
public class DarknetConnectionsToadlet extends ConnectionsToadlet {
	...
	protected class DarknetComparator extends ComparatorByStatus {
		...
		protected int customCompare(
			...
			if (sortBy.equals("name")) {
				...
				int ret =
					((DarknetPeerNodeStatus) firstNode).getOurVisibility()
						.compareTo(
	...
	protected void drawVisibilityColumn(
		...
		String content =
			((DarknetPeerNodeStatus) peerNodeStatus).getOurVisibility().name();
		if (advancedModeEnabled) content +=
		" (" +
	...
	protected void drawPeerActionSelectBox(
		...
		peerForm.addChild(
			...
			new String[] {
				"submit",
				"doSendMessageToPeers",
				l10n("sendConfidentialMessage"),
			}
		);
		peerForm.addChild("br");

		HTMLNode actionSelect = peerForm.addChild(
			"select",
			new String[] { "id", "name" },
		...
		if (advancedModeEnabled) {
			...
			actionSelect.addChild(
				...
				"value",
				"set_burst_only",
				l10n("peersSetBurstOnly")
			);
			actionSelect.addChild(
				"option",
				"value",
				"clear_burst_only",
				l10n("peersClearBurstOnly")
			);
			actionSelect.addChild(
				"option",
				"value",
				"set_listen_only",
				l10n("peersSetListenOnly")
			);
			actionSelect.addChild(
				"option",
				"value",
				"clear_listen_only",
				l10n("peersClearListenOnly")
			);
			actionSelect.addChild(
				"option",
				"value",
				"set_allow_local",
				l10n("peersSetAllowLocal")
			);
			actionSelect.addChild(
				"option",
				"value",
				"clear_allow_local",
				l10n("peersClearAllowLocal")
			);
			actionSelect.addChild(
				"option",
				"value",
			...
			actionSelect.addChild(
				...
				"value",
				"set_dont_route",
				l10n("peersSetDontRoute")
			);
			actionSelect.addChild(
				"option",
				"value",
				"clear_dont_route",
				l10n("peersClearDontRoute")
			);
		}
		...
		peerForm.addChild(
			...
			new String[] {
				"submit",
				"doChangeTrust",
				l10n("changeTrustButton"),
			}
		);
		HTMLNode changeTrustLevelSelect = peerForm.addChild(
			"select",
			new String[] { "id", "name" },
		...
		peerForm.addChild(
			...
			new String[] {
				"submit",
				"doChangeVisibility",
				l10n("changeVisibilityButton"),
			}
		);
		HTMLNode changeVisibilitySelect = peerForm.addChild(
			"select",
			new String[] { "id", "name" },
	...
	protected void handleAltPost(
		...
		if (request.isPartSet("doSendMessageToPeers")) {
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("sendMessageTitle"), ctx);
			...
			request.isPartSet("doAction") &&
			request
				.getPartAsStringFailsafe("action", 25)
				.equals("set_burst_only")
		) {
			//int hashcode = Integer.decode(request.getParam("node")).intValue();
			...
			request
				.getPartAsStringFailsafe("action", 25)
				.equals("clear_ignore_source_port")
		) {
			//int hashcode = Integer.decode(request.getParam("node")).intValue();
			...
			request.isPartSet("changeTrust") &&
			request.isPartSet("doChangeTrust")
		) {
			FRIEND_TRUST trust = FRIEND_TRUST.valueOf(
				request.getPartAsStringFailsafe("changeTrust", 10)
			);
			DarknetPeerNode[] peerNodes = node.getDarknetConnections();
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("node_" + pn.hashCode())) {
			...
			request.isPartSet("changeVisibility") &&
			request.isPartSet("doChangeVisibility")
		) {
				...
				if (request.isPartSet("node_" + pn.hashCode())) {
					if (
						...
						if (logMINOR) Logger.minor(
							this,
							"Removed node: node_" + pn.hashCode()
						);
					} else {
						if (logMINOR) Logger.minor(
							this,
						...
						PageNode page = ctx
							.getPageMaker()
							.getPageNode(l10n("confirmRemoveNodeTitle"), ctx);
						HTMLNode pageNode = page.outer;
						HTMLNode contentNode = page.content;
					...
					if (logMINOR) Logger.minor(
						this,
						"Part not set: node_" + pn.hashCode()
					);
				}
			}
			redirectHere(ctx);
			...
			for (DarknetPeerNode pn : peerNodes) {
				if (request.isPartSet("node_" + pn.hashCode())) {
					long id = Long.parseLong(
						request.getPartAsStringFailsafe("id", 32)
	...
	private void redirectHere(ToadletContext ctx)
	...
	private boolean tryHandlePeerNoderef(
		URI uri,
		HTTPRequest request,
		ToadletContext ctx
	) throws ToadletContextClosedException, IOException {
		...
		if (path.endsWith(".fref") && path.startsWith(path() + "friend-")) {
			...
			String input_hashcode_string = path.substring(
				(path() + "friend-").length()
			);
			input_hashcode_string = input_hashcode_string.substring(
				0,
			...
			String filename = FileUtil.sanitizeFileNameWithExtras(
				peernode_name + ".fref",
				"\" "
			);
