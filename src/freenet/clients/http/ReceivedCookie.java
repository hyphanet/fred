...
public final class ReceivedCookie extends Cookie {
	...
	protected static ArrayList<ReceivedCookie> parseHeader(String httpHeader)
		...
		if (logMINOR) Logger.minor(
			ReceivedCookie.class,
			"Received HTTP cookie header:" + httpHeader
		);

		char[] header = httpHeader.toCharArray();

		String currentCookieName = null;
		Hashtable<String, String> currentCookieContent = new Hashtable<
			String,
			String
		>(16);
		...
		try {
			for (int i = 0; i < header.length;) {
				...
				// Parse key
				{
					...
					if (
						keyEndIndex >= header.length ||
						header[keyEndIndex] == ';'
					) value = "";

					while (
						Character.isWhitespace(header[keyEndIndex - 1])
					) --keyEndIndex; // Remove trailing whitespace
					...
					if (key.length() == 0) throw new ParseException(
						"Invalid cookie: Contains an empty key: " + httpHeader,
						i
					);
				...
				if (value == null && i < header.length) {
					...
					if (header[i] == '\"') { // Value is quoted
						...
						while (
							i < header.length && header[i] != valueEndChar
						) ++i;
					}
					...
					while (
						...
						Character.isWhitespace(header[valueEndIndex - 1])
					) --valueEndIndex; // Remove trailing whitespace
				...
				if (currentCookieName == null) { // We have not found the name yet, the first key/value pair must be the name and the value of the cookie.
					...
					if (key.charAt(0) == '$') currentCookieContent.put(
						key,
						value
					);
						...
						); // Store the previous cookie.

						currentCookieName = key;
						currentCookieContent = new Hashtable<String, String>(
							16
						);
						currentCookieContent.put(currentCookieName, value);
					}
		...
		if (currentCookieName != null) cookies.add(
			new ReceivedCookie(currentCookieName, currentCookieContent)
		);
