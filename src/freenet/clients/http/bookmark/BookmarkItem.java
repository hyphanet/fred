...
public class BookmarkItem extends Bookmark {
	...
	private String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("BookmarkItem." + key, patterns, values);
	}
	...
	public synchronized boolean setEdition(long ed, NodeClientCore node) {
		if (key.getSuggestedEdition() >= ed) {
			if (logMINOR) Logger.minor(
				this,
				"Edition " + ed + " is too old, not updating " + key
			);
			return false;
		}
		...
		if (o instanceof BookmarkItem) {
			...
			if (!b.key.equals(key)) {
				if ("USK".equals(b.key.getKeyType())) {
					if (
						!b.key
							.setSuggestedEdition(key.getSuggestedEdition())
							.equals(key)
					) {
						return false;
					}
	...
	public String getDescription() {
		if (desc == null) return "";
		if (desc.toLowerCase().startsWith("l10n:")) return NodeL10n.getBase()
			.getString(
				"Bookmarks.Defaults.Description." +
	...
	public String getShortDescription() {
		...
		if (
			shortDescription.toLowerCase().startsWith("l10n:")
		) return NodeL10n.getBase()
			.getString(
