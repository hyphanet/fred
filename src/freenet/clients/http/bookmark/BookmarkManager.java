...
public class BookmarkManager implements RequestClient {
	...
	static {
		...
		try {
			...
			try (InputStream in = loader.getResourceAsStream(name)) {
				...
				if (in != null) defaultBookmarks = SimpleFieldSet.readFrom(
					in,
					false,
					false
				);
			...
			Logger.error(
				...
				"Error while loading the default bookmark file from " +
				name +
				" :" +
				e.getMessage(),
				e
	...
	public BookmarkManager(NodeClientCore n, boolean publicGateway) {
		...
		try {
			...
			if (
				!bookmarksFile.exists() || bookmarksFile.length() == 0
			) throw new IOException();
			Logger.normal(
				this,
				"Attempting to read the bookmark file from " +
				bookmarksFile.toString()
			);
			SimpleFieldSet sfs = SimpleFieldSet.readFrom(
				bookmarksFile,
				false,
				true
			);
			...
			Logger.error(
				this,
				"Error reading the bookmark file (" +
				bookmarksFile.toString() +
				"):" +
				ioe.getMessage(),
				ioe
			...
			try {
				if (
					backupBookmarksFile.exists() &&
					backupBookmarksFile.canRead() &&
					backupBookmarksFile.length() > 0
				) {
					Logger.normal(
						this,
						"Attempting to read the backup bookmark file from " +
				...
				Logger.error(
					this,
					"Error reading the backup bookmark file !" + e.getMessage(),
					e
				);
			}
		}
	...
	public void reAddDefaultBookmarks() {
		BookmarkCategory bc = new BookmarkCategory(
			l10n("defaultBookmarks") + " - " + new Date()
		);
		addBookmark("/", bc);
	...
	private class USKUpdatedCallback implements USKCallback {
		...
		public void onFoundEdition(
			...
			if (!newKnownGood) {
				...
				node
					.makeClient(PRIORITY_PROGRESS, false, false)
					.prefetch(
						uri,
						MINUTES.toMillis(60),
			...
			if (updated) {
				...
				Logger.error(
					this,
					"No match for bookmark " + key + " edition " + edition
				);
			}
		}
	...
	public BookmarkItem getItemByPath(String path) {
		if (
			getBookmarkByPath(path) instanceof BookmarkItem
	...
	public void addBookmark(String parentPath, Bookmark bookmark) {
		if (logMINOR) Logger.minor(
			this,
			"Adding bookmark " + bookmark + " to " + parentPath
		);
		...
		if (bookmark instanceof BookmarkItem) subscribeToUSK(
	...
	public void removeBookmark(String path) {
		...
		if (bookmark instanceof BookmarkCategory) {
			...
			for (int i = 0; i < cat.size(); i++) removeBookmark(
				path +
				cat.get(i).getName() +
	...
	private void putPaths(String path, Bookmark b) {
		...
		if (b instanceof BookmarkCategory) for (
			int i = 0;
			i < ((BookmarkCategory) b).size();
			i++
		) {
	...
	private void removePaths(String path) {
		if (getBookmarkByPath(path) instanceof BookmarkCategory) {
			...
			for (int i = 0; i < cat.size(); i++) removePaths(
				path +
				cat.get(i).getName() +
				(cat.get(i) instanceof BookmarkCategory ? "/" : "")
			);
	...
	public void storeBookmarks() {
		Logger.normal(
			this,
			"Attempting to save bookmarks to " + bookmarksFile.toString()
		);
		...
		try {
			...
			fos = null;
			if (
				!FileUtil.renameTo(backupBookmarksFile, bookmarksFile)
			) Logger.error(
				this,
				"Unable to rename " +
				backupBookmarksFile.toString() +
			...
			Logger.error(
				this,
				"An error has occured saving the bookmark file :" +
				ioe.getMessage(),
				ioe
			);
		} finally {
	...
	private void subscribeToUSK(BookmarkItem item) {
		if ("USK".equals(item.getKeyType())) try {
			USK u = item.getUSK();
			...
			if (sfs == null) {
				...
				try {
					...
					for (int i = 0; i < nbBookmarks; i++) {
						SimpleFieldSet subset = sfs.getSubset(
							BookmarkItem.NAME + i
						);
						try {
					...
					for (int i = 0; i < nbCategories; i++) {
						SimpleFieldSet subset = sfs.getSubset(
							BookmarkCategory.NAME + i
						);
						BookmarkCategory currentCategory = new BookmarkCategory(
							subset
						);
	...
	public SimpleFieldSet toSimpleFieldSet() {
		...
		synchronized (bookmarks) {
			sfs.putAllOverwrite(
				BookmarkManager.toSimpleFieldSet(MAIN_CATEGORY)
			);
		}
	...
	public static SimpleFieldSet toSimpleFieldSet(BookmarkCategory cat) {
		...
		for (int i = 0; i < bi.size(); i++) sfs.put(
			BookmarkItem.NAME + i,
