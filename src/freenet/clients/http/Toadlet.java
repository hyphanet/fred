...
public abstract class Toadlet {
    ...
    FetchResult fetch(FreenetURI uri, long maxSize, RequestClient clientContext, FetchContext fctx)
        ...
        @SuppressWarnings("unused") ClientGetter getter = client.fetch(uri, 1, clientContext, fw, fctx);
     ...
     */
    protected void writeHTMLReply(ToadletContext ctx, int code, String desc, MultiValueTable<String, String> headers,
                                  String reply, boolean forceDisableJavascript)
            throws ToadletContextClosedException, IOException {
    ...
    static void writePermanentRedirect(ToadletContext ctx, String msg, String location)
        ...
        String redirDoc = "<html><head><title>" + msg + "</title></head><body><h1>"
                          + l10n("permRedirectWithReason", "reason", msg) + "</h1><a href=\""
                          + HTMLEncoder.encode(location) + "\">" + l10n("clickHere") + "</a></body></html>";
        byte[] buf;
    ...
    protected void writeTemporaryRedirect(ToadletContext ctx, String msg, String location)
        ...
        String redirDoc = "<html><head><title>" + msg + "</title></head><body><h1>"
                          + l10n("tempRedirectWithReason", "reason", msg) + "</h1><a href=\""
                          + HTMLEncoder.encode(location) + "\">" + l10n("clickHere") + "</a></body></html>";
        byte[] buf;
