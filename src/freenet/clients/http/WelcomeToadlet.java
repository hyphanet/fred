...
public class WelcomeToadlet extends Toadlet {
        ...
        if (items.size() > 0) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if (item.hasAnActivelink() &&!noActiveLinks) {
                    ...
                    cell.addChild("a", "href", '/' + item.getKey()).addChild("img", new String[] { "src", "height",
                            "width", "alt", "title" }, new String[] { key, "36", "108", "activelink",
                            item.getDescription() });
                } else {
                    cell.addChild("#", " ");
                }
                ...
                cell.addChild("a", new String[] { "href", "title", "class" }, new String[] { '/' + item.getKey(),
                        item.getDescription(), "bookmark-title" }, item.getVisibleName());
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (request.getPartAsStringFailsafe("updateconfirm", 32).length() > 0) {
            ...
            updateForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "cancel",
                    NodeL10n.getBase().getString("Toadlet.cancel") });
            updateForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
                    "updateconfirm", l10n("update") });
            writeHTMLReply(ctx, 200, "OK", pageNode.generate());
            ...
            if (node.isUsingWrapper()) {
                ctx.getPageMaker().getInfobox("#", l10n("threadDumpSubTitle"), contentNode, "thread-dump-generation",
                                              true).addChild("#",
                ...
                ctx.getPageMaker().getInfobox("infobox-error", l10n("threadDumpSubTitle"), contentNode,
                                              "thread-dump-generation", true).addChild("#",
            ...
            try {
                ...
                NodeL10n.getBase().addL10nSubstitution(content, "WelcomeToadlet.keyInsertedSuccessfullyWithKeyAndName",
                        new String[] { "link",
                                       "name" }, new HTMLNode[] { HTMLNode.link("/" + u), HTMLNode.text(u) });
            ...
            shutdownForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "cancel",
                    NodeL10n.getBase().getString("Toadlet.cancel") });
            shutdownForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
                    "shutdownconfirm", l10n("shutdown") });
            writeHTMLReply(ctx, 200, "OK", pageNode.generate());
            ...
            restartForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "cancel",
                    NodeL10n.getBase().getString("Toadlet.cancel") });
            restartForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
                    "restartconfirm", l10n("restart") });
            writeHTMLReply(ctx, 200, "OK", pageNode.generate());
    ...
    public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (ctx.isAllowedFullAccess()) {
            if (request.isParameterSet("latestlog")) {
                ...
                addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "key",
                        key });

                if (request.isParameterSet("hasAnActivelink")) {
                    addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden",
                            "hasAnActivelink", request.getParam("hasAnActivelink") });
                }

                addForm.addChild("label", "for", "name",
                                 NodeL10n.getBase().getString("BookmarkEditorToadlet.nameLabel") + ' ');
                addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "text", "name",
                        request.getParam("desc") });
                addForm.addChild("br");
                addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "bookmark",
                        "/" });
                addForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "action",
                        "addItem" });
                addForm.addChild("label", "for", "descB",
                                 NodeL10n.getBase().getString("BookmarkEditorToadlet.descLabel") + ' ');
                addForm.addChild("br");
                addForm.addChild("textarea", new String[] { "id", "name", "row", "cols" }, new String[] { "descB",
                        "descB", "3", "70" });
                ...
                if (node.getDarknetConnections().length > 0) {
                    ...
                    peerTable.addChild("th", "colspan", "2",
                                       NodeL10n.getBase().getString("BookmarkEditorToadlet.recommendToFriends"));
        ...
        bookmarkBoxHeader.addChild("a", new String[] { "class", "title" }, new String[] { "bookmarks-header-text",
                NodeL10n.getBase().getString(
                    "BookmarkEditorToadlet.myBookmarksExplanation") }, NodeL10n.getBase().getString(
        ...
        if (ctx.isAllowedFullAccess()) {
            ...
            bookmarkBoxHeader.addChild("span", "id", "bookmarkedit").addChild("a", new String[] { "href", "class" },
                                       new String[] { "/bookmarkEditor/",
                    "interfacelink" }, NodeL10n.getBase().getString("BookmarkEditorToadlet.edit"));
            bookmarkBoxHeader.addChild("span", "class", "edit-bracket", "]");
        }
        ...
        // Search form
        if ((node.pluginManager != null) && node.pluginManager.isPluginLoaded("plugins.Library.Main")) {
            ...
            searchForm.addChild("input", new String[] { "type", "size", "name" }, new String[] { "text", "80",
                    "search" });
            searchForm.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "find",
                    l10n("searchFreenet") });

            // Search must be in a new window so that the user is able to browse the bookmarks.
            searchForm.addAttribute("target", "_blank");
        } else if ((node.pluginManager == null) || node.pluginManager.isPluginLoadedOrLoadingOrWantLoad("Library")) {
        ...
        if (ctx.isAllowedFullAccess()) {
            ...
            shutdownForm.addChild("input", new String[] { "type", "value" }, new String[] { "submit",
                    l10n("shutdownNode") });
    ...
    private void putFetchKeyBox(ToadletContext ctx, HTMLNode contentNode) {
        ...
        HTMLNode fetchKeyForm = fetchKeyContent.addChild("form", new String[] { "action", "method" },
                                    new String[] { "/",
                "get" }).addChild("div");
    ...
    static HTMLNode sendRestartingPageInner(ToadletContext ctx) {
        ...
        ctx.getPageMaker().getInfobox("infobox-information", l10n("restartingTitle"), contentNode,
                                      "shutdown-progressing", true).addChild("#", l10n("restarting"));
