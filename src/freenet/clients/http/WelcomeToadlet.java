...
public class WelcomeToadlet extends Toadlet {
	...
	void redirectToRoot(ToadletContext ctx)
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.hasAnActivelink() && !noActiveLinks) {
					...
					String key =
						...
						"activelink.png";
					cell
						.addChild("div", "style", "height: 36px; width: 108px;")
	...
	public void addSearchBox(HTMLNode contentNode) {
		...
		HTMLNode searchBox = contentNode.addChild(
			"div",
			"class",
		...
		HTMLNode searchBoxContent = searchBox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		...
		if (showSearchBox()) {
			...
			searchBoxContent.addChild(
				...
				"class",
				"search-warning-text",
				l10n("searchBoxWarningSlow")
			);
			HTMLNode searchForm = container.addFormChild(
				searchBoxContent,
				"/library/",
				"searchform"
			);
			...
			HTMLNode textSpan = searchBoxContent.addChild(
				"span",
				"class",
				"search-not-availible-warning"
			);
			NodeL10n.getBase()
				.addL10nSubstitution(
					textSpan,
	...
	public void handleMethodPOST(
		...
		if (request.getPartAsStringFailsafe("updateconfirm", 32).length() > 0) {
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("updatingTitle"), ctx);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("nodeUpdateConfirmTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content = ctx
				.getPageMaker()
			...
			HTMLNode updateForm = ctx.addFormChild(
				content,
				"/",
				"updateConfirmForm"
			);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("threadDumpTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			...
			for (UserAlert alert : alerts) {
				if (request.getIntPart("disable", -1) == alert.hashCode()) {
					...
					if (
						...
						Logger.normal(
							this,
							"Unregistering the userAlert " + alert.hashCode()
						);
						ctx.getAlertManager().unregister(alert);
					} else {
						Logger.normal(
							this,
							"Disabling the userAlert " + alert.hashCode()
						);
						alert.isValid(false);
					}
			...
			FreenetURI key = new FreenetURI(
				request.getPartAsStringFailsafe("key", Short.MAX_VALUE)
			);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("insertedTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content;
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("shutdownConfirmTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content = ctx
				.getPageMaker()
			...
			HTMLNode shutdownForm = ctx.addFormChild(
				content.addChild("p"),
				"/",
				"confirmShutdownForm"
			);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("restartConfirmTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			HTMLNode content = ctx
				.getPageMaker()
			...
			HTMLNode restartForm = ctx.addFormChild(
				content.addChild("p"),
				"/",
				"confirmRestartForm"
			);
			...
			if (!ctx.checkFormPassword(request)) return;
			String alertsToDump = request.getPartAsStringFailsafe(
				"events",
				Integer.MAX_VALUE
			);
			...
			try {
				int outputBandwidthLimit = Fields.parseInt(
					request.getPartAsStringFailsafe(
						"outputBandwidthLimit",
						Byte.MAX_VALUE
					)
				);
				BandwidthManager.checkOutputBandwidthLimit(
					outputBandwidthLimit
				);
			} catch (NumberFormatException e) {
				errorMessage = NodeL10n.getBase()
					.getString(
						"UpgradeConnectionSpeedUserAlert.InvalidValue",
						"type",
						"upload"
					);
			...
			try {
				int inputBandwidthLimit = Fields.parseInt(
					request.getPartAsStringFailsafe(
						"inputBandwidthLimit",
						Byte.MAX_VALUE
					)
	...
	public void handleMethodGET(
		...
		if (ctx.isAllowedFullAccess()) {
			if (request.isParameterSet("latestlog")) {
				...
				if (
					(!request.isParameterSet("formPassword")) ||
					!request
						.getParam("formPassword")
						.equals(ctx.getFormPassword())
				) {
					redirectToRoot(ctx);
					return;
				}
				...
				PageNode page = ctx
					.getPageMaker()
					.getPageNode(l10n("confirmAddBookmarkTitle"), ctx);
				HTMLNode pageNode = page.outer;
				HTMLNode contentNode = page.content;
				...
				HTMLNode addForm = ctx.addFormChild(
					infoboxContent,
					"/bookmarkEditor/",
					"editBookmarkForm"
				);
				...
				if (node.getDarknetConnections().length > 0) {
					...
					HTMLNode peerTable = addForm.addChild(
						"table",
						"class",
						"darknet_connections"
					);
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10n("homepageFullTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
		...
		// Bookmarks
		HTMLNode bookmarkBox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal bookmarks-box"
		);
		HTMLNode bookmarkBoxHeader = bookmarkBox.addChild(
			"div",
			"class",
			"infobox-header"
		);
		bookmarkBoxHeader.addChild(
			...
			new String[] {
				...
				NodeL10n.getBase()
					.getString("BookmarkEditorToadlet.myBookmarksExplanation"),
			},
			NodeL10n.getBase()
		...
		HTMLNode bookmarkBoxContent = bookmarkBox.addChild(
			"div",
			"class",
			"infobox-content"
		);

		HTMLNode bookmarksList = bookmarkBoxContent.addChild(
			"ul",
			"id",
			BOOKMARKS_ANCHOR
		);
		...
		if (
			!node.config.get("fproxy").getBoolean("fetchKeyBoxAboveBookmarks")
		) {
			this.putFetchKeyBox(ctx, contentNode);
		}

		// Version info and Quit Form
		HTMLNode versionContent = ctx
			.getPageMaker()
			.getInfobox(
				"infobox-information",
				l10n("versionHeader"),
	...
	private void putFetchKeyBox(ToadletContext ctx, HTMLNode contentNode) {
		...
		HTMLNode fetchKeyForm = fetchKeyContent
			.addChild(
				"form",
				new String[] { "method" },
				new String[] { "POST" }
	...
	public static void maybeDisplayWrapperLogfile(
		ToadletContext ctx,
		...
		if (logs.exists() && logs.isFile() && logs.canRead() && (logSize > 0)) {
			HTMLNode logInfoboxContent = ctx
				.getPageMaker()
				.getInfobox(
					"infobox-info",
					"Current status",
	 ...
	 */
	private static String readLogTail(File logfile, long byteLimit)
		throws IOException {
