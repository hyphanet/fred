...
public class SecurityLevelsToadlet extends Toadlet {
	...
	public void handleMethodPOST(
		...
		if (request.isPartSet("seclevels")) {
			...
			String networkThreatLevel = request.getPartAsStringFailsafe(
				configName,
				128
			);
			...
			if (newThreatLevel != null) {
				if (
					...
					if (
						!request.isPartSet(confirm) &&
						!request.isPartSet(tryConfirm)
					) {
						HTMLNode warning =
							node.securityLevels.getConfirmWarning(
								newThreatLevel,
								confirm
							);
						if (warning != null) {
							PageNode page = ctx
								...
								.getPageNode(
									NodeL10n.getBase()
										.getString("ConfigToadlet.fullTitle"),
									ctx
								);
							pageNode = page.outer;
							content = page.content;
							formNode = ctx.addFormChild(
								content,
								".",
								"configFormSecLevels"
							);
							...
							HTMLNode infobox = seclevelGroup.addChild(
								"div",
								"class",
			...
			String physicalThreatLevel = request.getPartAsStringFailsafe(
				configName,
				128
			);
			...
			if (newPhysicalLevel != null) {
				if (
					newPhysicalLevel == oldPhysicalLevel &&
					newPhysicalLevel == PHYSICAL_THREAT_LEVEL.HIGH
				) {
					String password = request.getPartAsStringFailsafe(
						"masterPassword",
						MAX_PASSWORD_LENGTH
					);
					String oldPassword = request.getPartAsStringFailsafe(
						"oldPassword",
						MAX_PASSWORD_LENGTH
					);
					String confirmPassword = request.getPartAsStringFailsafe(
				...
				if (newPhysicalLevel != oldPhysicalLevel) {
					...
					if (
						...
						if (
							...
							try {
								...
								else core.node.setMasterPassword(
									password,
									false
								);
							} catch (AlreadySetPasswordException e) {
					...
					if (
						(newPhysicalLevel == PHYSICAL_THREAT_LEVEL.LOW ||
						...
						if (!password.isEmpty()) {
							...
							try {
								...
								if (
									!core.node.getMasterPasswordFile().exists()
								) {
									// Ok.
									System.out.println(
									...
									System.err.println(
										"Cannot change password as cannot write new passwords file: " +
										e
									);
									e.printStackTrace();
									String msg =
										"<html><head><title>" +
										l10nSec(
											"cantWriteNewMasterKeysFileTitle"
										) +
										"</title></head><body><h1>" +
										l10nSec(
											"cantWriteNewMasterKeysFileTitle"
			...
			if (request.isPartSet("masterPassword")) {
				String masterPassword = request.getPartAsStringFailsafe(
					"masterPassword",
					1024
				);
				...
				if (request.isPartSet("redirect")) {
					String to = request.getPartAsStringFailsafe(
						"redirect",
						100
					);
					if (to.startsWith("/")) {
	...
	private void sendCantDeleteMasterKeysFile(
		ToadletContext ctx,
		String physicalSecurityLevel
	) throws ToadletContextClosedException, IOException {
		HTMLNode pageNode = sendCantDeleteMasterKeysFileInner(
			ctx,
	...
	public static HTMLNode sendCantDeleteMasterKeysFileInner(
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10nSec("cantDeletePasswordFileTitle"), ctx);
	...
	private static void sendCantDeleteMasterKeysFileInner(
		...
		content.addChild(
			"p",
			l10nSec("cantDeletePasswordFile", "filename", filename)
		);
	}
	...
	private void sendChangePasswordForm(
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10nSec("changePasswordTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
	...
	private void sendPasswordPage(
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10nSec("setPasswordTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
	...
	private static void addBackToSeclevelsLink(HTMLNode content) {
		content
			.addChild("p")
			.addChild("a", "href", PATH, l10nSec("backToSecurityLevels"));
	}
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(
				NodeL10n.getBase().getString("SecurityLevelsToadlet.fullTitle"),
				ctx
			);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
	...
	private void drawSecurityLevelsPage(
		...
		HTMLNode configNode = infobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode formNode = ctx.addFormChild(
			configNode,
			".",
			"configFormSecLevels"
		);
		...
		seclevelGroup
			.addChild("p")
			.addChild("b", l10nSec("networkThreatLevel.opennetFriendsWarning"));
				...
				new HTMLNode[] {
					...
					HTMLNode.linkInNewWindow(
						ExternalLinkToadlet.escape(
							l10nSec("physicalThreatLevelFDELink")
						)
					),
				}
			);
		...
		if (os == FileUtil.OperatingSystem.Windows) {
			swapWarning.addChild(
				"#",
				" " + WizardL10n.l10nSec("physicalThreatLevelSwapfileWindows")
			);
		}
		...
		for (PHYSICAL_THREAT_LEVEL level : PHYSICAL_THREAT_LEVEL.values()) {
			...
			if (level == PHYSICAL_THREAT_LEVEL.MAXIMUM && node.hasDatabase()) {
				inner.addChild(
					"b",
					" " + l10nSec("warningMaximumWillDeleteQueue")
				);
			}
	...
	private void addPasswordChangeForm(HTMLNode inner) {
		...
		cell.addChild(
			...
			"for",
			"confirmPasswordBox",
			l10nSec("confirmNewPasswordLabel")
		);
		cell = row.addChild("td");
		cell.addChild(
			"input",
	...
	private static String l10nSec(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("SecurityLevels." + key, pattern, value);
	}
	...
	void sendPasswordFileCorruptedPage(
		...
		HTMLNode page = sendPasswordFileCorruptedPageInner(
			tooBig,
			ctx,
			forSecLevels,
			forFirstTimeWizard,
	...
	public static HTMLNode sendPasswordFileCorruptedPageInner(
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10nSec("passwordFileCorruptedTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
		HTMLNode infoBox = ctx
			.getPageMaker()
			.getInfobox(
				"infobox-error",
				l10nSec("passwordFileCorruptedTitle"),
	...
	private static void sendPasswordFileCorruptedPageInner(
		...
		infoBox.addChild(
			"p",
			l10nSec("passwordFileCorrupted", "file", masterPasswordFile)
		);
	...
	private void sendPasswordFormPage(boolean wasWrong, ToadletContext ctx)
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10nSec("passwordPageTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
	...
	public static void generatePasswordFormPage(
		...
		String postTo = forFirstTimeWizard
