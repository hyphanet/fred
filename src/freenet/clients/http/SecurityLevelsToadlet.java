...
public class SecurityLevelsToadlet extends Toadlet {
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (request.isPartSet("seclevels")) {
            ...
            if (newThreatLevel != null) {
                if (newThreatLevel != node.securityLevels.getNetworkThreatLevel()) {
                    if (!request.isPartSet(confirm) &&!request.isPartSet(tryConfirm)) {
                        ...
                        if (warning != null) {
                            PageNode page =
                                ctx.getPageMaker().getPageNode(NodeL10n.getBase().getString("ConfigToadlet.fullTitle"),
                                                               ctx);
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "New physical threat level: " + newPhysicalLevel + " old = "
            ...
            if (newPhysicalLevel != null) {
                ...
                if (newPhysicalLevel != oldPhysicalLevel) {
                    ...
                    if ((newPhysicalLevel == PHYSICAL_THREAT_LEVEL.HIGH)
                        ...
                        if ((password != null) && (password.length() > 0)) {
                            try {
                                ...
                                content = ctx.getPageMaker().getInfobox("infobox-error", l10nSec("passwordWrongTitle"),
                                        contentNode, "wrong-password", true).addChild("div", "class",
                                            "infobox-content");
                                SecurityLevelsToadlet.generatePasswordFormPage(true, ctx.getContainer(), content,
                                        false, false, true, newPhysicalLevel.name(), null);
                            ...
                            sendPasswordPage(ctx, (password != null) && (password.length() == 0),
                    ...
                    if (((newPhysicalLevel == PHYSICAL_THREAT_LEVEL.LOW) || (newPhysicalLevel == PHYSICAL_THREAT_LEVEL
                            .NORMAL)) && (oldPhysicalLevel == PHYSICAL_THREAT_LEVEL.HIGH)) {
                        ...
                        if ((password != null) && (password.length() > 0)) {
                            ...
                            try {
                                ...
                                if (!core.node.getMasterPasswordFile().exists()) {
                                    ...
                                    System.err.println("Cannot change password as cannot write new passwords file: "
                                                       + e);
                                    e.printStackTrace();

                                    String msg = "<html><head><title>" + l10nSec("cantWriteNewMasterKeysFileTitle")
                                                 + "</title></head><body><h1>"
                                                 + l10nSec("cantWriteNewMasterKeysFileTitle") + "</h1><p>"
                                ...
                                content = ctx.getPageMaker().getInfobox("infobox-error", l10nSec("passwordWrongTitle"),
                                        contentNode, "wrong-password", true).addChild("div", "class",
                                            "infobox-content");
                                SecurityLevelsToadlet.generatePasswordFormPage(true, ctx.getContainer(), content,
                                        false, true, false, newPhysicalLevel.name(), null);
                                addBackToSeclevelsLink(content);
                                writeHTMLReply(ctx, 200, "OK", pageNode.generate());
                            ...
                            content = ctx.getPageMaker().getInfobox("infobox-error",
                                    l10nSec("passwordForDecryptTitle"), contentNode, "password-prompt",
                                    false).addChild("div", "class", "infobox-content");
            ...
            if (pageNode != null) {
                formNode.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden",
                        "seclevels", "on" });
    ...
    public static HTMLNode sendCantDeleteMasterKeysFileInner(ToadletContext ctx, String filename,
        ...
        HTMLNode form = forFirstTimeWizard
    ...
    private static void sendCantDeleteMasterKeysFileInner(HTMLNode content, HTMLNode form, String filename,
        ...
        form.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden",
                "security-levels.physicalThreatLevel", physicalSecurityLevel });
        form.addChild("input", new String[] { "type", "name", "value" },
                      new String[] { "hidden", "seclevels", "true" });
        form.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "tryAgain",
                l10nSec("cantDeletePasswordFileButton") });
    ...
    private void sendChangePasswordForm(ToadletContext ctx, boolean wrongPassword, boolean emptyPassword,
        ...
        HTMLNode pageNode = page.outer;
        HTMLNode contentNode = page.content;
        HTMLNode content = ctx.getPageMaker().getInfobox("infobox-error", l10nSec("changePasswordTitle"), contentNode,
                               "password-change", true).addChild("div", "class", "infobox-content");
        ...
        if (physicalSecurityLevel != null) {
            form.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden",
                    "security-levels.physicalThreatLevel", physicalSecurityLevel });
            form.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "seclevels",
                    "true" });
        }
    ...
    private void sendPasswordPage(ToadletContext ctx, boolean emptyPassword, String threatlevel)
        ...
        HTMLNode pageNode = page.outer;
        HTMLNode contentNode = page.content;
        HTMLNode content = ctx.getPageMaker().getInfobox("infobox-error", l10nSec("setPasswordTitle"), contentNode,
                               "password-prompt", false).addChild("div", "class", "infobox-content");
    ...
    private void drawSecurityLevelsPage(HTMLNode contentNode, ToadletContext ctx) {
        ...
        for (NETWORK_THREAT_LEVEL level : NETWORK_THREAT_LEVEL.getOpennetValues()) {
            ...
            if (level == networkLevel) {
                input = div.addChild("p").addChild("input", new String[] { "type", "checked", "name", "value" },
                                     new String[] { "radio",
                        "on", controlName, level.name() });
            } else {
                input = div.addChild("p").addChild("input", new String[] { "type", "name", "value" },
                                     new String[] { "radio",
                        controlName, level.name() });
            }
        ...
        for (NETWORK_THREAT_LEVEL level : NETWORK_THREAT_LEVEL.getDarknetValues()) {
            ...
            NodeL10n.getBase().addL10nSubstitution(inner, "SecurityLevels.networkThreatLevel.desc." + level,
                    new String[] { "bold",
                                   "link" }, new HTMLNode[] { HTMLNode.STRONG,
                                   HTMLNode.link("/wizard/?step=OPENNET") });
        }
        ...
        NodeL10n.getBase().addL10nSubstitution(seclevelGroup.addChild("p").addChild("i"),
                "SecurityLevels.physicalThreatLevelTruecrypt", new String[] { "bold",
                "truecrypt" }, new HTMLNode[] { HTMLNode.STRONG,
        ...
        swapWarning.addChild("#",
                             NodeL10n.getBase().getString("SecurityLevels.physicalThreatLevelSwapfile",
        ...
        for (PHYSICAL_THREAT_LEVEL level : PHYSICAL_THREAT_LEVEL.values()) {
            ...
            if (level == physicalLevel) {
                input = seclevelGroup.addChild("p").addChild("input", new String[] { "type", "checked", "name",
                        "value" }, new String[] { "radio", "on", controlName, level.name() });
            } else {
                input = seclevelGroup.addChild("p").addChild("input", new String[] { "type", "name", "value" },
                                               new String[] { "radio",
                        controlName, level.name() });
            }
            ...
            if ((level != PHYSICAL_THREAT_LEVEL.LOW) && (physicalLevel == PHYSICAL_THREAT_LEVEL.LOW)
                    && node.hasDatabase() &&!node.isDatabaseEncrypted()) {
            ...
            } else if ((level == PHYSICAL_THREAT_LEVEL.LOW) && (physicalLevel != PHYSICAL_THREAT_LEVEL.LOW)
                       && node.hasDatabase() && node.isDatabaseEncrypted()) {
                if (node.autoChangeDatabaseEncryption()) {
            ...
            if (level == PHYSICAL_THREAT_LEVEL.HIGH) {
                if (physicalLevel == level) {
                    ...
                    p.addChild("input", new String[] { "id", "type", "name" }, new String[] { "passwordBox", "password",
                            "masterPassword" });
                }
            }
        }
        ...
        formNode.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "seclevels",
                "on" });
        formNode.addChild("input", new String[] { "type", "value" }, new String[] { "submit", l10n("apply") });
        formNode.addChild("input", new String[] { "type", "value" }, new String[] { "reset", l10n("undo") });
    }
    ...
    private void addPasswordChangeForm(HTMLNode inner) {
        ...
        cell.addChild("input", new String[] { "id", "type", "name", "size" }, new String[] { "oldPasswordBox",
                "password", "oldPassword", "100" });
        row = table.addChild("tr");
        cell = row.addChild("td");
        cell.addChild("label", "for", "newPasswordBox", l10nSec("newPasswordLabel"));
        cell = row.addChild("td");
        cell.addChild("input", new String[] { "id", "type", "name", "size" }, new String[] { "passwordBox", "password",
                "masterPassword", "100" });

        HTMLNode p = inner.addChild("p");

        p.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit", "changePassword",
                l10nSec("changePasswordButton") });
    }
    ...
    public static void generatePasswordFormPage(boolean wasWrong, HTMLNode formNode, HTMLNode content,
        ...
        formNode.addChild("input", new String[] { "type", "name", "size" }, new String[] { "password", "masterPassword",
                "100" });
        ...
        if (redirect != null) {
            formNode.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "redirect",
                    redirect });
        }

        formNode.addChild("input", new String[] { "type", "value" }, new String[] { "submit",
                l10nSec("passwordSubmit") });
    }
}
