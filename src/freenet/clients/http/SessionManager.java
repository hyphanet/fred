...
public final class SessionManager {
    ...
    private synchronized void removeExpiredSessions(long time) {
        for (Session session = mSessionsByID.peekValue(); (session != null) && session.isExpired(time);
    ...
    private synchronized void verifyQueueOrder() {
        ...
        while (sessions.hasMoreElements()) {
            ...
            if (session.getExpirationTime() < previousTime) {
                long sessionAge = HOURS.convert(CurrentTimeUTC.getInMillis() - session.getExpirationTime(),
                                                MILLISECONDS);

                Logger.error(this,
                             "Session LRU queue out of order! Found session which is " + sessionAge + " hour old: "
                             + session);
