...
public final class SessionManager {
	...
	public SessionManager(URI myCookiePath) {
		if (myCookiePath.isAbsolute()) throw new IllegalArgumentException(
			"Illegal cookie path, must be relative: " + myCookiePath
		);

		if (
			myCookiePath.toString().startsWith("/") == false
		) throw new IllegalArgumentException(
			"Illegal cookie path, must start with /: " + myCookiePath
		);
	...
	public SessionManager(String myCookieNamespace) {
		if (myCookieNamespace.length() == 0) throw new IllegalArgumentException(
	...
	private UUID getSessionID(ToadletContext context) {
		...
		try {
			ReceivedCookie sessionCookie = context.getCookie(
				null,
				mCookiePath,
				mCookieName
			);
			...
			return sessionCookie == null
				? null
				: UUID.fromString(sessionCookie.getValue());
			...
			Logger.error(
				this,
				"Getting the value of the session cookie failed",
				e
			);
			return null;
		}
	...
	private synchronized void removeExpiredSessions(long time) {
		for (
			Session session = mSessionsByID.peekValue();
	...
	private synchronized void verifySessionsByUserIDTable() {
		...
		while (sessions.hasMoreElements()) {
			...
			if (mSessionsByID.containsKey(session.getID()) == false) {
				Logger.error(
					this,
					"Sessions by user ID hashtable contains deleted session, removing it: " +
					session
				);
