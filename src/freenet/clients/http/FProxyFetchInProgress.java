...
public class FProxyFetchInProgress
	implements ClientEventListener, ClientGetCallback {
	...
	public FProxyFetchInProgress(
		...
		this.fctx = fctx;
		this.rc = rc;
		FetchContext alteredFctx = new FetchContext(
			fctx,
		...
		getter = new ClientGetter(
			this,
			uri,
			alteredFctx,
			FProxyToadlet.PRIORITY,
			null,
			null,
			null
		);
	...
	synchronized FProxyFetchResult innerGetResult(boolean hasWaited) {
		...
		if (data != null) res = new FProxyFetchResult(
			this,
			data,
			mimeType,
			timeStarted,
	...
	public void start(ClientContext context) throws FetchException {
		try {
			...
			synchronized (this) {
				this.failed = new FetchException(
	...
	private boolean checkCache(ClientContext context) {
		...
		CacheFetchResult result = context.downloadCache == null
			? null
			: context.downloadCache.lookupInstant(
				uri,
				!fctx.filterData,
				false,
				null
		...
		if ((!fctx.filterData) && (!result.alreadyFiltered)) {
			if (
				fctx.overrideMIME == null ||
				fctx.overrideMIME.equals(result.getMimeType())
			) {
				// Works as-is.
				...
				onSuccess(
					new FetchResult(
						new ClientMetadata(fctx.overrideMIME),
						result.asBucket()
					),
					null
				);
			...
			if (
				refilterPolicy == REFILTER_POLICY.RE_FETCH || !fctx.filterData
			) {
				// Can't use it.
				return false;
		...
		if (mimeType == null || mimeType.isEmpty()) mimeType =
			DefaultMIMETypes.DEFAULT_MIME_TYPE;
		if (fctx.overrideMIME != null && !result.alreadyFiltered) mimeType =
			fctx.overrideMIME;
		else if (
			fctx.overrideMIME != null && !mimeType.equals(fctx.overrideMIME)
		) {
			// Doesn't work.
			return false;
		}
		...
		if (type == null || ((!type.safeToRead) && type.readFilter == null)) {
			UnknownContentTypeException e = new UnknownContentTypeException(
				mimeType
			);
			data.free();
			onFailure(
			...
			tracker.removeFetcher(this);
			onSuccess(
				new FetchResult(new ClientMetadata(mimeType), data),
				null
			);
			return true;
		} else {
			...
			try {
				...
				this.onSuccess(
						new FetchResult(
							new ClientMetadata(fullMimeType),
							output
						),
						null
					);
				...
				Logger.normal(
					this,
					"Failed filtering coalesced data in fproxy"
				);
				// Failed. :|
				// Let it run normally.
				return false;
	...
	private boolean shouldAcceptCachedFilteredData(
		FetchContext fctx,
		...
		if (fctx.overrideMIME == null) {
			...
			if (fctx.overrideMIME.equals(finalMIME)) return true;
				...
				ContentFilter.stripMIMEType(finalMIME).equals(
					fctx.overrideMIME
				) &&
				fctx.charset == null
			) return true;
		...
		try {
			if (ce instanceof SplitfileProgressEvent) {
				...
				synchronized (this) {
					int oldReq =
						requiredBlocks -
						(fetchedBlocks + failedBlocks + fatallyFailedBlocks);
			...
			for (FProxyFetchListener l : new ArrayList<FProxyFetchListener>(
				listener
			)) {
				l.onEvent();
			}
	...
	public synchronized boolean canCancel() {
		...
		if (
			lastTouched + LIFETIME >= System.currentTimeMillis() &&
			!requestImmediateCancel
		) {
			if (logMINOR) Logger.minor(
				this,
				"Not able to cancel for " + this + " : " + uri + " : " + maxSize
			);
			return false;
		}
		if (logMINOR) Logger.minor(
			this,
			"Can cancel for " + this + " : " + uri + " : " + maxSize
		);
		return true;
	}
	...
	public void finishCancel() {
		if (logMINOR) Logger.minor(
			this,
			"Finishing cancel for " + this + " : " + uri + " : " + maxSize
		);
		try {
	...
	public synchronized long getETA() {
		...
		return (
			((System.currentTimeMillis() - timeStarted) *
				(requiredBlocks - fetchedBlocksPreNetwork)) /
	...
	public synchronized boolean notFinishedOrFatallyFinished() {
		...
		if (
			failed != null &&
			(System.currentTimeMillis() - timeFailed < 1000 || fetched < 2)
		) return true; // Once for javascript and once for the user when it re-pulls.
		return false;
	}
	...
	public boolean fetchContextEquivalent(FetchContext context) {
		...
		if (
			this.fctx.charset != null &&
			!this.fctx.charset.equals(context.charset)
		) return false;
		if (
			this.fctx.overrideMIME == null && context.overrideMIME != null
		) return false;
		if (
			this.fctx.overrideMIME != null &&
			!this.fctx.overrideMIME.equals(context.overrideMIME)
		) return false;
		return true;
	}
