...
public class FirstTimeWizardNewToadlet extends WebTemplateToadlet {
	...
	private void showForm(ToadletContext ctx, Map<String, Object> model)
		...
		PageNode page = ctx
			...
			.getPageNode(
				l10n("homepageTitle"),
				ctx,
				new PageMaker.RenderParameters()
	...
	private class FormModel {
		...
		private final String minStorageLimit = String.format(
			Locale.ENGLISH,
			"%.2f",
		...
		FormModel() {
			...
			Option<Long> sizeOption = (Option<Long>) config
				.get("node")
				.getOption("storeSize");
			if (!sizeOption.isDefault()) {
				@SuppressWarnings("unchecked")
				Option<Long> clientCacheSizeOption = (Option<Long>) config
					.get("node")
					.getOption("clientCacheSize");
				@SuppressWarnings("unchecked")
				Option<Long> slashdotCacheSizeOption = (Option<Long>) config
					.get("node")
					.getOption("slashdotCacheSize");
				long totalSize =
					sizeOption.getValue() +
				...
				long autodetectedDatastoreSize =
					DatastoreUtil.autodetectDatastoreSize(core, config);
				if (autodetectedDatastoreSize > 0) {
					storage = (float) autodetectedDatastoreSize /
		...
		FormModel(HTTPRequest request) {
			...
			if (haveMonthlyLimit.isEmpty()) {
				try {
					long downloadLimit = this.downloadLimit.isEmpty()
						? 0
						: Fields.parseInt(this.downloadLimit + "KiB");
					...
					errors.put(
						"downloadLimitError",
						l10n("valid.number.prefix.downloadLimit") +
						" " +
						e.getMessage()
					);
				}
				...
				try {
					long uploadLimit = this.uploadLimit.isEmpty()
						? 0
						: Fields.parseInt(this.uploadLimit + "KiB");
					...
					if (nanosInSecond < uploadLimit) { // see Node set outputBandwidthLimit
						errors.put(
							"uploadLimitError",
							l10n(
								"valid.uploadLimitMax",
								Integer.toString(nanosInSecond / KiB)
							)
						);
					}
				} catch (NumberFormatException e) {
					errors.put(
						"uploadLimitError",
						l10n("valid.number.prefix.uploadLimit") +
						" " +
						e.getMessage()
					);
				}
				...
				try {
					...
					if (monthlyLimit < BandwidthLimit.minMonthlyLimit) {
						errors.put(
							...
							l10n(
								...
								String.format(
									"%.2f",
									BandwidthLimit.minMonthlyLimit
								)
							)
						);
					}
					...
					errors.put(
						"bandwidthMonthlyLimitError",
						l10n("valid.number.prefix.bandwidthMonthlyLimit") +
						" " +
						e.getMessage()
					);
				}
			...
			try {
				long storageLimit = this.storageLimit.isEmpty()
					? 0
					: Fields.parseLong(this.storageLimit + "GiB");
				if (storageLimit < MIN_STORAGE_LIMIT) { // min store size + 10% for client cache + 10% for slashdot cache
					errors.put(
						"storageLimitError",
						NodeL10n.getBase()
							.getString("Node.invalidMinStoreSizeWithCaches")
					);
				} else {
				...
				errors.put(
					"storageLimitError",
					l10n("valid.number.prefix.storageLimit") +
					" " +
					e.getMessage()
				);
			}
			...
			if (!setPassword.isEmpty()) {
				if (password.isEmpty()) {
					errors.put(
						"passwordError",
						NodeL10n.getBase()
							.getString("SecurityLevels.passwordNotZeroLength")
					);
				}
				if (
					password.length() >
					...
					errors.put(
						"passwordError",
						NodeL10n.getBase()
							.getString("SecurityLevels.passwordTooLong")
					);
				}
				if (!password.equals(passwordConfirmation)) {
					errors.put(
						"passwordError",
						NodeL10n.getBase()
							.getString("SecurityLevels.passwordsDoNotMatch")
					);
				}
			}
		}
		...
		private void detectBandwidthLimit() {
			try {
				...
				downloadLimitDetected = Long.toString(
					detected.downBytes / 2 / KiB
				);
		...
		private Map<String, Object> toModel() {
			...
			model.put(
				"connectToStrangers",
				connectToStrangers.length() > 0 ? "checked" : ""
			);
			model.put(
				"haveMonthlyLimit",
				haveMonthlyLimit.length() > 0 ? "checked" : ""
			);
		...
		private void save() {
			if (knowSomeone.isEmpty()) {
				...
				core.node.securityLevels.setThreatLevel(
					SecurityLevels.NETWORK_THREAT_LEVEL.NORMAL
				);
			} else {
				if (connectToStrangers.isEmpty()) {
					// Darknet
					core.node.securityLevels.setThreatLevel(
						SecurityLevels.NETWORK_THREAT_LEVEL.HIGH
					);
				} else {
					// Opennet + Darknet
					core.node.securityLevels.setThreatLevel(
			...
			try {
				if (haveMonthlyLimit.isEmpty()) { // save download & uploadLimit
					config
						.get("node")
						.set("inputBandwidthLimit", downloadLimit + "KiB");
					config
						.get("node")
						.set("outputBandwidthLimit", uploadLimit + "KiB");
				} else { // save bandwidthMonthlyLimit
					BandwidthLimit bandwidth = new BandwidthLimit(
						Fields.parseLong(bandwidthMonthlyLimit + "GiB")
					);
					config
						.get("node")
						.set(
							"inputBandwidthLimit",
							Long.toString(bandwidth.downBytes)
						);
					config
						.get("node")
						.set(
							"outputBandwidthLimit",
							Long.toString(bandwidth.upBytes)
						);
				}
			...
			if (!isPasswordAlreadySet) {
				try {
					...
					Logger.error(
						this,
						"Should not happen, please report! " + e,
						e
					);
				}
			}
