	...
	*/
	...
	public boolean equals(Object obj) {
		...
		if (myDomain != null) {
			if (
				otherDomain == null ||
				!otherDomain.toString().equals(myDomain.toString())
			) return false;
		} else if (otherDomain != null) return false;

		if (
			!getPath().toString().equals(other.getPath().toString())
		) return false;
	...
	public static URI validateDomain(String domainString)
		throws URISyntaxException {
	...
	public static URI validateDomain(URI domain) {
		...
		if (
			...
			"Illegal cookie domain, must be http or https: " + domain
		);
		...
		if (
			...
			"Illegal cookie domain, contains a path: " + domain
		);

		return domain;
	}

	public static URI validatePath(String stringPath)
		throws URISyntaxException {
	...
	public static URI validatePath(URI path) {
		...
		if (path.isAbsolute()) throw new IllegalArgumentException(
			"Illegal cookie path, must be relative: " + path
		);

		if (
			path.toString().startsWith("/") == false
		) throw new IllegalArgumentException(
			"Illegal cookie path, must start with /: " + path
		);
	...
	public static String validateName(String name) {
		...
		if (!isUSASCII(name)) throw new IllegalArgumentException(
			"Invalid name, contains non-US-ASCII characters: " + name
		);
		...
		*/
		...
		for (Character c : name.toCharArray()) {
			if (Character.isWhitespace(c)) throw new IllegalArgumentException(
				"Invalid name, contains whitespace: " + name
			);

			// From isISOControl javadoc: A character is considered to be an ISO control character if its in the range [0,31] or [127,159]
			if (Character.isISOControl(c)) throw new IllegalArgumentException(
				"Invalid name, contains control characters."
			);

			if (
				httpSeparatorCharacters.contains(c)
			) throw new IllegalArgumentException(
				"Invalid name, contains one of the explicitely disallowed characters: " +
				name
			);
		}
	...
	public static String validateValue(String value) {
		if (!isUSASCII(value)) throw new IllegalArgumentException(
		...
		*/
		...
		for (Character c : value.toCharArray()) {
			...
			if (
				invalidValueCharacters.contains(c)
			) throw new IllegalArgumentException(
				"Invalid value, contains one of the explicitely disallowed characters: " +
				value
			);
		}
	...
	public static Date validateExpirationDate(Date expirationDate) {
		if (
			...
			"Illegal expiration date, is in past: " + expirationDate
		);
