...
public class TranslationToadlet extends Toadlet {
	...
	public void handleMethodGET(
		...
		if (request.isParameterSet("getOverrideTranlationFile")) {
			SimpleFieldSet sfs =
				this.base.getOverrideForCurrentLanguageTranslation();
			if (sfs == null) {
				super.sendErrorPage(
					ctx,
			...
			head.put(
				"Content-Disposition",
				"attachment; filename=\"" +
				this.base.getL10nOverrideFileName(
						this.base.getSelectedLanguage()
					) +
				'"'
			);
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("translationUpdatedTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;

			HTMLNode translationNode = contentNode.addChild(
				"div",
				"class",
				"translation"
			);
			HTMLNode legendTable = translationNode.addChild(
				"table",
				"class",
				"translation"
			);
			...
			legendRow.addChild(
				...
				"class",
				"translation-key",
				l10n("translationKeyLabel")
			);
			legendRow.addChild(
				"td",
				"class",
				"translation-key",
				l10n("originalVersionLabel")
			);
			legendRow.addChild(
				"td",
				"class",
				"translation-key",
				l10n("currentTranslationLabel")
			);
			...
			HTMLNode footer = translationNode.addChild(
				"div",
				"class",
				"warning"
			);
			footer
				.addChild(
					"a",
					"href",
					TOADLET_URL + "?getOverrideTranlationFile"
				)
			...
			footer
				.addChild(
					"a",
					"href",
					TOADLET_URL + (showEverything ? "" : "?toTranslateOnly")
				)
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("translationUpdateTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			...
			HTMLNode legendTable = updateForm.addChild(
				"table",
				"class",
				"translation"
			);
			...
			if (!showEverything) updateForm.addChild(
				"input",
				new String[] { "type", "name", "value" },
			...
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("removeOverrideTitle"), ctx);
			HTMLNode pageNode = page.outer;
			HTMLNode contentNode = page.content;
			...
			HTMLNode removeForm = ctx.addFormChild(
				content.addChild("p"),
				TOADLET_URL,
				"remove_confirmed"
			);
			if (!showEverything) removeForm.addChild(
				"input",
				new String[] { "type", "name", "value" },
				new String[] { "hidden", "toTranslateOnly", key }
			);
		...
		final HTMLNode translatingForBox = ctx
			.getPageMaker()
			.getInfobox(null, l10n("selectTranslation"), contentNode);
		...
		final HTMLNode translatingForForm = ctx
			.addFormChild(
				translatingForBox,
				TOADLET_URL,
				"ChooseWhatToTranslate"
			)
			.addChild("p", l10n("showTranslationOf"));
		final HTMLNode translatingForOption = translatingForForm.addChild(
			"select",
			"name",
			"translating_for"
		);
		for (String element : elementsToTranslate) {
			final HTMLNode option = translatingForOption.addChild(
				"option",
				"name",
				element,
				element
		...
		KeyIterator it =
			this.base.getDefaultLanguageTranslation().keyIterator("");
		...
		while (it.hasNext()) {
			...
			if (
				!showEverything &&
				(isOverriden || (this.base.getString(key, true) != null))
			) continue;
			HTMLNode contentRow = legendTable.addChild("tr");
			contentRow.addChild("td", "class", "translation-key", key);
			contentRow.addChild(
			...
			contentRow
				.addChild("td", "class", "translation-new")
				.addChild(
					_setOrRemoveOverride(key, isOverriden, showEverything)
				);
		}
	...
	public void handleMethodPOST(
		...
		if (request.isPartSet("translating_for")) {
			final String translateFor = request.getPartAsStringFailsafe(
				"translating_for",
				255
			);
			...
			for (PluginInfoWrapper pluginInfo : this.core.node.pluginManager.getPlugins()) {
				if (
					translateFor.equals(pluginInfo.getPluginClassName()) &&
					pluginInfo.isBaseL10nPlugin()
				) {
					FredPluginBaseL10n plugin =
						(FredPluginBaseL10n) pluginInfo.getPlugin();
		...
		if (
			request.getPartAsStringFailsafe("translation_update", 32).length() >
			0
		) {
			...
			this.base.setOverride(
					key,
					new String(
						BucketTools.toByteArray(request.getPart("trans")),
			...
			if (
				"on".equalsIgnoreCase(
						request.getPartAsStringFailsafe("gotoNext", 7)
					)
			) {
				KeyIterator it = base
				...
				while (it.hasNext()) {
					...
					if (
						isOverriden ||
						(this.base.getString(newKey, true) != null)
					) continue;
					redirectTo(
						ctx,
						TOADLET_URL +
			...
			request.getPartAsStringFailsafe("remove_confirmed", 32).length() > 0
		) {
			String key = request
				.getPartAsStringFailsafe("remove_confirm", 256)
				.trim();
			...
			return;
		} else redirectTo(ctx, "/"); // Shouldn't reach that point!
	}
	...
	private HTMLNode _setOrRemoveOverride(
		String key,
		boolean isOverriden,
		...
		HTMLNode translationField = new HTMLNode(
			"span",
			"class",
		...
		if (value == null) {
			...
			if (isOverriden) translationField
				.addChild(
					"a",
					"href",
					TranslationToadlet.TOADLET_URL +
