...
public class ConfigToadlet extends Toadlet implements LinkEnabledCallback {
	...
	public void handleMethodPOST(
		...
		if (request.isPartSet("confirm-reset-to-defaults")) {
			PageNode page = ctx
				.getPageMaker()
				.getPageNode(l10n("confirmResetTitle"), ctx);
			...
			String subconfig = request.getPartAsStringFailsafe(
				"subconfig",
		...
		// not applied.
		if (
			request.isPartSet(LocalFileBrowserToadlet.selectDir) ||
		...
		String prefix = request.getPartAsStringFailsafe(
			"subconfig",
			MAX_PARAM_VALUE_SIZE
		);
		if (logMINOR) {
		...
		for (Option<?> o : config.get(prefix).getOptions()) {
			...
			if (logMINOR) {
				Logger.minor(
					this,
					"Checking option " + prefix + '.' + configName
				);
			}
			...
			if (request.isPartSet(prefix + '.' + configName)) {
				...
				if (resetToDefault) {
					...
					// shortcuts etc.
					if (
						prefix.equals("fproxy") && configName.equals("port")
					) continue;
					value = o.getDefault();
				} else {
					value = request.getPartAsStringFailsafe(
						prefix + '.' + configName,
						MAX_PARAM_VALUE_SIZE
					);
				}
		...
		if (request.isPartSet(wrapperConfigName)) {
			...
			if (
				!WrapperConfig.getWrapperProperty(wrapperConfigName).equals(
					value
				)
			) {
				if (logMINOR) {
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(l10n("appliedTitle"), ctx);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
	...
	public void handleMethodGET(URI uri, HTTPRequest req, ToadletContext ctx)
		...
		PageNode page = ctx
			.getPageMaker()
			.getPageNode(
				NodeL10n.getBase().getString("ConfigToadlet.fullTitle"),
				ctx
			);
		HTMLNode pageNode = page.outer;
		HTMLNode contentNode = page.content;
		 ...
		 */
		if (
			subConfig.getPrefix().equals("node") &&
			...
			if (req.isPartSet(configName)) {
				curValue = req.getPartAsStringFailsafe(
					configName,
					MAX_PARAM_VALUE_SIZE
				);
			}
			if (curValue != null) {
				...
				item
					.addChild("span", "class", "configlongdesc")
					.addChild(
						NodeL10n.getBase()
							.getHTMLNode(
		...
		if (
			req.isPartSet("select-for") &&
			req.isPartSet(LocalFileBrowserToadlet.selectDir)
		) {
			overriddenOption = req.getPartAsStringFailsafe(
				"select-for",
				MAX_PARAM_VALUE_SIZE
			);
			overriddenValue = req.getPartAsStringFailsafe(
				"filename",
				MAX_PARAM_VALUE_SIZE
			);
		}
		...
		for (Option<?> o : subConfig.getOptions()) {
			if (!((!advancedModeEnabled) && o.isExpert())) {
				...
				if (value == null) {
					Logger.error(
						this,
						fullName + "has returned null from config!);"
					);
					continue;
				}
				...
				if (callback instanceof EnumerableOptionCallback) {
					...
					callback instanceof ProgramDirectory.DirectoryCallback &&
					!callback.isReadOnly()
				) {
				...
				configItemNode
					...
					.addChild(shortDesc);
				HTMLNode configItemValueNode = configItemNode.addChild(
					"span",
					"class",
					"config"
				);
				...
				if (req.isPartSet(fullName)) {
					value = req.getPartAsStringFailsafe(
						fullName,
						MAX_PARAM_VALUE_SIZE
					);
				}
				if (
					overriddenOption != null &&
				...
				switch (optionType) {
					case DROP_DOWN:
						configItemValueNode.addChild(
							addComboBox(
								value,
								(EnumerableOptionCallback) callback,
								fullName,
								callback.isReadOnly()
							)
						);
					...
					case DIRECTORY:
						configItemValueNode.addChild(
							addTextBox(value, fullName, o, false)
						);
						configItemValueNode.addChild(
							"input",
							new String[] { "type", "name", "value" },
					...
					case TEXT_READ_ONLY:
						configItemValueNode.addChild(
							addTextBox(value, fullName, o, true)
						);
						break;
					case TEXT:
						configItemValueNode.addChild(
	 ...
	 *         value of the option.
	 */
	public static HTMLNode addTextBox(
		String value,
		String fullName,
		Option<?> o,
		boolean disabled
	 ...
	 */
	public static HTMLNode addBooleanComboBox(
		boolean value,
		String fullName,
		boolean disabled
	) {
