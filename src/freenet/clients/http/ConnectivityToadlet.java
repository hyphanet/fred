...
public class ConnectivityToadlet extends Toadlet {
	...
	public void handleMethodGET(
		...
		PageNode page = pageMaker.getPageNode(
			NodeL10n.getBase().getString("ConnectivityToadlet.title"),
			ctx
		);
		...
		// our ports
		HTMLNode portInfobox = contentNode.addChild(
			"div",
			"class",
			"infobox infobox-normal"
		);
		portInfobox.addChild(
			"div",
			"class",
			"infobox-header",
			l10nConn("nodePortsTitle")
		);
		HTMLNode portInfoboxContent = portInfobox.addChild(
			"div",
			"class",
			"infobox-content"
		);
		HTMLNode portInfoList = portInfoboxContent.addChild("ul");
		SimpleFieldSet fproxyConfig = node.config
			.get("fproxy")
			.exportFieldSet(true);
		SimpleFieldSet fcpConfig = node.config.get("fcp").exportFieldSet(true);
		SimpleFieldSet tmciConfig = node.config
			.get("console")
			.exportFieldSet(true);
		...
		if (opennetPort > 0) portInfoList.addChild(
			"li",
			NodeL10n.getBase()
				.getString(
				...
				portInfoList.addChild(
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] { "port" },
							new String[] {
								Integer.toString(fproxyConfig.getInt("port")),
							}
						)
				);
			} else {
				...
				portInfoList.addChild(
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] { "port" },
							new String[] {
								Integer.toString(fcpConfig.getInt("port")),
							}
						)
				);
			} else {
				...
				portInfoList.addChild(
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] { "port" },
							new String[] {
								Integer.toString(tmciConfig.getInt("port")),
							}
						)
				);
			} else {
			...
			for (UdpSocketHandler handler : handlers) {
				...
				HTMLNode portsContent = pageMaker.getInfobox(
					...
					NodeL10n.getBase()
						.getString(
							...
							new String[] {
								handler.getTitle(),
								AddressTracker.statusString(
									tracker.getPortForwardStatus()
								),
								TimeUtil.formatTime(
				...
				for (PeerAddressTrackerItem item : items) {
					...
					row.addChild(
						"td",
						TimeUtil.formatTime(
							item.timeFromStartupToFirstSentPacket()
						)
					);
					...
					row.addChild(
						"td",
						TimeUtil.formatTime(
							item.timeFromStartupToFirstReceivedPacket()
						)
					);
