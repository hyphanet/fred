...
public class SortedVectorByNumber {
    ...
    private synchronized boolean verify(ObjectContainer container) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (i > 0) {
                if (item.getNumber() <= lastItem.getNumber()) {
                    throw new IllegalStateException("Verify failed! at " + i + " this=" + item.getNumber()
                                                    + " but last=" + lastItem.getNumber());
                }
            }
        ...
        for (int i = length; i < data.length; i++) {
            if (data[i] != null) {
                throw new IllegalStateException("length=" + length + ", data.length=" + data.length + " but [" + i
                                                + "] != null");
            }
        }
    ...
    public void removeFrom(ObjectContainer container) {
        for (int i = 0; i < data.length; i++) {
            if (data[i] != null) {
                throw new IllegalStateException("Still have contents: " + i);
            }
        }
