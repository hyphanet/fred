...
public class CPUID {
    ...
    public static CPUInfo getInfo() throws UnknownCPUException {
        if (!_nativeOk) {
            throw new UnknownCPUException(
                "Failed to read CPU information from the system. Please verify the existence of the jcpuid dll/so.");
        }

        if (!isX86) {
            throw new UnknownCPUException(
                "Failed to read CPU information from the system. The CPUID instruction exists on x86 CPU's only");
        }
        ...
        try {
            ...
            if (wantNative) {
                ...
                if (loaded) {
                    ...
                    if (_doLog) {
                        System.err.println("INFO: Native CPUID library '" + getResourceName()
                    ...
                    if (loaded) {
                        ...
                        if (_doLog) {
                            System.err.println("INFO: Native CPUID library '" + getLibraryMiddlePart()
                                               + "' loaded from somewhere in the path");
                        }
                    } else {
                        _nativeOk = false;
                ...
                if (_doLog) {
                    System.err.println(
                        "INFO: Native CPUID library jcpuid not loaded - will not be able to read CPU information using CPUID");
                }
            }
        } catch (Exception e) {
            if (_doLog) {
                System.err.println("INFO: Native CPUID library jcpuid not loaded, reason: '" + e.getMessage()
    ...
    private static boolean loadFromResource() {
        ...
        try {
            ...
            try {
                ...
                System.err.println("freenet.support.CPUInformation.CPUID: Can't create the temporary file in "
