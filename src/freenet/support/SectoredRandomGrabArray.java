...
public class SectoredRandomGrabArray<T, C extends RemoveRandomWithObject<T>>
	...
	public SectoredRandomGrabArray(
	...
	public void addGrabber(T client, C requestGrabber, ClientContext context) {
		synchronized (root) {
			if (
				requestGrabber.getObject() != client
		...
		synchronized (root) {
			while (true) {
				...
				if (grabArrays.length == 2) {
					RemoveRandomReturn ret = removeRandomTwoOnly(
						excluding,
						context,
						now
					);
				...
				RandomGrabArrayItem item = removeRandomLimited(
					excluding,
					context,
					now
				);
	...
	private RemoveRandomReturn removeRandomExhaustive(
		...
		synchronized (root) {
			...
			for (int i = 0; i < grabArrays.length; i++) {
				...
				RemoveRandomReturn val = rga.removeRandom(
					excluding,
					context,
					now
				);
				if (val != null) {
					if (val.item != null) item = val.item;
						...
						if (wakeupTime > val.wakeupTime) wakeupTime =
							val.wakeupTime;
					}
				}
				if (logMINOR) Logger.minor(
				...
				if (item != null) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Removing grab array " + x + " : " + rga + " (is empty)"
					);
					removeElement(x);
				}
			}
		...
		synchronized (root) {
			...
			while (true) {
				...
				if (rga == null) {
					...
					Logger.error(
						this,
						"Slot " + x + " is null for client " + grabClients[x]
					);
					excluded++;
	...
	private RemoveRandomReturn removeRandomTwoOnly(
		...
		synchronized (root) {
			...
			if (item != null) {
				if (logMINOR) Logger.minor(
					this,
					"Returning (two items only) " + item + " for " + rga
				);
				return new RemoveRandomReturn(item);
			} else {
				x = 1 - x;
				...
				if (
					...
					if (logMINOR) Logger.minor(
						this,
						"Removing first: " + firstRGA + " is empty on " + this
					);
	...
	private RemoveRandomReturn removeRandomOneOnly(
		...
		synchronized (root) {
			...
			if (rga.isEmpty()) {
				if (logMINOR) Logger.minor(
					this,
					"Removing only grab array (0) : " + rga
				);
				grabArrays = newGrabberArray(0);
				grabClients = newClientArray(0);
			}
			if (logMINOR) Logger.minor(
				this,
				"Returning (one item only) " + item + " for " + rga
			);
			if (item == null) {
				if (grabArrays.length == 0) {
					if (logMINOR) Logger.minor(
						this,
						"Arrays are empty on " + this
					);
	...
	private void removeElement(int x) {
		synchronized (root) {
			...
			if (x < grabArraysLength - 1) System.arraycopy(
				grabArrays,
				x + 1,
				newArray,
				x,
	...
	public void maybeRemove(RemoveRandom r, ClientContext context) {
		...
		synchronized (root) {
			while (true) {
				...
				if (found != -1) {
					count++;
					if (count > 1) Logger.error(
						this,
						"Found " + r + " many times in " + this,
						new Exception("error")
					);
					removeElement(found);
				} else {
	...
	private RemoveRandomWithObject<T>[] asGrabberArray(
		RemoveRandomWithObject<T> grabber
	) {
	...
	private RemoveRandomWithObject<T>[] newGrabberArray(int length) {
		return (RemoveRandomWithObject<T>[]) new RemoveRandomWithObject<
			?
		>[length];
	}
}
