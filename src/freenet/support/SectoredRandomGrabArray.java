...
public class SectoredRandomGrabArray implements RemoveRandom, RemoveRandomParent, HasCooldownCacheItem {
    ...
    public synchronized void add(Object client, RandomGrabArrayItem item, ObjectContainer container,
        ...
        if (item.persistent() != persistent) {
            throw new IllegalArgumentException("item.persistent()=" + item.persistent() + " but array.persistent="
                                               + persistent + " item=" + item + " array=" + this);
        }
    ...
    public synchronized void addGrabber(Object client, RemoveRandomWithObject requestGrabber,
        ...
        if (requestGrabber.getObject() != client) {
            throw new IllegalArgumentException("Client not equal to RemoveRandomWithObject's client: client=" + client
                                               + " rr=" + requestGrabber + " his object=" + requestGrabber.getObject());
        }
    ...
    @Override
    public synchronized RemoveRandomReturn removeRandom(RandomGrabArrayItemExclusionList excluding,
    ...
    private synchronized RemoveRandomReturn removeRandomExhaustive(RandomGrabArrayItemExclusionList excluding,
            ObjectContainer container, ClientContext context, long now) {
        ...
        for (int i = 0; i < grabArrays.length; i++) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "RGA has picked " + x + "/" + grabArrays.length + ": " + item + " rga.isEmpty="
                             + rga.isEmpty(container));
            }
    ...
    private synchronized RandomGrabArrayItem removeRandomLimited(RandomGrabArrayItemExclusionList excluding,
            ObjectContainer container, ClientContext context, long now) {
        ...
        while (true) {
            ...
            if (rga == null) {
                ...
                if (excluded > MAX_EXCLUDED) {
                    Logger.normal(this,
                                  "Too many sub-arrays are entirely excluded on " + this + " length = "
    ...
    private synchronized RemoveRandomReturn removeRandomTwoOnly(RandomGrabArrayItemExclusionList excluding,
            ObjectContainer container, ClientContext context, long now) {
        long wakeupTime = Long.MAX_VALUE;
    ...
    private synchronized RemoveRandomReturn removeRandomOneOnly(RandomGrabArrayItemExclusionList excluding,
            ObjectContainer container, ClientContext context, long now) {
        long wakeupTime = Long.MAX_VALUE;
    ...
    public void maybeRemove(RemoveRandom r, ObjectContainer container, ClientContext context) {
        ...
        if ((finalSize == 0) && (parent != null)) {
            ...
            if (persistent) {
                active = container.ext().isActive(parent);
            }
        ...
        for (int i = 0; i < grabArrays.length; i++) {
            ...
            if (existingGrabber != null) {
                ...
                if ((grabber.getObject() == null) && (client != null)) {
                    Logger.error(this,
                                 "Minor corruption on migration: client is " + client
                                 + " but grabber reports null, correcting");
