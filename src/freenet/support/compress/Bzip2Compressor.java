...
public class Bzip2Compressor extends AbstractCompressor {
	...
	public static final byte[] BZ_HEADER =
		"BZ".getBytes(StandardCharsets.ISO_8859_1);
	...
	public long compress(
		...
		try {
			...
			bz2os = new BZip2CompressorOutputStream(
				HeaderStreams.dimOutput(BZ_HEADER, cos)
			);
			long read = 0;
			...
			long iterationToCheckCompressionRatio =
				amountOfDataToCheckCompressionRatio / bufferSize;
			int i = 0;
			while (true) {
	...
	public long decompress(
		...
		BZip2CompressorInputStream bz2is = new BZip2CompressorInputStream(
			HeaderStreams.augInput(BZ_HEADER, is)
		);
		long written = 0;
		int bufSize = 32768;
		...
		while (true) {
			int expectedBytesRead = (int) Math.min(
				buffer.length,
				maxLength - written
			);
			...
			if (expectedBytesRead < bytesRead) {
				...
				if (maxCheckSizeBytes > 0) {
					written += bytesRead;
					while (true) {
						expectedBytesRead = (int) Math.min(
							buffer.length,
							maxLength + maxCheckSizeBytes - written
						);
