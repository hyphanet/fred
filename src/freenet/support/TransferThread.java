...
public abstract class TransferThread
	implements PrioRunnable, ClientGetCallback, ClientPutCallback {
	...
	public void run() {
		...
		try {
			...
			Logger.error(
				this,
				"Error in iterate() or getSleepTime() probably",
				e
			);
		} finally {
			Logger.debug(
				...
				MINUTES.convert(sleepTime, MILLISECONDS) +
				" minutes."
			);
	...
	protected void abortFetches() {
		...
		if (mFetches != null) synchronized (mFetches) {
			ClientGetter[] fetches = mFetches.toArray(
				new ClientGetter[mFetches.size()]
			);
			int fcounter = 0;
	...
	protected void abortInserts() {
		...
		if (mInserts != null) synchronized (mInserts) {
			BaseClientPutter[] inserts = mInserts.toArray(
				new BaseClientPutter[mInserts.size()]
			);
			int icounter = 0;
