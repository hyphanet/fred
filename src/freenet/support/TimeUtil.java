...
public class TimeUtil {
  ...
  public static long toMillis(String timeInterval) {
    ...
    for (String term : terms) {
      ...
      switch (measure) {
        case 'w':
          millis +=
              7 * MILLISECONDS.convert(Long.parseLong(term.substring(0, term.length() - 1)), DAYS);
          break;
        case 'd':
          millis +=
              MILLISECONDS.convert(Short.parseShort(term.substring(0, term.length() - 1)), DAYS);
          break;
        case 'h':
          millis +=
              MILLISECONDS.convert(Short.parseShort(term.substring(0, term.length() - 1)), HOURS);
          break;
        case 'm':
          millis +=
              MILLISECONDS.convert(Short.parseShort(term.substring(0, term.length() - 1)), MINUTES);
          break;
        case 's':
          if (term.contains(".")) {
            ...
            millis +=
                MILLISECONDS.convert(
                    Short.parseShort(term.substring(0, term.length() - 1)), SECONDS);
          }
          break;
        default:
          throw new NumberFormatException(
              "Unknown format: " + (sign > 0 ? "" : "-") + timeInterval);
      }
    }
  ...
  // FIXME: For me it returns a parsed time with 2 hours difference, so it seems to parse localtime.
  // WHY?
