...
public interface DoublyLinkedList<T extends DoublyLinkedList.Item<?>> extends Iterable<T> {
	...
	public interface Item<T extends DoublyLinkedList.Item<?>> {
		 ...
		 * {@code null} if this is the last {@code Item}.
		 * 
		...
		T setNext(Item<?> i);
        /**
	 * Get previous {@link Item}. May or may not return {@code null}
	 * if this is the first {@code Item}.
	 * 
	 * @see DoublyLinkedList#hasNext()
	 */
		T getPrev();
    ...
    int size();
    /** Check if this list is empty. @return {@code true} if this list is empty, {@code false} otherwise. */
    boolean isEmpty();
    /** Get a {@link Enumeration} of {@link DoublyLinkedList.Item}. */
	Enumeration<T> elements(); // for consistency w/ typical Java API

    /**
     * Returns true if the list contains an item i where {@code i.equals(item)} is true.
     */
	public boolean contains(T item);
    
    /**
     * Returns the first item.
     * @return  the item at the head of the list, or {@code null} if empty
     */
	T head();
    /**
     * Returns the last item.
     * @return  the item at the tail of the list, or {@code null} if empty
     */
	T tail();
	...
	T shift();
    /**
     * Remove <tt>n</tt> elements from head and return them as a {@code DoublyLinkedList}.
     */
	...
	T pop();
    /**
     * Remove <tt>n</tt> elements from tail and return them as a {@code DoublyLinkedList}.
     */
	DoublyLinkedList<T> pop(int n);

    /** @return {@code true} if {@code i} has next item. (ie. not the last item); {@code false} otherwise */
	boolean hasNext(T i);
    /** @return {@code true} if {@code i} has previous item. (ie. not the first item); {@code false} otherwise */
	boolean hasPrev(T i);

    /** @return next item of {@code i}. If this is the last element, return {@code null} */
	T next(T i);
    /** @return previous item of {@code i}. If this is the first element, return {@code null} */
	T prev(T i);
    /** Remove and return a element 
     * @return  this item, or {@code null} if the item was not in the list
     */
	T remove(T i);
    /**
     * Inserts item {@code j} before item {@code i}.
     */
	void insertPrev(T i, T j);
    /**
     * Inserts item {@code j} after item <code>i</code.
     */
