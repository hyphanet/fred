...
public abstract class WebInterfaceToadlet
	...
	protected boolean isFormPassword(HTTPRequest req) {
		...
		if (passwd == null) passwd = req.getPartAsStringFailsafe(
			"formPassword",
			32
		);
		return (
			(passwd != null) &&
			passwd.equals(pluginContext.clientCore.formPassword)
		);
	}
	...
	public HTMLNode createErrorBox(
		...
		InfoboxNode box = pluginContext.pageMaker.getInfobox(
			"infobox-alert",
			"ERROR"
		);
