		...
		Outer:for (Options option : options) {
			switch (option) {
				case NOMETASTRINGS: {
					if (tempURI.hasMetaStrings()) {
						if (errors != null) {
							...
							errors.add(
								"URI did contain meta strings, removed it for you"
							);
							if (breakOnErrors) break Outer;
						} else {
							throw new MalformedURLException(
								"URIs with meta strings not supported"
							);
						}
					}
					break;
				...
				case SSKFORUSK: {
					if (tempURI.isUSK()) {
						if (errors != null) {
							...
							errors.add(
								"URI was an USK, converted it to SSK for you"
							);
							if (breakOnErrors) break Outer;
						} else {
							throw new MalformedURLException(
								"USK not supported, use underlying SSK instead."
							);
						}
					}
					break;
				...
				default:
					Logger.error(
						URISanitizer.class,
						"Illegal Option, how can this happen?"
					);
			}
		}
		return tempURI;
