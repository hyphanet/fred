...
public class PrioritizedSerialExecutor implements Executor {
			...
			synchronized (jobs) {
				if (current != null) {
					if (current.isAlive()) {
						Logger.error(
							this,
							"Already running a thread for " + this + " !!",
							new Exception("error")
						);
						return;
					}
				...
				while (true) {
					...
					try {
						...
						if (logMINOR) {
							Logger.minor(
								this,
								"Job " + job + " took " + (end - start) + "ms"
							);
						}

						if (statistics != null) {
							statistics.reportDatabaseJob(
								job.toString(),
								end - start
							);
						}
	...
	public PrioritizedSerialExecutor(
		...
		ArrayDeque<Runnable>[] jobs = (ArrayDeque<Runnable>[]) new ArrayDeque<
			?
	...
	private void reallyStart() {
		synchronized (jobs) {
			if (running) {
				Logger.error(
					this,
					"Not reallyStart()ing: ALREADY RUNNING",
					new Exception("error")
				);
				return;
			}
	...
	public void execute(Runnable job, String jobName) {
		...
		if (job instanceof PrioRunnable) prio =
			((PrioRunnable) job).getPriority();
	...
	public void executeNoDupes(Runnable job, int prio, String jobName) {
		synchronized (jobs) {
			if (jobs[prio].contains(job)) {
				if (logMINOR) Logger.minor(
					this,
					"Not queueing job: Job already queued: " + job
				);
				return;
			}
	...
	public boolean anyQueued() {
		synchronized (jobs) {
			for (int i = 0; i < jobs.length; i++) if (
				jobs[i].size() > 0
			) return true;
		}
		return false;
