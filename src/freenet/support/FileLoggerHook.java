...
public class FileLoggerHook extends LoggerHook implements Closeable {
	...
	/** Verbosity types */
	public static final int DATE = 1, CLASS = 2, HASHCODE = 3, THREAD =
		4, PRIORITY = 5, MESSAGE = 6, UNAME = 7;
	...
	public void setInterval(String intervalName) throws IntervalParseException {
		...
		else if (intervalName.equalsIgnoreCase("HOUR")) INTERVAL =
			Calendar.HOUR;
		else if (intervalName.equalsIgnoreCase("DAY")) INTERVAL =
			Calendar.DAY_OF_MONTH;
		else if (intervalName.equalsIgnoreCase("WEEK")) INTERVAL =
			Calendar.WEEK_OF_YEAR;
		else if (intervalName.equalsIgnoreCase("MONTH")) INTERVAL =
			Calendar.MONTH;
		else if (intervalName.equalsIgnoreCase("YEAR")) INTERVAL =
			Calendar.YEAR;
		else throw new IntervalParseException(
	...
	class WriterThread extends Thread {
			...
			if (baseFilename != null) {
				...
				switch (INTERVAL) {
					...
					case Calendar.WEEK_OF_YEAR:
						if (INTERVAL == Calendar.WEEK_OF_YEAR) gc.set(
				...
				if (INTERVAL_MULTIPLIER > 1) {
					...
					gc.set(
						INTERVAL,
						(x / INTERVAL_MULTIPLIER) * INTERVAL_MULTIPLIER
					);
				}
						...
						while (o == null) {
							...
							try {
								if (thisTime < maxWait) {
									...
									list.wait(
										Math.min(500L, maxWait - thisTime)
									);
									thisTime = System.currentTimeMillis();
									if (listBytes < LIST_WRITE_THRESHOLD) {
										...
										if (closed) o = list.poll(); // If closing, write stuff ASAP.
					...
					if (died) {
						...
						if (altLogStream != null) {
							try {
								...
								System.err.println(
									"Failed to close compressed log stream: " +
									e
								);
							}
						}
		...
		private File rotateLog(
			...
			if (latestFile != null) {
				...
				if (
					previousFile != null && latestFile.exists()
	...
	public void findOldLogFiles(GregorianCalendar gc) {
		...
		System.out.println(
			"Finding old log files. New log file is " + currentFilename
		);
		...
		for (File f : files) {
			...
			if (name.toLowerCase().startsWith(prefix)) {
				if (
					name.equals(previousFile.getName()) ||
				...
				Logger.normal(
					this,
					"Unknown file: " + name + " in the log directory"
				);
			}
		}
		...
		if (currentFilename != null && currentFilename.exists()) {
			System.out.println(
				"Old log file exists for this time period: " + currentFilename
			);
			for (int a = 1;; a++) {
				numericSameDateFilename = new File(getHourLogName(gc, a, true));
				if (
					numericSameDateFilename == null ||
					!numericSameDateFilename.exists()
				) {
					if (numericSameDateFilename != null) {
						System.out.println(
							"Renaming to: " + numericSameDateFilename
						);
						FileUtil.renameTo(
							currentFilename,
	...
	public void log(
		...
		for (int f : fmt) {
			switch (f) {
				...
				case HASHCODE:
					sb.append(
						o == null ? "<none>" : Integer.toHexString(o.hashCode())
					);
					break;
				case THREAD:
		...
		synchronized (list) {
			...
			if (listBytes > MAX_LIST_BYTES) {
				...
				if (!list.offer(buf)) {
					byte[] ss = list.poll();
					if (ss != null) listBytes -= ss.length + LINE_OVERHEAD;
					if (list.offer(buf)) listBytes +=
					(buf.length + LINE_OVERHEAD);
	...
	class CloserThread extends Thread {
		...
		public void run() {
			synchronized (list) {
				closed = true;
				long deadline =
					System.currentTimeMillis() + SECONDS.toMillis(10);
	...
	public void listAvailableLogs(OutputStreamWriter writer)
		...
		DateFormat tempDF = DateFormat.getDateTimeInstance(
			DateFormat.SHORT,
			DateFormat.SHORT,
			Locale.ENGLISH
		);
	...
	public void sendLogByContainedDate(long time, OutputStream os)
		throws IOException {
	...
	public void waitForSwitch() {
		...
		synchronized (this) {
			...
			while (
				((now = System.currentTimeMillis()) < endTime) &&
