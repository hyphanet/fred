...
public class FileLoggerHook extends LoggerHook implements Closeable {
    ...
    public static final int
        DATE = 1, CLASS = 2, HASHCODE = 3, THREAD = 4, PRIORITY = 5, MESSAGE = 6, UNAME = 7;
    ...
    public FileLoggerHook(boolean rotate, String baseFilename, String fmt, String dfmt, String logRotateInterval,
                          LogLevel threshold, boolean assumeWorking, boolean logOverwrite,
                          long maxOldLogfilesDiskUsage, int maxListSize)
            throws IOException, IntervalParseException {
    ...
    public FileLoggerHook(boolean rotate, String baseFilename, String fmt, String dfmt, String threshold,
                          String logRotateInterval, boolean assumeWorking, boolean logOverwrite,
                          long maxOldLogFilesDiskUsage, int maxListSize)
            throws IOException, InvalidThresholdException, IntervalParseException {
    ...
    public void trimOldLogFiles() {
        synchronized (trimOldLogFilesLock) {
            while (oldLogFilesDiskSpaceUsage > maxOldLogfilesDiskUsage) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Deleting " + olf.filename + " - saving " + olf.size + " bytes, disk usage now: "
    ...
    public void sendLogByContainedDate(long time, OutputStream os) throws IOException {
        ...
        synchronized (logFiles) {
            for (OldLogFile olf : logFiles) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Checking " + time + " against " + olf.filename + " : start=" + olf.start + ", end="
                                 + olf.end);
                }
