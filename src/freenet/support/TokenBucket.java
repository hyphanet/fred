...
public class TokenBucket {
	...
	public synchronized boolean instantGrab(long tokens) {
		if (tokens < 0) throw new IllegalArgumentException(
			"Can't grab negative tokens: " + tokens
		);
		if (logMINOR) Logger.minor(
			this,
			"instant grab: " + tokens + " current=" + current + " max=" + max
		);
		addTokens();
		if (logMINOR) Logger.minor(
			this,
	...
	public synchronized void forceGrab(long tokens) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Removed tokens, balance now " + current
		);
	}
	...
	public synchronized void innerBlockingGrab(long tokens) {
		...
		if (current >= 0) {
			if (logMINOR) Logger.minor(
				this,
				"Got tokens instantly, current=" + current
			);
			return;
		} else {
			if (logMINOR) Logger.minor(
		...
		long minDelayMS = MILLISECONDS.convert(
			minDelayNS + MILLISECONDS.toNanos(1) - 1,
			NANOSECONDS
		);
		long now = System.currentTimeMillis();
		...
		if (logMINOR) Logger.minor(
			this,
			"Blocking grab finished: current=" + current
		);
	}

	public synchronized void recycle(long tokens) {
		if (tokens < 0) throw new IllegalArgumentException(
			"Can't recycle negative tokens: " + tokens
		);
		current += tokens;
	...
	public synchronized void changeNanosAndBucketSize(
		long nanosPerTick,
		long newMax
	) {
		if (nanosPerTick <= 0) throw new IllegalArgumentException();
	...
	public synchronized void addTokens() {
		...
		if (logMINOR) Logger.minor(
			this,
			"addTokens: Clipped, current=" + current
		);
	}
	...
	public synchronized void addTokensNoClip() {
		...
		if (logMINOR) Logger.minor(
			this,
			"addTokensNoClip: Added " + add + " tokens, current=" + current
		);
	...
	synchronized long tokensToAdd() {
		long nowNS = NANOSECONDS.convert(
		...
		if (timeLastTick > nowNS) {
			System.err.println(
				...
				TimeUtil.formatTime(
					MILLISECONDS.convert(timeLastTick - nowNS, NANOSECONDS),
					2,
					true
				)
			);
			System.err.println(
				"FREENET WILL BREAK SEVERELY IF THIS KEEPS HAPPENING!"
			);
			Logger.error(
				this,
				"CLOCK SKEW DETECTED! CLOCK WENT BACKWARDS BY AT LEAST " +
