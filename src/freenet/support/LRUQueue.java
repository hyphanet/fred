...
public class LRUQueue<T> {
	 ...
	 * overall improvement.
	 */
	private final DoublyLinkedListImpl<QItem<T>> list =
		new DoublyLinkedListImpl<QItem<T>>();
	...
	public synchronized Object[] toArrayOrdered() {
		...
		int x = 0;
		for (
			Enumeration<QItem<T>> e = list.reverseElements();
			e.hasMoreElements();
		) {
	...
	public synchronized <E> E[] toArrayOrdered(E[] array) {
		...
		if (array.length != listSize) throw new IllegalStateException(
