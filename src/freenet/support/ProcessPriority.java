...
public class ProcessPriority {
	...
	public interface WindowsHolder extends StdCallLibrary {
		WindowsHolder INSTANCE = (WindowsHolder) Native.loadLibrary(
			"kernel32",
			WindowsHolder.class
		);
	...
	public static boolean enterBackgroundMode() {
		if (!background) {
			if (Platform.isWindows()) {
				...
				if (
					lib.SetPriorityClass(
						lib.GetCurrentProcess(),
						WindowsHolder.BELOW_NORMAL_PRIORITY_CLASS
					)
				) {
					...
					System.err.println(
						"SetPriorityClass() failed :" + lib.GetLastError()
					);
					return false;
				}
