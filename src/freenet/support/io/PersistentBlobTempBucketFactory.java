					...
					for(long l = 0; l < blockSize + 16383; l += 16384) {
						...
						synchronized(PersistentBlobTempBucketFactory.this) {
							while (tags.hasNext()) {
								...
								if(logMINOR) {
									Logger.minor(this, "Adding slot " + tag.index + " to freeSlots (has a free tag and no taken tag)");
								}
		...
		if(tag == null) {
			if(!container.ext().isActive(bucket)) {
				...
				if(!results.hasNext()) {
					...
					tag = results.next();
					if(tag.index != index) // Crazy things are happening, may as well check the impossible!
					{
	...
	boolean maybeShrink(ObjectContainer container) {
		...
		synchronized(this) {
			...
			if(now - lastCheckedEnd > 60 * 1000) {
				...
				if(full > 0.8) {
					if(logMINOR) {
						Logger.minor(this, "Not shrinking, last not committed block is at " + full * 100 + "% (" + lastNotCommitted + " of " + blocks + ")");
					}
					lastCheckedEnd = now;
					queueMaybeShrink();
					return false;
