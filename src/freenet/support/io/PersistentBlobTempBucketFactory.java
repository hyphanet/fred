...
public class PersistentBlobTempBucketFactory {
    ...
    void onInit(ObjectContainer container, DBJobRunner jobRunner2, Random fastWeakRandom, File storageFile2,
                long blockSize2, Ticker ticker)
            throws IOException {
        ...
        if (blockSize != blockSize2) {
            throw new IllegalStateException("My block size is " + blockSize2 + " but stored block size is " + blockSize
...
//      }
        ...
        while (tags.hasNext()) {
            ...
            if (tag.isFree) {
                if (tag.bucket != null) {
                    ...
                    if (!tag.bucket.freed()) {
                        Logger.error(this,
                                     "Bucket " + tag.bucket + " for index " + tag.index
                                     + " is not free even though tag is!");
                    }
                }
            ...
            if (tag.index >= freeBlocksCache.getSize()) {
                if (tag.isFree) {
                    if (tag.bucket != null) {
                        Logger.error(
                            this,
                            "Block is marked free, is beyond the end of the file, yet has a bucket!! Freeing anyway...");
                        container.activate(tag.bucket, 1);
                        tag.bucket.onFree();
                ...
                System.err.println(
                    "Your downloads database is corrupt. A persistent temp bucket is in use yet is beyond the length of the persistent-blob.tmp file.");
                System.err.println(
                    "We will free the bucket, but this may cause internal errors and similar problems later on. This was probably caused by a bug at some point but that bug may have already been fixed. Sorry...");
                container.activate(tag.bucket, 1);
                tag.bucket.onFree();
                container.delete(tag.bucket);
                container.delete(tag);
    ...
    private void initRangeDump(ObjectContainer container) {
        ...
        for (long l = 0; l < ptr; l++) {
            ...
            Logger.error(this,
                         "FOUND EMPTY SLOT: " + l
                         + " when scanning the blob file because tags in database < length of file");
...
outer:
                    while (true) {
                        ...
                        synchronized (PersistentBlobTempBucketFactory.this) {
                            ...
                            if (freeSlots.containsKey((long) first)) {
                                continue;
                            }

                            if (almostFreeSlots.containsKey((long) first)) {
                                continue;
                            }

                            if (notCommittedBlobs.containsKey((long) first)) {
                                continue;
                            }
                        }
                        ...
                        while (tags.hasNext()) {
                            ...
                            if (tag.factory != PersistentBlobTempBucketFactory.this) {
                                continue;
                            }
                        ...
                        PersistentBlobTempBucketTag tag =
                            new PersistentBlobTempBucketTag(PersistentBlobTempBucketFactory.this, first);
                        ...
                        synchronized (PersistentBlobTempBucketFactory.this) {
                            while (tags.hasNext()) {
                                ...
                                if (!tag.isFree) {
                                    ...
                                    if (tag.bucket == null) {
                                        Logger.error(this,
                                                     "Tag flagged non-free yet has no bucket for index " + tag.index);
                                        tag.isFree = true;
                                        container.store(tag);
                                        changedTags = true;
                                    } else {
                                ...
                                if (notCommittedBlobs.containsKey(tag.index)) {
                                    continue;
                                }

                                if (almostFreeSlots.containsKey(tag.index)) {
                                    continue;
                                }
                                ...
                                if (tag.bucket != null) {
                                    Logger.error(this,
                                                 "Bucket is occupied but not in notCommittedBlobs?!: " + tag + " : "
                                                 + tag.bucket);
                                ...
                                if (tag.index > ptr) {
                                    if (logMINOR) {
                                        Logger.minor(
                                            this,
                                            "Not adding slot " + tag.index
                                            + " to freeSlots because it is past the end of the file (but it is free)");
                                    }
                                ...
                                if (logMINOR) {
                                    Logger.minor(this,
                                                 "Adding slot " + tag.index
                                                 + " to freeSlots (has a free tag and no taken tag)");
                                }
                    ...
                    if (inDB < ptr) {
                        ...
                        for (long l = ptr - 1; l >= 0; l--) {
                            ...
                            PersistentBlobTempBucketTag tag =
                                new PersistentBlobTempBucketTag(PersistentBlobTempBucketFactory.this, l);
                    ...
                    query = container.query();
                    query.constrain(PersistentBlobTempBucketTag.class);
                    query.descend("index").constrain(blocks - 1).greater().and(
                        query.descend("factory").constrain(PersistentBlobTempBucketFactory.this));
                    ...
                    while (results.hasNext()) {
                        ...
                        if (!tag.isFree) {
                            Logger.error(this,
                                         "Block already exists beyond the end of the file (" + blocks
                                         + "), yet is occupied: block " + tag.index);
                        }
    ...
    public PersistentBlobTempBucket makeBucket() throws DatabaseDisabledException {
                ...
                if (logMINOR) {
                    try {
                        if (slot * blockSize > channel.size()) {
                            Logger.error(this,
                                         "Free slot " + slot + " but file length is " + channel.size() + " = "
                                         + (channel.size() / blockSize) + " blocks");
        ...
        Logger.error(
            this,
            "Returning null, unable to create a bucket for some reason, node will fallback to file-based buckets");
    ...
    public synchronized void freeBucket(long index, PersistentBlobTempBucket bucket) {
        ...
        if (!bucket.persisted()) {
            if (bucket.getTag() == null) {
                Logger.error(this, "freeBucket but tag is null for " + bucket + " - not activated???",
                             new Exception("error"));
            } else {
    ...
    public synchronized void remove(PersistentBlobTempBucket bucket, ObjectContainer container) {
        if (logMINOR) {
            Logger.minor(this, "Removing bucket " + bucket + " for slot " + bucket.getIndex() + " from database",
                         new Exception("debug"));
        }
        ...
        if (tag != null) {
            ...
            if (!bucket.freed()) {
                Logger.error(this, "Removing bucket " + bucket + " for slot " + index + " but not freed!",
                             new Exception("debug"));
        ...
        synchronized (this) {
            ...
            if ((now - lastCheckedEnd > SECONDS.toMillis(60)) || DISABLE_SANITY_CHECKS_DEFRAG) {
                ...
                if (lastNotCommitted < lastAlmostFreed) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Last almost freed: " + lastAlmostFreed + " replacing last not committed: "
                                     + lastNotCommitted);
                    }
                ...
                if (((full > 0.8) &&!DISABLE_SANITY_CHECKS_DEFRAG) || (lastNotCommitted == blocks)) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Not shrinking, last not committed block is at " + full * 100 + "% ("
                                     + lastNotCommitted + " of " + blocks + ")");
                    }
...
outer:
                    while (true) {
                        synchronized (this) {
                            ...
                            if (notCommittedBlobs.containsKey((long) last)) {
                                continue;
                            }
                        }
                        ...
                        PersistentBlobTempBucketTag tag =
                            new PersistentBlobTempBucketTag(PersistentBlobTempBucketFactory.this, last);
                    ...
                    if (lastCommitted == -1) {
                        ...
                        Logger.normal(this,
                                      "No used slots in persistent temp file (but last not committed = "
                                      + lastNotCommitted + ")");
                        lastCommitted = 0;
                        query = null;
                    }
                    ...
                    if ((full > 0.8) || DISABLE_SANITY_CHECKS_DEFRAG) {
                        if (full > 0.8) {
                            if (logMINOR) {
                                Logger.minor(this, "Not shrinking, last committed block is at " + full * 100 + "%");
                            }
                        ...
                        while (true) {
                            ...
                            if (firstSlot < lastCommitted) {
                                ...
                                synchronized (lastBucket) {
                                    if (shadow != null) {
                                        synchronized (shadow) {
                                            if (!innerDefrag(lastBucket, shadow, lastTag, newTag, container)) {
                                                return false;
                                            }
                                        }
                                    } else {
                                ...
                                if (logMINOR) {
                                    Logger.minor(this,
                                                 "First available slot " + firstSlot + " is after slot to move "
                                                 + lastCommitted);
                                }
                ...
                jobRunner.queue(new DBJob() {
                    ...
                    public boolean run(ObjectContainer container, ClientContext context) {
                        ...
                        while (tags.hasNext()) {
                            ...
                            if (tag.bucket != null) {
                                Logger.error(this,
                                             "Tag with bucket beyond end of file! index=" + tag.index + " bucket="
                                             + tag.bucket);
    ...
    private boolean innerDefrag(PersistentBlobTempBucket lastBucket, PersistentBlobTempBucket shadow,
                                PersistentBlobTempBucketTag lastTag, PersistentBlobTempBucketTag newTag,
    ...
    private synchronized long getSize() {
        if ((cachedSize != Long.MAX_VALUE) && (cachedSize != 0)) {
            return cachedSize;
        }
