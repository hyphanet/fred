...
public class ReadOnlyFileSliceBucket implements Bucket, Serializable {
	...
	protected ReadOnlyFileSliceBucket(DataInputStream dis)
		...
		if (!file.exists()) throw new StorageFormatException(
			"File does not exist any more"
		);
		if (file.length() < startAt + length) throw new StorageFormatException(
			"Slice does not fit in file"
		);
	}
}
