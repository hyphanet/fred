...
public class DiskSpaceCheckingRandomAccessBufferFactory
	...
	public DiskSpaceCheckingRandomAccessBufferFactory(
		LockableRandomAccessBufferFactory underlying,
		File dir,
		long minDiskSpace
	) {
	...
	public LockableRandomAccessBuffer makeRAF(long size) throws IOException {
		...
		try {
			if (
				dir.getUsableSpace() > size + minDiskSpace
	...
	public PooledFileRandomAccessBuffer createNewRAF(
		File file,
		long size,
		Random random
	) throws IOException {
		...
		try {
			...
			if (dir.getUsableSpace() > size + minDiskSpace) {
				ret = new PooledFileRandomAccessBuffer(
					file,
					false,
					size,
					random,
	...
	public boolean checkDiskSpace(File file, int toWrite, int bufferSize) {
		if (!FileUtil.isParent(dir, file)) {
			Logger.error(
				this,
				"Not checking disk space because " +
				file +
				" is not child of " +
				dir
			);
		...
		try {
			if (
				dir.getUsableSpace() - (toWrite + bufferSize) < minDiskSpace
			) return false;
			else return true;
		} finally {
			lock.unlock();
