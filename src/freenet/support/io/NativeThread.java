...
public class NativeThread extends Thread {
	...
	static {
		...
		Logger.debug(
			NativeThread.class,
			"Run init(): should loadNative=" + maybeLoadNative
		);
		if (maybeLoadNative) {
			...
			JAVA_PRIORITY_RANGE;
			if (
				!(HAS_ENOUGH_NICE_LEVELS && HAS_THREE_NICE_LEVELS)
			) System.err.println(
		...
		Logger.minor(
			NativeThread.class,
			"Run init(): _loadNative = " + _loadNative
		);
	}
	...
	public final void run() {
		if (!setNativePriority(currentPriority)) System.err.println(
			"setNativePriority(" + currentPriority + ") has failed!"
		);
		super.run();
		realRun();
	}
	...
	private boolean setNativePriority(int prio) {
		...
		if (_disabled) {
			Logger.normal(
				this,
				"Not setting native priority as disabled due to renicing"
			);
			return false;
		}
		...
		if (prio < currentPriority) throw new IllegalStateException(
		...
		return (
			LinuxNativeThread.setpriority(0, 0, linuxPriority) > -1
				? true
				: false
		);
	}
	...
	public static String normalizeName(String name) {
		if (name.indexOf(" for ") != -1) name = name.substring(
			0,
			name.indexOf(" for ")
		);
		if (name.indexOf('@') != -1) name = name.substring(
			0,
			name.indexOf('@')
		);
		if (name.indexOf('(') != -1) name = name.substring(
