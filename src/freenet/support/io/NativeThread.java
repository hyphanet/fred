    ...
    private boolean setNativePriority(int prio) {
        ...
        if (prio < currentPriority) {
            throw new IllegalStateException("You're trying to set a thread priority"
                                            ...
                                            + " and shouldn't ever occur in our code. (asked=" + prio + ':'
                                            + linuxPriority + " currentMax=" + +currentPriority + ':'
                                            + NATIVE_PRIORITY_BASE + ") SHOUDLN'T HAPPEN, please report!");
        }

        Logger.minor(this,
                     "Setting native priority to " + linuxPriority + " (base=" + NATIVE_PRIORITY_BASE + ") for "
                     + this);
