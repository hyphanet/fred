...
abstract class SwitchableProxyRandomAccessBuffer
	...
	SwitchableProxyRandomAccessBuffer(
		LockableRandomAccessBuffer initialWrap,
		long size
	) throws IOException {
		this.underlying = initialWrap;
		this.size = size;
		if (underlying.size() < size) throw new IOException(
			"Underlying must be >= size given"
		);
	}
	...
	public void pread(long fileOffset, byte[] buf, int bufOffset, int length)
		...
		if (fileOffset + length > size) throw new IOException(
			"Tried to read past end of file"
		);
		try {
			lock.readLock().lock();
			if (underlying == null || closed) throw new IOException(
				"Already closed"
			);
	...
	public RAFLock lockOpen() throws IOException {
		try {
			lock.writeLock().lock();
			if (closed || underlying == null) throw new IOException(
				"Already closed"
			);
