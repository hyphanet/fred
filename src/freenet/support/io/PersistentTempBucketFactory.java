...
public class PersistentTempBucketFactory
	implements BucketFactory, PersistentFileTracker {
	...
	public PersistentTempBucketFactory(
		...
		if (!dir.isDirectory()) throw new IOException(
			"Directory is not a directory: " + dir
		);
		...
		File[] files = dir.listFiles(
			new FileFilter() {
				...
				public boolean accept(File pathname) {
					if (
						!pathname.exists() || pathname.isDirectory()
					) return false;
	...
	public void register(File file) {
		synchronized (this) {
			if (originalFiles == null) throw new IllegalStateException(
				"completed Init has already been called!"
			);
			...
			if (!originalFiles.remove(file)) Logger.error(
				this,
	...
	public synchronized void completedInit() {
		if (originalFiles == null) {
			Logger.error(
				this,
				"Completed init called twice",
				new Exception("error")
			);
			return;
		}
		for (File f : originalFiles) {
			if (Logger.shouldLog(LogLevel.MINOR, this)) Logger.minor(
				this,
				"Deleting old tempfile " + f
			);
			f.delete();
		}
		originalFiles = null;
	...
	public RandomAccessBucket makeBucket(long size) throws IOException {
		...
		if (rawBucket == null) rawBucket = new PersistentTempFileBucket(
			fg.makeRandomFilename(),
			fg,
			this
		);
		synchronized (encryptLock) {
		...
		if (mustWrap) rawBucket = new DelayedFreeRandomAccessBucket(
			this,
			rawBucket
		);
		return rawBucket;
	...
	public DelayedFree[] grabBucketsToFree() {
		synchronized (this) {
			...
			DelayedFree[] buckets = bucketsToFree.toArray(
				new DelayedFree[bucketsToFree.size()]
			);
			bucketsToFree.clear();
			commitID++;
			return buckets;
