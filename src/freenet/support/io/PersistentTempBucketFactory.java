...
public class PersistentTempBucketFactory implements BucketFactory, PersistentFileTracker {
    ...
    public PersistentTempBucketFactory(File dir, final String prefix, RandomSource strongPRNG, Random weakPRNG,
                                       boolean encrypt, long nodeDBHandle)
            throws IOException {
        blobFactory = new PersistentBlobTempBucketFactory(BLOB_SIZE, nodeDBHandle,
                new File(dir, "persistent-blob.tmp"));
...
//      }
    ...
    public static PersistentTempBucketFactory load(File dir, String prefix, RandomSource random, Random fastWeakRandom,
            ObjectContainer container, final long nodeDBHandle, boolean encrypt, DBJobRunner jobRunner, Ticker ticker)
            throws IOException {
        ...
        if (results.hasNext()) {
            ...
            factory.blobFactory.onInit(container, jobRunner, fastWeakRandom, new File(dir, "persistent-blob.tmp"),
                                       BLOB_SIZE, ticker);

            return factory;
        } else {
            PersistentTempBucketFactory factory = new PersistentTempBucketFactory(dir, prefix, random, fastWeakRandom,
                                                      encrypt, nodeDBHandle);
