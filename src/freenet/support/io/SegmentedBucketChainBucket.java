                        ...
                        public boolean run(ObjectContainer container, ClientContext context) {
                            if (container.ext().isStored(oldSeg)) {
                                if (!container.ext().isActive(oldSeg)) {
                                    Logger.error(this, "OLD SEGMENT STORED BUT NOT ACTIVE: " + oldSeg,
                                                 new Exception("error"));
    ...
    protected SegmentedChainBucketSegment makeSegment(int index, final SegmentedChainBucketSegment oldSeg) {
        ...
        synchronized (this) {
            ...
            if (segments.size() != index) {
                throw new IllegalArgumentException("Asked to add segment " + index + " but segments length is "
                                                   + segments.size());
            }
