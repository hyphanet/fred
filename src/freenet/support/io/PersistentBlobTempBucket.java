...
public class PersistentBlobTempBucket implements Bucket {
    ...
    public PersistentBlobTempBucket(PersistentBlobTempBucketFactory factory2, long blockSize2, long slot,
                                    PersistentBlobTempBucketTag tag, boolean shadow) {
        factory = factory2;
    ...
    public InputStream getInputStream() throws IOException {
        ...
        return new InputStream() {
            ...
            public int read(byte[] buffer, int bufOffset, int length) throws IOException {
                ...
                if (length < 0) {
                    throw new IllegalStateException("offset=" + bufOffset + " length=" + length + " buf len = "
                                                    + buffer.length + " my offset is " + offset + " my size is " + max
                                                    + " for " + this + " for " + PersistentBlobTempBucket.this);
                }
    ...
    public OutputStream getOutputStream() throws IOException {
        ...
        return new OutputStream() {
            ...
            public void write(byte[] buffer, int bufOffset, int length) throws IOException {
                ...
                if (length > remaining) {
                    throw new IOException("Writing too many bytes: written " + offset + " of " + blockSize
                                          + " and now want to write " + length);
                }
    ...
    public void storeTo(ObjectContainer container) {
        ...
        synchronized (this) {
            if (freed) {
                Logger.error(this, "Storing freed bucket " + this + " formerly for slot " + index,
                             new Exception("error"));
            ...
            if (tag == null) {
                ...
                if (results.isEmpty()) {
                    ...
                    if (tag.bucket == null) {
                        ...
                        // Can we auto-repair???
                        throw new NullPointerException("Active but tag null! (Other tag is also null) shadow=" + shadow
                                                       + " freed=" + freed + " persisted=" + persisted + " stored="
                                                       + container.ext().isStored(this) + " index=" + index + " for "
                                                       + this);
                        ...
                        throw new NullPointerException("Active but tag null, and other tag points to other bucket!: "
                                                       + tag.bucket + " not " + this + " details: shadow=" + shadow
                                                       + " freed=" + freed + " persisted=" + persisted + " stored="
                                                       + container.ext().isStored(this) + " index=" + index);
                    }
                }
            }
    ...
    public boolean objectCanNew(ObjectContainer container) {
        ...
        Logger.error(this,
                     "objectOnNew() called but we haven't been stored yet! for " + this + " for " + factory + " index "
                     + index, new Exception("error"));
