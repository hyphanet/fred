	...
	public InputStream getInputStream() throws IOException {
		...
		return new InputStream() {
			...
			public int read(byte[] buffer, int bufOffset, int length) throws IOException {
				...
				if(length < 0) {
					throw new IllegalStateException("offset=" + bufOffset + " length=" + length + " buf len = " + buffer.length + " my offset is " + offset + " my size is " + max + " for " + this + " for " + PersistentBlobTempBucket.this);
				}
	...
	public OutputStream getOutputStream() throws IOException {
		...
		return new OutputStream() {
			...
			public void write(byte[] buffer, int bufOffset, int length) throws IOException {
				...
				if(length > remaining) {
					throw new IOException("Writing too many bytes: written " + offset + " of " + blockSize + " and now want to write " + length);
				}
				ByteBuffer buf = ByteBuffer.wrap(buffer, bufOffset, length);
				int written = 0;
