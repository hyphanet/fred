...
public class PooledFileRandomAccessBuffer
	...
	PooledFileRandomAccessBuffer(
		...
		try {
			...
			if (forceLength >= 0 && forceLength != currentLength) {
				if (readOnly) throw new IOException(
					"Read only but wrong length"
				);
	...
	public void pwrite(long fileOffset, byte[] buf, int bufOffset, int length)
		...
		RAFLock lock = lockOpen();
		try {
			if (fileOffset + length > this.length) throw new IOException(
	...
	private PooledFileRandomAccessBuffer pollFirstClosable() {
		synchronized (fds) {
			Iterator<PooledFileRandomAccessBuffer> it =
				fds.closables.iterator();
			if (it.hasNext()) {
	...
	public void onResume(ClientContext context) throws ResumeFailedException {
		if (!file.exists()) throw new ResumeFailedException(
			"File does not exist: " + file
		);
		if (length > file.length()) throw new ResumeFailedException(
			"Bad length"
		);
		if (persistentTempID != -1) context.persistentFileTracker.register(
			file
		);
	}
	...
	PooledFileRandomAccessBuffer(
		...
		if (persistentTempID != -1) {
			...
			if (!f.exists()) throw new ResumeFailedException(
				"Persistent tempfile lost " + f
			);
		} else {
			file = f;
	...
	public int hashCode() {
		...
		result = prime * result +
		(int) (persistentTempID ^ (persistentTempID >>> 32));
