...
public class DatastoreUtil {
	...
	public static long autodetectDatastoreSize(
		NodeClientCore core,
		Config config
	) {
		...
		if (freeSpace <= 0) {
			...
			if (freeSpace > 50 * oneGiB) {
				...
				shortSize = Math.max(
					...
					Math.min(
						freeSpace / 10,
						Math.min(diskIoMax, bloomFilter128MiBMax)
					)
				);
