...
public class MaybeEncryptedRandomAccessBufferFactory
		...
		synchronized (this) {
			if (reallyEncrypt && this.secret != null) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Encrypting and padding " + size + " to " + paddedSize
				);
			}
		}
		LockableRandomAccessBuffer raf = factory.makeRAF(paddedSize);
		if (secret != null) {
			if (realSize != paddedSize) raf = new PaddedRandomAccessBuffer(
				raf,
				realSize
			);
			try {
				raf = new EncryptedRandomAccessBuffer(
	...
	public void setEncryption(boolean value) {
		...
		if (
			factory instanceof PooledFileRandomAccessBufferFactory
