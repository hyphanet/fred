...
public class BucketChainBucket implements Bucket {
    ...
    private BucketChainBucket(Vector<Bucket> newBuckets, long bucketSize2, long size2, boolean readOnly,
                              BucketFactory bf2, boolean cacheWholeBucket) {
    ...
    protected OutputStream makeBucketOutputStream(int i) throws IOException {
        ...
        synchronized (this) {
            ...
            if (buckets.size() != i + 1) {
                throw new IllegalStateException("Added bucket, size should be " + (i + 1) + " but is "
                                                + buckets.size());
            }
