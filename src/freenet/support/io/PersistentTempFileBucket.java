...
public class PersistentTempFileBucket
	...
	public OutputStream getOutputStreamUnbuffered() throws IOException {
		...
		os = new DiskSpaceCheckingOutputStream(
			os,
			tracker,
			getFile(),
			BUFFER_SIZE
	...
	public RandomAccessBucket createShadow() {
		PersistentTempFileBucket ret = new PersistentTempFileBucket(
			filenameID,
			generator,
			tracker,
			false
		...
		ret.setReadOnly();
		if (!getFile().exists()) Logger.error(
			this,
			"File does not exist when creating shadow: " + getFile()
		);
		return ret;
	}
