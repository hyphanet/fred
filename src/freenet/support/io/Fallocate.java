...
public final class Fallocate {
	...
	public Fallocate fromOffset(long offset) {
		if (
			offset < 0 || offset > final_filesize
	...
	public void execute() throws IOException {
		...
		if (IS_LINUX) {
			...
			errno = FallocateHolderPOSIX.posix_fallocate(
				fd,
				offset,
				final_filesize - offset
			);
		...
		if (isUnsupported || errno != 0) {
			Logger.normal(
				this,
				"fallocate() failed; using legacy method; errno=" + errno
			);
	...
	private static class FallocateHolder {
		...
		private static native int fallocate(
			int fd,
			int mode,
			long offset,
			long length
	...
	private static class FallocateHolderPOSIX {
		static {
			Native.register(
				FallocateHolderPOSIX.class,
				Platform.C_LIBRARY_NAME
			);
		}

		private static native int posix_fallocate(
			int fd,
			long offset,
			long length
		);
	...
	private static int getDescriptor(FileDescriptor descriptor) {
		try {
			...
			final Field field = descriptor
				.getClass()
				.getDeclaredField(IS_ANDROID ? "descriptor" : "fd");
			field.setAccessible(true);
