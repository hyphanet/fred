...
public class LineReadingInputStream
	...
	public String readLine(int maxLength, int bufferSize, boolean utf)
		...
		if (maxLength <= bufferSize) bufferSize = maxLength + 1; // Buffer too big, shrink it (add 1 for the optional \r)

		if (!markSupported()) return readLineWithoutMarking(
			maxLength,
			bufferSize,
			utf
		);
	...
	protected String readLineWithoutMarking(
		int maxLength,
		int bufferSize,
		boolean utf
	) throws IOException {
