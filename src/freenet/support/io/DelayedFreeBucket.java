...
public class DelayedFreeBucket implements Bucket, Serializable, DelayedFree {
	...
	public OutputStream getOutputStream() throws IOException {
		synchronized (this) {
			if (migrated) throw new IOException(
				"Already migrated to a RandomAccessBucket"
			);
	...
	public void free() {
		...
		if (logMINOR) Logger.minor(
			this,
			"Freeing " + this + " underlying=" + bucket,
			new Exception("debug")
		);
	...
	protected DelayedFreeBucket(
		...
		bucket = (RandomAccessBucket) BucketTools.restoreFrom(
			dis,
			fg,
	...
	public synchronized RandomAccessBucket toRandomAccessBucket()
		throws IOException {
		if (freed) throw new IOException("Already freed");
