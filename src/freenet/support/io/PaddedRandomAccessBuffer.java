...
public class PaddedRandomAccessBuffer
	...
	public void pread(long fileOffset, byte[] buf, int bufOffset, int length)
		throws IOException {
		if (fileOffset + length > realSize) throw new IOException(
	...
	public PaddedRandomAccessBuffer(
		...
		raf = BucketTools.restoreRAFFrom(
			dis,
			fg,
			persistentFileTracker,
			masterSecret
		);
		if (realSize > raf.size()) throw new ResumeFailedException(
			"Padded file is smaller than expected length"
		);
	}
