...
public class BucketTools {
    ...
    public static Bucket makeImmutableBucket(BucketFactory bucketFactory, byte[] data, int offset, int length)
            throws IOException {
    ...
    public static long copyTo(Bucket decodedData, OutputStream os, long truncateLength) throws IOException {
        ...
        try {
            ...
            while (moved < truncateLength) {
                ...
                if (bytes <= 0) {
                    throw new IllegalStateException("bytes=" + bytes + ", truncateLength=" + truncateLength
                                                    + ", moved=" + moved);
                }
                ...
                if (bytes <= 0) {
                    ...
                    IOException ioException = new IOException("Could not move required quantity of data in copyTo: "
                                                  + bytes + " (moved " + moved + " of " + truncateLength
        ...
        if (origData instanceof BucketChainBucket) {
            ...
            if (data.bucketSize == splitSize) {
                ...
                Logger.error(BucketTools.class,
                             "Incompatible split size splitting a BucketChainBucket: his split size is "
                             + data.bucketSize + " but mine is " + splitSize
                             + " - we will copy the data, but this suggests a bug", new Exception("debug"));
            }
        }
