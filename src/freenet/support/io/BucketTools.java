...
public class BucketTools {
	...
	public static int toByteArray(Bucket bucket, byte[] output)
		throws IOException {
		long size = bucket.size();
		if (size > output.length) throw new IllegalArgumentException(
			"Data does not fit in provided buffer"
		);
		InputStream is = null;
		try {
	...
	public static byte[] hash(Bucket data) throws IOException {
		...
		try {
			...
			try {
				...
				if (
					(bytesRead < bucketLength) && (bucketLength > 0)
	...
	public static long copyTo(
		...
		try {
			...
			if (truncateLength > 0 && truncateLength < bufferSize) bufferSize =
				(int) truncateLength;
			...
			while (moved < truncateLength) {
				...
				if (bytes <= 0) throw new IllegalStateException(
					"bytes=" +
					bytes +
					", truncateLength=" +
					truncateLength +
	...
	public static Bucket[] split(
		...
		if (
			length > ((long) Integer.MAX_VALUE) * splitSize
	...
	public static Bucket pad(
		...
		try {
			...
			os.close();
			os = null;
			if (b.size() != blockLength) throw new IllegalStateException(
				"The bucket's size is " +
				b.size() +
	...
	public static byte[] pad(byte[] orig, int blockSize, int length)
		throws IOException {
	...
	@Deprecated
	public static void fill(Bucket bucket, Random random, long length)
		throws IOException {
	...
	public static Bucket restoreFrom(
		...
		switch (magic) {
			...
			case PaddedRandomAccessBucket.MAGIC:
				return new PaddedRandomAccessBucket(
					dis,
					fg,
					persistentFileTracker,
					masterKey
			...
			default:
				throw new StorageFormatException(
					"Unknown magic value for bucket " + magic
				);
		}
	}
	...
	public static LockableRandomAccessBuffer restoreRAFFrom(
		...
		switch (magic) {
			...
			default:
				throw new StorageFormatException(
					"Unknown magic value for RAF " + magic
				);
		}
	}

	public static RandomAccessBucket toRandomAccessBucket(
		Bucket bucket,
		BucketFactory bf
	) throws IOException {
		if (
			bucket instanceof RandomAccessBucket
		) return (RandomAccessBucket) bucket;
		if (bucket instanceof DelayedFreeBucket) {
			RandomAccessBucket ret =
				((DelayedFreeBucket) bucket).toRandomAccessBucket();
