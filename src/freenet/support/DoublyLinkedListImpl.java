...
public class DoublyLinkedListImpl<T extends DoublyLinkedList.Item<? extends T>>
	...
	public DoublyLinkedList<T> shift(int n) {
		...
		DoublyLinkedList<T> newlist = new DoublyLinkedListImpl<T>(
			_firstItem,
			newTailItem,
			n
		);
	...
	public T remove(T i) {
		...
		if (i.getParent() != this) throw new PromiscuousItemException(
			i,
			i.getParent()
		);
	...
	public void insertPrev(T i, T j) {
		if (j.getParent() != null) throw new PromiscuousItemException(
			j,
			j.getParent()
		);
		if (
			(j.getNext() != null) || (j.getPrev() != null)
		...
		if (i == null) {
			...
			// insert in middle
			if (i.getParent() == null) throw new PromiscuousItemException(
				i,
				i.getParent()
