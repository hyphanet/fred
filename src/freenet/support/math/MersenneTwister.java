...
public class MersenneTwister extends org.spaceroots.mantissa.random.MersenneTwister {
	...
	*/
	...
	/**
	 * <p>The default java implementation of this function from java.util.Random behave  
	 ...
	 * the next call to the method is made when they will be used.</p>
	 * 
	 * <p>The main reason we want this is because sometimes we will use a specific seed to 
	 * produce a stream of bytes, e.g. in padding CHKs, and we want the stream to be 
	 * predictable even if we fetch it in different sized chunks depending on the implementation 
	 * details. Also, it's faster. Note that all this is only relevant if we're just using a 
	 * MersenneTwister as a source of bytes; calling the other methods e.g. nextInt(), 
	 * nextDouble() breaks any such guarantees. */
	@Override
