...
public final class TimeDecayingRunningAverage
	...
	public TimeDecayingRunningAverage(
		...
		if (
			Double.isInfinite(curValue) || Double.isNaN(curValue)
		) throw new IOException("Invalid weightedTotal: " + curValue);
		if ((curValue < min) || (curValue > max)) throw new IOException(
			"Out of range: curValue = " + curValue
		);
	...
	public void report(double d) {
		synchronized (this) {
			...
			if (Double.isInfinite(d) || Double.isNaN(d)) {
				Logger.error(
					this,
					"Reported infinity or NaN to " + this + " : " + d,
					new Exception("error")
				);
				return;
			}
			...
			if (!started) {
				...
				if (logDEBUG) Logger.debug(
					this,
					"Reported " + d + " on " + this + " when just started"
				);
				...
				double changeFactor = Math.pow(
					0.5,
					(thisInterval) / thisHalfLife
				);
