...
public class RandomGrabArray implements RemoveRandom, HasCooldownCacheItem {
    ...
    public void add(RandomGrabArrayItem req, ObjectContainer container, ClientContext context) {
        if (req.persistent() != persistent) {
            throw new IllegalArgumentException("req.persistent()=" + req.persistent() + " but array.persistent="
                                               + persistent + " item=" + req + " array=" + this);
        }
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                ...
                if ((i != (blocks.length - 1)) && (block.reqs.length != BLOCK_SIZE)) {
                    Logger.error(this,
                                 "Block " + i + " of " + blocks.length + " is wrong size: " + block.reqs.length
    ...
    private RandomGrabArrayItem removeRandomLimited(RandomGrabArrayItemExclusionList excluding,
        ...
        while (true) {
            ...
            for (int i = 0; i < index; i++) {
                ...
                if (item == null) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Found null item at offset " + offset + " i=" + i + " block = " + blockNumReading
                                     + " on " + this);
                    }
    ...
    private void remove(int blockNo, int i, ObjectContainer container) {
        ...
        if ((blocks.length == 1) || (blockNo == endBlock)) {
            ...
            if (persistent) {
                container.activate(blocks[endBlock], 1);
            }
    ...
    public void remove(RandomGrabArrayItem it, ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR && (container != null)) {
            ...
            if ((!persistent) && (stored || active)) {
                Logger.error(this, "persistent=" + persistent + " stored=" + stored + " active=" + active,
                             new Exception("error"));
            } else {
                Logger.minor(this, "persistent=" + persistent + " stored=" + stored + " active=" + active,
                             new Exception("error"));
            }
        }
            ...
            for (Block block : blocks) {
                ...
                for (RandomGrabArrayItem item : block.reqs) {
                    if (item != null) {
                        ...
                        if (count >= index) {
                            Logger.error(this,
                                         "ITEM AT INDEX " + count + " : " + item + " EVEN THOUGH MAX INDEX IS " + index
                                         + " on " + this, new Exception("error"));
                        } else {
                            Logger.error(this,
                                         "VALID ITEM WHILE DELETING BLOCK: " + item + " on " + this + " at index "
                                         + count + " of " + index, new Exception("error"));
                        }
                    }
