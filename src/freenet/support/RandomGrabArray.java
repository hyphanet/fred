...
public class RandomGrabArray implements RemoveRandom, RequestSelectionTreeNode {
	...
	public void add(RandomGrabArrayItem req, ClientContext context) {
		if (
			context != null &&
			req.getWakeupTime(context, System.currentTimeMillis()) < 0
		) {
		...
		synchronized (root) {
			...
			if (blocks.length == 1 && index < BLOCK_SIZE) {
				...
				if (index >= blocks[0].reqs.length) {
					blocks[0].reqs = Arrays.copyOf(
						blocks[0].reqs,
						Math.min(BLOCK_SIZE, blocks[0].reqs.length * 2)
					);
				}
				blocks[0].reqs[index++] = req;
				if (logMINOR) Logger.minor(
					this,
					"Added " + req + " before index " + index
				);
				return;
			}
			...
			for (int i = 0; i < blocks.length; i++) {
				...
				if (
					i != (blocks.length - 1) && block.reqs.length != BLOCK_SIZE
				) {
					Logger.error(
						this,
						"Block " +
			...
			if (logMINOR) Logger.minor(
				this,
				"Added: " + req + " to " + this + " size now " + index
			);
		}
	}
	...
	public RemoveRandomReturn removeRandom(
		...
		if (logMINOR) Logger.minor(
			this,
			"removeRandom() on " + this + " index=" + index
		);
		synchronized (root) {
			if (index == 0) {
			...
			RandomGrabArrayItem ret = removeRandomLimited(
				excluding,
				context,
				now
			);
	...
	private RandomGrabArrayItem removeRandomLimited(
		...
		while (true) {
			...
			if (itemWakeTime == -1) {
				if (logMINOR) Logger.minor(
					this,
					"Not returning because cancelled: " + ret
				);
				ret = null;
			...
			if (itemWakeTime == 0) itemWakeTime = excluding.exclude(
				ret,
				context,
				now
			);
			...
			if (
				...
				blocks[0].reqs = Arrays.copyOf(
					blocks[0].reqs,
					Math.max(index * 2, MIN_SIZE)
				);
			} else if (
				blocks.length > 1 &&
				(newBlockCount = (((index + (BLOCK_SIZE / 2)) / BLOCK_SIZE) +
							1)) <
					blocks.length
			) {
	...
	private RemoveRandomReturn removeRandomExhaustiveSearch(
		...
		if (logMINOR) Logger.minor(
			this,
			"Doing exhaustive search and compaction on " + this
		);
		...
		while (true) {
			...
			if (valid == 0 && exclude == 0) {
				if (logMINOR) Logger.minor(
					this,
					"No valid or excluded items total " + index
				);
				...
				if (logMINOR) Logger.minor(
					this,
					"No valid or excluded items apart from " +
					ret +
					" total " +
					index
				);
	...
	public void remove(RandomGrabArrayItem it, ClientContext context) {
		...
		else if (oldArray != null) Logger.error(
			this,
			"Removing item " +
			it +
			" from " +
	...
	public void moveElementsTo(
		RandomGrabArray existingGrabber,
		boolean canCommit
	) {
