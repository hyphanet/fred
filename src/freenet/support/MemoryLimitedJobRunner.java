...
public class MemoryLimitedJobRunner {
	...
	public MemoryLimitedJobRunner(
		...
		this.counter = 0;
		this.jobs = (ArrayDeque<MemoryLimitedJob>[]) new ArrayDeque<
			?
		>[priorities];
	...
	private synchronized void maybeStartJobs() {
		...
		while (true) {
			...
			if (job == null) return;
			if (
				job.initialAllocation + counter <= capacity &&
	...
	private synchronized void startJob(final MemoryLimitedJob job) {
		...
		executor.execute(
			new PrioRunnable() {
				@Override
				public void run() {
					MemoryLimitedChunk chunk = new MemoryLimitedChunk(
						MemoryLimitedJobRunner.this,
