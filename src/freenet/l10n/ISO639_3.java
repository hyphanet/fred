...
public final class ISO639_3 {
	...
	public static final class LanguageCode implements Comparable<LanguageCode> {
		...
		public static enum Scope {
			...
			private static Scope fromTabFile(String abbreviation) {
				if (abbreviation.equals("I")) return Scope.Individual;
					...
					"Unknown scope abbreviation: " + abbreviation
				);
			}
		}
		...
		public static enum Type {
			...
			private static Type fromTabFile(String abbreviation) {
				if (abbreviation.equals("A")) return Type.Ancient;
					...
					"Unknwon type abbreviation: " + abbreviation
				);
			}
		}
		...
		private LanguageCode(
			...
			if (
				myPart2B != null && myPart2B.length > 3
			) throw new IllegalArgumentException();
			if (
				myPart2T != null && myPart2T.length > 3
			) throw new IllegalArgumentException();
			if (
				myPart1 != null && myPart1.length > 2
			) throw new IllegalArgumentException();
			...
			if (
				myReferenceName.length() > 150
			) throw new IllegalArgumentException();
			if (
				myComment != null && myComment.length() > 150
			) throw new IllegalArgumentException();
	...
	private static Hashtable<String, LanguageCode> loadFromTabFile() {
		final Hashtable<String, LanguageCode> codes = new Hashtable<
			String,
			LanguageCode
		>(7705 * 2);
		...
		try {
			...
			for (
				...
				if (
					tokens.length != 8 && tokens.length != 7
				) throw new RuntimeException(
					"Line with invalid token amount: " + line
				);
				...
				if (
					codes.put(newCode.id, newCode) != null
				) throw new RuntimeException(
					"Duplicate language code: " + newCode
				);
			}
		} catch (Exception e) {
	...
	public final Hashtable<String, LanguageCode> getLanguagesByScopeAndType(
		...
		for (final LanguageCode c : all.values()) {
			if (c.scope.equals(scope) && c.type.equals(type)) result.put(
				c.id,
				c
			); // We do not clone the code because all its fields are final.
		}
