		...
		private void complete(boolean failed) {
			...
			synchronized(BulkTransmitter.this) {
				if(failed) {
					...
					if(Logger.shouldLog(LoggerPriority.MINOR, this)) Logger.minor(this, "Packet failed for "+BulkTransmitter.this);
				} else {
					inFlightPackets--;
					if(inFlightPackets <= 0)
						BulkTransmitter.this.notifyAll();
					if(Logger.shouldLog(LoggerPriority.MINOR, this)) Logger.minor(this, "Packet sent "+BulkTransmitter.this+" remaining in flight: "+inFlightPackets);
				}
			}
		}
