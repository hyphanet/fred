...
public class BulkTransmitter {
	...
	public void completed() {
		...
		if (logMINOR) Logger.minor(
			this,
			"Completed transfer successfully " + this
		);
	}
	...
	public boolean send() throws DisconnectedException {
		...
		outer:while (true) {
			...
			if (peer.getBootID() != peerBootID) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Failed to send " + uid + ": peer restarted: " + peer
				);
			...
			if (buf == null) {
				if (logMINOR) Logger.minor(
					...
					"Block " +
					blockNo +
					" is null, presumably the send is cancelled: " +
					this
				);
			...
			try {
				...
				Message msg = DMT.createFNPBulkPacketSend(
					uid,
					blockNo,
					buf,
					realTime
				...
				synchronized (this) {
					while (inFlightPackets >= max && !failedPacket) try {
						wait(1000);
					} catch (InterruptedException e) {
						// Ignore
					}
		...
		if (allSentCallback != null) {
			...
			synchronized (this) {
				...
				if (unsentPackets == 0 && !calledAllSent) {
					if (logMINOR) Logger.minor(
						this,
						"Calling all sent callback on " + this
					);
					callAllSent = true;
					calledAllSent = true;
					...
					if (logMINOR) Logger.minor(
						this,
						"Still waiting for " + unsentPackets
					);
				}
			}
			if (callAllSent) {
		...
		private void complete(boolean failed) {
			...
			synchronized (BulkTransmitter.this) {
				if (failed) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Packet failed for " + BulkTransmitter.this
					);
				} else {
					inFlightPackets--;
					BulkTransmitter.this.notifyAll();
