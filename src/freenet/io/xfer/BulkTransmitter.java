...
public class BulkTransmitter {
     ...
     */
    public BulkTransmitter(PartiallyReceivedBulk prb, PeerContext peer, long uid, boolean noWait, ByteCounter ctr,
                           boolean realTime, AllSentCallback cb)
            throws DisconnectedException {
        this.prb = prb;
        this.peer = peer;
        this.uid = uid;
        ...
        try {
            prb.usm.addAsyncFilter(
                MessageFilter.create().setNoTimeout().setSource(peer).setType(DMT.FNPBulkReceiveAborted).setField(
                    DMT.UID, uid), new AsyncMessageFilterCallback() {
                @Override
            ...
            prb.usm.addAsyncFilter(
                MessageFilter.create().setNoTimeout().setSource(peer).setType(DMT.FNPBulkReceivedAll).setField(
                    DMT.UID, uid), new AsyncMessageFilterCallback() {
                @Override
                public void onMatched(Message m) {
        ...
        private void complete(boolean failed) {
            ...
            synchronized (BulkTransmitter.this) {
                if (failed) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Packet sent " + BulkTransmitter.this + " remaining in flight: "
                                     + inFlightPackets);
                    }
                }
            }
