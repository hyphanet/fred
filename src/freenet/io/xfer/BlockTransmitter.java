...
public class BlockTransmitter {
        ...
        public void onMatched(Message m) {
            if (logMINOR) {
                ...
                synchronized (avgTimeTaken) {
                    ...
                    Logger.minor(this,
                                 "Block send took " + transferTime + " : " + avgTimeTaken + " on "
    ...
    public BlockTransmitter(MessageCore usm, Ticker ticker, PeerContext destination, long uid,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Starting block transmit for " + uid + " to " + destination.shortToString() + " realtime="
                         + realTime);
        }
    }
    ...
    public boolean maybeAllSent() {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "maybeAllSent: block sends pending = " + blockSendsPending + " unsent = " + _unsent.size()
                         + " sent = " + getNumSent() + " on " + this);
        }
    ...
    private void onDisconnect() {
        Logger.normal(this,
                      "Terminating send " + _uid + " to " + _destination + " from " + _destination.getSocketHandler()
                    ...
                    public void packetReceived(int packetNo) {
                        synchronized (_senderThread) {
                            ...
                            if (_unsent.contains(packetNo)) {
                                Logger.error(this,
                                             "Already in unsent: " + packetNo + " for " + this + " unsent is "
                                             + _unsent, new Exception("error"));

                                return;
                            }

                            if (_sentPackets.bitAt(packetNo)) {
                                Logger.error(this, "Already sent packet in packetReceived: " + packetNo + " for "
                                             + this + " unsent is " + _unsent + " sent is "
                                             + _sentPackets, new Exception("error"));
    ...
    class BlockSenderJob implements PrioRunnable {
                ...
                while (true) {
                    ...
                    synchronized (_senderThread) {
                        ...
                        if (_unsent.size() == 0) {
                            ...
                            if (_sentPackets.bitAt(packetNo)) {
                                Logger.error(this, "Already sent packet in run(): " + packetNo + " for " + this
                                             + " unsent is " + _unsent + " sent is "
                                             + _sentPackets, new Exception("error"));
        ...
        public void schedule() {
            if (_failed || _receivedSendCompletion || _completed) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not scheduling for " + _uid + " to " + _destination + " :"
                                 + (_failed ? "(failed) " : "")
        ...
        private void complete(boolean failed) {
            ...
            synchronized (_senderThread) {
                ...
                if (lastSentPacket > 0) {
                    ...
                    long threshold = (realTime
                                      ? BlockReceiver.RECEIPT_TIMEOUT_REALTIME : BlockReceiver.RECEIPT_TIMEOUT_BULK);

                    if (delta > threshold) {
                        Logger.warning(this,
                                       "Time between packets on " + BlockTransmitter.this + " : "
                                       + TimeUtil.formatTime(delta, 2, true) + " ( " + delta + "ms) realtime="
                                       + realTime);
