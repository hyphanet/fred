    ...
    private AsyncMessageFilterCallback notificationWaiter = new SlowAsyncMessageFilterCallback() {
            ...
            if ((m1 != null) && (m1.getSpec().equals(DMT.packetTransmit))) {
                ...
                try {
                    ...
                    if (CHECK_DUPES && _prb.isReceived(packetNo)) {
                        ...
                        Logger.error(this,
                                     "Already received the packet - DoS??? on " + this + " uid " + _uid + " from "
                                     + _sender);
        ...
        public void onTimeout() {
            ...
            try {
                ...
                MessageFilter mfSendAborted = MessageFilter.create().setTimeout(ACK_TRANSFER_FAILED_TIMEOUT).setType(
...
//  private final boolean _doTooLong;
    ...
    public BlockReceiver(MessageCore usm, PeerContext sender, long uid, PartiallyReceivedBlock prb, ByteCounter ctr,
                         Ticker ticker, boolean doTooLong, boolean realTime,
...
//      _doTooLong = doTooLong;
    ...
    private void complete(int reason, String description) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Transfer failed: (" + (_realTime ? "realtime" : "bulk") + ") " + reason + " : " + description
    ...
    private MessageFilter relevantMessages(long timeout) {
        MessageFilter mfPacketTransmit =
            MessageFilter.create().setTimeout(timeout).setType(DMT.packetTransmit).setField(DMT.UID,
                _uid).setSource(_sender);
        MessageFilter mfAllSent = MessageFilter.create().setTimeout(timeout).setType(DMT.allSent).setField(DMT.UID,
                                      _uid).setSource(_sender);
        MessageFilter mfSendAborted =
            MessageFilter.create().setTimeout(timeout).setType(DMT.sendAborted).setField(DMT.UID,
                _uid).setSource(_sender);
