...
public class PartiallyReceivedBlock {
	...
	public PartiallyReceivedBlock(int packets, int packetSize, byte[] data) {
		if (data.length != packets * packetSize) {
			throw new RuntimeException(
				"Length of data (" +
				data.length +
				") doesn't match packet number and size"
			);
		}
		_data = data;
	...
	public synchronized Deque<Integer> addListener(
	...
	public void addPacket(int position, Buffer packet) throws AbortedException {
		...
		synchronized (this) {
			...
			prls = _packetReceivedListeners.toArray(
	...
	public synchronized boolean allReceived() throws AbortedException {
		if (_receivedCount == _packets) {
			if (logDEBUG) Logger.debug(
				this,
				"Received " + _receivedCount + " of " + _packets + " on " + this
			);
			return true;
		}
		if (_aborted) {
	...
	public synchronized byte[] getBlock() throws AbortedException {
		...
		throw new RuntimeException(
			"Tried to get block before all packets received"
		);
	}
	...
	public byte[] abort(
		...
		synchronized (this) {
			...
			listeners = _packetReceivedListeners.toArray(
				new PacketReceivedListener[_packetReceivedListeners.size()]
			);
