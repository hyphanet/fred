...
public class Peer implements WritableToDataOutputStream {
	...
	public static final String VERSION =
		"$Id: Peer.java,v 1.4 2005/08/25 17:28:19 amphibian Exp $";
	...
	public Peer(InetAddress address, int port) {
		...
		_port = port;
		if (_port > 65535 || _port < 0) throw new IllegalArgumentException(
			"bogus port"
		);
	}
	...
	public Peer(String physical, boolean allowUnknown)
		...
		try {
			...
			if (_port < 0 || _port > 65535) throw new PeerParseException(
				"Invalid port " + _port
			);
	...
	public Peer(
		...
		int offset = physical.lastIndexOf(':'); // ipv6
		if (offset < 0) throw new PeerParseException(
			"No port number: \"" + physical + "\""
		);
		String host = physical.substring(0, offset);
		addr = new FreenetInetAddress(
			host,
			allowUnknown,
	...
	public boolean isRealInternetAddress(
		...
		return addr.isRealInternetAddress(
			lookup,
			defaultVal,
			allowLocalAddresses
		);
	...
	public static class PeerComparator implements Comparator<Peer> {
		...
		public int compare(Peer p0, Peer p1) {
			...
			return InetAddressIpv6FirstComparator.COMPARATOR.compare(
				p0.getAddress(),
				p1.getAddress()
			);
		}
