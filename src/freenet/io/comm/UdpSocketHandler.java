...
//			}
	...
	private void realRun(DatagramPacket packet) {
		...
		if (gotPacket) {
			...
			long packetEndTime = System.currentTimeMillis();
			if(packetEndTime - packetStartTime > 50) {
				if(packetEndTime-packetStartTime > 3000) {
					Logger.error(this, "packet creation took "+(packetEndTime-packetStartTime)+"ms");
				} else {
					if(logMINOR) Logger.minor(this, "packet creation took "+(packetEndTime-packetStartTime)+"ms");
				}
			}
			...
			try {
				...
				if(packetEndTime - packetStartTime > 50) {
					if(packetEndTime-packetStartTime > 3000) {
						Logger.error(this, "processing packet took "+(packetEndTime-packetStartTime)+"ms");
					} else {
						if(logMINOR) Logger.minor(this, "processing packet took "+(packetEndTime-packetStartTime)+"ms");
					}
				}
