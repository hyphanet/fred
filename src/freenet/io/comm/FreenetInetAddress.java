...
public class FreenetInetAddress {
	...
	public FreenetInetAddress(DataInput dis, boolean checkHostnameOrIPSyntax)
		...
		if (checkHostnameOrIPSyntax && null != hostname) {
			if (
				!HostnameUtil.isValidHostname(hostname, true)
	...
	public FreenetInetAddress(
		...
		if (checkHostnameOrIPSyntax && null != this.hostname) {
			if (
				!HostnameUtil.isValidHostname(this.hostname, true)
			) throw new HostnameSyntaxException();
		}
		// we're created with a hostname so delay the lookup of the address
		// until it's needed to work better with dynamic DNS hostnames
	...
	public boolean laxEquals(FreenetInetAddress addr) {
		if (hostname != null) {
			if (addr.hostname == null) {
				...
				if (
					(_address != null) && (addr._address == null)
				) addr._address = _address;
				if ((addr._address != null) && (_address == null)) _address =
					addr._address;
	...
	public boolean equals(Object o) {
		...
		if (hostname != null) {
			...
			// Now that we know we have the same hostname, we can propagate the IP.
			if ((_address != null) && (addr._address == null)) addr._address =
				_address;
			if ((addr._address != null) && (_address == null)) _address =
				addr._address;
			// Except if we actually do have two different looked-up IPs!
	...
	public InetAddress getHandshakeAddress() {
		...
		if ((_address != null) && (hostname == null)) {
			if (logMINOR) Logger.minor(
				this,
				"hostname is null, returning " + _address
			);
			return _address;
		} else {
			if (logMINOR) Logger.minor(
				this,
				"Looking up '" + hostname + "' in DNS",
				new Exception("debug")
			);
			/*
			...
			try {
				...
				if (logMINOR) Logger.minor(
					this,
					"Look up got '" + addresses + '\''
				);
				if (addresses.length > 1) {
					/* sort by IPv6 first */
					Arrays.sort(
						addresses,
						InetAddressIpv6FirstComparator.COMPARATOR
					);
					/*
					 ...
					 */
					this._address = InetAddress.getByAddress(
						addresses[0].getAddress()
					);
					if (logMINOR) Logger.minor(
						this,
						"Setting address to " + _address
					);
				}
				return addresses[0];
			} catch (UnknownHostException e) {
				if (logMINOR) Logger.minor(
					...
					"DNS said hostname '" +
					hostname +
					"' is an unknown host, returning null"
				);
				return null;
			}
		}
	...
	public boolean isRealInternetAddress(
		...
		if (_address != null) {
			...
			if (lookup) {
				...
				if (a != null) return IPUtil.isValidAddress(
					a,
					allowLocalAddresses
				);
			}
