...
public class UdpSocketHandlerImpl implements PrioRunnable, PortForwardSensitiveSocketHandler, UdpSocketHandler {
	...
	private final IOStatisticCollector collector;
        ...
        static {
            Logger.registerClass(UdpSocketHandlerImpl.class);
        }
	private static class socketOptions {
		...
		private static int getFd(DatagramSocket s) {
			...
			try {
			   ...
			   Logger.error(UdpSocketHandlerImpl.class, e.getMessage(), e);
			}
			return ret;
		}
		...
		public static boolean setAddressPreference(DatagramSocket s, SOCKET_ADDR_PREFERENCE p) {
			...
			} catch(Exception e) { Logger.normal(UdpSocketHandlerImpl.class, e.getMessage(),e); } //if it fails that's fine
			return (ret == 0 ? true : false);
		}
	}

	public UdpSocketHandlerImpl(int listenPort, InetAddress bindto, Node node, long startupTime, String title, IOStatisticCollector collector) throws SocketException {
		this.node = node;
