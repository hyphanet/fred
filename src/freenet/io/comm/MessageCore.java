...
public class MessageCore {
	...
	public Message decodeSingleMessage(
		...
		try {
			return Message.decodeMessageFromPacket(
				data,
				offset,
				length,
				peer,
			...
			new Runnable() {
				...
				public void run() {
					...
					try {
						...
						Logger.error(
							this,
							"Failed to remove timed out filters: " + t,
							t
						);
					} finally {
						ticker.queueTimedJob(
	...
	long removeTimedOutFilters(long nextTimeout) {
			...
			for (
				...
				if (f.timedOut(tStart)) {
					...
					if (timedOutFilters == null) timedOutFilters = new HashSet<
						MessageFilter
					>();
					if (!timedOutFilters.add(f)) Logger.error(
						this,
						"Filter " + f + " is in filter list twice!"
					);
					if (logMINOR) {
						for (
							ListIterator<Message> it =
								_unclaimed.listIterator();
							it.hasNext();
						) {
							Message m = it.next();
							...
							if (status == MATCHED.MATCHED) {
								...
								Logger.error(
									this,
									"Timed out but should have matched in _unclaimed: " +
									m +
									" for " +
									f
								);
					...
					if (
						f.hasCallback() && nextTimeout > f.getTimeout()
		...
		if (tEnd - tStart > 50) {
			if (tEnd - tStart > 3000) Logger.error(
				this,
	...
	public void checkFilters(Message m, PacketSocketHandler from) {
		...
		if (logMINOR) Logger.minor(
			this,
			"checkFilters: " + m + " from " + m.getSource()
		);
		if ((m.getSource()) instanceof PeerNode) {
			((PeerNode) m.getSource()).addToLocalNodeReceivedMessagesFromStatistic(
					m
				);
		}
		...
		synchronized (_filters) {
			for (
				...
				if (f.matched()) {
					Logger.error(
						this,
						"removed pre-matched message filter found in _filters: " +
						f
					);
					i.remove();
					continue;
				...
				if (
					...
					status == MATCHED.TIMED_OUT_AND_MATCHED
				) {
		...
		if (!matched) {
			...
			synchronized (_filters) {
				if (logMINOR) Logger.minor(
					this,
					"Rechecking filters and adding message"
				);
				for (
					ListIterator<MessageFilter> i = _filters.listIterator();
					i.hasNext();
				...
				if (!matched) {
					while (_unclaimed.size() > MAX_UNMATCHED_FIFO_SIZE) {
						...
						long messageLifeTime =
							...
							removed.localInstantiationTime;
	...
	public void addAsyncFilter(
		...
		if (logMINOR) Logger.minor(
			this,
			"Adding async filter " + filter + " for " + callback
		);
		Message ret = null;
		...
		synchronized (_filters) {
			...
			for (
				ListIterator<Message> i = _unclaimed.listIterator();
				i.hasNext();
			) {
				Message m = i.next();
	 ...
	 */
	public Message waitFor(MessageFilter filter, ByteCounter ctr)
		throws DisconnectedException {
		if (logDEBUG) Logger.debug(this, "Waiting for " + filter);

		if (filter.hasCallback()) {
			throw new IllegalArgumentException(
				"waitFor called with a filter that has a callback"
			);
		}
		...
		if (logDEBUG) Logger.debug(
			this,
			"Returning in " + (endTime - startTime) + "ms"
		);
		if ((ctr != null) && (ret != null)) ctr.receivedBytes(
			ret._receivedByteCount
		);
		return ret;
	}
	...
	public void send(PeerContext destination, Message m, ByteCounter ctr)
		...
		if (m.getSpec().isInternalOnly()) {
			Logger.error(
				this,
				"Trying to send internal-only message " +
				m +
				" of spec " +
				m.getSpec(),
				new Exception("debug")
