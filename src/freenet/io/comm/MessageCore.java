...
public class MessageCore {
    ...
    public void checkFilters(Message m, PacketSocketHandler from) {
                ...
                if (!matched) {
                    while (_unclaimed.size() > MAX_UNMATCHED_FIFO_SIZE) {
                        ...
                        if ((removed.getSource()) instanceof PeerNode) {
                            Logger.normal(this,
                                          "Dropping unclaimed from " + removed.getSource().getPeer() + ", lived "
                                          + TimeUtil.formatTime(messageLifeTime, 2, true) + " (quantity)" + ": "
                                          + removed);
                        } else {
                            Logger.normal(this,
                                          "Dropping unclaimed, lived " + TimeUtil.formatTime(messageLifeTime, 2, true)
                                          + " (quantity)" + ": " + removed);
                        }
                    }
        ...
        if (dT > 50) {
            if (dT > 3000) {
                Logger.error(this,
                             "checkFilters took " + (dT) + "ms with unclaimedFIFOSize of " + _unclaimed.size()
                             + " for matched: " + matched);
            } else if (logMINOR) {
                Logger.minor(this,
                             "checkFilters took " + (dT) + "ms with unclaimedFIFOSize of " + _unclaimed.size()
        ...
        synchronized (_filters) {
            ...
            for (ListIterator<Message> i = _unclaimed.listIterator(); i.hasNext(); ) {
                ...
                if (status == MATCHED.MATCHED) {
                    ...
                    if ((m.getSource()) instanceof PeerNode) {
                        Logger.normal(this,
                                      "Dropping unclaimed from " + m.getSource().getPeer() + ", lived "
            ...
            if ((ret == null) && (timeout >= System.currentTimeMillis())) {
                ...
                while (true) {
                    ...
                    if (mf.getTimeout() > timeout) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Added in middle - mf timeout=" + mf.getTimeout() + " - my timeout="
                                         + filter.getTimeout());
                        }
    ...
    public Message waitFor(MessageFilter filter, ByteCounter ctr) throws DisconnectedException {
        ...
        if (tEnd - now > 50) {
            if (tEnd - now > 3000) {
                Logger.error(this,
                             "waitFor _unclaimed iteration took " + (tEnd - now) + "ms with unclaimedFIFOSize of "
