...
public class CHKStore extends StoreCallback<CHKBlock> {
	...
	public CHKBlock construct(byte[] data, byte[] headers,
			byte[] routingKey, byte[] fullKey, boolean canReadClientCache, boolean canReadSlashdotCache, boolean mustBeMarkedAsPostCachingChanges, DSAPublicKey ignored) throws KeyVerifyException {
	...
	public CHKBlock fetch(NodeCHK chk, boolean dontPromote, boolean mustBeMarkedAsPostCachingChanges) throws IOException {
		return store.fetch(chk.getRoutingKey(), null, dontPromote, false, false, mustBeMarkedAsPostCachingChanges);
	}
