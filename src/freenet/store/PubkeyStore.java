...
public class PubkeyStore extends StoreCallback<DSAPublicKey> {
	...
	public DSAPublicKey construct(byte[] data, byte[] headers, byte[] routingKey,
			byte[] fullKey, boolean canReadClientCache, DSAPublicKey ignored) throws KeyVerifyException {
	...
	public DSAPublicKey fetch(byte[] hash, boolean dontPromote, boolean canReadClientCache) throws IOException {
		return store.fetch(hash, null, dontPromote, false);
	}
