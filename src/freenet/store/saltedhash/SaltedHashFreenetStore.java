...
public class SaltedHashFreenetStore<T extends StorableBlock> implements FreenetStore<T> {
    ...
    private SaltedHashFreenetStore(File baseDir, String name, StoreCallback<T> callback, Random random, long maxKeys,
                                   boolean enableSlotFilters, SemiOrderedShutdownHook shutdownHook,
                                   boolean preallocate, boolean resizeOnStart, byte[] masterKey)
            throws IOException {
        ...
        if (!slotFilterDisabled) {
            ...
            if (slotFilterFile.exists()) {
                if (slotFilterFile.delete()) {
                    ...
                    System.err.println(
                        "Old slot filter file " + slotFilterFile
                ...
                while (currentHdLen < newHdLen) {
                    ...
                    if (currentHdLen % (1024 * 1024 * 1024L) == 0) {
                        ...
                        if (starting) {
                            ...
                            if (x++ % 32 == 0) {
                                System.err.println("Preallocating space for " + name + ": " + currentHdLen + "/"
                                                   + newHdLen);
                            }
                        }
                    }
    ...
    private boolean loadConfigFile(byte[] masterKey) throws IOException {
        ...
        if (!configFile.exists()) {
            ...
            if (masterKey != null) {
                ...
                if (logDEBUG) {
                    Logger.debug(this,
                                 "Encrypting with " + HexUtil.bytesToHex(newsalt) + " from "
            ...
            try {
                ...
                try {
                    ...
                    if (masterKey != null) {
                        ...
                        if (logDEBUG) {
                            Logger.debug(this,
                                         "Encrypting (new) with " + HexUtil.bytesToHex(salt) + " from "
                                         + HexUtil.bytesToHex(diskSalt));
                        }
                    }
        ...
        private void resizeStore(final long _prevStoreSize, final boolean sleep) {
            ...
            BatchProcessor<T> resizeProcesser = new BatchProcessor<T>() {
                ...
                public void init() {
                    ...
                    WrapperManager.signalStarting((int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(30)
                            + SECONDS.toMillis(1)));
                }
                @Override
        ...
        private void rebuildBloom(boolean sleep) {
            ...
            BatchProcessor<T> rebuildBloomProcessor = new BatchProcessor<T>() {
                ...
                public void init() {
                    ...
                    WrapperManager.signalStarting((int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(5)
                            + SECONDS.toMillis(1)));
                }
                @Override
                public Entry process(Entry entry) {
        ...
        public String getShortText() {
            if (cleaner.isResizing) {
                ...
                return NodeL10n.getBase().getString("SaltedHashFreenetStore.shortRebuildProgress"
                        + (slotFilter.isNew() ? "New" : ""), new String[] { "name",
                        "processed", "total" },    //
                        new String[] { name, String.valueOf(cleaner.entriesTotal - cleaner.entriesLeft),
                                       String.valueOf(cleaner.entriesTotal) });
            }
        ...
        public String getText() {
            if (cleaner.isResizing) {
                ...
                return NodeL10n.getBase().getString("SaltedHashFreenetStore.longRebuildProgress"
                        + (slotFilter.isNew() ? "New" : ""), new String[] { "name",
                        "processed", "total" }, new String[] { name,
    ...
    class Entry {
        ...
        private T getStorableBlock(byte[] routingKey, byte[] fullKey, boolean canReadClientCache,
                                   boolean canReadSlashdotCache, BlockMetadata meta, DSAPublicKey knownKey)
