...
public class CachingFreenetStore<T extends StorableBlock>
	...
	public T fetch(
		...
		if (block != null) {
			try {
				...
				Logger.error(
					this,
					"Error in fetching for CachingFreenetStore: " + e,
					e
				);
			}
		}
	...
	long pushLeastRecentlyBlock() {
		...
		try {
			...
			Logger.error(
				this,
				"Error in pushAll for CachingFreenetStore: " + e,
				e
			);
		} catch (KeyCollisionException e) {
			if (logMINOR) Logger.minor(
				this,
				"KeyCollisionException in pushAll for CachingFreenetStore: " +
				e,
				e
			);
		}
		...
		try {
			...
			if (
				currentVersionOfBlock != null &&
				currentVersionOfBlock.block.equals(block.block)
			) {
