...
public class JceLoader {
	...
	private static boolean checkUse(String prop, String def) {
		return "true".equalsIgnoreCase(
				System.getProperty("freenet.jce." + prop, def)
			);
	}
	...
	static {
		...
		if (checkUse("use.BC.I.know.what.I.am.doing")) {
			try {
				...
				final String msg =
					"SERIOUS PROBLEM: Unable to load or use BouncyCastle provider.";
		...
		if (checkUse("use.SunJCE")) {
			try {
				...
				Logger.warning(
					NSSLoader.class,
					"Error with SunJCE. Unlimited policy file not installed.",
					e
				);
			}
	...
	private static class BouncyCastleLoader {
		...
		private Provider load() throws Throwable {
			...
			if (p == null) {
				...
				Logger.debug(
					BouncyCastleLoader.class,
					"Loaded BouncyCastle provider: " + p
				);
			} else {
				Logger.debug(
					BouncyCastleLoader.class,
					"Found BouncyCastle provider: " + p
				);
			}
			try {
				...
				throw new Error(
					"Cannot use required algorithm from BouncyCaste provider",
					e
				);
			}
			return p;
	...
	private static class NSSLoader {
		...
		private Provider load(boolean atfirst) throws Throwable {
			...
			if (nssProvider == null) {
				...
				nssProvider = (Provider) constructor.newInstance(
					nssFile.getPath()
				);
				...
				Logger.debug(
					NSSLoader.class,
					"Loaded NSS provider " + nssProvider
				);
			} else {
				Logger.debug(
					NSSLoader.class,
					"Found NSS provider " + nssProvider
				);
			}
			return nssProvider;
		}
