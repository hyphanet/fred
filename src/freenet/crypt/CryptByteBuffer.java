...
public final class CryptByteBuffer implements Serializable {
	...
	public CryptByteBuffer(
		...
		try {
			if (type.cipherName.equals("RIJNDAEL")) {
				blockCipher = new Rijndael(
					type.keyType.keySize,
					type.blockSize
				);
	...
	public void encrypt(
		...
		if (type == CryptByteBufferType.RijndaelPCFB) {
			...
			if (
				...
				blockCipher.encipher(
					Arrays.copyOfRange(input, offset, offset + len),
					result
				);
			...
			try {
				int copied = encryptCipher.update(
					input,
					offset,
					len,
					output,
				...
				if (copied != len) throw new IllegalStateException(
					"Not a stream cipher???"
				);
	...
	public ByteBuffer encryptCopy(ByteBuffer input) {
		if (input.hasArray()) return ByteBuffer.wrap(
			encryptCopy(
				input.array(),
	...
	public void encrypt(ByteBuffer input, ByteBuffer output) {
		if (input.hasArray() && output.hasArray()) {
			...
			!(type == CryptByteBufferType.RijndaelPCFB ||
				type.cipherName.equals("RIJNDAEL"))
		) {
			...
			try {
				...
				if (copied != copy) throw new IllegalStateException(
					"Not a stream cipher???"
				);
			} catch (ShortBufferException e) {
	...
	public void decrypt(
		...
		if (type == CryptByteBufferType.RijndaelPCFB) {
			...
			try {
				int copied = decryptCipher.update(
					input,
					offset,
					len,
					output,
