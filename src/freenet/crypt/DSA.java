...
public class DSA {
     ...
     * of the message (m)
     */
    static DSASignature sign(DSAGroup g, DSAPrivateKey x, BigInteger r, BigInteger kInv, BigInteger m,
                             RandomSource random) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        freenet.support.SimpleFieldSet fs = ((args.length >= 1) && (args[0].length() != 0))
                                            ? freenet.support.SimpleFieldSet.readFrom(new java.io.File(args[0]), false,
                                                false) : null;
        ...
        while (true) {
            ...
            System.out.println("\tavg=" + ((double) totalTimeSigning / timeSigning.length) + "\tmed="
                               + timeSigning[timeSigning.length / 2] + "\tmin=" + timeSigning[0] + "\tmax="
                               + timeSigning[timeSigning.length - 1]);
            System.out.println("Total time verifying: " + totalTimeVerifying);
            java.util.Arrays.sort(timeVerifying);
            System.out.println("\tavg=" + ((double) totalTimeVerifying / timeVerifying.length) + "\tmed="
                               + timeVerifying[timeVerifying.length / 2] + "\tmin=" + timeVerifying[0] + "\tmax="
                               + timeVerifying[timeVerifying.length - 1]);
