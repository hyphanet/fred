	...
	public OCBBlockCipher_v149(BlockCipher hashCipher, BlockCipher mainCipher) {
		...
		if (hashCipher.getBlockSize() != BLOCK_SIZE) {
			throw new IllegalArgumentException(
				"'hashCipher' must have a block size of " + BLOCK_SIZE
			);
		}
		if (mainCipher == null) {
		...
		if (mainCipher.getBlockSize() != BLOCK_SIZE) {
			throw new IllegalArgumentException(
				"'mainCipher' must have a block size of " + BLOCK_SIZE
			);
		}

		if (
			!hashCipher.getAlgorithmName().equals(mainCipher.getAlgorithmName())
		) {
			throw new IllegalArgumentException(
	...
	public void init(boolean forEncryption, CipherParameters parameters)
		...
		if (parameters instanceof AEADParameters) {
			...
			if (macSizeBits < 64 || macSizeBits > 128 || macSizeBits % 8 != 0) {
				throw new IllegalArgumentException(
					"Invalid value for MAC size: " + macSizeBits
				);
			}
			...
			throw new IllegalArgumentException(
				"invalid parameters passed to OCB"
			);
		}

		this.hashBlock = new byte[16];
		this.mainBlock = new byte[forEncryption
			? BLOCK_SIZE
			: (BLOCK_SIZE + macSize)];
		...
		if (N.length > 16 || (N.length == 16 && (N[0] & 0x80) != 0)) {
			 ...
			 */
			throw new IllegalArgumentException(
				"IV must be no more than 127 bits"
			);
		}
		...
		if (bits == 0) {
			...
			for (int i = 0; i < 16; ++i) {
				...
				this.OffsetMAIN_0[i] = (byte) ((b1 << bits) |
					(b2 >>> (8 - bits)));
			}
		}
