...
public class DiffieHellman {
    ...
    public static NativeBigInteger[] getParams() {
        ...
        Logger.normal(DiffieHellman.class,
                      "DiffieHellman had to generate a parameter on thread! (report if that happens often)");
    ...
    public static boolean checkDHExponentialValidity(Class<?> caller, BigInteger exponential) {
        ...
        if ((onesCount < 16) || (zerosCount < 16)) {
            Logger.error(caller,
                         "The provided exponential contains " + zerosCount + " zeros and " + onesCount
                         + " ones wich is unacceptable!");
        ...
        if (MAX_EXPONENTIAL_VALUE.compareTo(exponential) < 1) {
            Logger.error(DiffieHellman.class,
                         "The provided exponential is bigger than (p - 2^24) which is unacceptable!");
