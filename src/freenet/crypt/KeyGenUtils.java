...
public final class KeyGenUtils {
	...
	public static KeyPair getKeyPair(
		...
		return getKeyPair(
			type,
			Fields.copyToArray(pub),
			Fields.copyToArray(pri)
		);
	}
	...
	public static SecretKey getSecretKey(KeyType type, byte[] key) {
		if (
			!type.name().startsWith("HMAC") && key.length != type.keySize >> 3
		) {
			throw new IllegalArgumentException(
				"Key size does not match KeyType"
			);
		}
	 ...
	 */
	private static ByteBuffer deriveBytes(
		SecretKey kdfKey,
		Class<?> c,
		String kdfString
	) throws InvalidKeyException {
		...
		return kdf.genMac(
			(c.getName() + kdfString).getBytes(StandardCharsets.UTF_8)
		);
	}
	...
	public static IvParameterSpec deriveIvParameterSpec(
		...
		return getIvParameterSpec(
			deriveBytesTruncated(kdfKey, c, kdfString, ivType.ivSize >> 3)
		);
	}
}
