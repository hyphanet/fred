...
public class HashResult
	implements Comparable<HashResult>, Cloneable, Serializable {
	...
	public static HashResult[] readHashes(DataInputStream dis)
		throws IOException {
	...
	private static HashResult readFrom(HashType h, DataInputStream dis)
		throws IOException {
	...
	public static void write(HashResult[] hashes, DataOutputStream dos)
		throws IOException {
		...
		for (HashResult h : hashes) {
			if (
				prev == h.type ||
				(prev != null && prev.bitmask == h.type.bitmask)
			) throw new IllegalArgumentException(
				"Multiple hashes of the same type!"
			);
			prev = h.type;
		}
	...
	public static boolean strictEquals(
		HashResult[] results,
		HashResult[] hashes
	) {
		...
		for (int i = 0; i < results.length; i++) {
			if (results[i].type != hashes[i].type) {
				// FIXME Db4o kludge
				if (
					HashType.valueOf(results[i].type.name()) !=
					HashType.valueOf(hashes[i].type.name())
				) {
					Logger.error(
						HashResult.class,
			...
			if (!Arrays.equals(results[i].result, hashes[i].result)) {
				Logger.error(
					HashResult.class,
					"Hash " + results[i].type.name() + " not equal"
				);
				return false;
			}
		}
	...
	public static boolean contains(HashResult[] hashes, HashType type) {
		for (HashResult res : hashes) if (
			res.type == type || type.name().equals(res.type.name())
		) return true;
		return false;
	}
