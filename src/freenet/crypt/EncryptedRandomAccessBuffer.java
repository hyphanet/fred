...
public final class EncryptedRandomAccessBuffer
	...
	private void setup(MasterSecret masterKey, boolean newFile)
		...
		if (!newFile && END_MAGIC != magic) {
			throw new IOException(
				"This is not an EncryptedRandomAccessBuffer!"
			);
		}

		version = type.bitmask;
		if (newFile) {
			this.headerEncIV = KeyGenUtils.genIV(
				type.encryptType.ivSize
			).getIV();
			...
			if (readVersion != version) {
				throw new IOException(
					"Version of the underlying RandomAccessBuffer is " +
		...
		try {
			KeyParameter cipherKey = new KeyParameter(
	...
	private void writeHeader() throws IOException, GeneralSecurityException {
		...
		try {
			...
			byte[] macResult = Fields.copyToArray(
				mac.genMac(headerEncIV, unencryptedBaseKey.getEncoded(), ver)
			);
	...
	private boolean verifyHeader() throws IOException, InvalidKeyException {
		...
		try {
			...
			unencryptedBaseKey = KeyGenUtils.getSecretKey(
				type.encryptKey,
		...
		byte[] ver = ByteBuffer.allocate(4).putInt(version).array();
		MessageAuthCode authcode = new MessageAuthCode(
			type.macType,
			headerMacKey
		);
		return authcode.verifyData(
			mac,
			headerEncIV,
	...
	public static LockableRandomAccessBuffer create(
		...
		if (type == null) throw new StorageFormatException(
			"Unknown EncryptedRandomAccessBufferType"
		);
		LockableRandomAccessBuffer underlying = BucketTools.restoreRAFFrom(
			dis,
			fg,
	...
	public int hashCode() {
		...
		result = prime * result +
		((underlyingBuffer == null) ? 0 : underlyingBuffer.hashCode());
		return result;
	}
