...
public abstract class ChecksumChecker {
	 ...
	 * underlying stream. */
	public abstract OutputStream checksumWriter(
		OutputStream os,
		int skipPrefix
	);
	...
	public void verifyChecksum(
		...
		if (
			!checkChecksum(data, offset, length, checksum)
		) throw new ChecksumFailedException();
	}

	/** Verify a checksum or report.
	 * @return True if the checksum is correct, false otherwise. */
	public abstract boolean checkChecksum(
		byte[] data,
		int offset,
		int length,
		byte[] checksum
	...
	public abstract byte[] generateChecksum(
		byte[] bufToChecksum,
		int offset,
		int length
	);
	 ...
	 * @throws IOException
	 * @throws ChecksumFailedException */
	public abstract void copyAndStripChecksum(
		InputStream is,
		OutputStream os,
		long length
	 ...
	 * be zero'ed out. */
	public abstract void readAndChecksum(
		DataInput is,
		byte[] buf,
		int offset,
		int length
	...
	public InputStream checksumReaderWithLength(
		InputStream dis,
		BucketFactory bf,
		long maxLength
	) throws IOException, ChecksumFailedException {
		...
		if (length < 0 || length > maxLength) {
			throw new IOException(
				"Bad length: " + length + "; maxLength: " + maxLength
			);
		}
	...
	public void writeAndChecksum(ObjectOutputStream oos, byte[] buf)
		throws IOException {
