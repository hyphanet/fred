...
public class EncryptedRandomAccessBucket
	...
	private SkippingStreamCipher setup(InputStream is)
		...
		if (!verifyHeader(fullHeader)) throw new GeneralSecurityException(
			"MAC is incorrect"
		);
		setupKeys();
	...
	private boolean verifyHeader(byte[] fullHeader)
	...
	public LockableRandomAccessBuffer toRandomAccessBuffer()
		throws IOException {
		if (underlying.size() < type.headerLen) throw new IOException(
			"Converting empty bucket"
		);
		underlying.setReadOnly();
	...
	public EncryptedRandomAccessBucket(
		...
		if (type == null) throw new ResumeFailedException(
			"Unknown EncryptedRandomAccessBucket type"
		);
