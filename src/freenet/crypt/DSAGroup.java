...
public class DSAGroup extends CryptoKey {
	...
	public DSAGroup(BigInteger p, BigInteger q, BigInteger g) {
		...
		this.g = g;
		if (
			p.signum() != 1 || q.signum() != 1 || g.signum() != 1
	...
	public static CryptoKey read(InputStream i)
		...
		try {
			...
			throw (CryptFormatException) new CryptFormatException(
				"Invalid group: " + e
			).initCause(e);
		}
	}
	...
	public static DSAGroup create(SimpleFieldSet fs)
		...
		String myG = fs.get("g");
		if (
			myP == null || myQ == null || myG == null
		) throw new FSParseException(
			"The given SFS doesn't contain required fields!"
		);
