...
public class ECDSA {
	...
	public enum Curves {
		...
		private static KeyPair selftest(
			KeyPairGenerator kg,
			KeyFactory kf,
			int modulusSize
			...
			if (
				pubkey.length > modulusSize || pubkey.length == 0
			) throw new Error(
			...
			if (!Arrays.equals(pub2.getEncoded(), pubkey)) throw new Error(
				"Pubkey encoding mismatch"
			);
		...
		private Curves(
			...
			try {
				...
				try {
					...
					kg = KeyPairGenerator.getInstance(
						"EC",
			...
			Logger.normal(
				this,
				name + ": using " + kgProvider + " for KeyPairGenerator(EC)"
			);
			Logger.normal(
				this,
				name + ": using " + kfProvider + " for KeyFactory(EC)"
			);
			Logger.normal(
				this,
				name +
		...
		try {
			while (true) {
				...
				Signature sig = Signature.getInstance(
					curve.defaultHashAlgorithm.replace("ECDSA", "ECDDSA"),
					JceLoader.BouncyCastle
				);
				sig.initSign(key.getPrivate());
	...
	private static int actualSignatureLength(
		byte[] signature,
		int sigOff,
		int sigLen
	...
	public static boolean verify(
		...
		if (
			key == null || curve == null || signature == null || data == null
		) return false;
		boolean result = false;
		try {
			Signature sig = Signature.getInstance(
				curve.defaultHashAlgorithm,
				curve.sigProvider
			);
			sig.initVerify(key);
			for (byte[] d : data) sig.update(d);
	...
	public SimpleFieldSet asFieldSet(boolean includePrivate) {
		...
		if (includePrivate) fsCurve.putSingle(
			"pri",
			Base64.encode(key.getPrivate().getEncoded())
		);
