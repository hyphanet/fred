	...
	public FreenetURI checkSource(String source)
		...
		try {
			...
			throw new PluginNotFoundException(
				"not a valid freenet key: " + source,
				e
			);
		}
	}
	...
	InputStream getInputStream(final PluginProgress progress)
		...
		while (true) {
			try {
				...
				if (
					(e.getMode() == FetchExceptionMode.PERMANENT_REDIRECT) ||
				...
				Logger.error(
					this,
					"error while fetching plugin: " + getSource(),
					e
				);
				throw new PluginNotFoundException(
					"error while fetching plugin: " +
					e.getMessage() +
					" for key " +
					getSource(),
					e
