...
public class PluginTalker {
	...
	public PluginTalker(
		...
		replysender = new PluginReplySenderDirect(
			node2,
			fpt,
			pluginname2,
			clientIdentifier,
	...
	public PluginTalker(
		...
		replysender = new PluginReplySenderFCP(
			handler,
			pluginname2,
			clientIdentifier,
			clientSideIdentifier
	...
	public void sendSyncInternalOnly(
		final SimpleFieldSet plugparams,
		final Bucket data2
	) {
		try {
			...
			if (plug == null) {
				...
				Logger.error(
					this,
					"Connection to plugin '" + pluginName + "' lost.",
					new Exception("FIXME")
				);
				return;
			}
