...
public class PluginRespirator {
	...
	private static final ArrayList<SessionManager> sessionManagers =
		new ArrayList<SessionManager>(4);
	...
	public PluginRespirator(Node node, PluginInfoWrapper pi) {
		this.node = node;
		this.hlsc = node.clientCore.makeClient(
	...
	public FilterCallback makeFilterCallback(String path) {
		try {
			return node.clientCore.createFilterCallback(
				URIPreEncoder.encodeURI(path),
				null
			);
	...
	public HTMLNode addFormChild(
		...
		HTMLNode formNode = parentNode.addChild(
			"form",
			new String[] {
				"action",
				"method",
	 ...
	 */
	@Deprecated
	public PluginTalker getPluginTalker(
		FredPluginTalker fpt,
		String pluginname,
		String identifier
	...
	public FCPPluginConnection connectToOtherPlugin(
		...
		if (messageHandler == null) throw new NullPointerException(
	 ...
	 *     discard it. */
	public FCPPluginConnection getPluginConnectionByID(UUID connectionID)
		throws IOException {
		return node.clientCore
			.getFCPServer()
	...
	public PluginStore getStore() throws PersistenceDisabledException {
		synchronized (this) {
			...
			store = stores.loadPluginStore(
				this.plugin.getClass().getCanonicalName()
			);
	...
	public void putStore(final PluginStore store)
		...
		try {
			...
			System.err.println(
				"Unable to write plugin data for " + name + " : " + e
			);
			return;
		}
	}
