...
public class PluginStores {
	...
	public PluginStores(Node node, SubConfig installConfig)
		throws NodeInitException {
		this.node = node;
		pluginStoresDir = node.setupProgramDir(
			installConfig,
			"pluginStoresDir",
			"plugin-data",
		...
		if (
			...
			System.err.println(
				"Unable to create folder for plugin data: " +
				pluginStoresDir.dir()
			);
		}
	}
	...
	private void writePluginStoreInner(
		...
		Bucket bucket = makePluginStoreBucket(
			storeIdentifier,
			isEncrypted,
			backup
		);
	...
	private File getPluginStoreFile(
		String storeIdentifier,
		boolean encrypted,
		boolean backup
	) {
	...
	private Bucket makePluginStoreBucket(
		String storeIdentifier,
		boolean isEncrypted,
		boolean backup
	...
	public PluginStore loadPluginStore(String storeIdentifier) {
		...
		PluginStore store = loadPluginStore(
			storeIdentifier,
			isEncrypted,
			false
		);
	...
	private PluginStore loadPluginStore(
		...
		try {
			try {
				...
				SimpleFieldSet fs = SimpleFieldSet.readFrom(
					is,
					false,
					false,
					true,
			...
			System.err.println(
				"Unable to load plugin data for " + storeIdentifier + " : " + e
			);
			System.err.println(
				"This could be caused by data corruption or bugs in Freenet."
			);
	...
	public void writePluginStore(String storeIdentifier, PluginStore store)
		...
		if (main.exists()) {
			if (!main.renameTo(backup)) System.err.println(
				"Unable to rename " +
				main +
				" to " +
				backup +
