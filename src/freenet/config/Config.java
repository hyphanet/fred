...
public class Config {
	...
	public void register(SubConfig sc) {
		synchronized (this) {
			if (
				configsByPrefix.containsKey(sc.prefix)
			) throw new IllegalArgumentException(
				"Already registered " + sc.prefix + ": " + sc
			);
	...
	public void finishedInit() {
		...
		for (SubConfig config : configs) {
			if (!config.hasFinishedInitialization()) Logger.error(
				this,
				"Not finished initialization: " + config.prefix
			);
		}
	}
