...
public class NodeSSK extends Key {
	...
	public NodeSSK(byte[] pkHash, byte[] ehDocname, byte cryptoAlgorithm) {
		...
		this.pubKey = null;
		if (
			ehDocname.length != E_H_DOCNAME_SIZE
		) throw new IllegalArgumentException(
			"ehDocname must be " + E_H_DOCNAME_SIZE + " bytes"
		);
		if (
			pkHash.length != PUBKEY_HASH_SIZE
		) throw new IllegalArgumentException(
			"pubKeyHash must be " + PUBKEY_HASH_SIZE + " bytes"
		);
	...
	public NodeSSK(
		...
		if (pubKey != null) {
			...
			if (!Arrays.equals(hash, pkHash)) throw new SSKVerifyException(
	...
	public static Key readSSK(DataInput raf, byte cryptoAlgorithm)
		throws IOException {
		...
		try {
			...
			throw (AssertionError) new AssertionError("Impossible").initCause(
				e
			);
		}
	}
	...
	@Override
	public void writeToDataOutputStream(DataOutputStream stream)
		throws IOException {
		write(stream);
	}
			...
			if (pubKey2 != null) {
				...
				if (Arrays.equals(pubKeyHash, newPubKeyHash)) {
					if (pubKey != null) {
						...
						Logger.error(
							this,
							"Found SHA-256 collision or something... WTF?"
						);
	...
	public boolean equals(Object o) {
		...
		if (
			!Arrays.equals(key.encryptedHashedDocname, encryptedHashedDocname)
		) return false;
	...
	public static NodeSSK construct(byte[] buf) throws SSKVerifyException {
		if (buf[0] != 2) throw new SSKVerifyException(
			"Unknown type byte " + buf[0]
		);
		...
		byte[] encryptedHashedDocname = Arrays.copyOfRange(
			buf,
			2,
			2 + E_H_DOCNAME_SIZE
		);
		byte[] pubkeyHash = Arrays.copyOfRange(
			buf,
			2 + E_H_DOCNAME_SIZE,
	...
	public boolean grabPubkey(
		GetPubkey pubkeyCache,
		boolean canReadClientCache,
		boolean forULPR,
		BlockMetadata meta
	) {
		if (pubKey != null) return false;
		pubKey = pubkeyCache.getKey(
			pubKeyHash,
			canReadClientCache,
			forULPR,
			meta
	...
	public static byte[] routingKeyFromFullKey(byte[] keyBuf) {
		if (keyBuf.length != FULL_KEY_LENGTH) {
			Logger.error(
				NodeSSK.class,
				"routingKeyFromFullKey() on buffer length " + keyBuf.length
			);
		}
		byte[] encryptedHashedDocname = Arrays.copyOfRange(
			keyBuf,
	...
	public int compareTo(Key arg0) {
		...
		int result = Fields.compareBytes(
			encryptedHashedDocname,
