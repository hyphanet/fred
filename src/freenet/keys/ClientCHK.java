...
public class ClientCHK extends ClientKey implements Serializable {
	...
	static {
		try {
			TEST_KEY = new ClientCHK(
				FreenetURI.generateRandomCHK(new Random())
			);
	...
	public ClientCHK(
		...
		Fields.hashCode(encKey) ^
		compressionAlgorithm;
	}

	public ClientCHK(byte[] routingKey, byte[] encKey, byte[] extra)
		throws MalformedURLException {
		this.routingKey = routingKey;
		this.cryptoKey = encKey;
		if (
			(extra == null) || (extra.length < 5)
		) throw new MalformedURLException(
			"No extra bytes in CHK - maybe a 0.5 key?"
		);
		...
		compressionAlgorithm = (short) (((extra[3] & 0xff) << 8) +
			(extra[4] & 0xff));
		hashCode = Fields.hashCode(routingKey) ^
	...
	public ClientCHK(FreenetURI uri) throws MalformedURLException {
		if (!uri.getKeyType().equals("CHK")) throw new MalformedURLException(
			"Not CHK"
		);
	...
	static HashSet<ByteArrayWrapper> standardExtras = new HashSet<
		ByteArrayWrapper
	>();
	...
	static {
		for (
			...
			for (
				...
				compressionAlgorithm <=
				(short) (COMPRESSOR_TYPE.countCompressors());
				compressionAlgorithm++
			) {
				standardExtras.add(
					new ByteArrayWrapper(
						getExtra(cryptoAlgorithm, compressionAlgorithm, true)
					)
				);
				standardExtras.add(
					new ByteArrayWrapper(
						getExtra(cryptoAlgorithm, compressionAlgorithm, false)
					)
				);
			}
		}
	 ...
	 */
	public static ClientCHK readRawBinaryKey(DataInputStream dis)
		throws IOException {
