...
public class ClientSSKBlock implements ClientKeyBlock {
	...
	public Bucket decode(
		...
		byte[] dataDecryptKey = Arrays.copyOf(
			decryptedHeaders,
		...
		compressionAlgorithm =
			(short) (((decryptedHeaders[DATA_DECRYPT_KEY_LENGTH + 2] & 0xff) <<
					8) +
		...
		if (dontDecompress) {
			if (
				compressionAlgorithm == (short) -1
			) return BucketTools.makeImmutableBucket(
				factory,
				dataOutput,
				dataLength
			);
			else if (dataLength < 2) throw new SSKDecodeException(
				"Data length is less than 2 yet compressed!"
			);
			else return BucketTools.makeImmutableBucket(
				factory,
				dataOutput,
				2,
				dataLength - 2
	...
	public boolean isMetadata() {
		if (!decoded) throw new IllegalStateException(
			"Cannot read isMetadata before decoded"
		);
		return isMetadata;
	}
	 ...
	 */
	public byte[] memoryDecode(boolean dontDecompress)
		throws KeyDecodeException {
		try {
