...
public class NodeCHK extends Key {
	...
	@Override
	public final void writeToDataOutputStream(DataOutputStream stream)
		throws IOException {
		write(stream);
	}
	...
	public boolean equals(Object key) {
		...
		if (key instanceof NodeCHK) {
			...
			return (
				java.util.Arrays.equals(chk.routingKey, routingKey) &&
	...
	public static byte[] routingKeyFromFullKey(byte[] keyBuf) {
		...
		if (keyBuf.length != FULL_KEY_LENGTH) {
			Logger.error(
				NodeCHK.class,
				"routingKeyFromFullKey() on " + keyBuf.length + " bytes"
			);
			return null;
		}
		if (
			...
			if (
				keyBuf[keyBuf.length - 1] == 0 && keyBuf[keyBuf.length - 2] == 0
			) {
				// We are certain it's a routing-key
				Logger.minor(
					NodeCHK.class,
					"Recovering routing-key stored wrong as full-key (two nulls at end)"
				);
			} else {
				// It might be a routing-key or it might be random data
				Logger.error(
					NodeCHK.class,
					"Maybe recovering routing-key stored wrong as full-key"
				);
			}
