...
public class USK
	...
	implements Comparable<USK>, Serializable {
	...
	public USK(
		...
		if (
			cryptoKey.length != ClientSSK.CRYPTO_KEY_LENGTH
		) throw new MalformedURLException(
	...
	public ClientSSK getSSK(String string) {
		try {
			...
			Logger.error(
				this,
				"Caught " + e + " should not be possible in USK.getSSK",
				e
			);
			throw new Error(e);
		}
	...
	public boolean equals(Object o, boolean includeVersion) {
		if (o instanceof USK) {
			...
			if (
				includeVersion && (suggestedEdition != u.suggestedEdition)
			) return false;
			return true;
		}
		return false;
	...
	public FreenetURI turnMySSKIntoUSK(FreenetURI uri) {
		if (
			...
			try {
				...
				Logger.normal(
					this,
					"Trying to turn SSK back into USK: " +
					uri +
					" doc=" +
					doc +
					" caught " +
