...
public class SSKBlock implements KeyBlock {
	...
	public SSKBlock(
		...
		if (
			headers.length != TOTAL_HEADERS_LENGTH
		...
		if (data.length != DATA_LENGTH) throw new SSKVerifyException(
			"Data length wrong: " + data.length + " should be " + DATA_LENGTH
		);
		this.pubKey = nodeKey.getPubKey();
		if (pubKey == null) throw new SSKVerifyException(
			"PubKey was null from " + nodeKey
		);
		// Now verify it
		hashIdentifier = (short) (((headers[0] & 0xff) << 8) +
			(headers[1] & 0xff));
		if (hashIdentifier != HASH_SHA256) throw new SSKVerifyException(
			"Hash not SHA-256"
		);
		int x = 2;
		...
		if (
			x + SIG_R_LENGTH + SIG_S_LENGTH > headers.length
		...
		if (!dontVerify || logMINOR) { // force verify on log minor
			...
			if (
				!(dsa.verifySignature(Global.truncateHash(overallHash), r, s) ||
					dsa.verifySignature(overallHash, r, s))
			) {
				if (dontVerify) Logger.error(
					this,
					"DSA verification failed with dontVerify!!!!"
				);
				throw new SSKVerifyException(
		...
		if (
			!Arrays.equals(ehDocname, nodeKey.encryptedHashedDocname)
		) throw new SSKVerifyException(
