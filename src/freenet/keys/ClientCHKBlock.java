...
public class ClientCHKBlock implements ClientKeyBlock {
	 ...
	 */
	public ClientCHKBlock(CHKBlock block, ClientCHK key2)
		throws CHKVerifyException {
	...
	public byte[] memoryDecode() throws CHKDecodeException {
		try {
			ArrayBucket a = (ArrayBucket) decode(
	...
	public Bucket decodeOld(
		...
		if (
			cryptoKey.length < Node.SYMMETRIC_KEY_LENGTH
		...
		if (!Arrays.equals(iv, predIV)) throw new CHKDecodeException(
			"Check failed: Decrypted IV == H(decryption key)"
		);
		// Checks complete
	...
	private static long benchmark(Mac hmac) throws GeneralSecurityException {
		...
		System.arraycopy(
			...
			key,
			0,
			Math.min(key.length, output.length)
		);
	...
	static {
		try {
			...
			SecretKeySpec dummyKey = new SecretKeySpec(
				new byte[Node.SYMMETRIC_KEY_LENGTH],
				algo
			);
	 ...
	 * @throws IOException If there is a bucket error.
	 */
	public Bucket decodeNew(
		BucketFactory bf,
		int maxLength,
		boolean dontCompress
	) throws CHKDecodeException, IOException {
		if (
			key.cryptoAlgorithm != Key.ALGO_AES_CTR_256_SHA256
		) throw new UnsupportedOperationException();
		byte[] headers = block.headers;
		byte[] data = block.data;
		...
		try {
			...
			int size =
				((plaintext[data.length] & 0xff) << 8) +
				(plaintext[data.length + 1] & 0xff);
			if ((size > 32768) || (size < 0)) {
				throw new CHKDecodeException("Invalid size: " + size);
			}
			// Check the hash.
			...
			if (!Arrays.equals(hash, hashCheck)) {
				throw new CHKDecodeException(
					"HMAC is wrong, wrong decryption key?"
				);
			}
			return Key.decompress(
				dontCompress ? false : key.isCompressed(),
			...
			throw new CHKDecodeException(
				"Problem with JCA, should be impossible!",
				e
			);
		}
	}
	...
	public static ClientCHKBlock encodeSplitfileBlock(
		...
		if (
			data.length != CHKBlock.DATA_LENGTH
		...
		if (cryptoAlgorithm == Key.ALGO_AES_PCFB_256_SHA256) return innerEncode(
			data,
			...
			"Unknown crypto algorithm: " + cryptoAlgorithm
		);
	...
	static ClientCHKBlock encode(
		...
		if (cryptoAlgorithm == 0) {
			...
			Logger.error(
				ClientCHKBlock.class,
				"Passed in 0 crypto algorithm",
	...
	public static ClientCHKBlock encodeNew(
		...
		if (
			...
			"Unsupported crypto algorithm " + cryptoAlgorithm
		);
		try {
			...
			if (
				blockHashAlgorithm != KeyBlock.HASH_SHA256
			) throw new IllegalArgumentException(
				"Unsupported block hash algorithm " + cryptoAlgorithm
			);
			...
			ClientCHK finalKey = new ClientCHK(
				finalHash,
				encKey,
				asMetadata,
				cryptoAlgorithm,
