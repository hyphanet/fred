...
public class ClientCHKBlock implements ClientKeyBlock {
    ...
    public Bucket decodeOld(BucketFactory bf, int maxLength, boolean dontCompress)
        ...
        return Key.decompress(dontCompress ? false : key.isCompressed(), dbuf, size, bf,
                              Math.min(maxLength, CHKBlock.MAX_LENGTH_BEFORE_COMPRESSION), key.compressionAlgorithm,
                              false);
    }
    ...
    static public ClientCHKBlock encode(Bucket sourceData, boolean asMetadata, boolean dontCompress,
            short alreadyCompressedCodec, long sourceLength, String compressorDescriptor, boolean pre1254,
        ...
        return encode(sourceData, asMetadata, dontCompress, alreadyCompressedCodec, sourceLength, compressorDescriptor,
                      pre1254, cryptoKey, cryptoAlgorithm, false);
    }

    // forceNoJCA for unit tests.
    static ClientCHKBlock encode(Bucket sourceData, boolean asMetadata, boolean dontCompress,
                                 short alreadyCompressedCodec, long sourceLength, String compressorDescriptor,
                                 boolean pre1254, byte[] cryptoKey, byte cryptoAlgorithm, boolean forceNoJCA)
            throws CHKEncodeException, IOException {
        byte[] finalData = null;
        byte[] data;
    ...
    static public ClientCHKBlock encode(byte[] sourceData, boolean asMetadata, boolean dontCompress,
            short alreadyCompressedCodec, int sourceLength, String compressorDescriptor, boolean pre1254)
        ...
        try {
            return encode(new ArrayBucket(sourceData), asMetadata, dontCompress, alreadyCompressedCodec, sourceLength,
                          compressorDescriptor, pre1254, null, Key.ALGO_AES_CTR_256_SHA256);
        } catch (IOException e) {
