...
public abstract class Key
	implements WritableToDataOutputStream, Comparable<Key> {
	...
	public static Key read(DataInput raf) throws IOException {
		...
		} else if (type == NodeSSK.BASE_TYPE) return NodeSSK.readSSK(
			raf,
			subtype
		);
	...
	public static KeyBlock createBlock(
		...
		if (type == NodeCHK.BASE_TYPE) {
			...
			throw new KeyVerifyException(
				"No such key type " + Integer.toHexString(type)
			);
		}
	}
	...
	static Bucket decompress(
		...
		if (input.length < inputLength) throw new IndexOutOfBoundsException(
		...
		if (isCompressed) {
			if (logMINOR) Logger.minor(
				...
				"Decompressing " +
				inputLength +
				" bytes in decode with codec " +
				compressionAlgorithm
			);
			final int inputOffset = (shortLength ? 2 : 4);
			if (inputLength < inputOffset + 1) throw new CHKDecodeException(
				"No bytes to decompress"
			);
			// Decompress
			// First get the length
			int len;
			if (shortLength) len = ((input[0] & 0xff) << 8) + (input[1] & 0xff);
			else len = ((((((input[0] & 0xff) << 8) + (input[1] & 0xff)) << 8) +
					(input[2] & 0xff)) <<
				8) +
			(input[3] & 0xff);
			if (len > maxLength) throw new TooBigException(
				"Invalid precompressed size: " + len + " maxlength=" + maxLength
			);
			COMPRESSOR_TYPE decompressor =
				COMPRESSOR_TYPE.getCompressorByMetadataID(compressionAlgorithm);
			if (decompressor == null) throw new CHKDecodeException(
				"Unknown compression algorithm: " + compressionAlgorithm
			);
			InputStream inputStream = null;
			OutputStream outputStream = null;
			Bucket inputBucket = new SimpleReadOnlyArrayBucket(
				input,
				inputOffset,
	...
	public static Compressed compress(
		...
		if (
			sourceData.size() > MAX_LENGTH_BEFORE_COMPRESSION
		...
		if ((!dontCompress) || (alreadyCompressedCodec >= 0)) {
			...
			if (alreadyCompressedCodec >= 0) {
				...
				if (
					sourceLength > MAX_LENGTH_BEFORE_COMPRESSION
				) throw new TooBigException("Too big");
			} else {
				if (sourceData.size() > maxCompressedDataLength) {
					// Determine the best algorithm
					COMPRESSOR_TYPE[] comps =
						COMPRESSOR_TYPE.getCompressorsArray(
	...
	public static ClientKeyBlock createKeyBlock(ClientKey key, KeyBlock block)
		throws KeyVerifyException {
		if (key instanceof ClientSSK) return ClientSSKBlock.construct(
			(SSKBlock) block,
			(ClientSSK) key
		);
		else return new ClientCHKBlock((CHKBlock) block, (ClientCHK) key); //if(key instanceof ClientCHK
	}
