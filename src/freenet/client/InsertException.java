...
public class InsertException extends Exception implements Cloneable {
	...
	public InsertException(
		...
		if (mode == InsertExceptionMode.INTERNAL_ERROR) Logger.error(
			this,
			"Internal error: " + this
		);
		else if (logMINOR) Logger.minor(
			this,
			"Creating InsertException: " + getMessage(mode) + ": " + msg,
			this
		);
	}
	...
	public InsertException(InsertExceptionMode m, FreenetURI expectedURI) {
		...
		if (mode == InsertExceptionMode.INTERNAL_ERROR) Logger.error(
			...
			this,
			"Creating InsertException: " + getMessage(mode),
			this
		);
	}
	...
	public InsertException(
		...
		if (mode == InsertExceptionMode.INTERNAL_ERROR) Logger.error(
			...
			this,
			"Creating InsertException: " + getMessage(mode) + ": " + e,
			this
		);
	}
	...
	public static String getMessage(InsertExceptionMode mode) {
		...
		String ret = NodeL10n.getBase()
			.getString("InsertException.longError." + mode.code);
	...
	public static String getShortMessage(InsertExceptionMode mode) {
		// FIXME change the l10n to use the keyword not the code
		String ret = NodeL10n.getBase()
			.getString("InsertException.shortError." + mode.code);
		if (ret == null) return "Unknown error " + mode;
		else return ret;
	}
	...
	public static boolean isFatal(InsertExceptionMode mode) {
		switch (mode) {
			...
			default:
				Logger.error(
					InsertException.class,
					"Error unknown to isFatal(): " + getMessage(mode)
				);
				return false;
		}
	}
