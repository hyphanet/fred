...
public class FECJob {
    ...
    public FECJob(FECCodec codec, FECQueue queue, Bucket[] dataBlocks, Bucket[] checkBlocks, int blockLength,
                  BucketFactory bucketFactory, FECCallback callback, boolean isADecodingJob, short priority,
                  boolean persistent) {
    ...
    public FECJob(FECCodec codec, FECQueue queue, SplitfileBlock[] dataBlockStatus, SplitfileBlock[] checkBlockStatus,
                  int blockLength, BucketFactory bucketFactory, FECCallback callback, boolean isADecodingJob,
...
//      this.checkBlockStatus = checkBlockStatus;
    ...
    public FECCodec getCodec() {
        if (codec == null) {
            ...
            if (codec == null) {
                Logger.error(this,
                             "No codec found for algo " + fecAlgo + " data blocks length " + dataBlocks.length
                             + " check blocks length " + checkBlocks.length);
            }
        }
    ...
    public boolean activateForExecution(ObjectContainer container) {
        ...
        if (!isADecodingJob) {
            ...
            if (hasDataBlocks && (countNullDataBlocks > 0)) {
                Logger.error(this,
                             "Invalid job: Encoding: " + countDataBlocks + " blocks but " + countNullDataBlocks
                             + " are null!");
    ...
    public void storeBlockStatuses(ObjectContainer container, boolean failure) {
        ...
        if (failure && ((countNullsData > 0) || (countNullsCheck > 0))) {
            Logger.normal(this,
                          "After failed, storing block statuses, " + countNullsData + "/" + dataBlockStatus.length
                          + " nulls in data blocks " + countNullsCheck + "/" + checkBlockStatus.length
                          + " nulls in check blocks");
        } else if (failure) {
            if (logMINOR) {
                Logger.minor(this,
                             "After failed, storing block statuses, " + countNullsData + "/" + dataBlockStatus.length
            ...
            if (isADecodingJob) {
                if (countNullsData != 0) {
                    Logger.normal(this,
                                  "After successful decode, storing block statuses, " + countNullsData + "/"
                                  + dataLength + " nulls in data blocks " + countNullsCheck + "/" + checkLength
                                  + " nulls in check blocks");
                } else {
                    if (logMINOR) {
                        Logger.minor(this,
    ...
    public void dump(ObjectContainer container) {
        ...
        if (dataBlockStatus != null) {
            ...
            if (dataCount == dataBlockStatus.length) {
                ...
                System.out.println("Does not have all data block statuses: " + dataCount + " of "
