...
class Fetcher {
	...
	throws FetchException, MetadataParseException, ArchiveFailureException, ArchiveRestartException {
		...
		return runMetadata(dm, recursionLevel, key, metaStrings, metadata, null, key.getURI());
	}
	 ...
	 * @throws ArchiveRestartException 
	 */
	private FetchResult runMetadata(ClientMetadata dm, int recursionLevel, ClientKey key, LinkedList metaStrings, Metadata metadata, ArchiveHandler container, FreenetURI thisKey) throws MetadataParseException, FetchException, ArchiveFailureException, ArchiveRestartException {
		...
		if(metadata.isSimpleManifest()) {
			...
			Bucket metadataBucket = container.getMetadata(archiveContext, ctx, dm, recursionLevel);
			...
			if(container == null)
				...
				Bucket result = container.get(metadata.getZIPInternalName(), archiveContext, ctx, dm, recursionLevel);
			...
			FetchResult res = runMetadata(dm, recursionLevel, key, metaStrings, metadata, container, thisKey);
