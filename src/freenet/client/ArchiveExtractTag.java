...
class ArchiveExtractTag {
    ...
    ArchiveExtractTag(ArchiveHandlerImpl handler, Bucket data, boolean freeBucket, ArchiveContext actx, String element,
                      ArchiveExtractCallback callback, long nodeDBHandle) {
        if (handler == null) {
    ...
    public boolean checkBroken(ObjectContainer container, ClientContext context) {
        ...
        if ((data == null) || (actx == null) || (callback == null) || (handler == null)) {
            String error = "Archive extract tag is broken: data=" + data + " actx=" + actx + " callback=" + callback
                           + " handler=" + handler;
