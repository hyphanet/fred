...
public class FlacPacketFilter implements CodecPacketFilter {
	...
	public CodecPacket parse(CodecPacket packet) throws IOException {
		...
		DataInputStream input = new DataInputStream(
			new ByteArrayInputStream(packet.toArray())
		);
		switch (currentState) {
			case UNINITIALIZED:
				if (
					!(packet instanceof FlacMetadataBlock) &&
					((FlacMetadataBlock) packet).getMetadataBlockType() !=
				...
				minimumFrameSize = (input.readUnsignedShort() << 8) |
				input.readUnsignedByte();
				maximumFrameSize = (input.readUnsignedShort() << 8) |
				input.readUnsignedByte();
			...
			case STREAMINFO_FOUND:
				...
				switch (((FlacMetadataBlock) packet).getMetadataBlockType()) {
					case APPLICATION:
						...
						((FlacMetadataBlock) packet).setMetadataBlockType(
								BlockType.PADDING
							);
						break;
					case VORBIS_COMMENT:
		...
		if (packet instanceof FlacMetadataBlock && logMINOR) Logger.minor(
			this,
			"Returning packet of type" +
			((FlacMetadataBlock) packet).getMetadataBlockType()
		);
		return packet;
	}
