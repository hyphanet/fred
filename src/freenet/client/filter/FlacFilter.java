...
public class FlacFilter implements ContentDataFilter {
	...
	public void readFilter(
		...
		for (byte magicCharacter : magicNumber) {
			if (magicCharacter != in.readByte()) throw new DataFilterException(
		...
		while (currentState != State.STREAM_FINISHED) {
			...
			try {
				if (currentState == State.METADATA_FOUND) frameHeader =
				...
				switch (currentState) {
					case UNINITIALIZED:
						if (logMINOR) Logger.minor(
							this,
							"Reading metadata packet"
						);
						int header = in.readInt();
						payload = new byte[header & 0x00FFFFFF];
						if (logMINOR) Logger.minor(
							this,
							"About to read " + payload.length + " bytes"
						);
					...
					case METADATA_FOUND:
						if (logMINOR) Logger.minor(
							this,
							"Reading audio packet"
						);
						boolean firstHalfOfSyncHeaderFound = false;
						ArrayList<Byte> buffer = new ArrayList<Byte>();
						int data = 0;
						buffer.add(
							Byte.valueOf((byte) ((frameHeader & 0xFF00) >>> 8))
						);
