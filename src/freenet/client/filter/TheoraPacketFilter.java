...
public class TheoraPacketFilter implements CodecPacketFilter {
	...
	static final byte[] magicNumber = new byte[] {
		't',
		'h',
		'e',
		'o',
	...
	public CodecPacket parse(CodecPacket packet) throws IOException {
		...
		BitInputStream input = new BitInputStream(
			new ByteArrayInputStream(packet.payload)
		);
		try {
	...
	private void verifyIdentificationHeader(BitInputStream input)
		throws IOException {
	...
	private void verifySetupHeader(BitInputStream input) throws IOException {
		...
		int NBMS =
			checkHeaderField("Setup", "NBMS", input, 9, not(v -> v > 383)) + 1;
				...
				if (NEWQR == 0) {
					...
					if (NEWQR != 1) {
						throw new UnknownContentTypeException(
							"SetupHeader NEWQR: " + NEWQR + "(MUST be 0|1)"
						);
					}
					...
					while (true) {
						QRSIZES[qti][pli][qri] = input.readInt(ilog(62 - qi)) +
						1;
	...
	private void verifyTypeAndHeader(
		String headerName,
		BitInputStream input,
		int expectedHeaderType
	) throws IOException {
		try {
			checkHeaderField(
				headerName,
				...
				input,
				8,
				v -> v == expectedHeaderType
			);
		} catch (UnknownContentTypeException e) {
			throw new UnknownContentTypeException(
				e.getType() + "; expected: " + expectedHeaderType
			);
		}
	...
	private int checkHeaderField(
		...
		if (!validator.test(value)) {
			throw new UnknownContentTypeException(
				headerName + "Header " + fieldName + ": " + value
			);
		}
		return value;
	}
