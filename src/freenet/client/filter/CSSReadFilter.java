...
public class CSSReadFilter implements ContentDataFilter, CharsetExtractor {
	...
	public void readFilter(
		...
		if (logDEBUG) Logger.debug(
			this,
			"running " + this + "with charset" + charset
		);
		Reader r = null;
		Writer w = null;
		try {
	...
	public String getCharset(byte[] input, int length, String charset)
		...
		if (logDEBUG) Logger.debug(
			this,
			"Fetching charset for CSS with initial charset " + charset
		);
	...
	static final byte[] utf16be = parse(
		"00 40 00 63 00 68 00 61 00 72 00 73 00 65 00 74 00 20 00 22"
	);
	static final byte[] utf16le = parse(
		"40 00 63 00 68 00 61 00 72 00 73 00 65 00 74 00 20 00 22 00"
	);
	...
	static final int maxBOMLength = Math.max(
		utf16be.length,
		Math.max(
			utf16le.length,
			Math.max(
				...
				Math.max(
					...
					Math.max(
						...
						Math.max(
							...
							Math.max(
								utf32_2143.length,
								Math.max(utf32_3412.length, gsm.length)
							)
						)
					)
				)
	...
	public BOMDetection getCharsetByBOM(byte[] input, int length)
		throws DataFilterException, IOException {
		if (
			ContentFilter.startsWith(input, ascii, length)
		) return new BOMDetection("UTF-8", true);
		if (
			ContentFilter.startsWith(input, utf16be, length)
		) return new BOMDetection("UTF-16BE", true);
		if (
			ContentFilter.startsWith(input, utf16le, length)
		) return new BOMDetection("UTF-16LE", true);
		if (
			ContentFilter.startsWith(input, utf32_be, length)
		) return new BOMDetection("UTF-32BE", true);
		if (
			ContentFilter.startsWith(input, utf32_le, length)
		) return new BOMDetection("UTF-32LE", true);
		if (
			ContentFilter.startsWith(input, ebcdic, length)
		) return new BOMDetection("IBM01140", true);
		if (
			ContentFilter.startsWith(input, ibm1026, length)
		) return new BOMDetection("IBM1026", true);

		// Unsupported BOMs

		if (
			ContentFilter.startsWith(input, utf32_2143, length)
		) throw new UnsupportedCharsetInFilterException("UTF-32-2143");
		if (
			ContentFilter.startsWith(input, utf32_3412, length)
		) throw new UnsupportedCharsetInFilterException("UTF-32-3412");
		if (
			ContentFilter.startsWith(input, gsm, length)
