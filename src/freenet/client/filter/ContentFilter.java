...
public class ContentFilter {
	...
	public static final String[] HTML_MIME_TYPES = new String[] {
		"text/html",
	...
	public static FilterStatus filter(
		...
		if (logMINOR) Logger.minor(
			ContentFilter.class,
			"Filtering data of type" + typeName
		);
		...
		if (handler == null) throw new UnknownContentTypeException(typeName);
			...
			if (handler.readFilter != null) {
				if (
					handler.takesACharset &&
					((charset == null) || (charset.length() == 0))
				) {
					int bufferSize =
						handler.charsetExtractor.getCharsetBufferSize();
	...
	public static String detectCharset(
		...
		if ((charset == null) && (handler.charsetExtractor != null)) {
			BOMDetection bom = handler.charsetExtractor.getCharsetByBOM(
				input,
				length
			);
			if (bom != null) {
				...
				if (charset != null) {
					...
					try {
						if (
							(charset = handler.charsetExtractor.getCharset(
									input,
									length,
									charset
								)) !=
		...
		if (
			handler.useMaybeCharset &&
			maybeCharset != null &&
			(maybeCharset.length() != 0)
		) return maybeCharset;
	 ...
	 * @throws IOException
	 */
	private static String detectBOM(byte[] input, int length)
		throws IOException {
	...
	static byte[] bom_utf32_be = new byte[] {
		(byte) 0,
		(byte) 0,
		(byte) 0xFE,
		(byte) 0xFF,
	};
	static byte[] bom_utf32_le = new byte[] {
		(byte) 0xFF,
		(byte) 0xFE,
		(byte) 0,
		(byte) 0,
	};
	static byte[] bom_scsu = new byte[] {
		(byte) 0x0E,
		(byte) 0xFE,
		(byte) 0xFF,
	};
	static byte[] bom_utf7_1 = new byte[] {
		(byte) 0x2B,
		(byte) 0x2F,
		(byte) 0x76,
		(byte) 0x38,
	};
	static byte[] bom_utf7_2 = new byte[] {
		(byte) 0x2B,
		(byte) 0x2F,
		(byte) 0x76,
		(byte) 0x39,
	};
	static byte[] bom_utf7_3 = new byte[] {
		(byte) 0x2B,
		(byte) 0x2F,
		(byte) 0x76,
		(byte) 0x2B,
	};
	static byte[] bom_utf7_4 = new byte[] {
		(byte) 0x2B,
		(byte) 0x2F,
		(byte) 0x76,
		(byte) 0x2F,
	};
	static byte[] bom_utf7_5 = new byte[] {
		(byte) 0x2B,
		(byte) 0x2F,
		(byte) 0x76,
		(byte) 0x38,
	...
	static byte[] bom_utf_ebcdic = new byte[] {
		(byte) 0xDD,
		(byte) 0x73,
		(byte) 0x66,
		(byte) 0x73,
	};
	static byte[] bom_bocu_1 = new byte[] {
		(byte) 0xFB,
		(byte) 0xEE,
		(byte) 0x28,
	};
	...
	static byte[] bom_utf32_2143 = new byte[] {
		(byte) 0x00,
		(byte) 0x00,
		(byte) 0xff,
		(byte) 0xfe,
	};
	static byte[] bom_utf32_3412 = new byte[] {
		(byte) 0xfe,
		(byte) 0xff,
		(byte) 0x00,
		(byte) 0x00,
	...
	public static UnsafeContentTypeException checkMIMEType(
		String expectedMIME
	) {
		FilterMIMEType handler = getMIMEType(expectedMIME);
		if (
			handler == null ||
			(handler.readFilter == null && !handler.safeToRead)
		) {
			if (handler == null) {
				if (logMINOR) Logger.minor(
					ContentFilter.class,
					"Unable to get filter handler for MIME type " + expectedMIME
				);
				return new UnknownContentTypeException(expectedMIME);
			} else {
				if (logMINOR) Logger.minor(
					ContentFilter.class,
					"Unable to filter unsafe MIME type " + expectedMIME
				);
