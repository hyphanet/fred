	...
	public CodecPacket parse(CodecPacket packet) throws IOException {
		...
		switch (currentState) {
			case UNINITIALIZED:
				...
				if (
					(blocksize & (0xf0 >>> 4)) > (blocksize & 0x0f)
				) return null;
			...
			case IDENTIFICATION_FOUND:
				...
				if (logMINOR) Logger.minor(
					this,
					"Read a vendor length of " + vendor_length
				);
				byte[] vendor_string = new byte[(int) vendor_length];
				input.readFully(vendor_string);
				long user_comment_list_length = Integer.reverseBytes(
					input.readInt()
				);
				for (long i = 0; i < user_comment_list_length; i++) {
					for (
						long j = Integer.reverseBytes(input.readInt());
						j > 0;
						j--
					) {
						input.skipBytes(1);
