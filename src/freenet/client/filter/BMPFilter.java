...
public class BMPFilter implements ContentDataFilter {
	...
	public int readInt(DataInputStream dis) throws IOException {
		...
		if (result < 0) throw new EOFException(); // end of file reached

		result = (unsignedByte(data[2]) << 16) |
		(unsignedByte(data[1]) << 8) |
		unsignedByte(data[0]);
	...
	public int readShort(DataInputStream dis) throws IOException {
		...
		int r2 = dis.read();
		if (r2 < 0) throw new EOFException(); // end of file reached
	...
	public void readFilter(
		...
		if (
			(!Arrays.equals(StartWord, bmpHeaderwindows)) &&
			...
			(!Arrays.equals(StartWord, bmpHeaderos2Pointer))
		...
		if (size_bitmapinfoheader != 40) {
			throwHeaderError(
				l10n("InvalidBitMapInfoHeaderSizeT"),
				l10n("InvalidBitMapInfoHeaderSizeD")
			);
		}
		...
		if (imageWidth < 0 || imageHeight < 0) {
			throwHeaderError(
				l10n("InvalidDimensionT"),
				l10n("InvalidDimensionD")
			);
		}
		...
		if (no_plane != 1) { // No of planes should be 1
			throwHeaderError(
				l10n("InvalidNoOfPlanesT"),
				l10n("InvalidNoOfPlanesD")
			);
		}
		...
		if (
			...
			throwHeaderError(
				l10n("InvalidBitDepthT"),
				l10n("InvalidBitDepthD")
			);
		}
		...
		if (!(compression_type >= 0 && compression_type <= 3)) {
			throwHeaderError(
				...
				l10n(
					"Compression type field is set to " +
					compression_type +
					" instead of 0-3"
				)
			);
		...
		if (fileSize != headerSize + imagedatasize) {
			throwHeaderError(
				l10n("InvalidFileSizeT"),
				l10n("InvalidFileSizeD")
			);
		}
		...
		if (compression_type == 0) {
			...
			if (calculatedsize != imagedatasize) {
				throwHeaderError(
					l10n("InvalidImageDataSizeT"),
					l10n("InvalidImageDataSizeD")
				);
			}
		}
		dis.reset();
	...
	private void throwHeaderError(String shortReason, String reason)
