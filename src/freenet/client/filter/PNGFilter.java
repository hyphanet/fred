            ...
            while (!hasSeenIEND) {
                ...
                if (length > 0) {
                    ...
                    if (logMINOR) {
                        if (logDEBUG) {
                            Logger.minor(this,
                                         "data (offset=0x" + Long.toHexString(offset) + ") "
                ...
                if (checkCRCs) {
                    long readCRC = (((crcLengthBytes[0] & 0xff) << 24) + ((crcLengthBytes[1] & 0xff) << 16)
                                    ...
                                    + (crcLengthBytes[3] & 0xff)) & 0x00000000ffffffffL;
                    CRC32 crc = new CRC32();
                ...
                if (!skip && "IHDR".equals(chunkTypeString)) {    // http://www.w3.org/TR/PNG/#11IHDR
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Info from IHDR: width=" + width + "px height=" + height + "px bitDepth="
                                     + bitDepth + " colourType=" + colourType + " compressionMethod="
                                     + compressionMethod + " filterMethod=" + filterMethod + " interlaceMethod="
                                     + interlaceMethod);
                    }
