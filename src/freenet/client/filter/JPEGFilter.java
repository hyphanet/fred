...
public class JPEGFilter implements ContentDataFilter {
	...
	public void readFilter(
		...
		while (!finished) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Marker type: " + Integer.toHexString(markerType)
			);
			long countAtStart = cis.count(); // After marker but before type
			int blockLength;
			if (
				markerType == MARKER_EOI ||
				(markerType >= MARKER_RST0 && markerType <= MARKER_RST7)
			) blockLength = 0;
			else {
			...
			if (markerType == 0xDA) {
				...
				// Copy marker
				if (blockLength < 2) throwError(
					"Invalid frame length",
					"The file includes an invalid frame (length " +
					blockLength +
					")."
				);
				...
				Logger.minor(
					this,
					"Copied start-of-frame marker length " + (blockLength - 2)
				);
				...
				while (true) {
					...
					if (
						prevChar == 0xFF &&
						x != 0 &&
						!(x >= MARKER_RST0 && x <= MARKER_RST7)
					) { // reset markers can occur in the scan
						forceMarkerType = x;
						if (logMINOR) Logger.minor(
							this,
							"Moved scan at " +
				...
				if (type.equals("JFIF")) {
					...
					if (majorVersion != 1) throwError(
						"Invalid header",
						"Unrecognized major version " + majorVersion + "."
					);
					dos.write(majorVersion);
					int minorVersion = dis.readUnsignedByte();
					if (minorVersion > 2) throwError(
						"Invalid header",
						"Unrecognized version 1." + minorVersion + "."
					);
					dos.write(minorVersion);
					int units = dis.readUnsignedByte();
					if (units > 2) throwError(
						"Invalid header",
						"Unrecognized units type " + units + "."
					);
					dos.write(units);
			...
			if (cis.count() != countAtStart + blockLength) throwError(
				"Invalid frame",
	...
	private void writeNullTerminatedString(
	...
	private String readNullTerminatedAsciiString(DataInputStream dis)
		...
		while (true) {
			int x = dis.read();
			if (x == -1) throwError(
				"Invalid extension frame",
				"Could not read an extension frame name."
			);
			if (x == 0) break;
			char c = (char) x; // ASCII
			if (x > 128 || (c < 32 && c != 10 && c != 13)) throwError(
				"Invalid extension frame name",
	...
	private void skipRest(
		...
		if (skip < 0) throwError(
			"Invalid " + thing,
			"The file includes an invalid " + thing + '.'
		);
		if (skip == 0) return;
	...
	private void assertHeader(DataInputStream dis, byte[] expected)
		throws IOException {
		byte[] read = new byte[expected.length];
		dis.readFully(read);
		if (!Arrays.equals(read, expected)) throwError(
			"Invalid header",
			"The file does not start with a valid JPEG (JFIF) header."
		);
	}
