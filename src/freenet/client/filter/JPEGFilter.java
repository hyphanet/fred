...
public class JPEGFilter implements ContentDataFilter {
        ...
        while (!finished) {
            ...
            if (markerType == 0xDA) {
                ...
                while (true) {
                    ...
                    if ((prevChar == 0xFF) && (x != 0) &&!((x >= MARKER_RST0) && (x <= MARKER_RST7))) {    // reset markers can occur in the scan
                        forceMarkerType = x;

                        if (logMINOR) {
                            Logger.minor(this,
                                         "Moved scan at " + cis.count() + ", found a marker type "
                ...
                if (valid) {
                    ...
                    dos.write(buf);
                    Logger.minor(this,
                                 "Essential frame type " + Integer.toHexString(markerType) + " length "
                    ...
                    if ((markerType >= 0xE0) && (markerType <= 0xEF)) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Dropping application marker type " + Integer.toHexString(markerType)
                                         + " length " + blockLength);
                        }
                    } else {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Dropping unknown frame type " + Integer.toHexString(markerType)
                                         + " blockLength");
                        }
                    }
    ...
    private void skipRest(int blockLength, long countAtStart, CountedInputStream cis, DataInputStream dis,
                          DataOutputStream dos, String thing)
            throws IOException {
