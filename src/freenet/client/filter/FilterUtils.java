...
public class FilterUtils {
	...
	private static final int MAX_NTH = 999999; // Limit range of numbers allowed in isNth, due to incorrect behavior found in webkit based browsers.
	...
	public static boolean isNumber(String strNumber) {
		try {
			...
			if (strNumber.indexOf('e') > 0) {
				...
				strInteger = strNumber.substring(
					strDecimal.indexOf('e') + 1,
					strNumber.length()
				);
	...
	public static boolean isValidCSSShape(String value) {
		if (
			value.indexOf("rect(") == 0 &&
			value.indexOf(')') == value.length() - 1
		) {
			String[] shapeParts = value
			...
			if (shapeParts.length == 4) {
				for (String s : shapeParts) {
					s = s.trim();
					if (
						!(s.equalsIgnoreCase("auto") || isLength(s, false))
					) return false;
				}
				return true;
			}
	...
	public static boolean isColor(String value) {
		...
		if (
			CSScolorKeywords.contains(value) ||
		...
		if (
			...
			for (int i = 0; i < colorParts.length && isValidColorParts; i++) {
				if (
					!(isPercentage(colorParts[i].trim()) ||
						isInteger(colorParts[i].trim()))
		...
		if (
			...
			for (
				int i = 0;
				i < colorParts.length - 1 && isValidColorParts;
				i++
			) {
				if (
					!(isPercentage(colorParts[i].trim()) ||
	...
	public static boolean isCSSTransform(String value) {
		...
		if (logDEBUG) Logger.debug(
			FilterUtils.class,
			"isCSSTransform(\"" + value + "\")"
		);

		if (
			value.indexOf("matrix(") == 0 &&
			value.indexOf(')') == value.length() - 1
		) {
		...
		if (
			...
			if (isPercentage(part.trim()) || isLength(part.trim(), false)) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a translateX()"
				);
				return true;
			}
		}
		...
		if (
			...
			if (isPercentage(part.trim()) || isLength(part.trim(), false)) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a translateY()"
				);
				return true;
			}
		}
		...
		if (
			value.indexOf("translate(") == 0 &&
			value.indexOf(')') == value.length() - 1
		) {
			String[] parts = value.substring(10, value.length() - 1).split(",");
			if (
				parts.length == 1 &&
				(isPercentage(parts[0].trim()) ||
					isLength(parts[0].trim(), false))
			) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a translate()"
				);
				return true;
			} else if (
				parts.length == 2 &&
				(isPercentage(parts[0].trim()) ||
					isLength(parts[0].trim(), false)) &&
				(isPercentage(parts[1].trim()) ||
					isLength(parts[1].trim(), false))
			) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a translate()"
		...
		if (
			...
			if (parts.length == 1 && isNumber(parts[0].trim())) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a scale()"
				);
				return true;
			} else if (
				parts.length == 2 &&
				isNumber(parts[0].trim()) &&
				isNumber(parts[1].trim())
			) {
				if (logDEBUG) Logger.debug(
					FilterUtils.class,
					"isCSSTransform found a scale()"
	...
	public static boolean isFrequency(String value) {
		...
		if (value.indexOf("khz") != -1) {
			...
			if (
				!("khz".equals(value.substring(index, value.length()).trim()))
			) {
	...
	public static boolean isTime(String value) {
		...
		String intValue;
		if (value.indexOf("ms") > -1 && value.length() > 2) intValue =
			value.substring(0, value.length() - 2);
		else if (value.indexOf('s') > -1 && value.length() > 1) intValue =
	...
	public static String[] removeWhiteSpace(
		...
		for (String value : values) {
			value = value.trim();
			if (stripQuotes) value = CSSTokenizerFilter.removeOuterQuotes(
				value
			).trim();
			if (value != null && !(value.trim().isEmpty())) arrayToReturn.add(
				value
			);
		}
	...
	public static String[] splitOnCharArray(String value, String splitOn) {
		...
		for (i = 0; i < value.length(); i++) {
			if (splitOn.indexOf(value.charAt(i)) != -1) {
				...
				while (
					i < value.length() && splitOn.indexOf(value.charAt(i)) != -1
				) {
					i++;
				}
				prev = i;
	...
	public static boolean isNth(String value) {
		if (
			value.equals("odd") ||
			value.equals("even") ||
			isIntegerInRange(value, -MAX_NTH, MAX_NTH)
		) {
			return true;
		} else {
				...
				if (
					...
					if (
						...
						((value.charAt(bIndex) == '+' ||
								value.charAt(bIndex) == '-') &&
							isIntegerInRange(
								value.substring(bIndex, value.length()),
								-MAX_NTH,
								MAX_NTH
							))
					) {
