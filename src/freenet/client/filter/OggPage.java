...
public class OggPage {
	...
	public OggPage(OggPage oldPage, Collection<CodecPacket> packets)
		...
		Logger.minor(
			this,
			"Header type: " + Integer.toBinaryString(this.headerType)
		);
		...
		for (CodecPacket packet : packets) {
			...
			if (concludingPartialSegment != 0) {
				segmentSizes.add(
					Byte.valueOf(intToUnsignedByte(concludingPartialSegment))
				);
			}
			Logger.minor(
				this,
				"Writing packet sized: " + packet.payload.length
			);
	...
	public boolean isPacketContinued() {
		if (logMINOR) Logger.minor(
			this,
			"Packet continued: " + (headerType & 0x1)
		);
	...
	public byte[] toArray() {
		ByteBuffer bb = ByteBuffer.allocate(
			27 + byteToUnsigned(segments) + payload.length
		);
		bb.put(magicNumber);
		bb.put(version);
		bb.put(headerType);
	...
	public byte[] calculateCRC() {
		...
		for (int i = 0; i < array.length; i++) {
			...
			crc_reg = (crc_reg << 8) ^
			crc_lookup[((crc_reg >>> 24) & 0xff) ^ (array[i] & 0xff)];
		}
		return new byte[] {
			(byte) crc_reg,
			(byte) (crc_reg >>> 8),
	...
	public void recalculateSegmentLacing(LinkedList<Integer> packetSizes) {
		...
		for (int packet : packetSizes) {
			if (logMINOR) Logger.minor(
				this,
				"Setting segments for packet sized " + packet
			);
			for (
				int packetSegment = 0;
				packetSegment < packet / 255;
				packetSegment++
			) {
				if (logMINOR) Logger.minor(
					this,
					"Setting segment " + segment + " to full."
				);
			...
			if (remainder != 0) {
				if (logMINOR) Logger.minor(
					this,
					"Partially filling segment " + segment
				);
	...
	public Collection<CodecPacket> asPackets() {
		Logger.minor(
			this,
			"Creating packets for " + byteToUnsigned(segments) + " segments"
		);
		...
		for (int i = 0; i < segmentTable.length; i++) {
			if (
				(byteToUnsigned(segmentTable[i]) % 255 != 0) ||
