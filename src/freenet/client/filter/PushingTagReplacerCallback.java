...
public class PushingTagReplacerCallback implements TagReplacerCallback {
	 ...
	 *            - The current ToadletContext
	 */
	public PushingTagReplacerCallback(
	...
	public static String getClientSideLocalizationScript() {
		...
		for (String key : NodeL10n.getBase()
			.getAllNamesWithPrefix("fproxy.push")) {
			l10nBuilder.append(
		...
		String l10n = isNamePresentAtLeastOnce
	...
	public String processTag(ParsedTag pt, URIProcessor uriProcessor) {
		...
		if (
			...
			if (pt.element.toLowerCase().compareTo("img") == 0) {
				...
				for (String attr : pt.unparsedAttrs) {
					...
					if (name.compareTo("src") == 0) {
						...
						try {
							...
							src = uriProcessor.makeURIAbsolute(
								uriProcessor.processURI(
									value,
									null,
									false,
									false
				...
				pt.element.toLowerCase().compareTo("body") == 0 &&
				pt.startSlash == true
			) {
				// After the <body>, we need to insert the requestId and the l10n script
				return "".concat(
						/*new XmlAlertElement(ctx).generate()*/"".concat(
					...
					.concat(
						"<script type=\"text/javascript\" language=\"javascript\">".concat(
								getClientSideLocalizationScript()
							).concat("</script>")
					)
					.concat("</body>");
