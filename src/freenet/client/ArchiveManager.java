...
public class ArchiveManager {
    ...
    public void extractToCache(FreenetURI key, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE ctype, final Bucket data,
                               ArchiveContext archiveContext, ArchiveStoreContext ctx, String element,
                               ArchiveExtractCallback callback, ObjectContainer container, ClientContext context)
            ...
            if ((ctype == null) || (ARCHIVE_TYPE.ZIP == archiveType)) {
                ...
                context.mainExecutor.execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            Compressor.COMPRESSOR_TYPE.LZMA_NEW.decompress(is = data.getInputStream(), pos,
    ...
    private void handleTARArchive(ArchiveStoreContext ctx, FreenetURI key, InputStream data, String element,
                                  ArchiveExtractCallback callback, MutableBoolean gotElement, boolean throwAtExit,
            ...
            while (true) {
                ...
                if ((size > maxArchivedFileSize) &&!name.equals(element)) {
                    addErrorElement(ctx, key, name,
                                    "File too big: " + size + " greater than current archived file size limit "
...
outerZIP:
    ...
    private ArchiveStoreItem generateMetadata(ArchiveStoreContext ctx, FreenetURI key, Set<String> names,
            MutableBoolean gotElement, String element2, ArchiveExtractCallback callback, ObjectContainer container,
        ...
        while (true) {
            try {
                ...
                return addStoreElement(ctx, key, ".metadata", bucket, gotElement, element2, callback, container,
                                       context);
    ...
    private int resolve(MetadataUnresolvedException e, int x, Bucket bucket, ArchiveStoreContext ctx, FreenetURI key,
    ...
    private ArchiveStoreItem addStoreElement(ArchiveStoreContext ctx, FreenetURI key, String name, Bucket temp,
            MutableBoolean gotElement, String callbackName, ArchiveExtractCallback callback, ObjectContainer container,
            ClientContext context)
            throws ArchiveFailureException {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Adding store element: " + element + " ( " + key + ' ' + name + " size " + element.spaceUsed()
                         + " )");
        }
        ...
        synchronized (this) {
            while (true) {
                ...
                if (storedData.isEmpty()) {
                    ...
                    Logger.error(this,
                                 "storedData is empty but still over limit: cachedData=" + cachedData + " / "
                                 + maxCachedData);
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Dropping " + item + " : cachedData=" + cachedData + " of " + maxCachedData
