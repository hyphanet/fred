...
public class HighLevelSimpleClientImpl
	implements HighLevelSimpleClient, RequestClient, Cloneable {
	...
	static final int SPLITFILE_BLOCK_RETRIES = Math.min(
		3,
		RequestScheduler.COOLDOWN_RETRIES - 1
	);
	/** Number of retries allowed on non-splitfile fetches. */
	static final int NON_SPLITFILE_RETRIES = Math.min(
		3,
		RequestScheduler.COOLDOWN_RETRIES - 1
	);
	...
	public FreenetURI insert(
		...
		ClientPutter put = new ClientPutter(
			pw,
			insert.getData(),
			insert.desiredURI,
	...
	@Override
	public FreenetURI insertRedirect(
	...
	public FreenetURI insertManifest(
		...
		try {
			putter = new DefaultManifestPutter(
				pw,
				BaseManifestPutter.bucketsByNameToManifestEntries(
					bucketsByName
				),
				priorityClass,
				insertURI,
