...
public class HighLevelSimpleClientImpl implements HighLevelSimpleClient, RequestClient, Cloneable {
    ...
    public FetchResult fetchFromMetadata(Bucket initialMetadata) throws FetchException {
        ...
        FetchContext context = getFetchContext();
        FetchWaiter fw = new FetchWaiter();
        ClientGetter get = new ClientGetter(fw, FreenetURI.EMPTY_CHK_URI, context, priorityClass, this, null, null,
                               initialMetadata);
    ...
    @Override
    public ClientGetter fetch(FreenetURI uri, long maxSize, RequestClient clientContext, ClientGetCallback callback,
                              FetchContext fctx)
    ...
    @Override
    public ClientGetter fetch(FreenetURI uri, long maxSize, RequestClient clientContext, ClientGetCallback callback,
                              FetchContext fctx, short prio)
            throws FetchException {
        return fetch(uri, clientContext, callback, fctx, prio);
    }

    @Override
    public ClientGetter fetch(FreenetURI uri, RequestClient clientContext, ClientGetCallback callback,
                              FetchContext fctx, short prio)
            throws FetchException {
        if (uri == null) {
            throw new NullPointerException();
    ...
    @Override
    public ClientGetter fetchFromMetadata(Bucket initialMetadata, RequestClient clientContext,
            ClientGetCallback callback, FetchContext fctx, short prio)
            throws FetchException {
        if (initialMetadata == null) {
            throw new NullPointerException();
        }

        ClientGetter get = new ClientGetter(callback, FreenetURI.EMPTY_CHK_URI, fctx, prio, clientContext, null, null,
                               initialMetadata);
    ...
    @Override
    public ClientPutter insert(InsertBlock insert, boolean getCHKOnly, String filenameHint, boolean isMetadata,
                               InsertContext ctx, ClientPutCallback cb)
            throws InsertException {
    ...
    @Override
    public ClientPutter insert(InsertBlock insert, boolean getCHKOnly, String filenameHint, boolean isMetadata,
                               InsertContext ctx, ClientPutCallback cb, short priority)
            throws InsertException {
        ClientPutter put = new ClientPutter(cb, insert.getData(), insert.desiredURI, insert.clientMetadata, ctx,
                               priority, getCHKOnly, isMetadata, this, filenameHint, false, core.clientContext, null,
                               -1);
    ...
    @Override
    public FreenetURI insertManifest(FreenetURI insertURI, HashMap<String, Object> bucketsByName, String defaultName,
                                     short priorityClass, byte[] forceCryptoKey)
            throws InsertException {
        PutWaiter pw = new PutWaiter();
        SimpleManifestPutter putter = new SimpleManifestPutter(pw,
                                          ...
                                          false, false, forceCryptoKey, null, core.clientContext);
    ...
    public FetchContext getFetchContext(long overrideMaxSize) {
        ...
        return new FetchContext(maxLength, maxTempLength, curMaxMetadataLength, MAX_RECURSION, MAX_ARCHIVE_RESTARTS,
                                MAX_ARCHIVE_LEVELS, DONT_ENTER_IMPLICIT_ARCHIVES, SPLITFILE_BLOCK_RETRIES,
                                NON_SPLITFILE_RETRIES, USK_RETRIES, FETCH_SPLITFILES, FOLLOW_REDIRECTS,
                                LOCAL_REQUESTS_ONLY, FILTER_DATA, MAX_SPLITFILE_BLOCKS_PER_SEGMENT,
                                MAX_SPLITFILE_CHECK_BLOCKS_PER_SEGMENT, bucketFactory, eventProducer, false,
                                CAN_WRITE_CLIENT_CACHE, null, null);
    }

    @Override
    public InsertContext getInsertContext(boolean forceNonPersistent) {
        return new InsertContext(INSERT_RETRIES, CONSECUTIVE_RNFS_ASSUME_SUCCESS, SPLITFILE_BLOCKS_PER_SEGMENT,
                                 SPLITFILE_CHECK_BLOCKS_PER_SEGMENT, eventProducer, CAN_WRITE_CLIENT_CACHE_INSERTS,
                                 Node.FORK_ON_CACHEABLE_DEFAULT, false, Compressor.DEFAULT_COMPRESSORDESCRIPTOR,
                                 EXTRA_INSERTS_SINGLE_BLOCK, EXTRA_INSERTS_SPLITFILE_HEADER,
