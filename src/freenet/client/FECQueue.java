...
public class FECQueue implements OOMHook {
    ...
    public static FECQueue create(final long nodeDBHandle, ObjectContainer container, FECQueue transientQueue) {
        @SuppressWarnings("serial") ObjectSet<FECQueue> result = container.query(new Predicate<FECQueue>() {
            @Override
            public boolean match(FECQueue queue) {
                if (queue.nodeDBHandle == nodeDBHandle) {
                    return true;
                }
    ...
    public void init(int priorities, int maxCacheSize, DBJobRunner dbJobRunner, Executor exec,
                    ...
                    while (true) {
                        ...
                        try {
                            ...
                            if (job.persistent) {
                                ...
                                int prio = job.isADecodingJob
                                           ? NativeThread.NORM_PRIORITY + 1 : NativeThread.NORM_PRIORITY;
                                ...
                                databaseJobRunner.queue(new DBJob() {
                                    ...
                                    public boolean run(ObjectContainer container, ClientContext context) {
                                        try {
                                            ...
                                            Logger.error(this, "Caught storing block statuses for " + job + " : " + t,
                                                         t);
                                        ...
                                        Logger.minor(this,
                                                     "Activating " + job.callback + " is active="
                                                     + container.ext().isActive(job.callback));
                            ...
                            if (!job.persistent) {
                                if (job.isADecodingJob) {
                                    job.callback.onDecodedSegment(null, clientContext, job, job.dataBlocks,
                                                                  job.checkBlocks, job.dataBlockStatus,
                                                                  job.checkBlockStatus);
                                } else {
                                ...
                                databaseJobRunner.queue(new DBJob() {
                                    ...
                                    public boolean run(ObjectContainer container, ClientContext context) {
                                        try {
                                            ...
                                            Logger.error(this, "Caught storing block statuses on " + this + " : " + t,
                                                         t);
                                            ...
                                            try {
                                                ...
                                                Logger.error(this, "Caught " + t1 + " in FECQueue callback failure",
                                                             t1);
                                            } finally {
                                        ...
                                        try {
                                            if (job.isADecodingJob) {
                                                job.callback.onDecodedSegment(container, clientContext, job,
                                                                              job.dataBlocks, job.checkBlocks,
                                                                              job.dataBlockStatus,
                                                                              job.checkBlockStatus);
                                            } else {
                                                job.callback.onEncodedSegment(container, clientContext, job,
                                                                              job.dataBlocks, job.checkBlocks,
                                                                              job.dataBlockStatus,
                                                                              job.checkBlockStatus);
                                            }
                ...
                while (true) {
                    ...
                    for (short prio = 0; prio < priorities; prio++) {
                        ...
                        synchronized (FECQueue.this) {
                            ...
                            if (newCached >= maxPersistentQueueCacheSize) {
                                return false;
                            }
                        ...
                        @SuppressWarnings("unchecked") ObjectSet<FECJob> results = query.execute();
                            ...
                            for (int j = 0; (j < grab) && results.hasNext(); j++) {
                                    ...
                                    if (job.callback != null) {
                                        ...
                                        try {
                                            job.callback.onFailed(
                                                new NullPointerException("Not all data blocks present"), container,
                                                context);
                                        } catch (Throwable t) {
                                            try {
                                                Logger.error(this,
                                                             "Caught " + t + " while calling failure callback on "
                                                             + job, t);
                                            } catch (Throwable t1) {
                                ...
                                synchronized (FECQueue.this) {
                                    ...
                                    if (persistentQueueCache[prio].contains(job)) {
                                        ...
                                        if (logMINOR) {
                                            Logger.minor(this,
                                                         "Not adding as on persistent queue cache for " + prio + " : "
                                                         + job);
                                        }
                                    ...
                                    for (ListIterator<FECJob> it = persistentQueueCache[prio].listIterator();
                                            it.hasNext(); ) {
                                        FECJob cmp = it.next();
    ...
    private synchronized int getMaxRunningFECThreads() {
        ...
        if ((osName.indexOf("Windows") == -1)
                && ((osName.toLowerCase().indexOf("mac os x") > 0) || (!NativeThread.usingNativeCode()))) {
