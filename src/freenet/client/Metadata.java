...
//  static final short FLAGS_SPLIT_USE_LENGTHS = 64; FIXME not supported, reassign to something else if we need a new flag
    ...
    public Metadata(DataInputStream dis, long length) throws IOException, MetadataParseException {
        ...
        if ((!splitfile) && ((documentType == SIMPLE_REDIRECT) || (documentType == ARCHIVE_MANIFEST))) {
            ...
            if (!((splitfileAlgorithm == SPLITFILE_NONREDUNDANT) || (splitfileAlgorithm == SPLITFILE_ONION_STANDARD))) {
            ...
            if (segmentCount == 1) {
                ...
                for (int i = 0; i < segments.length; i++) {
                    ...
                    if (i == segments.length - 1) {
                        ...
                        if ((copyCheckBlocks <= 0) || (copyDataBlocks <= 0)) {
                            throw new MetadataParseException("Last segment has bogus block count: total data blocks "
                                                             + splitfileBlocks + " total check blocks "
                                                             + splitfileCheckBlocks + " segment size "
                                                             + blocksPerSegment + " data " + checkBlocksPerSegment
                                                             + " check " + crossCheckBlocks
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "REQUESTING: Segment " + i + " of " + segments.length + " : " + copyDataBlocks
    ...
    public Metadata(byte docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, FreenetURI uri,
                    ClientMetadata cm) {
     ...
     */
    public Metadata(byte docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, String arg,
                    ClientMetadata cm) {
        hashCode = super.hashCode();
        ...
        if ((docType == ARCHIVE_INTERNAL_REDIRECT) || (docType == SYMBOLIC_SHORTLINK)) {
            ...
            while (true) {
                ...
                if (targetName.charAt(0) == '/') {
                    targetName = targetName.substring(1);
                    Logger.error(this,
                                 "Stripped initial slash from archive internal redirect on creating metadata: \"" + arg
                                 + "\"", new Exception("debug"));
     ...
     */
    public Metadata(byte docType, ARCHIVE_TYPE archiveType, COMPRESSOR_TYPE compressionCodec, FreenetURI uri,
                    ClientMetadata cm, long origDataLength, long origCompressedDataLength, int reqBlocks,
                    int totalBlocks, boolean topDontCompress, short topCompatibilityMode, HashResult[] hashes) {
        hashCode = super.hashCode();
    ...
    public Metadata(short algo, ClientCHK[] dataURIs, ClientCHK[] checkURIs, int segmentSize, int checkSegmentSize,
                    int deductBlocksFromSegments, ClientMetadata cm, long dataLength, ARCHIVE_TYPE archiveType,
                    COMPRESSOR_TYPE compressionCodec, long decompressedLength, boolean isMetadata, HashResult[] hashes,
                    byte[] hashThisLayerOnly, long origDataSize, long origCompressedDataSize, int requiredBlocks,
                    int totalBlocks, boolean topDontCompress, short topCompatibilityMode,
        ...
        if ((topSize != 0) || (topCompressedSize != 0) || (topBlocksRequired != 0) || (topBlocksTotal != 0)
                || (hashes != null) || (topCompatibilityMode != 0)) {
            parsedVersion = 1;
        }
    ...
    public String getSymbolicShortlinkTargetName() {
        if (documentType != SYMBOLIC_SHORTLINK) {
            throw new IllegalArgumentException();
        }
    ...
    public void writeTo(DataOutputStream dos) throws IOException, MetadataUnresolvedException {
        ...
        boolean hasTopBlocks = (topBlocksRequired != 0) || (topBlocksTotal != 0) || (topSize != 0)
                               || (topCompressedSize != 0) || (topCompatibilityMode != 0);
    ...
    public void dump(int indent, StringBuffer sb) {
        ...
        dumpline(indent, sb,
                 "Flags: sf=" + splitfile + " dbr=" + dbr + " noMIME=" + noMIME + " cmime=" + compressedMIME
