...
public class ClientPutter extends BaseClientPutter implements PutCompletionCallback {
     ...
     */
    public ClientPutter(ClientPutCallback client, Bucket data, FreenetURI targetURI, ClientMetadata cm,
                        InsertContext ctx, short priorityClass, boolean getCHKOnly, boolean isMetadata,
                        RequestClient clientContext, String targetFilename, boolean binaryBlob, ClientContext context,
                        byte[] overrideSplitfileCrypto, long metadataThreshold) {
     ...
     * @throws InsertException If the insert cannot be started for some reason.
     */
    public boolean start(boolean earlyEncode, boolean restart, ObjectContainer container, ClientContext context)
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (currentState != null) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Can't " + (restart ? "restart" : "start") + " : currentState != null : "
                                     + currentState);
                    }
                ...
                if (!cancel) {
                    if (!binaryBlob) {
                        ...
                        currentState = new SingleFileInserter(this, this,
                                new InsertBlock(data, meta, persistent() ? targetURI.clone() : targetURI), isMetadata,
                                ctx, realTimeFlag, false, getCHKOnly, false, null, null, false, targetFilename,
                                earlyEncode, false, persistent(), 0, 0, null, cryptoAlgorithm, cryptoKey,
                                metadataThreshold);
                    } else {
        ...
        if ((super.failedBlocks > 0) || (super.fatallyFailedBlocks > 0)
            ...
            if (!uri.isUSK()) {
                Logger.error(this,
                             ...
                             + " but success?! on " + this + " from " + state, new Exception("debug"));
            }
        }
     ...
     * instead of a URI. 
     */
    public void onMetadata(Bucket finalMetadata, ClientPutState state, ObjectContainer container,
     ...
     */
    public boolean restart(boolean earlyEncode, ObjectContainer container, ClientContext context)
            throws InsertException {
