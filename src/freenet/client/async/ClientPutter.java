...
public class ClientPutter
	 ...
	 * @throws InsertException If the insert cannot be started for some reason.
	 */
	public boolean start(boolean restart, ClientContext context)
		throws InsertException {
		if (logMINOR) Logger.minor(
			this,
			"Starting " + this + " for " + targetURI
		);
		...
		try {
			...
			boolean randomiseSplitfileKeys = randomiseSplitfileKeys(
				targetURI,
				ctx,
				persistent()
			);

			if (data == null) throw new InsertException(
			...
			synchronized (this) {
				if (restart) {
					...
					if (currentState != null && !finished) {
						if (logMINOR) Logger.minor(
							this,
							"Can't restart, not finished and currentState != null : " +
							currentState
						);
						return false;
					}
				...
				if (overrideSplitfileCrypto != null) {
					...
					if (cryptoKey.length != 32) throw new InsertException(
				...
				if (!cancel) {
					if (!binaryBlob) {
						...
						if (meta != null) meta = persistent()
							? meta.clone()
							: meta;
						currentState = new SingleFileInserter(
							this,
							this,
					...
					} else currentState = new BinaryBlobInserter(
						data,
						this,
						getClient(),
						false,
			...
			if (logMINOR) Logger.minor(
				this,
				"Starting insert: " + currentState
			);
			if (
				currentState instanceof SingleFileInserter
	...
	public static boolean randomiseSplitfileKeys(
		...
		boolean randomiseSplitfileKeys =
			targetURI.isSSK() || targetURI.isKSK() || targetURI.isUSK();
	...
	public void onSuccess(ClientPutState state, ClientContext context) {
		...
		if (
			...
			if (!uri.isUSK() && !ctx.getCHKOnly) Logger.error(
				this,
				"Failed blocks: " +
				failedBlocks +
	...
	public void onEncode(
		...
		synchronized (this) {
			...
			if (gotFinalMetadata) {
				Logger.error(
					this,
					"Generated URI *and* sent final metadata??? on " +
					this +
					" from " +
					state
				);
	...
	public void onMetadata(
		...
		synchronized (this) {
			...
			if (gotFinalMetadata) {
				Logger.error(
					this,
					"onMetadata called twice - already sent metadata to client for " +
					this
				);
				freeIt = true;
			} else {
