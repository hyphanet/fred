...
public class SplitFileFetcherGet extends SendableGet implements HasKeyListener {
	...
	public SplitFileFetcherGet(
		SplitFileFetcher fetcher,
	...
	public ClientKey getKey(SendableRequestItem token) {
		SplitFileFetcherStorageKey key =
			(SplitFileFetcherStorage.SplitFileFetcherStorageKey) token;
	...
	public void onFailure(
		...
		if (fe.isDefinitelyFatal()) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Fatal failure: " + fe + " for " + token
			);
			parent.fail(fe);
		} else {
			SplitFileFetcherStorage.SplitFileFetcherStorageKey key =
				(SplitFileFetcherStorageKey) token;
			if (key.get != storage) throw new IllegalArgumentException();
	...
	public long getCooldownWakeup(
		...
		return storage.segments[key.segmentNumber].getCooldownTime(
				key.blockNumber
			);
	}
	...
	public void schedule(ClientContext context, boolean ignoreStore) {
		ClientRequestScheduler sched = context.getChkFetchScheduler(
			realTimeFlag
		);
