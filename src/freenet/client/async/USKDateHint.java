...
public class USKDateHint {
	...
	public enum Type {
		...
		public boolean alwaysMorePreciseThan(Type type) {
			...
			if (this.equals(DAY)) { // Day beats everything.
				...
				return type.equals(YEAR);
			} else return false; // if(this.equals(YEAR)) - everything beats year
		}
	}
	...
	public FreenetURI[] getInsertURIs(InsertableUSK key) {
		...
		int x = 0;
		for (Type t : Type.values) uris[x++] = key
			.getInsertableSSK(key.siteName + PREFIX + get(t))
			.getInsertURI();
		return uris;
	}
	...
	public ClientSSK[] getRequestURIs(USK key) {
		...
		int x = 0;
		for (Type t : Type.values) uris[x++] = key.getSSK(
			key.siteName + PREFIX + get(t)
		);
		return uris;
	}
}
