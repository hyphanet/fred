...
public class MultiPutCompletionCallback implements PutCompletionCallback, ClientPutState {
    ...
    @Override
    public synchronized void onMetadata(Bucket metadata, ClientPutState state, ObjectContainer container,
            ClientContext context) {
        if (persistent) {
            container.activate(cb, 1);
        }
    ...
    public void removeFrom(ObjectContainer container, ClientContext context) {
        ...
        if (!waitingForFetchable.isEmpty()) {
            Logger.error(this,
                         "waitingForFetchable not empty in removeFrom() on " + this + " : " + waitingForFetchable);
        }
