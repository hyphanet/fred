...
class ClientRequestSchedulerCore extends ClientRequestSchedulerBase {
    ...
    ClientRequestSchedulerCore(Node node, boolean forInserts, boolean forSSKs, boolean forRT,
                               ObjectContainer selectorContainer, long cooldownTime) {
     ...
     * @return
     */
    public static ClientRequestSchedulerCore create(Node node, final boolean forInserts, final boolean forSSKs,
            final boolean forRT, final long nodeDBHandle, ObjectContainer selectorContainer, long cooldownTime,
            PrioritizedSerialExecutor databaseExecutor, ClientRequestScheduler sched, ClientContext context) {
...
//                  });
                    ...
                    for (int i = RequestStarter.MAXIMUM_PRIORITY_CLASS; i <= RequestStarter.MINIMUM_PRIORITY_CLASS;
                            i++) {
                        Query query = container.query();

                        query.constrain(RegisterMe.class);
                        query.descend("core").constrain(ClientRequestSchedulerCore.this).and(
...
//                  query.descend("key").descend("addedTime").orderAscending();
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "RegisterMe query took " + (tEnd - tStart) + " hasNext=" + registerMeSet.hasNext()
                                     + " for insert=" + isInsertScheduler + " ssk=" + isSSKScheduler);
                    }
...
//                          Logger.minor(this, "RegisterMe query returned: "+registerMeSet.size());
                    ...
                    try {
                        context.jobRunner.queue(registerMeRunner, (NativeThread.NORM_PRIORITY - 1) + (boost ? 1 : 0),
                                                true);
    ...
    class RegisterMeRunner implements DBJob {
        ...
        public boolean run(ObjectContainer container, ClientContext context) {
            ...
            if (registerMeSet == null) {
                Logger.error(this,
                             "registerMeSet is null for " + ClientRequestSchedulerCore.this + " ( " + this + " )");
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                if (reg.core != ClientRequestSchedulerCore.this) {
                    ...
                    if (reg.core == null) {
                        Logger.error(
                            this,
                            "Leftover RegisterMe " + reg
                            + " : core already deleted. THIS IS AN ERROR unless you have seen \"Old core not active\" messages before this point.");
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Ignoring RegisterMe " + reg + " as doesn't belong to me: my insert="
                                     + isInsertScheduler + " my ssk=" + isSSKScheduler + " his insert="
                                     + reg.core.isInsertScheduler + " his ssk=" + reg.core.isSSKScheduler);
                    }
...
//              }
                if (logMINOR) {
                    Logger.minor(this,
                                 "Running RegisterMe " + reg + " for " + reg.nonGetRequest + " : " + reg.addedTime
                                 + " : " + reg.priority);
                }
                ...
                if (reg.nonGetRequest != null) {
                    ...
                    if (reg.nonGetRequest.isStorageBroken(container)) {
                        ...
                        Logger.error(this,
                                     "Stored SingleBlockInserter is broken, maybe leftover from database leakage?: "
                                     + toString);
