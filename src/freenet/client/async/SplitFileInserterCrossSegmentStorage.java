...
public class SplitFileInserterCrossSegmentStorage {
	...
	SplitFileInserterCrossSegmentStorage(
		...
		for (int i = 0; i < totalBlocks; i++) {
			...
			if (
				readSegmentNumber < 0 ||
				readSegmentNumber >= parent.segments.length
			) throw new StorageFormatException(
				"Bogus segment number " + readSegmentNumber
			);
			int readBlockNumber = dis.readInt();
			SplitFileInserterSegmentStorage segment =
				parent.segments[readSegmentNumber];
			if (
				readBlockNumber < 0 ||
				(readBlockNumber >=
					segment.dataBlockCount + segment.crossCheckBlockCount) ||
				(i < dataBlockCount &&
					readBlockNumber >= segment.dataBlockCount) ||
				(i >= dataBlockCount &&
					readBlockNumber < segment.dataBlockCount)
			) throw new StorageFormatException(
		...
		try {
			...
			innerStoreStatus(dos);
			dos.close();
			int computedStatusLength =
				(int) cos.written() + parent.checker.checksumLength();
			if (
				computedStatusLength > statusLength
			) throw new StorageFormatException(
				"Stored status length smaller than required"
			);
		} catch (IOException e) {
			throw new Error(e); // Impossible
		}
					...
					try {
						...
						try {
							if (!shutdown) {
								...
								synchronized (
									SplitFileInserterCrossSegmentStorage.this
								) {
									encoding = false;
								}
	...
	private void writeCheckBlock(int checkBlockNo, byte[] buf)
		...
		if (DEBUG_ENCODE) {
			SplitFileInserterSegmentStorage segment =
				segments[checkBlockNo + dataBlockCount];
	 ...
	 * @throws IOException */
	byte[] readCheckBlock(
		int slotNumberWithinCrossSegment,
		int segmentNumber,
		int blockNoWithinSegment
	) throws IOException {
		assert (blockNumbers[slotNumberWithinCrossSegment] ==
			blockNoWithinSegment);
		assert (segments[slotNumberWithinCrossSegment].segNo == segmentNumber);
		return parent.readCheckBlock(
			segNo,
			slotNumberWithinCrossSegment - dataBlockCount
		);
	}
	...
	private byte[][] readDataBlocks() throws IOException {
		...
		try {
			...
			for (int i = 0; i < dataBlockCount; i++) {
				...
				if (DEBUG_ENCODE) {
					ClientCHK key =
						segments[i].encodeBlock(data[i]).getClientKey();
		...
		try {
			dos = new DataOutputStream(
				parent.writeChecksummedTo(
					parent.crossSegmentStatusOffset(segNo),
					statusLength
				)
			);
			innerStoreStatus(dos);
