...
public class SplitFileInserterStorage {
	...
	private final RandomArrayIterator<
		SplitFileInserterSegmentStorage
	...
	public SplitFileInserterStorage(
		...
		if (
			dataLength > ((long) Integer.MAX_VALUE) * CHKBlock.DATA_LENGTH
		) throw new InsertException(InsertExceptionMode.TOO_BIG);
		totalDataBlocks = (int) ((dataLength + CHKBlock.DATA_LENGTH - 1) /
		...
		if (cmode == CompatibilityMode.COMPAT_1250_EXACT) {
			...
			if (
				...
				int lastSegmentSize =
					totalDataBlocks - (segmentSize * (segs - 1));
		...
		checkSegmentSize = codec.getCheckBlocks(
			segmentSize + crossCheckBlocks,
			cmode
		);
		...
		if (splitfileCryptoKey != null) {
			...
			if (hashThisLayerOnly != null) {
				this.splitfileCryptoKey = Metadata.getCryptoKey(
					hashThisLayerOnly
				);
			} else {
		...
		if (crossCheckBlocks != 0) {
			byte[] seed = Metadata.getCrossSegmentSeed(
				hashes,
				hashThisLayerOnly
			);
			if (logMINOR) Logger.minor(
				this,
				"Cross-segment seed: " + HexUtil.bytesToHex(seed)
			);
			...
			for (int i = 0; i < crossSegments.length; i++) {
				if (logMINOR) Logger.minor(
					this,
					"Allocating blocks for cross segment " + i
				);
				...
				SplitFileInserterCrossSegmentStorage seg =
					new SplitFileInserterCrossSegmentStorage(
						this,
						i,
						persistent,
						segLen,
		...
		if (crossSegments != null) {
			...
			if (persistent) offsetCrossSegmentStatus =
				new long[crossSegments.length];
		...
		if (dataLength % CHKBlock.DATA_LENGTH != 0) {
			...
			long from =
				(dataLength / CHKBlock.DATA_LENGTH) * CHKBlock.DATA_LENGTH;
			byte[] buf = new byte[(int) (dataLength - from)];
			this.originalData.pread(from, buf, 0, buf.length);
			paddedLastBlock = BucketTools.pad(
				buf,
				CHKBlock.DATA_LENGTH,
				buf.length
			);
		...
		if (crossSegments != null) {
			for (int i = 0; i < crossSegments.length; i++) {
				...
				ptr +=
				crossSegments[i].crossCheckBlockCount * CHKBlock.DATA_LENGTH;
			}
		}
		...
		if (hasPaddedLastBlock) raf.pwrite(
			offsetPaddedLastBlock,
			paddedLastBlock,
			0,
			paddedLastBlock.length
		);
		if (persistent) {
	...
	public SplitFileInserterStorage(
		...
		InputStream is = checker.checksumReaderWithLength(
			ois,
		...
		LockableRandomAccessBuffer rafOrig = BucketTools.restoreRAFFrom(
			dis,
			persistentFG,
		...
		if (originalData == null) {
			...
			if (!originalData.equals(rafOrig)) throw new StorageFormatException(
		...
		if (dataLength <= 0) throw new StorageFormatException(
			"Bad data length"
		);
		if (dataLength != originalData.size()) throw new ResumeFailedException(
		...
		if (atype == -1) {
			...
			if (archiveType == null) throw new StorageFormatException(
				"Unknown archive type " + atype
			);
		}
		try {
		...
		if (codec == (short) -1) compressionCodec = null;
			...
			if (compressionCodec == null) throw new StorageFormatException(
				"Unknown compression codec ID " + codec
			);
		}
		...
		if (segmentSize <= 0) throw new StorageFormatException(
			"Bad segment size"
		);
		this.checkSegmentSize = dis.readInt();
		if (checkSegmentSize <= 0) throw new StorageFormatException(
			"Bad check segment size"
		);
		this.crossCheckBlocks = dis.readInt();
		if (crossCheckBlocks < 0) throw new StorageFormatException(
			"Bad cross-check block count"
		);
		if (
			segmentSize + checkSegmentSize + crossCheckBlocks >
			FECCodec.MAX_TOTAL_BLOCKS_PER_SEGMENT
		) throw new StorageFormatException(
		...
		if (
			!Metadata.isValidSplitfileCryptoAlgorithm(splitfileCryptoAlgorithm)
		) throw new StorageFormatException(
		...
		if (
			deductBlocksFromSegments < 0 ||
			deductBlocksFromSegments > segmentCount
		...
		if (consecutiveRNFsCountAsSuccess < 0) throw new StorageFormatException(
			"Bad consecutiveRNFsCountAsSuccess"
		);
		...
		if (hasPaddedLastBlock) {
			offsetPaddedLastBlock = readOffset(
				dis,
				rafLength,
				"offsetPaddedLastBlock"
			);
		...
		if (overallStatusLength < 0) throw new StorageFormatException(
			"Negative overall status length"
		);
		if (
			overallStatusLength < FailureCodeTracker.getFixedLength(true)
		...
		if (crossSegments != null) {
			...
			for (
				int i = 0;
				i < crossSegments.length;
				i++
			) offsetCrossSegmentBlocks[i] = readOffset(
				dis,
				rafLength,
				"cross-segment block offset"
			);
		} else {
			offsetCrossSegmentBlocks = null;
		}
		offsetSegmentCheckBlocks = new long[segmentCount];
		for (int i = 0; i < segmentCount; i++) offsetSegmentCheckBlocks[i] =
			readOffset(dis, rafLength, "segment check block offset");
		offsetSegmentStatus = new long[segmentCount];
		for (int i = 0; i < segmentCount; i++) offsetSegmentStatus[i] =
			readOffset(dis, rafLength, "segment status offset");
		if (crossSegments != null) {
			...
			for (
				...
				i < crossSegments.length;
				i++
			) offsetCrossSegmentStatus[i] = readOffset(
				dis,
				rafLength,
				"cross-segment status offset"
			);
		} else {
			offsetCrossSegmentStatus = null;
		}
		offsetSegmentKeys = new long[segmentCount];
		for (int i = 0; i < segmentCount; i++) offsetSegmentKeys[i] =
			readOffset(dis, rafLength, "segment keys offset");
		dis.close();
		// Set up segments...
		...
		for (int i = 0; i < segmentCount; i++) {
			segments[i] = new SplitFileInserterSegmentStorage(
				this,
				dis,
				i,
				keyLength,
		...
		dis.close();
		if (blocks != totalDataBlocks) throw new StorageFormatException(
			"Total data blocks should be " +
			totalDataBlocks +
			" but is " +
			blocks
		);
		if (crossSegments != null) {
			...
			dis = new DataInputStream(is);
			for (int i = 0; i < crossSegments.length; i++) {
				crossSegments[i] = new SplitFileInserterCrossSegmentStorage(
					this,
					dis,
					i
				);
		...
		ois.close();
		ois = new RAFInputStream(
			raf,
			offsetOverallStatus,
			rafLength - offsetOverallStatus
		);
		dis = new DataInputStream(
			checker.checksumReaderWithLength(
				ois,
				new ArrayBucketFactory(),
				maxLength
			)
	...
	private byte[] encodeOverallStatus() {
		...
		try {
			...
			OutputStream cos = checker.checksumWriterWithLength(
				os,
				new ArrayBucketFactory()
			);
	 ...
	 */
	private Bucket encodeCrossSegmentSettings(BucketFactory bf)
		throws IOException {
	...
	private byte[] encodeHeader() {
		...
		try {
			...
			OutputStream os = checker.checksumWriterWithLength(
				baos,
				new ArrayBucketFactory()
			);
	...
	private void allocateCrossDataBlock(
		SplitFileInserterCrossSegmentStorage segment,
		Random xsRandom
	) {
		int x = 0;
		for (int i = 0; i < 10; i++) {
	...
	private void allocateCrossCheckBlock(
		...
		for (int i = 0; i < 10; i++) {
			x = xsRandom.nextInt(segments.length);
			SplitFileInserterSegmentStorage seg = segments[x];
			int blockNum = seg.allocateCrossCheckBlock(
				segment,
				xsRandom,
	...
	private SplitFileInserterSegmentStorage[] makeSegments(
		...
		SplitFileInserterSegmentStorage[] segments =
			new SplitFileInserterSegmentStorage[segCount];
		if (segCount == 1) {
			// Single segment
			int checkBlocks = codec.getCheckBlocks(
			...
			for (int i = segmentSize;;) {
				this.underlyingOffsetDataSegments[segNo] = (long) j *
				CHKBlock.DATA_LENGTH;
				...
				if (data > (i - j)) {
					...
					data = i - j;
					check = codec.getCheckBlocks(
						data + crossCheckBlocks,
						cmode
					);
				}
				j = i;
				...
				if (deductBlocksFromSegments != 0) if (logMINOR) Logger.minor(
					this,
					"INSERTING: Segment " +
					segNo +
					" of " +
	...
	public void start() {
		...
		if (crossSegments != null) Logger.normal(
			this,
			"Starting splitfile, " +
	...
	private void startSegmentEncode() {
		...
		for (SplitFileInserterSegmentStorage segment : segments) segment.startEncode(
			prio
		);
	}
	...
	private void startCrossSegmentEncode() {
		...
		for (SplitFileInserterCrossSegmentStorage segment : crossSegments) segment.startEncode(
			prio
		);
	}
	...
	public void onFinishedEncoding(
		SplitFileInserterCrossSegmentStorage completed
	) {
	 ...
	 */
	public void onFinishedEncoding(
		final SplitFileInserterSegmentStorage completed
	) {
		jobRunner.queueNormalOrDrop(
			new PersistentJob() {
				@Override
	...
	public void onHasKeys(
		SplitFileInserterSegmentStorage splitFileInserterSegmentStorage
	) {
		for (SplitFileInserterSegmentStorage segment : segments) {
	 ...
	 * @throws IOException
	 */
	void writeCheckBlock(int segNo, int checkBlockNo, byte[] buf)
		throws IOException {
		synchronized (this) {
			if (
				status == Status.ENCODED ||
		...
		long offset =
			offsetCrossSegmentBlocks[segNo] +
			checkBlockNo * CHKBlock.DATA_LENGTH;
		raf.pwrite(offset, buf, 0, buf.length);
	}

	public byte[] readCheckBlock(int segNo, int checkBlockNo)
		throws IOException {
		assert (segNo >= 0 && segNo < crossSegments.length);
		assert (checkBlockNo >= 0 && checkBlockNo < crossCheckBlocks);
		long offset =
	...
	public byte[] readSegmentDataBlock(int segNo, int blockNo)
		throws IOException {
		...
		long offset =
			underlyingOffsetDataSegments[segNo] +
			blockNo * CHKBlock.DATA_LENGTH;
	...
	public void writeSegmentCheckBlock(int segNo, int checkBlockNo, byte[] buf)
		throws IOException {
		assert (segNo >= 0 && segNo < segments.length);
		assert (checkBlockNo >= 0 &&
			checkBlockNo < segments[segNo].checkBlockCount);
		assert (buf.length == CHKBlock.DATA_LENGTH);
		long offset =
			offsetSegmentCheckBlocks[segNo] +
			checkBlockNo * CHKBlock.DATA_LENGTH;
		raf.pwrite(offset, buf, 0, buf.length);
	}

	public byte[] readSegmentCheckBlock(int segNo, int checkBlockNo)
		throws IOException {
		assert (segNo >= 0 && segNo < segments.length);
		assert (checkBlockNo >= 0 &&
			checkBlockNo < segments[segNo].checkBlockCount);
	...
	Metadata encodeMetadata() throws IOException, MissingKeyException {
		ClientCHK[] dataKeys = new ClientCHK[totalDataBlocks +
		...
		for (int segNo = 0; segNo < segments.length; segNo++) {
			...
			for (
				int i = 0;
				i < segment.dataBlockCount + segment.crossCheckBlockCount;
				i++
			) {
				dataKeys[dataPtr++] = segment.readKey(i);
			}
			for (int i = 0; i < segment.checkBlockCount; i++) {
				checkKeys[checkPtr++] = segment.readKey(
					i + segment.dataBlockCount + segment.crossCheckBlockCount
				);
			}
		}
	...
	void innerWriteSegmentKey(int segNo, int blockNo, byte[] buf)
		throws IOException {
		assert (buf.length ==
			SplitFileInserterSegmentStorage.getKeyLength(this));
		assert (segNo >= 0 && segNo < segments.length);
	...
	public void segmentSucceeded(
		final SplitFileInserterSegmentStorage completedSegment
	) {
		if (logMINOR) Logger.minor(
			this,
			"Succeeded segment " + completedSegment + " for " + callback
		);
		jobRunner.queueNormalOrDrop(
			new PersistentJob() {
				@Override
				public boolean run(ClientContext context) {
					...
					if (allSegmentsSucceeded()) {
						...
						try {
							...
							InsertException e1 = new InsertException(
								InsertExceptionMode.BUCKET_ERROR
							);
							synchronized (this) {
								failing = e1;
						...
						if (logMINOR) Logger.minor(
							this,
							"Not all segments succeeded for " + this
						);
					}
					return true;
				}
		...
		if (allSegmentsCompletedOrFailed()) {
			...
			synchronized (this) {
				...
				if (hasFinished()) {
					if (logMINOR) Logger.minor(
						this,
						"Maybe fail returning true because already finished"
					);
					return true;
				}
				status = Status.FAILED;
			...
			if (logMINOR) Logger.minor(
				this,
				"Maybe fail returning true with error " + e
			);
			callback.onFailed(e);
			return true;
		} else {
	...
	void fail(final InsertException e) {
		...
		if (
			e.mode == InsertExceptionMode.BUCKET_ERROR ||
			e.mode == InsertExceptionMode.INTERNAL_ERROR
	...
	public long countAllKeys() {
		long total = 0;
		for (SplitFileInserterSegmentStorage segment : segments) total +=
