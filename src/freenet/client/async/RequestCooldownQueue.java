...
public class RequestCooldownQueue implements CooldownQueue {
    ...
    private synchronized void add(Key key, SendableGet client, long removeTime) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Adding key " + key + " client " + client + " remove time " + removeTime + " startPtr="
                         + startPtr + " endPtr=" + endPtr + " keys.length=" + keys.length);
        }
        ...
        if (logMINOR) {
            Logger.minor(this, "Added at " + ptr + " startPtr=" + startPtr + " endPtr=" + endPtr);
        }
     ...
     */
    @Override
    public synchronized Object removeKeyBefore(long now, long dontCareAfterMillis, ObjectContainer container,
            int maxKeys) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Remove key before " + now + " : startPtr=" + startPtr + " endPtr=" + endPtr + " holes="
                         + holes + " keys.length=" + keys.length);
        }
        ...
        if (foundIT) {
            if (logMINOR) {
                Logger.minor(this, "FOUND IT!");    // FIXME remove
            }
        }
    ...
    private boolean bigLog() {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (cr instanceof ClientGetter) {
                ...
                if (ctr == null) {
                    ctr = Integer.valueOf(1);
                } else {
                    ctr = Integer.valueOf(ctr.intValue() + 1);
                }
...
//      System.err.println();
    ...
    public synchronized boolean removeKey(Key key, SendableGet client, long time, ObjectContainer container) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Remove key " + key + " client " + client + " at time " + time + " startPtr=" + startPtr
                         + " endPtr=" + endPtr + " holes=" + holes + " keys.length=" + keys.length);
        }
    ...
    private synchronized void expandQueue() {
        ...
        if (endPtr > startPtr) {
            for (int i = startPtr; i < endPtr; i++) {
                ...
                if (lastTime > times[i]) {
                    Logger.error(this,
                                 "RequestCooldownQueue INCONSISTENCY: times[" + i + "] = times[i] but lastTime="
                                 + lastTime);
                }
            ...
            Logger.error(this,
                         "RequestCooldownQueue: expandQueue() called with endPtr == startPtr == " + startPtr + " !!");
