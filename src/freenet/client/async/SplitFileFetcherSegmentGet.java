...
public class SplitFileFetcherSegmentGet extends SendableGet implements SupportsBulkCallFailure {
    ...
    public void onFailure(FetchException e, Object token, ObjectContainer container, ClientContext context) {
        ...
        if (e.isFatal() && (token == null)) {
            ...
            segment.onFatalFailure(e, ((SplitFileFetcherSegmentSendableRequestItem) token).blockNum, container,
                                   context);
        } else {
            segment.onNonFatalFailure(e, ((SplitFileFetcherSegmentSendableRequestItem) token).blockNum, container,
                                      context);
        }
    ...
    public long getCooldownWakeup(Object token, ObjectContainer container, ClientContext context) {
        ...
        return segment.getCooldownWakeup(((SplitFileFetcherSegmentSendableRequestItem) token).blockNum,
                                         segment.getMaxRetries(container), container, context);
    }
    ...
    void reschedule(ObjectContainer container, ClientContext context) {
        if (this.getParentGrabArray() != null) {
            if (logMINOR) {
                Logger.minor(this, "Not rescheduling as already scheduled on " + getParentGrabArray());
            }
        ...
        try {
            getScheduler(container, context).register(null, new SendableGet[] { this }, persistent, container,
                         getContextBlocks(container), true);
    ...
    public SendableRequestItem chooseKey(KeysFetchingLocally fetching, ObjectContainer container,
        ...
        while (true) {
            ...
            if (segment.checkRecentlyFailed(x, container, context, fetching, System.currentTimeMillis())) {
                continue;
            }
