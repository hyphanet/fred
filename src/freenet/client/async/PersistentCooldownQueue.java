...
public class PersistentCooldownQueue implements CooldownQueue {
    ...
    public long add(Key key, SendableGet client, ObjectContainer container) {
        ...
        PersistentCooldownQueueItem persistentCooldownQueueItem = new PersistentCooldownQueueItem(client,
                                                                      key.cloneKey(), removeTime, this);
        ...
        if (!itemsFromLastTime.isEmpty()) {
            ...
            for (Iterator<PersistentCooldownQueueItem> it = itemsFromLastTime.iterator();
                ...
                if ((i.client == null) ||!container.ext().isStored(i.client)) {
                    Logger.normal(this,
                                  "Client has been removed but not the persistent cooldown queue item: time " + i.time
                                  + " for key " + i.key);
                }

                if (i.key == null) {
                    Logger.error(this,
                                 "Key is null on cooldown queue! i = " + i + " client=" + i.client + " key as bytes = "
                                 + i.keyAsBytes);
                } else {
