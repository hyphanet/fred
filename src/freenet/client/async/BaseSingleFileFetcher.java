...
public abstract class BaseSingleFileFetcher
	...
	protected BaseSingleFileFetcher(
		...
		if (logMINOR) Logger.minor(
			this,
			"Creating BaseSingleFileFetcher for " + key
		);
		retryCount = 0;
	...
	public SendableRequestItem chooseKey(
		...
		if (l > 0 && l > now) {
			if (
				maxRetries == -1 ||
				(maxRetries >= RequestScheduler.COOLDOWN_RETRIES)
			) {
				// FIXME synchronization!!!
				if (logMINOR) Logger.minor(
					this,
					"RecentlyFailed -> cooldown until " +
			...
			if (cachedCooldownTries == 0 || r % cachedCooldownTries == 0) {
				...
				if (cooldownWakeupTime > now) {
					Logger.error(
						...
						this +
						" until " +
						freenet.support.TimeUtil.formatTime(
	...
	protected abstract void onBlockDecodeError(
		SendableRequestItem token,
		ClientContext context
	);
	...
	public void schedule(ClientContext context) {
		if (key == null) throw new NullPointerException();
		getScheduler(context).register(
			this,
			new SendableGet[] { this },
			persistent,
			ctx.blocks,
			false
		);
	...
	public byte[] getWantedKey() {
		...
		return newKey instanceof NodeSSK
			? ((NodeSSK) newKey).getPubKeyHash()
			: newKey.getRoutingKey();
	}
