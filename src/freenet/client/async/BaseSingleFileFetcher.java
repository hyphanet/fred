...
public abstract class BaseSingleFileFetcher extends SendableGet implements HasKeyListener, HasCooldownTrackerItem {
    ...
    @Override
    public SendableRequestItem chooseKey(KeysFetchingLocally fetching, ObjectContainer container,
        ...
        if (fetching.hasKey(k, this, persistent, container)) {
            return null;
        }
        ...
        if ((l > 0) && (l > now)) {
            if ((maxRetries == -1) || (maxRetries >= RequestScheduler.COOLDOWN_RETRIES)) {
                ...
                this.onFailure(new LowLevelGetException(LowLevelGetException.RECENTLY_FAILED), null, container,
                               context);
    ...
    protected boolean retry(ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Attempting to retry... (max " + maxRetries + ", current " + r + ") on " + this + " finished="
            ...
            if ((cachedCooldownTries == 0) || (r % cachedCooldownTries == 0)) {
                ...
                if (tracker.cooldownWakeupTime > now) {
                    Logger.error(this,
                                 ...
                                 + freenet.support.TimeUtil.formatTime(tracker.cooldownWakeupTime
                                     - now), new Exception("error"));
                } else {
                    if (logMINOR) {
                    ...
                    context.cooldownTracker.setCachedWakeup(tracker.cooldownWakeupTime, this, getParentGrabArray(),
                            persistent, container, context, true);
    ...
    public short getPriorityClass(ObjectContainer container) {
        if (persistent) {
            container.activate(parent, 1);    // Not much point deactivating it
        }
    ...
    public void onSuccess(KeyBlock lowLevelBlock, boolean fromStore, Object token, ObjectContainer container,
                          ClientContext context) {
        if (persistent) {
    ...
    protected abstract void onBlockDecodeError(Object token, ObjectContainer container, ClientContext context);

    /** Called when/if the low-level request succeeds. */
    public abstract void onSuccess(ClientKeyBlock block, boolean fromStore, Object token, ObjectContainer container,
                                   ClientContext context);
    ...
    public void requeueAfterCooldown(Key key, long time, ObjectContainer container, ClientContext context) {
        ...
        if (!(key.equals(this.key.getNodeKey(false)))) {
            Logger.error(this,
                         "Got requeueAfterCooldown for wrong key: " + key + " but mine is "
                         + this.key.getNodeKey(false) + " for " + this.key);
    ...
    public void schedule(ObjectContainer container, ClientContext context) {
        ...
        try {
            getScheduler(container, context).register(this, new SendableGet[] { this }, persistent, container,
                         ctx.blocks, false);
    ...
    public void reschedule(ObjectContainer container, ClientContext context) {
        ...
        try {
            getScheduler(container, context).register(null, new SendableGet[] { this }, persistent, container,
                         ctx.blocks, true);
        } catch (KeyListenerConstructionException e) {
            Logger.error(this, "Impossible: " + e + " on " + this, e);
        }
    ...
    public Key[] listKeys(ObjectContainer container) {
        if ((container != null) &&!persistent) {
            Logger.error(this,
                         "listKeys() on " + this + " but persistent=false, stored is " + container.ext().isStored(this)
                         + " active is " + container.ext().isActive(this));
        }
    ...
    public List<PersistentChosenBlock> makeBlocks(PersistentChosenRequest request, RequestScheduler sched,
            KeysFetchingLocally keysFetching, ObjectContainer container, ClientContext context) {
        if (persistent) {
            container.activate(key, 5);
        }
    ...
    public KeyListener makeKeyListener(ObjectContainer container, ClientContext context, boolean onStartup) {
        ...
        if (parent == null) {
            ...
            if (container != null) {
                Logger.error(this,
                             "Stored = " + container.ext().isStored(this) + " active = "
