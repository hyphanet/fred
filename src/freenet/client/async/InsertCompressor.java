...
public class InsertCompressor implements CompressJob {
	...
	public void init(final ClientContext ctx) {
		synchronized (this) {
			...
			if (scheduled) {
				Logger.error(
					this,
					"Already scheduled compression, not rescheduling"
				);
				return;
			}
			scheduled = true;
	...
	@Override
	public void tryCompress(final ClientContext context)
		...
		try {
			COMPRESSOR_TYPE[] comps = COMPRESSOR_TYPE.getCompressorsArray(
				compressorDescriptor
			);
			boolean first = true;
			long amountOfDataToCheckCompressionRatio = config
				.get("node")
				.getLong("amountOfDataToCheckCompressionRatio");
			int minimumCompressionPercentage = config
				.get("node")
				.getInt("minimumCompressionPercentage");
			int maxTimeForSingleCompressor = config
				.get("node")
				.getInt("maxTimeForSingleCompressor");
				...
				try {
					...
					if (persistent) {
						...
						try {
							...
							Logger.error(
								this,
								"Transient insert callback threw " + t,
								t
							);
						}
					}
					...
					try {
						...
						if (first && generateHashes != 0) {
							if (logMINOR) Logger.minor(
								this,
								"Generating hashes: " + generateHashes
							);
							is = hasher = new MultiHashInputStream(
								is,
								generateHashes
							);
						}
					...
					long resultNumberOfBlocks =
						resultSize / CHKBlock.DATA_LENGTH;
					...
					if (!context.jobRunner.shuttingDown()) Logger.error(
						this,
						"Database disabled compressing data",
						new Exception("error")
					);
					...
					new PrioRunnable() {
						...
						public void run() {
							try {
								...
								Logger.error(
									this,
									"Caught " + t + " running compression job",
									t
								);
							}
						}
			...
			if (
				bestCompressedData != null && bestCompressedData != origData
			) bestCompressedData.free();
	...
	public static InsertCompressor start(
		...
		InsertCompressor compressor = new InsertCompressor(
			inserter,
			origData,
			minSize,
			bf,
