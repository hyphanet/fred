...
public class InsertCompressor implements CompressJob {
    ...
    public InsertCompressor(long nodeDBHandle2, SingleFileInserter inserter2, Bucket origData2, int minSize2,
                            BucketFactory bf, boolean persistent, long generateHashes, boolean pre1254) {
    ...
    public void init(ObjectContainer container, final ClientContext ctx) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Compressing " + this + " : origData.size=" + origData.size() + " for " + inserter
    ...
    public void tryCompress(final ClientContext context) throws InsertException {
        ...
        try {
            BucketChainBucketFactory bucketFactory2 = new BucketChainBucketFactory(bucketFactory, CHKBlock.DATA_LENGTH,
                                                          persistent ? context.jobRunner : null, 1024, true);
            ...
            for (final COMPRESSOR_TYPE comp : comps) {
                ...
                try {
                            ...
                            public boolean run(ObjectContainer container, ClientContext context) {
                                if (!container.ext().isStored(inserter)) {
                                    if (InsertCompressor.logMINOR) {
                                        Logger.minor(this,
                                                     "Already deleted (start compression): " + inserter + " for "
                    ...
                    if (resultSize < bestCompressedDataSize) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "New size " + resultSize + " better than old best " + bestCompressedDataSize);
                        }
                ...
                context.jobRunner.queue(new DBJob() {
                    ...
                    public boolean run(ObjectContainer container, ClientContext context) {
                        if (!container.ext().isStored(inserter)) {
                            if (InsertCompressor.logMINOR) {
                                Logger.minor(this,
                                             "Already deleted (on failed): " + inserter + " for "
                                             + InsertCompressor.this);
                            }
    ...
    public static InsertCompressor start(ObjectContainer container, ClientContext context, SingleFileInserter inserter,
        ...
        if (persistent != (container != null)) {
            throw new IllegalStateException("Starting compression, persistent=" + persistent + " but container="
                                            + container);
        }

        InsertCompressor compressor = new InsertCompressor(context.nodeDBHandle, inserter, origData, minSize, bf,
                                          persistent, generateHashes, pre1254);
