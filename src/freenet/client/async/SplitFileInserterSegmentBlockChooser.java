...
public class SplitFileInserterSegmentBlockChooser extends SimpleBlockChooser {
	...
	public SplitFileInserterSegmentBlockChooser(
		...
		if (consecutiveRNFsCountAsSuccess > 0) consecutiveRNFs =
			new int[blocks];
	...
	public void onRNF(int blockNo) {
		synchronized (this) {
			...
			if (
				++consecutiveRNFs[blockNo] < consecutiveRNFsCountAsSuccess
			) return;
		}
		onSuccess(blockNo);
	}
	...
	public synchronized boolean pushRNFs(int blockNo) {
		...
		for (int i = 0; i < ret; i++) if (
			onNonFatalFailure(blockNo)
		) return true;
		return false;
	}
