...
public class ClientRequestScheduler implements RequestScheduler {
    ...
    private final transient LinkedList<PersistentChosenRequest> starterQueue =
    ...
    public ClientRequestScheduler(boolean forInserts, boolean forSSKs, boolean forRT, RandomSource random,
                                  RequestStarter starter, Node node, NodeClientCore core, String name,
    ...
    public void startCore(NodeClientCore core, long nodeDBHandle, ObjectContainer container) {
        schedCore = ClientRequestSchedulerCore.create(node, isInsertScheduler, isSSKScheduler, isRTScheduler,
                nodeDBHandle, container, COOLDOWN_PERIOD, core.clientDatabaseExecutor, this, clientContext);
        ...
        while (true) {
            ...
            try {
                ...
                if (listener != null) {
                    ...
                    if (logMINOR) {
                        Logger.minor(ClientRequestScheduler.class,
            ...
            if ((getters != null) &&!noCheckStore) {
                ...
                for (SendableGet getter : getters) {
                    if (!(getter.isCancelled(null)
                            || (getter.getCooldownTime(container, clientContext, System.currentTimeMillis()) != 0))) {
                        anyValid = true;
                    }
                }
    ...
    private void innerRegister(final HasKeyListener hasListener, final SendableGet[] getters, final BlockSet blocks,
                               boolean noCheckStore, ObjectContainer container)
            throws KeyListenerConstructionException {
        final KeyListener listener;
    ...
    void finishRegister(final SendableGet[] getters, boolean persistent, ObjectContainer container,
                        final boolean anyValid, final DatastoreCheckerItem reg) {
        if (logMINOR) {
            Logger.minor(this,
                         "finishRegister for " + Fields.commaList(getters) + " anyValid=" + anyValid + " reg=" + reg
    ...
    public ChosenBlock getBetterNonPersistentRequest(short prio) {
        ...
        return selector.removeFirstTransient(fuzz, random, offeredKeys, starter, schedTransient, prio, isRTScheduler,
                clientContext, null);
    }
    ...
    public void removeRunningRequest(SendableRequest request, ObjectContainer container) {
        synchronized (starterQueue) {
            if (runningPersistentRequests.remove(request)) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Removed running request " + request + " size now "
    ...
    boolean addToStarterQueue(SendableRequest request, ObjectContainer container) {
        ...
        try {
            chosen = new PersistentChosenRequest(request, request.getPriorityClass(container), container,
        ...
        synchronized (starterQueue) {
            ...
            if (!dumpNew) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Added to running persistent requests, size now " + runningPersistentRequests.size()
                                 + " : " + request);
                }
    ...
    private void fillRequestStarterQueue(ObjectContainer container, ClientContext context) {
        ...
        try {
            if (logMINOR) {
                Logger.minor(this, "Filling request queue... (SSK=" + isSSKScheduler + " insert=" + isInsertScheduler);
            }
            ...
            synchronized (starterQueue) {
                ...
                for (PersistentChosenRequest req : starterQueue) {
                    ...
                    if ((old != null) && (old.request == req.request)) {
                        Logger.error(this,
                                     "DUPLICATE REQUEST ON QUEUE: " + old + " vs " + req + " both " + req.request);
                    }
                    ...
                    if (!ignoreActive) {
                        if (container.ext().isActive(req.request)) {
                            Logger.warning(this,
                                           "REQUEST ALREADY ACTIVATED: " + req.request + " for " + req
                                           + " while checking request queue in filling request queue");
                        } else if (logMINOR) {
                            Logger.minor(this,
                                         "Not already activated for " + req
            ...
            while (true) {
                ...
                synchronized (this) {
                    r = selector.removeFirstInner(fuzz, random, offeredKeys, starter, schedCore, schedTransient, false,
                                                  true, Short.MAX_VALUE, isRTScheduler, context, container, now);
                }
            ...
            for (PersistentChosenRequest old : starterQueue) {
                ...
                if (!ignoreActive) {
                    if (container.ext().isActive(old.request)) {
                        Logger.warning(this,
                                       "REQUEST ALREADY ACTIVATED: " + old.request + " for " + old
                                       + " while checking request queue in maybeAddToStarterQueue for " + req);
                    } else if (logDEBUG) {
                        Logger.debug(this,
                                     "Not already activated for " + old
                                     + " in while checking request queue in maybeAddToStarterQueue for " + req);
                    }
                } else if (logMINOR) {
                    Logger.minor(this,
                                 "Ignoring active because just registered: " + old.request
                                 + " in maybeAddToStarterQueue for " + req);
                }
    ...
    private long moveKeysFromCooldownQueue(CooldownQueue queue, boolean persistent, ObjectContainer container) {
        ...
        Object ret;
        ClientRequestScheduler otherScheduler = ((!isSSKScheduler)
        ...
        if (queue instanceof PersistentCooldownQueue) {
            ret = ((PersistentCooldownQueue) queue).removeKeyBefore(now, WAIT_AFTER_NOTHING_TO_START, container,
                    MAX_KEYS, (PersistentCooldownQueue) otherScheduler.persistentCooldownQueue);
        } else {
    ...
    private void restoreKey(Key key, ObjectContainer container, long now) {
        SendableGet[] reqs = (container == null)
                             ? null
    ...
    public void removeFromAllRequestsByClientRequest(ClientRequester clientRequest, SendableRequest get,
            boolean dontComplain, ObjectContainer container) {
        if (get.persistent()) {
