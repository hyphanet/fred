...
public class USKInserter implements ClientPutState, USKFetcherCallback, PutCompletionCallback {
    ...
    public USKInserter(BaseClientPutter parent, Bucket data, short compressionCodec, FreenetURI uri, InsertContext ctx,
                       PutCompletionCallback cb, boolean isMetadata, int sourceLength, int token, boolean getCHKOnly,
                       boolean addToParent, Object tokenObject, ObjectContainer container, ClientContext context,
                       boolean freeData, boolean persistent, boolean realTimeFlag, int extraInserts,
                       byte cryptoAlgorithm, byte[] forceCryptoKey)
    ...
    public void onFoundEdition(long l, USK key, ObjectContainer container, ClientContext context,
                               boolean lastContentWasMetadata, short codec, byte[] hisData, boolean newKnownGood,
                               boolean newSlotToo) {
        ...
        for (FreenetURI uri : hintURIs) {
            try {
                ...
                SingleBlockInserter sb = new SingleBlockInserter(parent, bucket, (short) -1, uri, ctx, realTimeFlag, m,
                                             false, sourceLength, token, getCHKOnly, true, true /* we don't use it */,
                                             null, container, context, persistent, false, extraInserts,
                ...
                if (!added) {
                    cb.onFailure(new InsertException(InsertException.BUCKET_ERROR, e, pubUSK.getSSK(edition).getURI()),
    ...
    private void scheduleInsert(ObjectContainer container, ClientContext context) {
        ...
        synchronized (this) {
            ...
            sbi = new SingleBlockInserter(parent, data, compressionCodec,
                                          privUSK.getInsertableSSK(edition).getInsertURI(), ctx, realTimeFlag, this,
                                          isMetadata, sourceLength, token, getCHKOnly, false,
                                          true /* we don't use it */, tokenObject, container, context, persistent,
                                          false, extraInserts, cryptoAlgorithm, forceCryptoKey);
        }
    ...
    public void cancel(ObjectContainer container, ClientContext context) {
        ...
        if (tag != null) {
            ...
            if (persist) {
                container.activate(this, 1);    // May have been deactivated by callbacks
            }
        }
