...
public class USKInserter
	...
	private void scheduleFetcher(ClientContext context) {
		synchronized (this) {
			if (logMINOR) Logger.minor(
				this,
				"scheduling fetcher for " + pubUSK.getURI()
			);
			if (finished) return;
			fetcher = context.uskManager.getFetcherForInsertDontSchedule(
	...
	private void insertSucceeded(ClientContext context, long edition) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Inserted to edition " + edition + " - inserting USK date hints..."
		);
		...
		for (FreenetURI uri : hintURIs) {
			try {
				Bucket bucket = BucketTools.makeImmutableBucket(
					context.getBucketFactory(persistent),
					hintData
				);
				SingleBlockInserter sb = new SingleBlockInserter(
					parent,
					bucket,
					(short) -1,
					uri,
				...
				Logger.error(
					this,
					"Unable to insert USK date hints due to disk I/O error: " +
					e,
					e
				);
				if (!added) {
				...
				Logger.error(
					this,
					"Unable to insert USK date hints due to error: " + e,
					e
				);
				if (!added) {
	...
	private void scheduleInsert(ClientContext context) {
		...
		synchronized (this) {
			...
			if (logMINOR) Logger.minor(
				this,
				"scheduling insert for " + pubUSK.getURI() + ' ' + edition
			);
			sbi = new SingleBlockInserter(
				parent,
				data,
				compressionCodec,
	...
	public synchronized void onSuccess(
		...
		if (!targetURI.equals(realURI)) Logger.error(
			this,
			"URI should be " + targetURI + " actually is " + realURI
		);
		else {
			if (logMINOR) Logger.minor(
				this,
	...
	public void onFailure(
		...
		synchronized (this) {
			...
			if (e.getMode() == InsertExceptionMode.COLLISION) {
				...
				if (consecutiveCollisions > MAX_TRIED_SLOTS) scheduleFetcher(
					context
				);
	...
	public void onFailure(ClientContext context) {
		if (logMINOR) Logger.minor(
			this,
			"Fetcher failed to find the given edition or any later edition on " +
			this
		);
		scheduleInsert(context);
	}
	...
	public void onTransition(
		...
		Logger.error(
			this,
			"Got onTransition(" + oldState + ',' + newState + ')'
		);
	}
