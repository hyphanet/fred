...
class USKChecker extends BaseSingleFileFetcher {
	...
	USKChecker(
		...
		if (logMINOR) Logger.minor(
			USKChecker.class,
			"Created USKChecker for " + key + " : " + this
		);
	}
	...
	public void onFailure(
		...
		switch (e.code) {
			...
			default:
				Logger.error(
					this,
					"Unknown low-level fetch error code: " + e.code,
					new Exception("error")
				);
				canRetry = true;
		}
	...
	@Override
	protected void onBlockDecodeError(
		SendableRequestItem token,
		ClientContext context
	) {
		onFailure(
			new LowLevelGetException(LowLevelGetException.DECODE_FAILED),
			token,
			context
		);
	}
