...
public class SimpleHealingQueue
	...
	static final RequestClient REQUEST_CLIENT = new RequestClientBuilder()
		.build();
	...
	public boolean innerQueue(
		...
		synchronized (this) {
			...
			try {
				...
				Logger.error(
					this,
					"Caught trying to insert healing block: " + e,
					e
				);
				return false;
			}
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Started healing insert " + ctr + " for " + data
			);
			return true;
		} catch (Throwable e) {
			Logger.error(
	...
	public void onTransition(
		...
		Logger.error(
			this,
			"impossible: onTransition on SimpleHealingQueue from " +
			oldState +
			" to " +
			newState,
			new Exception("debug")
	...
	public void onMetadata(
		...
		Logger.error(
			this,
			"onMetadata() in SimpleHealingQueue - impossible",
			new Exception("error")
		);
		meta.free();
	}
