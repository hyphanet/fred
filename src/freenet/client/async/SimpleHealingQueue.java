...
public class SimpleHealingQueue extends BaseClientPutter implements HealingQueue, PutCompletionCallback {
    ...
    public boolean innerQueue(Bucket data, byte[] cryptoKey, byte cryptoAlgorithm, ClientContext context) {
        ...
        synchronized (this) {
            ...
            try {
                sbi = new SingleBlockInserter(this, data, (short) -1, FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag,
                                              this, false, CHKBlock.DATA_LENGTH, ctr, false, false, false, data, null,
                                              context, false, true, 0, cryptoAlgorithm, cryptoKey);
            } catch (Throwable e) {
    ...
    public void onSuccess(ClientPutState state, ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Successfully inserted healing block: " + sbi.getURINoEncode() + " for " + data + " ("
                         + sbi.token + ')');
        }
    ...
    public void onFailure(InsertException e, ClientPutState state, ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Failed to insert healing block: " + sbi.getURINoEncode() + " : " + e + " for " + data + " ("
                         + sbi.token + ')', e);
        }
