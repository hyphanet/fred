...
public class DatastoreChecker implements PrioRunnable {
    ...
    public void loadPersistentRequests(ObjectContainer container, final ClientContext context) {
        ...
        for (short p = RequestStarter.MAXIMUM_PRIORITY_CLASS; p <= RequestStarter.MINIMUM_PRIORITY_CLASS; p++) {
            ...
            query.descend("nodeDBHandle").constrain(context.nodeDBHandle).and(query.descend("prio").constrain(prio));

            @SuppressWarnings("unchecked") ObjectSet<DatastoreCheckerItem> results = query.execute();

            for (DatastoreCheckerItem item : results) {
                if (item.chosenBy == context.bootID) {
                    continue;
                }
                ...
                if ((getter == null) ||!container.ext().isStored(getter)) {
                    if (logMINOR) {
                        Logger.minor(
                            this,
                            "Ignoring DatastoreCheckerItem because the SendableGet has already been deleted from the database");
                    }
                ...
                try {
                    ...
                    synchronized (this) {
                        if (persistentQueue[prio].contains(persist)) {
                            continue;
                        }
                    ...
                    Logger.error(this,
                                 "NPE for getter in DatastoreChecker: " + e
                                 + " - probably leftover data from an incomplete deletion", e);
    ...
    public void queueTransientRequest(SendableGet getter, BlockSet blocks) {
        ...
        synchronized (this) {
            ...
            TransientItem queueItem = new TransientItem(finalKeysToCheck.toArray(new Key[finalKeysToCheck.size()]),
                                          getter, blocks);
    ...
    public void queuePersistentRequest(SendableGet getter, BlockSet blocks, ObjectContainer container,
        ...
        synchronized (this) {
            ...
            if (queueSize > MAX_PERSISTENT_KEYS) {
                return;
            }
            ...
            PersistentItem queueItem = new PersistentItem(finalKeysToCheck.toArray(new Key[finalKeysToCheck.size()]),
            ...
            while (true) {
                for (short prio = 0; prio < transientQueue.length; prio++) {
                    ...
                    if ((trans = transientQueue[prio].pollFirst()) != null) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Checking transient request " + getter + " prio " + prio + " of "
                                         + transientQueue[prio].size());
                        }
                ...
                if (waited && notPersistent) {
                    return;    // Re-check queueSize after a failed wait.
                }
...
//          }
                        ...
                        try {
                            ...
                            try {
                                get.onFailure(new LowLevelGetException(LowLevelGetException.INTERNAL_ERROR,
                                        "Internal error: " + t, t), null, container, context);
                            } catch (Throwable t1) {
    ...
    @SuppressWarnings("unchecked")
    public void removeRequest(SendableGet request, boolean persistent, ObjectContainer container,
                              ClientContext context, short prio) {
        if (logMINOR) {
        ...
        if (!persistent) {
            synchronized (this) {
                if (!transientQueue[prio].remove(requestMatcher)) {
                    return;
                }
            }
            ...
            for (DatastoreCheckerItem item : results) {
                if (item.nodeDBHandle != context.nodeDBHandle) {
                    continue;
                }
    ...
    private static final class PersistentItem extends QueueItem {
        ...
        PersistentItem(Key[] keys, SendableGet getter, ClientRequestScheduler scheduler,
                       DatastoreCheckerItem checkerItem, BlockSet blockSet) {
            super(getter);
            this.keys = keys;
