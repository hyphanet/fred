...
public class SplitFileInserter implements ClientPutState {
    ...
    public SplitFileInserter(BaseClientPutter put, PutCompletionCallback cb, Bucket data, COMPRESSOR_TYPE bestCodec,
                             long decompressedLength, ClientMetadata clientMetadata, InsertContext ctx,
                             boolean getCHKOnly, boolean isMetadata, Object token, ARCHIVE_TYPE archiveType,
                             boolean freeData, boolean persistent, boolean realTimeFlag, ObjectContainer container,
                             ClientContext context, HashResult[] hashes, byte[] hashThisLayerOnly, long origTopSize,
                             long origTopCompressedSize, byte cryptoAlgorithm, byte[] splitfileKey)
        ...
        try {
            dataBuckets = BucketTools.split(data, CHKBlock.DATA_LENGTH,
                                            persistent ? context.persistentBucketFactory : context.tempBucketFactory,
                                            freeData, persistent, container);

            if (dataBuckets[dataBuckets.length - 1].size() < CHKBlock.DATA_LENGTH) {
                Bucket oldData = dataBuckets[dataBuckets.length - 1];

                dataBuckets[dataBuckets.length - 1] = BucketTools.pad(oldData, CHKBlock.DATA_LENGTH,
                        context.getBucketFactory(persistent), (int) oldData.size());
        ...
        segments = splitIntoSegments(segmentSize, crossCheckBlocks, segs, deductBlocksFromSegments, dataBuckets,
                                     context.mainExecutor, container, context, persistent, put, cryptoAlgorithm,
                                     splitfileCryptoKey);
        ...
        if (crossCheckBlocks != 0) {
            ...
            for (int i = 0; i < crossSegments.length; i++) {
                ...
                SplitFileInserterCrossSegment seg = new SplitFileInserterCrossSegment(persistent, segLen,
                                                        crossCheckBlocks, put, splitfileAlgorithm, this, i);
     ...
     */
    private SplitFileInserterSegment[] splitIntoSegments(int segmentSize, int crossCheckBlocks, int segCount,
            int deductBlocksFromSegments, Bucket[] origDataBlocks, Executor executor, ObjectContainer container,
            ClientContext context, boolean persistent, BaseClientPutter putter, byte cryptoAlgorithm,
        ...
        if (segCount == 1) {
            ...
            // Single segment
            SplitFileInserterSegment onlySeg = new SplitFileInserterSegment(this, persistent, realTimeFlag, putter,
                                                   splitfileAlgorithm, crossCheckBlocks,
                                                   FECCodec.getCheckBlocks(splitfileAlgorithm,
                                                       origDataBlocks.length + crossCheckBlocks,
                                                       cmode), origDataBlocks, ctx, getCHKOnly, 0, cryptoAlgorithm,
                                                           splitfileCryptoKey, container);
            ...
            for (int i = segmentSize; ; ) {
                ...
                for (int x = 0; x < seg.length; x++) {
                    if (seg[x] == null) {
                        throw new NullPointerException("In splitIntoSegs: " + x + " is null of " + seg.length + " of "
                                                       + segNo);
                    }
                }

                SplitFileInserterSegment s = new SplitFileInserterSegment(this, persistent, realTimeFlag, putter,
                                                 splitfileAlgorithm, crossCheckBlocks, check, seg, ctx, getCHKOnly,
                                                 segNo, cryptoAlgorithm, splitfileCryptoKey, container);
                ...
                if (deductBlocksFromSegments != 0) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "INSERTING: Segment " + segNo + " of " + segCount + " : " + data + " data blocks "
                                     + check + " check blocks");
                    }
                }
    ...
    private void encodeMetadata(ObjectContainer container, ClientContext context,
                                SplitFileInserterSegment dontDeactivateSegment) {
        context.jobRunner.setCommitThisTransaction();
        ...
        synchronized (this) {
            ...
            if (!missingURIs) {
                ...
                if (persistent) {
                    h = HashResult.copy(hashes);
                } else {
                    h = hashes;
                }

                if (persistent) {
                    container.activate(compressionCodec, Integer.MAX_VALUE);
                }

                if (persistent) {
                    container.activate(archiveType, Integer.MAX_VALUE);
                }

                m = new Metadata(splitfileAlgorithm, dataURIs, checkURIs, segmentSize, checkSegmentSize,
                                 deductBlocksFromSegments, meta, dataLength, archiveType, compressionCodec,
                                 decompressedLength, isMetadata, h, hashThisLayerOnly, data, compressed, req, total,
                                 topDontCompress, topCompatibilityMode, splitfileCryptoAlgorithm, splitfileCryptoKey,
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < segments.length; i++) {
                ...
                if (!segments[i].isFinished()) {
                    if (logMINOR) {
                        Logger.minor(this, "Segment not finished: " + i + ": " + segments[i] + " for " + this);
                    }
