...
public class SplitFileInserter
	...
	SplitFileInserter(
		...
		try {
			storage = new SplitFileInserterStorage(
				originalData,
				decompressedLength,
				this,
				compressionCodec,
			...
			parent.addRedundantBlocksInsert(
				storage.topTotalBlocks - topTotalBlocks - mustSucceed
			);
	...
	public void onResume(ClientContext context)
		...
		try {
			...
			this.storage = new SplitFileInserterStorage(
				raf,
				originalData,
				this,
				context.fastWeakRandom,
				...
				new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						if (logMINOR) Logger.minor(
							this,
							"Succeeding on " + SplitFileInserter.this
						);
						unregisterSender();
						if (!(ctx.earlyEncode || ctx.getCHKOnly)) {
							reportMetadata(metadata);
