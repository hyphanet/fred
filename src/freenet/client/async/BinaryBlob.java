...
public abstract class BinaryBlob {
	...
	public static void writeBinaryBlobHeader(DataOutputStream binaryBlobStream)
		throws IOException {
	...
	public static void writeKey(
		DataOutputStream binaryBlobStream,
		KeyBlock block,
		Key key
	) throws IOException {
	...
	public static void writeEndBlob(DataOutputStream binaryBlobStream)
		...
		writeBlobHeader(
			binaryBlobStream,
			BinaryBlob.BLOB_END,
			BinaryBlob.BLOB_END_VERSION,
			0
		);
	}

	public static void readBinaryBlob(
		DataInputStream dis,
		BlockSet blocks,
		boolean tolerant
		...
		if (
			magic != BinaryBlob.BINARY_BLOB_MAGIC
		) throw new BinaryBlobFormatException("Bad magic");
		short version = dis.readShort();
		if (
			version != BinaryBlob.BINARY_BLOB_OVERALL_VERSION
		...
		while (true) {
			...
			if (blobType == BinaryBlob.BLOB_END) {
				...
				if (blobLength != total) throw new BinaryBlobFormatException(
					"Binary blob not same length as data: blobLength=" +
					blobLength +
					" total=" +
					total
				);
