...
class SingleFileInserter implements ClientPutState, Serializable {
	...
	void onCompressedInner(CompressionOutput output, ClientContext context)
		...
		if (hashes != null) {
			...
			ctx.eventProducer.produceEvent(
				new ExpectedHashesEvent(clientHashes),
				context
			);
		...
		if (
			(fitsInOneBlockAsIs || fitsInOneCHK) && origSize > Integer.MAX_VALUE
		) throw new InsertException(
			InsertExceptionMode.INTERNAL_ERROR,
			"2GB+ should not encode to one block!",
			null
		);

		boolean noMetadata =
			((block.clientMetadata == null) ||
				block.clientMetadata.isTrivial()) &&
			targetFilename == null;
		if ((noMetadata || metadata) && archiveType == null) {
			if (fitsInOneBlockAsIs) {
				if (persistent && (data instanceof NotPersistentBucket)) data =
					fixNotPersistent(data, context);
				// Just insert it
				ClientPutState bi = createInserter(
					parent,
					data,
					codecNumber,
					ctx,
				...
				if (logMINOR) Logger.minor(
					this,
					"Inserting without metadata: " + bi + " for " + this
				);
				cb.onTransition(this, bi, context);
				if (
					ctx.earlyEncode &&
					bi instanceof SingleBlockInserter &&
					isCHK
		...
		if (fitsInOneCHK) {
			...
			if (reportMetadataOnly) {
				SingleBlockInserter dataPutter = new SingleBlockInserter(
					parent,
					data,
					codecNumber,
				...
				if (logMINOR) Logger.minor(
					this,
					"Inserting with metadata: " + dataPutter + " for " + this
				);
				Metadata meta = makeMetadata(
					archiveType,
					dataPutter.getURI(context),
					hashes
				);
				...
				MultiPutCompletionCallback mcb = new MultiPutCompletionCallback(
					cb,
					parent,
					token,
					persistent,
				...
				if (logMINOR) Logger.minor(
					this,
					"Inserting data: " + dataPutter + " for " + this
				);
				Metadata meta = makeMetadata(
					archiveType,
					dataPutter.getURI(context),
				...
				try {
					...
					throw new InsertException(
						InsertExceptionMode.INTERNAL_ERROR,
						"Got MetadataUnresolvedException in SingleFileInserter: " +
						e.toString(),
						null
					);
				}
				ClientPutState metaPutter = createInserter(
					parent,
					metadataBucket,
					(short) -1,
					ctx,
				...
				if (logMINOR) Logger.minor(
					this,
					"Inserting metadata: " + metaPutter + " for " + this
				);
				...
				Logger.minor(
					this,
					"" + mcb + " : data " + dataPutter + " meta " + metaPutter
				);
				mcb.arm(context);
				dataPutter.schedule(context);
				if (
					ctx.earlyEncode && metaPutter instanceof SingleBlockInserter
		...
		if (reportMetadataOnly) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Inserting as splitfile: " + sfi + " for " + this
			);
			...
			if (logMINOR) Logger.minor(
				this,
				"Inserting as splitfile: " + sfi + " for " + sh + " for " + this
			);
	...
	private RandomAccessBucket fixNotPersistent(
		...
		try {
			...
			Logger.error(
				this,
				"Caught " + e + " while copying non-persistent data",
				e
			);
			throw new InsertException(
				InsertExceptionMode.BUCKET_ERROR,
	...
	private void tryCompress(ClientContext context) throws InsertException {
		...
		boolean tryCompress =
			(origSize > blockSize) && (!ctx.dontCompress) && (!dontCompress);
		if (tryCompress) {
			...
			if (wantHashes != 0) {
				...
				MultiHashOutputStream hasher = new MultiHashOutputStream(
					nos,
					wantHashes
				);
				try {
			...
			final CompressionOutput output = new CompressionOutput(
				data,
				null,
				hashes
			);
	...
	private Metadata makeMetadata(
		...
		if (archiveType != null) meta = new Metadata(
		...
		else meta = new Metadata( // redirect
	...
	private ClientPutState createInserter(
		...
		if (uri.getKeyType().equals("USK")) {
			...
			SingleBlockInserter sbi = new SingleBlockInserter(
				parent,
				data,
				compressionCodec,
				uri,
	...
	public class SplitHandler
		...
		public SplitHandler(
			...
			this.origCompressedDataLength = allowSizes
				? origCompressedDataLength
				: 0;
		}
		...
		public void onMetadata(
			...
			if (logMINOR) Logger.minor(
				this,
				"Got metadata for " + this + " from " + state
			);
			synchronized (this) {
				if (finished) return;
				if (reportMetadataOnly) {
					if (state != sfi) {
						Logger.error(
							this,
							"Got metadata from unknown object " +
							state +
			...
			try {
				...
				fail(
					(InsertException) new InsertException(
						InsertExceptionMode.INTERNAL_ERROR,
						"MetadataUnresolvedException in SingleFileInserter.SplitHandler: " +
						e1,
						null
					).initCause(e1),
					context
			...
			RandomAccessBucket metadataBucket;
			try {
				metadataBucket = BucketTools.makeImmutableBucket(
					context.getBucketFactory(persistent),
					metaBytes
				);
			...
			InsertBlock newBlock = new InsertBlock(
				metadataBucket,
				m,
				block.desiredURI
			);
			synchronized (this) {
				...
				metadataPutter = new SingleFileInserter(
					parent,
					this,
					newBlock,
					true,
		...
		public void onBlockSetFinished(
			...
			synchronized (this) {
				if (state == sfi) splitInsertSetBlocks = true;
					...
					this,
					"Unrecognised: " + state + " in onBlockSetFinished()"
				);
			...
			synchronized (this) {
				...
				if (meta) {
					if (!metaInsertStarted) {
						Logger.error(
							this,
							"Metadata insert not started yet got onFetchable for it: " +
							state +
							" on " +
							this
						);
					...
					if (state != sfi) {
						Logger.error(
							this,
							"onFetchable for unknown state " + state
						);
						return;
					}
		...
		public void onMetadata(
			...
			if (logMINOR) Logger.minor(
				this,
				"Got metadata bucket for " + this + " from " + state
			);
		...
		public void onResume(ClientContext context)
			...
			if (metadataPutter != null) {
				if (
					ctx.earlyEncode || sfi == null || metaInsertStarted
		...
		public void onShutdown(ClientContext context) {
			...
			if (splitfileInserter != null) splitfileInserter.onShutdown(
				context
			);
	...
	public void onStartCompression(
		...
		if (parent == cb) {
			...
			ctx.eventProducer.produceEvent(
				new StartedCompressionEvent(ctype),
				context
			);
		}
	}
	...
	public final void onResume(ClientContext context)
		...
		if (block != null && block.getData() != null) block
			.getData()
			.onResume(context);
