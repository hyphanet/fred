...
class SingleFileInserter implements ClientPutState {
    ...
    SingleFileInserter(BaseClientPutter parent, PutCompletionCallback cb, InsertBlock block, boolean metadata,
                       InsertContext ctx, boolean realTimeFlag, boolean dontCompress, boolean getCHKOnly,
                       boolean reportMetadataOnly, Object token, ARCHIVE_TYPE archiveType, boolean freeData,
                       String targetFilename, boolean earlyEncode, boolean forSplitfile, boolean persistent,
                       long origDataLength, long origCompressedDataLength, HashResult[] origHashes,
                       byte cryptoAlgorithm, byte[] forceCryptoKey, long metadataThreshold) {
    ...
    void onCompressedInner(CompressionOutput output, ObjectContainer container, ClientContext context)
            throws InsertException {
        ...
        if ((noMetadata || metadata) && (archiveType == null)) {
            if (fitsInOneBlockAsIs) {
                ...
                ClientPutState bi = createInserter(parent, data, codecNumber, ctx, cb, metadata, (int) origSize, -1,
                                                   getCHKOnly, true, container, context, shouldFreeData, forSplitfile);
        ...
        if (fitsInOneCHK) {
            ...
            if (reportMetadataOnly) {
                ...
                SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
                                                     persistent
                                                     ? FreenetURI.EMPTY_CHK_URI.clone()
                                                     : FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, cb, metadata,
                                                         (int) origSize, -1, getCHKOnly, true, true, token, container,
                                                         context, persistent, shouldFreeData,
                                                         forSplitfile
                                                         ? ctx.extraInsertsSplitfileHeaderBlock
                ...
                SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
                                                     persistent
                                                     ? FreenetURI.EMPTY_CHK_URI.clone()
                                                     : FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, mcb, metadata,
                                                         (int) origSize, -1, getCHKOnly, true, false, token, container,
                                                         context, persistent, shouldFreeData,
                                                         forSplitfile
                                                         ? ctx.extraInsertsSplitfileHeaderBlock
                                                         : ctx.extraInsertsSingleBlock, cryptoAlgorithm,
                ...
                try {
                    metadataBucket = BucketTools.makeImmutableBucket(context.getBucketFactory(persistent),
                    ...
                    throw new InsertException(InsertException.INTERNAL_ERROR,
                ...
                ClientPutState metaPutter = createInserter(parent, metadataBucket, (short) -1, ctx, mcb, true,
                                                (int) origSize, -1, getCHKOnly, true, container, context, true, false);
        ...
        if (reportMetadataOnly) {
            SplitFileInserter sfi = new SplitFileInserter(parent, cb, data, bestCodec, origSize, block.clientMetadata,
                                        ctx, getCHKOnly, metadata, token, archiveType, shouldFreeData, persistent,
                                        realTimeFlag, container, context, hashes, hashThisLayerOnly, origDataLength,
                                        origCompressedDataLength, cryptoAlgorithm, forceCryptoKey);
            ...
            SplitFileInserter sfi = new SplitFileInserter(parent, sh, data, bestCodec, origSize, block.clientMetadata,
                                        ctx, getCHKOnly, metadata, token, archiveType, shouldFreeData, persistent,
                                        realTimeFlag, container, context, HashResult.copy(hashes), hashThisLayerOnly,
                                        origDataLength, origCompressedDataLength, cryptoAlgorithm, forceCryptoKey);
    ...
    private void tryCompress(ObjectContainer container, ClientContext context) throws InsertException {
        ...
        if (tryCompress) {
            InsertCompressor.start(container, context, this, origData, oneBlockCompressedSize,
                                   context.getBucketFactory(persistent), persistent, wantHashes, !atLeast1254);
        } else {
            if (logMINOR) {
                Logger.minor(this,
    ...
    private Metadata makeMetadata(ARCHIVE_TYPE archiveType, FreenetURI uri, HashResult[] hashes,
        ...
        if (archiveType != null) {
            meta = new Metadata(Metadata.ARCHIVE_MANIFEST, archiveType, null, uri, block.clientMetadata, data,
                                compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
        } else {    // redirect
            meta = new Metadata(Metadata.SIMPLE_REDIRECT, archiveType, null, uri, block.clientMetadata, data,
                                compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
        }
    ...
    private ClientPutState createInserter(BaseClientPutter parent, Bucket data, short compressionCodec,
            ...
            boolean getCHKOnly, boolean addToParent, ObjectContainer container, ClientContext context,
            boolean freeData, boolean forSplitfile)
            throws InsertException {
        ...
        if (uri.getKeyType().equals("USK")) {
            try {
                return new USKInserter(
                    parent, data, compressionCodec, uri, ctx, cb, isMetadata, sourceLength, token, getCHKOnly,
                    addToParent, this.token, container, context, freeData, persistent, realTimeFlag,
            ...
            SingleBlockInserter sbi = new SingleBlockInserter(parent, data, compressionCodec, uri, ctx, realTimeFlag,
                                          cb, isMetadata, sourceLength, token, getCHKOnly, addToParent, false,
                                          this.token, container, context, persistent, freeData,
                                          forSplitfile
                                          ? ctx.extraInsertsSplitfileHeaderBlock
                                          : ctx.extraInsertsSingleBlock, cryptoAlgorithm, forceCryptoKey);
    ...
    public class SplitHandler implements PutCompletionCallback, ClientPutState {
        ...
        public void onFailure(InsertException e, ClientPutState state, ObjectContainer container,
            ...
            synchronized (this) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "onFailure(): " + e + " on " + state + " on " + this + " sfi = " + sfi
        ...
        public void onMetadata(Metadata meta, ClientPutState state, ObjectContainer container, ClientContext context) {
            ...
            synchronized (this) {
                ...
                if (reportMetadataOnly) {
                    if (state != sfi) {
                        Logger.error(this,
                                     "Got metadata from unknown object " + state
                                     + " when expecting to report metadata");
                    ...
                    e = new InsertException(InsertException.INTERNAL_ERROR,
                    ...
                    Logger.error(this, "Got metadata from unknown state " + state + " sfi=" + sfi + " metadataPutter="
                                 + metadataPutter + " on " + this + " persistent="
            ...
            try {
                ...
                fail((InsertException) new InsertException(InsertException.INTERNAL_ERROR,
                        "MetadataUnresolvedException in SingleFileInserter.SplitHandler: " + e1,
                        null).initCause(e1), container, context);
            ...
            if (targetFilename != null) {
                if (metaBytes.length <= Short.MAX_VALUE) {
                    ...
                    try {
                        ...
                        fail((InsertException) new InsertException(InsertException.INTERNAL_ERROR,
                                "MetadataUnresolvedException in SingleFileInserter.SplitHandler(2): " + e1,
                                null).initCause(e1), container, context);
            ...
            synchronized (this) {
                ...
                metadataPutter = new SingleFileInserter(parent, this, newBlock, true, ctx, realTimeFlag, false,
                        getCHKOnly, false, token, archiveType, true, metaPutterTargetFilename, earlyEncode, true,
                        persistent, origDataLength, origCompressedDataLength, origHashes, cryptoAlgorithm,
                        forceCryptoKey, metadataThreshold);
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Created metadata putter for " + this + " : " + metadataPutter + " bucket "
                ...
                if (!(earlyEncode || splitInsertSuccess)) {
                    return;
                }
            }
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Putting metadata on " + metadataPutter + " from " + sfi + " ("
            ...
            synchronized (this) {
                ...
                if (meta) {
                    if (!metaInsertStarted) {
                        Logger.error(this,
                                     "Metadata insert not started yet got onFetchable for it: " + state + " on "
                                     + this);
                    }
