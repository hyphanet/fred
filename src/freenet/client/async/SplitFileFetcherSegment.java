...
public class SplitFileFetcherSegment implements FECCallback, HasCooldownTrackerItem {
    ...
    public SplitFileFetcherSegment(short splitfileType, SplitFileSegmentKeys keys, SplitFileFetcher fetcher,
                                   ArchiveContext archiveContext, FetchContext blockFetchContext, long maxTempLength,
                                   int recursionLevel, ClientRequester requester, int segNum,
                                   boolean ignoreLastDataBlock, boolean pre1254, int crossCheckBlocks,
                                   byte cryptoAlgorithm, byte[] forceCryptoKey, int maxRetries, boolean realTimeFlag)
            throws MetadataParseException, FetchException {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Created " + this + " for " + parentFetcher + " : " + dataBuckets.length + " data blocks "
                         + checkBuckets.length + " check blocks");
        }
    ...
    public long writeDecodedDataTo(OutputStream os, long truncateLength, ObjectContainer container) throws IOException {
        ...
        if (decodedData != null) {
            if (persistent) {
                container.activate(decodedData, Integer.MAX_VALUE);
            }
            ...
            for (int i = 0; i < dataBuckets.length - crossCheckBlocks; i++) {
                ...
                if (data == null) {
                    throw new NullPointerException("Data bucket " + i + " of " + dataBuckets.length
                                                   + " is null in writeDecodedData on " + this + " status = " + status
                                                   + " number " + status.getNumber() + " data " + status.getData()
                                                   + " persistence = " + persistent
                                                   + (persistent
                                                      ? (" (block active = " + container.ext().isActive(status)
                                                          + " block ID = " + container.ext().getID(status)
    ...
    private short onSuccessInner(Bucket data, int blockNo, ObjectContainer container, ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (startedDecode) {
                ...
                if (tooSmall && ((!ignoreLastDataBlock) || (blockNo != dataBuckets.length - 1))) {
                    fail(new FetchException(FetchException.INVALID_METADATA,
                                            "Block too small in splitfile: block " + blockNo + " of "
                                            + dataBuckets.length + " data keys, " + checkBuckets.length
                                            + " check keys"), container, context, true);
                ...
                if (logMINOR) {
                    Logger.minor(this, "Fetched " + fetchedBlocks + " blocks in onSuccess(" + blockNo + ")");
                }
                ...
                if (!decodeNow) {
                    ...
                    allFailed = failedBlocks + fatallyFailedBlocks
                                > (dataBuckets.length + checkBuckets.length - minFetched);
                }
            }
            ...
            if (dontNotify) {
                res |= ON_SUCCESS_DONT_NOTIFY;
            }

            if (allFailed) {
                res |= ON_SUCCESS_ALL_FAILED;
            }

            if (decodeNow) {
                res |= ON_SUCCESS_DECODE_NOW;
            }
        }
        ...
        if (crossSegment != null) {
            ...
            if (persistent) {
                ...
                if (!active) {
                    container.activate(crossSegment, 1);
                }
            }
            ...
            if (!active) {
                container.deactivate(crossSegment, 1);
            }
        }
    ...
    private synchronized boolean checkAndDecodeNow(ObjectContainer container, int blockNo, boolean tooSmall,
            boolean haveDataBlocks) {
        if (startedDecode) {
            return false;    // Caller checks anyway but worth checking.
        }
...
//                              lastBlockTruncated = false;
        ...
        for (int i = 0; i < checkBuckets.length; i++) {
            ...
            if (!active) {
                container.deactivate(checkBuckets[i], 1);
            }
        }

        if (count < dataBuckets.length) {
            Logger.error(this,
                         "Attempting to decode but only " + count + " of " + dataBuckets.length + " blocks available!",
                         new Exception("error"));
            decodeNow = false;
     ...
     * @return
     */
    public boolean onSuccess(Bucket data, int blockNo, ClientCHKBlock block, ObjectContainer container,
                             ClientContext context, SplitFileFetcherSubSegment sub) {
        if (persistent) {
...
//      }
        if (logMINOR) {
            Logger.minor(this,
                         "Fetched block " + blockNo + " in " + this + " data=" + dataBuckets.length + " check="
                         + checkBuckets.length);
        }

        try {
            if (!maybeAddToBinaryBlob(data, block, blockNo, container, context,
                                      (block == null) ? "CROSS-SEGMENT FEC" : "UNKNOWN")) {
                if ((ignoreLastDataBlock && (blockNo == dataBuckets.length - 1))
                        || (ignoreLastDataBlock && (fetchedDataBlocks == dataBuckets.length))) {

                    // Ignore
                } else if (block == null) {

                    // Cross-segment, just return false.
                    Logger.error(this, "CROSS-SEGMENT DECODED/ENCODED BLOCK INVALID: " + blockNo,
                                 new Exception("error"));
                    onFatalFailure(new FetchException(FetchException.INTERNAL_ERROR,
                                                      "Invalid block from cross-segment decode"), blockNo, container,
                                                          context);
                    data.free();
                    ...
                    onFatalFailure(new FetchException(FetchException.INTERNAL_ERROR, "Invalid block"), blockNo,
                                   container, context);
                    data.free();
    ...
    private void finishOnSuccess(short result, ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "finishOnSuccess: result = " + result + " dontNotify=" + dontNotify + " allFailed="
                         + allFailed + " decodeNow=" + decodeNow);
        }
    ...
    public void decode(ObjectContainer container, ClientContext context) {
        ...
        // Now decode
        if (logMINOR) {
            Logger.minor(this, "Decoding " + SplitFileFetcherSegment.this);
        }
        ...
        if (oldGetter != null) {
            if (persistent) {
                container.activate(oldGetter, 1);
            }
        ...
        if (splitfileType != Metadata.SPLITFILE_NONREDUNDANT) {
            ...
            if (count < dataBuckets.length) {
                Logger.error(this,
                             "Attempting to decode but only " + count + " of " + dataBuckets.length
                             + " blocks available!", new Exception("error"));

                // startedDecode and finishing are already set, so we can't recover.
                fail(new FetchException(FetchException.INTERNAL_ERROR,
                                        "Not enough blocks to decode but decoding anyway?!"), container, context, true);
            ...
            if (lastBlock != null) {
                ...
                if (ignoreLastDataBlock && (lastBlock.size() < CHKBlock.DATA_LENGTH)) {
                    ...
                    fail(new FetchException(FetchException.INVALID_METADATA,
                                            "Last data block is not the standard size"), container, context, true);
                }
            }
            ...
            FECJob job = new FECJob(codec, queue, dataBuckets, checkBuckets, CHKBlock.DATA_LENGTH,
                                    context.getBucketFactory(persistent), this, true, parent.getPriorityClass(),
                                    persistent);

            codec.addToQueue(job, queue, container);
    ...
    @Override
    public void onDecodedSegment(ObjectContainer container, ClientContext context, FECJob job, Bucket[] dataBuckets2,
                                 Bucket[] checkBuckets2, SplitfileBlock[] dataBlockStatus,
                                 SplitfileBlock[] checkBlockStatus) {
        if (persistent) {
            container.activate(parent, 1);
        ...
        if (persistent) {
            for (int i = 0; i < dataBuckets.length; i++) {
                ...
                if (dataBlockStatus[i] != dataBuckets[i]) {
                    ...
                    if (theirID == ourID) {
                        Logger.error(this,
                                     "DB4O BUG DETECTED IN DECODED SEGMENT!: our block: " + dataBuckets[i]
                                     + " block from decode " + dataBlockStatus[i] + " both have ID " + ourID + " = "
                                     + theirID);
        ...
        if (crossCheckBlocks != 0) {
            for (int i = 0; i < dataBuckets.length; i++) {
                if (persistent) {
                    container.activate(dataBuckets[i], 1);    // onFetched might deactivate blocks.
                }
                ...
                if (dataBuckets[i].flag) {
                    ...
                    if (persistent) {
                        ...
                        if (!active) {
                            container.activate(crossSegmentsByBlock[i], 1);
                        }
                    }
                    ...
                    if (!active) {
                        container.deactivate(crossSegmentsByBlock[i], 1);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < checkBuckets.length; i++) {
            if (checkBuckets[i].getData() == null) {
                ...
                if (persistent) {
                    checkBuckets[i].storeTo(container);
                }
            }
        }
        ...
        try {
            ...
            codec.addToQueue(new FECJob(codec, context.fecQueue, dataBuckets, checkBuckets, 32768,
                                        context.getBucketFactory(persistent), this, false, parent.getPriorityClass(),
                                        persistent), context.fecQueue, container);
    ...
    @Override
    public void onEncodedSegment(ObjectContainer container, ClientContext context, FECJob job, Bucket[] dataBuckets2,
                                 Bucket[] checkBuckets2, SplitfileBlock[] dataBlockStatus,
                                 SplitfileBlock[] checkBlockStatus) {
        try {
            if (persistent) {
            ...
            synchronized (this) {
                ...
                for (int i = 0; i < dataBuckets.length; i++) {
                    ...
                    if (dataBuckets[i] != dataBlockStatus[i]) {
                        Logger.error(this,
                                     "Data block " + i + " : ours is " + dataBuckets[i] + " codec's is "
                                     + dataBlockStatus[i]);

                        if (persistent) {
                            if (container.ext().getID(dataBuckets[i]) == container.ext().getID(dataBlockStatus[i])) {
                                Logger.error(this,
                                             "DB4O BUG DETECTED: SAME UID FOR TWO OBJECTS: " + dataBuckets[i] + "="
                                             + container.ext().getID(dataBuckets[i]) + " and " + dataBlockStatus[i]
                                             + "=" + container.ext().getID(dataBlockStatus[i])
                                             + " ... attempting workaround ...");
                            }

                            Logger.error(this,
                                         "Ours is " + (container.ext().isStored(dataBuckets[i]) ? "stored " : "")
                                         + (container.ext().isActive(dataBuckets[i]) ? "active " : "") + " UUID "
                                         + container.ext().getID(dataBuckets[i]));
                            Logger.error(this,
                                         "Theirs is " + (container.ext().isStored(dataBlockStatus[i]) ? "stored " : "")
                    ...
                    if (data == null) {
                        Logger.error(this,
                                     "Data bucket " + i + " has null contents in onEncodedSegment on " + this
                                     + " for block " + dataBuckets[i]);
                ...
                for (int i = 0; i < checkBuckets.length; i++) {
                    ...
                    if (checkBuckets[i] != checkBlockStatus[i]) {
                        Logger.error(this,
                                     "Check block " + i + " : ours is " + checkBuckets[i] + " codec's is "
                                     + checkBlockStatus[i]);

                        if (persistent) {
                            if (container.ext().getID(checkBuckets[i]) == container.ext().getID(checkBlockStatus[i])) {
                                Logger.error(this,
                                             "DB4O BUG DETECTED: SAME UID FOR TWO OBJECTS: " + checkBuckets[i] + "="
                                             + container.ext().getID(checkBuckets[i]) + " and " + checkBlockStatus[i]
                                             + "=" + container.ext().getID(checkBlockStatus[i])
                                             + " ... attempting workaround ...");
                            }

                            Logger.error(this,
                                         "Ours is " + (container.ext().isStored(checkBuckets[i]) ? "stored " : "")
                    ...
                    if (data == null) {
                        Logger.error(this,
                                     "Check bucket " + i + " has null contents in onEncodedSegment on " + this
                                     + " for block " + checkBuckets[i]);
                    ...
                    try {
                        if (!maybeAddToBinaryBlob(data, null, i + dataBuckets.length, container, context,
                                                  "FEC ENCODE")) {
                            heal = false;
    ...
    private boolean maybeAddToBinaryBlob(Bucket data, ClientCHKBlock block, int blockNo, ObjectContainer container,
            ClientContext context, String dataSource)
        ...
        if (FORCE_CHECK_FEC_KEYS || (parent instanceof ClientGetter)) {
            if (FORCE_CHECK_FEC_KEYS || ((ClientGetter) parent).collectingBinaryBlob()) {
                try {
                    ...
                    if (block == null) {
                        block = ClientCHKBlock.encodeSplitfileBlock(buf, forceCryptoKey, getCryptoAlgorithm());
                    }
                    ...
                    if (key != null) {
                        if (!(key.equals(block.getClientKey()))) {
                            if (ignoreLastDataBlock && (blockNo == dataBuckets.length - 1)
                                    && dataSource.equals("FEC DECODE")) {
                                if (logMINOR) {
                                    Logger.minor(
                                        this, "Last block wrong key, ignored because expected due to padding issues");
                                }
                            } else if (ignoreLastDataBlock && (fetchedDataBlocks == dataBuckets.length)
                                       && dataSource.equals("FEC ENCODE")) {
                                ...
                                Logger.error(this, "INVALID KEY FROM " + dataSource + ": Block " + blockNo + " (data "
                                             + dataBuckets.length + " check " + checkBuckets.length
                                             + " ignore last block=" + ignoreLastDataBlock + ") : key "
                                             + block.getClientKey().getURI() + " should be "
                                             + key.getURI(), new Exception("error"));
                            }
                        ...
                        if ((dataSource.equals("FEC ENCODE") || dataSource.equals("FEC DECODE")
                            ...
                            if (logMINOR) {
                                Logger.minor(this,
                                             "Key is null for block " + blockNo
                                             + " when checking key / adding to binary blob, key source is "
                                             + dataSource, new Exception("error"));
                            }
                        } else {
                            Logger.error(this, "Key is null for block " + blockNo
                                         + " when checking key / adding to binary blob, key source is "
                                         + dataSource, new Exception("error"));
                        }
                    }
    ...
    public void onFatalFailure(FetchException e, int blockNo, ObjectContainer container, ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this, "Permanently failed block: " + blockNo + " on " + this + " : " + e, e);
        }
        ...
        synchronized (this) {
            if (isFinishing(container)) {
                return;    // this failure is now irrelevant, and cleanup will occur on the decoder thread
            }
        ...
        if (allFailed) {
            if (persistent) {
                container.activate(errors, Integer.MAX_VALUE);
            }
    ...
    private void onNonFatalFailure(FetchException e, int blockNo, ObjectContainer container, ClientContext context,
                                   boolean callStore) {
        if (persistent) {
            container.activate(blockFetchContext, 1);
        }
    ...
    public void onNonFatalFailure(FetchException[] failures, int[] blockNos, ObjectContainer container,
     ...
     */
    private boolean onNonFatalFailure(FetchException e, int blockNo, ObjectContainer container, ClientContext context,
                                      RequestScheduler sched, int maxTries, boolean callStore) {
        if (logMINOR) {
        ...
        synchronized (this) {
            ...
            if (blockNo < dataBuckets.length) {
                ...
                if ((tries > maxTries) && (maxTries >= 0)) {
                    ...
                    if ((cachedCooldownTries == 0) || (tries % cachedCooldownTries == 0)) {
                        long now = System.currentTimeMillis();

                        if (dataCooldownTimes[blockNo] > now) {
                            Logger.error(this,
                                         "Already on the cooldown queue! for " + this + " data block no " + blockNo,
                                         new Exception("error"));
                        } else {
                            SplitFileFetcherSegmentGet getter = makeGetter(container, context);
                            ...
                            if (getter != null) {
                                ...
                                if (logMINOR) {
                                    Logger.minor(this,
                                                 "Putting data block " + blockNo + " into cooldown until "
                                                 + (dataCooldownTimes[blockNo] - now));
                                }
                            }
                        }
                ...
                if ((tries > maxTries) && (maxTries >= 0)) {
                    ...
                    if ((cachedCooldownTries == 0) || (tries % cachedCooldownTries == 0)) {
                        ...
                        if (checkCooldownTimes[checkNo] > now) {
                            Logger.error(this,
                                         "Already on the cooldown queue! for " + this + " check block no " + blockNo,
                                         new Exception("error"));
                        } else {
                            SplitFileFetcherSegmentGet getter = makeGetter(container, context);
                            ...
                            if (getter != null) {
                                ...
                                if (logMINOR) {
                                    Logger.minor(this,
                                                 "Putting check block " + blockNo + " into cooldown until "
                                                 + (checkCooldownTimes[checkNo] - now));
                                }
                            }
                        }
        ...
        if (failed) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Not retrying block " + blockNo + " on " + this + " : tries=" + tries + "/" + maxTries);
            }
    ...
    private void innerCheckCachedCooldownData(ObjectContainer container) {
        ...
        if (!active) {
            container.deactivate(blockFetchContext, 1);
        }
    }
    ...
    private void fail(FetchException e, ObjectContainer container, ClientContext context, boolean dontDeactivateParent,
                      boolean checkingStoreOnly) {
        if (logMINOR) {
            Logger.minor(this, "Failing segment " + this, e);
        }
        ...
        synchronized (this) {
            ...
            if (startedDecode && checkingStoreOnly) {
                return;
            }

            if (checkingStoreOnly) {
                e = new FetchException(FetchException.DATA_NOT_FOUND);
            }
    ...
    public synchronized long getCooldownWakeup(int blockNum, int maxTries, ObjectContainer container,
            ClientContext context) {
        MyCooldownTrackerItem tracker = makeCooldownTrackerItem(container, context);
        long[] dataCooldownTimes = tracker.dataCooldownTimes;
        long[] checkCooldownTimes = tracker.checkCooldownTimes;
    ...
    synchronized void setMaxCooldownWakeup(long until, int blockNum, int maxTries, ObjectContainer container,
            ClientContext context) {
        MyCooldownTrackerItem tracker = makeCooldownTrackerItem(container, context);
        long[] dataCooldownTimes = tracker.dataCooldownTimes;
        long[] checkCooldownTimes = tracker.checkCooldownTimes;
        ...
        if (blockNum < dataBuckets.length) {
            ...
            checkCooldownTimes[blockNum - dataBuckets.length] =
                Math.max(checkCooldownTimes[blockNum - dataBuckets.length], until);
        }
    }
    ...
    public boolean requeueAfterCooldown(Key key, long time, ObjectContainer container, ClientContext context,
            SplitFileFetcherSubSegment dontDeactivate) {
        if (persistent) {
            container.activate(this, 1);
        }
            ...
            for (int i : matches) {
                ...
                if (k.getNodeKey(false).equals(key)) {
                    if (getCooldownWakeup(i, maxTries, container, context) > time) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Not retrying after cooldown for data block " + i
                                         + " as deadline has not passed yet on " + this + " remaining time: "
                                         + (getCooldownWakeup(i, maxTries, container, context) - time) + "ms");
                        }
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Retrying after cooldown on " + this + ": block " + i + " on " + this
                                     + " : tries=" + getRetries(i, container, context) + "/" + maxTries);
                    }
    ...
    public synchronized Integer[] getKeyNumbersAtRetryLevel(int retryCount, ObjectContainer container,
        ...
        for (int i = 0; i < checkRetries.length; i++) {
            if (foundKeys[i + dataBuckets.length]) {
                continue;
            }
    ...
    public boolean onGotKey(Key key, KeyBlock block, ObjectContainer container, ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (finished || startedDecode || fetcherFinished) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Rejecting block because " + (finished ? "finished " : "")
                                 + (startedDecode ? "started decode " : "")
                                 + (fetcherFinished ? "fetcher finished " : ""));
                }
        ...
        if (fatal != null) {
            ...
            if (!cb.isMetadata()) {
                ...
                onFatalFailure(new FetchException(FetchException.INVALID_METADATA, "Metadata where expected data"),
                               blockNum, container, context);
    ...
    public Bucket getBlockBucket(int blockNum, ObjectContainer container) {
        ...
        if (block == null) {
            Logger.error(this,
                         "Block is null: " + blockNum + " on " + this + " activated = "
                         + container.ext().isActive(this) + " finished = " + finished + " encoder finished = "
                         + encoderFinished + " fetcher finished = " + fetcherFinished);
    ...
    protected Bucket extract(ClientKeyBlock block, int blockNum, ObjectContainer container, ClientContext context) {
        ...
        try {
            data = block.decode(context.getBucketFactory(persistent),
                                (int) (Math.min(this.blockFetchContext.maxOutputLength, Integer.MAX_VALUE)), false);
            ...
            this.onFatalFailure(new FetchException(FetchException.BLOCK_DECODE_ERROR, e1.getMessage()), blockNum,
                                container, context);
            ...
            this.onFatalFailure(new FetchException(FetchException.TOO_BIG, e.getMessage()), blockNum, container,
                                context);
    ...
    public void removeFrom(ObjectContainer container, ClientContext context) {
        if (!finished) {
            Logger.error(this,
                         "Removing " + this + " but not finished, fetcher finished " + fetcherFinished
                         + " fetcher half finished " + fetcherHalfFinished + " encoder finished " + encoderFinished);
        }
    ...
    public int allocateCrossCheckBlock(SplitFileFetcherCrossSegment seg, Random random) {
        if (crossCheckBlocksAllocated == crossCheckBlocks) {
            return -1;
        }
        ...
        for (int i = 0; i < crossCheckBlocks; i++) {
            ...
            if (x == dataBuckets.length) {
                x = dataBuckets.length - crossCheckBlocks;
            }
    ...
    public ArrayList<Integer> validBlockNumbers(KeysFetchingLocally fetching, boolean onlyLowestTries,
        ...
        synchronized (this) {
            ...
            if (startedDecode || isFinishing(container)) {
                return null;
            }
            ...
            for (int i = 0; i < dataBuckets.length + checkBuckets.length; i++) {
                ...
                if (getCooldownWakeup(i, maxTries, container, context) > now) {
                    continue;
                }
                ...
                if (fetching.hasKey(key, getter, persistent, container)) {
                    continue;
                }
    ...
    public boolean checkRecentlyFailed(int blockNum, ObjectContainer container, ClientContext context,
                                       KeysFetchingLocally fetching, long now) {
        if (this.keys == null) {
            migrateToKeys(container);
        } else {
            if (persistent) {
        ...
        if ((maxRetries == -1) || (maxRetries >= RequestScheduler.COOLDOWN_RETRIES)) {
            if (logMINOR) {
                Logger.minor(this,
                             "RecentlyFailed -> cooldown until " + TimeUtil.formatTime(timeout - now) + " on " + this);
            }
     ...
     * @return
     */
    public List<PersistentChosenBlock> makeBlocks(PersistentChosenRequest request, RequestScheduler sched,
            KeysFetchingLocally fetching, SplitFileFetcherSegmentGet getter, ObjectContainer container,
            ClientContext context) {
        long now = System.currentTimeMillis();
        ...
        synchronized (this) {
            ...
            for (int i = 0; i < dataBuckets.length + checkBuckets.length; i++) {
                ...
                list.add(new PersistentChosenBlock(false, request, new SplitFileFetcherSegmentSendableRequestItem(i),
                                                   key, ckey, sched));
            }
        }
        ...
        for (Iterator<PersistentChosenBlock> i = list.iterator(); i.hasNext(); ) {
            ...
            if ((maxTries == -1) || (maxTries >= RequestScheduler.COOLDOWN_RETRIES)) {
                ...
                // Concurrency is fine here, it won't go away before the given time.
                setMaxCooldownWakeup(l, ((SplitFileFetcherSegmentSendableRequestItem) block.token).blockNum, maxTries,
                                     container, context);
            } else {
                FetchException e = new FetchException(FetchException.RECENTLY_FAILED);

                incErrors(e, container);
                onNonFatalFailure(e, ((SplitFileFetcherSegmentSendableRequestItem) (block.token)).blockNum, container,
                                  context);
            }
        }
    ...
    public long getCooldownTime(ObjectContainer container, ClientContext context, HasCooldownCacheItem parentRGA,
                                long now) {
        if (keys == null) {
            migrateToKeys(container);
        } else {
    ...
    public synchronized SplitFileFetcherSegmentGet makeGetter(ObjectContainer container, ClientContext context) {
        if (finishing || startedDecode || finished) {
            return null;
        }
