...
public class SimpleSingleFileFetcher extends BaseSingleFileFetcher implements ClientGetState {
    ...
    SimpleSingleFileFetcher(ClientKey key, int maxRetries, FetchContext ctx, ClientRequester parent,
                            ...
                            ObjectContainer container, ClientContext context, boolean deleteFetchContext,
    ...
    @Override
    public void onFailure(LowLevelGetException e, Object reqTokenIgnored, ObjectContainer container,
    ...
    @Override
    public void onSuccess(ClientKeyBlock block, boolean fromStore, Object reqTokenIgnored, ObjectContainer container,
                          ClientContext context) {
        if (persistent) {
            container.activate(parent, 1);
        }
        ...
        if (!block.isMetadata()) {
            ...
            onFailure(new FetchException(FetchException.INVALID_METADATA, "Metadata where expected data"), false,
                      container, context);
        }
    }
    ...
    protected Bucket extract(ClientKeyBlock block, ObjectContainer container, ClientContext context) {
        ...
        try {
            ...
            onFailure(new FetchException(FetchException.BLOCK_DECODE_ERROR, e1.getMessage()), false, container,
                      context);
    ...
    protected void onBlockDecodeError(Object token, ObjectContainer container, ClientContext context) {
        onFailure(
            new FetchException(
                FetchException.BLOCK_DECODE_ERROR,
                "Could not decode block with the URI given, probably invalid as inserted, possible the URI is wrong"), true, container, context);
    }
}
