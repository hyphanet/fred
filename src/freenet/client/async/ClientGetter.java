...
public class ClientGetter extends BaseClientGetter implements WantsCooldownCallback {
    ...
    public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
                        RequestClient clientContext, Bucket returnBucket) {
        this(client, uri, ctx, priorityClass, clientContext, returnBucket, null, null);
    }

    public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
                        RequestClient clientContext, Bucket returnBucket, BinaryBlobWriter binaryBlobWriter) {
    ...
    public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
                        RequestClient clientContext, Bucket returnBucket, BinaryBlobWriter binaryBlobWriter,
                        boolean dontFinalizeBlobWriter, Bucket initialMetadata) {
        this(client, uri, ctx, priorityClass, clientContext, returnBucket, binaryBlobWriter, dontFinalizeBlobWriter,
    ...
    public ClientGetter(ClientGetCallback client, FreenetURI uri, FetchContext ctx, short priorityClass,
                        RequestClient clientContext, Bucket returnBucket, BinaryBlobWriter binaryBlobWriter,
                        boolean dontFinalizeBlobWriter, Bucket initialMetadata, String forceCompatibleExtension) {
     ...
     */
    public boolean start(boolean restart, FreenetURI overrideURI, ObjectContainer container, ClientContext context)
        ...
        try {
            ...
            synchronized (this) {
                ...
                currentState = SingleFileFetcher.create(this, this, uri, ctx, actx, ctx.maxNonSplitfileRetries, 0,
                        false, -1, true, true, container, context, realTimeFlag, initialMetadata != null);
            }
    ...
    public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
                          List<? extends Compressor> decompressors, ClientGetState state, ObjectContainer container,
        ...
        try {
            ...
            onFailure(new FetchException(FetchException.BUCKET_ERROR, "Failed to close binary blob stream: " + ioe),
                      null, container, context);

            return;
        } catch (BinaryBlobAlreadyClosedException e) {
            onFailure(new FetchException(FetchException.BUCKET_ERROR,
                                         "Failed to close binary blob stream, already closed: " + e, e), null,
                                             container, context);
        ...
        if ((forceCompatibleExtension != null) && ctx.filterData) {
            if (mimeType == null) {
                onFailure(new FetchException(FetchException.MIME_INCOMPATIBLE_WITH_EXTENSION,
        ...
        try {
            ...
            worker = new ClientGetWorkerThread(dataInput, output, uri, mimeType, hashes, ctx.filterData, ctx.charset,
                                               ctx.prefetchHook, ctx.tagReplacer, context.linkFilterExceptionProvider);
            worker.start();
        ...
        if (ex != null) {
            ...
            if ((finalResult != null) && (finalResult != returnBucket)) {
                ...
                if (persistent()) {
                    finalResult.removeFrom(container);
                }
            ...
            if (!alreadyFinished) {
                try {
                    ...
                    if ((e.mode != FetchException.CANCELLED) &&!force) {
                        e = new FetchException(FetchException.BUCKET_ERROR,
    ...
    public void cancel(ObjectContainer container, ClientContext context) {
        ...
        Logger.error(this,
                     "Cancelling " + currentState
                     + " did not call onFailure(), so did not removeFrom() or call callback");
    ...
    public void notifyClients(ObjectContainer container, ClientContext context) {
        ...
        ctx.eventProducer.produceEvent(new SplitfileProgressEvent(total, this.successfulBlocks, this.failedBlocks,
                this.fatallyFailedBlocks, minSuccess, 0, finalized), container, context);
    }
    ...
    public void onTransition(ClientGetState oldState, ClientGetState newState, ObjectContainer container) {
        synchronized (this) {
            if (currentState == oldState) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Transition: " + oldState + " -> " + newState + " on " + this + " persistent = "
                ...
                if (logMINOR) {
                    Logger.minor(this, "Ignoring transition: " + oldState + " -> " + newState + " because current = "
                                 + currentState + " on " + this + " persistent = "
...
//          }
     ...
     */
    public boolean restart(FreenetURI redirect, boolean filterData, ObjectContainer container, ClientContext context)
            throws FetchException {
    ...
    protected void addKeyToBinaryBlob(ClientKeyBlock block, ObjectContainer container, ClientContext context) {
        ...
        try {
            ...
            onFailure(new FetchException(FetchException.BUCKET_ERROR,
                                         "Failed to write key to binary blob stream: " + e), null, container, context);
        } catch (BinaryBlobAlreadyClosedException e) {
            Logger.error(this, "Failed to write key to binary blob stream (already closed??): " + e, e);
            onFailure(new FetchException(FetchException.BUCKET_ERROR,
                                         "Failed to write key to binary blob stream (already closed??): " + e), null,
                                             container, context);
        }
    }
     ...
     */
    @Override
    public void onExpectedMIME(ClientMetadata clientMetadata, ObjectContainer container, ClientContext context)
            throws FetchException {
    ...
    public void onExpectedTopSize(long size, long compressed, int blocksReq, int blocksTotal,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "New format metadata has top data: original size " + size + " (compressed " + compressed
    ...
    public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] customSplitfileKey,
            ...
            ClientContext context) {
        ctx.eventProducer.produceEvent(new SplitfileCompatibilityModeEvent(min, max, customSplitfileKey, dontCompress,
                bottomLayer || definitiveAnyway), container, context);
    }
    ...
    public void onHashes(HashResult[] hashes, ObjectContainer container, ClientContext context) {
        synchronized (this) {
            if (this.hashes != null) {
                if (persistent()) {
                    container.activate(this.hashes, Integer.MAX_VALUE);
                }
