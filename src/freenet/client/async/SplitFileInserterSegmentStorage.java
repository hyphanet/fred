...
public class SplitFileInserterSegmentStorage {
	...
	public SplitFileInserterSegmentStorage(
		...
		totalBlockCount = dataBlockCount +
		crossCheckBlockCount +
		checkBlockCount;
		this.keyLength = keyLength;
		crossSegmentBlockSegments =
			new SplitFileInserterCrossSegmentStorage[crossCheckBlocks];
		...
		blockChooser = new SplitFileInserterSegmentBlockChooser(
			this,
			totalBlockCount,
			random,
			maxRetries,
		...
		try {
			...
			dos.close();
			statusLength = (int) cos.written() +
			parent.checker.checksumLength();
		} catch (IOException e) {
	...
	public SplitFileInserterSegmentStorage(
		...
		if (dataBlockCount < 0) throw new StorageFormatException(
			"Bogus data block count"
		);
		crossCheckBlockCount = dis.readInt();
		if (crossCheckBlockCount < 0) throw new StorageFormatException(
			"Bogus cross-check block count"
		);
		if (
			(crossCheckBlockCount == 0) != (parent.crossSegments == null)
		) throw new StorageFormatException(
			"Cross-check block count inconsistent with parent"
		);
		checkBlockCount = dis.readInt();
		if (checkBlockCount < 0) throw new StorageFormatException(
			"Bogus check block count"
		);
		totalBlockCount = dataBlockCount +
		crossCheckBlockCount +
		checkBlockCount;
		if (
			totalBlockCount > FECCodec.MAX_TOTAL_BLOCKS_PER_SEGMENT
		) throw new StorageFormatException("Bogus total block count");
		this.statusLength = dis.readInt();
		if (statusLength < 0) throw new StorageFormatException(
			"Bogus status length"
		);
		crossSegmentBlockSegments =
			new SplitFileInserterCrossSegmentStorage[crossCheckBlockCount];
		crossSegmentBlockNumbers = new int[crossCheckBlockCount];
		blocksHaveKeys = new boolean[totalBlockCount];
		this.splitfileCryptoAlgorithm = splitfileCryptoAlgorithm;
		this.splitfileCryptoKey = splitfileCryptoKey;
		crossDataBlocksAllocated = new boolean[dataBlockCount +
		crossCheckBlockCount];
		blockChooser = new SplitFileInserterSegmentBlockChooser(
			this,
			totalBlockCount,
		...
		try {
			...
			dos.close();
			int minStatusLength =
				(int) cos.written() + parent.checker.checksumLength();
			if (
				minStatusLength > statusLength
	...
	int allocateCrossCheckBlock(
		...
		int x =
			crossCheckBlockCount - (1 + random.nextInt(crossCheckBlockCount));
			...
			synchronized (this) {
				...
				try {
					dos = new DataOutputStream(
						parent.writeChecksummedTo(
							parent.segmentStatusOffset(segNo),
							statusLength
						)
					);
					innerStoreStatus(dos);
	...
	public void readStatus()
		...
		parent.preadChecksummed(
			parent.getOffsetSegmentStatus(segNo),
			data,
			0,
			data.length
		);
		...
		if (dis.readInt() != segNo) throw new StorageFormatException(
			"Bad segment number"
		);
	...
	void setKey(int blockNumber, ClientCHK key) throws IOException {
		...
		try {
			...
			if (!oldKey.equals(key)) throw new IOException(
	...
	public void checkKeys() {
		...
		try {
			...
			Logger.error(
				this,
				"Missing key even though segment encoded. Recovering by re-encoding..."
			);
			synchronized (this) {
				encoded = false;
			}
	...
	private void writeCheckBlock(int checkBlockNo, byte[] buf)
		throws IOException {
	...
	public synchronized void startEncode(final short prio) {
		...
		encoding = true;
		int totalBlockCount =
			dataBlockCount + checkBlockCount + crossCheckBlockCount;
		long limit =
			totalBlockCount * CHKBlock.DATA_LENGTH +
			Math.max(
				parent.codec.maxMemoryOverheadDecode(
					dataBlockCount,
					crossCheckBlockCount
				),
				parent.codec.maxMemoryOverheadEncode(
					dataBlockCount,
					crossCheckBlockCount
				)
			);
					...
					try {
						...
						try {
							if (!shutdown) {
								...
								synchronized (
									SplitFileInserterSegmentStorage.this
								) {
									encoding = false;
								}
							...
							if (lock != null) lock.unlock(
								false,
								MemoryLimitedJobRunner.THREAD_PRIORITY
							);
						}
					}
					return true;
	 ...
	 * @throws IOException */
	private void generateKeys(byte[][] dataBlocks, int offset)
		throws IOException {
	...
	private byte[] readBlock(int blockNo) throws IOException {
		...
		if (blockNo < dataBlockCount) return readDataBlock(blockNo);
			...
			blockNo < dataBlockCount + crossCheckBlockCount
		) return readCrossCheckBlock(blockNo - dataBlockCount);
		else return readCheckBlock(
			blockNo - (dataBlockCount + crossCheckBlockCount)
		);
	}
	...
	ClientCHKBlock encodeBlock(byte[] buf) {
		...
		try {
			block = ClientCHKBlock.encodeSplitfileBlock(
				buf,
				splitfileCryptoKey,
	...
	void onInsertedAllBlocks() {
		if (logMINOR) Logger.minor(
			this,
			"Inserted all blocks in segment " + this
		);
		synchronized (this) {
			if (!encoded) return;
		}
	...
	public void onFailure(int blockNo, InsertException e) {
		...
		if (e.isFatal()) {
			...
			if (
				e.mode == InsertExceptionMode.ROUTE_NOT_FOUND &&
				...
				try {
					...
					Logger.error(
						this,
						"RNF but no key on block " + blockNo + " on " + this
					);
	...
	static final class BlockInsert
		implements SendableRequestItemKey, SendableRequestItem {
	...
	void setCrossCheckBlock(
		...
		crossSegmentBlockSegments[segmentBlockNumber - dataBlockCount] =
			crossSegment;
		crossSegmentBlockNumbers[segmentBlockNumber - dataBlockCount] =
			crossSegmentBlockNumber;
	}
