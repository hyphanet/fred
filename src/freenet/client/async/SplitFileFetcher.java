...
public class SplitFileFetcher
	...
	public SplitFileFetcher(
		...
		blockFetchContext = new FetchContext(
			fetchContext,
			FetchContext.SPLITFILE_DEFAULT_BLOCK_MASK,
			true,
			null
		);
		if (parent.isCancelled()) throw new FetchException(
		...
		try {
			...
			if (
				...
				FileGetCompletionCallback fileCallback =
					((FileGetCompletionCallback) cb);
				File targetFile = fileCallback.getCompletionFile();
				if (targetFile != null) {
					callbackCompleteViaTruncation = fileCallback;
					fileCompleteViaTruncation = FileUtil.createTempFile(
						targetFile.getName(),
						".freenet-tmp",
			...
			storage = new SplitFileFetcherStorage(
				metadata,
				this,
				decompressors,
				clientMetadata,
				...
				persistent
					? context.persistentRAFFactory
					: context.tempRAFFactory,
			...
			Logger.error(
				this,
				"Failed to start splitfile fetcher because of disk I/O error?: " +
				e,
				e
			);
			throw new FetchException(FetchExceptionMode.BUCKET_ERROR, e);
		}
		long eventualLength = Math.max(
			storage.decompressedLength,
	...
	public void fail(FetchException e) {
		...
		if (storage != null) context
			.getChkFetchScheduler(realTimeFlag)
			.removePendingKeys(storage.keyListener, true);
	...
	public void onSuccess() {
		...
		synchronized (this) {
			if (failed) {
				...
				if (succeeded) {
					Logger.error(
						this,
						"Called onSuccess() twice on " + this,
						new Exception("debug")
					);
					return;
				} else {
		...
		if (this.callbackCompleteViaTruncation != null) {
			...
			cb.onSuccess(
				storage.streamGenerator(),
				storage.clientMetadata,
				storage.decompressors,
				this,
				context
			);
			storage.finishedFetcher();
	...
	public void queueHeal(byte[] data, byte[] cryptoKey, byte cryptoAlgorithm) {
		try {
			...
			Logger.error(
				this,
				"I/O error, failed to queue healing block: " + e,
				e
			);
		}
	}
	...
	public void onResume(
		...
		for (int i = 0; i < succeededBlocks - 1; i++) parent.completedBlock(
			true,
			context
		);
		if (succeededBlocks > 0) parent.completedBlock(false, context);
		for (int i = 0; i < failedBlocks - 1; i++) parent.failedBlock(
			true,
			context
		);
	...
	public void restartedAfterDataCorruption() {
		...
		Logger.error(
			this,
			"Restarting download " + this + " after data corruption"
		);
	...
	public void onResume(ClientContext context) throws FetchException {
		if (logMINOR) Logger.minor(
			this,
			"Restarting SplitFileFetcher from storage..."
		);
		boolean resumed =
			parent instanceof ClientGetter &&
			((ClientGetter) parent).resumedFetcher();
		this.context = context;
		try {
			KeySalter salter = getSalter();
			raf.onResume(context);
			this.storage = new SplitFileFetcherStorage(
				raf,
				realTimeFlag,
				this,
				blockFetchContext,
			...
			Logger.error(
				this,
				"Failed to resume storage file: " + e + " for " + raf,
				e
			);
			throw new FetchException(FetchExceptionMode.BUCKET_ERROR, e);
		} catch (IOException e) {
			...
			Logger.error(
				this,
				"Failed to resume due to I/O error: " + e + " raf = " + raf,
				e
			);
			throw new FetchException(FetchExceptionMode.BUCKET_ERROR, e);
			...
			Logger.error(
				this,
				"Failed to resume due to storage error: " + e + " raf = " + raf,
				e
			);
			throw new FetchException(
	...
	public SplitFileFetcher(
		...
		if (completeViaTruncation) {
			...
			if (
				!fileCompleteViaTruncation.exists()
			...
			if (
				fileCompleteViaTruncation.length() != rafSize
			) throw new ResumeFailedException(
				"Storage file is not of the correct length"
			);
			// FIXME check against finalLength too, maybe we can finish straight away.
			this.raf = new PooledFileRandomAccessBuffer(
				fileCompleteViaTruncation,
				false,
				rafSize,
				null,
			...
			this.raf = BucketTools.restoreRAFFrom(
				dis,
