...
public class SplitFileFetcher implements ClientGetState, HasKeyListener {
    ...
    public SplitFileFetcher(Metadata metadata, GetCompletionCallback rcb, ClientRequester parent2, FetchContext newCtx,
                            boolean deleteFetchContext, boolean realTimeFlag,
                            List<? extends Compressor> decompressors2, ClientMetadata clientMetadata,
                            ArchiveContext actx, int recursionLevel, long token2, boolean topDontCompress,
        ...
        if (splitfileType == Metadata.SPLITFILE_NONREDUNDANT) {
            if (splitfileCheckBlocks > 0) {
                Logger.error(this,
                             "Splitfile type is SPLITFILE_NONREDUNDANT yet " + splitfileCheckBlocks
                             + " check blocks found!! : " + this);

                throw new FetchException(FetchException.INVALID_METADATA,
                                         "Splitfile type is non-redundant yet have " + splitfileCheckBlocks
                                         + " check blocks");
            }
            ...
            cb.onSplitfileCompatibilityMode(minCompatMode, maxCompatMode, metadata.getCustomSplitfileKey(),
                                            dontCompress, true, topCompatibilityMode != 0, container, context);

            if ((blocksPerSegment > fetchContext.maxDataBlocksPerSegment)
                    || (checkBlocksPerSegment > fetchContext.maxCheckBlocksPerSegment)) {
                throw new FetchException(FetchException.TOO_MANY_BLOCKS_PER_SEGMENT,
                                         "Too many blocks per segment: " + blocksPerSegment + " data, "
                                         + checkBlocksPerSegment + " check");
            }
        } else {
        ...
        if (finalLength > overrideLength) {
            if (finalLength - overrideLength > CHKBlock.DATA_LENGTH) {
                throw new FetchException(FetchException.INVALID_METADATA,
                                         "Splitfile is " + finalLength + " but length is " + finalLength);
            }
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Creating block filter for " + this + ": keys=" + (splitfileDataBlocks + splitfileCheckBlocks)
                         + " main bloom size " + mainBloomFilterSizeBytes + " bytes, K=" + mainBloomK + ", filename="
                         + mainBloomFile + " alt bloom filter: filename=" + altBloomFile + " segments: "
                         + segments.length + " each is " + perSegmentBloomFilterSizeBytes + " bytes k=" + perSegmentK);
        }

        try {
            tempListener = new SplitFileFetcherKeyListener(this, keyCount, mainBloomFile, altBloomFile,
                    mainBloomFilterSizeBytes, mainBloomK, localSalt, segments.length, perSegmentBloomFilterSizeBytes,
                    perSegmentK, persistent, true, null, null, container, false, realTimeFlag);
        } catch (IOException e) {
        ...
        boolean pre1254 = !((minCompatMode == CompatibilityMode.COMPAT_CURRENT)
                            || (minCompatMode.ordinal() >= CompatibilityMode.COMPAT_1255.ordinal()));
        boolean pre1250 = ((minCompatMode == CompatibilityMode.COMPAT_UNKNOWN)
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            segments[i] = new SplitFileFetcherSegment(splitfileType, keys, this, archiveContext, blockFetchContext,
                    maxTempLength, recursionLevel, parent, i, pre1250, pre1254, crossCheckBlocks, cryptoAlgorithm,
                    splitfileCryptoKey, maxRetries, realTimeFlag);
        ...
        if (crossCheckBlocks != 0) {
            Random random = new MersenneTwister(Metadata.getCrossSegmentSeed(metadata.getHashes(),
            ...
            for (int i = 0; i < crossSegments.length; i++) {
                ...
                SplitFileFetcherCrossSegment seg = new SplitFileFetcherCrossSegment(persistent, segLen,
                                                       crossCheckBlocks, parent, this, metadata.getSplitfileType());
     ...
     */
    private SplitFileStreamGenerator finalStatus(final ObjectContainer container, final ClientContext context)
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Segment " + i + " decoded length " + sz + " total length now " + length + " for "
                             + s.dataBuckets.length + " blocks which should be "
                             + (s.dataBuckets.length * CHKBlock.DATA_LENGTH));
            }
        ...
        if (length > overrideLength) {
            if (length - overrideLength > CHKBlock.DATA_LENGTH) {
                throw new FetchException(FetchException.INVALID_METADATA,
                                         "Splitfile is " + length + " but length is " + length);
            }
            ...
            if (allDone) {
                if (allSegmentsFinished) {
                    if (logMINOR) {
                        ...
                        Logger.minor(this, "Was already finished! (segmentFinished(" + segment + ')',
                                     new Exception("debug"));
                    }
                } else {
    ...
    private void finish(ObjectContainer container, ClientContext context) {
        ...
        if ((crossCheckBlocks != 0) &&!persistent) {
    ...
    public KeyListener makeKeyListener(ObjectContainer container, ClientContext context, boolean onStartup)
        ...
        synchronized (this) {
            ...
            try {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Attempting to read Bloom filter for " + this + " main file=" + main + " alt file="
                                 + alt);
                }

                tempListener = new SplitFileFetcherKeyListener(this, keyCount, main, alt, mainBloomFilterSizeBytes,
                        mainBloomK, localSalt, segments.length, perSegmentBloomFilterSizeBytes, perSegmentK,
                        persistent, false, cachedMainBloomFilter, cachedSegmentBloomFilters, container, onStartup,
                        realTimeFlag);
                ...
                if (main != null) {
                    try {
                        ...
                        System.err.println("Failed to delete old bloom filter file: " + main
                                           + " - this may leak information about a download : " + e);
                        e.printStackTrace();
                    }
                }
                ...
                if (alt != null) {
                    try {
                        ...
                        System.err.println("Failed to delete old segment filters file: " + alt
                                           + " - this may leak information about a download : " + e);
                    }
                }
            } catch (IOException e) {
                ...
                try {
                    tempListener = new SplitFileFetcherKeyListener(this, keyCount, mainBloomFile, altBloomFile,
                            mainBloomFilterSizeBytes, mainBloomK, localSalt, segments.length,
                            perSegmentBloomFilterSizeBytes, perSegmentK, persistent, true, cachedMainBloomFilter,
                            cachedSegmentBloomFilters, container, onStartup, realTimeFlag);
                } catch (IOException e1) {
                    throw new KeyListenerConstructionException(new FetchException(FetchException.BUCKET_ERROR,
                            "Unable to reconstruct Bloom filters: " + e1, e1));
                }
            }
            ...
            for (int i = 0; i < crossSegments.length; i++) {
                if (crossSegments[i] != null) {
                    ...
                    if (persistent) {
                        ...
                        if (!active) {
                            container.activate(crossSegments[i], 1);
                        }
                    }
                    ...
                    if (!crossSegments[i].isFinished()) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this, "Waiting for " + crossSegments[i] + " in removeFrom()");
                        }
                    }

                    if (!active) {
                        container.deactivate(crossSegments[i], 1);
                    }
                }
            }
    ...
    public boolean objectCanUpdate(ObjectContainer container) {
        if (hashCode == 0) {
            Logger.error(this,
                         "Trying to update with hash 0 => already deleted! active=" + container.ext().isActive(this)
    ...
    public boolean objectCanNew(ObjectContainer container) {
        if (hashCode == 0) {
            Logger.error(this,
                         "Trying to write with hash 0 => already deleted! active=" + container.ext().isActive(this)
                         + " stored=" + container.ext().isStored(this), new Exception("error"));
     ...
     */
    public boolean onFinishedCrossSegment(ObjectContainer container, ClientContext context,
        ...
        for (int i = 0; i < crossSegments.length; i++) {
            if (crossSegments[i] != null) {
                ...
                if (!crossSegments[i].isFinished()) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this, "Waiting for " + crossSegments[i]);
                    }
                }
