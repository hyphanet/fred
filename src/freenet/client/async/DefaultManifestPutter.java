...
public class DefaultManifestPutter extends BaseManifestPutter {
	...
	private void verifyManifest(HashMap<String, Object> metadata) {
		for (Map.Entry<String, Object> entry : metadata.entrySet()) {
			...
			throw new IllegalArgumentException(
				"FATAL: unknown manifest element: " + o
			);
		}
	}
	...
	private long makePutHandlers(
		...
		if (logMINOR) Logger.minor(
			this,
			"STAT: handling " + ((parentName == null) ? "<root>?" : parentName)
		);
		...
		if (maxSize == DEFAULT_MAX_CONTAINERSIZE) maxSize =
			DEFAULT_MAX_CONTAINERSIZE - DEFAULT_CONTAINERSIZE_SPARE;

		// first get the size (the whole one)
		ContainerSize wholeSize = ContainerSizeEstimator.getSubTreeSize(
			manifestElements,
		...
		if (wholeSize.getSizeTotalNoLimit() <= maxSize) {
			...
			if (logMINOR) Logger.minor(
				this,
				"PackStat2: the whole tree (unlimited) fits into container (no externals)"
			);
			makeEveryThingUnlimitedPutHandlers(
				containerBuilder,
				manifestElements,
				defaultName,
				prefix
		...
		if (wholeSize.getSizeTotal() <= maxSize) {
			...
			if (logMINOR) Logger.minor(
				this,
				"PackStat2: the whole tree fits into container (with externals)"
			);
			makeEveryThingPutHandlers(
				containerBuilder,
				manifestElements,
				defaultName,
				prefix
		...
		if (
			(wholeSize.getSizeFiles() < maxSize) ||
				...
				> entry : manifestElements.entrySet()) {
					...
					if (o instanceof ManifestElement) {
						...
						if (
							...
							name,
							me.getData(),
							me.getMimeTypeOverride(),
							name.equals(defaultName)
						);
						else containerBuilder.addItem(
							name,
							prefix + name,
							me,
							name.equals(defaultName)
		...
		{
			Iterator<Map.Entry<String, Object>> iter = manifestElements
				.entrySet()
				.iterator();
			while (iter.hasNext()) {
		...
		if (
			(wholeSize.getSizeSubTrees() + tmpSize + minUsageForFiles <
				maxSize) ||
			(wholeSize.getSizeSubTreesNoLimit() + tmpSize + minUsageForFiles <
				maxSize)
		) {
			...
			if (
				wholeSize.getSizeSubTreesNoLimit() +
					tmpSize +
					minUsageForFiles <
				maxSize
			) {
				...
				> entry : manifestElements.entrySet()) {
					...
					if (o instanceof HashMap) {
						@SuppressWarnings("unchecked")
						HashMap<String, Object> hm = (HashMap<
								String,
								Object
							>) o;
						containerBuilder.pushCurrentDir();
			...
			if (logMINOR) Logger.minor(
				this,
				"PackStat2: sub dirs does not fit into container, make each its own"
			);
			for (Map.Entry<
				String,
				Object
			> entry : manifestElements.entrySet()) {
				...
				if (o instanceof HashMap) {
					@SuppressWarnings("unchecked")
					HashMap<String, Object> hm = (HashMap<String, Object>) o;
					ContainerBuilder subC = containerBuilder.makeSubContainer(
						name
					);
					makePutHandlers(
						subC,
		...
		while (!itemsLeft.isEmpty()) {
			if (logMINOR) Logger.minor(
				this,
				"ItemsLeft checker: " + itemsLeft.size()
			);
			...
			if (itemsLeft.size() == 1) {
				...
				for (Map.Entry<String, Object> entry : itemsLeft.entrySet()) {
					...
					containerBuilder.addElement(
						lname,
						me,
						lname.equals(defaultName)
					);
				}
				itemsLeft.clear();
				continue;
			...
			final long leftLimit =
				DEFAULT_MAX_CONTAINERSIZE - DEFAULT_CONTAINERSIZE_SPARE;
			ContainerSize leftSize = ContainerSizeEstimator.getSubTreeSize(
				itemsLeft,
				DEFAULT_MAX_CONTAINERITEMSIZE,
				leftLimit,
				0
			...
			if (
				...
				for (Map.Entry<String, Object> entry : itemsLeft.entrySet()) {
					String lname = entry.getKey();
					ManifestElement me = (ManifestElement) entry.getValue();
					containerBuilder.addArchiveItem(
						archive,
						lname,
						me,
						lname.equals(defaultName)
			...
			if (
				((leftSize.getSizeFiles() - (512 * itemsLeft.size())) == 0) &&
			...
			Iterator<Map.Entry<String, Object>> iter = itemsLeft
				.entrySet()
				.iterator();
			while (iter.hasNext()) {
				Map.Entry<String, Object> entry = iter.next();
	...
	private void makeEveryThingPutHandlers(
		...
		for (Map.Entry<String, Object> entry : manifestElements.entrySet()) {
			...
			if (o instanceof ManifestElement) {
				ManifestElement element = (ManifestElement) o;
				if (
					element.getSize() > DEFAULT_MAX_CONTAINERITEMSIZE
				) containerBuilder.addExternal(
					name,
					element.getData(),
