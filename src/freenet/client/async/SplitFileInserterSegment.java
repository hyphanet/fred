...
public class SplitFileInserterSegment extends SendableInsert implements FECCallback, Encodeable {
    ...
    public SplitFileInserterSegment(SplitFileInserter parent, boolean persistent, boolean realTimeFlag,
                                    BaseClientPutter putter, short splitfileAlgo, int crossCheckBlocks,
                                    int checkBlockCount, Bucket[] origDataBlocks, InsertContext blockInsertContext,
                                    boolean getCHKOnly, int segNo, byte cryptoAlgorithm, byte[] cryptoKey,
    ...
    public void start(ObjectContainer container, ClientContext context) throws InsertException {
        synchronized (this) {
            if (crossCheckBlocks != 0) {
                ...
                if (logMINOR) {
                    Logger.minor(this, "Starting segment " + segNo);
                }
            }
        ...
        if (logMINOR) {
            ...
            Logger.minor(this,
                         "Starting segment " + segNo + " of " + parent + " (" + parent.dataLength + "): " + this
                         + " ( finished=" + finished + " encoded=" + encoded + " hasURIs=" + hasURIs + " persistent="
                         + persistent + ')');
        }
        ...
        if (!encoded) {
            if (logMINOR) {
                Logger.minor(this,
                             "Segment " + segNo + " of " + parent + " (" + parent.dataLength + ") is not encoded");
            }

            splitfileAlgo = FECCodec.getCodec(this.splitfileAlgo, dataBlocks.length, checkBlocks.length);

            if (logMINOR) {
                Logger.minor(this,
                             "Encoding segment " + segNo + " of " + parent + " (" + parent.dataLength + ") persistent="
                             + persistent);
            }
            ...
            synchronized (this) {
                if (!encoded) {
                    ...
                    job = encodeJob = new FECJob(splitfileAlgo, context.fecQueue, dataBlocks, checkBlocks,
                                                 CHKBlock.DATA_LENGTH,
                                                 persistent
                                                 ? context.persistentBucketFactory : context.tempBucketFactory, this,
                                                     false, parent.parent.getPriorityClass(), persistent);
                }
            }
        ...
        for (int i = 0; i < dataBlocks.length; i++) {
            if ((dataURIs[i] == null) && (dataBlocks[i] != null)) {
                try {
                    ...
                    ClientCHK key = encodeBucket(dataBlocks[i], compressorDescriptor, cryptoAlgorithm,
                                                 cryptoKey).getClientKey();
                ...
                fail(new InsertException(InsertException.INTERNAL_ERROR,
                                         "Data block " + i + " cannot be encoded: no data", null), container, context);
            }
        }
            ...
            for (int i = 0; i < checkBlocks.length; i++) {
                if ((checkURIs[i] == null) && (checkBlocks[i] != null)) {
                    try {
                        ...
                        ClientCHK key = encodeBucket(checkBlocks[i], compressorDescriptor, cryptoAlgorithm,
                                                     cryptoKey).getClientKey();
                    ...
                    fail(new InsertException(InsertException.INTERNAL_ERROR,
                                             "Data block " + i + " cannot be encoded: no data", null), container,
                                                 context);
                }
            }
        }
    ...
    public void onDecodedSegment(ObjectContainer container, ClientContext context, FECJob job, Bucket[] dataBuckets,
                                 Bucket[] checkBuckets, SplitfileBlock[] dataBlockStatus,
                                 SplitfileBlock[] checkBlockStatus) {}    // irrevelant
    ...
    public void onEncodedSegment(ObjectContainer container, ClientContext context, FECJob job, Bucket[] dataBuckets,
        ...
        try {
            ...
            InsertException ex = new InsertException(InsertException.INTERNAL_ERROR, t, null);
    ...
    private void finish(ObjectContainer container, ClientContext context, SplitFileInserter parent) {
        ...
        synchronized (this) {
            ...
            if (blocksSucceeded < blocksCompleted) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Blocks succeeded " + blocksSucceeded + " blocks completed " + blocksCompleted
                                 + " gives error " + toThrow + " on " + this);
                }
            } else if (!hasURIs) {
                fail(new InsertException(InsertException.INTERNAL_ERROR, "Completed but not encoded?!", null),
                     container, context);
    ...
    private boolean onEncode(int x, ClientCHK key, ObjectContainer container, final ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Blocks got URI: " + blocksGotURI + " of " + (dataBlocks.length + checkBlocks.length));
            }
    ...
    private BlockItem getBlockItem(ObjectContainer container, ClientContext context, int blockNum, byte cryptoAlgorithm)
            throws IOException {
            ...
            if (blockNum >= dataBlocks.length) {
                ...
                if (checkFinished[checkNum]) {
                    if (checkFailed[checkNum]) {
                        Logger.error(this,
                                     "Got onFailure() but block has already failed! Check block " + checkNum + " on "
                                     + this);
                    } else {
                        Logger.error(this,
                                     "Got onFailure() but block has already succeeded: Check block " + checkNum
                                     + " on " + this);
                    }
                ...
                if (!treatAsSuccess) {
                    ...
                    if ((checkRetries[checkNum] > maxRetries) && (maxRetries != -1)) {
                        ...
                        if (checkBlocks[checkNum] != null) {
                            ...
                            Logger.error(this,
                                         "Check block " + checkNum + " failed on " + this
                                         + " but bucket is already nulled out!");
                        }
                    }
                    ...
                    if (checkBlocks[checkNum] != null) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Repeated RNF, treating as success for check block " + checkNum + " on "
                                         + this);
                        }
                    } else {
                        Logger.error(this,
                                     "Check block " + checkNum + " succeeded (sort of) on " + this
                                     + " but bucket is already nulled out!");
                    }
                }
            } else {
                ...
                if (dataFinished[blockNum]) {
                    if (dataFailed[blockNum]) {
                        Logger.error(this,
                                     "Got onFailure() but block has already failed! Data block " + blockNum + " on "
                                     + this);
                    } else {
                        Logger.error(this,
                                     "Got onFailure() but block has already succeeded: Data block " + blockNum + " on "
                                     + this);
                    }
                ...
                if (!treatAsSuccess) {
                    ...
                    if ((dataRetries[blockNum] > maxRetries) && (maxRetries != -1)) {
                        ...
                        if (encoded && (dataBlocks[blockNum] != null)) {
                            ...
                            Logger.error(this,
                                         "Data block " + blockNum + " failed on " + this
                                         + " but bucket is already nulled out!");
                        }
                    }
                    ...
                    if ((dataBlocks[blockNum] != null) && encoded) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Repeated RNF, treating as success for data block " + blockNum + " on "
                                         + this);
                        }
                    } else {
                        Logger.error(this,
                                     "Data block " + blockNum + " succeeded (sort of) on " + this
                                     + " but bucket is already nulled out!");
                    }
                }
            }
        ...
        synchronized (this) {
            ...
            if (blockNum >= dataBlocks.length) {
                ...
                if (!checkFinished[checkNum]) {
                    ...
                    if (checkFailed[checkNum]) {
                        Logger.error(this,
                                     "Got onSuccess() but block has already failed! Check block " + checkNum + " on "
                                     + this);
                    } else {
                        Logger.error(this,
                                     "Got onSuccess() but block has already succeeded: Check block " + checkNum
                                     + " on " + this);
                    }
                ...
                if (checkBlocks[checkNum] != null) {
                    ...
                    Logger.error(this,
                                 "Check block " + checkNum + " succeeded on " + this
                                 + " but bucket is already nulled out!");
                }
            } else {
                ...
                if (!dataFinished[blockNum]) {
                    ...
                    if (dataFailed[blockNum]) {
                        Logger.error(this,
                                     "Got onSuccess() but block has already failed! Data block " + blockNum + " on "
                                     + this);
                    } else {
                        Logger.error(this,
                                     "Got onSuccess() but block has already succeeded: Data block " + blockNum + " on "
                                     + this);
                    }
                ...
                if (encoded && (dataBlocks[blockNum] != null)) {
                    ...
                    Logger.error(this,
                                 "Data block " + blockNum + " succeeded on " + this
                                 + " but bucket is already nulled out!");

                    if (persistent) {
                        Logger.minor(this, "Activation state: " + container.ext().isActive(this));
                    }
                }
            }
    ...
    protected static ClientCHKBlock encodeBucket(Bucket copyBucket, String compressorDescriptor, byte cryptoAlgorithm,
            byte[] cryptoKey)
    ...
    public void checkHasDataBlocks(boolean log, ObjectContainer container) {
        ...
        for (int i = 0; i < dataBlocks.length; i++) {
            ...
            if (data == null) {
                ...
                if (data.size() != CHKBlock.DATA_LENGTH) {
                    System.err.println("Size of data block " + i + " is " + data.size() + " should be "
                                       + CHKBlock.DATA_LENGTH);
                } else {
                    dataCount++;
                }
        ...
        for (int i = 0; i < checkBlocks.length; i++) {
            ...
            if (data == null) {
                ...
                if (data.size() != CHKBlock.DATA_LENGTH) {
                    System.err.println("Size of check block " + i + " is " + data.size() + " should be "
                                       + CHKBlock.DATA_LENGTH);
                } else {
                    checkCount++;
                }
    ...
    public int allocateCrossCheckBlock(SplitFileInserterCrossSegment seg, Random random) {
        ...
        for (int i = 0; i < crossCheckBlocks; i++) {
            ...
            if (x == dataBlocks.length) {
                x = dataBlocks.length - crossCheckBlocks;
            }
    ...
    public void onEncodedCrossCheckBlock(int blockNum, Bucket data, ObjectContainer container, ClientContext context) {
        synchronized (this) {
            ...
            if (logMINOR && (encodedCrossCheckBlocks != crossCheckBlocks)) {
                Logger.minor(this,
                             "Segment " + segNo + " has " + encodedCrossCheckBlocks + " encoded of " + crossCheckBlocks
                             + ", still waiting...");
            }
        }
    ...
    private static class BlockItem implements SendableRequestItem {
        ...
        BlockItem(SplitFileInserterSegment parent, int blockNum, Bucket bucket, boolean persistent,
    ...
    private static class BlockItemKey implements SendableRequestItemKey {
        ...
        public boolean equals(Object o) {
            if (o instanceof BlockItemKey) {
                if (((BlockItemKey) o).parent == parent && ((BlockItemKey) o).blockNum == blockNum) {
                    return true;
                }
            }
    ...
    static class MySendableRequestSender implements SendableRequestSender {
        ...
        public boolean send(NodeClientCore core, RequestScheduler sched, final ClientContext context,
            ...
            try {
                ...
                try {
                    encodedBlock = encodeBucket(block.copyBucket, compressorDescriptor, block.cryptoAlgorithm,
                                                block.cryptoKey);
                    b = encodedBlock.getBlock();
                } catch (CHKEncodeException e) {
                    throw new LowLevelPutException(LowLevelPutException.INTERNAL_ERROR,
                                                   e.toString() + ":" + e.getMessage() + " for " + block.copyBucket, e);
