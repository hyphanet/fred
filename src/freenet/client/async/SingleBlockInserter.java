...
public class SingleBlockInserter extends SendableInsert implements ClientPutState, Encodeable {
    ...
    public SingleBlockInserter(BaseClientPutter parent, Bucket data, short compressionCodec, FreenetURI uri,
                               InsertContext ctx, boolean realTimeFlag, PutCompletionCallback cb, boolean isMetadata,
                               int sourceLength, int token, boolean getCHKOnly, boolean addToParent,
                               boolean dontSendEncoded, Object tokenObject, ObjectContainer container,
                               ClientContext context, boolean persistent, boolean freeData, int extraInserts,
    ...
    protected ClientKeyBlock innerEncode(RandomSource random, ObjectContainer container) throws InsertException {
        ...
        boolean pre1254 = !((cmode == CompatibilityMode.COMPAT_CURRENT)
        ...
        try {
            return innerEncode(random, uri, sourceData, isMetadata, compressionCodec, sourceLength,
                               ctx.compressorDescriptor, pre1254, cryptoAlgorithm, cryptoKey);
    ...
    protected static ClientKeyBlock innerEncode(RandomSource random, FreenetURI uri, Bucket sourceData,
            boolean isMetadata, short compressionCodec, int sourceLength, String compressorDescriptor, boolean pre1254,
            byte cryptoAlgorithm, byte[] cryptoKey)
            throws InsertException, CHKEncodeException, IOException, SSKEncodeException, MalformedURLException,
        ...
        if (uriType.equals("CHK")) {
            return ClientCHKBlock.encode(sourceData, isMetadata, compressionCodec == -1, compressionCodec,
                                         sourceLength, compressorDescriptor, pre1254, cryptoKey, cryptoAlgorithm);
    ...
    protected ClientKeyBlock encode(ObjectContainer container, ClientContext context, boolean calledByCB)
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Encoded " + resultingURI + " for " + this + " shouldSend=" + shouldSend + " dontSendEncoded="
                         + dontSendEncoded);
        }
        ...
        synchronized (this) {
            if ((extraInserts > 0) &&!getCHKOnly) {
                if (++completedInserts <= extraInserts) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Completed inserts " + completedInserts + " of extra inserts " + extraInserts
                                     + " on " + this);
                    }
    ...
    @Override
    public SendableRequestItem chooseKey(KeysFetchingLocally ignored, ObjectContainer container,
    ...
    private BlockItem getBlockItem(BlockItemKey key, ObjectContainer container, ClientContext context)
            throws InsertException {
        try {
            synchronized (this) {
                if (finished) {
            ...
            if (u.getKeyType().equals("CHK") &&!persistent) {
            ...
            return new BlockItem(key, data, isMetadata, compressionCodec, sourceLength, u, persistent, pre1254,
...
//  
    ...
    private static class BlockItem implements SendableRequestItem {
        ...
        BlockItem(BlockItemKey key, Bucket bucket, boolean meta, short codec, int srclen, FreenetURI u,
                  boolean persistent, boolean pre1254, byte cryptoAlgorithm, byte[] cryptoKey) {
            this.key = key;
            ...
            try {
                try {
                    encodedBlock = innerEncode(context.random, block.uri, block.copyBucket, block.isMetadata,
                                               ...
                                               block.pre1254, block.cryptoAlgorithm, block.cryptoKey);
                ...
                if (req.localRequestOnly) {
                    try {
                        ...
                        KeyBlock collided = core.node.fetch(k.getNodeKey(), true, req.canWriteClientCache, false,
                                                false, null);
                ...
                if (e.code == LowLevelPutException.COLLISION) {
                    ...
                    try {
                        ...
                        if ((collided.isMetadata() == block.isMetadata)
                            ...
                            if (SingleBlockInserter.logMINOR) {
                                Logger.minor(this,
                                             "Apparently real collision: collided.isMetadata=" + collided.isMetadata()
                                             + " block.isMetadata=" + block.isMetadata + " collided.codec="
                                             + collided.getCompressionCodec() + " block.codec="
