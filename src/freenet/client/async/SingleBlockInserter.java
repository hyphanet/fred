...
public class SingleBlockInserter
	...
	protected ClientKeyBlock innerEncode(RandomSource random)
		...
		try {
			...
			Logger.error(
				SingleBlockInserter.class,
				"Caught " + e + " encoding data " + sourceData,
				e
			);
			throw new InsertException(
	...
	protected static ClientKeyBlock innerEncode(
		...
		throws InsertException, CHKEncodeException, IOException, SSKEncodeException, MalformedURLException, InvalidCompressionCodecException {
	...
	protected ClientKeyBlock encode(ClientContext context, boolean calledByCB)
		...
		synchronized (this) {
			...
			if (sourceData == null) {
				Logger.error(
					this,
					"Source data is null on " + this + " but not finished!"
				);
				return null;
			}
		...
		if (shouldSend && !dontSendEncoded) cb.onEncode(
			block.getClientKey(),
			this,
			context
		);
	...
	public void onFailure(
		...
		switch (e.code) {
			...
			default:
				Logger.error(
					this,
					"Unknown LowLevelPutException code: " + e.code
				);
				errors.inc(InsertExceptionMode.INTERNAL_ERROR);
		}
		if (
			e.code == LowLevelPutException.ROUTE_NOT_FOUND ||
		...
		if ((retries > ctx.maxInsertRetries) && (ctx.maxInsertRetries != -1)) {
			fail(
				InsertException.construct(persistent ? errors.clone() : errors),
				context
			);
			return;
		}
	...
	public void onSuccess(
		...
		synchronized (this) {
			...
			if (resultingKey == null) {
				...
				if (!resultingKey.equals(key)) Logger.error(
					this,
					"Different key: " +
					resultingKey +
					" -> " +
		...
		if (shouldSendKey) cb.onEncode(key, this, context); // In case of race conditions etc, especially for LocalRequestOnly.
	...
	static class MySendableRequestSender implements SendableRequestSender {
		...
		public boolean send(
			...
			final ClientKey key;
			ClientKey k = null;
			if (SingleBlockInserter.logMINOR) Logger.minor(
				this,
				"Starting request"
			);
			...
			try {
				...
				if (req.localRequestOnly) try {
					...
					KeyBlock collided = core.node.fetch(
						k.getNodeKey(),
						true,
						req.canWriteClientCache,
						false,
				...
				if (e.code == LowLevelPutException.COLLISION) {
					// Collision
					try {
						ClientSSKBlock collided = ClientSSKBlock.construct(
							((SSKBlock) e.getCollidedBlock()),
							(ClientSSK) k
						);
						byte[] data = collided.memoryDecode(true);
						byte[] inserting = BucketTools.toByteArray(
							block.copyBucket
						);
						if (
							collided.isMetadata() == block.isMetadata &&
						...
						Logger.error(
							this,
							"Caught " + e1 + " when checking collision!",
							e1
						);
	...
	private BlockItem getBlockItem(BlockItemKey key, ClientContext context)
		throws InsertException {
		try {
			synchronized (this) {
				if (finished) return null;
	...
	public void innerOnResume(ClientContext context)
		...
		if (resultingKey != null) cb.onEncode(
			resultingKey,
