...
class ClientRequestSelector implements KeysFetchingLocally {
    private static final short[] tweakedPrioritySelector = {
        ...
        RequestStarter.MAXIMUM_PRIORITY_CLASS, RequestStarter.MAXIMUM_PRIORITY_CLASS,
        RequestStarter.MAXIMUM_PRIORITY_CLASS, RequestStarter.MAXIMUM_PRIORITY_CLASS,
        RequestStarter.MAXIMUM_PRIORITY_CLASS, RequestStarter.INTERACTIVE_PRIORITY_CLASS,
        RequestStarter.INTERACTIVE_PRIORITY_CLASS, RequestStarter.INTERACTIVE_PRIORITY_CLASS,
        RequestStarter.INTERACTIVE_PRIORITY_CLASS, RequestStarter.INTERACTIVE_PRIORITY_CLASS,
        RequestStarter.INTERACTIVE_PRIORITY_CLASS, RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
        ...
        RequestStarter.BULK_SPLITFILE_PRIORITY_CLASS, RequestStarter.PREFETCH_PRIORITY_CLASS,
        RequestStarter.PREFETCH_PRIORITY_CLASS, RequestStarter.MINIMUM_PRIORITY_CLASS
    };
    private static final short[] prioritySelector = {
        RequestStarter.MAXIMUM_PRIORITY_CLASS, RequestStarter.INTERACTIVE_PRIORITY_CLASS,
        RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, RequestStarter.UPDATE_PRIORITY_CLASS,
        RequestStarter.BULK_SPLITFILE_PRIORITY_CLASS, RequestStarter.PREFETCH_PRIORITY_CLASS,
    ...
    private long removeFirstAccordingToPriorities(int fuzz, RandomSource random, ClientRequestSchedulerCore schedCore,
            ClientRequestSchedulerNonPersistent schedTransient, boolean transientOnly, short maxPrio,
                    ...
                    if (cooldownTime > 0) {
                        ...
                        if (logMINOR) {
                            if (cooldownTime == Long.MAX_VALUE) {
                                Logger.minor(this,
                                             "Priority " + priority
                                             + " (persistent) is waiting until a request finishes or is empty");
                            } else {
                                Logger.minor(this,
                                             "Priority " + priority + " (persistent) is in cooldown for another "
                    ...
                    if (cooldownTime > 0) {
                        ...
                        if (logMINOR) {
                            if (cooldownTime == Long.MAX_VALUE) {
                                Logger.minor(this,
                                             "Priority " + priority
                                             + " (transient) is waiting until a request finishes or is empty");
                            } else {
                                Logger.minor(this,
                                             "Priority " + priority + " (transient) is in cooldown for another "
                                             + (cooldownTime - now) + " " + TimeUtil.formatTime(cooldownTime - now));
                            }
                        }
    ...
    ChosenBlock removeFirstTransient(int fuzz, RandomSource random, OfferedKeysList offeredKeys,
                                     RequestStarter starter, ClientRequestSchedulerNonPersistent schedTransient,
        ...
        for (int i = 0; i < 5; i++) {
            ...
            synchronized (sched) {
                r = removeFirstInner(fuzz, random, offeredKeys, starter, null, schedTransient, true, false, maxPrio,
                                     realTime, context, container, now);
            }
            ...
            if (isInsertScheduler && (req instanceof SendableGet)) {
                IllegalStateException e =
                    new IllegalStateException("removeFirstInner returned a SendableGet on an insert scheduler!!");
    ...
    public ChosenBlock maybeMakeChosenRequest(SendableRequest req, ObjectContainer container, ClientContext context,
        ...
        if (token == null) {
            ...
            ret = new TransientChosenBlock(req, token, key, ckey, localRequestOnly, ignoreStore, canWriteClientCache,
                                           forkOnCacheable, realTimeFlag, sched);
    ...
    SelectorReturn removeFirstInner(int fuzz, RandomSource random, OfferedKeysList offeredKeys, RequestStarter starter,
                                    ClientRequestSchedulerCore schedCore,
                                    ClientRequestSchedulerNonPersistent schedTransient, boolean transientOnly,
                                    boolean notTransient, short maxPrio, boolean realTime, ClientContext context,
        ...
        long l = removeFirstAccordingToPriorities(fuzz, random, schedCore, schedTransient, transientOnly, maxPrio,
                     container, context, now);
...
outer:
            ...
            while (true) {
                ...
                if ((perm == null) && (trans == null)) {
                    ...
                    if (cooldownTime > 0) {
                        ...
                        Logger.normal(this,
                                      "Priority " + choosenPriorityClass + " (transient) is in cooldown for another "
                                      + (cooldownTime - now) + " " + TimeUtil.formatTime(cooldownTime - now));
                    ...
                    if (cooldownTime > 0) {
                        ...
                        Logger.normal(this,
                                      "Priority " + choosenPriorityClass + " (persistent) is in cooldown for another "
                                      + (cooldownTime - now) + " " + TimeUtil.formatTime(cooldownTime - now));
                    ...
                    if (cooldownTime > 0) {
                        ...
                        Logger.normal(this,
                                      "Priority " + choosenPriorityClass + " (perm=" + choosePerm
                                      + ") is in cooldown for another " + (cooldownTime - now) + " "
                ...
                RemoveRandomReturn val = chosenTracker.removeRandom(starter, persistent ? container : null, context,
                                             now);
                SendableRequest req;
                ...
                if (val == null) {
                    ...
                    if (val.wakeupTime == -1) {
                        ...
                        Logger.normal(this,
                                      "Priority " + choosenPriorityClass + " returned cooldown time of "
                ...
                if (chosenTracker.persistent() != persistent) {
                    Logger.error(this,
                                 "Tracker.persistent()=" + chosenTracker.persistent()
                                 + " but is in the queue for persistent=" + persistent + " for " + chosenTracker);
                ...
                if (req.persistent() != persistent) {
                    Logger.error(this,
                                 "Request.persistent()=" + req.persistent() + " but is in the queue for persistent="
                                 + chosenTracker.persistent() + " for " + req);
                ...
                if (req.getPriorityClass(container) != choosenPriorityClass) {
                    ...
                    Logger.normal(this,
                                  "In wrong priority class: " + req + " (req.prio=" + req.getPriorityClass(container)
                                  + " but chosen=" + choosenPriorityClass + ')');

                    // Remove it.
                    SectoredRandomGrabArrayWithObject clientGrabber =
                        (SectoredRandomGrabArrayWithObject) chosenTracker.getGrabber(req.getClient(container));
                    ...
                    if (clientGrabber != null) {
                        ...
                        Logger.error(this,
                                     "Could not find client grabber for client " + req.getClient(container) + " from "
                                     + chosenTracker);
                    }
                ...
                if (!req.persistent() &&!isInsertScheduler) {
                    ...
                    if ((altReq != null) && (l = altReq.getCooldownTime(container, context, now)) != 0) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Ignoring recently succeeded item, cooldown time = " + l
                        ...
                        if (prio <= choosenPriorityClass) {
                            ...
                            if (logMINOR) {
                                Logger.minor(this,
                                             ...
                                             + altReq.getPriorityClass(container) + ") is better than " + req
                                             + " (prio=" + req.getPriorityClass(container) + "), using that");
                            }
                            ...
                            if (logMINOR) {
                                Logger.minor(this,
                                             "Chosen req " + req + " is better, reregistering recently succeeded "
                                             + altReq);
                            }
                        ...
                        if (container.ext().isStored(altRGA) &&!altRGA.isEmpty(container)) {
                            ...
                            if (val != null) {
                                if (val.item == null) {
                                    if (logMINOR) {
                                        Logger.minor(
                                            this,
                                            "Ignoring recently succeeded item, removeRandom returned cooldown time "
                                            + val.wakeupTime
                                            + ((val.wakeupTime > 0)
                            ...
                            if ((altReq != null) && (altReq != req)) {
                                ...
                                if (useRecent) {
                                    ...
                                    if (logMINOR) {
                                        Logger.minor(this,
                                                     "Recently succeeded (persistent) req " + altReq + " (prio="
                                                     + altReq.getPriorityClass(container) + ") is better than " + req
                                                     + " (prio=" + req.getPriorityClass(container) + "), using that");
                                    }
                                    ...
                                    if (logMINOR) {
                                        Logger.minor(this,
                                                     "Chosen (persistent) req " + req
                                                     + " is better, reregistering recently succeeded " + altRGA
                                                     + " for " + altReq);
                                    }
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "removeFirst() returning " + req + " (prio " + req.getPriorityClass(container)
                                 + ", client " + req.getClient(container) + ", client-req " + req.getClientRequest()
                                 + ')');
                }
    ...
    public void succeeded(BaseSendableGet succeeded, ObjectContainer container) {
        ...
        synchronized (recentSuccesses) {
            if (recentSuccesses.contains(array)) {
                return;
            }
