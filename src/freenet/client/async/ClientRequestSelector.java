...
public class ClientRequestSelector implements KeysFetchingLocally {
	...
	static class ClientRequestRGANode
		extends SectoredRandomGrabArraySimple<
			RequestClient,
	...
	ClientRequestSelector(
		...
		priorities =
			new RequestClientRGANode[RequestStarter.NUMBER_OF_PRIORITY_CLASSES];
	}
	...
	private synchronized long choosePriority(
		...
		while (iteration++ < RequestStarter.NUMBER_OF_PRIORITY_CLASSES + 1) {
			priority = fuzz < 0
				? tweakedPrioritySelector[random.nextInt(
						tweakedPrioritySelector.length
					)]
			...
			if (result != null) {
				...
				if (cooldownTime > 0) {
					...
					if (logMINOR) {
						if (cooldownTime == Long.MAX_VALUE) Logger.minor(
							this,
							"Priority " +
							priority +
							" is waiting until a request finishes or is empty"
						);
						else Logger.minor(
							this,
							"Priority " +
			...
			if (logMINOR) Logger.minor(
				this,
				"Priority " + priority + " is null (fuzz = " + fuzz + ')'
			);
			fuzz++;
		}
	...
	ChosenBlock chooseRequest(
		...
		for (int i = 0; i < 5; i++) {
			SelectorReturn r = chooseRequestInner(
				fuzz,
				random,
				offeredKeys,
				starter,
	...
	public ChosenBlock maybeMakeChosenRequest(
		...
		if (token == null) {
			...
			if (isInsertScheduler) {
				...
				if (req instanceof SendableGet) ckey =
					((SendableGet) req).getKey(token);
				else ckey = null;
			}
			ChosenBlock ret;
			if (
				key != null && key.getRoutingKey() == null
			...
			if (req instanceof SendableGet) {
				...
				if (req instanceof SendableInsert) {
					canWriteClientCache =
						((SendableInsert) req).canWriteClientCache();
					forkOnCacheable = ((SendableInsert) req).forkOnCacheable();
					localRequestOnly =
						((SendableInsert) req).localRequestOnly();
	...
	SelectorReturn chooseRequestInner(
		...
		if (tryOfferedKeys) {
			if (
				offeredKeys.getWakeupTime(context, now) == 0
		...
		if (l > Integer.MAX_VALUE) {
			if (logMINOR) Logger.minor(
				this,
				"No priority available for the next " +
				TimeUtil.formatTime(l - now)
			);
		...
		outer:for (
			...
			if (logMINOR) Logger.minor(
				this,
				"Using priority " + choosenPriorityClass
			);
			RequestClientRGANode chosenTracker =
				priorities[choosenPriorityClass];
			if (chosenTracker == null) {
				if (logMINOR) Logger.minor(
					this,
					"No requests to run: chosen priority empty"
				);
			...
			while (true) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Got priority tracker " + chosenTracker
				);
				...
				if (val == null) {
					...
					if (val.wakeupTime == -1) Logger.normal(
						this,
						"Priority " +
						choosenPriorityClass +
						...
						if (
							val.wakeupTime > 0 && val.wakeupTime < wakeupTime
				...
				if (req.getPriorityClass() != choosenPriorityClass) {
					...
					// Remove it.
					ClientRequestRGANode clientGrabber =
						chosenTracker.getGrabber(req.getClient());
					if (clientGrabber != null) {
						RandomGrabArray baseRGA = clientGrabber.getGrabber(
							req.getSchedulerGroup()
						);
						if (baseRGA != null) {
				...
				if (!isInsertScheduler) {
					...
					if (altReq != null && (altReq.isCancelled())) {
						if (logMINOR) Logger.minor(
							this,
							"Ignoring cancelled recently succeeded item " +
							altReq
						);
						altReq = null;
					}
					if (
						altReq != null &&
						(l = altReq.getWakeupTime(context, now)) != 0
					) {
						if (logMINOR) {
							Logger.minor(
								this,
					...
					if (altReq != null && altReq != req) {
						...
						if (prio <= choosenPriorityClass) {
							...
							if (logMINOR) Logger.minor(
								...
								"Chosen req " +
								req +
								" is better, reregistering recently succeeded " +
								altReq
							);
							synchronized (recentSuccesses) {
	...
	public boolean hasKey(Key key, BaseSendableGet getterWaiting) {
		...
		synchronized (keysFetching) {
			...
			if (getterWaiting != null) {
				WeakReference<BaseSendableGet>[] waiting =
					transientRequestsWaitingForKeysFetching.get(key);
				if (waiting == null) {
					...
					WeakReference<BaseSendableGet>[] newWaiting = Arrays.copyOf(
						waiting,
						waiting.length + 1
					);
					newWaiting[waiting.length] = new WeakReference<
						BaseSendableGet
					>(getterWaiting);
					transientRequestsWaitingForKeysFetching.put(
						key,
	...
	public void removeFetchingKey(final Key key) {
		...
		if (key != null) {
			synchronized (keysFetching) {
				...
				transientWaiting =
					this.transientRequestsWaitingForKeysFetching.remove(key);
			}
			if (transientWaiting != null) {
				if (transientWaiting != null) {
					for (WeakReference<
						BaseSendableGet
					> ref : transientWaiting) {
	...
	public void removeRunningInsert(SendableRequestItemKey token) {
		if (logMINOR) Logger.minor(
			this,
			"Removing from runningInserts: " + token
		);
		synchronized (runningInserts) {
	...
	void addToGrabArray(
		...
		// Client
		synchronized (this) {
			ClientRequestRGANode requestGrabber = makeSRGAForClient(
				priorityClass,
				client,
				context
			);
	...
	private ClientRequestRGANode makeSRGAForClient(
		...
		if (clientGrabber == null) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Registering client tracker for priority " +
				priorityClass +
				" : " +
				clientGrabber
			);
		...
		if (requestGrabber == null) {
			requestGrabber = new ClientRequestRGANode(
				client,
				clientGrabber,
				this
			);
	...
	public void reregisterAll(
		...
		synchronized (this) {
			...
			if (clientGrabber == null) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Changing priority but request not running " + request,
					new Exception("debug")
				);
				return;
			}
			// Then by RequestClient
			ClientRequestRGANode requestGrabber = clientGrabber.getGrabber(
				client
			);
			if (requestGrabber == null) {
				if (logMINOR) Logger.minor(
			...
			RandomGrabArrayWithObject<ClientRequestSchedulerGroup> rga =
	...
	public synchronized long countQueuedRequests(ClientContext context) {
		...
		for (int i = 0; i < priorities.length; i++) {
			...
			if (prio == null || prio.isEmpty()) System.out.println(
				"Priority " + i + " : empty"
			);
			else {
				...
				for (int k = 0; k < prio.size(); k++) {
					...
					ClientRequestRGANode requestGrabber = prio.getGrabber(
						client
					);
	...
	void innerRegister(
		SendableRequest req,
		ClientContext context,
		SendableRequest[] maybeActive
	) {
		if (
			isInsertScheduler && req instanceof BaseSendableGet
		) throw new IllegalArgumentException(
			"Adding a SendableGet to an insert scheduler!!"
		);
		if (
			(!isInsertScheduler) && req instanceof SendableInsert
		) throw new IllegalArgumentException(
			"Adding a SendableInsert to a request scheduler!!"
		);
		if (
			isInsertScheduler != req.isInsert()
		) throw new IllegalArgumentException(
			"Request isInsert=" +
			req.isInsert() +
		...
		if (logMINOR) Logger.minor(
			this,
			"Registered " + req + " on prioclass=" + prio
		);
	}
	...
	public void succeeded(BaseSendableGet succeeded) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Recording successful fetch from " + succeeded
		);
		synchronized (recentSuccesses) {
