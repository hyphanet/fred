...
class KeyListenerTracker implements KeySalter {
	...
	protected KeyListenerTracker(
		...
		singleKeyListeners = this.isSSKScheduler
			? new TreeMap<ByteArrayWrapper, Object>(
	...
	public void addPendingKeys(KeyListener listener) {
		...
		ByteArrayWrapper wrapper = wantedKey != null
			? new ByteArrayWrapper(saltKey(wantedKey))
			: null;
		assert (Arrays.equals(
				wantedKey,
				listener.getHasKeyListener().getWantedKey()
			));
		synchronized (this) {
			...
			if (wantedKey != null) {
				...
				if (o == null) {
					...
					KeyListener[] newListeners = Arrays.copyOf(
						listeners,
						listeners.length + 1
					);
			...
			if (wantedKey != null) {
				...
				if (o == null) {
					...
					KeyListener[] newListeners =
						new KeyListener[listeners.length - 1];
					int x = 0;
					for (KeyListener l : listeners) {
						if (listener == l) {
							assert (!ret);
		...
		synchronized (this) {
			if (wantedKey != null) {
				...
				if (o == null) {
					...
					if (
						(ret = ((KeyListener) o).getHasKeyListener() ==
							hasListener)
					) {
						singleKeyListeners.remove(wrapper);
					...
					if (!ret) {
						...
						if (x < newListeners.length) newListeners =
							Arrays.copyOf(newListeners, x);
						if (newListeners.length == 0) {
							singleKeyListeners.remove(wrapper);
						} else if (newListeners.length == 1) {
							singleKeyListeners.put(wrapper, newListeners[0]);
			...
			for (
				Iterator<KeyListener> i = keyListeners.iterator();
				i.hasNext();
			) {
	...
	private synchronized ArrayList<KeyListener> probablyMatches(
		Key key,
		byte[] saltedKey
	) {
	...
	public short getKeyPrio(Key key, short priority, ClientContext context) {
		...
		for (KeyListener listener : matches) {
			short prio;
			try {
				prio = listener.definitelyWantKey(
					key,
					saltedKey,
					sched.clientContext
				);
				...
				Logger.error(
					this,
					format(
						"Error in definitelyWantKey callback for %s",
						listener
					),
					t
				);
	...
	public synchronized long countWaitingKeys() {
		...
		for (Object o : singleKeyListeners.values()) {
			if (o == null) {
				...
				@SuppressWarnings("unchecked")
				KeyListener[] listeners = (KeyListener[]) o;
				for (KeyListener listener : listeners) count +=
				listener.countKeys();
			}
		}
		for (KeyListener listener : keyListeners) {
			try {
				...
				Logger.error(
					this,
					format("Error in countKeys callback for %s", listener),
					t
				);
			}
		}
	...
	public synchronized boolean anyProbablyWantKey(
		Key key,
		...
		for (KeyListener listener : keyListeners) {
			try {
				...
				Logger.error(
					this,
					format(
						"Error in probablyWantKey callback for %s",
						listener
					),
					t
				);
			...
			for (KeyListener listener : matches) {
				try {
					...
					Logger.error(
						this,
						format(
							"Error in handleBlock callback for %s",
							listener
						),
						t
					);
				...
				if (listener.isEmpty()) {
					try {
						...
						Logger.error(
							this,
							format("Error while removing %s", listener),
							t
						);
					}
				}
		...
		for (KeyListener listener : matches) {
			...
			try {
				...
				Logger.error(
					this,
					format(
						"Error in getRequestsForKey callback for %s",
						listener
					),
					t
				);
