...
public abstract class BaseManifestPutter extends ManifestPutter {
	...
	private final class ArchivePutHandler extends PutHandler {
		...
		private ArchivePutHandler(
			...
			this.origSFI = new ContainerInserter(
				this,
				this,
				data,
				insertURI,
		...
		public void onEncode(
			...
			synchronized (BaseManifestPutter.this) {
				...
				ArrayList<PutHandler> phv = putHandlersArchiveTransformMap.get(
					this
				);
				if (phv == null) return; // Already encoded.
				for (PutHandler ph : phv) {
					HashMap<String, Object> hm = putHandlersTransformMap.get(
						ph
					);
					perContainerPutHandlersWaitingForMetadata
						.get(ph.parentPutHandler)
						.remove(ph);
					if (
						ph.targetInArchive == null
					...
					Metadata m = new Metadata(
						...
						key
							.getURI()
							.setMetaString(new String[] { ph.targetInArchive }),
						cm
					);
		...
		public void onSuccess(ClientPutState state, ClientContext context) {
			if (logMINOR) Logger.minor(
				this,
				"Completed '" + this.itemName + "' " + this
			);
			if (
				!containerPutHandlers.remove(this)
			) throw new IllegalStateException(
				"was not in containerPutHandlers"
			);
	...
	private final class ContainerPutHandler extends PutHandler {
		...
		public void onSuccess(ClientPutState state, ClientContext context) {
			...
			if (rootContainerPutHandler == this) {
				if (
					containerPutHandlers.contains(this)
				) throw new IllegalStateException(
					"was in containerPutHandlers"
				);
	...
	private final class ExternPutHandler extends PutHandler {
		...
		private ExternPutHandler(
			...
			this.origSFI = new SingleFileInserter(
				this,
				this,
				block,
				false,
		...
		public void onMetadata(
			...
			if (freeformMode) {
				...
				synchronized (BaseManifestPutter.this) {
					...
					if (!allMetadatas) {
						if (logMINOR) Logger.minor(
							...
							putHandlersWaitingForMetadata.size()
						);
					}
				}
				if (allMetadatas) {
					...
					try {
						if (
							m.writtenLength() > Metadata.MAX_SIZE_IN_MANIFEST
				...
				throw new RuntimeException(
					"Neiter container nor freeform mode. Hu?"
				);
			}
	...
	private final class MetaPutHandler extends PutHandler {
		...
		private MetaPutHandler(
			...
			if (logMINOR) Logger.minor(
				this,
				"Inserting root metadata: " + origSFI
			);
		}
	...
	private final class JokerPutHandler extends PutHandler {
		...
		public JokerPutHandler(
			...
			Metadata m = new Metadata(
				DocumentType.SYMBOLIC_SHORTLINK,
				null,
				null,
				target,
				null
	...
	abstract class PutHandler
		...
		private PutHandler(
			...
			if (runningMap != null) {
				synchronized (runningMap) {
					if (runningMap.contains(this)) {
						Logger.error(
							this,
							"PutHandler already in 'runningMap': " + runningMap,
							new Error("error")
						);
					} else {
						runningMap.add(this);
			...
			synchronized (putHandlerWaitingForBlockSets) {
				if (putHandlerWaitingForBlockSets.contains(this)) {
					Logger.error(
						this,
						"PutHandler already in 'waitingForBlockSets'!",
						new Error("error")
					);
				} else {
			...
			synchronized (putHandlersWaitingForFetchable) {
				if (putHandlersWaitingForFetchable.contains(this)) {
					Logger.error(
						this,
						"PutHandler already in 'waitingForFetchable'!",
						new Error("error")
					);
				} else {
		...
		public void start(ClientContext context) throws InsertException {
			...
			if (logDEBUG) Logger.debug(
				this,
				"Starting a PutHandler for '" + this.itemName + "' " + this
			);

			if (origSFI == null) {
				fail(
					new IllegalStateException(
						"origSFI is null on start(), impossible"
					),
					context
				);
			}
			...
			if ((!(this instanceof MetaPutHandler)) && (metadata != null)) {
				fail(
					new IllegalStateException(
						"metdata=" + metadata + " on start(), impossible"
					),
					context
				);
			}
			...
			boolean ok;
			if (
				(this instanceof ContainerPutHandler) ||
				(this instanceof ArchivePutHandler)
			) {
				if (this != rootContainerPutHandler) {
					...
					if (!ok) {
						throw new IllegalStateException(
							"Starting a PutHandler thats not in 'containerPutHandlers'! " +
							this
						);
					}
				}
				...
				if (this != rootMetaPutHandler) {
					...
					if (!ok) {
						throw new IllegalStateException(
							"Starting a PutHandler thats not in 'runningPutHandlers'! " +
							this
						);
					}
				}
		...
		public boolean isFinished() {
			...
			return (
				BaseManifestPutter.this.finished ||
				cancelled ||
		...
		public void onSuccess(ClientPutState state, ClientContext context) {
			if (logDEBUG) {
				...
				if (
					!("BaseManifestPutter.java".equals(te.getFileName()) &&
			...
			if (
				putHandlersWaitingForFetchable.contains(this)
			...
			synchronized (BaseManifestPutter.this) {
				...
				if (!runningPutHandlers.isEmpty()) {
					if (logMINOR) {
						Logger.minor(
							this,
							"Running put handlers: " + runningPutHandlers.size()
						);
		...
		public void onBlockSetFinished(
			...
			synchronized (BaseManifestPutter.this) {
				...
				if (freeformMode) {
					allBlockSets = hasResolvedBase &&
					putHandlerWaitingForBlockSets.isEmpty();
				} else {
					allBlockSets = putHandlerWaitingForBlockSets.isEmpty();
				}
			}
			if (allBlockSets) BaseManifestPutter.this.blockSetFinalized(
					context
				);
		}
	...
	private static final String[] defaultDefaultNames = new String[] {
		"index.html",
		"index.htm",
		"default.html",
		"default.htm",
	...
	public BaseManifestPutter(
		...
		if (
			!(mode == CompatibilityMode.COMPAT_CURRENT ||
		...
		putHandlersArchiveTransformMap = new HashMap<
			...
			ArrayList<PutHandler>
		>();
		if (defaultName == null) defaultName = findDefaultName(
			manifestElements
		);
	...
	private String findDefaultName(HashMap<String, Object> manifestElements) {
		...
		for (String name : defaultDefaultNames) {
			...
			for (Map.Entry<
				String,
				Object
			> entry : manifestElements.entrySet()) {
	...
	public void start(ClientContext context) throws InsertException {
		...
		synchronized (this) {
			running = runningPutHandlers.toArray(
				new PutHandler[runningPutHandlers.size()]
			);
			if (containerMode) {
		...
		try {
			...
			if (containerMode) {
				for (int i = 0; i < containers.length; i++) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Started " + i + " of " + containers.length
					);
					if (isFinished()) {
						if (logMINOR) Logger.minor(
							this,
	...
	private PutHandler[] getContainersToStart(boolean excludeRoot) {
		PutHandler[] maybeStartPH = containerPutHandlers.toArray(
			new PutHandler[containerPutHandlers.size()]
		);
	...
	private void resolveAndStartBase(ClientContext context) {
		...
		while (true) {
			try {
				bucket = baseMetadata.toBucket(
					context.getBucketFactory(persistent())
				);
				if (logMINOR) Logger.minor(
					this,
					"Metadata bucket is " + bucket.size() + " bytes long"
				);
				break;
			} catch (IOException e) {
				fail(
				...
				try {
					...
					if (logMINOR) Logger.minor(
						this,
						"Main metadata needs resolving: " + e
					);
					resolve(e, context);
					return;
				} catch (IOException e1) {
		...
		for (Metadata m : metas) {
			...
			if (m.isResolved()) {
				Logger.error(
					...
					"Already resolved: " +
					m +
					" in resolve() - race condition???"
				);
				continue;
			}
			try {
				MetaPutHandler ph = new MetaPutHandler(
					this,
					null,
					m,
					context.getBucketFactory(persistent())
	...
	private void tryComplete(ClientContext context) {
		...
		if (logDEBUG) Logger.debug(
			this,
			"try complete",
			new Error("trace tryComplete()")
		);
		synchronized (this) {
			if (finished || cancelled) {
				if (logMINOR) Logger.minor(
					this,
					"Already " + (finished ? "finished" : "cancelled")
				);
				return;
			}
			if (!runningPutHandlers.isEmpty()) {
				if (logDEBUG) Logger.debug(
					this,
					"Not finished, runningPutHandlers not empty."
				);
				return;
			}
			if (!containerPutHandlers.isEmpty()) {
				if (logDEBUG) Logger.debug(
					this,
					"Not finished, containerPutHandlers not empty."
				);
				return;
			}
			if (containerMode) {
				if (rootContainerPutHandler != null) {
					if (logDEBUG) Logger.debug(
						this,
						"Not finished, rootContainerPutHandler not empty."
					);
					return;
				}
			} else {
				if (rootMetaPutHandler != null) {
					if (logDEBUG) Logger.debug(
						this,
						"Not finished, rootMetaPutHandler not empty."
					);
					return;
				}
			}
	...
	private void cancelAndFinish(ClientContext context) {
		...
		if (logMINOR) Logger.minor(
			this,
			"PutHandler's to cancel: " + running.length
		);
	...
	private synchronized boolean checkFetchable(PutHandler handler) {
		...
		if (!putHandlersWaitingForFetchable.remove(handler)) {
			throw new IllegalStateException(
				"was not in putHandlersWaitingForFetchable! : " + handler
			);
		}
	...
	public void onTransition(
		...
		Logger.error(
			this,
			"Ignoring transition from " + from + " to " + to + " on " + this
		);
		// Ignore
	}
	...
	private void tryStartParentContainer(
		PutHandler containerHandle2,
		ClientContext context
	) throws InsertException {
		//new Error("RefactorME").printStackTrace();
	...
	protected final ClientMetadata guessMime(String name, String mimetype) {
		...
		if ((mimeType == null) && (name != null)) mimeType =
	...
	protected ContainerBuilder getRootContainer() {
		if (freeformMode) throw new IllegalStateException(
			"Already in freeform mode!"
		);
		if (!containerMode) {
			containerMode = true;
	...
	protected abstract class ManifestBuilder implements Serializable {
		...
		private HashMap<String, Object> makeSubDir(
			HashMap<String, Object> parentDir,
			String name
		) {
		...
		public final void addElement(
			...
			throw new IllegalStateException(
				"ME is neither a redirect nor dircet data. " + element
			);
		}
	...
	protected final class ContainerBuilder extends ManifestBuilder {
		...
		private ContainerBuilder(
			...
			if (!containerMode) {
				throw new IllegalStateException(
					"You can not add containers in free form mode!"
				);
			}
			_rootDir = new HashMap<String, Object>();
			if (isArchive) selfHandle = new ArchivePutHandler(
				BaseManifestPutter.this,
				parent,
				name,
				_rootDir,
			...
			else selfHandle = new ContainerPutHandler(
				BaseManifestPutter.this,
				parent,
				name,
				_rootDir,
			...
			perContainerPutHandlersWaitingForMetadata.put(
				selfHandle,
				new HashSet<PutHandler>()
			);
			//perContainerPutHandlersWaitingForFetchable.put(selfHandle, new HashSet<PutHandler>());
			if (isArchive) putHandlersArchiveTransformMap.put(
				(ArchivePutHandler) selfHandle,
				new ArrayList<PutHandler>()
			);
		}
		...
		public void addItem(
			...
			ManifestElement me = new ManifestElement(
				element,
				name,
				nameInArchive
			);
		...
		public void addExternal(
			...
			PutHandler ph = new ExternPutHandler(
				BaseManifestPutter.this,
				selfHandle,
				name,
				data,
		...
		public void addArchiveItem(
			...
			archive.addItem(
				name,
				new ManifestElement(element, name, name),
				false
			);
			PutHandler ph = new JokerPutHandler(
				BaseManifestPutter.this,
				selfHandle,
				name,
	...
	public static HashMap<String, Object> bucketsByNameToManifestEntries(
		...
		for (Map.Entry<String, Object> entry : bucketsByName.entrySet()) {
			...
			if (o instanceof ManifestElement) {
				...
				manifestEntries.put(
					name,
					new ManifestElement(name, data, null, data.size())
				);
			} else if (o instanceof HashMap) {
				manifestEntries.put(
					name,
					bucketsByNameToManifestEntries(Metadata.forceMap(o))
				);
	...
	public static ManifestElement[] flatten(
		HashMap<String, Object> manifestElements
	) {
	...
	public void onShutdown(ClientContext context) {
		...
		if (rootContainerPutHandler != null) rootContainerPutHandler.onShutdown(
			context
		);
	...
	protected void innerOnResume(ClientContext context)
		...
		if (rootContainerPutHandler != null) rootContainerPutHandler.onResume(
			context
		);
		if (containerPutHandlers != null) {
