...
abstract class ClientRequestSchedulerBase {
     ...
     */
    void innerRegister(SendableRequest req, ObjectContainer container, ClientContext context,
        ...
        if (req.persistent() != persistent()) {
            throw new IllegalArgumentException("innerRegister for persistence=" + req.persistent()
                                               + " but our persistence is " + persistent());
        }
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Still registering " + req + " at prio " + prio + " for " + req.getClientRequest() + " ssk="
    ...
    protected void addToRequestsByClientRequest(ClientRequester clientRequest, SendableRequest req,
        ...
        if ((clientRequest != null) || persistent()) {    // Client request null is only legal for transient requests
            ...
            if (deactivate) {
                container.deactivate(clientRequest, 1);
            }
        }
    }
     ...
     */
    void addToGrabArray(short priorityClass, RequestClient client, ClientRequester cr, SendableRequest req,
        ...
        if ((priorityClass > RequestStarter.MINIMUM_PRIORITY_CLASS)
            ...
            throw new IllegalStateException("Invalid priority: " + priorityClass + " - range is "
                                            + RequestStarter.MAXIMUM_PRIORITY_CLASS + " (most important) to "
                                            + RequestStarter.MINIMUM_PRIORITY_CLASS + " (least important)");
        }
        ...
        synchronized (this) {
            ...
            synchronized (clientGrabber) {
                ...
                if (requestGrabber == null) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Creating new grabber: " + requestGrabber + " for " + client + " from "
    ...
    public void reregisterAll(ClientRequester request, RequestScheduler lock, ObjectContainer container,
                              ClientContext context, short oldPrio) {
        ...
        for (int i = 0; i < reqs.length; i++) {
            ...
            if (req.persistent() != persistent()) {
                Logger.error(this,
                             "Request persistence is " + req.persistent() + " but scheduler's is " + persistent()
                             + " on " + this + " for " + req);
            ...
            if (persistent()) {
                sched.removeFromStarterQueue(req, container, true);
            }
    ...
    public boolean removePendingKeys(KeyListener listener) {
        ...
        synchronized (this) {
            ...
            while (logMINOR && keyListeners.remove(listener)) {
                Logger.error(this, "Still in pending keys after removal, must be in twice or more: " + listener,
                             new Exception("error"));
            }
    ...
    public synchronized boolean removePendingKeys(HasKeyListener hasListener) {
        ...
        for (Iterator<KeyListener> i = keyListeners.iterator(); i.hasNext(); ) {
            ...
            if (listener.getHasKeyListener() == hasListener) {
                ...
                listener.onRemove();
                Logger.normal(this,
                              "Removed pending keys from " + this + " : size now " + keyListeners.size() + " : "
                              + listener);
            }
        }
        ...
        for (int i = 0; i < newPriorities.length; i++) {
            ...
            if ((prio == null) || prio.isEmpty(container)) {
                ...
                for (int k = 0; k < prio.size(); k++) {
                    ...
                    SectoredRandomGrabArrayWithObject requestGrabber =
                        (SectoredRandomGrabArrayWithObject) prio.getGrabber(client);
