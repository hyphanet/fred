...
public class PersistentChosenRequest {
    ...
    PersistentChosenRequest(SendableRequest req, short prio, ObjectContainer container, RequestScheduler sched,
                            ClientContext context)
            throws NoValidBlocksException {
        request = req;
        this.prio = prio;
    ...
    void onFinished(PersistentChosenBlock block, ClientContext context) {
        ...
        synchronized (this) {
            ...
            while (blocksNotStarted.remove(block)) {    // It's an ArrayList so we must loop.
                Logger.error(this, "Block finished but was in blocksNotStarted: " + block + " for " + this,
                             new Exception("error"));
            }
            ...
            if (!(blocksNotStarted.isEmpty() && blocksStarted.isEmpty())) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not finishing yet: blocks not started: " + blocksNotStarted.size() + " started: "
                                 + blocksStarted.size() + " finished: " + blocksFinished.size() + " for " + this);
                }
    ...
    private void finish(ObjectContainer container, ClientContext context, boolean dumping, boolean alreadyActive) {
        if (!container.ext().isStored(request)) {
            if (logMINOR) {
                Logger.minor(this, "Request apparently already deleted: " + request + " on " + this);
            }
        ...
        if (request instanceof SendableGet) {
            ...
            if (bulkFailItems != null) {
                ((SupportsBulkCallFailure) request).onFailure(
                    bulkFailItems.toArray(new BulkCallFailureItem[bulkFailItems.size()]), container, context);
            }
