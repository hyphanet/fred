...
public class SimpleManifestPutter extends ManifestPutter implements PutCompletionCallback {
    ...
    public SimpleManifestPutter(ClientPutCallback cb, HashMap<String, Object> manifestElements, short prioClass,
                                FreenetURI target, String defaultName, InsertContext ctx, boolean getCHKOnly,
                                RequestClient clientContext, boolean earlyEncode, boolean persistent,
                                ObjectContainer container, ClientContext context) {
        this(cb, manifestElements, prioClass, target, defaultName, ctx, getCHKOnly, clientContext, earlyEncode,
             persistent, null, container, context);
    }

    public SimpleManifestPutter(ClientPutCallback cb, HashMap<String, Object> manifestElements, short prioClass,
                                FreenetURI target, String defaultName, InsertContext ctx, boolean getCHKOnly,
                                RequestClient clientContext, boolean earlyEncode, boolean persistent,
                                byte[] forceCryptoKey, ObjectContainer container, ClientContext context) {
        ...
        this.forceCryptoKey = (forceCryptoKey != null)
                              ? forceCryptoKey : getRandomSplitfileKeys(target, ctx, persistent, container, context);
        this.cb = cb;
        this.ctx = ctx;
    ...
    private static byte[] getRandomSplitfileKeys(FreenetURI target, InsertContext ctx, boolean persistent,
            ObjectContainer container, ClientContext context) {
    ...
    static private void checkDefaultName(HashMap<String, Object> manifestElements, String defaultName) {
        int idx;
        ...
        while ((idx = defaultName.indexOf('/')) != -1) {
            ...
            if (!(o instanceof HashMap)) {
                throw new IllegalArgumentException("Default name dir \"" + dir + "\" is not a directory in \""
                                                   + defaultName + "\"");
            }
    ...
    private void makePutHandlers(HashMap<String, Object> manifestElements, HashMap<String, Object> putHandlersByName,
                                 String ZipPrefix, boolean persistent) {
        for (Map.Entry<String, Object> entry : manifestElements.entrySet()) {
            ...
            if (o instanceof HashMap) {
                ...
                if (logDEBUG) {
                    Logger.debug(this,
                                 "Sub map for " + name + " : " + subMap.size() + " elements from " + elements.size());
                }
            } else {
                ...
                if (element.targetURI != null) {
                    ...
                    (bytesOnZip + sz < (2038 - 64) * 1024)) {    // totally dumb heuristic!
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Inserting separately as PutHandler: " + name + " : " + ph + " persistent="
                                         + ph.persistent() + ":" + ph.persistent + " " + persistent());
                        }
    ...
    private void innerGotAllMetadata(ObjectContainer container, ClientContext context) {
        ...
        if (defaultName != null) {
            ...
            if (meta == null) {
                fail(new InsertException(InsertException.INVALID_URI,
                                         "Default name " + defaultName + " does not exist", null), container, context);
        ...
        baseMetadata = Metadata.mkRedirectionManifestWithMetadata(namesToByteArrays);
    ...
    private void resolveAndStartBase(ObjectContainer container, ClientContext context) {
        ...
        while (true) {
            try {
                ...
                bucket = BucketTools.makeImmutableBucket(context.getBucketFactory(persistent()),
        ...
        if (!(elementsToPutInArchive.isEmpty())) {
            ...
            try {
                ...
                String mimeType = ((archiveType == ARCHIVE_TYPE.TAR)
                                   ? createTarBucket(bucket, os, container) : createZipBucket(bucket, os, container));
                ...
                block = new InsertBlock(outputBucket, new ClientMetadata(mimeType),
                                        persistent() ? targetURI.clone() : targetURI);
                isMetadata = false;
            } catch (IOException e) {
                fail(new InsertException(InsertException.BUCKET_ERROR, e, null), container, context);
        ...
        try {
            ...
            metadataInserter = new SingleFileInserter(this, this, block, isMetadata, ctx, realTimeFlag,
                    (archiveType == ARCHIVE_TYPE.ZIP), getCHKOnly, false, baseMetadata, archiveType, true, null,
                    earlyEncode, true, persistent(), 0, 0, null, cryptoAlgorithm, ckey, -1);

            if (logMINOR) {
                Logger.minor(this,
                             "Inserting main metadata: " + metadataInserter + " for " + baseMetadata + " for " + this);
            }
    ...
    private String createTarBucket(Bucket inputBucket, OutputStream os, ObjectContainer container) throws IOException {
        ...
        for (PutHandler ph : elementsToPutInArchive) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Putting into tar: " + ph + " data length " + ph.data.size() + " name "
                             + ph.targetInArchive);
            }
     ...
     * @throws IOException
     */
    private void resolve(MetadataUnresolvedException e, ObjectContainer container, ClientContext context)
        ...
        for (Metadata m : metas) {
            ...
            try {
                ...
                SingleFileInserter metadataInserter = new SingleFileInserter(this, this, ib, true, ctx, realTimeFlag,
                                                          false, getCHKOnly, false, m, null, true, null, earlyEncode,
                                                          false, persistent(), 0, 0, null, cryptoAlgorithm, null, -1);
    ...
    private void namesToByteArrays(HashMap<String, Object> putHandlersByName,
        ...
        for (Map.Entry<String, Object> entry : putHandlersByName.entrySet()) {
            ...
            if (o instanceof PutHandler) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Putting hashmap into base metadata: " + name + " size " + elements.size()
                                 + " active = "
                                 + ((container == null) ? "null" : Boolean.toString(container.ext().isActive(o))));
    ...
    private void removePutHandlers(ObjectContainer container, ClientContext context) {
        ...
        if (!runningPutHandlers.isEmpty()) {
            Logger.error(this,
                         "Running put handlers not part of putHandlersByName: " + runningPutHandlers.size()
                         + " in removePutHandlers() on " + this, new Exception("error"));
        ...
        if (!putHandlersWaitingForMetadata.isEmpty()) {
            Logger.error(this, "Put handlers waiting for metadata, not part of putHandlersByName: "
                         + putHandlersWaitingForMetadata.size() + " in removePutHandlers() on "
                         + this, new Exception("error"));

            PutHandler[] handlers =
                putHandlersWaitingForMetadata.toArray(new PutHandler[putHandlersWaitingForMetadata.size()]);
        ...
        if (!waitingForBlockSets.isEmpty()) {
            Logger.error(this,
                         "Put handlers waiting for block sets, not part of putHandlersByName: "
                         + waitingForBlockSets.size() + " in removePutHandlers() on " + this, new Exception("error"));
        ...
        if (!putHandlersWaitingForFetchable.isEmpty()) {
            Logger.error(this, "Put handlers waiting for fetchable, not part of putHandlersByName: "
                         + putHandlersWaitingForFetchable.size() + " in removePutHandlers() on "
                         + this, new Exception("error"));

            PutHandler[] handlers =
                putHandlersWaitingForFetchable.toArray(new PutHandler[putHandlersWaitingForFetchable.size()]);
        ...
        if (!elementsToPutInArchive.isEmpty()) {
            Logger.error(this,
                         "Elements to put in archive, not part of putHandlersByName: " + elementsToPutInArchive.size()
                         + " in removePutHandlers() on " + this, new Exception("error"));
     ...
     */
    private void removePutHandlersByName(ObjectContainer container, ClientContext context,
    ...
    private void cancelAndFinish(ObjectContainer container, ClientContext context) {
        ...
        synchronized (this) {
            runningMeta =
                metadataPuttersByMetadata.values().toArray(new ClientPutState[metadataPuttersByMetadata.size()]);
        }
    ...
    public void onSuccess(ClientPutState state, ObjectContainer container, ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (oldState != null) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Did not remove metadata putter " + state + " for " + token + " because not present");
                }
            }
                ...
                if (logMINOR) {
                    ...
                    for (Map.Entry<Metadata, ClientPutState> entry : metadataPuttersByMetadata.entrySet()) {
                        ...
                        if (persistent()) {
                            ...
                            if (!active) {
                                container.activate(metaActive, 1);
                            }
                        }
                        ...
                        if (persistent()) {
                            Logger.minor(this, "Key id is " + container.ext().getID(key));
                        }
    ...
    public void onTransition(ClientPutState oldState, ClientPutState newState, ObjectContainer container) {
        ...
        synchronized (this) {
            ...
            if (prevState != null) {
                ...
                Logger.error(this,
                             "onTransition() but metadataPuttersByMetadata does not contain metadata tag " + m
                             + " for " + oldState + " should -> " + newState);
            }
        }
    ...
    public void notifyClients(ObjectContainer container, ClientContext context) {
        ...
        ctx.eventProducer.produceEvent(new SplitfileProgressEvent(this.totalBlocks, this.successfulBlocks,
                this.failedBlocks, this.fatallyFailedBlocks, this.minSuccessBlocks, this.minSuccessFetchBlocks,
...
//      ctx.removeFrom(container);
    ...
    public void objectOnUpdate(ObjectContainer container) {
        if (logDEBUG) {
            Logger.debug(this,
                         "Updating " + this + " activated=" + container.ext().isActive(this) + " stored="
                         + container.ext().isStored(this), new Exception("debug"));
        }
    }
    ...
    public boolean objectCanNew(ObjectContainer container) {
        ...
        if (logDEBUG) {
            Logger.debug(this,
                         "Storing " + this + " activated=" + container.ext().isActive(this) + " stored="
                         + container.ext().isStored(this), new Exception("debug"));
        }
    ...
    protected final void addRedirect(String name, ManifestElement me, ClientMetadata cm,
                                     HashMap<String, Object> putHandlersByName2) {
        PutHandler ph;
        Bucket data = me.data;
    ...
    private class PutHandler extends BaseClientPutter implements PutCompletionCallback {
        ...
        protected PutHandler(final SimpleManifestPutter smp, String name, Bucket data, ClientMetadata cm,
                             boolean getCHKOnly, boolean persistent) {
            super(smp.priorityClass, smp.client);
            this.persistent = persistent;
            this.cm = cm;
            this.data = data;

            InsertBlock block = new InsertBlock(data, cm,
                                    persistent() ? FreenetURI.EMPTY_CHK_URI.clone() : FreenetURI.EMPTY_CHK_URI);

            this.origSFI = new SingleFileInserter(this, this, block, false, ctx, realTimeFlag, false, getCHKOnly, true,
                    null, null, false, null, earlyEncode, false, persistent, 0, 0, null, cryptoAlgorithm,
                    forceCryptoKey, -1);
            metadata = null;
            containerHandle = null;
        }

        protected PutHandler(final SimpleManifestPutter smp, String name, String targetInArchive, ClientMetadata cm,
                             Bucket data, boolean persistent) {
            super(smp.getPriorityClass(), smp.client);
            this.persistent = persistent;
            this.cm = cm;
            this.data = data;
        ...
        public void onSuccess(ClientPutState state, ObjectContainer container, ClientContext context) {
            ...
            synchronized (SimpleManifestPutter.this) {
                ...
                if (putHandlersWaitingForMetadata.remove(this)) {
                    ...
                    Logger.error(this,
                                 "PutHandler was in waitingForMetadata in onSuccess() on " + this + " for "
                ...
                if (waitingForBlockSets.remove(this)) {
                    ...
                    Logger.error(this,
                                 "PutHandler was in waitingForBlockSets in onSuccess() on " + this + " for "
                                 + SimpleManifestPutter.this);
                }
        ...
        public void onTransition(ClientPutState oldState, ClientPutState newState, ObjectContainer container) {
            ...
            synchronized (this) {
                if (currentState == oldState) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "onTransition: cur=" + currentState + ", old=" + oldState + ", new=" + newState
                                     + " for " + this);
                    }
                ...
                Logger.error(this,
                             "Ignoring onTransition: cur=" + currentState + ", old=" + oldState + ", new=" + newState
                             + " for " + this);
            }
        }
        ...
        public void onMetadata(Metadata m, ClientPutState state, ObjectContainer container, ClientContext context) {
            if (logMINOR) {
                Logger.minor(this,
                             "Assigning metadata: " + m + " for " + this + " from " + state + " persistent="
                             + persistent, new Exception("debug"));
            }
        ...
        public void removeFrom(ObjectContainer container, ClientContext context) {
            ...
            if (oldState != null) {
                Logger.error(this,
                             "currentState is set in removeFrom() on " + this + " for " + SimpleManifestPutter.this,
                             new Exception("debug"));
                container.activate(oldState, 1);
                oldState.cancel(container, context);
                oldState.removeFrom(container, context);
