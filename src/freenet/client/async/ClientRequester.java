...
public abstract class ClientRequester
	implements Serializable, ClientRequestSchedulerGroup {
	...
	public void blockSetFinalized(ClientContext context) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Finalized set of blocks for " + this,
			new Exception("debug")
		);
	...
	public void addBlock() {
		...
		if (wasFinalized) {
			if (
				LogLevel.MINOR.matchesThreshold(Logger.globalGetThresholdNew())
			) Logger.error(
				this,
				"addBlock() but set finalized! on " + this,
				new Exception("error")
			);
	...
	public void addBlocks(int num) {
		...
		if (wasFinalized) {
			if (
				...
				this,
				"addBlocks() but set finalized! on " + this,
				new Exception("error")
			);
			else Logger.error(
				this,
	...
	static final transient UserAlert brokenClientAlert = new SimpleUserAlert(
		true,
	...
	protected static byte[] getClientDetail(
		PersistentClientCallback callback,
