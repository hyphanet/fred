...
public abstract class ClientRequester {
    transient static final UserAlert brokenClientAlert =
        new SimpleUserAlert(
            true, "Some broken downloads/uploads were cancelled. Please restart them.",
    ...
    public void addBlock(ObjectContainer container) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "addBlock(): total=" + totalBlocks + " successful=" + successfulBlocks + " failed="
    ...
    public void completedBlock(boolean dontNotify, ObjectContainer container, ClientContext context) {
        ...
        if (checkForBrokenClient(container, context)) {
            return;
        }
        ...
        if ((container != null) && (client == null)) {
            if (container.ext().isStored(this) && container.ext().isActive(this)) {
                ...
                if (!isFinished()) {
                    ...
                    System.err.println(
                        "Cancelling download/upload because of bug causing database corruption. The bug has been fixed but the download/upload will be cancelled. You can restart it.");
                }
    ...
    public synchronized void addMustSucceedBlocks(int blocks, ObjectContainer container) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "addMustSucceedBlocks(" + blocks + "): total=" + totalBlocks + " successful="
