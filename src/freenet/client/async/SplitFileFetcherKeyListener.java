...
public class SplitFileFetcherKeyListener implements KeyListener {
	...
	public SplitFileFetcherKeyListener(
		...
		if (segBlocks <= 0) {
			throw new FetchException(
				FetchExceptionMode.INTERNAL_ERROR,
				"Cannot listen for non-positive number of blocks per segment: " +
				segBlocks
			);
		}
		if (segments <= 0) {
		...
		if (elementsLong > Integer.MAX_VALUE) throw new FetchException(
		...
		double acceptableFalsePositives =
			ACCEPTABLE_BLOOM_FALSE_POSITIVES_ALL_SEGMENTS / segments;
		int perSegmentBitsPerKey = (int) Math.ceil(
			Math.log(acceptableFalsePositives) / Math.log(0.6185)
		);
		...
		byte[] segmentsFilterBuffer = new byte[perSegmentBloomFilterSizeBytes *
		segments];
		...
		for (int i = 0; i < segments; i++) {
			...
			segmentFilters[i] = new BinaryBloomFilter(
				slice,
				perSegmentBloomFilterSizeBytes * 8,
				perSegmentK
			);
	...
	public SplitFileFetcherKeyListener(
		...
		if (mainBloomFilterSizeBytes < 0) throw new StorageFormatException(
			"Bad main bloom filter size"
		);
		mainBloomK = dis.readInt();
		if (mainBloomK < 1) throw new StorageFormatException(
			"Bad main bloom filter K"
		);
		...
		if (perSegmentK < 0) throw new StorageFormatException(
			"Bad per segment bloom filter K"
		);
	...
	synchronized void addKey(Key key, int segNo, KeySalter salter) {
		if (
			finishedSetup &&
			!(mustRegenerateMainFilter || mustRegenerateSegmentFilters)
	...
	void initialWriteSegmentBloomFilters(long fileOffset) throws IOException {
		OutputStream cos = storage.writeChecksummedTo(
			fileOffset,
			totalSegmentBloomFiltersSize()
		);
	...
	int totalSegmentBloomFiltersSize() {
		return (
			perSegmentBloomFilterSizeBytes * segmentFilters.length +
			storage.checksumLength
		);
	}
	...
	public boolean handleBlock(
		...
		if (logMINOR) Logger.minor(
			this,
			"handleBlock(" + key + ") on " + this + " for " + fetcher,
			new Exception("debug")
		);
		for (int i = 0; i < segmentFilters.length; i++) {
			boolean match;
			...
			if (match) {
				try {
					found = storage.segments[i].onGotKey(
							(NodeCHK) key,
							(CHKBlock) block
						);
				} catch (IOException e) {
