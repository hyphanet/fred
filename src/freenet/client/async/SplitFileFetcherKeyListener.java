...
public class SplitFileFetcherKeyListener implements KeyListener {
     ...
     * @throws IOException
     */
    public SplitFileFetcherKeyListener(SplitFileFetcher parent, int keyCount, File bloomFile, File altBloomFile,
                                       int mainBloomSizeBytes, int mainBloomK, byte[] localSalt, int segments,
                                       int segmentFilterSizeBytes, int segmentBloomK, boolean persistent,
                                       boolean newFilter, CountingBloomFilter cachedMainFilter,
                                       BinaryBloomFilter[] cachedSegFilters, ObjectContainer container,
                                       boolean onStartup, boolean realTime)
            throws IOException {
        fetcher = parent;
        ...
        if (cachedSegFilters != null) {
            for (int i = 0; i < cachedSegFilters.length; i++) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Restored segment " + i + " filter for " + parent + " : k="
                                 + cachedSegFilters[i].getK() + " size = " + cachedSegFilters[i].getSizeBytes()
                                 + " bytes = " + cachedSegFilters[i].getLength() + " elements, filled: "
                                 + cachedSegFilters[i].getFilledCount());
                }
            }
        } else {
            ...
            if (persistent) {
                for (int i = 0; i < segments; i++) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Storing segment " + i + " filter to database for " + parent + " : k="
                                     + segmentFilters[i].getK() + " size = " + segmentFilters[i].getSizeBytes()
                                     + " bytes = " + segmentFilters[i].getLength() + " elements, filled: "
                                     + segmentFilters[i].getFilledCount());
                    }
        ...
        if (cachedMainFilter != null) {
            ...
            if (persistent) {
                container.activate(filter, Integer.MAX_VALUE);
            }
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Restored filter for " + parent + " : k=" + filter.getK() + " size = "
                             + filter.getSizeBytes() + " bytes = " + filter.getLength() + " elements, filled: "
                             + filter.getFilledCount());
            }
        } else if (newFilter) {
            ...
            if (persistent) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Storing filter to database for " + parent + " : k=" + filter.getK() + " size = "
                                 + filter.getSizeBytes() + " bytes = " + filter.getLength() + " elements, filled: "
                                 + filter.getFilledCount());
                }
...
//              Logger.error(this, "Key added but not in filter: "+key+" on "+this);
    ...
    public boolean probablyWantKey(Key key, byte[] saltedKey) {
        if (filter == null) {
            Logger.error(this, "Probably want key: filter = null for " + this + " fetcher = " + fetcher);
        }
    ...
    public short definitelyWantKey(Key key, byte[] saltedKey, ObjectContainer container, ClientContext context) {
        ...
        for (int i = 0; i < segmentFilters.length; i++) {
            if (segmentFilters[i].checkFilter(salted)) {
                ...
                if (!found) {
                    Logger.error(this,
                                 "Found block in primary and segment bloom filters but segment doesn't want it: "
                                 + segment + " on " + this);
                }
    ...
    @Override
    public boolean handleBlock(Key key, byte[] saltedKey, KeyBlock block, ObjectContainer container,
    ...
    public void writeFilters(ObjectContainer container, String reason) throws IOException {
        ...
        for (int i = 0; i < segmentFilters.length; i++) {
            if (logMINOR) {
                Logger.minor(this,
                             "Storing segment " + i + " filter to database (" + reason + ") k="
                             + segmentFilters[i].getK() + " size = " + segmentFilters[i].getSizeBytes() + " bytes = "
                             + segmentFilters[i].getLength() + " elements, filled: "
                             + segmentFilters[i].getFilledCount());
            }
    ...
    public synchronized int killSegment(SplitFileFetcherSegment segment, ObjectContainer container,
            ClientContext context) {
        ...
        for (Key removeKey : removeKeys) {
            ...
            if (filter.checkFilter(salted)) {
                ...
                // Huh??
                Logger.error(this,
                             "Removing key " + removeKey + " for " + this + " from " + segment
                             + " : NOT IN BLOOM FILTER!", new Exception("debug"));
            }
        }
    ...
    public synchronized void removeKey(Key key, SplitFileFetcherSegment segment, ObjectContainer container,
                                       ClientContext context) {
        if (logMINOR) {
        ...
        if (filter.checkFilter(salted)) {
            ...
            // Huh??
            Logger.error(this, "Removing key " + key + " for " + this + " from " + segment + " : NOT IN BLOOM FILTER!",
                         new Exception("debug"));
        }
                    ...
                    public boolean run(ObjectContainer container, ClientContext context) {
                        synchronized (SplitFileFetcherKeyListener.this) {
                            try {
                                ...
                                Logger.error(
                                    this,
                                    "Failed to write bloom filters, we will have more false positives on already-found blocks which aren't in the store: "
                                    + e, e);
                            } finally {
