...
public class SingleFileFetcher extends SimpleSingleFileFetcher {
    ...
    public SingleFileFetcher(SingleFileFetcher fetcher, boolean persistent, boolean deleteFetchContext,
        ...
        super(persistent ? fetcher.key.cloneKey() : fetcher.key, fetcher.maxRetries, ctx2, fetcher.parent, callback,
              false, true, fetcher.token, container, context, deleteFetchContext, fetcher.realTimeFlag);
    ...
    public SingleFileFetcher(ClientRequester parent, GetCompletionCallback cb, ClientMetadata metadata, ClientKey key,
                             ...
                             Metadata archiveMetadata, int maxRetries, int recursionLevel, boolean dontTellClientGet,
                             long l, boolean isEssential, boolean isFinal, boolean topDontCompress,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Creating SingleFileFetcher for " + key + " from " + origURI + " meta="
    ...
    @Override
    public void onSuccess(ClientKeyBlock block, boolean fromStore, Object token, ObjectContainer container,
    ...
    private void handleMetadata(Bucket data, ObjectContainer container, ClientContext context) {
        if (!ctx.followRedirects) {
            onFailure(new FetchException(FetchException.INVALID_METADATA,
                                         "Told me not to follow redirects (splitfile block??)"), false, container,
                                             context);
            data.free();
    ...
    protected void onSuccess(final FetchResult result, ObjectContainer container, final ClientContext context) {
        ...
        if ((!ctx.ignoreTooManyPathComponents) && (!metaStrings.isEmpty()) && isFinal) {
            ...
            if (addedMetaStrings > 0) {
                ...
                rcb.onFailure(new FetchException(FetchException.INVALID_METADATA,
                                                 "Invalid metadata: too many path components in redirects",
                                                 thisKey), this, container, context);
            } else {
                ...
                rcb.onFailure(new FetchException(FetchException.TOO_MANY_PATH_COMPONENTS, result.size(),
                                                 (rcb == parent), result.getMimeType(), tryURI), this, container,
                                                     context);
            }
            ...
            rcb.onFailure(new FetchException(FetchException.TOO_BIG, result.size(), (rcb == parent),
            ...
            if (persistent()) {
                rcb.onSuccess(new SingleFileStreamGenerator(result.asBucket(), persistent), result.getMetadata(),
                              decompressors, this, container, context);
            } else {
                ...
                context.mainExecutor.execute(new Runnable() {
                    @Override
                    public void run() {
                        rcb.onSuccess(new SingleFileStreamGenerator(result.asBucket(), persistent),
                                      result.getMetadata(), decompressors, SingleFileFetcher.this, null, context);
                    }
                });
            }
        }
     ...
     */
    private synchronized void handleMetadata(final ObjectContainer container, final ClientContext context)
        ...
        while (true) {
            ...
            if (metadata.isSimpleManifest()) {
                ...
                if (name == null) {
                    ...
                    if (metadata == null) {
                        throw new FetchException(FetchException.NOT_ENOUGH_PATH_COMPONENTS, -1, false, null,
                    ...
                    if (!persistent) {
                        ...
                        if ((metadata != null) && metadata.isSymbolicShortlink()) {
                            ...
                            if (oldName.equals(name)) {
                                throw new FetchException(FetchException.INVALID_METADATA, "redirect loop: " + name);
                            }
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Is archive manifest (type=" + metadata.getArchiveType() + " codec="
                ...
                if ((ah == null) ||!ah.getKey().equals(thisKey)) {
                    ...
                    ah = context.archiveManager.makeHandler(thisKey, metadata.getArchiveType(),
                            ...
                            ((parent instanceof ClientGetter)
                             ? ((ClientGetter) parent).collectingBinaryBlob() : false), persistent);
                }
                    ...
                    fetchArchive(false, archiveMetadata, ArchiveManager.METADATA_NAME, new ArchiveExtractCallback() {
                        ...
                        public void gotBucket(Bucket data, ObjectContainer container, ClientContext context) {
                            ...
                            try {
                                ...
                                // Invalid metadata
                                onFailure(new FetchException(FetchException.INVALID_METADATA, e), false, container,
                                          context);
                                ...
                                // Bucket error?
                                onFailure(new FetchException(FetchException.BUCKET_ERROR, e), false, container,
                                          context);
                        ...
                        public void notInArchive(ObjectContainer container, ClientContext context) {
                            ...
                            onFailure(
                                new FetchException(
                                    FetchException.INTERNAL_ERROR,
                                    "No metadata in container! Cannot happen as ArchiveManager should synthesise some!"), false, container, context);
                        ...
                        @Override
                        public void onFailed(ArchiveRestartException e, ObjectContainer container,
                                             ClientContext context) {
                            if (persistent) {
                                container.activate(SingleFileFetcher.this, 1);
                            }
                        ...
                        @Override
                        public void onFailed(ArchiveFailureException e, ObjectContainer container,
                                             ClientContext context) {
                            if (persistent) {
                                container.activate(SingleFileFetcher.this, 1);
                            }
                    ...
                    fetchArchive(true, archiveMetadata, filename, new ArchiveExtractCallback() {
                        ...
                        public void gotBucket(Bucket data, ObjectContainer container, ClientContext context) {
                            ...
                            try {
                                ...
                                onFailure(new FetchException(FetchException.INVALID_METADATA), false, container,
                                          context);
                            } finally {
                                data.free();
                            }
                ...
                final SingleFileFetcher f = new SingleFileFetcher(this, persistent, false, metadata,
                                                new MultiLevelMetadataCallback(), ctx, container, context);
                ...
                final SingleFileFetcher f = new SingleFileFetcher(parent, rcb, clientMetadata, redirectedKey,
                                                metaStrings, this.uri, addedMetaStrings, ctx, deleteFetchContext,
                                                realTimeFlag, actx, ah, archiveMetadata, maxRetries, recursionLevel,
                                                false, token, true, isFinal, topDontCompress, topCompatibilityMode,
                                                container, context, false);
                ...
                if ((redirectedKey instanceof ClientCHK) &&!((ClientCHK) redirectedKey).isMetadata()) {
                    ...
                    rcb.onSplitfileCompatibilityMode(metadata.getMinCompatMode(), metadata.getMaxCompatMode(),
                                                     redirectedCryptoKey, !((ClientCHK) redirectedKey).isCompressed(),
                                                     true, true, container, context);
                }
                ...
                if (isFinal &&!ctx.ignoreTooManyPathComponents) {
                    if (!metaStrings.isEmpty()) {
                        ...
                        if (addedMetaStrings > 0) {
                            ...
                            tryURI = tryURI.dropLastMetaStrings(metaStrings.size());
                            rcb.onFailure(new FetchException(FetchException.TOO_MANY_PATH_COMPONENTS,
                ...
                if ((uncompressedLen > ctx.maxOutputLength) || (len > ctx.maxTempLength)) {
                    ...
                    throw new FetchException(FetchException.TOO_BIG, uncompressedLen,
                ...
                SplitFileFetcher sf = new SplitFileFetcher(metadata, rcb, parent, ctx, deleteFetchContext,
                                          realTimeFlag, decompressors, clientMetadata, actx, recursionLevel, token,
                                          topDontCompress, topCompatibilityMode, container, context);
    ...
    private void fetchArchive(boolean forData, Metadata meta, String element, ArchiveExtractCallback callback,
                              final ObjectContainer container, final ClientContext context)
            throws FetchException, MetadataParseException, ArchiveFailureException, ArchiveRestartException {
        if (logMINOR) {
    ...
    public static ClientGetState create(ClientRequester requester, GetCompletionCallback cb, FreenetURI uri,
            ...
            long l, boolean isEssential, boolean isFinal, ObjectContainer container, ClientContext context,
            boolean realTimeFlag, boolean hasInitialMetadata)
        ...
        if ((!uri.hasMetaStrings()) && (ctx.allowSplitfiles == false) && (ctx.followRedirects == false)
            ...
            return new SimpleSingleFileFetcher((ClientKey) key, maxRetries, ctx, requester, cb, isEssential, false, l,
                                               container, context, false, realTimeFlag);
        }

        if ((key instanceof ClientKey) || hasInitialMetadata) {
            return new SingleFileFetcher(requester, cb, null, (ClientKey) key,
                                         new ArrayList<String>(uri.listMetaStrings()), uri, 0, ctx, false,
                                         realTimeFlag, actx, null, null, maxRetries, recursionLevel, dontTellClientGet,
                                         l, isEssential, isFinal, false, (short) 0, container, context,
                                         hasInitialMetadata);
        } else {
            return uskCreate(requester, realTimeFlag, cb, (USK) key, new ArrayList<String>(uri.listMetaStrings()), ctx,
                             actx, maxRetries, recursionLevel, dontTellClientGet, l, isEssential, isFinal, container,
                             context);
        }
    }
        ...
        if (usk.suggestedEdition >= 0) {
            ...
            if (edition <= usk.suggestedEdition) {
                ...
                if (edition > usk.suggestedEdition) {
                    ...
                    cb.onFailure(new FetchException(FetchException.PERMANENT_REDIRECT,
                                                    usk.copy(edition).getURI().addMetaStrings(metaStrings)), null,
                                                        container, context);

                    return null;
                } else if ((edition == -1) && (context.uskManager.lookupLatestSlot(usk) == -1)) {    // We do not want to be going round and round here!
                    ...
                    USKFetcherTag tag = context.uskManager.getFetcher(usk.copy(usk.suggestedEdition), ctx, false,
                                            ...
                                            new MyUSKFetcherCallback(requester, cb, usk, metaStrings, ctx, actx,
                                                realTimeFlag, maxRetries, recursionLevel, dontTellClientGet, l,
                                                requester.persistent(), true), false, container, context, true);
                    ...
                    SingleFileFetcher sf = new SingleFileFetcher(requester, myCB, null, usk.getSSK(), metaStrings,
                                               usk.getURI().addMetaStrings(metaStrings), 0, ctx, false, realTimeFlag,
                                               actx, null, null, maxRetries, recursionLevel, dontTellClientGet, l,
                                               isEssential, isFinal, false, (short) 0, container, context, false);
            ...
            USKFetcherTag tag = context.uskManager.getFetcher(usk.copy(-usk.suggestedEdition), ctx, false,
                                    ...
                                    new MyUSKFetcherCallback(requester, cb, usk, metaStrings, ctx, actx, realTimeFlag,
                                        maxRetries, recursionLevel, dontTellClientGet, l, requester.persistent(),
                                        false), false, container, context, false);
...
//  
    ...
    class ArchiveFetcherCallback implements GetCompletionCallback {
        ...
        public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
            ...
            try {
                ...
                if (decompressors != null) {
                    ...
                    ClientGetWorkerThread worker = new ClientGetWorkerThread(pipeIn, output, null, null, null, false,
                                                       null, null, null, context.linkFilterExceptionProvider);
        ...
        private void innerSuccess(Bucket data, ObjectContainer container, ClientContext context) {
            try {
                if (hashes != null) {
                    ...
                    try {
                        if (persistent()) {
                            container.activate(hashes, Integer.MAX_VALUE);
                        }
                        ...
                        if (!HashResult.strictEquals(results, hashes)) {
                            onFailure(new FetchException(FetchException.CONTENT_HASH_FAILED), SingleFileFetcher.this,
                                      container, context);

                            return;
                        }
                    } catch (IOException e) {
                        onFailure(new FetchException(FetchException.BUCKET_ERROR, e), SingleFileFetcher.this,
                                  container, context);
        ...
        public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] splitfileKey,
                boolean dontCompress, boolean bottomLayer, boolean definitiveAnyway, ObjectContainer container,
                ClientContext context) {
            boolean wasActive = true;
    ...
    class MultiLevelMetadataCallback implements GetCompletionCallback {
        ...
        public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
            ...
            try {
                ...
                SingleFileFetcher.this.onFailure(new FetchException(FetchException.INVALID_METADATA, e), false,
                                                 container, context);
                ...
                // Bucket error?
                SingleFileFetcher.this.onFailure(new FetchException(FetchException.BUCKET_ERROR, e), false, container,
                                                 context);
        ...
        public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] splitfileKey,
            ...
            rcb.onSplitfileCompatibilityMode(min, max, splitfileKey, dontCompress, false, definitiveAnyway, container,
                                             context);
    ...
    public static class MyUSKFetcherCallback implements USKFetcherTagCallback {
        ...
        public MyUSKFetcherCallback(ClientRequester requester, GetCompletionCallback cb, USK usk,
                                    ArrayList<String> metaStrings, FetchContext ctx, ArchiveContext actx,
                                    boolean realTimeFlag, int maxRetries, int recursionLevel,
                                    boolean dontTellClientGet, long l, boolean persistent, boolean datastoreOnly) {
        ...
        public void onFoundEdition(long l, USK newUSK, ObjectContainer container, ClientContext context,
                                   boolean metadata, short codec, byte[] data, boolean newKnownGood,
                                   boolean newSlotToo) {
            if (persistent) {
                container.activate(this, 2);
            }
            ...
            try {
                if (l == usk.suggestedEdition) {
                    SingleFileFetcher sf = new SingleFileFetcher(parent, cb, null, key, metaStrings,
                                               key.getURI().addMetaStrings(metaStrings), 0, ctx, false, realTimeFlag,
                                               actx, null, null, maxRetries, recursionLevel + 1, dontTellClientGet,
                                               token, false, true, false, (short) 0, container, context, false);
        ...
        public void onFailure(ObjectContainer container, ClientContext context) {
            ...
            if (datastoreOnly) {
                ...
                try {
                    onFoundEdition(usk.suggestedEdition, usk, container, context, false, (short) -1, null, false,
                                   false);
            ...
            if (e == null) {
                e = new FetchException(FetchException.DATA_NOT_FOUND, "No USK found");
            }
            ...
            if (cb == null) {
                throw new NullPointerException("Callback is null in " + this + " for usk " + usk
                                               + " with datastoreOnly=" + datastoreOnly);
            }
