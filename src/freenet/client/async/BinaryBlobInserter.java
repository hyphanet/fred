...
public class BinaryBlobInserter implements ClientPutState {
	...
	BinaryBlobInserter(
		...
		for (Key key : blocks.keys()) {
			...
			MySendableInsert inserter = new MySendableInsert(
				x++,
				block,
				prioClass,
				getScheduler(block, context),
				clientContext
			);
			myInserters.add(inserter);
		}

		inserters = myInserters.toArray(
			new MySendableInsert[myInserters.size()]
		);
	...
	private ClientRequestScheduler getScheduler(
		...
		if (block instanceof CHKBlock) return context.getChkInsertScheduler(
			realTimeFlag
		);
		else if (
			...
			"Unknown block type " + block.getClass() + " : " + block
		);
	}
	...
	public void maybeFinish(ClientContext context) {
		...
		} else if (wasFatal) parent.onFailure(
			new InsertException(
