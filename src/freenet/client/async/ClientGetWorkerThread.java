...
public class ClientGetWorkerThread extends Thread {
     ...
     */
    public ClientGetWorkerThread(PipedInputStream input, OutputStream output, FreenetURI uri, String mimeType,
                                 HashResult[] hashes, boolean filterData, String charset,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Created worker thread for " + uri + " mime type " + mimeType + " filter data = " + filterData
                         + " charset " + charset);
        }
    }
    ...
    public void run() {
        if (logMINOR) {
            Logger.minor(this,
                         "Starting worker thread for " + uri + " mime type " + mimeType + " filter data = "
        ...
        try {
            ...
            if (filterData) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Running content filter... Prefetch hook: " + prefetchHook + " tagReplacer: "
                                 + tagReplacer);
                }
                ...
                FilterStatus filterStatus = ContentFilter.filter(input, output, mimeType, uri, prefetchHook,
                                                tagReplacer, charset, linkFilterExceptionProvider);
            ...
            if (hashes != null) {
                ...
                if (!HashResult.strictEquals(results, hashes)) {
                    Logger.error(this,
                                 "Hashes failed verification (length read is " + hashStream.getReadBytes() + ") "
                                 + " for " + uri);
