...
public class USKManager {
        ...
        public void run() {
            ...
            for (final USK key : toFetch) {
                ...
                final ClientGetter get = new ClientGetter(new ClientGetCallback() {
                   ...
                   rcBulk, new NullBucket(), null, null);
    ...
    public USKFetcherTag getFetcher(USK usk, FetchContext ctx, boolean keepLast, boolean persistent, boolean realTime,
                                    USKFetcherCallback callback, boolean ownFetchContext, ObjectContainer container,
                                    ClientContext context, boolean checkStoreOnly) {
        return USKFetcherTag.create(usk, callback, context.nodeDBHandle, persistent, realTime, container, ctx,
                                    keepLast, 0, ownFetchContext, checkStoreOnly || ctx.localRequestOnly);
    }
    ...
    public USKFetcherTag getFetcherForInsertDontSchedule(USK usk, short prioClass, USKFetcherCallback cb,
            RequestClient client, ObjectContainer container, ClientContext context, boolean persistent,
            boolean ignoreUSKDatehints) {
        FetchContext fctx = ignoreUSKDatehints ? backgroundFetchContextIgnoreDBR : backgroundFetchContext;

        return getFetcher(usk, persistent ? new FetchContext(fctx, FetchContext.IDENTICAL_MASK, false, null) : fctx,
                          true, client.persistent(), client.realTimeFlag(), cb, true, container, context, false);
    }
    ...
    public void hintUpdate(USK usk, long edition, ClientContext context) {
        ...
        final ClientGetter get = new ClientGetter(new NullClientCallback(), uri,
                                     new FetchContext(backgroundFetchContext, FetchContext.IDENTICAL_MASK, false,
                                         null), RequestStarter.UPDATE_PRIORITY_CLASS, rcBulk, new NullBucket(), null,
                                             null);
    ...
    public void hintUpdate(FreenetURI uri, ClientContext context, short priority) throws MalformedURLException {
        if (uri.getSuggestedEdition() < lookupLatestSlot(USK.create(uri))) {
            if (logMINOR) {
                Logger.minor(this,
                             "Ignoring hint because edition is " + uri.getSuggestedEdition() + " but latest is "
        ...
        final ClientGetter get = new ClientGetter(new NullClientCallback(), uri,
                                     new FetchContext(backgroundFetchContext, FetchContext.IDENTICAL_MASK, false,
                                         null), priority, rcBulk, new NullBucket(), null, null);
    ...
    public void hintCheck(FreenetURI uri, final Object token, ClientContext context, short priority,
        ...
        }, uri, new FetchContext(backgroundFetchContext, FetchContext.IDENTICAL_MASK, false, null), priority, rcBulk,
...
//          }
            ...
            if (f == null) {
                f = new USKFetcher(usk, this,
                                   ...
                                   new USKFetcherWrapper(usk, RequestStarter.UPDATE_PRIORITY_CLASS,
            ...
            if (runBackgroundFetch) {
                ...
                if (f == null) {
                    f = new USKFetcher(origUSK, this,
                                       ...
                                       new USKFetcherWrapper(origUSK, RequestStarter.UPDATE_PRIORITY_CLASS, client), 3,
                                       true, false, false);
                    sched = f;
    ...
    public USKRetriever subscribeContentCustom(USK origUSK, USKRetrieverCallback cb, FetchContext fctx, short prio,
        ...
        USKFetcher f = new USKFetcher(origUSK, this, fctx, new USKFetcherWrapper(origUSK, prio, client), 3, true,
                                      false, false);
    ...
    public void checkUSK(FreenetURI uri, boolean persistent, ObjectContainer container, boolean isMetadata) {
        try {
            if (persistent) {
