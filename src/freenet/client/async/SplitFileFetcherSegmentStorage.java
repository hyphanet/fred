...
public class SplitFileFetcherSegmentStorage {
	...
	public SplitFileFetcherSegmentStorage(
		...
		boolean ignoreLastBlock =
			(segNo == parent.segments.length - 1 &&
				parent.lastBlockMightNotBePadded());
		this.blockChooser = new SplitFileFetcherSegmentBlockChooser(
			total,
			parent.random,
			parent.maxRetries,
		...
		if (crossCheckBlocks != 0) crossSegmentsByBlock =
			new SplitFileFetcherCrossSegmentStorage[minFetched];
		else crossSegmentsByBlock = null;
		blocksFetched = new int[minFetched];
		for (int i = 0; i < blocksFetched.length; i++) blocksFetched[i] = -1;
		segmentStatusPaddedLength = paddedStoredSegmentStatusLength(
			dataBlocks,
			checkBlocks,
			crossCheckBlocks,
			writeRetries,
		...
		segmentKeyListLength = storedKeysLength(
			blocksForDecode(),
			checkBlocks,
		...
		if (segmentCrossCheckDataOffset == -1) {
			segmentCrossCheckDataOffset = segmentBlockDataOffset +
			dataBlocks * CHKBlock.DATA_LENGTH;
		}
	...
	public SplitFileFetcherSegmentStorage(
		...
		if (
			crossSegmentCheckBlocks < 0 || crossSegmentCheckBlocks > 256
		) throw new StorageFormatException(
			"Bad cross-segment check block count"
		);
		this.checkBlocks = dis.readInt();
		if (
			checkBlocks < 0 || checkBlocks > 256
		) throw new StorageFormatException("Bad check block count");
		int total = dataBlocks + checkBlocks + crossSegmentCheckBlocks;
		if (total > 256) throw new StorageFormatException(
			"Too many blocks in segment"
		);
		boolean ignoreLastBlock =
			(segNo == parent.segments.length - 1 &&
				parent.lastBlockMightNotBePadded());
		blockChooser = new SplitFileFetcherSegmentBlockChooser(
			total,
			parent.random,
			parent.maxRetries,
			parent.cooldownTries,
		...
		int minFetched = blocksForDecode();
		if (crossSegmentCheckBlocks != 0) crossSegmentsByBlock =
			new SplitFileFetcherCrossSegmentStorage[minFetched];
		else crossSegmentsByBlock = null;
		blocksFetched = new int[minFetched];
		for (int i = 0; i < blocksFetched.length; i++) blocksFetched[i] = -1;
	...
	SplitFileSegmentKeys readSegmentKeys()
		throws IOException, ChecksumFailedException {
		SplitFileSegmentKeys keys = new SplitFileSegmentKeys(
			blocksForDecode(),
			checkBlocks,
			parent.splitfileSingleCryptoKey,
			parent.splitfileSingleCryptoAlgorithm
		);
		byte[] buf = new byte[SplitFileSegmentKeys.storedKeysLength(
			blocksForDecode(),
			checkBlocks,
			parent.splitfileSingleCryptoKey != null
		)];
	...
	public boolean tryStartDecode() {
		synchronized (this) {
			...
			if (
				!corruptMetadata &&
				blockChooser.successCount() < blocksForDecode()
			) return false;
				...
				public boolean start(MemoryLimitedChunk chunk) {
					...
					try {
						...
						try {
							...
							if (!shutdown) parent.finishedEncoding(
	...
	private void innerDecode(MemoryLimitedChunk chunk) throws IOException {
		...
		synchronized (this) {
			...
			for (short i = 0; i < blocksFetched.length; i++) {
				if (blocksFetched[i] < 0 || blocksFetched[i] > totalBlocks) {
					...
					maybeBlocks.add(
						new SplitFileFetcherBlock(allBlocks[i], (short) -1, i)
					);
					continue;
					...
					if (logMINOR) Logger.minor(
						this,
						"Found block " + blocksFetched[i] + " in slot " + i
					);
					maybeBlocks.add(
						new SplitFileFetcherBlock(
							allBlocks[i],
							blocksFetched[i],
							i
						)
		...
		for (SplitFileFetcherBlock test : maybeBlocks) {
			...
			byte[] buf = test.buf;
			ClientCHK decodeKey = blockNumber == -1
				? null
				: keys.getKey(blockNumber, null, false);
			// Encode it to check whether the key is the same.
			try {
				ClientCHKBlock block = ClientCHKBlock.encodeSplitfileBlock(
					buf,
					decodeKey.getCryptoKey(),
			...
			if (!failed) {
				...
				if (blockNumber < dataBlocks.length) dataBlocks[blockNumber] =
					buf;
		...
		if (logMINOR) Logger.minor(
			this,
			"Finished decoding " + this + " for " + parent
		);
	}
	...
	private void checkDecodedDataBlocks(
		...
		for (int i = 0; i < dataBlocks.length; i++) {
			...
			try {
				...
				if (capturingBinaryBlob) parent.fetcher.maybeAddToBinaryBlob(
					block
				);
			} catch (CHKEncodeException e) {
				// Impossible!
				...
				Logger.error(
					this,
					"Impossible: Decoded block could not be encoded"
				);
				return;
			}
		}
	...
	private boolean checkEncodedDataBlocks(
		...
		for (int i = 0; i < checkBlocks.length; i++) {
			...
			ClientCHK decodeKey = keys.getKey(
				i + blocksForDecode(),
				null,
				false
			);
	...
	private void triggerAllCrossSegmentCallbacks() {
		...
		synchronized (this) {
			...
			crossSegmentsByBlockCopy = Arrays.copyOf(
				this.crossSegmentsByBlock,
	...
	public boolean onGotKey(NodeCHK key, CHKBlock block) throws IOException {
		...
		synchronized (this) {
			...
			if (blockChooser.hasSucceeded(blockNumber)) return false; // Even if this is inaccurate, it will be corrected on a FEC attempt.
		...
		do {
			...
			synchronized (this) {
				if (succeeded || failed || finished) {
					if (logMINOR) Logger.minor(
						this,
						"Already succeeded/finished/failed"
					);
				...
				if (blockChooser.hasSucceeded(blockNumber)) {
					if (logMINOR) Logger.minor(
						this,
						"Already have block " + blockNumber
					);
					blockNumber = blockChooser.getBlockNumber(keys, key);
					if (logMINOR) Logger.minor(
						this,
						"Trying block " + blockNumber
					);
					continue;
				}
				...
				try {
					...
					Logger.error(
						this,
						"Unable to write downloaded block to disk: " + e,
						e
					);
					throw e;
				} finally {
				...
				if (
					...
					blockNumber < crossSegmentsByBlock.length
				) {
					callback = crossSegmentsByBlock[blockNumber];
				}
				nextBlockNumber = (short) blockChooser.getBlockNumber(
					keys,
					key
				);
				metadataDirty = true;
			}
			if (callback != null) callback.onFetchedRelevantBlock(
				this,
				blockNumber
			);
			lazyWriteMetadata();
	 ...
	 * @throws IOException */
	private synchronized void writeDownloadedBlock(int slotNumber, byte[] data)
		throws IOException {
	...
	long blockOffset(int slotNumber) {
		if (slotNumber < dataBlocks) {
			...
			return (
				segmentCrossCheckBlockDataOffset +
				slotNumber * CHKBlock.DATA_LENGTH
			);
		}
	}
	...
	private void innerWriteMetadata(boolean force) throws IOException {
		...
		synchronized (this) {
			...
			if (logMINOR) Logger.debug(
				this,
				"Writing metadata for " + segNo + " for " + parent,
				new Exception("debug")
			);
			OutputStream cos = parent.writeChecksummedTo(
				segmentStatusOffset,
	...
	void readMetadata()
		...
		for (int i = 0; i < blocksFetched.length; i++) {
			int s = dis.readInt();
			if (s < -1 || s >= totalBlocks()) throw new StorageFormatException(
				"Bogus block number in blocksFetched[" + i + "]: " + s
			);
			blocksFetched[i] = s;
			if (s >= 0) {
				if (!blockChooser.hasSucceeded(s)) {
					...
					throw new StorageFormatException(
						"Duplicated block number in blocksFetched in " + this
					);
				}
			}
		}
	...
	void writeToInner(OutputStream os) throws IOException {
		...
		for (int i = 0; i < dataBlocks; i++) { // Don't include cross-check blocks.
			...
			if (
				i == dataBlocks - 1 && this.segNo == parent.segments.length - 1
			) {
	...
	public void onNonFatalFailure(int blockNumber) {
		...
		synchronized (this) {
			...
			if (blockChooser.onNonFatalFailure(blockNumber)) {
				if (logMINOR) Logger.minor(
					this,
					"Giving up on block " + blockNumber + " on " + this
				);
				givenUp = true;
				failedBlocks++;
	 ...
	 */
	public int allocateCrossDataBlock(
	 ...
	 */
	public int allocateCrossCheckBlock(
		SplitFileFetcherCrossSegmentStorage seg,
		Random random
	) {
		...
		int x =
			dataBlocks +
			crossSegmentCheckBlocks -
			(1 + random.nextInt(crossSegmentCheckBlocks));
	...
	void writeKeysWithChecksum(SplitFileSegmentKeys keys) throws IOException {
		...
		assert (this.dataBlocks + this.crossSegmentCheckBlocks ==
			keys.dataBlocks);
	...
	public synchronized void getUnfetchedKeys(List<Key> keys)
		throws IOException {
		if (finished || tryDecode) return;
		SplitFileSegmentKeys keyList = getSegmentKeys();
		for (int i = 0; i < totalBlocks(); i++) {
			if (!blockChooser.hasSucceeded(i)) keys.add(
				keyList.getNodeKey(i, null, false)
			);
		}
	}
	...
	public int chooseRandomKey() {
		...
		synchronized (this) {
			...
			if (tryDecode) {
				if (logMINOR) Logger.minor(
					this,
					"Segment decoding so not choosing a key on " + this
				);
				return -1;
			}
	...
	public synchronized byte[] checkAndGetBlockData(int blockNum)
		throws IOException {
		...
		Logger.error(
			...
			"Block " +
			blockNum +
			" in blocksFound but not in blocksFetched on " +
			this
		);
		return null;
	}

	synchronized void resumeCallback(
		int blockNo,
