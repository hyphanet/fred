...
public class USKFetcher implements ClientGetState, USKCallback, HasKeyListener, KeyListener {
    ...
    private void checkFinishedForNow(ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (!dbrAttempts.isEmpty()) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not finished because still waiting for DBR attempts on " + this + " : "
                                 + dbrAttempts);
                }
    ...
    private void notifyFinishedForNow(ClientContext context) {
        if (logMINOR) {
            Logger.minor(this,
                         "Notifying finished for now on " + this + " for " + origUSK
    ...
    void onDNF(USKAttempt att, ClientContext context) {
        ...
        synchronized (this) {
            ...
            if (runningAttempts.isEmpty()) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "latest: " + curLatest + ", last fetched: " + lastFetchedEdition
            ...
            for (USKFetcherCallback c : cb) {
                try {
                    if (ed == -1) {
                        ...
                        c.onFoundEdition(ed, origUSK.copy(ed), null, context, lastWasMetadata, lastCompressionCodec,
                                         data, false, false);
                    }
                } catch (Exception e) {
                    Logger.error(this,
                                 "An exception occured while dealing with a callback:" + c.toString() + "\n"
                                 + e.getMessage(), e);
                }
            }
        }
    ...
    private void updatePriorities() {
        ...
        if ((localCallbacks.length == 0) && (fetcherCallbacks.length == 0)) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Updating priorities: normal = " + normalPollPriority + " progress = "
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Updating priorities: normal=" + normalPrio + " progress=" + progressPrio + " for " + this
                         + " for " + origUSK);
        }
    ...
    @Override
    public void onFoundEdition(long ed, USK key, ObjectContainer container, final ClientContext context,
                               boolean metadata, short codec, byte[] data, boolean newKnownGood, boolean newSlotToo) {
    ...
    private void registerAttempts(ClientContext context) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Registering " + attempts.length + " USKChecker's for " + this + " running="
    ...
    @Override
    public boolean handleBlock(Key key, byte[] saltedKey, KeyBlock found, ObjectContainer container,
                               ClientContext context) {
    ...
    class DBRAttempt implements GetCompletionCallback {
        ...
        DBRAttempt(ClientKey key, ClientContext context, USKDateHint.Type type) {
            fetcher = new DBRFetcher(key, ctxDBR.maxUSKRetries, ctxDBR, parent, this, false, true, 0, null, context,
                                     false, isRealTime());
            this.type = type;
        ...
        private void innerSuccess(Bucket bucket, ObjectContainer container, ClientContext context) {
            byte[] data;
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Found DBR hint edition " + hint + " for " + this.fetcher.getKey(null, container).getURI()
                             + " for " + USKFetcher.this);
            }
        ...
        public void onFailure(FetchException e, ClientGetState state, ObjectContainer container,
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Failed to fetch hint " + fetcher.getKey(null, container) + " for " + this + " for "
                             + USKFetcher.this);
            }
    ...
    class DBRFetcher extends SimpleSingleFileFetcher {
        DBRFetcher(ClientKey key, int maxRetries, FetchContext ctx, ClientRequester parent, GetCompletionCallback rcb,
                   boolean isEssential, boolean dontAdd, long l, ObjectContainer container, ClientContext context,
                   boolean deleteFetchContext, boolean realTimeFlag) {
            super(key, maxRetries, ctx, parent, rcb, isEssential, dontAdd, l, container, context, deleteFetchContext,
                  realTimeFlag);
        }
    ...
    class StoreCheckerGetter extends SendableGet {
        ...
        public boolean preRegister(ObjectContainer container, ClientContext context, boolean toNetwork) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Checked datastore, finishing registration for " + attempts.length + " checkers for "
    ...
    private class USKWatchingKeys {
        ...
        public synchronized ToFetch getEditionsToFetch(long lookedUp, Random random, List<Lookup> alreadyRunning,
            ...
            if (doRandom) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Running random requests: " + runningRandom + " total allowed: " + allowedRandom
        ...
        public synchronized USKStoreChecker getDatastoreChecker(long lastSlot) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Getting datastore checker from " + lastSlot + " for " + origUSK + " on "
        ...
        class KeyList {
            ...
            public synchronized void getRandomEditions(List<Lookup> toFetch, long lookedUp,
                ...
                for (int i = 0; i < allowed; i++) {
                    while (true) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Trying random future edition " + fetch + " for " + origUSK
            ...
            private long match(NodeSSK key, long curBaseEdition, RemoveRangeArrayList<byte[]> ehDocnames) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Matching " + key + " cur base edition " + curBaseEdition + " first slot was "
            ...
            public class StoreSubChecker {
                ...
                private StoreSubChecker(NodeSSK[] keysToCheck, long checkFrom, long checkTo) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Checking datastore from " + checkFrom + " to " + checkTo + " for "
