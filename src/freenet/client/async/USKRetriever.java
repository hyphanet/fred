...
public class USKRetriever extends BaseClientGetter implements USKCallback {
    ...
    public void onFoundEdition(long l, USK key, ObjectContainer container, ClientContext context, boolean metadata,
        ...
        if (l < origUSK.suggestedEdition) {
            Logger.warning(this,
                           "Found edition prior to that specified by the client: " + l + " < "
                           + origUSK.suggestedEdition, new Exception("error"));
    ...
    public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
                          List<? extends Compressor> decompressors, final ClientGetState state,
                          ObjectContainer container, ClientContext context) {
        if (logMINOR) {
            Logger.minor(this,
                         "Success on " + this + " from " + state + " : length " + streamGenerator.size() + "mime type "
        ...
        try {
            ...
            if (decompressors != null) {
                ...
                ClientGetWorkerThread worker = new ClientGetWorkerThread(pipeIn, output, null, null, null, false, null,
                                                   null, null, context.linkFilterExceptionProvider);
    ...
    public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max, byte[] splitfileKey,
            boolean compressed, boolean bottomLayer, boolean definitiveAnyway, ObjectContainer container,
