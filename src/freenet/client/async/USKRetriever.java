...
public class USKRetriever extends BaseClientGetter implements USKCallback {
	...
	public USKRetriever(
		...
		if (client.persistent()) throw new UnsupportedOperationException(
			"USKRetriever cannot be persistent"
		);
		this.ctx = fctx;
		this.cb = cb;
	...
	public void onFoundEdition(
		...
		if (l < 0) {
			Logger.error(
				this,
				"Found negative edition: " + l + " for " + key + " !!!"
			);
			return;
		}
		...
		if (logMINOR) Logger.minor(
			this,
			"Found edition " + l + " for " + this + " - fetching..."
		);
		...
		try {
			SingleFileFetcher getter =
				(SingleFileFetcher) SingleFileFetcher.create(
					this,
					this,
					uri,
					ctx,
			...
			Logger.error(
				this,
				"Could not start fetcher for " + uri + " : " + e,
				e
			);
		}
	}
	...
	public void onSuccess(
		...
		try {
			...
			Logger.error(this, "Caught " + e, e);
			onFailure(
				new FetchException(FetchExceptionMode.INTERNAL_ERROR, e),
				state,
				context
			);
		} catch (Throwable t) {
	...
	public void onFailure(
		...
		switch (e.mode) {
			...
			case PERMANENT_REDIRECT:
				context.uskManager.updateKnownGood(
					origUSK,
					state.getToken(),
					context
				);
