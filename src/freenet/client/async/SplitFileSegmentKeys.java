...
public class SplitFileSegmentKeys implements Cloneable, Serializable {
	...
	public SplitFileSegmentKeys(
		...
		if (splitfileSingleCryptoKey != null) {
			...
			commonExtraBytes = ClientCHK.getExtra(
			...
			decryptKeys = new byte[ClientCHK.CRYPTO_KEY_LENGTH *
			(dataBlocks + checkBlocks)];
			extraBytesForKeys = new byte[EXTRA_BYTES_LENGTH *
			(dataBlocks + checkBlocks)];
		}
	}
	...
	private ClientCHK getKey(int x, boolean copy) {
		...
		if (commonDecryptKey != null) {
			...
			decryptKey = Arrays.copyOfRange(
				decryptKeys,
				offset,
				offset + ClientCHK.CRYPTO_KEY_LENGTH
			);
		}
		byte[] extra;
		if (commonExtraBytes != null) {
			...
			extra = Arrays.copyOfRange(
				extraBytesForKeys,
				offset,
				offset + EXTRA_BYTES_LENGTH
			);
		}
		try {
	...
	private NodeCHK getNodeKey(int x, boolean copy) {
		...
		byte[] routingKey = Arrays.copyOfRange(
			routingKeys,
			xr,
			xr + NodeCHK.KEY_LENGTH
		);
		byte[] extra;
		if (commonExtraBytes != null) {
			extra = commonExtraBytes;
	...
	public void readKeys(DataInputStream dis, boolean check)
		throws IOException {
	...
	public void writeKeys(DataOutputStream dos, boolean check)
		throws IOException {
		int count = check ? checkBlocks : dataBlocks;
		int offset = check ? dataBlocks : 0;
		if (commonDecryptKey != null) {
	...
	public static int storedKeysLength(
		int dataBlocks,
		int checkBlocks,
	...
	public void setKey(int i, ClientCHK key) {
		...
		if (decryptKeys != null) {
			...
			System.arraycopy(
				...
				0,
				decryptKeys,
				i * ClientCHK.CRYPTO_KEY_LENGTH,
				ClientCHK.CRYPTO_KEY_LENGTH
			);
		}
	...
	public boolean equals(Object obj) {
		...
		if (
			!Arrays.equals(commonDecryptKey, other.commonDecryptKey)
		) return false;
		if (
			!Arrays.equals(commonExtraBytes, other.commonExtraBytes)
		) return false;
		if (dataBlocks != other.dataBlocks) return false;
		if (!Arrays.equals(decryptKeys, other.decryptKeys)) return false;
		if (
			!Arrays.equals(extraBytesForKeys, other.extraBytesForKeys)
		) return false;
