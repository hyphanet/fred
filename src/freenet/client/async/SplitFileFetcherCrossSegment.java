...
public class SplitFileFetcherCrossSegment implements FECCallback {
    ...
    public SplitFileFetcherCrossSegment(boolean persistent, int blocksPerSegment, int crossCheckBlocks,
            ClientRequester parent, SplitFileFetcher fetcher, short splitfileType) {
    ...
    public void onFetched(SplitFileFetcherSegment segment, int blockNo, ObjectContainer container,
        ...
        synchronized (this) {
            ...
            if (shouldRemove || finishedEncoding || startedDecoding || startedEncoding) {
                return;
            }
            ...
            if (totalFound < dataBlocks) {
                Logger.normal(this,
                              "Not decoding " + this + " : found " + totalFound + " blocks of " + dataBlocks
    ...
    private boolean decodeOrEncode(SplitFileFetcherSegment segment, ObjectContainer container, ClientContext context) {
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            if (found) {
                ...
                if (data == null) {
                    Logger.error(this,
                                 "Cannot decode/encode: Found block " + i + " : " + blockNumbers[i] + " of "
                                 + segments[i] + " but is gone now!", new Exception("error"));
                    data = seg.getBlockBucket(blockNumbers[i], container);

                    if (data == null) {
                        if (seg.isFinished(container)) {
                            Logger.error(this,
                                         "SEGMENT IS FINISHED ALREADY when trying to decode/encode splitfile block: "
                                         + segments[i]);
                        } else if (seg.isFinishing(container)) {
                            Logger.error(this,
                                         "SEGMENT IS FINISHING ALREADY when trying to decode/encode splitfile block: "
                                         + segments[i]);
                        }
                        ...
                        if (fetcher != null) {
                            ...
                            fetcher.onFailed(new FetchException(FetchException.INTERNAL_ERROR,
                                    "Cannot decode/encode cross blocks: lost a block"), container, context);
                        }
        ...
        FECJob job = new FECJob(codec, queue, decodeData, decodeCheck, CHKBlock.DATA_LENGTH,
                                context.getBucketFactory(persistent), this, needsDecode, getPriorityClass(container),
                                persistent);
            ...
            if (found) {
                ...
                if (segData != data) {
                    ...
                    if (persistent && (container.ext().getID(segData) == container.ext().getID(data))) {
                        Logger.error(this,
                                     "SAME ID BUG FOUND IN CROSS SEGMENT DECODE: " + segData + " has same ID as "
                                     + data);
    ...
    public void onEncodedSegment(ObjectContainer container, ClientContext context, FECJob job, Bucket[] dataBuckets,
        ...
        for (int i = 0; i < checkBlocks.length; i++) {
            ...
            synchronized (this) {
                ...
                if (data == null) {
                    Logger.error(this,
                                 "Check block " + i + " is null in onEncodedSegment on " + this + " - shouldRemove = "
                                 + shouldRemove);
            ...
            if (persistent) {
                container.delete(checkBlocks[i]);
            }
        }
