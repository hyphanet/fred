...
public class SplitFileFetcherStorage {
	...
	private final RandomArrayIterator<
		SplitFileFetcherSegmentStorage
	> randomSegmentIterator;
	...
	public SplitFileFetcherStorage(
		...
		if (decompressors.size() > 1) {
			Logger.error(
				this,
				"Multiple decompressors: " +
				decompressors.size() +
				" - this is almost certainly a bug",
				new Exception("debug")
			);
		}
		this.clientMetadata = clientMetadata == null
			? new ClientMetadata()
		...
		this.splitfileSingleCryptoAlgorithm =
			metadata.getSplitfileCryptoAlgorithm();
		...
		if (completeViaTruncation) {
			storedCrossCheckBlocksLength = (long) totalCrossCheckBlocks *
			CHKBlock.DATA_LENGTH;
			storedBlocksLength = (long) splitfileDataBlocks *
			CHKBlock.DATA_LENGTH;
		} else {
		...
		if (segmentCount <= 0) {
			throw new AssertionError(
				"A splitfile has to have at least one segment"
			);
		}
		...
		if (splitfileType == SplitfileAlgorithm.NONREDUNDANT) {
			if (splitfileCheckBlocks > 0) {
				...
				throw new FetchException(
					...
					"Splitfile type is non-redundant yet have " +
					splitfileCheckBlocks +
					" check blocks"
				);
			}
			...
			if (topCompatibilityMode != 0) {
				...
				if (
					minCompatMode == CompatibilityMode.COMPAT_UNKNOWN ||
					!(minCompatMode.ordinal() > topCompatibilityMode ||
						maxCompatMode.ordinal() < topCompatibilityMode)
				) {
					minCompatMode = maxCompatMode =
						CompatibilityMode.values()[topCompatibilityMode];
					dontCompress = topDontCompress;
				} else throw new FetchException(
					FetchExceptionMode.INVALID_METADATA,
			...
			if (
				(blocksPerSegment > origFetchContext.maxDataBlocksPerSegment) ||
				(checkBlocksPerSegment >
					origFetchContext.maxCheckBlocksPerSegment)
			) throw new FetchException(
		...
		} else throw new MetadataParseException(
			"Unknown splitfile format: " + splitfileType
		);
		...
		randomSegmentIterator = new RandomArrayIterator<
			SplitFileFetcherSegmentStorage
		>(segments);

		long checkLength =
			1L *
			(splitfileDataBlocks - segmentCount * crossCheckBlocks) *
			CHKBlock.DATA_LENGTH;
		if (checkLength > finalLength) {
			if (
				checkLength - finalLength > CHKBlock.DATA_LENGTH
			) throw new FetchException(
				FetchExceptionMode.INVALID_METADATA,
				"Splitfile is " +
				checkLength +
				" bytes long but length is " +
				finalLength +
				" bytes"
			);
		}
		...
		keyListener = new SplitFileFetcherKeyListener(
			fetcher,
			this,
			false,
			localSalt,
		...
		if (persistent) {
			offsetGeneralProgress = offsetSegmentStatus +
			storedSegmentStatusLength;
			this.offsetMainBloomFilter = offsetGeneralProgress +
			generalProgress.length;
			this.offsetSegmentBloomFilters = offsetMainBloomFilter +
			keyListener.paddedMainBloomFilterSize();
			...
			offsetGeneralProgress = offsetMainBloomFilter =
				offsetSegmentBloomFilters = offsetOriginalMetadata =
					offsetSegmentStatus;
		}
		...
		for (int i = 0; i < segments.length; i++) {
			...
			if (
				(dataBlocks > origFetchContext.maxDataBlocksPerSegment) ||
			...
			segments[i] = new SplitFileFetcherSegmentStorage(
				this,
				i,
				splitfileType,
				dataBlocks,
			...
			if (!completeViaTruncation) {
				...
				crossCheckBlocksOffset +=
				crossCheckBlocks * CHKBlock.DATA_LENGTH;
			}
			segmentKeysOffset +=
			SplitFileFetcherSegmentStorage.storedKeysLength(
				dataBlocks + crossCheckBlocks,
			...
			for (
				int j = 0;
				j < (dataBlocks + crossCheckBlocks + checkBlocks);
				j++
			) {
				keyListener.addKey(
					keys.getKey(j, null, false).getNodeKey(false),
					i,
					salt
				);
			}
		...
		if (completeViaTruncation) assert (crossCheckBlocksOffset ==
			storedCrossCheckBlocksLength + storedBlocksLength);
		...
		if (crossCheckBlocks != 0) {
			...
			int deductBlocksFromSegments =
				metadata.getDeductBlocksFromSegments();
			for (int i = 0; i < crossSegments.length; i++) {
				Logger.normal(
					this,
					"Allocating blocks (on fetch) for cross segment " + i
				);
		...
		if (persistent) {
			...
			try {
				...
				MultiHashOutputStream mos = new MultiHashOutputStream(
					bos,
					HashType.SHA256.bitmask
				);
			...
			encodedURI = encodeAndChecksumOriginalDetails(
				thisKey,
				origKey,
				clientDetails,
				isFinalFetch
			);
			this.offsetBasicSettings = offsetOriginalDetails +
			encodedURI.length;
		...
		if (storageFile != null) {
			if (!storageFile.exists()) throw new IOException(
				"Must have already created storage file"
			);
			if (storageFile.length() > 0) throw new IOException(
				"Storage file must be empty"
			);
			raf = diskSpaceCheckingRAFFactory.createNewRAF(
				storageFile,
				totalLength,
				random
			);
			Logger.normal(
				this,
				"Creating splitfile storage file for complete-via-truncation: " +
				storageFile
			);
		} else {
		...
		try {
			...
			if (persistent) {
				...
				keyListener.initialWriteSegmentBloomFilters(
				...
				byte[] bufToChecksum = Arrays.copyOf(
					bufToWrite,
					bufToWrite.length + version.length
				);
				System.arraycopy(
					version,
					0,
				...
				byte[] checksum = checksumChecker.generateChecksum(
					bufToChecksum
				);
				// Pointers.
				raf.pwrite(
					offsetBasicSettings + encodedBasicSettings.length,
					bufToWrite,
					0,
					bufToWrite.length
				);
		...
		if (logMINOR) Logger.minor(
			this,
			"Fetching " + thisKey + " on " + this + " for " + fetcher
		);
	}
	...
	public SplitFileFetcherStorage(
		...
		rafLength = raf.size();
		if (raf.size() < 8/* FIXME more! */) throw new StorageFormatException(
			"Too short"
		);
		...
		if (dis.readLong() != END_MAGIC) throw new StorageFormatException(
			"Wrong magic bytes"
		);
		...
		if (version != 1) throw new StorageFormatException(
		...
		if (
			checksumType != ChecksumChecker.CHECKSUM_CRC
		...
		if (flags != 0) throw new StorageFormatException(
			"Unknown flags: " + flags
		);
		...
		if (
			!checksumChecker.checkChecksum(buf, 0, 14, checksum)
		) throw new StorageFormatException(
			"Checksum failed on basic settings length and version"
		);
		dis = new DataInputStream(new ByteArrayInputStream(buf));
		int basicSettingsLength = dis.readInt();
		if (
			basicSettingsLength < 0 ||
		...
		long basicSettingsOffset =
			rafLength - (18 + 4 + checksumLength * 2 + basicSettingsLength);
		try {
			preadChecksummed(
				basicSettingsOffset,
				basicSettingsBuffer,
		...
		dis = new DataInputStream(
			new ByteArrayInputStream(basicSettingsBuffer)
		);
		try {
			...
			if (
				!Metadata.isValidSplitfileCryptoAlgorithm(
					splitfileSingleCryptoAlgorithm
				)
			) throw new StorageFormatException(
				"Invalid splitfile crypto algorithm " + splitfileType
			);
			if (dis.readBoolean()) {
			...
			if (finalLength < 0) throw new StorageFormatException(
				"Invalid final length " + finalLength
			);
			decompressedLength = dis.readLong();
			if (decompressedLength < 0) throw new StorageFormatException(
				"Invalid decompressed length " + decompressedLength
			);
			try {
			...
			if (decompressorCount < 0) throw new StorageFormatException(
				"Invalid decompressor count " + decompressorCount
			);
			decompressors = new ArrayList<COMPRESSOR_TYPE>(decompressorCount);
			for (int i = 0; i < decompressorCount; i++) {
				short type = dis.readShort();
				COMPRESSOR_TYPE d = COMPRESSOR_TYPE.getCompressorByMetadataID(
					type
				);
				if (d == null) throw new StorageFormatException(
					"Invalid decompressor ID " + type
				);
				decompressors.add(d);
			}
			offsetKeyList = dis.readLong();
			if (
				offsetKeyList < 0 || offsetKeyList > rafLength
			) throw new StorageFormatException("Invalid offset (key list)");
			offsetSegmentStatus = dis.readLong();
			if (
				offsetSegmentStatus < 0 || offsetSegmentStatus > rafLength
			) throw new StorageFormatException(
				"Invalid offset (segment status)"
			);
			offsetGeneralProgress = dis.readLong();
			if (
				offsetGeneralProgress < 0 || offsetGeneralProgress > rafLength
			) throw new StorageFormatException(
				"Invalid offset (general progress)"
			);
			offsetMainBloomFilter = dis.readLong();
			if (
				offsetMainBloomFilter < 0 || offsetMainBloomFilter > rafLength
			) throw new StorageFormatException(
				"Invalid offset (main bloom filter)"
			);
			offsetSegmentBloomFilters = dis.readLong();
			if (
				offsetSegmentBloomFilters < 0 ||
				offsetSegmentBloomFilters > rafLength
			) throw new StorageFormatException(
				"Invalid offset (segment bloom filters)"
			);
			offsetOriginalMetadata = dis.readLong();
			if (
				offsetOriginalMetadata < 0 || offsetOriginalMetadata > rafLength
			) throw new StorageFormatException(
				"Invalid offset (original metadata)"
			);
			offsetOriginalDetails = dis.readLong();
			if (
				offsetOriginalDetails < 0 || offsetOriginalDetails > rafLength
			) throw new StorageFormatException(
				"Invalid offset (original metadata)"
			);
			offsetBasicSettings = dis.readLong();
			if (
				offsetBasicSettings != basicSettingsOffset
			) throw new StorageFormatException(
				"Invalid basic settings offset (not the same as computed)"
			);
			if (
				completeViaTruncation != dis.readBoolean()
			) throw new StorageFormatException(
				"Complete via truncation flag is wrong"
			);
			int compatMode = dis.readInt();
			if (
				compatMode < 0 || compatMode > CompatibilityMode.values().length
			) throw new StorageFormatException(
				"Invalid compatibility mode " + compatMode
			);
			finalMinCompatMode = CompatibilityMode.values()[compatMode];
			int segmentCount = dis.readInt();
			if (segmentCount <= 0) throw new StorageFormatException(
				"Invalid segment count " + segmentCount
			);
			...
			if (totalDataBlocks < 0) throw new StorageFormatException(
				"Invalid total data blocks " + totalDataBlocks
			);
			int totalCheckBlocks = dis.readInt();
			if (totalCheckBlocks < 0) throw new StorageFormatException(
				"Invalid total check blocks " + totalDataBlocks
			);
			int totalCrossCheckBlocks = dis.readInt();
			if (totalCrossCheckBlocks < 0) throw new StorageFormatException(
				"Invalid total cross-check blocks " + totalDataBlocks
			);
			if (
				totalDataBlocks + totalCheckBlocks + totalCrossCheckBlocks <= 0
			) {
				throw new StorageFormatException(
					"Total number of blocks in splitfile is non-positive"
				);
			}
			long dataOffset = 0;
			...
			for (int i = 0; i < segments.length; i++) {
				...
				dataOffset += dataBlocks * CHKBlock.DATA_LENGTH;
				if (completeViaTruncation) crossCheckBlocksOffset +=
				crossCheckBlocks * CHKBlock.DATA_LENGTH;
				...
				if (dataOffset > rafLength) throw new StorageFormatException(
					"Data offset past end of file " +
					dataOffset +
					" of " +
					rafLength
				);
				if (
					segments[i].segmentCrossCheckBlockDataOffset > rafLength
				) throw new StorageFormatException(
			...
			if (
				countDataBlocks != totalDataBlocks
			) throw new StorageFormatException(
				"Total data blocks " +
				countDataBlocks +
				" but expected " +
			...
			if (
				countCheckBlocks != totalCheckBlocks
			) throw new StorageFormatException(
				"Total check blocks " +
				countCheckBlocks +
				" but expected " +
			...
			if (
				countCrossCheckBlocks != totalCrossCheckBlocks
			) throw new StorageFormatException(
			...
			else this.crossSegments =
				new SplitFileFetcherCrossSegmentStorage[crossSegments];
			for (int i = 0; i < crossSegments; i++) {
				this.crossSegments[i] = new SplitFileFetcherCrossSegmentStorage(
					this,
					i,
					dis
				);
			}
			this.keyListener = new SplitFileFetcherKeyListener(
				this,
				fetcher,
				dis,
				false,
			...
			throw new StorageFormatException(
				"Cannot read basic settings even though passed checksum: " + e,
				e
			);
		}
		for (SplitFileFetcherSegmentStorage segment : segments) {
			...
			try {
				...
				if (segment.hasFailed()) {
					...
					throw new FetchException(
						FetchExceptionMode.SPLITFILE_ERROR,
						errors
					);
				}
			} catch (ChecksumFailedException e) {
			...
			if (needsDecode) {
				if (segmentsToTryDecode == null) segmentsToTryDecode =
					new ArrayList<SplitFileFetcherSegmentStorage>();
	...
	private void readGeneralProgress() throws IOException {
		try {
			...
			if ((flags & HAS_CHECKED_DATASTORE_FLAG) != 0) hasCheckedDatastore =
				true;
	...
	private byte[] encodeGeneralProgress() {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try {
			OutputStream ccos = checksumChecker.checksumWriterWithLength(
				baos,
	...
	public boolean start(boolean resume) {
		if (resume) {
			int splitfileDataBlocks = 0, splitfileCheckBlocks =
				0, totalCrossCheckBlocks = 0;
						...
						new PersistentJob() {
							...
							public boolean run(ClientContext context) {
								...
								try {
									...
									keyListener.innerWriteMainBloomFilter(
								...
								Logger.warning(
									this,
									"Finished regenerating filters for " +
									SplitFileFetcherStorage.this
								);
								System.out.println(
									"Finished regenerating filters for " +
	...
	private byte[] innerEncodeBasicSettings(
		...
		try {
			...
			for (COMPRESSOR_TYPE c : decompressors) dos.writeShort(
				c.metadataID
			);
	...
	public void finishedSuccess(SplitFileFetcherSegmentStorage segment) {
		...
		if (
			!(completeViaTruncation || fetcher.wantBinaryBlob())
		) maybeComplete();
	}
	...
	private void maybeComplete() {
		if (allSucceeded()) {
			...
			// Some failed.
			fail(
				new FetchException(FetchExceptionMode.SPLITFILE_ERROR, errors)
			);
		}
	}
		...
		public boolean run(ClientContext context) {
			try {
				...
				try {
					...
					keyListener.maybeWriteMainBloomFilter(
						offsetMainBloomFilter
					);
				} finally {
					lock.unlock();
	...
	public void fail(final FetchException e) {
		if (logMINOR) Logger.minor(
			this,
			"Failing " + this + " with error " + e + " and codes " + errors
		);
		jobRunner.queueNormalOrDrop(
			new PersistentJob() {
				@Override
	...
	public long countUnfetchedKeys() {
		long total = 0;
		for (SplitFileFetcherSegmentStorage segment : segments) total +=
		segment.countUnfetchedKeys();
		return total;
	}
	...
	public Key[] listUnfetchedKeys() {
		try {
			...
			for (SplitFileFetcherSegmentStorage segment : segments) segment.getUnfetchedKeys(
				keys
			);
	...
	public long countSendableKeys() {
		...
		long total = 0;
		for (SplitFileFetcherSegmentStorage segment : segments) total +=
		segment.countSendableKeys(now, maxRetries);
		return total;
	}

	final class SplitFileFetcherStorageKey
		implements SendableRequestItem, SendableRequestItemKey {

		public SplitFileFetcherStorageKey(
			int n,
			int segNo,
		...
		public boolean equals(Object o) {
			...
			return (
				k.blockNumber == blockNumber &&
				k.segmentNumber == segmentNumber &&
				k.get == get
			);
		}
	...
	public SplitFileFetcherStorageKey chooseRandomKey() {
		...
		synchronized (randomSegmentIterator) {
			...
			while (randomSegmentIterator.hasNext()) {
				SplitFileFetcherSegmentStorage segment =
					randomSegmentIterator.next();
				int ret = segment.chooseRandomKey();
				if (ret != -1) {
					return new SplitFileFetcherStorageKey(
						ret,
						segment.segNo,
						this
					);
	...
	public ClientKey getKey(SplitFileFetcherStorageKey key) {
		try {
			return segments[key.segmentNumber].getSegmentKeys()
				.getKey(key.blockNumber, null, false);
		} catch (IOException e) {
	...
	public boolean lastBlockMightNotBePadded() {
		return (
			finalMinCompatMode == CompatibilityMode.COMPAT_UNKNOWN ||
			finalMinCompatMode.ordinal() <
				CompatibilityMode.COMPAT_1416.ordinal()
		);
	}
			...
			new PersistentJob() {
				...
				public boolean run(ClientContext context) {
					...
					synchronized (cooldownLock) {
						...
						if (
							overallCooldownWakeupTime < oldCooldownTime
						) return false;
					}
	...
	byte[] preadChecksummedWithLength(long fileOffset)
		...
		try {
			...
			long len = new DataInputStream(
				new ByteArrayInputStream(lengthBuf)
			).readLong();
			if (
				len + fileOffset > rafLength ||
			...
			raf.pread(
				fileOffset + length + lengthBuf.length,
				checksumBuf,
				0,
				checksumLength
			);
	...
	OutputStream writeChecksummedTo(final long fileOffset, final int length) {
		...
		return new FilterOutputStream(cos) {
			public void close() throws IOException {
				out.close();
				byte[] buf = baos.toByteArray();
				if (buf.length != length) throw new IllegalStateException(
