...
public class SplitFileFetcherCrossSegmentStorage {
	...
	public void onFetchedRelevantBlock(
	...
	private synchronized void tryDecodeOrEncode(final short prio) {
		...
		long limit =
			...
			Math.max(
				parent.fecCodec.maxMemoryOverheadDecode(
					dataBlockCount,
					crossCheckBlockCount
				),
				parent.fecCodec.maxMemoryOverheadEncode(
					dataBlockCount,
					crossCheckBlockCount
				)
			);
					...
					try {
						...
						try {
							if (!shutdown) {
								...
								synchronized (
									SplitFileFetcherCrossSegmentStorage.this
								) {
									tryDecode = false;
								}
								parent.finishedEncoding(
									SplitFileFetcherCrossSegmentStorage.this
								);
							}
						} finally {
	...
	private void innerDecode(MemoryLimitedChunk chunk) throws IOException {
		if (logMINOR) Logger.minor(
			this,
			"Trying to decode " + this + " for " + parent
		);
		...
		if (logMINOR) Logger.minor(
			this,
			"Completed a cross-segment: decoded=" +
			decoded +
			" encoded=" +
			encoded
		);
	...
	private void checkDecodedBlock(int i, byte[] data) {
		...
		if (block == null || !key.getNodeCHK().equals(block.getKey())) {
			Logger.error(
				this,
				decoded + " cross-segment block " + i + " failed!"
			);
			failOffThread(
				new FetchException(
				...
				public boolean run(ClientContext context) {
					try {
						...
						SplitFileSegmentKeys keys =
							segments[blockNo].getSegmentKeys();
						...
						boolean success =
							segments[blockNo].innerOnGotKey(
									...
									block,
									keys,
									blockNumbers[blockNo],
									data
								);
						if (success) {
							if (logMINOR) Logger.minor(
								this,
								"Successfully decoded cross-segment block"
							);
						} else {
							// Not really a big deal, but potentially interesting...
							Logger.warning(
								this,
								"Decoded cross-segment block but not wanted by segment"
							);
						}
					} catch (IOException e) {
						parent.failOnDiskError(e);
	...
	private byte[][] readBlocks(boolean checkBlocks) {
		...
		for (int i = start; i < end; i++) {
			try {
				byte[] block =
					segments[i].checkAndGetBlockData(blockNumbers[i]);
