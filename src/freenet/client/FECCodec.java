...
public abstract class FECCodec {
    ...
    public static FECCodec getCodec(short splitfileType, int dataBlocks, int checkBlocks) {
        if (logMINOR) {
            Logger.minor(FECCodec.class,
                         "getCodec: splitfileType=" + splitfileType + " dataBlocks=" + dataBlocks + " checkBlocks="
                         + checkBlocks);
        }
    ...
    protected void realDecode(SplitfileBlock[] dataBlockStatus, SplitfileBlock[] checkBlockStatus, int blockLength,
                              BucketFactory bf)
            throws IOException {
        loadFEC();
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Doing decode: " + dataBlockStatus.length + " data blocks, " + checkBlockStatus.length
                         + " check blocks, block length " + blockLength + " with " + this, new Exception("debug"));
        }
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                packets[i] = new Buffer(realBuffer, i * stripeSize, stripeSize);
            }
                ...
                if (buckets[i] == null) {
                    ...
                    if (sz < blockLength) {
                        if (i != dataBlockStatus.length - 1) {
                            throw new IllegalArgumentException(
                                "All buckets must be the full size (caller must pad if needed) but data bucket " + i
                                + " of " + dataBlockStatus.length + " (" + dataBlockStatus[i] + ") is " + sz + " not "
                                + blockLength);
                        }
                    } else {
                        if (logMINOR) {
            ...
            for (int offset = 0; offset < blockLength; offset += stripeSize) {
                ...
                for (int i = 0; i < k; i++) {
                    ...
                    try {
                        dis.readFully(realBuffer, i * stripeSize, stripeSize);
                    } finally {
                        if (stripeSize == blockLength) {
                            dis.close();
                        }
                ...
                for (int i = 0; i < k; i++) {
                    if (toWrite[i]) {
                        ...
                        try {
                            os.write(realBuffer, i * stripeSize, stripeSize);
                        } finally {
                            if (stripeSize == blockLength) {
                                os.close();
                            }
        ...
        for (int i = 0; i < dataBlockStatus.length; i++) {
            ...
            if (data.size() != blockLength) {
                throw new IllegalStateException("Block " + i + ": " + data + " : " + dataBlockStatus[i] + " length "
                                                + data.size() + " whereas blockLength=" + blockLength);
            }
            ...
            if ((existingData != null) && (existingData != data)) {
                if (logMINOR) {
                    Logger.minor(this, "Discarding block " + i + " as now unneeded");
                }
     ...
     */
    protected void realEncode(Bucket[] dataBlockStatus, Bucket[] checkBlockStatus, int blockLength, BucketFactory bf)
            throws IOException {
        if (bf == null) {
...
//      Runtime.getRuntime().runFinalization();
        if (logMINOR) {
            ...
            Logger.minor(this,
                         "Memory in use at start: " + memUsedAtStart + " max=" + Runtime.getRuntime().maxMemory());
            Logger.minor(this,
                         "Doing encode: " + dataBlockStatus.length + " data blocks, " + checkBlockStatus.length
                         + " check blocks, block length " + blockLength + " with " + this);
        }

        if ((dataBlockStatus.length + checkBlockStatus.length != n) || (dataBlockStatus.length != k)) {
            throw new IllegalArgumentException("Data blocks: " + dataBlockStatus.length + ", Check blocks: "
                                               + checkBlockStatus.length + ", n: " + n + ", k: " + k);
        }
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                dataPackets[i] = new Buffer(realBuffer, i * stripeSize, stripeSize);
            }
            ...
            for (int i = 0; i < dataBlockStatus.length; i++) {
                ...
                if (sz < blockLength) {
                    throw new IllegalArgumentException(
                        "All buckets must be the full size: caller must pad the last one if needed");
                }
            ...
            if (numberToEncode > 0) {
                ...
                for (int offset = 0; offset < blockLength; offset += stripeSize) {
                    ...
                    if (logMINOR) {
                        ...
                        long memUsedBeforeStripe = Runtime.getRuntime().totalMemory()
                    ...
                    if (logMINOR) {
                        long memUsedAfterStripe = Runtime.getRuntime().totalMemory()
                                                  - Runtime.getRuntime().freeMemory();
                        ...
                        Logger.minor(this,
                                     "Stripe encode took " + (endTime - startTime) + "ms for k=" + k + ", n=" + n
