...
public class FetchException extends Exception implements Cloneable {
	...
	public FetchException(FetchExceptionMode m) {
		...
		newURI = null;
		expectedSize = -1;
		if (mode == FetchExceptionMode.INTERNAL_ERROR) Logger.error(
			this,
			"Internal error: " + this
		);
	...
	public FetchException(MetadataParseException e) {
		super(
			getMessage(FetchExceptionMode.INVALID_METADATA) +
			": " +
			e.getMessage()
		);
	...
	public FetchException(ArchiveFailureException e) {
		super(
			getMessage(FetchExceptionMode.ARCHIVE_FAILURE) +
			": " +
			e.getMessage()
		);
		extraMessage = e.getMessage();
	...
	public FetchException(ArchiveRestartException e) {
		super(
			getMessage(FetchExceptionMode.ARCHIVE_RESTART) +
			": " +
			e.getMessage()
		);
		extraMessage = e.getMessage();
	...
	public FetchException(
		...
		super(
			...
			" " +
			NodeL10n.getBase()
				.getString("FetchException.unsafeContentDetails") +
			" " +
			t.getMessage()
		);
		extraMessage = t.getMessage();
	...
	public FetchException(FetchException e, FetchExceptionMode newMode) {
		super(
			getMessage(newMode) +
			(e.extraMessage != null ? ": " + e.extraMessage : "")
		);
		this.mode = newMode;
	...
	public static String getShortMessage(FetchExceptionMode mode) {
		...
		int code = mode.code;
		String ret = NodeL10n.getBase()
			.getString("FetchException.shortError." + code);
	...
	public static String getMessage(FetchExceptionMode mode) {
		...
		// FIXME change the l10n to use the names rather than codes
		String ret = NodeL10n.getBase()
			.getString("FetchException.longError." + code);
	...
	public static enum FetchExceptionMode {
		...
		FetchExceptionMode(int code) {
			this.code = code;
			if (
				code < 0 || code >= UPPER_LIMIT_ERROR_CODE
		...
		switch (mode) {
			...
			default:
				Logger.error(
					...
					"Do not know if error code is fatal: " + getMessage(mode)
				);
	...
	public static boolean isErrorCode(int code) {
		return (
			code >= 0 && code <= MAX_ERROR_CODE && code < UPPER_LIMIT_ERROR_CODE
		);
	}
}
