...
public class InsertContext implements Cloneable, Serializable {
	...
	public static enum CompatibilityMode {
		...
		static {
			...
			for (CompatibilityMode mode : CompatibilityMode.values) {
				if (cmodes.containsKey(mode.code)) throw new Error(
					"Duplicated code!"
				);
	...
	public InsertContext(
		...
		this.extraInsertsSplitfileHeaderBlock =
	...
	public boolean equals(Object obj) {
		...
		if (compressorDescriptor == null) {
			...
			!compressorDescriptor.equals(other.compressorDescriptor)
		) return false;
		if (
			consecutiveRNFsCountAsSuccess != other.consecutiveRNFsCountAsSuccess
		) return false;
		...
		if (
			extraInsertsSplitfileHeaderBlock !=
			other.extraInsertsSplitfileHeaderBlock
		) return false;
		...
		if (
			splitfileSegmentCheckBlocks != other.splitfileSegmentCheckBlocks
		) return false;
		if (
			splitfileSegmentDataBlocks != other.splitfileSegmentDataBlocks
		) return false;
		return true;
	}
	...
	public void onResume() {
		...
		if (realCompatMode == null) realCompatMode = CompatibilityMode.byCode(
			(short) compatibilityMode
		);
