...
public class FailureCodeTracker implements Cloneable, Serializable {
	...
	public synchronized void inc(int k) {
		if (k == 0) {
			Logger.error(
				this,
				"Can't increment 0, not a valid failure mode",
				new Exception("error")
			);
		}
	...
	public void merge(FetchException e) {
		if (insert) throw new IllegalStateException(
			"Merging a FetchException in an insert!"
		);
	...
	public synchronized SimpleFieldSet toFieldSet(boolean verbose) {
		...
		if (map != null) {
			for (Map.Entry<Integer, Integer> e : map.entrySet()) {
				...
				if (verbose) sfs.putSingle(
					Integer.toString(code) + ".Description",
					getMessage(code)
				);
	...
	public synchronized boolean isFatal(boolean insert) {
		...
		for (Map.Entry<Integer, Integer> e : map.entrySet()) {
			...
			if (insert) {
				if (
					InsertException.isFatal(InsertExceptionMode.getByCode(code))
				) return true;
			} else {
				if (
					FetchException.isFatal(FetchExceptionMode.getByCode(code))
				) return true;
			}
		}
		return false;
	...
	public void merge(InsertException e) {
		if (!insert) throw new IllegalArgumentException(
			"This is not an insert yet merge(" + e + ") called!"
		);
	...
	public synchronized boolean isDataFound() {
		...
		for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
			...
			if (
				FetchException.isDataFound(
					FetchExceptionMode.getByCode(entry.getKey()),
					null
				)
			) return true;
		}
	...
	public static int getFixedLength(boolean insert) {
		int upperLimit = insert
			? InsertException.UPPER_LIMIT_ERROR_CODE
			: FetchException.UPPER_LIMIT_ERROR_CODE;
	...
	public synchronized void writeFixedLengthTo(DataOutputStream dos)
		throws IOException {
		int upperLimit = insert
			? InsertException.UPPER_LIMIT_ERROR_CODE
			: FetchException.UPPER_LIMIT_ERROR_CODE;
	...
	public FailureCodeTracker(boolean insert, DataInputStream dis)
		...
		this.insert = insert;
		if (dis.readInt() != MAGIC) throw new StorageFormatException(
			"Bad magic for FailureCodeTracker"
		);
		if (dis.readInt() != VERSION) throw new StorageFormatException(
			"Bad version for FailureCodeTracker"
		);
		int upperLimit = insert
			? InsertException.UPPER_LIMIT_ERROR_CODE
			: FetchException.UPPER_LIMIT_ERROR_CODE;
		if (dis.readInt() != upperLimit) throw new StorageFormatException(
			"Bad upper limit for FailureCodeTracker"
		);
