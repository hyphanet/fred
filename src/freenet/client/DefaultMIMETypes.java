...
public class DefaultMIMETypes {
	...
	private static HashMap<String, Short> mimeTypesByName = new HashMap<
		String,
		Short
	>();
	...
	private static HashMap<String, Short> mimeTypesByExtension = new HashMap<
		String,
		Short
	>();

	/** Primary extension by MIME type number. */
	private static HashMap<Short, String> primaryExtensionByMimeNumber =
	...
	protected static synchronized void addMIMEType(
		...
		if (extensions != null) {
			for (String ext : extensions) {
				...
				if (s != null) {
					...
					if (
						(outExtension == null) && (extensions.length == 1)
		...
		if (outExtension != null) primaryExtensionByMimeNumber.put(
			t,
			outExtension
		);
	}
	...
	public static synchronized String guessMIMEType(
		String arg,
		boolean noDefault
	) {
		int x = arg.lastIndexOf('.');
		if ((x == -1) || (x == arg.length() - 1)) return noDefault
			? null
			: DEFAULT_MIME_TYPE;
	...
	public static synchronized boolean isValidExt(
		...
		for (String extension : extensions) if (
			oldExt.equalsIgnoreCase(extension)
		) return true;
		return false;
	}

	public static boolean isValidExt(
	...
	private static final String PARAM =
		"(?>;\\s*" + CHARS + "=" + "((" + CHARS + ")|(\".*\")))";
	private static Pattern MIME_TYPE = Pattern.compile(
		TOP_LEVEL + "/" + CHARS + "\\s*" + PARAM + "*"
	);

	private static Pattern INFOCALYPSE_DIRTY_HACK = Pattern.compile(
		"application/mercurial-bundle;[0-9]{1,6}"
	);
