...
class ArchiveHandlerImpl implements ArchiveHandler {
    ...
    @Override
    public Bucket getMetadata(ArchiveContext archiveContext, ArchiveManager manager, ObjectContainer container)
            throws ArchiveFailureException, ArchiveRestartException, MetadataParseException, FetchException {
    ...
    public void extractToCache(Bucket bucket, ArchiveContext actx, String element, ArchiveExtractCallback callback,
                               ArchiveManager manager, ObjectContainer container, ClientContext context)
        ...
        manager.extractToCache(key, archiveType, compressorType, bucket, actx, ctx, element, callback, container,
                               context);
    }
     ...
     */
    @Override
    public void extractPersistentOffThread(Bucket bucket, boolean freeBucket, ArchiveContext actx, String element,
            ArchiveExtractCallback callback, ObjectContainer container, final ClientContext context) {
        assert(element != null);    // no callback would be called...

        final ArchiveManager manager = context.archiveManager;
        final ArchiveExtractTag tag = new ArchiveExtractTag(this, bucket, freeBucket, actx, element, callback,
    ...
    private static void runPersistentOffThread(final ArchiveExtractTag tag, final ClientContext context,
            final ArchiveManager manager, final BucketFactory bf) {
        final ProxyCallback proxyCallback = new ProxyCallback();

        if (logMINOR) {
            Logger.minor(ArchiveHandlerImpl.class,
                         "Scheduling off-thread extraction: " + tag.data + " for " + tag.handler.key + " element "
        ...
        context.mainExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Extracting off-thread: " + tag.data + " for " + tag.handler.key + " element "
                    ...
                    context.jobRunner.queue(new DBJob() {
                        ...
                        public boolean run(ObjectContainer container, ClientContext context) {
                            if (logMINOR) {
                                Logger.minor(this,
                                             "Calling callback for " + tag.data + " for " + tag.handler.key
    ...
    static void init(ObjectContainer container, ClientContext context, final long nodeDBHandle) {
        ...
        while (set.hasNext()) {
            ...
            if (tag.checkBroken(container, context)) {
                continue;
            }
    ...
    public void removeFrom(ObjectContainer container) {
        if (key == null) {
            Logger.error(this,
                         "removeFrom() : key = null for " + this + " I exist = " + container.ext().isStored(this)
                         + " I am active: " + container.ext().isActive(this), new Exception("error"));
        } else {
