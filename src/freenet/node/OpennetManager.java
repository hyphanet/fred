...
public class OpennetManager {
    ...
    public enum ConnectionType { PATH_FOLDING, ANNOUNCE, RECONNECT }
    ...
    public OpennetPeerNode addNewOpennetNode(SimpleFieldSet fs, ConnectionType connectionType, boolean allowExisting)
        ...
        try {
            ...
            if (wantPeer(pn, true, false, false, connectionType)) {
                return pn;
            } else {
                return null;
            }
    ...
    public boolean wantPeer(PeerNode nodeToAddNow, boolean addAtLRU, boolean justChecking, boolean oldOpennetPeer,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "wantPeer(" + addAtLRU + "," + justChecking + "," + oldOpennetPeer + "," + connectionType
                         + ")");
        }
        ...
        if (outdated) {
            if (tooManyOutdatedPeers()) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Rejecting TOO OLD peer from " + connectionType + " (too many already): "
                                 + nodeToAddNow);
                }
        ...
        synchronized (this) {
            ...
            if ((size == maxPeers) && (nodeToAddNow == null)) {
                ...
                if (toDrop == null) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "No more peers to drop (in first bit), still " + peersLRU.size()
                    ...
                    if (toDrop.isConnected()
                            && enforcePerTypeGracePeriodLimits(maxPeers, connectionType, nodeToAddNow != null)) {
                        if (nodeToAddNow != null) {
                ...
                while (canAdd && (size = getSize()) > maxPeers - (((nodeToAddNow == null) || outdated) ? 0 : 1)) {
                    ...
                    if (toDrop == null) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "No more peers to drop, still " + peersLRU.size()
                                         + " peers, cannot accept peer"
                                         + ((nodeToAddNow == null) ? "" : nodeToAddNow.toString()));
                        }
                    ...
                    if ((nodeToAddNow != null) || (size > maxPeers)) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Drop opennet peer: " + toDrop + " (connected=" + toDrop.isConnected()
                                         + ") of " + peersLRU.size() + ":" + getSize());
                        }
    ...
    private synchronized boolean enforcePerTypeGracePeriodLimits(int maxPeers, ConnectionType type,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Per type grace period limits: total peers: " + maxPeers + " announce " + announceMax
        ...
        for (OpennetPeerNode pn : peers) {
            ...
            if (++count >= myLimit) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Per type grace period limit rejected peer of type " + type + " count is " + count
                                 + " limit is " + myLimit);
                }
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Per type grace period limit allowed connection of type " + type + " count is " + count
    ...
    void dropExcessPeers() {
        ...
        while (getSize() > maxPeers) {
            ...
            if (toDrop == null) {
                toDrop = peerToDrop(false, true, false, null, maxPeers);
            }
    ...
    private OpennetPeerNode peerToDrop(boolean noDisconnect, boolean force, boolean addingNode,
                                       ConnectionType connectionType, int maxPeers) {
        if (getSize() < maxPeers) {

            // Don't drop any peers
            if (logMINOR) {
                Logger.minor(this,
                             "peerToDrop(): Not dropping any peer (force=" + force + " addingNode=" + addingNode
        ...
        synchronized (this) {
            ...
            for (OpennetPeerNode pn : peers) {
                ...
                if (!pn.isConnected()) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Possibly dropping opennet peer " + pn + " as is disconnected (reason=" + reason
    ...
    public int getNumberOfConnectedPeersToAimIncludingDarknet() {
        ...
        if (ENABLE_PEERS_PER_KB_OUTPUT) {
            ...
            int targetPeers = (int) Math.round(Math.min(MAX_PEERS_FOR_SCALING,
    ...
    public static void waitForOpennetNoderef(final boolean isReply, final PeerNode source, final long uid,
        ...
        MessageFilter mf = MessageFilter.create().setSource(source).setField(DMT.UID,
                               uid).setTimeout(RequestSender.OPENNET_TIMEOUT).setType(isReply
                               ? DMT.FNPOpennetConnectReplyNew : DMT.FNPOpennetConnectDestinationNew);

        // Also waiting for an ack
        MessageFilter mfAck = MessageFilter.create().setSource(source).setField(DMT.UID,
        ...
        MessageFilter mfAckTimeout =
            MessageFilter.create().setSource(source).setField(DMT.UID,
                uid).setTimeout(RequestSender.OPENNET_TIMEOUT).setType(DMT.FNPOpennetCompletedTimeout);
    ...
    static byte[] innerWaitForOpennetNoderef(long xferUID, int paddedLength, int realLength, PeerNode source,
        ...
        if (logMINOR) {
            Logger.minor(OpennetManager.class,
                         "Receiving noderef (reply=" + isReply + ") as bulk transfer for request uid " + uid
                         + " with transfer " + xferUID + " from " + source);
        }
