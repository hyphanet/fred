...
public class SSKInsertSender
	...
	SSKInsertSender(
		...
		if (pubKey == null) throw new IllegalArgumentException(
			"Must have pubkey to insert data!!"
		);
	...
	protected void routeRequests() {
		...
		while (true) {
			...
			if (
				...
				Logger.normal(
					this,
					"FORKING SSK INSERT " + origUID + " to " + uid
				);
				nodesRoutedTo.clear();
	...
	private void handleNoPubkeyAccepted(PeerNode next, InsertTag thisTag) {
		...
		Logger.error(
			this,
			"Timeout waiting for FNPSSKPubKeyAccepted on " + next
		);
	...
	private MessageFilter makeSearchFilter(PeerNode next, int searchTimeout) {
		...
		MessageFilter mfSSKDataFoundHeaders = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
			.setTimeout(searchTimeout)
		...
		return mfRouteNotFound.or(
			mfInsertReply.or(
				mfRejectedOverload.or(
					mfDataInsertRejected.or(mfSSKDataFoundHeaders)
				)
			)
		);
	}
	...
	private void handleRouteNotFound(
		Message msg,
		PeerNode next,
		InsertTag thisTag
	) {
	...
	private DO handleSSKDataFoundHeaders(
		Message msg,
		PeerNode next,
		InsertTag thisTag
	) {
		...
		if (dataMessage == null) {
			Logger.error(
				this,
				"Got headers but not data for datareply for insert from " + this
			);
			next.noLongerRoutingTo(thisTag, false);
			return DO.NEXT_PEER;
		}
		...
		try {
			...
			Logger.error(
				this,
				"Invalid SSK from remote on collusion: " + this + ":" + block
			);
			finish(INTERNAL_ERROR, next);
			return DO.FINISHED;
		}
	...
	private void finish(int code, PeerNode next) {
		...
		if (next != null) {
			...
			if (forkedRequestTag != null) next.noLongerRoutingTo(
				forkedRequestTag,
				false
			);
		}
		...
		synchronized (this) {
			if (
				status != NOT_FINISHED && status != TIMED_OUT
		...
		if (logMINOR) Logger.minor(
			this,
			"Set status code: " + getStatusString()
		);
	...
	protected Message createDataRequest() {
		...
		if (preferInsert != Node.PREFER_INSERT_DEFAULT) {
			request.addSubMessage(
				DMT.createFNPSubInsertPreferInsert(preferInsert)
			);
		}
	...
	protected void onAccepted(PeerNode next) {
		...
		Message headersMsg = DMT.createFNPSSKInsertRequestHeaders(
			uid,
			headers,
			realTimeFlag
		);
		Message dataMsg = DMT.createFNPSSKInsertRequestData(
			uid,
			data,
			realTimeFlag
		);
		...
		if (needPubKey) {
			...
			try {
				...
				if (logMINOR) Logger.minor(
					this,
					"Node disconnected while sending pubkey: " + next
				);
				next.noLongerRoutingTo(thisTag, false);
				routeRequests();
				return;
				...
				Logger.warning(
					this,
					"Took too long to send pubkey to " + next + " on " + this
				);
				next.noLongerRoutingTo(thisTag, false);
				routeRequests();
				return;
		...
		while (true) {
			...
			if (msg == null) {
				...
				Logger.warning(
					this,
					"Timeout waiting for reply after Accepted in " +
					this +
					" from " +
					next
				);
				next.localRejectedOverload(
					"AfterInsertAcceptedTimeout",
					realTimeFlag
				);
				forwardRejectedOverload();
