...
public class SSKInsertSender extends BaseSender implements PrioRunnable, AnyInsertSender, ByteCounter {
    ...
    SSKInsertSender(SSKBlock block, long uid, InsertTag tag, short htl, PeerNode source, ProtectedNode node, boolean fromStore, boolean canWriteClientCache, boolean forkOnCacheable, boolean preferInsert, boolean ignoreLowBackoff, boolean realTimeFlag) {
    ...
    protected void routeRequests() {
        ...
        while(true) {
            ...
            if( node.canWriteDatastoreInsert(htl) && (!canWriteStorePrev) && forkOnCacheable && forkedRequestTag == null) {
            	...
            	nodesRoutedTo.clear();
            	node.getRequestTracker().lockUID(forkedRequestTag);
            }
            
            // Route it
            next = node.getPeerManager().closerPeer(forkedRequestTag == null ? source : null, nodesRoutedTo, target, true, node.isAdvancedModeEnabled(), -1, null,
