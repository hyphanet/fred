...
public class SSKInsertHandler implements PrioRunnable, ByteCounter {
	...
	public void run() {
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Exiting InsertHandler.run() for " + uid
			);
			tag.unlockHandler();
		}
	}
	...
	private void realRun() {
		...
		while (headers == null || data == null || pubKey == null) {
			...
			if (headers == null) {
				MessageFilter m = MessageFilter.create()
			...
			if (msg == null) {
				...
				Message failed = DMT.createFNPDataInsertRejected(
					uid,
				...
				headers = ((ShortBuffer) msg.getObject(
						DMT.BLOCK_HEADERS
					)).getData();
				...
				try {
					...
					Logger.error(
						this,
						"Invalid pubkey from " + source + " on " + uid
					);
					msg = DMT.createFNPDataInsertRejected(
						uid,
				...
				Logger.error(
					this,
					"Unexpected message? " + msg + " on " + this
				);
			}
		}
		...
		SSKBlock storedBlock = node.fetch(
			key,
			false,
			false,
			false,
		...
		if ((storedBlock != null) && !storedBlock.equals(block)) {
			try {
				...
				Logger.error(
					this,
					"Took too long to send ssk datareply to " +
					uid +
				...
				Logger.error(
					this,
					"Took too long to send ssk datareply to " + uid
				);
				return;
			}
			block = storedBlock;
		...
		while (true) {
			synchronized (sender) {
				try {
					if (
						sender.getStatus() == SSKInsertSender.NOT_FINISHED
					) sender.wait(5000);
			...
			if (
				(status == SSKInsertSender.ROUTE_NOT_FOUND) ||
	...
	private void finish(int code) {
		...
		if (
			code != SSKInsertSender.TIMED_OUT &&
			...
			node.nodeStats.remoteSskInsertBytesReceivedAverage.report(
				totalReceived
			);
			if (code == SSKInsertSender.SUCCESS) {
				// Can report both sides
				node.nodeStats.successfulSskInsertBytesSentAverage.report(
					totalSent
				);
				node.nodeStats.successfulSskInsertBytesReceivedAverage.report(
					totalReceived
				);
			}
		}
