...
public class BandwidthUsageHistory implements
	...
	public BandwidthUsageSample getSample(int idx) {
		/*
		 * It should not be necessary for clients of this class to use values of
	...
	/**
	 * Calculates a new <code>BandwidthUsageHistory</code> with a smaller amount
	 ...
	 * <code>this.getSampleCount()</code> then the oldest remaining samples from
	...
	public Iterator<BandwidthUsageSample> iterator() {
		return new Iterator<BandwidthUsageSample>() {
			...
			public BandwidthUsageSample next() {
				...
				/*
				 * We do not clone() it to prevent generation of insane amounts
				 * of objects, the client classes are trusted not to damage the
