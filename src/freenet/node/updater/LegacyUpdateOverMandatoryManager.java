...
public class LegacyUpdateOverMandatoryManager implements RequestClient {
    ...
    public void handleRequestJar(Message m, final PeerNode source, final boolean isExt) {
        ...
        try {
            ...
            msg = isExt
                  ? DMT.createUOMSendingExtra(uid, length, NodeUpdateManager.transitionExtJarURIAsUSK.toString(),
                                              version) : DMT.createUOMSendingMain(uid, length,
                                                  NodeUpdateManager.transitionMainJarURIAsUSK.toString(), version);
        ...
        try {
            source.sendAsync(msg, new AsyncMessageCallback() {
                ...
                public void disconnected() {
                    ...
                    Logger.error(this,
                                 "Peer " + source + " asked us for the blob file for the " + name
                                 + " jar, then disconnected when we tried to send the UOMSendingMain");
                    source.finishedSendingUOMJar(isExt);
                }
                @Override
                public void fatalError() {
                    ...
                    Logger.error(this,
                                 "Peer " + source + " asked us for the blob file for the " + name
                                 + " jar, then got a fatal error when we tried to send the UOMSendingMain");
                    source.finishedSendingUOMJar(isExt);
                }
                @Override
                public void sent() {
            ...
            Logger.error(this,
                         "Peer " + source + " asked us for the blob file for the " + name
                         + " jar, then disconnected when we tried to send the UOMSendingExt: " + e, e);
