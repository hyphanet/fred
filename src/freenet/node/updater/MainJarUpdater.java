...
public class MainJarUpdater extends NodeUpdater implements Deployer {
	...
	MainJarUpdater(
		...
		dependencies = new MainJarDependenciesChecker(
			this,
			manager.node.executor
		);
	}
	...
	private class DependencyJarFetcher
		implements
			JarFetcher, ClientGetCallback, RequestClient, ClientEventListener {
		...
		DependencyJarFetcher(
			...
			FetchContext myCtx = new FetchContext(
				dependencyCtx,
		...
		public synchronized void receive(
			...
			if (ce instanceof SplitfileProgressEvent) lastProgress =
		...
		public synchronized HTMLNode renderRow() {
			...
			if (uomFetcher != null) row
				.addChild("td")
				.addChild("#", l10n("fetchingFromUOM"));
			else if (lastProgress == null) row.addChild(
		...
		public void fetchFromUOM() {
			...
			UOMDependencyFetcher f = manager.uom.fetchDependency(
				expectedHash,
				expectedLength,
				filename,
				executable,
			...
			synchronized (this) {
				if (uomFetcher != null) {
					Logger.error(
						this,
						"Started UOMFetcher twice for " + filename,
						new Exception("error")
					);
					return;
				}
	...
	public JarFetcher fetch(
		...
		if (logMINOR) Logger.minor(
			this,
			"Fetching " + uri + " to " + downloadTo + " for next update"
		);
		DependencyJarFetcher fetcher = new DependencyJarFetcher(
			downloadTo,
			uri,
			expectedLength,
			expectedHash,
	...
	public void cleanupDependencies() {
		InputStream is = getClass()
			.getResourceAsStream("/" + DEPENDENCIES_FILE);
		if (is == null) {
			System.err.println(
				...
				new UserAlert() {
					private String l10n(String key) {
						return NodeL10n.getBase()
							.getString(
								"MainJarUpdater.ConfirmMultiFileUpdater." + key
							);
					}
					...
					public String anchor() {
						return (
							"multi-file-update-confirm-" + atomicDeployer.name
						);
					}
