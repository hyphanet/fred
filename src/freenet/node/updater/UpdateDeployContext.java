...
public class UpdateDeployContext {
	...
	UpdateDeployContext(MainJarDependencies deps)
		throws UpdaterParserException {
		this.deps = deps;
		...
		for (int propNo = 1; true; propNo++) {
			...
			if (mainJar == null) {
				...
				if ((name.startsWith("freenet") && (name.endsWith(".jar")))) {
					...
					(name.startsWith("freenet") && (name.endsWith(".jar.new")))
				) {
					mainJar = f;
		...
		if (mainJar == null) throw new UpdaterParserException(
			l10n("cannotUpdateNoMainJar")
		);
	...
	public static String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("UpdateDeployContext." + key, patterns, values);
	}

	public static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("UpdateDeployContext." + key, pattern, value);
	}
	...
	void rewriteWrapperConf(boolean writtenNewJar)
		...
		String newMain = mainJarAbsolute
			? newMainJar.getAbsolutePath()
			: newMainJar.getPath();
			...
			if (lowcaseLine.startsWith("wrapper.java.classpath.")) {
				...
				if (idx != -1) {
					...
					if (
						...
						Dependency dep = findDependencyByRHSFilename(
							new File(rhs)
						);
						if (dep != null) {
							if (dep.oldFilename() != null) System.out.println(
								"Found old dependency " + dep.oldFilename()
							);
							else System.out.println(
								"Found new dependency " + dep.newFilename()
							);
						} else { // dep == null
							System.out.println(
								"Found unknown jar in classpath, will keep: " +
								rhs
							);
				...
				lowcaseLine.equals("wrapper.restart.reload_configuration=true")
			) {
				writtenReload = true;
			} else if (lowcaseLine.startsWith("wrapper.anchorfile=")) {
				writtenAnchor = true;
			} else if (
				lowcaseLine.startsWith("wrapper.anchor.poll_interval=")
			) {
		...
		for (Dependency d : deps.dependencies) {
			...
			bw.write(
				"wrapper.java.classpath." + count + "=" + d.newFilename() + '\n'
			);
			count++;
		}
	...
	private Dependency findDependencyByRHSFilename(File rhs) {
		...
		for (Dependency dep : deps.dependencies) {
			...
			if (p != null) {
				if (
					p.matcher(rhs.getName().toLowerCase()).matches()
				) return dep;
			}
		}
		return null;
	...
	public static CHANGED tryIncreaseMemoryLimit(
		int extraMemoryMB,
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.startsWith("wrapper.java.maxmemory=")) {
					try {
						int memoryLimit = Integer.parseInt(
							line.substring("wrapper.java.maxmemory=".length())
						);
						...
						bw.write(
							"wrapper.java.maxmemory=" + newMemoryLimit + '\n'
						);
						success = true;
						continue;
			...
			newConfig.delete();
			System.err.println(
				"Unable to rewrite wrapper.conf with new memory limit."
			);
			return CHANGED.FAIL;
		} finally {
			Closer.close(br);
		...
		if (success) {
			if (!newConfig.renameTo(oldConfig)) {
				...
				if (!newConfig.renameTo(oldConfig)) {
					...
					System.err.println(
						"FREENET WILL NOT START UNTIL YOU RENAME " +
						newConfig +
						" to " +
						oldConfig
					);
