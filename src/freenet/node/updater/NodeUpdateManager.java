...
public class NodeUpdateManager {
    ...
    public final static String UPDATE_URI =
        "USK@sabn9HY9MKLbFPp851AO98uKtsCtYHM9rqB~A5cCGW4,3yps2z06rLnwf50QU4HvsILakRBYd4vBlPtLv0elUts,AQACAAE/jar/"
        + Version.buildNumber();

    /** Might as well be the SSK */
    public final static String LEGACY_UPDATE_URI =
        "freenet:SSK@BFa1voWr5PunINSZ5BGMqFwhkJTiDBBUrOZ0MYBXseg,BOrxeLzUMb6R9tEZzexymY0zyKAmBNvrU4A9Q0tAqu0,AQACAAE/update-"
        + TRANSITION_VERSION;
     ...
     * updater of its own.
     */
    public final static String LEGACY_EXT_URI =
        "freenet:SSK@BFa1voWr5PunINSZ5BGMqFwhkJTiDBBUrOZ0MYBXseg,BOrxeLzUMb6R9tEZzexymY0zyKAmBNvrU4A9Q0tAqu0,AQACAAE/ext-"
    ...
    public static final String transitionExtJarFilename = "legacy-freenet-ext-jar-" + TRANSITION_VERSION_EXT + ".fblob";
    ...
    public NodeUpdateManager(Node node, Config config) throws InvalidConfigValueException {
        ...
        LegacyJarFetcher.LegacyFetchCallback legacyFetcherCallback = new LegacyJarFetcher.LegacyFetchCallback() {
            ...
            public void onSuccess(LegacyJarFetcher fetcher) {
                if (transitionMainJarFetcher.fetched() && transitionExtJarFetcher.fetched()) {
        ...
        transitionMainJarFetcher = new LegacyJarFetcher(transitionMainJarURI, transitionMainJarFile, node.clientCore,
                legacyFetcherCallback);
        transitionExtJarFetcher = new LegacyJarFetcher(transitionExtJarURI, transitionExtJarFile, node.clientCore,
                legacyFetcherCallback);
        updaterConfig.register("updateSeednodes", wasEnabledOnStartup, 6, true, true,
                               "NodeUpdateManager.updateSeednodes", "NodeUpdateManager.updateSeednodesLong",
                               new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        updaterConfig.register("updateInstallers", wasEnabledOnStartup, 6, true, true,
                               "NodeUpdateManager.updateInstallers", "NodeUpdateManager.updateInstallersLong",
                               new BooleanCallback() {
            @Override
            public Boolean get() {
    ...
    public void start() throws InvalidConfigValueException {
        ...
        if (updateSeednodes) {
            SimplePuller seedrefsGetter = new SimplePuller(getSeednodesURI(), Announcer.SEEDNODES_FILENAME);

            seedrefsGetter.start(RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, 1024 * 1024);
        }

        if (updateInstallers) {
            SimplePuller installerGetter = new SimplePuller(getInstallerWindowsURI(), NON_WINDOWS_FILENAME);
            SimplePuller wininstallerGetter = new SimplePuller(getInstallerNonWindowsURI(), WINDOWS_FILENAME);
        ...
        if (updateIPToCountry) {
            SimplePuller ip4Getter = new SimplePuller(getIPv4ToCountryURI(), IPV4_TO_COUNTRY_FILENAME);
    ...
    void broadcastUOMAnnouncesOld() {
        ...
        boolean extJarAvailable = (transitionExtJarFetcher == null) ? false : transitionExtJarFetcher.fetched();
        Message msg;

        if (!(mainJarAvailable && extJarAvailable)) {
            return;
        }
    ...
    private Message getOldUOMAnnouncement() {
        ...
        return DMT.createUOMAnnounce(transitionMainJarURIAsUSK.toString(), transitionExtJarURIAsUSK.toString(),
                                     ...
                                     revocationChecker.getRevocationDNFCounter(), revocationChecker.getBlobSize(),
    ...
    private Message getNewUOMAnnouncement(long blobSize) {
        ...
        return DMT.createUOMAnnouncement(updateURI.toString(), revocationURI.toString(), revocationChecker.hasBlown(),
                                         fetchedVersion, revocationChecker.lastSucceededDelta(),
                                         revocationChecker.getRevocationDNFCounter(), revocationChecker.getBlobSize(),
                                         blobSize, (int) node.nodeStats.getNodeAveragePingTime(),
                                         (int) node.nodeStats.getBwlimitDelayTime());
    }
    ...
    void startPluginUpdater(OfficialPluginDescription plugin) {
        ...
        PluginJarUpdater updater = new PluginJarUpdater(this, uri, (int) minVer, -1, Integer.MAX_VALUE, name + "-",
                                       name, node.pluginManager, autoDeployPluginsOnRestart);
    ...
    private boolean isReadyToDeployUpdate(boolean ignoreRevocation) {
        ...
        synchronized (this) {
            ...
            if (this.fetchedMainJarVersion != this.dependenciesValidForBuild) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not deploying because dependencies are older version " + dependenciesValidForBuild
                                 + " - new version " + fetchedMainJarVersion + " may not start");
                }
    ...
    private void deployUpdate() {
        ...
        try {
            ...
            node.clientCore.alerts.register(new SimpleUserAlert(false, error, error, error, UserAlert.CRITICAL_ERROR));
        } finally {
            if (started &&!success) {
                Bucket toFree = null;
    ...
    private boolean writeJar(File mainJar, File newMainJar, File backupMainJar, NodeUpdater mainUpdater, String name,
        ...
        if (tryEasyWay) {
            ...
            if (FileUtil.copyFile(mainJar, backupMainJar)) {
                System.err.println("Written backup of current main jar to " + backupMainJar
                                   + " (if freenet fails to start up try renaming " + backupMainJar + " over "
                                   + mainJar);
            }
        ...
        System.err.println(
            "Rewriting wrapper.conf to point to " + newMainJar + " rather than " + mainJar
    ...
    private void failUpdate(String reason) {
        ...
        node.clientCore.alerts.register(new SimpleUserAlert(true, l10n("updateFailedTitle"),
                l10n("updateFailed", "reason", reason), l10n("updateFailedShort", "reason", reason),
    ...
    public void blow(String msg, boolean disabledNotBlown) {
        ...
        synchronized (this) {
            if (hasBeenBlown) {
                ...
                Logger.error(this,
                             "The key has ALREADY been marked as blown! Message was " + revocationMessage
                             + " new message " + msg);
    ...
    class SimplePuller implements ClientGetCallback {
        ...
        public void start(short priority, long maxSize) {
            ...
            ClientGetter get = new ClientGetter(this, freenetURI, context, priority, node.nonPersistentClientBulk,
                                   null, null, null);
            ...
            try {
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    if (FileUtil.renameTo(temp, node.runDir().file(filename))) {
                        System.out.println("Successfully fetched " + filename + " for version "
                                           + Version.buildNumber());

                        break;
                    } else {
                        System.out.println("Failed to rename " + temp + " to " + filename
                ...
                System.err.println("Fetched but failed to write out " + filename
                                   + " - please check that the node has permissions to write in " + node.getRunDir()
