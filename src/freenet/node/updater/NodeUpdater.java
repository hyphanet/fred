...
public abstract class NodeUpdater implements ClientGetCallback, USKCallback, RequestClient {
    ...
    @Override
    public void onFoundEdition(long l, USK key, ObjectContainer container, ClientContext context, boolean wasMetadata,
        ...
        synchronized (this) {
            ...
            Logger.minor(this,
                         "Updating availableVersion from " + availableVersion + " to " + found
    ...
    public void maybeUpdate() {
        ...
        synchronized (this) {
            if (logMINOR) {
                Logger.minor(this,
                             "maybeUpdate: isFetching=" + isFetching + ", isRunning=" + isRunning
            ...
            try {
                if ((cg == null) || cg.isCancelled()) {
                    ...
                    uri = uri.sskForUSK();
                    cg = new ClientGetter(this, uri, ctx, RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS, this,
                                          null,
            ...
            if (!tempBlobFile.renameTo(blobFile)) {
                ...
                if (!tempBlobFile.renameTo(blobFile)) {
                    if (blobFile.exists() && tempBlobFile.exists() && (blobFile.length() == tempBlobFile.length())) {
                        Logger.minor(this,
                                     "Can't rename " + tempBlobFile + " over " + blobFile + " for " + fetchedVersion
                                     + " - probably not a big deal though as the files are the same size");
                    } else {
                        Logger.error(this,
                                     "Not able to rename binary blob for node updater: " + tempBlobFile + " -> "
    ...
    public void onFailure(FetchException e, ClientGetter state, ObjectContainer container) {
        ...
        if ((errorCode == FetchException.CANCELLED) ||!e.isFatal()) {
            ...
            if (requiredExt > -1) {
                ...
                if ((realAvailableVersion != availableVersion) && (availableVersion < requiredExt)
                    ...
                    System.err.println("Need minimum edition " + requiredExt + " for new jar, found "
                                       + availableVersion + "; scheduling fetch.");
                }
            }
        }
