...
public abstract class NodeUpdater
	...
	void start() {
		try {
			...
			Logger.error(
				this,
				"The auto-update URI isn't valid and can't be used"
			);
			manager.blow(
				"The auto-update URI isn't valid and can't be used",
				true
		...
		if (oldBlob.exists()) {
			...
			if (oldBlob.renameTo(temp)) {
				...
				try {
					...
					Logger.error(
						this,
						"Unable to process old blob, caught " + t,
						t
					);
				}
				temp.delete();
				...
				Logger.error(
					this,
					"Unable to rename old blob file " +
					oldBlob +
					" to " +
					temp +
					" so can't process it."
	...
	public void onFoundEdition(
		...
		synchronized (this) {
			...
			System.err.println(
				"Found " + jarName() + " update edition " + found
			);
			Logger.minor(
				this,
	...
	private void finishOnFoundEdition(int found) {
		...
		Logger.minor(
			this,
			"Fetching " + jarName() + " update edition " + found
		);
	}
	...
	public void maybeUpdate() {
		...
		synchronized (this) {
			...
			if (
				fetchingVersion < minDeployVersion ||
			...
			if (availableVersion > currentVersion) {
				Logger.normal(
					this,
					"Starting the update process (" + availableVersion + ')'
				);
				System.err.println(
			...
			try {
				if ((cg == null) || cg.isCancelled()) {
					...
					if (availableVersion > currentVersion) System.err.println(
						"Starting " +
						jarName() +
						" fetch for " +
						availableVersion
					);
					tempBlobFile = File.createTempFile(
						blobFilenamePrefix + availableVersion + "-",
						".fblob.tmp",
						manager.node.clientCore.getPersistentTempDir()
					);
				...
				Logger.error(
					this,
					"Error while starting the fetching: " + e,
					e
				);
				isFetching = false;
			}
	...
	void onSuccess(
		...
		synchronized (this) {
			...
			if (
				result == null ||
				result.asBucket() == null ||
				result.asBucket().size() == 0
			) {
				tempBlobFile.delete();
				Logger.error(
					this,
					"Cannot update: result either null or empty for " +
					availableVersion
				);
				System.err.println(
					"Cannot update: result either null or empty for " +
			...
			if (!tempBlobFile.renameTo(blobFile)) {
				blobFile.delete();
				if (!tempBlobFile.renameTo(blobFile)) if (
					blobFile.exists() &&
					tempBlobFile.exists() &&
			...
			System.out.println(
				"Found " + jarName() + " version " + fetchedVersion
			);
			if (fetchedVersion > currentVersion) Logger.normal(
				this,
				"Found version " +
				fetchedVersion +
	...
	static Properties parseProperties(InputStream is, String filename)
		throws IOException {
	...
	public void onFailure(FetchException e, ClientGetter state) {
		...
		if (errorCode == FetchExceptionMode.CANCELLED || !e.isFatal()) {
			...
			System.err.println(
				"Unexpected error fetching update: " + e.getMessage()
			);
			if (e.isFatal()) {
	...
	public void setMinMax(int requiredExt, int recommendedExt) {
		...
		synchronized (this) {
			...
			if (requiredExt > -1) {
				...
				if (
					realAvailableVersion != availableVersion &&
