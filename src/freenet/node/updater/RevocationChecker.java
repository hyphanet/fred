...
public class RevocationChecker implements ClientGetCallback, RequestClient {
	...
	public RevocationChecker(NodeUpdateManager manager, File blobFile) {
		...
		// big enough ?
		ctxRevocation.maxOutputLength =
			NodeUpdateManager.MAX_REVOCATION_KEY_LENGTH;
		ctxRevocation.maxTempLength =
	...
	public void start(boolean aggressive) {
		...
		if (blobFile.exists()) {
			...
			try {
				BucketTools.copy(
					new FileBucket(blobFile, true, false, false, true),
					bucket
				);
				...
				Logger.error(
					this,
					"Failed to read old revocation blob: " + e,
					e
				);
				System.err.println(
	...
	public boolean start(boolean aggressive, boolean reset) {
		if (manager.isBlown()) {
			Logger.error(
				this,
				"Not starting revocation checker: key already blown!"
			);
			return false;
		}
		...
		try {
			...
			synchronized (this) {
				...
				if (
					...
					if (logMINOR) Logger.minor(
						this,
						"Not queueing another revocation fetcher yet, old one still running"
					);
					reset = false;
					wasRunning = false;
				} else {
					if (reset) {
						if (logMINOR) Logger.minor(
							this,
							"Resetting DNF count from " + revocationDNFCounter,
							new Exception("debug")
						);
					...
					if (revocationGetter != null && logMINOR) Logger.minor(
						this,
					...
					cg = revocationGetter = new ClientGetter(
						this,
					...
					if (logMINOR) Logger.minor(
						this,
						"Queued another revocation fetcher (count=" +
			...
			if (e.mode == FetchExceptionMode.RECENTLY_FAILED) {
				Logger.error(
					this,
					"Cannot start revocation fetcher because recently failed"
				);
			} else {
				Logger.error(
					this,
					"Cannot start fetch for the auto-update revocation key: " +
					e,
					e
				);
				manager.blow(
	...
	void onSuccess(FetchResult result, ClientGetter state, Bucket blob) {
		...
		try {
			...
			msg = new String(
				buf,
				MediaType.getCharsetRobustOrUTF(result.getMimeType())
			);
		} catch (Throwable t) {
			try {
	...
	private void moveBlob(Bucket tmpBlob) {
		...
		if (tmpBlob instanceof ArrayBucket) {
			...
			try {
				ArrayBucket buf = new ArrayBucket(
					BucketTools.toByteArray(tmpBlob)
				);
				synchronized (this) {
					blobBucket = buf;
				}
				...
				System.err.println(
					"Unable to copy data from revocation bucket!"
				);
				System.err.println(
			...
			if (tmpBlob instanceof FileBucket) {
				...
				synchronized (this) {
					...
					if (
						FileUtil.getCanonicalFile(f).equals(
							FileUtil.getCanonicalFile(blobFile)
						)
					) return;
				}
			}
			System.out.println(
				"Unexpected blob file in revocation checker: " + tmpBlob
			);
		}
		...
		try {
			...
			System.err.println(
				"Got revocation but cannot write it to disk: " + e
			);
			System.err.println(
	...
	void onFailure(FetchException e, ClientGetter state, Bucket blob) {
		...
		if (e.isFatal()) {
			if (!e.isDefinitelyFatal()) {
				...
				String message = l10n(
					...
					new String[] {
						e.toUserFriendlyString(),
						manager.getRevocationURI().toASCIIString(),
					}
				);
		...
		synchronized (this) {
			if (errorCode == FetchExceptionMode.DATA_NOT_FOUND) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Incremented DNF counter to " + revocationDNFCounter
				);
			}
	...
	private String l10n(String key, String[] pattern, String[] value) {
		return NodeL10n.getBase()
			.getString("RevocationChecker." + key, pattern, value);
	}

	public void kill() {
		if (revocationGetter != null) revocationGetter.cancel(
			core.clientContext
		);
	}
	...
	public RandomAccessBuffer getBlobBuffer() {
		...
		synchronized (this) {
			if (blobBucket != null) {
				try {
					ByteArrayRandomAccessBuffer t =
						new ByteArrayRandomAccessBuffer(
							blobBucket.toByteArray()
						);
					t.setReadOnly();
					return t;
				} catch (IOException e) {
		...
		try {
			...
			Logger.error(
				this,
				"Error reading downloaded revocation blob file: " + e,
				e
			);
			return null;
		}
