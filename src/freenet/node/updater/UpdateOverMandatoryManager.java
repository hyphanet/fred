...
public class UpdateOverMandatoryManager implements RequestClient {
    ...
    private void tryFetchRevocation(final PeerNode source) throws NotConnectedException {
        ...
        updateManager.node.getTicker().queueTimedJob(new Runnable() {
            ...
            public void run() {
                ...
                System.err.println(
                    "Peer " + source + " (build #" + source.getSimpleVersion()
    ...
    private void handleMainJarOffer(long now, long mainJarFileLength, long mainJarVersion, PeerNode source,
        ...
        Logger.normal(this,
                      "We received a valid UOMAnnouncement (main) : (isOutdated=" + isOutdated + " version="
                      + mainJarVersion + " whenToTakeOverTheNormalUpdater="
                      + TimeUtil.formatTime(whenToTakeOverTheNormalUpdater - now) + ") file length "
                      + mainJarFileLength + " updateManager version " + updateManager.newMainJarVersion());
        ...
        if ((mainJarVersion > Version.buildNumber()) && (mainJarFileLength > 0)
            ...
            if ((isOutdated) || (whenToTakeOverTheNormalUpdater < now)) {
                ...
                if (!isOutdated) {
                    ...
                    Logger.error(this,
                                 "The update process seems to have been stuck for " + howLong
    ...
    private void sendUOMRequest(final PeerNode source, boolean addOnFail) {
        ...
        synchronized (this) {
            ...
            if (offeredVersion < updateVersion) {
                if (offeredVersion <= 0) {
                    Logger.error(this,
                                 "Not sending UOM " + lname + " request to " + source
            ...
            if (curVersion >= offeredVersion) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not fetching from " + source + " because current " + lname + " jar version "
            ...
            if (addOnFail && (askedSendJar.size() + sendingJar.size() >= MAX_NODES_SENDING_JAR)) {
                ...
                if (offeredJar.add(source)) {
                    System.err.println("Offered " + lname + " jar by " + source.userToString()
                                       + " (already fetching from " + sendingJar.size()
                ...
                if (sendingJar.contains(source)) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Not fetching " + lname + " jar from " + source.userToString()
    ...
    public boolean handleRequestRevocation(Message m, final PeerNode source) {
        ...
        try {
            ...
            Logger.error(
                this,
                "Peer " + source
                + " asked us for the blob file for the revocation key, we have downloaded it but we can't determine the file size: "
                + e, e);
            data.close();
        ...
        try {
            ...
            Logger.error(this,
                         "Peer " + source + " asked us for the blob file for the revocation key, then disconnected: "
                         + e, e);
            data.close();
        ...
        final Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    if (!bt.send()) {
                        Logger.error(this,
                                     "Failed to send revocation key blob to " + source.userToString() + " : "
                                     + bt.getCancelReason());
                    } else {
                    ...
                    Logger.warning(this,
                                   "Failed to send revocation key blob (disconnected) to " + source.userToString()
                                   + " : " + bt.getCancelReason());
                } finally {
                    data.close();
                }
    ...
    public boolean handleSendingRevocation(Message m, final PeerNode source) {
        ...
        if (length > NodeUpdateManager.MAX_REVOCATION_KEY_BLOB_LENGTH) {
            System.err.println(
                "Node " + source.userToString() + " offered us a revocation certificate " + SizeUtil.formatSize(length)
        ...
        if (length <= 0) {
            System.err.println(
                "Revocation key is zero bytes from " + source
        ...
        try {
            ...
            e.printStackTrace();
            updateManager.blow(
                "Cannot fetch the revocation certificate from our peer because we cannot write it to disk: " + e, true);
            cancelSend(source, uid);
        ...
        updateManager.node.executor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Logger.error(this,
                                 "Caught error while transferring revocation certificate from " + source + " : " + t,
                                 t);
                    System.err.println(
                        "Peer " + source
    ...
    void processRevocationBlob(final Bucket temp, final String source, final boolean fromDisk) {
        ...
        try {
            ...
            if (!fromDisk) {
                updateManager.blow("Somebody deleted " + temp + " ? We lost the revocation certificate from " + source
                                   + "!", true);
            }
            ...
            Logger.error(this,
                         ...
                         + ")", e);
            System.err.println("Peer " + source + " sent us an invalid revocation certificate!: " + e + " (data in "
                               + temp + ")");
            ...
            Logger.error(this,
                         "Could not read revocation cert from temp file " + temp + " from node " + source + " ! : "
                         + e, e);
        ...
        ClientGetCallback myCallback = new ClientGetCallback() {
            ...
            public void onFailure(FetchException e, ClientGetter state, ObjectContainer container) {
                if (e.mode == FetchException.CANCELLED) {
                    ...
                    String message =
                        "Failed to fetch revocation certificate from blob from " + source + " : " + e
                        + (fromDisk
        ...
        ClientGetter cg = new ClientGetter(myCallback, updateManager.getRevocationURI(), tempContext, (short) 0, this,
    ...
    protected void insertBlob(final Bucket bucket, final String type) {
        ...
        InsertContext ctx = updateManager.node.clientCore.makeClient(RequestStarter.INTERACTIVE_PRIORITY_CLASS, false,
                                false).getInsertContext(true);
        ClientPutter putter = new ClientPutter(callback, bucket, FreenetURI.EMPTY_CHK_URI, null, ctx,
                                  RequestStarter.INTERACTIVE_PRIORITY_CLASS, false, false, this, null, true,
                                  updateManager.node.clientCore.clientContext, null, -1);
    ...
    public void handleRequestJar(Message m, final PeerNode source) {
        ...
        if (source.isOpennet() && updateManager.dontAllowUOM()) {
            Logger.normal(this,
                          "Peer " + source + " asked us for the blob file for " + name
        ...
        if (version != Version.buildNumber()) {
            Logger.normal(this,
                          "Peer " + source
                          + " asked us for the blob file for the main jar but we are about to update...");
        ...
        try {
            ...
            try {
                ...
                Logger.error(this,
                             "Peer " + source + " asked us for the blob file for the " + name
                             + " jar, we have downloaded it but we can't determine the file size: " + e, e);
                raf.close();
            ...
            msg = DMT.createUOMSendingMainJar(uid, length, updateManager.getURI().toString(), version);
        ...
        final Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    if (!bt.send()) {
                        Logger.error(this,
                                     "Failed to send " + name + " jar blob to " + source.userToString() + " : "
                                     + bt.getCancelReason());
                    } else {
    ...
    public boolean handleSendingMain(Message m, final PeerNode source) {
        ...
        try {
            ...
            System.err.println("Failed receiving main jar " + version + " because URI not parsable: " + e + " for "
                               + key);
            e.printStackTrace();
            cancelSend(source, uid);
        ...
        try {
            ...
            Logger.error(this,
                         "Peer " + source + " sending us a main jar binary blob, but we lost the temp file " + temp
                         + " : " + e, e);
    ...
    protected void processMainJarBlob(final File temp, final PeerNode source, final int version, FreenetURI uri) {
        ...
        try {
            ...
            Logger.error(this,
                         "Could not read main jar (" + version + ") from temp file " + temp + " from node " + toString
                         + " !");
        ...
        try {
            ...
            Logger.error(this,
                         "Cannot share main jar from " + toString
        ...
        ClientGetCallback myCallback = new ClientGetCallback() {
            ...
            public void onFailure(FetchException e, ClientGetter state, ObjectContainer container) {
                if (e.mode == FetchException.CANCELLED) {
                    ...
                    temp.delete();
                    Logger.error(this,
                                 "Failed to fetch main jar " + version + " from " + toString
        ...
        ClientGetter cg = new ClientGetter(myCallback, uri, tempContext, (short) 0, this, null,
        ...
        for (File fileToDelete : oldTempFiles) {
            ...
            if (!fileToDelete.delete()) {
                if (fileToDelete.exists()) {
                    Logger.error(this,
                                 "Cannot delete temporary persistent file " + fileToDeleteName
        ...
        try {
            if (raf != null) {
                ...
                prb = new PartiallyReceivedBulk(updateManager.node.getUSM(), 0, Node.PACKET_SIZE,
                                                new ByteArrayRandomAccessThing(new byte[0]), true);
                fail = true;
            }
        } catch (IOException e) {
            Logger.error(this,
                         "Peer " + source + " asked us for the dependency with hash "
            ...
            updateManager.node.executor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        System.out.println("IOException while downloading " + saveTo + " from " + fetchFrom + " : "
                                           + e);
                        Logger.error(this,
                                     "IOException while downloading " + saveTo + " from " + fetchFrom + " : " + e, e);
                    } catch (RuntimeException e) {
