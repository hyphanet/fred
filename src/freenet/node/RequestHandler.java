...
public class RequestHandler implements PrioRunnable, ByteCounter, RequestSender.Listener {
	...
	private void realRun() throws NotConnectedException {
		...
		if(o instanceof KeyBlock) {
			...
			node.nodeStats.remoteRequest(key instanceof NodeSSK, true, htl);
			return;
		}
		...
		if(o == null) { // ran out of htl?
			...
			sendTerminal(dnf);
			node.nodeStats.remoteRequest(key instanceof NodeSSK, false, htl);
			return;
		} else {
	...
	public void onRequestSenderFinished(int status) {
		...
		node.nodeStats.remoteRequest(key instanceof NodeSSK, status == RequestSender.SUCCESS, htl);
