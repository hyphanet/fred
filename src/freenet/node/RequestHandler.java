...
public class RequestHandler implements PrioRunnable, ByteCounter, RequestSenderListener {
	...
	private void realRun() throws NotConnectedException {
		...
		if(o == null) { // ran out of htl?
			...
			sendTerminal(dnf);
			node.nodeStats.remoteRequest(key instanceof NodeSSK, false, false, htl, ValidLocation.fromKey(key), realTimeFlag, false);
			return;
		} else {
	...
	public void onRequestSenderFinished(int status, boolean fromOfferedKey, RequestSender rs) {
		...
		node.nodeStats.remoteRequest(key instanceof NodeSSK, status == RequestSender.SUCCESS, false, htl, ValidLocation.fromKey(key), realTimeFlag, fromOfferedKey);
	...
	private void returnLocalData(KeyBlock block) throws NotConnectedException {
		if(key instanceof NodeSSK) {
			...
			node.nodeStats.remoteRequest(true, true, true, htl, ValidLocation.fromKey(key), realTimeFlag, false);
			...
			BlockTransmitter bt =
				new BlockTransmitter(node.usm, node.getTicker(), source, uid, prb, this, BlockTransmitter.NEVER_CASCADE,
					...
					public void blockTransferFinished(boolean success) {
						...
						node.nodeStats.remoteRequest(false, success, true, htl, ValidLocation.fromKey(key), realTimeFlag, false);
					}
