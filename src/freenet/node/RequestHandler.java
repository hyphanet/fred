...
public class RequestHandler
	implements PrioRunnable, ByteCounter, RequestSenderListener {
	...
	public void run() {
		...
		try {
			...
			Logger.normal(
				this,
				"requestor gone, could not start request handler wait"
			);
			tag.handlerThrew(e);
		} catch (Throwable t) {
	...
	private void applyByteCounts() {
		synchronized (this) {
			if (disconnected) {
				Logger.normal(
					this,
					"Not applying byte counts as request source disconnected during receive"
				);
				return;
			}
		...
		if (key instanceof NodeSSK) {
			...
			if (status == RequestSender.SUCCESS) {
				...
				node.nodeStats.successfulSskFetchBytesReceivedAverage.report(
					rcvd
				);
			}
		} else {
			...
			if (status == RequestSender.SUCCESS) {
				...
				node.nodeStats.successfulChkFetchBytesReceivedAverage.report(
					rcvd
				);
			}
		}
	...
	public void onReceivedRejectOverload() {
		try {
			if (!sentRejectedOverload) {
				if (logMINOR) Logger.minor(
					this,
					"Propagating RejectedOverload on " + this
				);
			...
			Logger.normal(
				this,
				"requestor is gone, can't forward reject overload"
			);
		}
	}
	...
	public void onCHKTransferBegins() {
		if (tag.hasSourceReallyRestarted()) {
			Logger.normal(
				this,
				"requestor is gone, can't send terminal message"
			);
			applyByteCounts();
					...
					public void blockTransferFinished(boolean success) {
						synchronized (RequestHandler.this) {
							if (transferCompleted) {
								Logger.error(
									this,
									"Transfer already completed on " + this,
									new Exception("debug")
								);
								return;
							}
	...
	protected void transferFinished(boolean success) {
		if (logMINOR) Logger.minor(
			this,
			"Transfer finished (success=" + success + ")"
		);
		if (success) {
			status = rs.getStatus();
	...
	private synchronized boolean readyToFinishTransfer() {
		if (waitingForTransferSuccess) {
			Logger.error(
				this,
				"waitAndFinishCHKTransferOffThread called twice on " + this
			);
			return false;
		}
		...
		if (!transferCompleted) {
			if (logMINOR) Logger.minor(
				this,
				"Waiting for transfer to finish on " + this
			);
			return false; // Wait
		}
		return true;
	...
	public void onRequestSenderFinished(
		...
		synchronized (this) {
			if (this.status == RequestSender.NOT_FINISHED) this.status = status;
				...
				if (logMINOR) Logger.minor(
					this,
					"Ignoring onRequestSenderFinished as status is already " +
					this.status
				);
				return;
			}
		...
		if (status == RequestSender.NOT_FINISHED) Logger.error(
			this,
			"onFinished() but not finished?"
		);
		...
		try {
			switch (status) {
				...
				case RequestSender.RECENTLY_FAILED:
					Message rf = DMT.createFNPRecentlyFailed(
						uid,
				...
				case RequestSender.INTERNAL_ERROR:
					...
					Message reject = DMT.createFNPRejectedOverload(
						uid,
						true,
						true,
						realTimeFlag
				...
				case RequestSender.SUCCESS:
					if (key instanceof NodeSSK) sendSSK(
						rs.getHeaders(),
						rs.getSSKData(),
						needsPubKey,
				...
				case RequestSender.GET_OFFER_VERIFY_FAILURE:
					...
					reject = DMT.createFNPRejectedOverload(
						uid,
						true,
						true,
						realTimeFlag
		...
		synchronized (this) {
			if (disconnected) disconn = true;
				...
				Logger.error(
					...
					"Status is " +
					status +
					" but we never started a transfer on " +
					uid
				);
	...
	private void sendSSK(
		byte[] headers,
		final byte[] data,
		boolean needsPubKey2,
		DSAPublicKey pubKey
		...
		Message headersMsg = DMT.createFNPSSKDataFoundHeaders(
			uid,
			headers,
			realTimeFlag
		);
		source.sendAsync(headersMsg, mcb.make(), this);
		final Message dataMsg = DMT.createFNPSSKDataFoundData(
			uid,
			data,
			realTimeFlag
		);
	...
	static void sendSSK(
		...
		throws NotConnectedException, WaitedTooLongException, PeerRestartedException, SyncSendWaitedTooLongException {
		// SUCCESS requires that BOTH the pubkey AND the data/headers have been received.
		// The pubKey will have been set on the SSK key, and the SSKBlock will have been constructed.
	...
	private void sendTerminal(Message msg) {
		...
		if (sendTerminalCalled) throw new IllegalStateException(
			"sendTerminal should only be called once"
		);
		...
		try {
			source.sendAsync(
				msg,
				new TerminalMessageByteCountCollector(),
				this
			);
		} catch (NotConnectedException e) {
	...
	private class TerminalMessageByteCountCollector
		...
		public void acknowledged() {
			if (logMINOR) Logger.minor(
				this,
				"Acknowledged terminal message: " + RequestHandler.this
			);
		...
		public void disconnected() {
			if (logMINOR) Logger.minor(
				this,
				"Peer disconnected before terminal message sent for " +
				RequestHandler.this
			);
			complete();
		}
		...
		public void fatalError() {
			Logger.error(
				this,
				"Error sending terminal message?! for " + RequestHandler.this
			);
			complete();
		}
		...
		public void sent() {
			if (logMINOR) Logger.minor(
				this,
				"Sent terminal message: " + RequestHandler.this
			);
			complete();
		}
		...
		private void complete() {
			...
			if (logMINOR) Logger.minor(
				this,
				"Completing: " + RequestHandler.this
			);
	...
	private void finishOpennetInner(OpennetManager om) {
		...
		if (noderef == null) {
			if (logMINOR) Logger.minor(
				this,
				"Not relaying as no noderef on " + this
			);
			finishOpennetNoRelayInner(om);
			return;
		}
		if (
			...
			if (ref == null || om.alreadyHaveOpennetNode(ref)) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Resetting path folding on " + this
				);
				// Reset path folding.
	...
	private void finishOpennetNoRelayInner(final OpennetManager om) {
		if (logMINOR) Logger.minor(
			this,
			"Finishing opennet: sending own reference on " + this,
			new Exception("debug")
		);
		if (
			!om.wantPeer(null, false, false, false, ConnectionType.PATH_FOLDING)
		) {
			ackOpennet();
		...
		try {
			...
			Logger.normal(
				this,
				"Can't send opennet ref because node disconnected on " + this
			);
			// Oh well...
			applyByteCounts();
			unregisterRequestHandlerWithNode();
		...
		OpennetManager.waitForOpennetNoderef(
			...
			new NoderefCallback() {
				...
				public void gotNoderef(byte[] noderef) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Got noderef on " + RequestHandler.this
					);
				...
				public void timedOut() {
					if (logMINOR) Logger.minor(
						this,
						"Timed out waiting for noderef from " +
						source +
						" on " +
						RequestHandler.this
					);
	...
	private void finishOpennetNoRelayInner(OpennetManager om, byte[] noderef) {
		...
		try {
			if (
				...
				this,
				"Asked for opennet ref but didn't want it for " +
				this +
				" :\n" +
				ref
			);
			...
			Logger.error(
				this,
				"Could not parse opennet noderef for " +
				this +
				" from " +
				source,
				e
	...
	private void finishOpennetRelay(byte[] noderef, final OpennetManager om) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Finishing opennet: relaying reference from " +
			dataSource +
			" on " +
			this
		);
			...
			new NoderefCallback() {
				...
				public void gotNoderef(byte[] newNoderef) {
					if (newNoderef == null) {
						...
						if (
							OpennetManager.validateNoderef(
								newNoderef,
								0,
								newNoderef.length,
								source,
							...
							try {
								if (logMINOR) Logger.minor(
									this,
									"Relaying noderef from source to data source for " +
									RequestHandler.this
								);
								om.sendOpennetRef(
									true,
