...
public class RequestStarter
	implements Runnable, RandomGrabArrayItemExclusionList {
	...
	public static boolean isValidPriorityClass(int prio) {
		return !(
			(prio < MAXIMUM_PRIORITY_CLASS) || (prio > PAUSED_PRIORITY_CLASS)
		);
	}
	...
	void realRun() {
		...
		while (true) {
			...
			OpennetManager om;
			if (
				core.node.peers.countConnectedPeers() < 3 &&
				(om = core.node.getOpennet()) != null &&
				System.currentTimeMillis() - om.getCreationTime() <
					MINUTES.toMillis(5)
			) {
				try {
					synchronized (this) {
			...
			if (req != null) {
				if (logMINOR) Logger.minor(
					this,
					"Running " + req + " priority " + req.getPriority()
				);
				...
				if (!req.localRequestOnly) {
					reason = stats.shouldRejectRequest(
						true,
						isInsert,
						isSSK,
						true,
					...
					if (reason != null) {
						if (logMINOR) Logger.minor(
							this,
							"Not sending local request: " + reason
						);
			...
			if (!startRequest(req, logMINOR)) {
				...
				if (
					!((!req.isPersistent()) && req.isCancelled())
	...
	private boolean startRequest(ChosenBlock req, boolean logMINOR) {
		...
		if (req.key != null) {
			...
			if (
				!sched.addRunningInsert(
					(SendableInsert) (((ChosenBlockImpl) req).request),
					req.token.getKey()
				)
			) {
				req.onDumped();
		...
		if (logMINOR) Logger.minor(
			this,
			"Running request " + req + " priority " + req.getPriority()
		);
		core
			.getExecutor()
			.execute(
				new SenderThread(req, req.key),
	...
	private class SenderThread implements Runnable {
		...
		public void run() {
			...
			if (key != null) stats.reportOutgoingLocalRequestLocation(
				key.toNormalizedDouble()
			);
			if (!req.send(core, sched)) {
				if (!((!req.isPersistent()) && req.isCancelled())) Logger.error(
					this,
					"run() not able to send a request on " + req
				);
				else Logger.normal(
					this,
					"run() not able to send a request on " +
					req +
	...
	public long exclude(
		...
		if (sched.isRunningOrQueuedPersistentRequest((SendableRequest) item)) {
			Logger.normal(
				this,
				"Excluding already-running request: " + item,
				new Exception("debug")
			);
		...
		if (!(item instanceof BaseSendableGet)) {
			Logger.error(
				this,
				"On a request scheduler, exclude() called with " + item,
				new Exception("error")
			);
			return -1;
		}
