        ...
        private void checkOrder() {
            if (nonEmptyItemsWithID != null) {
                ...
                for (Items items : nonEmptyItemsWithID) {
                    ...
                    if (thisTime < prev) {
                        Logger.error(this, "Inconsistent order in non empty items with ID: prev timeout was " + prev
                                     + " for " + prevItems + " but this timeout is " + thisTime + " for "
                                     + items, new Exception("error"));
                    }
            ...
            if (itemsNonUrgent != null) {
                ...
                for (MessageItem item : itemsNonUrgent) {
                    if (item.submitted < prev) {
                        Logger.error(this,
                                     "Inconsistent order in itemsNonUrgent: prev submitted at " + prev
        ...
        MessageItem addPriorityMessages(long now, MutableBoolean addPeerLoadStatsRT,
            ...
            if (logMINOR) {
                ...
                if (nonEmpty + empty < byID) {
                    Logger.error(this,
                                 "Leaking itemsByID? non empty = " + nonEmpty + " empty = " + empty + " by ID = "
                                 + byID + " on " + this);
                } else if (logDEBUG) {
                    Logger.debug(this,
            ...
            while (true) {
                ...
                if ((list.timeLastSent == -1) || (now - list.timeLastSent > FORGET_AFTER)) {
                    ...
                    if (old == null) {
                        ...
                        Logger.error(this,
                                     "Different list in the items by ID tracker: " + old + " not " + list + " for "
                                     + list.id);
                    }
