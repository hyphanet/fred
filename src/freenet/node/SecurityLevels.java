...
public class SecurityLevels {
	...
	public SecurityLevels(Node node, PersistentConfig config) {
		...
		networkThreatLevelCallback = new MyCallback<NETWORK_THREAT_LEVEL>() {
			...
			protected void setValue(String val)
				...
				if (newValue == null) throw new InvalidConfigValueException(
		...
		physicalThreatLevelCallback = new MyCallback<PHYSICAL_THREAT_LEVEL>() {
			...
			protected void setValue(String val)
				throws InvalidConfigValueException {
				PHYSICAL_THREAT_LEVEL newValue = PHYSICAL_THREAT_LEVEL.valueOf(
					val
				);
				if (newValue != null) throw new InvalidConfigValueException(
	...
	private abstract class MyCallback<T>
		...
		public void addListener(SecurityLevelListener<T> listener) {
			if (listeners.contains(listener)) {
				Logger.error(
					this,
					"Already have listener " + listener + " in " + this
				);
				return;
			}
	 ...
	 * @return
	 */
	public HTMLNode getConfirmWarning(
	...
	private String l10n(String string, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("SecurityLevels." + string, pattern, value);
	}

	private String l10n(String string, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("SecurityLevels." + string, patterns, values);
	}
	...
	public static String localisedName(NETWORK_THREAT_LEVEL newThreatLevel) {
		return NodeL10n.getBase()
			.getString(
				"SecurityLevels.networkThreatLevel.name." +
				newThreatLevel.name()
			);
	}
	...
	public static String localisedName(PHYSICAL_THREAT_LEVEL newPhysicalLevel) {
		return NodeL10n.getBase()
			.getString(
				"SecurityLevels.physicalThreatLevel.name." +
	...
	public FRIEND_TRUST getDefaultFriendTrust() {
		synchronized (this) {
			if (friendsThreatLevel == null) {
				Logger.error(
					this,
					"Asking for default friend trust yet we have no friend security level!"
				);
				return FRIEND_TRUST.NORMAL;
			}
			if (
				friendsThreatLevel == FRIENDS_THREAT_LEVEL.HIGH
			) return FRIEND_TRUST.LOW;
			if (
				friendsThreatLevel == FRIENDS_THREAT_LEVEL.NORMAL
			) return FRIEND_TRUST.NORMAL;
			else return FRIEND_TRUST.HIGH; // friendsThreatLevel == FRIENDS_THREAT_LEVEL.LOW
		}
	}
}
