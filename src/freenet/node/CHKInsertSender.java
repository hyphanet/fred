...
public final class CHKInsertSender extends BaseSender implements PrioRunnable, AnyInsertSender, ByteCounter {
    ...
    CHKInsertSender(NodeCHK myKey, long uid, InsertTag tag, byte[] headers, short htl, PeerNode source, Node node,
                    PartiallyReceivedBlock prb, boolean fromStore, boolean canWriteClientCache,
    ...
    void start() {
        node.executor.execute(this,
                              "CHKInsertSender for UID " + uid + " on " + node.getDarknetPortNumber() + " at "
    ...
    protected void routeRequests() {
        ...
        while (true) {
            ...
            next = node.peers.closerPeer((forkedRequestTag == null) ? source : null, nodesRoutedTo, target, true,
                                         node.isAdvancedModeEnabled(), -1, null, null, htl,
                                         ignoreLowBackoff ? Node.LOW_BACKOFF : 0, source == null, realTimeFlag,
                                         newLoadManagement);
    ...
    private void handleRejectedTimeout(Message msg, PeerNode next) {
        ...
        Logger.warning(this,
                       "Node timed out waiting for our DataInsert (" + msg + " from " + next
    ...
    private void handleDataInsertRejected(Message msg, PeerNode next, InsertTag thisTag) {
        ...
        if (reason == DMT.DATA_INSERT_REJECTED_VERIFY_FAILED) {
            if (fromStore) {
                ...
                // That's odd...
                Logger.error(this,
                             "Verify failed on next node " + next
            ...
            if (recvFailed) {
                ...
                try {
                    if (prb.allReceived()) {
                        ...
                        if (prb.isAborted()) {
                            Logger.normal(this,
                                          "Send failed: aborted: " + prb.getAbortReason() + ": "
            ...
            node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
                ...
                public void onMatched(Message m) {
                    if ((m.getSpec() == DMT.FNPRejectedLoop) || (m.getSpec() == DMT.FNPRejectedOverload)) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Accepted after timeout on " + CHKInsertSender.this
    ...
    private boolean waitForBackgroundTransfers(BackgroundTransfer[] transfers) {
        ...
        while (true) {
            ...
            synchronized (backgroundTransfers) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Waiting: transfer completion=" + completedTransfers + " notification="
                ...
                Runnable r = new Runnable() {
                    ...
                    public void run() {
                        ...
                        MessageFilter mfRejectedOverload =
                            MessageFilter.create().setSource(waitingFor).setField(DMT.UID,
                                uid).setTimeout(searchTimeout).setType(DMT.FNPRejectedOverload);
                        ...
                        while (true) {
                            ...
                            try {
                                ...
                                Logger.normal(this,
                                              "Disconnected from " + waitingFor + " while waiting for InsertReply on "
                                              + CHKInsertSender.this);
                            ...
                            if (msg == null) {
                                ...
                                Logger.error(this,
                                             "Got second (local) timeout on " + CHKInsertSender.this + " from "
                                             + waitingFor);
                                transfer.onCompleted();
    ...
    private class BackgroundTransfer implements PrioRunnable, SlowAsyncMessageFilterCallback {
        ...
        private boolean receivedNotice(boolean success, boolean timeout, boolean kill) {
            ...
            synchronized (backgroundTransfers) {
                if (finishedWaiting) {
                    if (!(killed || kill)) {
                        Logger.error(this,
                                     "Finished waiting already yet receivedNotice(" + success + "," + timeout + ","
                ...
                if (killed) {
                    ...
                    if (receivedCompletionNotice) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "receivedNotice(" + success + "), already had receivedNotice("
                                         + completionSucceeded + ")");
                        }
        ...
        private MessageFilter getNotificationMessageFilter(boolean longTimeoutAnyway) {
            return MessageFilter.create().setField(DMT.UID,
                    uid).setType(DMT.FNPInsertTransfersCompleted).setSource(pn).setTimeout(longTimeoutAnyway
                    ? TRANSFER_COMPLETION_ACK_TIMEOUT_BULK : transferCompletionTimeout);
        }
