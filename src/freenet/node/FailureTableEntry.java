...
class FailureTableEntry implements TimedOutNodesList {
    ...
    public synchronized void failedTo(PeerNodeUnlocked routedTo, long rfTimeout, long ftTimeout, long now, short htl) {
        if (logMINOR) {
            Logger.minor(this,
                         "Failed sending request to " + routedTo.shortToString() + " : timeout " + rfTimeout + " / "
                         + ftTimeout);
        }
    ...
    synchronized int addRequestor(PeerNodeUnlocked requestor, long now, short origHTL) {
        ...
        @SuppressWarnings("unchecked") WeakReference<? extends PeerNodeUnlocked>[] newRequestorNodes =
    ...
    private synchronized int addRequestedFrom(PeerNodeUnlocked requestedFrom, short htl, long now) {
        ...
        for (int i = 0; i < requestedNodes.length; i++) {
            ...
            if ((got == requestedFrom)
                    && ((requestedTimeoutsRF[i] == -1) || (requestedTimeoutsFT[i] == -1)
        ...
        @SuppressWarnings("unchecked") WeakReference<? extends PeerNodeUnlocked>[] newRequestedNodes =
    ...
    public void offer() {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Sending offers to nodes which requested the key from us: (" + requestorNodes.length
                         + ") for " + key);
        }
        ...
        synchronized (this) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Sending offers to nodes which we sent the key to: (" + requestedNodes.length + ") for "
                             + key);
            }
