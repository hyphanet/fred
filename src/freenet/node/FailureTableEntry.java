...
class FailureTableEntry implements TimedOutNodesList {
	...
	public static final WeakReference<
		? extends PeerNodeUnlocked
	>[] EMPTY_WEAK_REFERENCE = (WeakReference<
			? extends PeerNodeUnlocked
		>[]) new WeakReference<?>[0];
	...
	synchronized int addRequestor(
		...
		if (logMINOR) Logger.minor(
			this,
			"Adding requestors: " + requestor + " at " + now
		);
		receivedTime = now;
		...
		for (int i = 0; i < requestorNodes.length; i++) {
			PeerNodeUnlocked got = requestorNodes[i] == null
				? null
		...
		if (nulls == 1 && !includedAlready) {
			...
			for (int i = 0; i < requestorNodes.length; i++) {
				if (
					requestorNodes[i] == null || requestorNodes[i].get() == null
				) {
		...
		WeakReference<? extends PeerNodeUnlocked>[] newRequestorNodes =
			(WeakReference<? extends PeerNodeUnlocked>[]) new WeakReference<
				?
			>[requestorNodes.length + notIncluded - nulls];
		long[] newRequestorTimes = new long[requestorNodes.length +
		notIncluded -
		nulls];
		long[] newRequestorBootIDs = new long[requestorNodes.length +
		notIncluded -
		nulls];
		short[] newRequestorHTLs = new short[requestorNodes.length +
		notIncluded -
		nulls];
		int toIndex = 0;
	 ...
	 */
	private synchronized int addRequestedFrom(
		...
		if (logMINOR) Logger.minor(
			this,
			"Adding requested from: " + requestedFrom + " at " + now
		);
		sentTime = now;
		boolean includedAlready = false;
		int nulls = 0;
		...
		for (int i = 0; i < requestedNodes.length; i++) {
			PeerNodeUnlocked got = requestedNodes[i] == null
				? null
		...
		if (nulls == 1 && !includedAlready) {
			...
			for (int i = 0; i < requestedNodes.length; i++) {
				if (
					requestedNodes[i] == null || requestedNodes[i].get() == null
				) {
		...
		WeakReference<? extends PeerNodeUnlocked>[] newRequestedNodes =
			(WeakReference<? extends PeerNodeUnlocked>[]) new WeakReference<
				?
			>[requestedNodes.length + notIncluded - nulls];
		double[] newRequestedLocs = new double[requestedNodes.length +
		notIncluded -
		nulls];
		long[] newRequestedBootIDs = new long[requestedNodes.length +
		notIncluded -
		nulls];
		long[] newRequestedTimes = new long[requestedNodes.length +
		notIncluded -
		nulls];
		long[] newRequestedTimeoutsFT = new long[requestedNodes.length +
		notIncluded -
		nulls];
		long[] newRequestedTimeoutsRF = new long[requestedNodes.length +
		notIncluded -
		nulls];
		short[] newRequestedTimeoutHTLs = new short[requestedNodes.length +
		notIncluded -
		nulls];
		...
		if (toIndex > newRequestedNodes.length + 2) {
			...
			newRequestedTimeoutsRF = Arrays.copyOf(
				newRequestedTimeoutsRF,
				toIndex
			);
			newRequestedTimeoutsFT = Arrays.copyOf(
				newRequestedTimeoutsFT,
				toIndex
			);
			newRequestedTimeoutHTLs = Arrays.copyOf(
				newRequestedTimeoutHTLs,
				toIndex
			);
		}
		...
		synchronized (this) {
			for (int i = 0; i < requestorNodes.length; i++) {
				...
				if (!set.add(pn)) {
					Logger.error(
						this,
						"Node is in requestorNodes twice: " + pn
					);
				}
			}
			if (logMINOR) Logger.minor(
	...
	public synchronized boolean askedFromPeer(PeerNodeUnlocked peer, long now) {
		...
		if (!anyValid) {
			requestedNodes = EMPTY_WEAK_REFERENCE;
			requestedTimes = requestedBootIDs = requestedTimeoutsRF =
				requestedTimeoutsFT = EMPTY_LONG_ARRAY;
			requestedTimeoutHTLs = EMPTY_SHORT_ARRAY;
		}
		return ret;
		...
		for (int i = 0; i < requestedNodes.length; i++) {
			...
			if (ref != null && ref.get() == peer) {
				if (requestedTimeoutHTLs[i] >= htl) {
					long thisTimeout = forPerNodeFailureTables
						? requestedTimeoutsFT[i]
						: requestedTimeoutsRF[i];
					if (thisTimeout > timeout && thisTimeout > now) timeout =
						thisTimeout;
				}
			}
		}
