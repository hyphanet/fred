...
public class RequestStarterGroup {
	...
	RequestStarterGroup(
		...
		throttleWindowBulk = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindow"),
			node
		);
		throttleWindowRT = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindowRT"),
			node
		);

		throttleWindowCHK = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindowCHK"),
			node
		);
		throttleWindowSSK = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindowSSK"),
			node
		);
		throttleWindowInsert = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindowInsert"),
			node
		);
		throttleWindowRequest = new ThrottleWindowManager(
			2.0,
			fs == null ? null : fs.subset("ThrottleWindowRequest"),
			node
		);
		chkRequestThrottleBulk = new MyRequestThrottle(
			5000,
			"CHK Request",
			fs == null ? null : fs.subset("CHKRequestThrottle"),
			32768,
			false
		);
		...
		chkRequestStarterBulk = new RequestStarter(
			core,
		...
		chkRequestStarterRT = new RequestStarter(
			core,
		...
		chkFetchSchedulerBulk = new ClientRequestScheduler(
			false,
			false,
			false,
			random,
		...
		chkFetchSchedulerRT = new ClientRequestScheduler(
			false,
			false,
			true,
			random,
		...
		chkInsertThrottleBulk = new MyRequestThrottle(
			20000,
			"CHK Insert",
			fs == null ? null : fs.subset("CHKInsertThrottle"),
			32768,
			false
		);
		...
		chkInsertStarterBulk = new RequestStarter(
			core,
		...
		chkInsertStarterRT = new RequestStarter(
			core,
			chkInsertThrottleRT,
			"CHK Insert starter (" + portNumber + ')',
			stats.localChkInsertBytesSentAverage,
		...
		chkPutSchedulerBulk = new ClientRequestScheduler(
			true,
			false,
			false,
			random,
		...
		chkPutSchedulerRT = new ClientRequestScheduler(
			true,
			false,
			true,
			random,
		...
		sskRequestThrottleBulk = new MyRequestThrottle(
			5000,
			"SSK Request",
			fs == null ? null : fs.subset("SSKRequestThrottle"),
			1024,
			false
		);
		sskRequestThrottleRT = new MyRequestThrottle(
			5000,
			"SSK Request (RT)",
		...
		sskRequestStarterBulk = new RequestStarter(
			core,
		...
		sskRequestStarterRT = new RequestStarter(
			core,
		...
		sskFetchSchedulerBulk = new ClientRequestScheduler(
			false,
			true,
			false,
			random,
		...
		sskFetchSchedulerRT = new ClientRequestScheduler(
			false,
			true,
			true,
			random,
		...
		//insertThrottle = new ChainedRequestThrottle(10000, 2.0F, requestThrottle);
		// FIXME reenable the above
		sskInsertThrottleBulk = new MyRequestThrottle(
			20000,
			"SSK Insert",
			fs == null ? null : fs.subset("SSKInsertThrottle"),
			1024,
			false
		);
		sskInsertThrottleRT = new MyRequestThrottle(
			20000,
			"SSK Insert",
			fs == null ? null : fs.subset("SSKInsertThrottleRT"),
			1024,
			true
		);
		sskInsertStarterBulk = new RequestStarter(
			core,
		...
		sskInsertStarterRT = new RequestStarter(
			core,
			sskInsertThrottleRT,
			"SSK Insert starter (" + portNumber + ')',
			stats.localSskInsertBytesSentAverage,
		...
		sskPutSchedulerBulk = new ClientRequestScheduler(
			true,
			true,
			false,
			random,
		...
		sskPutSchedulerRT = new ClientRequestScheduler(
			true,
			true,
			true,
			random,
	...
	private void registerSchedulerConfig(
		...
		callback.init(
			csRT,
			csBulk,
			schedulerConfig.getString(name + "_priority_policy")
		);
	}
	...
	public class MyRequestThrottle implements BaseRequestThrottle {
		...
		public MyRequestThrottle(
			...
			roundTripTime = new BootstrappingDecayingRunningAverage(
				rtt,
				10,
				MINUTES.toMillis(5),
				10,
		...
		public synchronized long getDelay() {
			...
			double _simulatedWindowSize = getThrottleWindow()
			...
			return Math.max(
				MIN_DELAY,
				Math.min((long) (rtt / _simulatedWindowSize), MAX_DELAY)
			);
		}
	...
	public static class PrioritySchedulerCallback
		...
		private final String[] possibleValues = new String[] {
			ClientRequestScheduler.PRIORITY_HARD,
			ClientRequestScheduler.PRIORITY_SOFT,
		};
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			if (val.equalsIgnoreCase(ClientRequestScheduler.PRIORITY_HARD)) {
				...
				val.equalsIgnoreCase(ClientRequestScheduler.PRIORITY_SOFT)
			) {
				value = ClientRequestScheduler.PRIORITY_SOFT;
			} else {
				throw new InvalidConfigValueException(
					"Invalid priority scheme"
				);
			}
	...
	public long countQueuedRequests() {
		return (
			chkFetchSchedulerBulk.countQueuedRequests() +
