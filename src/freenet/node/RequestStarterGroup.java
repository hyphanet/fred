...
public class RequestStarterGroup {
    ...
    RequestStarterGroup(Node node, NodeClientCore core, int portNumber, RandomSource random, Config config,
                        SimpleFieldSet fs, ClientContext ctx, long dbHandle, ObjectContainer container)
        ...
        chkRequestStarterBulk = new RequestStarter(core, chkRequestThrottleBulk,
                ...
                stats.localChkFetchBytesSentAverage, stats.localChkFetchBytesReceivedAverage, false, false, false);
        chkRequestStarterRT = new RequestStarter(core, chkRequestThrottleRT,
                "CHK Request starter (" + portNumber + ')', stats.requestOutputThrottle, stats.requestInputThrottle,
                stats.localChkFetchBytesSentAverage, stats.localChkFetchBytesReceivedAverage, false, false, true);
        chkFetchSchedulerBulk = new ClientRequestScheduler(false, false, false, random, chkRequestStarterBulk, node,
                core, "CHKrequester", ctx);

        if (container != null) {
            chkFetchSchedulerBulk.startCore(core, dbHandle, container);
        }

        chkFetchSchedulerRT = new ClientRequestScheduler(false, false, true, random, chkRequestStarterRT, node, core,
                "CHKrequester", ctx);
        ...
        chkInsertStarterBulk = new RequestStarter(core, chkInsertThrottleBulk,
                "CHK Insert starter (" + portNumber + ')', stats.requestOutputThrottle, stats.requestInputThrottle,
                stats.localChkInsertBytesSentAverage, stats.localChkInsertBytesReceivedAverage, true, false, false);
        chkInsertStarterRT = new RequestStarter(core, chkInsertThrottleRT, "CHK Insert starter (" + portNumber + ')',
                stats.requestOutputThrottle, stats.requestInputThrottle, stats.localChkInsertBytesSentAverage,
                stats.localChkInsertBytesReceivedAverage, true, false, true);
        chkPutSchedulerBulk = new ClientRequestScheduler(true, false, false, random, chkInsertStarterBulk, node, core,
                "CHKinserter", ctx);
        ...
        chkPutSchedulerRT = new ClientRequestScheduler(true, false, true, random, chkInsertStarterRT, node, core,
                "CHKinserter", ctx);
        ...
        sskRequestStarterBulk = new RequestStarter(core, sskRequestThrottleBulk,
                ...
                stats.localSskFetchBytesSentAverage, stats.localSskFetchBytesReceivedAverage, false, true, false);
        sskRequestStarterRT = new RequestStarter(core, sskRequestThrottleRT,
                "SSK Request starter (" + portNumber + ')', stats.requestOutputThrottle, stats.requestInputThrottle,
                stats.localSskFetchBytesSentAverage, stats.localSskFetchBytesReceivedAverage, false, true, true);
        sskFetchSchedulerBulk = new ClientRequestScheduler(false, true, false, random, sskRequestStarterBulk, node,
                core, "SSKrequester", ctx);

        if (container != null) {
            sskFetchSchedulerBulk.startCore(core, dbHandle, container);
        }

        sskFetchSchedulerRT = new ClientRequestScheduler(false, true, true, random, sskRequestStarterRT, node, core,
                "SSKrequester", ctx);
        ...
        sskInsertStarterBulk = new RequestStarter(core, sskInsertThrottleBulk,
                ...
                stats.localSskInsertBytesSentAverage, stats.localSskFetchBytesReceivedAverage, true, true, false);
        sskInsertStarterRT = new RequestStarter(core, sskInsertThrottleRT, "SSK Insert starter (" + portNumber + ')',
                stats.requestOutputThrottle, stats.requestInputThrottle, stats.localSskInsertBytesSentAverage,
                stats.localSskFetchBytesReceivedAverage, true, true, true);
        sskPutSchedulerBulk = new ClientRequestScheduler(true, true, false, random, sskInsertStarterBulk, node, core,
                "SSKinserter", ctx);
        ...
        sskPutSchedulerRT = new ClientRequestScheduler(true, true, true, random, sskInsertStarterRT, node, core,
                "SSKinserter", ctx);
    ...
    private void registerSchedulerConfig(SubConfig schedulerConfig, String name, ClientRequestScheduler csBulk,
            ClientRequestScheduler csRT, boolean forSSKs, boolean forInserts)
            throws InvalidConfigValueException {
        PrioritySchedulerCallback callback = new PrioritySchedulerCallback();

        schedulerConfig.register(name + "_priority_policy", ClientRequestScheduler.PRIORITY_SOFT, name.hashCode(),
                                 true, false,
                                 "RequestStarterGroup.scheduler" + (forSSKs ? "SSK" : "CHK")
                                 + (forInserts ? "Inserts" : "Requests"), "RequestStarterGroup.schedulerLong",
                                     callback);
    ...
    public long countTransientQueuedRequests() {
        return chkFetchSchedulerBulk.countTransientQueuedRequests()
               ...
               + sskFetchSchedulerRT.countTransientQueuedRequests() + chkPutSchedulerRT.countTransientQueuedRequests()
    ...
    public class MyRequestThrottle implements BaseRequestThrottle {
        ...
        public synchronized void successfulCompletion(long rtt) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Reported successful completion: " + rtt + " on " + this + " avg "
        ...
        public String toString() {
            return "rtt: " + roundTripTime.currentValue() + " _s=" + getThrottleWindow().currentValue(realTime)
                   + " RT=" + realTime;
        }
