...
public class FailureTable {
	...
	public void start() {
		offerExecutor.start(
			node.executor,
			"FailureTable offers executor for " + node.getDarknetPortNumber()
		);
	}
	...
	public void onFinalFailure(
		...
		synchronized (this) {
			...
			if (routedTo != null) entry.failedTo(
				routedTo,
				rfTimeout,
				ftTimeout,
				now,
	...
	private final class BlockOfferList {
		...
		public void deleteOffer(BlockOffer offer) {
			...
			synchronized (blockOfferListByKey) {
				...
				if (idx < newOffers.length) System.arraycopy(
					offers,
					idx + 1,
					newOffers,
					idx,
	...
	static final class BlockOffer {
		...
		public boolean isExpired(long now) {
			return (
				nodeRef.get() == null || now > (offeredTime + OFFER_EXPIRY_TIME)
			);
		}
	...
	protected void innerOnOffer(Key key, PeerNode peer, byte[] authenticator) {
		...
		synchronized (blockOfferListByKey) {
			...
			BlockOffer offer = new BlockOffer(
				peer,
				now,
				authenticator,
				peer.getBootID()
		...
		synchronized (blockOfferListByKey) {
			while (true) {
				...
				if (
					...
					if (logMINOR) Logger.minor(
						this,
						"Removing block offer list " +
						bl +
						" list size now " +
						blockOfferListByKey.size()
					);
	...
	protected void innerSendOfferedKey(
		...
		if (isSSK) {
			...
			if (block == null) {
				...
				source.sendAsync(
					DMT.createFNPGetOfferedKeyInvalid(
						uid,
						DMT.GET_OFFERED_KEY_REJECTED_NO_KEY
					),
					null,
					senderCounter
				);
			...
			final Message data = DMT.createFNPSSKDataFoundData(
				uid,
				block.getRawData(),
				realTimeFlag
			);
			Message headers = DMT.createFNPSSKDataFoundHeaders(
				uid,
				block.getRawHeaders(),
				realTimeFlag
			);
			...
			if (needPubKey) {
				Message pk = DMT.createFNPSSKPubKey(
					uid,
					block.getPubKey(),
					realTimeFlag
				);
			...
			final BlockTransmitter bt = new BlockTransmitter(
				node.usm,
				node.getTicker(),
				source,
				uid,
	...
	public final OfferedKeysByteCounter senderCounter =
		new OfferedKeysByteCounter();
	...
	class OfferList {
		...
		public BlockOffer getFirstOffer() {
			...
			if (!recentOffers.isEmpty()) {
				return (
					lastOffer = ListUtils.removeRandomBySwapLastSimple(
						node.random,
						recentOffers
					)
				);
	...
	public class FailureTableCleaner implements Runnable {
		...
		private void realRun() {
			...
			if (logMINOR) Logger.minor(
				this,
				"Finished FailureTable cleanup took " +
				(endTime - startTime) +
				"ms"
			);
		}
