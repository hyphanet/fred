...
public class FailureTable implements OOMHook {
    ...
    public void onFound(KeyBlock block) {
        ...
        if (!(node.enableULPRDataPropagation || node.enablePerNodeFailureTables)) {
            if (logMINOR) {
                Logger.minor(this,
                             "Ignoring onFound because enable ULPR = " + node.enableULPRDataPropagation
        ...
        synchronized (blockOfferListByKey) {
            while (true) {
                ...
                if (bl.isEmpty(now) || (bl.expires() < now) || (blockOfferListByKey.size() > MAX_OFFERS)) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Removing block offer list " + bl + " list size now "
    ...
    public void sendOfferedKey(final Key key, final boolean isSSK, final boolean needPubKey, final long uid,
                               final PeerNode source, final OfferReplyTag tag, final boolean realTimeFlag)
    ...
    protected void innerSendOfferedKey(Key key, final boolean isSSK, boolean needPubKey, final long uid,
        ...
        if (isSSK) {
            ...
            final BlockTransmitter bt = new BlockTransmitter(node.usm, node.getTicker(), source, uid, prb,
                                            senderCounter, BlockTransmitter.NEVER_CASCADE,
