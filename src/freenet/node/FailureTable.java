	...
	class OfferList {
		...
		public BlockOffer getFirstOffer() {
			...
			if(!recentOffers.isEmpty()) {
				return lastOffer = ListUtils.removeRandomBySwapLastSimple(node.getRNG(), recentOffers);
			}
			if(!expiredOffers.isEmpty()) {
				return lastOffer = ListUtils.removeRandomBySwapLastSimple(node.getRNG(), expiredOffers);
			}
			// No more offers.
			return null;
		}
