...
public class MessageWrapper {
        ...
        synchronized (sent) {
            synchronized (acks) {
                ...
                for (int[] range : acks) {
                    ...
                    Logger.warning(this,
                                   "Lost range (" + start + "->" + end + ") is overlapped by acked range (" + range[0]
                                   + "->" + range[1] + "). Adding " + toAddStart + "->" + toAddEnd + " to sent");
    ...
    public MessageFragment getMessageFragment(int maxLength) {
        ...
        synchronized (sent) {
            ...
            if (logDEBUG) {
                Logger.debug(this,
                             "Using range " + start + " to " + (start + dataLength - 1) + " gives " + sent + " on "
                             + messageID);
            }
        }
