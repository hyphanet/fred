...
public class UpdatedVersionAvailableUserAlert extends AbstractUserAlert {
	...
	private String l10n(String key) {
		return NodeL10n.getBase()
			.getString("UpdatedVersionAvailableUserAlert." + key);
	}

	private String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString(
				"UpdatedVersionAvailableUserAlert." + key,
				pattern,
				value
			);
	}
	...
	public HTMLNode getHTMLText() {
		...
		if (ut.formText != null) {
			...
			alertNode.addChild(
				...
				new String[] {
					"hidden",
					"formPassword",
					updater.node.clientCore.formPassword,
				}
			);
		}
		...
		if (updater.hasNewMainJar()) {
			...
			Logger.minor(
				this,
				"Showing version available notification but not fetching or fetched."
			);
			// Fallback
	...
	private UpdateThingy createUpdateThingy() {
		...
		if (updater.isArmed() && updater.inFinalCheck()) {
			...
			if (updater.canUpdateNow()) {
				if (updater.hasNewMainJar()) {
					sb.append(
						l10n(
							"downloadedNewJar",
							"version",
							Integer.toString(updater.newMainJarVersion())
						)
					);
					sb.append(' ');
				}
				...
				if (updater.fetchingFromUOM()) sb.append(
					l10n("fetchingUOM", "updateScript", getUpdateScriptName())
				);
				else {
					...
					if (fetchingNew) {
						sb.append(
							l10n(
								...
								"nodeVersion",
								Integer.toString(
									updater.fetchingNewMainJarVersion()
								)
							)
						);
					}
	...
	public short getPriorityClass() {
		...
		if (
			updater.inFinalCheck() ||
			updater.canUpdateNow() ||
			!updater.isArmed()
