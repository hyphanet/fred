...
public class IPUndetectedUserAlert extends AbstractUserAlert {
	...
	public boolean isValid() {
		if(node.isOpennetEnabled())
			return false;
		if(node.getPeerManager().countConnectiblePeers() >= 5 && (node.getUptime() < MINUTES.toMillis(1) || node.getIPDetector().isDetecting()))
			return false;
		return true;
	}
	...
	public HTMLNode getHTMLText() {
		...
		formNode.addChild("input", new String[] { "type", "name", "value" }, new String[] { "hidden", "formPassword", node.getClientCore().formPassword });
