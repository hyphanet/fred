...
public class IPUndetectedUserAlert extends AbstractUserAlert {
	...
	public String getText() {
		if (node.ipDetector.noDetectPlugins()) return l10n("noDetectorPlugins");
		if (node.ipDetector.isDetecting()) return l10n("detecting");
		else return (
			l10n(
				"unknownAddress",
				"port",
	...
	private String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("IPUndetectedUserAlert." + key, pattern, value);
	}

	private String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("IPUndetectedUserAlert." + key, patterns, values);
	}
	...
	public boolean isValid() {
		...
		if (
			node.peers.countConnectiblePeers() >= 5 &&
			(node.getUptime() < MINUTES.toMillis(1) ||
	...
	public HTMLNode getHTMLText() {
		...
		if (peers > 0) textNode.addChild(
			"p",
			l10n("noIPMaybeFromPeers", "number", Integer.toString(peers))
		);
		...
		if (node.ipDetector.noDetectPlugins()) {
			...
			NodeL10n.getBase()
				.addL10nSubstitution(
					...
					new HTMLNode[] {
						HTMLNode.link("/plugins/"),
						HTMLNode.link("/config/node"),
					}
				);
		} else if (
			!node.ipDetector.hasJSTUN() && !node.ipDetector.isDetecting()
		) {
			HTMLNode p = textNode.addChild("p");
			NodeL10n.getBase()
				.addL10nSubstitution(
		...
		HTMLNode formNode = textNode.addChild(
			"form",
		...
		itemNode
			...
			.addChild(
				...
				new String[] {
					"text",
					sc.getPrefix() + ".tempIPAddressHint",
					o.getValueDisplayString(),
				}
			);
		itemNode.addChild(
			"span",
		...
		formNode.addChild(
			...
			new String[] {
				"submit",
				NodeL10n.getBase().getString("UserAlert.apply"),
			}
		);
		formNode.addChild(
			"input",
			...
			new String[] {
				"reset",
				NodeL10n.getBase().getString("UserAlert.reset"),
			}
		);
	...
	public short getPriorityClass() {
		if (node.ipDetector.isDetecting()) return UserAlert.WARNING;
	...
	public String getShortText() {
		if (node.ipDetector.noDetectPlugins()) return l10n("noDetectorPlugins");
		if (node.ipDetector.isDetecting()) return l10n("detectingShort");
