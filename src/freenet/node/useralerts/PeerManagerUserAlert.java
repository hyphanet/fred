        ...
        synchronized (this) {
            ...
            if ((peers == 0) &&!isOpennetEnabled) {
                ...
                s = l10n("tooManyDisconnected", new String[] { "count", "max" },
                         new String[] { Integer.toString(disconnDarknetPeers),
                                        Integer.toString(MAX_DISCONN_PEER_ALERT_THRESHOLD) });
            } else if (darknetConns > MAX_DARKNET_CONN_ALERT_THRESHOLD) {
                s = l10n("tooManyConns", new String[] { "count", "max" }, new String[] { Integer.toString(conns),
        ...
        synchronized (this) {
            if (isOutdated) {
                ...
                NodeL10n.getBase().addL10nSubstitution(alertNode, "PeerManagerUserAlert.tooManyNeverConnectedWithLink",
                        new String[] { "link",
                                       "count" }, new HTMLNode[] { HTMLNode.link("/friends/myref.fref"),
                ...
                alertNode.addChild("#", l10n("connError", "count", Integer.toString(connError)));
                       ...
                       &&!darknetAssumeNAT) {
                alertNode.addChild("#",
                                   l10n("tooManyDisconnected", new String[] { "count", "max" },
                                        new String[] { Integer.toString(disconnDarknetPeers),
                        Integer.toString(MAX_DISCONN_PEER_ALERT_THRESHOLD) }));
            } else if (darknetConns > MAX_DARKNET_CONN_ALERT_THRESHOLD) {
                alertNode.addChild("#",
                                   l10n("tooManyConns", new String[] { "count", "max" },
        ...
        synchronized (this) {
            ret = (((peers == 0) &&!isOpennetEnabled) || ((conns < 3) &&!isOpennetEnabled)
                   ...
                   || ((disconnDarknetPeers > MAX_DISCONN_PEER_ALERT_THRESHOLD) &&!darknetDefinitelyPortForwarded
                       &&!darknetAssumeNAT) || (darknetConns > MAX_DARKNET_CONN_ALERT_THRESHOLD)
                           || (clockProblem > MIN_CLOCK_PROBLEM_PEER_ALERT_THRESHOLD)
                               || (connError > MIN_CONN_ERROR_ALERT_THRESHOLD)
                                   || (bwlimitDelayAlertRelevant
                                       && (bwlimitDelayTime
                                           > NodeStats
                                               .MAX_BWLIMIT_DELAY_TIME_ALERT_THRESHOLD)) || (nodeAveragePingAlertRelevant
                                                   && (nodeAveragePingTime
                                                       > NodeStats
