...
public class UpgradeConnectionSpeedUserAlert extends AbstractUserAlert {
	...
	private UpgradeConnectionSpeedUserAlert(
		Node node,
	...
	public HTMLNode getHTMLText() {
		...
		content.addChild(
			...
			l10n(
				...
				new String[] {
					SizeUtil.formatSize(
						node.config.get("node").getInt("inputBandwidthLimit")
					),
					SizeUtil.formatSize(
						node.config.get("node").getInt("outputBandwidthLimit")
					),
				}
			)
		);
		...
		HTMLNode form = content.addChild(
			"form",
			new String[] { "action", "method" },
			new String[] { "/", "post" }
		);
		HTMLNode bandwidthInput = form.addChild(
			"div",
			new String[] { "style" },
		...
		bandwidthInput.addChild(
			...
			new String[] {
				"text",
				"inputBandwidthLimit",
				SizeUtil.formatSize(bandwidthLimit.downBytes),
			}
		);
		...
		bandwidthInput.addChild(
			...
			new String[] {
				...
				SizeUtil.formatSize(bandwidthLimit.upBytes),
			}
		);
	...
	private String l10n(String key) {
		return NodeL10n.getBase()
			.getString("UpgradeConnectionSpeedUserAlert." + key);
	}

	private String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString(
				"UpgradeConnectionSpeedUserAlert." + key,
				patterns,
				values
			);
	}
