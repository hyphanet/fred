    ...
    public void removeRoutingTo(PeerNode next) {
        ...
        synchronized (this) {
            ...
            if (!currentlyRoutingTo.remove(next)) {
                Logger.warning(this, "Removing wrong node or removing twice? on " + this + " : " + next,
                               new Exception("debug"));
            }
            ...
            if (!(reassigned || wasLocal || sourceRestarted || timedOutButContinued)) {
                    ...
                    for (PeerNode pn : currentlyRoutingTo) {
                        if (handlingTimeouts.contains(pn)) {
                            if (logMINOR) {
                                Logger.debug(
                                    this,
                                    "Still waiting for " + pn.shortToString()
                ...
                if (!expected) {
                    if (handlingTimeouts != null) {
                        Logger.normal(
                            this,
                            "Unlocked handler but still routing to " + currentlyRoutingTo
        ...
        if ((fetchingOfferedKeyFrom != null) &&!fetchingOfferedKeyFrom.isEmpty()) {
            if (!(reassigned || wasLocal)) {
                ...
                if (!expected) {
                    ...
                    Logger.error(this,
                                 "Unlocked handler but still fetching offered keys from " + fetchingOfferedKeyFrom
                                 + " yet not reassigned on " + this, new Exception("debug"));
                } else {
                    reassignToSelf();
                }
