...
public class NodeDispatcher implements Dispatcher, Runnable {
        ...
        if (spec == DMT.FNPPing) {
            ...
            if ((OpennetManager.MAX_PEERS_FOR_SCALING < locs.length) && (source.isOpennet())) {
                if (locs.length > OpennetManager.PANIC_MAX_PEERS) {
                    ...
                    Logger.error(this,
                                 "We received " + locs.length + " locations from " + source.toString()
                    ...
                    Logger.normal(this,
                                  "Too many locations from " + source.toString() + " : " + locs.length
    ...
    private boolean handleGetOfferedKey(Message m, PeerNode source) {
        ...
        try {
            ...
            RejectReason reject = nodeStats.shouldRejectRequest(true, false, isSSK, false, true, source, false, false,
                                      realTimeFlag, tag);
    ...
    private void innerHandleDataRequest(Message m, PeerNode source, boolean isSSK) {
        ...
        RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, false, isSSK, false, false, source,
        ...
        if (rejectReason != null) {
            ...
            Logger.normal(this,
                          "Rejecting " + (isSSK ? "SSK" : "CHK") + " request from " + source.getPeer()
    ...
    private void handleInsertRequest(Message m, PeerNode source, boolean isSSK) {
        ...
        RejectReason rejectReason = nodeStats.shouldRejectRequest(!isSSK, true, isSSK, false, false, source, false,
                                        preferInsert, realTimeFlag, tag);
        ...
        if (m.getSpec().equals(DMT.FNPSSKInsertRequest)) {
            ...
            SSKInsertHandler rh = new SSKInsertHandler(key, data, headers, htl, source, id, node, now, tag,
                                      node.canWriteDatastoreInsert(htl), forkOnCacheable, preferInsert,
                ...
                cb = new AnnouncementCallback() {
                    ...
                    public void completed() {
                        ...
                        if (acceptedSomewhere) {
                            Logger.minor(this,
                                         "Announcement from " + origin + " completed (" + totalAdded + " added, "
                    ...
                    public void nodeNotWanted() {
                        ...
                        Logger.minor(this,
                                     "Announcement from " + origin + " returned node not wanted for a total of "
