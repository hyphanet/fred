...
public class NodeIPPortDetector {
	...
	FreenetInetAddress[] detectPrimaryIPAddress() {
		...
		return ipDetector.detectPrimaryIPAddress(
			!crypto.config.includeLocalAddressesInNoderefs
		);
	}
		...
		if (peerList != null) {
			...
			for (PeerNode pn : peerList) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Peer " + pn.getPeer() + " thinks we are " + p
				);
