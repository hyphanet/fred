...
public class NewPacketFormat implements PacketFormat {
	...
	private final HashMap<Integer, PartiallyReceivedBuffer> receiveBuffers =
		new HashMap<>();
	...
	public boolean handleReceivedPacket(
		...
		for (int i = 0; i < 3; i++) {
			...
			if (packet != null) {
				if (logDEBUG) Logger.debug(
					this,
					"Decrypted packet with tracker " + i
				);
				break;
			}
		}
		if (packet == null) {
			if (logMINOR) Logger.minor(
				this,
				"Could not decrypt received packet"
			);
			return false;
		}
		...
		if (logMINOR && !finished.isEmpty()) Logger.minor(
			this,
			"Decoded messages: " + finished.size()
		);
		DecodingMessageGroup group = pn.startProcessingDecryptedMessages(
			finished.size()
		);
	...
	List<byte[]> handleDecryptedPacket(
		NPFPacket packet,
		...
		if (packet.getError() || (packet.getFragments().size() == 0)) {
			if (logMINOR) Logger.minor(
				this,
				"Not acking because " +
				(packet.getError() ? "error" : "no fragments")
			);
			dontAck = true;
		}
		...
		if (l != null && !l.isEmpty()) {
			...
			if (logMINOR && lossyMessages.size() > 0) Logger.minor(
				this,
				"Successfully parsed " +
				lossyMessages.size() +
				" lossy packet messages"
			);
		...
		for (MessageFragment fragment : packet.getFragments()) {
			if (messageWindowPtrReceived + MSG_WINDOW_SIZE > NUM_MESSAGE_IDS) {
				int upperBound =
					(messageWindowPtrReceived + MSG_WINDOW_SIZE) %
					NUM_MESSAGE_IDS;
				if (
					(fragment.messageID > upperBound) &&
					(fragment.messageID < messageWindowPtrReceived)
				) {
					if (logMINOR) Logger.minor(
						this,
						"Received message " +
						fragment.messageID +
						" outside window, acking"
					);
					continue;
				}
			} else {
			...
			PartiallyReceivedBuffer recvBuffer = receiveBuffers.get(
				fragment.messageID
			);
			...
			if (recvBuffer == null) {
				if (logMINOR) Logger.minor(
					this,
					"Message id " + fragment.messageID + ": Creating buffer"
				);
				...
				if (fragment.firstFragment) {
					...
					synchronized (receiveBufferSizeLock) {
						if (
							(receiveBufferUsed + fragment.fragmentLength) >
							MAX_RECEIVE_BUFFER_SIZE
						) {
							if (logMINOR) Logger.minor(
								this,
								"Could not create buffer, would excede max size"
							);
							dontAck = true;
							continue;
						}
			...
			recvMap.add(
				...
				fragment.fragmentOffset + fragment.fragmentLength - 1
			);
			if (
				(recvBuffer.messageLength != -1) &&
				recvMap.contains(0, recvBuffer.messageLength - 1)
			) {
				...
				synchronized (receivedMessages) {
					...
					while (
						...
						messageWindowPtrReceived++;
						if (
							messageWindowPtrReceived == NUM_MESSAGE_IDS
				...
				if (logMINOR) Logger.minor(
					this,
					"Message id " + fragment.messageID + ": Completed"
				);
			} else {
				if (logDEBUG) Logger.debug(
					this,
					"Message id " + fragment.messageID + ": " + recvMap
				);
			}
		}
		...
		if (!dontAck) {
			...
			if (addedAck) {
				if (!wakeUp) {
					synchronized (sendBufferLock) {
						if (
							receiveBufferUsed > MAX_RECEIVE_BUFFER_SIZE / 2
						) wakeUp = true;
					}
				}
	...
	private NPFPacket tryDecipherPacket(
		byte[] buf,
		int offset,
		int length,
		SessionKey sessionKey
		...
		if (keyContext.seqNumWatchList == null) {
			if (logMINOR) Logger.minor(
				this,
				"Creating watchlist starting at " + keyContext.watchListOffset
			);
			...
			for (int i = 0; i < keyContext.seqNumWatchList.length; i++) {
				keyContext.seqNumWatchList[i] =
					NewPacketFormat.encryptSequenceNumber(seqNum++, sessionKey);
		...
		int oldHighestReceived = (int) ((0L +
				keyContext.watchListOffset +
				(keyContext.seqNumWatchList.length / 2)) %
			NUM_SEQNUMS);
		if (seqNumGreaterThan(highestReceivedSeqNum, oldHighestReceived, 31)) {
			...
			if (highestReceivedSeqNum > oldHighestReceived) {
				...
				moveBy = ((int) (NUM_SEQNUMS - oldHighestReceived)) +
			...
			if (moveBy > keyContext.seqNumWatchList.length) {
				...
				Logger.warning(
					this,
					"Tried moving watchlist pointer by " + moveBy
				);
				moveBy = 0;
			} else {
				if (logDEBUG) Logger.debug(
					this,
					"Moving watchlist pointer by " + moveBy
				);
			}
			...
			int seqNum = (int) ((0L +
				...
				NUM_SEQNUMS);
			for (
				int i = keyContext.watchListPointer;
				i < (keyContext.watchListPointer + moveBy);
				i++
			) {
				keyContext.seqNumWatchList[i %
					keyContext.seqNumWatchList.length] = encryptSequenceNumber(
					seqNum++,
					sessionKey
				);
				if (seqNum < 0) seqNum = 0;
			...
			keyContext.watchListPointer = (keyContext.watchListPointer +
				moveBy) %
			keyContext.seqNumWatchList.length;
			keyContext.watchListOffset = (int) ((0L +
					keyContext.watchListOffset +
					moveBy) %
				NUM_SEQNUMS);
		}
		...
		for (int i = 0; i < keyContext.seqNumWatchList.length; i++) {
			...
			int sequenceNumber = (int) ((0L + keyContext.watchListOffset + i) %
				NUM_SEQNUMS);
			if (logDEBUG) Logger.debug(
				this,
				"Received packet matches sequence number " + sequenceNumber
			);
			NPFPacket p = decipherFromSeqnum(
				buf,
				offset,
				length,
				sessionKey,
	...
	private NPFPacket decipherFromSeqnum(
		...
		byte[] localHash = Arrays.copyOf(
			HMAC.macWithSHA256(sessionKey.hmacKey, payload),
			HMAC_LENGTH
		);
		if (!MessageDigest.isEqual(hash, localHash)) {
			if (logMINOR) {
				Logger.minor(
					this,
					"Failed to validate the HMAC using TrackerID=" +
					sessionKey.trackerID
				);
			}
		...
		PCFBMode payloadCipher = PCFBMode.create(
			sessionKey.incommingCipher,
			IV
		);
		...
		synchronized (this) {
			if (
				seqNumGreaterThan(
					sequenceNumber,
					keyContext.highestReceivedSeqNum,
					31
				)
			) {
	...
	private boolean seqNumGreaterThan(long i1, long i2, int serialBits) {
		...
		return (
			((i1 < i2) && ((i2 - i1) > halfValue)) ||
			((i1 > i2) && (i1 - i2 < halfValue))
		);
	}
	...
	boolean maybeSendPacket(boolean ackOnly, SessionKey sessionKey)
		...
		NPFPacket packet = createPacket(
			maxPacketSize - HMAC_LENGTH,
			pn.getMessageQueue(),
			sessionKey,
			ackOnly
		);
		...
		if (pn.shouldPadDataPackets()) {
			...
			if (logDEBUG) Logger.debug(
				this,
				"Pre-padding length: " + packetLength
			);
			...
			if (packetLength < 64) {
				...
				if (paddedLen < maxPacketSize) {
					...
					pn
						.paddingGen()
						.nextInt(Math.min(64, maxPacketSize - paddedLen));
				} else if (
		...
		try {
			if (logMINOR) {
				...
				for (MessageFragment frag : packet.getFragments()) {
					if (fragments == null) fragments = String.valueOf(
						frag.messageID
					);
	...
	NPFPacket createPacket(
		...
		if (numAcks > 0) {
			if (logDEBUG) Logger.debug(
				this,
				"Added acks for " + this + " for " + pn.shortToString()
			);
		}
		...
		if (!ackOnly) {
			...
			while (true) {
				...
				synchronized (sendBufferLock) {
					...
					addOldLoop:for (Map<
						Integer,
						MessageWrapper
					> started : startedByPrio) {
						//Try to finish messages that have been started
						Iterator<MessageWrapper> it = started
							.values()
							.iterator();
						while (
							it.hasNext() && packet.getLength() < maxPacketSize
						) {
							MessageWrapper wrapper = it.next();
							while (packet.getLength() < maxPacketSize) {
								MessageFragment frag =
									wrapper.getMessageFragment(
										maxPacketSize - packet.getLength()
									);
								if (frag == null) break;
								mustSend = true;
								addedFragments = true;
								...
								if (wrapper.allSent()) {
									if (
										(haveAddedStatsBulk == null) &&
										wrapper.getItem().sendLoadBulk
									) {
										addStatsBulk = true;
									...
									if (
										(haveAddedStatsRT == null) &&
										wrapper.getItem().sendLoadRT
									) {
										addStatsRT = true;
										// Add the lossy message outside the lock.
										break addOldLoop;
			...
			if (addedFragments) {
				if (logDEBUG) Logger.debug(
					this,
					"Added fragments for " + this + " (must send)"
				);
			}
		}
		...
		if ((!mustSend) && packet.getLength() >= ((maxPacketSize * 4) / 5)) {
			if (logDEBUG) Logger.debug(
				this,
				"Must send because packet is big on acks alone"
			);
		...
		if ((!ackOnly) && (!mustSend)) {
			if (
				...
				if (logDEBUG) Logger.debug(
					this,
					"Must send because of message queue"
				);
				mustSend = true;
			}
		}
		...
		if ((!mustSend) && numAcks > 0) {
			...
			synchronized (sendBufferLock) {
				if (sendBufferUsed > maxSendBufferSize / 2) {
					if (logDEBUG) Logger.debug(
						this,
						"Must send because other side buffer size is " +
						sendBufferUsed
					);
					mustSend = true;
				}
		...
		if (DO_KEEPALIVES) {
			synchronized (this) {
				if (!mustSend) {
					if (
						now - timeLastSentPacket > Node.KEEPALIVE_INTERVAL
					) mustSend = true;
				}
				if (
					(!ackOnly) &&
			...
			fragments:for (int i = 0; i < startedByPrio.size(); i++) {
				prio:while (true) {
					...
					while ((packet.getLength() + 10) < maxPacketSize) { //Fragment header is max 9 bytes, allow min 1 byte data
						...
						MessageItem item = messageQueue.grabQueuedMessageItem(
							i
						);
						if (item == null) {
							if (mustSendKeepalive && packet.noFragments()) {
								...
								item.setDeadline(
									now + PacketSender.MAX_COALESCING_DELAY
								);
						...
						if (messageID == -1) {
							...
							if (!wasGeneratedPing) {
								messageQueue.pushfrontPrioritizedMessageItem(
									item
								);
						...
						MessageFragment frag = wrapper.getMessageFragment(
							maxPacketSize - packet.getLength()
						);
						if (frag == null) {
							messageQueue.pushfrontPrioritizedMessageItem(item);
						...
						Map<Integer, MessageWrapper> queue = startedByPrio.get(
							item.getPriority()
						);
						synchronized (sendBufferLock) {
		...
		if (logDEBUG && ackOnly) {
			Logger.debug(
				this,
				"Sending ack-only packet length " +
				packet.getLength() +
				" for " +
				this
			);
			...
			Logger.debug(
				this,
				"Sending packet length " + packet.getLength() + " for " + this
			);
		}
	...
	public long timeCheckForLostPackets() {
		...
		if (key != null) timeCheck = Math.min(
			timeCheck,
			((key.packetContext)).timeCheckForLostPackets(averageRTT)
		);
	...
	private long timeCheckForAcks() {
		...
		if (key != null) timeCheck = Math.min(
			timeCheck,
			(key.packetContext).timeCheckForAcks()
		);
		key = pn.getPreviousKeyTracker();
		if (key != null) timeCheck = Math.min(
			timeCheck,
	...
	public void checkForLostPackets() {
		...
		SessionKey key = pn.getCurrentKeyTracker();
		if (key != null) ((key.packetContext)).checkForLostPackets(
				averageRTT,
				curTime,
				pn
			);
		...
		if (ret > now) {
			...
			if (canSend && DO_KEEPALIVES) {
				synchronized (this) {
					ret = Math.min(
						ret,
						timeLastSentPayload + Node.KEEPALIVE_INTERVAL
					);
				}
			}
		}
	...
	public boolean canSend(SessionKey tracker) {
		...
		synchronized (this) {
			...
			canAllocateID = !seqNumGreaterThan(
				nextMessageID,
				(messageWindowPtrAcked + MSG_WINDOW_SIZE) % NUM_MESSAGE_IDS,
				28
			);
		}
		...
		if (canAllocateID) {
			...
			if (!keyContext.canAllocateSeqNum()) {
				...
				Logger.error(
					this,
					"Can't send because we would block on " + this
				);
				return false;
			}
		}
		...
		if (tracker != null && pn != null) {
			...
			if (throttle == null) {
				...
				int maxPackets = (int) Math.min(
					Integer.MAX_VALUE,
					pn.getThrottle().getWindowSize()
				);
		...
		if (logDEBUG && !canAllocateID) Logger.debug(
			this,
			"Cannot send because cannot allocate ID on " + this
		);
		...
		synchronized (this) {
			if (
				...
				if (blockedSince == -1) {
					...
					System.currentTimeMillis() - blockedSince >
					MAX_MSGID_BLOCK_TIME
				) {
					throw new BlockedTooLongException(
						System.currentTimeMillis() - blockedSince
					);
				}
				return -1;
		...
		public long acked(SessionKey key) {
			...
			while (msgIt.hasNext()) {
				...
				if (wrapper.ack(range[0], range[1], npf.pn)) {
					Map<Integer, MessageWrapper> started =
						npf.startedByPrio.get(wrapper.getPriority());
					...
					synchronized (npf.sendBufferLock) {
						...
						if (removed != null) {
							...
							if (logDEBUG) Logger.debug(
								...
								"Removed " +
								size +
								" from remote buffer. Total is now " +
								npf.sendBufferUsed
							);
						}
					}
					...
					if (removed != null) {
						...
						synchronized (npf) {
							...
							while (
								...
								npf.messageWindowPtrAcked++;
								if (
									npf.messageWindowPtrAcked == NUM_MESSAGE_IDS
								) npf.messageWindowPtrAcked = 0;
							}

							if (npf.messageWindowPtrAcked < oldWindow) {
								npf.ackedMessages.remove(
									oldWindow,
									NUM_MESSAGE_IDS - 1
								);
								npf.ackedMessages.remove(
	...
	private static class PartiallyReceivedBuffer {
		...
		private boolean setMessageLength(int messageLength) {
			if (
				this.messageLength != -1 && this.messageLength != messageLength
			) {
				Logger.warning(
					this,
					"Message length has already been set to a different length"
				);
			}
		...
		private boolean resize(int length) {
			if (logDEBUG) Logger.debug(
				this,
				"Resizing from " + buffer.length + " to " + length
			);
			...
			synchronized (npf.receiveBufferSizeLock) {
				if (
					...
					if (logMINOR) Logger.minor(
						this,
						"Could not resize buffer, would excede max size"
					);
					return false;
				}
	...
	public boolean fullPacketQueued(int maxPacketSize) {
		return pn
			.getMessageQueue()
			.mustSendSize(
				HMAC_LENGTH/* FIXME estimate headers */,
				maxPacketSize
			);
	}
}
