...
public class NewPacketFormat implements PacketFormat {
    ...
    private final HashMap<Integer, PartiallyReceivedBuffer> receiveBuffers = new HashMap<Integer,
                                                                                 PartiallyReceivedBuffer>();
        ...
        for (MessageFragment fragment : packet.getFragments()) {
            ...
            if ((recvBuffer.messageLength != -1) && recvMap.contains(0, recvBuffer.messageLength - 1)) {
                ...
                synchronized (sendBufferLock) {
                    ...
                    if (logDEBUG) {
                        Logger.debug(this,
                                     "Removed " + recvBuffer.messageLength + " from buffer. Total is now "
                                     + receiveBufferUsed);
                    }
                }
    ...
    private NPFPacket tryDecipherPacket(byte[] buf, int offset, int length, SessionKey sessionKey) {
        ...
        for (int i = 0; i < keyContext.seqNumWatchList.length; i++) {
            ...
            if (!Fields.byteArrayEqual(buf, keyContext.seqNumWatchList[index], offset + hmacLength, 0,
    ...
    private NPFPacket decipherFromSeqnum(byte[] buf, int offset, int length, SessionKey sessionKey,
        ...
        if (!HMAC.verifyWithSHA256(sessionKey.hmacKey, payload, hash)) {
            return null;
        }
    ...
    public boolean maybeSendPacket(long now, boolean ackOnly) throws BlockedTooLongException {
        ...
        if (sessionKey != null) {
            ...
            if (maybeSendPacket(now, true, sessionKey)) {
                return true;
            }
        }
    ...
    public boolean maybeSendPacket(long now, boolean ackOnly, SessionKey sessionKey) throws BlockedTooLongException {
        ...
        try {
            if (logMINOR) {
                ...
                Logger.minor(this,
                             "Sending packet " + packet.getSequenceNumber() + " (" + data.length
                             + " bytes) with fragments " + fragments + " and " + packet.getAcks().size() + " acks on "
                             + this);
            }
...
prio:
                while (true) {
                    ...
                    while ((packet.getLength() + 10) < maxPacketSize) {    // Fragment header is max 9 bytes, allow min 1 byte data
                        ...
                        synchronized (sendBufferLock) {
                            ...
                            if (logDEBUG) {
                                Logger.debug(this,
                                             "Added " + item.buf.length + " to remote buffer. Total is now "
    ...
    public List<MessageItem> onDisconnect() {
        ...
        synchronized (sendBufferLock) {
            ...
            if (sendBufferUsed != 0) {
                Logger.warning(this,
                               "Possible leak in transport code: Buffer size not empty after disconnecting on " + this
                               + " for " + pn + " after removing " + messageSize + " total was " + sendBufferUsed);
                sendBufferUsed = 0;
            }
        }
    ...
    public boolean canSend(SessionKey tracker) {
        ...
        if (canAllocateID) {
            ...
            if ((bufferUsage + MAX_MESSAGE_SIZE) > maxSendBufferSize) {
                if (logDEBUG) {
                    Logger.debug(this,
                                 "Cannot send: Would exceed remote buffer size. Remote at " + bufferUsage + " max is "
                                 + maxSendBufferSize + " on " + this);
                }
        ...
        if ((tracker != null) && (pn != null)) {
            ...
            if (throttle == null) {
                ...
                if (maxPackets <= packets.countSentPackets()) {
                    ...
                    if (logDEBUG) {
                        Logger.debug(this,
                                     "Cannot send because " + packets.countSentPackets() + " in flight of limit "
                                     + maxPackets + " on " + this);
                    }
    ...
    private static class PartiallyReceivedBuffer {
        ...
        private boolean resize(int length) {
            ...
            synchronized (npf.receiveBufferSizeLock) {
                ...
                if (logDEBUG) {
                    Logger.debug(this,
                                 "Added " + (length - buffer.length) + " to buffer. Total is now "
                ...
                if (wrapper.ack(range[0], range[1], npf.pn)) {
                    ...
                    synchronized (npf.sendBufferLock) {
                        ...
                        if (removed != null) {
                            ...
                            if (logDEBUG) {
                                Logger.debug(this,
                                             "Removed " + size + " from remote buffer. Total is now "
                                             + npf.sendBufferUsed);
                            }
                        }
                    }
                    ...
                    if ((removed == null) && logMINOR) {
                        ...
                        Logger.minor(this,
                                     "Completed message " + wrapper.getMessageID() + " but it is not in the map from "
                                     + wrapper);
                    }
