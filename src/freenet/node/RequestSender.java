...
public final class RequestSender implements PrioRunnable, ByteCounter {
	...
	private Message createDataRequest() {
		if(key instanceof NodeCHK) {
			...
			throw new IllegalStateException("Unknown keytype: "+key);
		}
	}

	private void verifyAndCommit(byte[] data) throws KeyVerifyException {
		if(key instanceof NodeCHK) {
			CHKBlock tmpBlock = new CHKBlock(data, headers, (NodeCHK)key);
			...
			node.storeShallow(tmpBlock, canWriteClientCache, canWriteDatastore, tryOffersOnly);
			if(node.random.nextInt(RANDOM_REINSERT_INTERVAL) == 0) {
				node.queueRandomReinsert(tmpBlock);
			}
		} else if (key instanceof NodeSSK) {
			try {
	...
	private final static MedianMeanRunningAverage avgTimeTaken = new MedianMeanRunningAverage();
...
>>>>>>> 1e0cc3dd82ea66b52eed0f968aff40e8039dc4ea
	...
	public void addListener(Listener l) {
		...
		if (tmpStatus!=NOT_FINISHED && sentFinished)
			l.onRequestSenderFinished(tmpStatus);
	}
