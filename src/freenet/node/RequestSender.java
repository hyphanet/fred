...
public final class RequestSender extends BaseSender implements PrioRunnable {
    ...
    public RequestSender(Key key, DSAPublicKey pubKey, short htl, long uid, RequestTag tag, Node n, PeerNode source,
                         boolean offersOnly, boolean canWriteClientCache, boolean canWriteDatastore,
        ...
        while (true) {
            ...
            if (next == null) {
                if (logMINOR && (rejectOverloads > 0)) {
                    Logger.minor(this,
                                 "no more peers, but overloads (" + rejectOverloads + "/" + routeAttempts
                                 + " overloaded)");
                }
    ...
    private OFFER_STATUS handleOfferTimeout(final BlockOffer offer, final PeerNode pn, OfferList offers) {
        try {
            node.usm.addAsyncFilter(getOfferedKeyReplyFilter(pn, GET_OFFER_LONG_TIMEOUT),
                ...
                public void onMatched(Message m) {
                    OFFER_STATUS status = isSSK
                                          ? handleSSKOfferReply(m, pn, offer) : handleCHKOfferReply(m, pn, offer, null);
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Forked get offered key due to two stage timeout completed with status " + status
    ...
    private OFFER_STATUS handleSSKOfferReply(Message reply, PeerNode pn, BlockOffer offer) {
        if (reply.getSpec() == DMT.FNPRejectedOverload) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Node " + pn + " rejected FNPGetOfferedKey as invalid with reason "
                    ...
                    public void blockReceiveFailed(RetrievalException e) {
                        ...
                        try {
                            if (e.getReason() == RetrievalException.SENDER_DISCONNECTED) {
                                ...
                                Logger.normal(this,
                                              "Transfer for offer failed (" + e.getReason() + "/"
                                              + RetrievalException.getErrString(e.getReason()) + "): " + e + " from "
                                              + pn, e);
                            }
            ...
            public void blockReceiveFailed(RetrievalException e) {
                try {
                    ...
                    if (e.getReason() == RetrievalException.SENDER_DISCONNECTED) {
                        ...
                        // A certain number of these are normal, it's better to track them through statistics than call attention to them in the logs.
                        Logger.normal(this, "Transfer failed (" + e.getReason() + "/"
                                      + RetrievalException.getErrString(e.getReason()) + "): " + e + " from "
                                      + next, e);
                    }

                    if (RequestSender.this.source == null) {
                        Logger.normal(this, "Local transfer failed: " + e.getReason() + " : "
                                      + RetrievalException.getErrString(e.getReason()) + "): " + e + " from "
                                      + next, e);
                    }
        ...
        while (true) {
            ...
            while (true) {
                ...
                try {
                    if (now >= deadline) {
                        Logger.error(this,
                                     "Waited more than 5 minutes for status change on " + this + " current = "
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Waiting for status change on " + this + " current is " + current + " status is "
                                     + status);
                    }
    ...
    private void finish(int code, PeerNode next, boolean fromOfferedKey) {
        ...
        synchronized (this) {
            if (status != NOT_FINISHED) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Status already set to " + status + " - returning on " + this + " would be setting "
                                 + code + " from " + next);
                }
        ...
        if (status == SUCCESS) {
            if ((!isSSK) && (transferTime > 0) && logMINOR) {
                ...
                synchronized (avgTimeTaken) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Successful CHK request transfer " + transferTime + " average "
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     ...
                                     + "ms, mean " + (avgTimeTaken.meanValue() - avgTimeTakenTransfer.meanValue())
                                     + "ms");
                    }
                }
            }
    ...
    protected void handleAcceptedRejectedTimeout(final PeerNode next, final UIDTag origTag) {
        ...
        try {
            node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
                ...
                public void onTimeout() {
                    Logger.error(this,
                                 "Fatal timeout waiting for Accepted/Rejected from " + next + " on "
                                 + RequestSender.this);
                    next.fatalTimeout(origTag, false);
                }
                @Override
