...
public class BandwidthManager {
				...
				public void run() {
					try {
						...
						if (
							!node.config
								.get("node")
								.getBoolean("connectionSpeedDetection") ||
							bandwidthIndicator == null
						) {
							return;
						}

						int detectedInputBandwidth =
							bandwidthIndicator.getDownstreamMaxBitRate() / 8;
						int detectedOutputBandwidth =
							bandwidthIndicator.getUpstramMaxBitRate() / 8;

						int currentInputBandwidth = node.config
							.get("node")
							.getInt("inputBandwidthLimit");
						int currentOutputBandwidth = node.config
							.get("node")
							.getInt("outputBandwidthLimit");

						if (
							(detectedInputBandwidth >
									currentInputBandwidth * 3 &&
								detectedInputBandwidth >
									lastOfferedInputBandwidth * 3) ||
							(detectedOutputBandwidth >
									currentOutputBandwidth * 3 &&
								detectedOutputBandwidth >
									lastOfferedOutputBandwidth * 3)
						) {
							lastOfferedInputBandwidth = Math.max(
								detectedInputBandwidth / 2,
								currentInputBandwidth
							);
							lastOfferedOutputBandwidth = Math.max(
								detectedOutputBandwidth / 2,
								currentOutputBandwidth
							);
	...
	public static void checkOutputBandwidthLimit(int obwLimit)
			...
			throw new InvalidConfigValueException(
				NodeL10n.getBase()
					.getString(
						...
						"max",
						Long.toString(SECONDS.toNanos(1))
					)
			);
		}
	}

	public static void checkInputBandwidthLimit(int ibwLimit)
		throws InvalidConfigValueException {
		...
		if (ibwLimit <= 1) {
			throw new InvalidConfigValueException(
				NodeL10n.getBase()
					.getString("Node.bandwidthLimitMustBePositiveOrMinusOne")
			);
		}
			...
			NodeL10n.getBase()
				.getString(
					...
					new String[] {
						...
						Integer.toString(Node.getMinimumBandwidth()),
					}
				)
		);
	}
