...
//  private static final double SOFT_REJECT_MAX_BANDWIDTH_USAGE = 1.0;
    ...
    NodeStats(Node node, int sortOrder, SubConfig statsConfig, int obwLimit, int ibwLimit, int lastVersion)
        ...
        statsConfig.register("aggressiveGC", aggressiveGCModificator, sortOrder++, true, false,
                             "NodeStat.aggressiveGC", "NodeStat.aggressiveGCLong", new IntCallback() {
            @Override
            public Integer get() {
        ...
        statsConfig.register("enableNewLoadManagementRT", false, sortOrder++, true, false,
                             "Node.enableNewLoadManagementRT", "Node.enableNewLoadManagementRTLong",
                             new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        statsConfig.register("enableNewLoadManagementBulk", false, sortOrder++, true, false,
                             "Node.enableNewLoadManagementBulk", "Node.enableNewLoadManagementBulkLong",
                             new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        localChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 2048 /* path folding */, 180000,
                0.0, 200 * 1024, (throttleFS == null) ? null : throttleFS.subset("LocalChkFetchBytesReceivedAverage"),
                node);
        ...
        remoteChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 1024 + 500
                + 2048 /* path folding */, 180000, 0.0, 200 * 1024,
                                           (throttleFS == null)
                                           ? null : throttleFS.subset("RemoteChkFetchBytesReceivedAverage"), node);
        ...
        successfulChkFetchBytesSentAverage = new TimeDecayingRunningAverage(32768 + 1024 + 500
                + 2048 /* path folding */, 180000, 0.0, 200 * 1024,
                                           (throttleFS == null)
                                           ? null : throttleFS.subset("SuccessfulChkFetchBytesSentAverage"), node);
        ...
        successfulChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(32768 + 1024 + 500
                + 2048 /* path folding */, 180000, 0.0, 200 * 1024,
                                           (throttleFS == null)
                                           ? null : throttleFS.subset("SuccessfulChkFetchBytesReceivedAverage"), node);
        ...
        requestOutputThrottle = new TokenBucket(Math.max(obwLimit * 60, 32768 * 20), SECONDS.toNanos(1) / obwLimit, 0);
        requestInputThrottle = new TokenBucket(Math.max(ibwLimit * 60, 32768 * 20), SECONDS.toNanos(1) / ibwLimit, 0);
...
//                  slowDown(">softThreadLimit", isLocal, isInsert, isSSK, isOfferReply, realTimeFlag, tag);
            ...
            if (hasInStore) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Maybe accepting extra request due to it being in datastore (limit now " + limit
                                 + "s)...");
                }
            }
            ...
            int maxTransfersOutPeerLimit = (int) Math.max(1,
                                               getPeerLimit(source,
                                                   maxTransfersOutUpperLimit - maxTransfersOutLowerLimit, false,
                                                   transfersPerInsert, realTimeFlag, peers,
            ...
            ret = checkBandwidthLiability(getInputBandwidthUpperLimit(limit), requestsSnapshot, peerRequestsSnapshot,
                                          true, limit, source, isLocal, isSSK, isInsert, isOfferReply, hasInStore,
                                          transfersPerInsert, realTimeFlag, maxOutputTransfers,
                                          maxTransfersOutPeerLimit, tag);
            ...
            ret = checkMaxOutputTransfers(maxOutputTransfers, maxTransfersOutUpperLimit, maxTransfersOutLowerLimit,
                                          maxTransfersOutPeerLimit, requestsSnapshot, peerRequestsSnapshot, isLocal,
                                          realTimeFlag, isInsert, isSSK, isOfferReply, tag);
...
//  }
    ...
    private double getNonOverheadFraction(long now) {
        ...
        if (nonOverheadFraction < MIN_NON_OVERHEAD) {
            ...
            Logger.warning(this,
                           "Non-overhead fraction is " + nonOverheadFraction
    ...
    private String checkBandwidthLiability(double bandwidthAvailableOutputUpperLimit,
        ...
        double thisAllocation = getPeerLimit(source,
                                    bandwidthAvailableOutputUpperLimit - bandwidthAvailableOutputLowerLimit, input,
                                    transfersPerInsert, realTimeFlag, peers,
...
//                      }
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Total usage is " + bandwidthLiabilityOutput + " below lower limit "
...
//                  slowDown("TooManyTransfers: Fair sharing between peers", isLocal, isInsert, isSSK, isOfferReply, realTime, tag);
    ...
    private void dumpByteCostAverages() {
        ...
        Logger.minor(this,
                     "Byte cost averages: LOCAL:" + " CHK insert " + localChkInsertBytesSentAverage.currentValue()
                     ...
                     + localChkFetchBytesReceivedAverage.currentValue() + " SSK fetch "
                     + localSskFetchBytesSentAverage.currentValue() + '/'
                     + localSskFetchBytesReceivedAverage.currentValue());
        Logger.minor(this,
                     "Byte cost averages: SUCCESSFUL:" + " CHK insert "
    ...
    public void maybeUpdatePeerManagerUserAlertStats(long now) {
        if (now > nextPeerManagerUserAlertStatsUpdateTime) {
            ...
            if (logDEBUG) {
                Logger.debug(this,
                             "mUPMUAS: " + now + ": " + getBwlimitDelayTime() + " >? "
    ...
    public void setOutputLimit(int obwLimit) {
        requestOutputThrottle.changeNanosAndBucketSize(SECONDS.toNanos(1) / obwLimit,
    ...
    public void setInputLimit(int ibwLimit) {
        requestInputThrottle.changeNanosAndBucketSize(SECONDS.toNanos(1) / ibwLimit,
    ...
    public void fillSuccessRateBox(HTMLNode parent) {
        ...
        final RunningAverage[] averages = new RunningAverage[] {
            globalFetchPSuccess, chkLocalFetchPSuccess, chkRemoteFetchPSuccess, sskLocalFetchPSuccess,
            sskRemoteFetchPSuccess, blockTransferPSuccessBulk, blockTransferPSuccessRT, blockTransferPSuccessLocal,
            blockTransferFailTimeout
        };
        final String[] names = new String[] {
            l10n("allRequests"), l10n("localCHKs"), l10n("remoteCHKs"), l10n("localSSKs"), l10n("remoteSSKs"),
            l10n("blockTransfersBulk"), l10n("blockTransfersRT"), l10n("blockTransfersLocal"), l10n("transfersTimedOut")
        };
    ...
    public synchronized void successfulBlockReceive(boolean realTimeFlag, boolean isLocal) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Successful receives: " + blockTransferPSuccess.currentValue() + " count="
    ...
    public void reportCHKOutcome(long rtt, boolean successful, double location, boolean isRealtime) {
        if (successful) {
            ...
            (isRealtime
             ? unsuccessfulLocalCHKFetchTimeAverageRT : unsuccessfulLocalCHKFetchTimeAverageBulk).report(rtt);
    ...
    public void reportSSKOutcome(long rtt, boolean successful, boolean isRealtime) {
        if (successful) {
            ...
            (isRealtime
             ? unsuccessfulLocalSSKFetchTimeAverageRT : unsuccessfulLocalSSKFetchTimeAverageBulk).report(rtt);
        }
    ...
    void remoteRequest(boolean ssk, boolean success, boolean local, short htl, double location, boolean realTime,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Remote request: sucess=" + success + " htl=" + htl + " locally answered=" + local
    ...
    public void reportAnnounceForwarded(int forwardedRefs, PeerNode source) {
        synchronized (this) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Announcements: " + totalAnnouncements + " average "
    ...
    public void drawNewLoadManagementDelayTimes(HTMLNode content) {
        ...
        content.addChild("p").addChild("#",
                         l10n("slotsWaiting", new String[] { "local", "remote" },
                              new String[] { Integer.toString(waitingSlots.local),
        ...
        synchronized (slotTimeoutsSync) {
            if (fatalTimeoutsInWaitLocal + fatalTimeoutsInWaitRemote + allocatedSlotLocal + allocatedSlotRemote > 0) {
                ...
                row = table.addChild("tr");
                row.addChild("td",
                             this.fix3p3pct.format(((double) fatalTimeoutsInWaitLocal)
                                                   / ((double) (fatalTimeoutsInWaitLocal + allocatedSlotLocal))));
                row.addChild("td",
                             this.fix3p3pct.format(((double) fatalTimeoutsInWaitRemote)
    ...
    public class PeerLoadStats {
        ...
        public PeerLoadStats(PeerNode peer, int transfersPerInsert, boolean realTimeFlag) {
            ...
            maxTransfersOutPeerLimit = (int) Math.max(1,
                    getPeerLimit(peer, maxTransfersOutUpperLimit - maxTransfersOutLowerLimit, false,
                                 transfersPerInsert, realTimeFlag, peers,
    ...
    class RunningRequestsSnapshot {
        ...
        public void log(PeerNode source) {
            String message = "Running (adjusted): CHK in: " + expectedTransfersInCHK + " out: "
                             ...
                             + expectedTransfersOutSSK + " total=" + totalRequests
