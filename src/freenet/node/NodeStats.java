...
//			defaultThreadLimit = 400;
	...
	public class PeerLoadStats {
		...
		public PeerLoadStats(PeerNode peer, boolean realTimeFlag, int transfersPerInsert) {
			this.peer = peer;
			...
			outputBandwidthPeerLimit = getPeerLimit(peer, outputBandwidthLowerLimit, false, realTimeFlag, true, transfersPerInsert);
			inputBandwidthPeerLimit = getPeerLimit(peer, inputBandwidthLowerLimit, true, realTimeFlag, true, transfersPerInsert);
	...
	public RejectReason shouldRejectRequest(boolean canAcceptAnyway, boolean isInsert, boolean isSSK, boolean isLocal, boolean isOfferReply, PeerNode source, boolean hasInStore, boolean preferInsert, boolean realTimeFlag) {
		...
		String ret = checkBandwidthLiability(getOutputBandwidthUpperLimit(totalSent, totalOverhead, uptime, limit, nonOverheadFraction), byteCountersSent, requestsSnapshot, false, limit,
				source, isLocal, isSSK, isInsert, isOfferReply, realTimeFlag, hasInStore, transfersPerInsert);  
	...
	private String checkBandwidthLiability(double bandwidthAvailableOutputUpperLimit,
			...
			PeerNode source, boolean isLocal, boolean isSSK, boolean isInsert, boolean isOfferReply, boolean realTimeFlag, boolean hasInStore, int transfersPerInsert) {
		...
		double thisAllocation = getPeerLimit(source, bandwidthAvailableOutputLowerLimit, input, realTimeFlag, false, transfersPerInsert);
		...
		if(bandwidthLiabilityOutput > bandwidthAvailableOutputLowerLimit) {
			...
			double peerUsedBytes = getPeerBandwidthLiability(source, isSSK, isInsert, isOfferReply, byteCountersSent, realTimeFlag, ignoreLocalVsRemoteBandwidthLiability, hasInStore, transfersPerInsert, input);
	...
	private double getPeerBandwidthLiability(PeerNode source, boolean isSSK, boolean isInsert, boolean isOfferReply, ByteCountersSnapshot byteCounters, boolean realTimeFlag, boolean ignoreLocalVsRemote, boolean hasInStore, int transfersOutPerInsert, boolean input) {
	...
	public PeerLoadStats createPeerLoadStats(PeerNode peer, boolean realTimeFlag, int transfersPerInsert) {
		return new PeerLoadStats(peer, realTimeFlag, transfersPerInsert);
	}
	...
	public RunningRequestsSnapshot getRunningRequestsTo(PeerNode peerNode, boolean realTimeFlag, int transfersPerInsert) {
		return new RunningRequestsSnapshot(node, peerNode, true, realTimeFlag, false, outwardTransfersPerInsert());
	}
