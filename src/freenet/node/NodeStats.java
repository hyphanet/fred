...
public class NodeStats implements Persistable, BlockTimeCallback {
	...
	public static final long DEFAULT_SUB_MAX_PING_TIME = MILLISECONDS.toMillis(
		700
	);
	/** Maximum overall average ping time. If ping is greater than this,
	 * we reject all requests. */
	public static final long DEFAULT_MAX_PING_TIME = MILLISECONDS.toMillis(
		1500
	);
	...
	private final RequestsByLocation incomingRequests = new RequestsByLocation(
		10
	);
	/** Locations of outgoing requests */
	private final RequestsByLocation outgoingLocalRequests =
		new RequestsByLocation(10);
	private final RequestsByLocation outgoingRequests = new RequestsByLocation(
		10
	);
	...
	NodeStats(
		...
		this.routingMissDistanceLocal = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.routingMissDistanceRemote = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.routingMissDistanceOverall = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.routingMissDistanceBulk = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.routingMissDistanceRT = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.backedOffPercent = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		pInstantRejectIncomingOverall = new BootstrappingDecayingRunningAverage(
			0.0,
			0.0,
			1.0,
			1000,
		...
		throttledPacketSendAverage = new BootstrappingDecayingRunningAverage(
			0,
			0,
			Long.MAX_VALUE,
			100,
		...
		throttledPacketSendAverageRT = new BootstrappingDecayingRunningAverage(
			0,
			0,
			Long.MAX_VALUE,
			100,
		...
		if (memoryLimit > 0 && memoryLimit < 100) {
			...
			Logger.minor(
				this,
				"Severe memory pressure, setting 200 thread limit. Freenet may not work well!"
			);
			...
			Logger.minor(
				this,
				"Setting standard 1000 thread limit. This should be enough for most nodes."
			);
		}
		statsConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					if (val < 100) throw new InvalidConfigValueException(
						l10n("valueTooLow")
					);
					threadLimit = val;
				}
		...
		statsConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					Logger.normal(
						this,
						"Changing aggressiveGCModificator to " + val
					);
		...
		ignoreLocalVsRemoteBandwidthLiability = statsConfig.getBoolean(
			"ignoreLocalVsRemoteBandwidthLiability"
		);
		...
		node.securityLevels.addNetworkThreatLevelListener(
			new SecurityLevelListener<NETWORK_THREAT_LEVEL>() {
				...
				public void onChange(
					...
					if (
						oldLevel == NETWORK_THREAT_LEVEL.MAXIMUM
		...
		localChkFetchBytesSentAverage = new TimeDecayingRunningAverage(
			500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalChkFetchBytesSentAverage"),
			node
		);
		localSskFetchBytesSentAverage = new TimeDecayingRunningAverage(
			500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalSskFetchBytesSentAverage"),
			node
		);
		localChkInsertBytesSentAverage = new TimeDecayingRunningAverage(
			32768,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalChkInsertBytesSentAverage"),
			node
		);
		localSskInsertBytesSentAverage = new TimeDecayingRunningAverage(
			2048,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalSskInsertBytesSentAverage"),
			node
		);
		localChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
			...
			throttleFS == null
				? null
				: throttleFS.subset("LocalChkFetchBytesReceivedAverage"),
			node
		);
		localSskFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
			2048,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalSskFetchBytesReceivedAverage"),
			node
		);
		localChkInsertBytesReceivedAverage = new TimeDecayingRunningAverage(
			1024,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("LocalChkInsertBytesReceivedAverage"),
			node
		);
		localSskInsertBytesReceivedAverage = new TimeDecayingRunningAverage(
			500,
			180000,
			0.0,
			200 * 1024,
		...
		remoteSskFetchBytesSentAverage = new TimeDecayingRunningAverage(
			1024 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
		...
		remoteChkInsertBytesSentAverage = new TimeDecayingRunningAverage(
			32768 + 32768 + 1024,
			180000,
			0.0,
			200 * 1024,
		...
		remoteSskInsertBytesSentAverage = new TimeDecayingRunningAverage(
			1024 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
		...
		remoteSskFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
			2048 + 500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("RemoteSskFetchBytesReceivedAverage"),
			node
		);
		remoteChkInsertBytesReceivedAverage = new TimeDecayingRunningAverage(
			32768 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
		...
		remoteSskInsertBytesReceivedAverage = new TimeDecayingRunningAverage(
			1024 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
		...
		successfulChkFetchBytesSentAverage = new TimeDecayingRunningAverage(
			...
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulChkFetchBytesSentAverage"),
			node
		);
		successfulSskFetchBytesSentAverage = new TimeDecayingRunningAverage(
			1024 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulSskFetchBytesSentAverage"),
			node
		);
		successfulChkInsertBytesSentAverage = new TimeDecayingRunningAverage(
			32768 + 32768 + 1024,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulChkInsertBytesSentAverage"),
			node
		);
		successfulSskInsertBytesSentAverage = new TimeDecayingRunningAverage(
			1024 + 1024 + 500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulSskInsertBytesSentAverage"),
			node
		);
		...
		successfulChkFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
			...
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulChkFetchBytesReceivedAverage"),
			node
		);
		successfulSskFetchBytesReceivedAverage = new TimeDecayingRunningAverage(
			2048 + 500,
			180000,
			0.0,
			200 * 1024,
			throttleFS == null
				? null
				: throttleFS.subset("SuccessfulSskFetchBytesReceivedAverage"),
			node
		);
		...
		this.avgCacheCHKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageCacheCHKLocation")
		);
		this.avgStoreCHKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageStoreCHKLocation")
		);
		this.avgSlashdotCacheCHKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageSlashdotCacheCHKLocation")
		);
		this.avgClientCacheCHKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageClientCacheCHKLocation")
		);

		this.avgCacheCHKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageCacheCHKSuccessLocation")
		);
		this.avgSlashdotCacheCHKSucess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
		...
		this.avgClientCacheCHKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageClientCacheCHKSuccessLocation")
		);
		this.avgStoreCHKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageStoreCHKSuccessLocation")
		);
		this.avgRequestLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageRequestLocation")
		);

		this.avgCacheSSKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageCacheSSKLocation")
		);
		this.avgStoreSSKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageStoreSSKLocation")
		);
		this.avgSlashdotCacheSSKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageSlashdotCacheSSKLocation")
		);
		this.avgClientCacheSSKLocation = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageClientCacheSSKLocation")
		);

		this.avgCacheSSKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageCacheSSKSuccessLocation")
		);
		this.avgSlashdotCacheSSKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
		...
		this.avgClientCacheSSKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageClientCacheSSKSuccessLocation")
		);
		this.avgStoreSSKSuccess = new DecayingKeyspaceAverage(
			nodeLoc,
			10000,
			throttleFS == null
				? null
				: throttleFS.subset("AverageStoreSSKSuccessLocation")
		);
	...
	protected String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("NodeStats." + key, patterns, values);
	}
	...
	public class PeerLoadStats {
		...
		public boolean equals(Object o) {
			...
			if (
				s.expectedTransfersOutCHK != expectedTransfersOutCHK
			) return false;
			if (
				s.expectedTransfersInCHK != expectedTransfersInCHK
			) return false;
			if (
				s.expectedTransfersOutSSK != expectedTransfersOutSSK
			) return false;
			if (
				s.expectedTransfersInSSK != expectedTransfersInSSK
			) return false;
			if (s.totalRequests != totalRequests) return false;
			if (
				s.averageTransfersOutPerInsert != averageTransfersOutPerInsert
			) return false;
			if (
				s.outputBandwidthLowerLimit != outputBandwidthLowerLimit
			) return false;
			if (
				s.outputBandwidthUpperLimit != outputBandwidthUpperLimit
			) return false;
			if (
				s.outputBandwidthPeerLimit != outputBandwidthPeerLimit
			) return false;
			if (
				s.inputBandwidthLowerLimit != inputBandwidthLowerLimit
			) return false;
			if (
				s.inputBandwidthUpperLimit != inputBandwidthUpperLimit
			) return false;
			if (
				s.inputBandwidthPeerLimit != inputBandwidthPeerLimit
			) return false;
			if (s.maxTransfersOut != maxTransfersOut) return false;
			if (
				s.maxTransfersOutPeerLimit != maxTransfersOutPeerLimit
			) return false;
			if (
				s.maxTransfersOutLowerLimit != maxTransfersOutLowerLimit
			) return false;
			if (
				s.maxTransfersOutUpperLimit != maxTransfersOutUpperLimit
			) return false;
			return true;
		}
		...
		public PeerLoadStats(
			...
			RunningRequestsSnapshot runningLocal = new RunningRequestsSnapshot(
				node.tracker,
				peer,
				false,
				ignoreLocalVsRemote,
			...
			outputBandwidthUpperLimit = getOutputBandwidthUpperLimit(
				limit,
				nonOverheadFraction
			);
			outputBandwidthLowerLimit = getLowerLimit(
				outputBandwidthUpperLimit,
				peers
			);

			inputBandwidthUpperLimit = getInputBandwidthUpperLimit(limit);
			inputBandwidthLowerLimit = getLowerLimit(
				inputBandwidthUpperLimit,
				peers
			);

			maxTransfersOutUpperLimit = getMaxTransfersUpperLimit(
				realTimeFlag,
				nonOverheadFraction
			);
			maxTransfersOutLowerLimit = (int) Math.max(
				1,
				getLowerLimit(maxTransfersOutUpperLimit, peers)
			);
			maxTransfersOutPeerLimit = (int) Math.max(
				1,
				getPeerLimit(
					peer,
			...
			maxTransfersOut = calculateMaxTransfersOut(
				peer,
				realTime,
				nonOverheadFraction,
				maxTransfersOutUpperLimit
			...
			outputBandwidthPeerLimit = getPeerLimit(
				peer,
			...
			inputBandwidthPeerLimit = getPeerLimit(
				peer,
			...
			RunningRequestsSnapshot runningGlobal = new RunningRequestsSnapshot(
				node.tracker,
				ignoreLocalVsRemote,
				transfersPerInsert,
				realTimeFlag
			);
			expectedTransfersInCHK = runningGlobal.expectedTransfersInCHK -
			runningLocal.expectedTransfersInCHK;
			expectedTransfersInSSK = runningGlobal.expectedTransfersInSSK -
			runningLocal.expectedTransfersInSSK;
			...
			totalRequests = runningGlobal.totalRequests -
			runningLocal.totalRequests;
		}
		...
		public PeerLoadStats(PeerNode source, Message m) {
			...
			if (m.getSpec() == DMT.FNPPeerLoadStatusInt) {
				...
				averageTransfersOutPerInsert = m.getInt(
					DMT.AVERAGE_TRANSFERS_OUT_PER_INSERT
				);
				maxTransfersOut = m.getInt(DMT.MAX_TRANSFERS_OUT);
				maxTransfersOutUpperLimit = m.getInt(
					DMT.MAX_TRANSFERS_OUT_UPPER_LIMIT
				);
				maxTransfersOutLowerLimit = m.getInt(
					DMT.MAX_TRANSFERS_OUT_LOWER_LIMIT
				);
				maxTransfersOutPeerLimit = m.getInt(
					DMT.MAX_TRANSFERS_OUT_PEER_LIMIT
				);
			} else if (m.getSpec() == DMT.FNPPeerLoadStatusShort) {
				expectedTransfersInCHK = m.getShort(
					DMT.OTHER_TRANSFERS_IN_CHK
				) &
				0xFFFF;
				expectedTransfersInSSK = m.getShort(
					DMT.OTHER_TRANSFERS_IN_SSK
				) &
				0xFFFF;
				expectedTransfersOutCHK = m.getShort(
					DMT.OTHER_TRANSFERS_OUT_CHK
				) &
				0xFFFF;
				expectedTransfersOutSSK = m.getShort(
					DMT.OTHER_TRANSFERS_OUT_SSK
				) &
				0xFFFF;
				averageTransfersOutPerInsert = m.getShort(
					DMT.AVERAGE_TRANSFERS_OUT_PER_INSERT
				) &
				0xFFFF;
				maxTransfersOut = m.getShort(DMT.MAX_TRANSFERS_OUT) & 0xFFFF;
				maxTransfersOutUpperLimit = m.getShort(
					DMT.MAX_TRANSFERS_OUT_UPPER_LIMIT
				) &
				0xFFFF;
				maxTransfersOutLowerLimit = m.getShort(
					DMT.MAX_TRANSFERS_OUT_LOWER_LIMIT
				) &
				0xFFFF;
				maxTransfersOutPeerLimit = m.getShort(
					DMT.MAX_TRANSFERS_OUT_PEER_LIMIT
				) &
				0xFFFF;
			} else if (m.getSpec() == DMT.FNPPeerLoadStatusByte) {
				expectedTransfersInCHK = m.getByte(DMT.OTHER_TRANSFERS_IN_CHK) &
				0xFF;
				expectedTransfersInSSK = m.getByte(DMT.OTHER_TRANSFERS_IN_SSK) &
				0xFF;
				expectedTransfersOutCHK = m.getByte(
					DMT.OTHER_TRANSFERS_OUT_CHK
				) &
				0xFF;
				expectedTransfersOutSSK = m.getByte(
					DMT.OTHER_TRANSFERS_OUT_SSK
				) &
				0xFF;
				averageTransfersOutPerInsert = m.getByte(
					DMT.AVERAGE_TRANSFERS_OUT_PER_INSERT
				) &
				0xFF;
				maxTransfersOut = m.getByte(DMT.MAX_TRANSFERS_OUT) & 0xFF;
				maxTransfersOutUpperLimit = m.getByte(
					DMT.MAX_TRANSFERS_OUT_UPPER_LIMIT
				) &
				0xFF;
				maxTransfersOutLowerLimit = m.getByte(
					DMT.MAX_TRANSFERS_OUT_LOWER_LIMIT
				) &
				0xFF;
				maxTransfersOutPeerLimit = m.getByte(
					DMT.MAX_TRANSFERS_OUT_PEER_LIMIT
				) &
				0xFF;
			} else throw new IllegalArgumentException();
			outputBandwidthLowerLimit = m.getInt(
				DMT.OUTPUT_BANDWIDTH_LOWER_LIMIT
			);
			outputBandwidthUpperLimit = m.getInt(
				DMT.OUTPUT_BANDWIDTH_UPPER_LIMIT
			);
			outputBandwidthPeerLimit = m.getInt(
				DMT.OUTPUT_BANDWIDTH_PEER_LIMIT
			);
			inputBandwidthLowerLimit = m.getInt(
				DMT.INPUT_BANDWIDTH_LOWER_LIMIT
			);
			inputBandwidthUpperLimit = m.getInt(
				DMT.INPUT_BANDWIDTH_UPPER_LIMIT
			);
	...
	class RunningRequestsSnapshot {
		...
		RunningRequestsSnapshot(
			...
			if (!requestsToNode) {
				this.expectedTransfersInCHKSR =
					countCHKSR.expectedTransfersIn();
				this.expectedTransfersInSSKSR =
					countSSKSR.expectedTransfersIn();
				this.expectedTransfersOutCHKSR =
					countCHKSR.expectedTransfersOut();
				this.expectedTransfersOutSSKSR =
					countSSKSR.expectedTransfersOut();
				this.totalRequestsSR = countCHKSR.total() + countSSKSR.total();
				this.expectedTransfersInCHK = countCHK.expectedTransfersIn() -
				expectedTransfersInCHKSR;
				this.expectedTransfersInSSK = countSSK.expectedTransfersIn() -
				expectedTransfersInSSKSR;
				this.expectedTransfersOutCHK = countCHK.expectedTransfersOut() -
				expectedTransfersOutCHKSR;
				this.expectedTransfersOutSSK = countSSK.expectedTransfersOut() -
				expectedTransfersOutSSKSR;
				this.totalRequests = (countCHK.total() + countSSK.total()) -
				totalRequestsSR;
			} else {
				this.expectedTransfersInCHK = countCHK.expectedTransfersIn();
		...
		public double calculate(
			...
			if (input) return (
				this.expectedTransfersInCHK * (32768 + 256) +
				...
				this.expectedTransfersOutCHK * (32768 + 256) +
		...
		public double calculateSR(
			...
			if (input) return (
				this.expectedTransfersInCHKSR * (32768 + 256) +
				...
				this.expectedTransfersOutCHKSR * (32768 + 256) +
	...
	public RejectReason shouldRejectRequest(
		...
		synchronized (serializeShouldRejectRequest) {
			...
			if (source != null) {
				if (source.isDisconnecting()) return new RejectReason(
					"disconnecting",
					false
				);
			}
			...
			if (threadLimit < threadCount) {
				...
				return new RejectReason(
					">threadLimit (" + threadCount + '/' + threadLimit + ')',
					false
				);
			}
			// FIXME slowdown
			...
			synchronized (this) {
				...
				if (pingTime > maxPingTime) {
					if (
						(now - lastAcceptedRequest > MAX_INTERREQUEST_TIME) &&
						canAcceptAnyway
					) {
					...
					double x =
						((pingTime - subMaxPingTime)) /
						(maxPingTime - subMaxPingTime);
			...
			int peers =
				...
				2 * node.peers.countConnectedDarknetPeers();
			...
			int maxTransfersOutUpperLimit = getMaxTransfersUpperLimit(
				realTimeFlag,
				nonOverheadFraction
			);

			int maxTransfersOutLowerLimit = (int) Math.max(
				1,
				getLowerLimit(maxTransfersOutUpperLimit, peers)
			);
			int maxTransfersOutPeerLimit = (int) Math.max(
				1,
				getPeerLimit(
					source,
					maxTransfersOutUpperLimit - maxTransfersOutLowerLimit,
			...
			String ret = checkBandwidthLiability(
			...
			if (source != null) {
				if (
					source.getMessageQueueLengthBytes() > MAX_PEER_QUEUE_BYTES
				) {
					rejected(
					...
					return new RejectReason(
						"Too many message bytes queued for peer",
						false
					);
					// FIXME slowDown?
				}
				if (source.getProbableSendQueueTime() > MAX_PEER_QUEUE_TIME) {
					...
					return new RejectReason(
						"Peer's queue will take too long to transfer",
						false
					);
					// FIXME slowDown?
				}
	...
	public int calculateMaxTransfersOut(
		...
		if (peer == null) return Integer.MAX_VALUE;
			...
			maxTransfersOutUpperLimit,
			peer.calculateMaxTransfersOut(
				getAcceptableBlockTime(realTime),
				nonOverheadFraction
			)
		);
	}
	...
	private double getNonOverheadFraction(long now) {
		...
		double totalCouldSend = Math.max(
			totalSent,
			(((node.getOutputBandwidthLimit() * uptime)) / 1000.0)
		);
		double nonOverheadFraction =
			(totalCouldSend - totalOverhead) / totalCouldSend;
		...
		if (timeFirstAnyConnections > 0) {
			...
			if (time < DEFAULT_ONLY_PERIOD) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Adjusted non-overhead fraction: " + nonOverheadFraction
				);
				...
				nonOverheadFraction = (time * nonOverheadFraction +
					(DEFAULT_TRANSITION_PERIOD - time) * DEFAULT_OVERHEAD) /
	...
	private double getOutputBandwidthUpperLimit(
		long limit,
		double nonOverheadFraction
	) {
		double outputAvailablePerSecond =
			node.getOutputBandwidthLimit() * nonOverheadFraction;
		return outputAvailablePerSecond * limit;
	}

	private int getMaxTransfersUpperLimit(
		boolean realTime,
		double nonOverheadFraction
	) {
		...
		return (int) Math.max(
			1,
			(getAcceptableBlockTime(realTime) * outputAvailablePerSecond) /
			1024.0
		);
	}
	...
	private String checkBandwidthLiability(
		...
		double bandwidthAvailableOutputLowerLimit = getLowerLimit(
			bandwidthAvailableOutputUpperLimit,
			peers
		);

		double bandwidthLiabilityOutput = requestsSnapshot.calculate(
			ignoreLocalVsRemoteBandwidthLiability,
			input
		);
		...
		double thisAllocation = getPeerLimit(
			source,
			bandwidthAvailableOutputUpperLimit -
			...
			realTimeFlag,
			peers,
			peerRequestsSnapshot.calculateSR(
				ignoreLocalVsRemoteBandwidthLiability,
				input
			)
		);
		...
		if (SEND_LOAD_STATS_NOTICES && source != null) {
			...
			if (!input) {
				source.onSetMaxOutputTransfers(
					realTimeFlag,
					maxOutputTransfers
				);
				source.onSetMaxOutputTransfersPeerLimit(
					realTimeFlag,
					maxOutputTransfersPeerLimit
				);
			}
		...
		if (bandwidthLiabilityOutput > bandwidthAvailableOutputLowerLimit) {
			...
			double peerUsedBytes = getPeerBandwidthLiability(
				peerRequestsSnapshot,
				source,
				isSSK,
				transfersPerInsert,
	...
	private String checkMaxOutputTransfers(
		...
		if (peerOutTransfers > maxOutputTransfers && !isLocal) {
			...
			rejected(
				"TooManyTransfers: Congestion control",
				isLocal,
				isInsert,
				isSSK,
				isOfferReply,
	...
	private double getPeerLimit(
		...
		if (logMINOR && sourceRestarted != 0) Logger.minor(
			this,
			"Allocation is " +
			thisAllocation +
			" source restarted is " +
	...
	private double getPeerBandwidthLiability(
		...
		return requestsSnapshot.calculate(
			ignoreLocalVsRemoteBandwidthLiability,
			input
		);
	}
	...
	public void maybeUpdatePeerManagerUserAlertStats(long now) {
		if (now > nextPeerManagerUserAlertStatsUpdateTime) {
			if (
				getBwlimitDelayTime() > MAX_BWLIMIT_DELAY_TIME_ALERT_THRESHOLD
			) {
	...
	public SimpleFieldSet persistThrottlesToFieldSet() {
		...
		fs.put(
			"AverageCacheCHKLocation",
			avgCacheCHKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageStoreCHKLocation",
			avgStoreCHKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageSlashdotCacheCHKLocation",
			avgSlashdotCacheCHKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageClientCacheCHKLocation",
			avgClientCacheCHKLocation.exportFieldSet(true)
		);

		fs.put(
			"AverageCacheCHKSuccessLocation",
			avgCacheCHKSuccess.exportFieldSet(true)
		);
		fs.put(
			"AverageSlashdotCacheCHKSuccessLocation",
			avgSlashdotCacheCHKSucess.exportFieldSet(true)
		);
		fs.put(
			"AverageClientCacheCHKSuccessLocation",
			avgClientCacheCHKSuccess.exportFieldSet(true)
		);
		fs.put(
			"AverageStoreCHKSuccessLocation",
			avgStoreCHKSuccess.exportFieldSet(true)
		);

		fs.put(
			"AverageCacheSSKLocation",
			avgCacheSSKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageStoreSSKLocation",
			avgStoreSSKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageSlashdotCacheSSKLocation",
			avgSlashdotCacheSSKLocation.exportFieldSet(true)
		);
		fs.put(
			"AverageClientCacheSSKLocation",
			avgClientCacheSSKLocation.exportFieldSet(true)
		);

		fs.put(
			"AverageCacheSSKSuccessLocation",
			avgCacheSSKSuccess.exportFieldSet(true)
		);
		fs.put(
			"AverageSlashdotCacheSSKSuccessLocation",
			avgSlashdotCacheSSKSuccess.exportFieldSet(true)
		);
		fs.put(
			"AverageClientCacheSSKSuccessLocation",
			avgClientCacheSSKSuccess.exportFieldSet(true)
		);
		fs.put(
			"AverageStoreSSKSuccessLocation",
			avgStoreSSKSuccess.exportFieldSet(true)
		);

		fs.put(
			"AverageRequestLocation",
			avgRequestLocation.exportFieldSet(true)
		);
	...
	public void maybeUpdateNodeIOStats(long now) {
		if (now > nextNodeIOStatsUpdateTime) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Last 2 seconds: input: " + indiff + " output: " + outdiff
			);
	...
	public SimpleFieldSet exportVolatileFieldSet() {
		...
		for (int t = 1; t < 7; t++) {
			...
			fs.put(
				"opennetSizeEstimate" + hour + "hourRecent",
				getOpennetSizeEstimate(limit)
			);
			fs.put(
				"networkSizeEstimate" + hour + "hourRecent",
				getDarknetSizeEstimate(limit)
			);
		}
		fs.put(
			"routingMissDistanceLocal",
			routingMissDistanceLocal.currentValue()
		);
		fs.put(
			"routingMissDistanceRemote",
			routingMissDistanceRemote.currentValue()
		);
		fs.put(
			"routingMissDistanceOverall",
			routingMissDistanceOverall.currentValue()
		);
		fs.put(
			"routingMissDistanceBulk",
			routingMissDistanceBulk.currentValue()
		);
		...
		fs.put(
			"RAMBucketPoolSize",
			node.clientCore.tempBucketFactory.getRamUsed()
		);
		...
		int numberOfConnected = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_CONNECTED
		);
		int numberOfRoutingBackedOff = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_ROUTING_BACKED_OFF
		);
		int numberOfTooNew = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_TOO_NEW
		);
		int numberOfTooOld = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_TOO_OLD
		);
		int numberOfDisconnected = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_DISCONNECTED
		);
		int numberOfNeverConnected = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED
		);
		int numberOfDisabled = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_DISABLED
		);
		int numberOfBursting = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_BURSTING
		);
		int numberOfListening = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_LISTENING
		);
		int numberOfListenOnly = PeerNodeStatus.getPeerStatusCount(
			peerNodeStatuses,
			PeerManager.PEER_NODE_STATUS_LISTEN_ONLY
		);

		int numberOfSimpleConnected =
		...
		fs.put(
			...
			node.tracker.getNumTransferringRequestSenders()
		);
		...
		fs.put(
			...
			node.tracker.getNumTransferringRequestHandlers()
		);
		...
		fs.put(
			...
			successfulLocalCHKFetchTimeAverageBulk.currentValue()
		);
		fs.put(
			"successfulLocalCHKFetchTimeRT",
			successfulLocalCHKFetchTimeAverageRT.currentValue()
		);
		fs.put(
		...
		fs.put(
			...
			unsuccessfulLocalCHKFetchTimeAverageRT.currentValue()
		);

		fs.put(
			"successfulLocalSSKFetchTimeBulk",
			successfulLocalSSKFetchTimeAverageBulk.currentValue()
		);
		fs.put(
			"successfulLocalSSKFetchTimeRT",
			successfulLocalSSKFetchTimeAverageRT.currentValue()
		);
		fs.put(
		...
		fs.put(
			...
			unsuccessfulLocalSSKFetchTimeAverageRT.currentValue()
		);
		...
		int total_payload_output_percent = (total[0] == 0)
			? -1
			: (int) ((100 * totalPayloadOutput) / total[0]);
		...
		double recent_output_rate = deltaMS == 0
			? 0
			: ((1000.0 * (rate[3] - rate[0])) / deltaMS);
		double recent_input_rate = deltaMS == 0
			? 0
			: ((1000.0 * (rate[4] - rate[1])) / deltaMS);
		...
		if (routingBackoffReasons.length != 0) {
			for (String routingBackoffReason : routingBackoffReasons) {
				fs.put(
					...
					peers.getPeerNodeRoutingBackoffReasonSize(
						routingBackoffReason,
						true
					)
				);
			}
		...
		if (routingBackoffReasons.length != 0) {
			for (String routingBackoffReason : routingBackoffReasons) {
				fs.put(
					"numberWithRoutingBackoffReasonsBulk." +
					routingBackoffReason,
					peers.getPeerNodeRoutingBackoffReasonSize(
						routingBackoffReason,
						false
		...
		fs.put(
			"numberOfRemotePeerLocationsSeenInSwaps",
			numberOfRemotePeerLocationsSeenInSwaps
		);
		double avgConnectedPeersPerNode = 0.0;
		if (
			(numberOfRemotePeerLocationsSeenInSwaps > 0.0) &&
			((swaps > 0.0) || (noSwaps > 0.0))
		) {
			avgConnectedPeersPerNode = numberOfRemotePeerLocationsSeenInSwaps /
			(swaps + noSwaps);
		}
		...
		long cachedStoreFalsePositives = node
			.getChkDatacache()
		...
		long storeFalsePositives = node
			.getChkDatastore()
			.getBloomFalsePositive();
		long storeAccesses = storeHits + storeMisses;
		double percentStoreHitsOfAccesses =
			(double) (storeHits * 100) / (double) storeAccesses;
		...
		fs.put(
			...
			blockTransferPSuccessRT.currentValue()
		);
		fs.put("blockTransferCountRT", blockTransferPSuccessRT.countReports());
		fs.put(
			"blockTransferPSuccessBulk",
			blockTransferPSuccessBulk.currentValue()
		);
		fs.put(
			"blockTransferCountBulk",
			blockTransferPSuccessBulk.countReports()
		);
		fs.put(
			"blockTransferFailTimeout",
			blockTransferFailTimeout.currentValue()
		);
	...
	public void fillSuccessRateBox(HTMLNode parent) {
		...
		for (int i = 0; i < averages.length; i++) {
			...
			if (averages[i].countReports() == 0) {
				...
				row.addChild(
					"td",
					thousandPoint.format(averages[i].countReports())
				);
			}
		}
		...
		row.addChild(
			"td",
			fix3p3pct.format(
				((double) bulkSuccess[1]) / ((double) bulkSuccess[0])
			)
		);
	...
	public final ByteCounter resendByteCounter = new ByteCounter() {
		...
		public void sentPayload(int x) {
			Logger.error(
				this,
				"Payload sent in resendByteCounter????",
				new Exception("error")
			);
		}
	};
	...
	ByteCounter setRoutingStatusCtr = new ByteCounter() {
		...
		public void receivedBytes(int x) {
			...
			Logger.error(
				this,
				"Routing status sender received bytes: " +
				x +
	...
	public synchronized void successfulBlockReceive(
		...
		RunningAverage blockTransferPSuccess = realTimeFlag
			? blockTransferPSuccessRT
			: blockTransferPSuccessBulk;
	...
	public void reportCHKOutcome(
		...
		if (successful) {
			(isRealtime
					? successfulLocalCHKFetchTimeAverageRT
					: successfulLocalCHKFetchTimeAverageBulk).report(rtt);
		...
		(isRealtime
				? localCHKFetchTimeAverageRT
				: localCHKFetchTimeAverageBulk).report(rtt);
	}

	public void reportSSKOutcome(
		long rtt,
		boolean successful,
		boolean isRealtime
	) {
		if (successful) {
			(isRealtime
					? successfulLocalSSKFetchTimeAverageRT
					: successfulLocalSSKFetchTimeAverageBulk).report(rtt);
		} else {
			(isRealtime
		...
		(isRealtime
				? localSSKFetchTimeAverageRT
				: localSSKFetchTimeAverageBulk).report(rtt);
	}
	...
	public void fillDetailedTimingsBox(HTMLNode html) {
		...
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) successfulLocalCHKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) successfulLocalCHKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) successfulLocalSSKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) successfulLocalSSKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
		...
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) unsuccessfulLocalCHKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) unsuccessfulLocalCHKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) unsuccessfulLocalSSKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) unsuccessfulLocalSSKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
		...
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) localCHKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) localCHKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) localSSKFetchTimeAverageBulk.currentValue(),
				2,
				true
			)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(long) localSSKFetchTimeAverageRT.currentValue(),
				2,
				true
			)
		);
	...
	void remoteRequest(
		...
		if (!fromOfferedKey) {
			if (realTime) hourlyStatsRT.remoteRequest(
				ssk,
				success,
				local,
				htl,
			...
			else hourlyStatsBulk.remoteRequest(
				ssk,
				success,
				local,
				htl,
	...
	private String sanitizeDBJobType(String jobType) {
		...
		if (typeBeginIndex < 0) typeBeginIndex = jobType.lastIndexOf(':'); // Strip "DBJobWrapper:" prefix
		...
		return new StoreLocationStats() {
			...
			public double distanceStats() throws StatsNotAvailableException {
				return cappedDistance(
					...
					node.getSskClientCache()
				);
			}
		};
	}

	private double cappedDistance(
		DecayingKeyspaceAverage avgLocation,
		StoreCallback<?> store
	) {
		double cachePercent =
			(1.0 * avgLocation.countReports()) / store.keyCount();
	...
	public TimedStats[] getMandatoryBackoffStatistics(boolean realtime) {
		if (realtime) {
			TimedStats[] entries =
				new TimedStats[avgMandatoryBackoffTimesRT.size()];
			int i = 0;
			...
			synchronized (avgMandatoryBackoffTimesRT) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgMandatoryBackoffTimesRT.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
			...
			TimedStats[] entries =
				new TimedStats[avgMandatoryBackoffTimesBulk.size()];
			int i = 0;
			...
			synchronized (avgMandatoryBackoffTimesBulk) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgMandatoryBackoffTimesBulk.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
	...
	public TimedStats[] getRoutingBackoffStatistics(boolean realtime) {
		if (realtime) {
			TimedStats[] entries =
				new TimedStats[avgRoutingBackoffTimesRT.size()];
			int i = 0;
			...
			synchronized (avgRoutingBackoffTimesRT) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgRoutingBackoffTimesRT.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
			...
			TimedStats[] entries =
				new TimedStats[avgRoutingBackoffTimesBulk.size()];
			int i = 0;
			...
			synchronized (avgRoutingBackoffTimesBulk) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgRoutingBackoffTimesBulk.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
	...
	public TimedStats[] getTransferBackoffStatistics(boolean realtime) {
		if (realtime) {
			TimedStats[] entries =
				new TimedStats[avgTransferBackoffTimesRT.size()];
			int i = 0;
			...
			synchronized (avgTransferBackoffTimesRT) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgTransferBackoffTimesRT.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
			...
			TimedStats[] entries =
				new TimedStats[avgTransferBackoffTimesBulk.size()];
			int i = 0;
			...
			synchronized (avgTransferBackoffTimesBulk) {
				for (Map.Entry<
					String,
					TrivialRunningAverage
				> entry : avgTransferBackoffTimesBulk.entrySet()) {
					TrivialRunningAverage avg = entry.getValue();
					entries[i++] = new TimedStats(
						entry.getKey(),
						avg.countReports(),
	...
	public TimedStats[] getDatabaseJobExecutionStatistics() {
		TimedStats[] entries =
			new TimedStats[avgDatabaseJobExecutionTimes.size()];
		int i = 0;
		...
		synchronized (avgDatabaseJobExecutionTimes) {
			for (Map.Entry<
				String,
				TrivialRunningAverage
			> entry : avgDatabaseJobExecutionTimes.entrySet()) {
				TrivialRunningAverage avg = entry.getValue();
				entries[i++] = new TimedStats(
					entry.getKey(),
					avg.countReports(),
	...
	public void reportAnnounceForwarded(int forwardedRefs, PeerNode source) {
		...
		) om.seedTracker.completedAnnounce(
	...
	public synchronized int getTransfersPerAnnounce() {
		...
		return (int) Math.max(
			1,
			Math.ceil((totalAnnounceForwards * 1.0) / totalAnnouncements)
		);
	}
	...
	public AnnouncementDecision shouldAcceptAnnouncement(long uid) {
		...
		synchronized (this) {
			...
			if (running >= MAX_ANNOUNCEMENTS) {
				if (logMINOR) Logger.minor(
					this,
					"Too many announcements running: " + running
				);
			...
			if (
				perTransfer * transfersPerAnnouncement * running >
				bandwidthIn30Secs
			) {
				if (logMINOR) Logger.minor(
					this,
					"Can't complete " + running + " announcements in 30 secs"
				);
				return AnnouncementDecision.OVERLOAD;
			}
			...
			return (
				ret ? AnnouncementDecision.ACCEPT : AnnouncementDecision.LOOP
			);
		}
	}
	...
	public void drawNewLoadManagementDelayTimes(HTMLNode content) {
		WaitingForSlots waitingSlots =
			node.tracker.countRequestsWaitingForSlots();
		content
			.addChild("p")
			.addChild(
				"#",
				l10n(
					...
					new String[] {
						Integer.toString(waitingSlots.local),
						Integer.toString(waitingSlots.remote),
					}
				)
			);
		...
		row.addChild(
			"td",
			TimeUtil.formatTime((int) nlmDelayRTLocal.currentValue(), 2, true)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime((int) nlmDelayRTRemote.currentValue(), 2, true)
		);
		row = table.addChild("tr");
		row.addChild("th", l10n("bulkHeader"));
		row.addChild(
			"td",
			TimeUtil.formatTime((int) nlmDelayBulkLocal.currentValue(), 2, true)
		);
		row.addChild(
			"td",
			TimeUtil.formatTime(
				(int) nlmDelayBulkRemote.currentValue(),
				2,
				true
			)
		);
				...
				synchronized (noisyRejectStats) { // Only used for accessing the bytes.
					for (int i = 0; i < REJECT_STATS_AVERAGERS.length; i++) {
						...
						if (r.countReports() < minReportsNoisyRejectStats) {
							...
							if (rejectStatsFuzz > 0) noisy = randomNoise(
								noisy,
								rejectStatsFuzz
							);
							if (noisy < 0) result = 0;
	...
	public final double getBandwidthLiabilityUsage() {
		...
		int transfersPerInsert = outwardTransfersPerInsert();
		RunningRequestsSnapshot requestsSnapshot = new RunningRequestsSnapshot(
			node.tracker,
			ignoreLocalVsRemoteBandwidthLiability,
			transfersPerInsert,
			false
		);
		double usedBytes = requestsSnapshot.calculate(
			ignoreLocalVsRemoteBandwidthLiability,
			false
		);
		double nonOverheadFraction = getNonOverheadFraction(now);
		double upperLimit = getOutputBandwidthUpperLimit(
			limit,
			nonOverheadFraction
		);
