...
public class LocationManager implements ByteCounter {
        ...
        node.executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    bw.write("" + df.format(new Date()) + " : " + getLocation()
                             + (randomReset
        ...
        synchronized (this) {
            if (!locked) {
                ...
                if ((!node.enableSwapQueueing) || (incomingMessageQueue.size() > MAX_INCOMING_QUEUE_LENGTH)) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Incoming queue length too large: " + incomingMessageQueue.size() + " rejecting "
                                     + msg);
                    }
                } else {
    ...
    public boolean handleSwapReply(Message m, PeerNode source) {
        ...
        if (source != item.routedTo) {
            Logger.error(this,
                         "Unmatched swapreply " + uid + " from wrong source: From " + source + " should be "
    ...
    public boolean handleSwapCommit(Message m, PeerNode source) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Forwarding SwapCommit " + uid + ',' + item.outgoingID + " from " + source + " to "
                         + item.routedTo);
        }
    ...
    public class IncomingSwapRequestHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                MessageFilter filter = MessageFilter.create().setType(DMT.FNPSwapCommit).setField(DMT.UID,
                ...
                if (commit == null) {
                    ...
                    Logger.error(
                        this,
                        "Timed out waiting for SwapCommit on " + uid
    ...
    public class OutgoingSwapRequestHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                MessageFilter filter1 = MessageFilter.create().setType(DMT.FNPSwapRejected).setField(DMT.UID,
                                            uid).setSource(pn).setTimeout(TIMEOUT);
                MessageFilter filter2 = MessageFilter.create().setType(DMT.FNPSwapReply).setField(DMT.UID,
                                            uid).setSource(pn).setTimeout(TIMEOUT);
                                        ...
                                        synchronized (l) {
                                            ...
                                            if (Location.equals(ploc, myLoc)) {
                                                ...
                                                if ((now - l.getLocationSetTime() > MINUTES.toMillis(2))
                                                    ...
                                                    Logger.error(this,
                                                                 "Randomizing location: my loc=" + myLoc + " but loc="
                                                                 + ploc + " for " + pn);
