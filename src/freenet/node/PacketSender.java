...
public class PacketSender implements Runnable {
	...
	static final long MIN_OLD_OPENNET_CONNECT_DELAY_NO_CONNS = SECONDS.toMillis(
		10
	);
	/** We send connect attempts to old-opennet-peers no more than once every
	 * this many milliseconds. */
	...
	private void realRun() {
		...
		if (count > MAX_PACKET_SIZE) canSendThrottled = true;
			...
			long canSendAt =
				node.outputThrottle.getNanosPerTick() *
				(MAX_PACKET_SIZE - count);
			canSendAt = MILLISECONDS.convert(
				canSendAt + MILLISECONDS.toNanos(1) - 1,
				NANOSECONDS
			);
			if (logMINOR) Logger.minor(
				this,
				"Can send throttled packets in " + canSendAt + "ms"
			);
		...
		for (PeerNode pn : nodes) {
			...
			lastReceivedPacketFromAnyNode = Math.max(
			...
			if (pn.isConnected()) {
				...
				if (
					now - pn.lastReceivedDataPacketTime() >
					...
					now - pn.lastReceivedAckTime() >
						pn.maxTimeBetweenReceivedAcks() &&
					!pn.isDisconnecting()
				) {
					...
					Logger.normal(
						...
						"Disconnecting from " +
						pn +
						" - haven't received acks recently"
					);
					// Do it properly.
				...
				if (canSendThrottled || !shouldThrottle) {
					...
					if (sendTime != Long.MAX_VALUE) {
						if (sendTime <= now) {
							...
							if (
								sendTime <= lowestFullPacketSendTime
				...
				if (canSendThrottled || !shouldThrottle) {
					...
					if (urgentTime < Long.MAX_VALUE && logMINOR) Logger.minor(
						this,
						"Next urgent time: " +
						urgentTime +
						"(in " +
			...
			if (handshakeTime != Long.MAX_VALUE) {
				...
				if (handshakeTime <= lowestHandshakeTime) handshakePeers.add(
					pn
				);
			}

			long tempNow = System.currentTimeMillis();
			if ((tempNow - oldTempNow) > SECONDS.toMillis(5)) Logger.error(
				this,
		...
		if (lowestUrgentSendTime <= now) {
			...
			toSendPacket = urgentSendPeers.get(
				localRandom.nextInt(urgentSendPeers.size())
			);
			t = lowestUrgentSendTime;
		} else if (lowestFullPacketSendTime < Long.MAX_VALUE) {
			toSendPacket = urgentFullPacketPeers.get(
				localRandom.nextInt(urgentFullPacketPeers.size())
			);
		...
		if (lowestHandshakeTime <= now && t > lowestHandshakeTime) {
			toSendHandshake = handshakePeers.get(
				localRandom.nextInt(handshakePeers.size())
			);
			toSendPacket = null;
			toSendAckOnly = null;
		}
		...
		if (toSendHandshake != null) {
			...
			toSendHandshake
				.getOutgoingMangler()
				.sendHandshake(toSendHandshake, false);
			long afterHandshakeTime = System.currentTimeMillis();
			if (
				(afterHandshakeTime - beforeHandshakeTime) > SECONDS.toMillis(2)
			) Logger.error(
				this,
		...
		if (om != null && node.getUptime() > SECONDS.toMillis(30)) {
			...
			for (OpennetPeerNode pn : peers) {
				...
				if (lastConnected <= 0) Logger.error(
					this,
					"Last connected is zero or negative for old-opennet-peer " +
					pn
				);
		...
		if ((now - oldNow) > SECONDS.toMillis(10)) Logger.error(
			this,
			"now is more than 10 seconds past oldNow (" +
			(now - oldNow) +
			") in PacketSender"
		);
		...
		if (now - node.startupTime > MINUTES.toMillis(5)) if (
	...
	protected String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("PacketSender." + key, patterns, values);
	}

	protected String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("PacketSender." + key, pattern, value);
	}
}
