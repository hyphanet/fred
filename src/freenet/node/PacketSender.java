    ...
    private void realRun() {
        ...
        for (PeerNode pn : nodes) {
            ...
            if ((tempNow - oldTempNow) > SECONDS.toMillis(5)) {
                Logger.error(this,
                             "tempNow is more than 5 seconds past oldTempNow (" + (tempNow - oldTempNow)
        ...
        if (toSendPacket != null) {
            try {
                ...
                Logger.error(this,
                             "Waited too long: " + TimeUtil.formatTime(e.delta)
                             + " to allocate a packet number to send to " + toSendPacket + " : "
                             + ("(new packet format)") + " (version " + toSendPacket.getVersionNumber()
                             + ") - DISCONNECTING!");
            ...
            try {
                ...
                Logger.error(this,
                             "Waited too long: " + TimeUtil.formatTime(e.delta)
                             + " to allocate a packet number to send to " + toSendAckOnly + " : "
                             + ("(new packet format)") + " (version " + toSendAckOnly.getVersionNumber()
                             + ") - DISCONNECTING!");
        ...
        if ((om != null) && (node.getUptime() > SECONDS.toMillis(30))) {
            ...
            for (PeerNode pn : peers) {
                ...
                if (now - lastConnected > OpennetManager.MAX_TIME_ON_OLD_OPENNET_PEERS) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Removing old opennet peer (too old): " + pn + " age is "
