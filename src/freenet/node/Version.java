...
public class Version {
	...
	private static final int buildNumber = 1243;

	/** Oldest build of Fred we will talk to */
	private static final int oldLastGoodBuild = 1241;
	private static final int newLastGoodBuild = 1243;
	static final long transitionTime;

	static {
		final Calendar _cal = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
		// year, month - 1 (or constant), day, hour, minute, second
		_cal.set( 2010, Calendar.MARCH, 20, 0, 0, 0 );
	...
	public static final boolean checkGoodVersion(
		...
		if (stableVersion(v)) {
			try {
				...
				Logger.minor(
					Version.class, "Not accepting (" + e + ") from " + version);
				return false;
			}
		}
		if(logDEBUG) {
