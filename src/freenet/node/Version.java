...
public class Version {
    ...
    private static final int newLastGoodBuild = 1451;
    ...
    static {
        ...
        _cal.set(2013, Calendar.AUGUST, 28, 0, 0, 0);
        ...
        if (stableVersion(v)) {
            try {
                ...
                if (build < lastGoodStableBuild) {
                    if (logDEBUG) {
                        Logger.debug(Version.class,
                                     "Not accepting stable from version" + version + "(lastGoodStableBuild="
                                     + lastGoodStableBuild + ')');
                    }
        ...
        if (sameArbitraryVersion(v, lgv)) {
            try {
                ...
                if (build < min_build) {
                    if (logDEBUG) {
                        Logger.debug(Version.class,
                                     "Not accepting unstable from version: " + version + "(lastGoodVersion="
                                     + lastGoodVersion + ')');
                    }
...
//                                                       + " or " + stableProtocolVersion
    ...
    public static void main(String[] args) throws Throwable {
        System.out.println("Freenet: " + nodeName + ' ' + nodeVersion + " (protocol " + protocolVersion + ") build "
                           + buildNumber + " (last good build: " + lastGoodBuild() + ')');
    }
}
