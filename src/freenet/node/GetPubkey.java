...
public class GetPubkey {
	 ...
	 */
	public DSAPublicKey getKey(byte[] hash, boolean canReadClientCache, boolean forULPR, boolean mustBeMarkedAsPostCachingChanges) {
		...
		try {
			...
			if(node.oldPKClientCache != null && canReadClientCache && key == null) {
				...
				if(pks != null) key = pks.fetch(hash, false, false);
			}
			// We can *read* from the datastore even if nearby, but we cannot promote in that case.
			if(key == null)
				key = pubKeyDatastore.fetch(hash, false, mustBeMarkedAsPostCachingChanges);
			if(key == null) {
				PubkeyStore pks = node.oldPK;
				if(pks != null) key = pks.fetch(hash, false, mustBeMarkedAsPostCachingChanges);
			}
			if (key == null)
				key = pubKeyDatacache.fetch(hash, false, mustBeMarkedAsPostCachingChanges);
			if(key == null) {
				PubkeyStore pks = node.oldPKCache;
				if(pks != null) key = pks.fetch(hash, false, mustBeMarkedAsPostCachingChanges);
			}
			if(key == null && pubKeySlashdotcache != null && forULPR)
				key = pubKeySlashdotcache.fetch(hash, false, false);
			if (key != null) {
