...
public class SimpleSendableInsert extends SendableInsert {
	...
	public SimpleSendableInsert(
		...
		if (block instanceof CHKBlock) scheduler =
			core.requestStarters.chkPutSchedulerBulk;
		else if (block instanceof SSKBlock) scheduler =
			core.requestStarters.sskPutSchedulerBulk;
		else throw new IllegalArgumentException(
			"Don't know what to do with " + block
		);
		if (!scheduler.isInsertScheduler()) throw new IllegalStateException(
			"Scheduler " + scheduler + " is not an insert scheduler!"
		);
	}
	...
	public SendableRequestSender getSender(ClientContext context) {
		return new SendableRequestSender() {
			...
			public boolean send(
				...
				sched.removeRunningInsert(
					SimpleSendableInsert.this,
					req.token.getKey()
				);
				return true;
			}
	...
	public synchronized long getWakeupTime(ClientContext context, long now) {
		if (isEmpty()) return -1;
		if (
			scheduler.fetchingKeys().hasInsert(new MySendableRequestItem(this))
