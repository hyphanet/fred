 ...
 * @author amphibian
 */
public class NodeImpl implements TimeSkewDetectorCallback, Node, ProtectedNode {
   ...
   public static boolean isTestnetEnabled() {
	 ...
	 */
	 NodeImpl(PersistentConfig config, RandomSource r, RandomSource weakRandom, LoggingConfigHandler lc, NodeStarter ns, Executor executor) throws NodeInitException {
		...
		Logger.normal(NodeImpl.class, "Creating node...");
		...
		if (obwLimit < minimumBandwidth) {
			...
			Logger.normal(NodeImpl.class, "Output bandwidth was lower than minimum bandwidth. Increased to minimum bandwidth.");
		}
		...
		if(ibwLimit == -1) {
			...
			Logger.normal(NodeImpl.class, "Input bandwidth was lower than minimum bandwidth. Increased to minimum bandwidth.");
		}
		inputBandwidthLimit = ibwLimit;
		try {
		...
		opennetConfig.register("enabled", false, 0, true, true, "Node.opennetEnabled", "Node.opennetEnabledLong", new BooleanCallback() {
			...
			public void set(Boolean val) throws InvalidConfigValueException {
				...
				synchronized(this) {
					...
					if(val) {
						try {
							o = opennet = new OpennetManager(NodeImpl.this, opennetCryptoConfig, System.currentTimeMillis(), isAllowedToConnectToSeednodes);
						} catch (NodeInitException e) {
							opennet = null;
	...
	*/
			...
			if(delay) {
				...
				this.getTicker().queueTimedJob(new Runnable() {
					...
					public void run() {
						...
						NodeImpl.this.chkDatastore = chkDatastore;
						NodeImpl.this.chkDatacache = chkDatacache;
						NodeImpl.this.pubKeyDatastore = pubKeyDatastore;
						NodeImpl.this.pubKeyDatacache = pubKeyDatacache;
						getPubKey.setDataStore(pubKeyDatastore, pubKeyDatacache);
						NodeImpl.this.sskDatastore = sskDatastore;
						NodeImpl.this.sskDatacache = sskDatacache;
			...
			if(delay) {
				...
				getTicker().queueTimedJob(new Runnable() {
					...
					public void run() {
						...
						NodeImpl.this.chkClientcache = chkClientcache;
						NodeImpl.this.pubKeyClientcache = pubKeyClientcache;
						getPubKey.setLocalDataStore(pubKeyClientcache);
						NodeImpl.this.sskClientcache = sskClientcache;
	...
	public void start(boolean noSwaps) throws NodeInitException {
		...
		dispatcher.start(); // must be before usm
		dnsr.start();
...
//		}
	...
	public void handleNodeToNodeTextMessageSimpleFieldSet(SimpleFieldSet fs, DarknetPeerNode source, int fileNumber) throws FSParseException {
		...
		fs.removeValue("n2nType");
		if(overallType == N2N_MESSAGE_TYPE_FPROXY) {
	...
	private void handleFproxyNodeToNodeTextMessageSimpleFieldSet(SimpleFieldSet fs, DarknetPeerNode source, int fileNumber) throws FSParseException {
		...
		if(type == N2N_TEXT_MESSAGE_TYPE_USERALERT) {
			source.handleFproxyN2NTM(fs, fileNumber);
		} else if(type == N2N_TEXT_MESSAGE_TYPE_FILE_OFFER) {
			source.handleFproxyFileOffer(fs, fileNumber);
		} else if(type == N2N_TEXT_MESSAGE_TYPE_FILE_OFFER_ACCEPTED) {
			source.handleFproxyFileOfferAccepted(fs, fileNumber);
		} else if(type == N2N_TEXT_MESSAGE_TYPE_FILE_OFFER_REJECTED) {
			source.handleFproxyFileOfferRejected(fs, fileNumber);
		} else if(type == N2N_TEXT_MESSAGE_TYPE_BOOKMARK) {
			source.handleFproxyBookmarkFeed(fs, fileNumber);
		} else if(type == N2N_TEXT_MESSAGE_TYPE_DOWNLOAD) {
	...
	public void connect(Node node, FRIEND_TRUST trust, FRIEND_VISIBILITY visibility) throws FSParseException, PeerParseException, ReferenceSignatureVerificationException, PeerTooOldException {
		peers.connect(node.getDarknetCrypto().exportPublicFieldSet(), darknetCrypto.getPacketMangler(), trust, visibility);
	}
    ...
    protected ProtectedLocationManager newLocationManager(RandomSource r, NodeImpl node) {
	...
	private ProtectedNodeCrypto newNodeCrypto(final NodeImpl node, final boolean isOpennet, NodeCryptoConfig config, long startupTime, boolean enableARKs) throws NodeInitException {
	    return (ProtectedNodeCrypto) new NodeCryptoImpl(node, isOpennet, config, startupTime, enableARKs);
    }

    @Override
    public NodeIPDetector getIPDetector() { return ipDetector; }
    ...
    @Override
    public RandomSource getRNG() { return random; }
