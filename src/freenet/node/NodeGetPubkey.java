...
public class NodeGetPubkey implements GetPubkey {
	...
	void setDataStore(
		PubkeyStore pubKeyDatastore,
		PubkeyStore pubKeyDatacache
	) {
	...
	public DSAPublicKey getKey(
		...
		if (logMINOR) Logger.minor(
			this,
			"Getting pubkey: " + HexUtil.bytesToHex(hash)
		);
		...
		try {
			...
			if (pubKeyClientcache != null && canReadClientCache) key =
				pubKeyClientcache.fetch(hash, false, false, meta);
			if (
				node.oldPKClientCache != null &&
				canReadClientCache &&
				key == null
			) {
				PubkeyStore pks = node.oldPKClientCache;
				if (pks != null) key = pks.fetch(hash, false, false, meta);
				if (key != null && logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from old client cache"
				);
			}
			...
			if (key == null) {
				...
				if (key != null && logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from store"
				);
			}
			if (key == null) {
				PubkeyStore pks = node.oldPK;
				if (pks != null) key = pks.fetch(
					hash,
					false,
					ignoreOldBlocks,
					meta
				...
				if (key != null && logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from old store"
				);
			}
			if (key == null) {
				...
				if (key != null && logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from cache"
				);
			}
			if (key == null) {
				...
				if (key != null && logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from old cache"
				);
			}
			if (key == null && pubKeySlashdotcache != null && forULPR) {
				key = pubKeySlashdotcache.fetch(
					hash,
					false,
					ignoreOldBlocks,
					meta
				...
				if (logMINOR) Logger.minor(
					this,
					"Got " + HexUtil.bytesToHex(hash) + " from slashdot cache"
				);
			}
			if (key != null) {
	...
	public void cacheKey(
		...
		if (logMINOR) Logger.minor(
			this,
			"Cache key: " + HexUtil.bytesToHex(hash) + " : " + key
		);
		ByteArrayWrapper w = new ByteArrayWrapper(hash);
		synchronized (cachedPubKeys) {
			DSAPublicKey key2 = cachedPubKeys.get(w);
			if (
				(key2 != null) && !key2.equals(key)
			) throw new IllegalArgumentException(
				"Wrong hash?? Already have different key with same hash!"
			);
			cachedPubKeys.push(w, key);
			while (
				cachedPubKeys.size() > MAX_MEMORY_CACHED_PUBKEYS
