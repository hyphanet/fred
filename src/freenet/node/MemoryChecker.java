...
public class MemoryChecker implements Runnable {
	...
	public void run() {
		...
		Logger.normal(
			this,
			"Memory in use: " + SizeUtil.formatSize((totalMemory - freeMemory))
		);
		...
		if (totalMemory == maxMemory || maxMemory == Long.MAX_VALUE) {
			...
			if (avgFreeMemory == null) avgFreeMemory = new SimpleRunningAverage(
				3,
				freeMemory
			);
			...
			if (
				avgFreeMemory.countReports() >= 3 &&
				avgFreeMemory.currentValue() < 4 * 1024 * 1024
			) { //  average free memory < 4 MB
				Logger.normal(
					this,
					"Reached threshold, checking for low memory ..."
				);
				System.gc();
		...
		if (aggressiveGCModificator > 0) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Memory in use before GC: " + beforeGCUsedMemory
			);
			...
			if (logMINOR) {
				Logger.minor(
					this,
					"Memory in use after GC: " + afterGCUsedMemory
				);
				Logger.minor(
					this,
					"GC completed after " +
