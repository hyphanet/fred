...
public class TextModeClientInterfaceServer implements Runnable {
	...
	TextModeClientInterfaceServer(
		...
		this.isEnabled = true;
		if (ssl) {
			networkInterface = SSLNetworkInterface.create(
				port,
				bindTo,
				allowedHosts,
				n.executor,
	...
	public static TextModeClientInterfaceServer maybeCreate(
		...
		if (TMCIEnabled) server = new TextModeClientInterfaceServer(
			node,
			core,
			port,
			bind_ip,
		...
		if (direct) {
			...
			TextModeClientInterface directTMCI = new TextModeClientInterface(
				node,
				core,
				client,
	...
	static class TMCISSLCallback extends BooleanCallback {
		...
		public void set(Boolean val) throws InvalidConfigValueException {
			...
			if (!SSL.available()) {
				throw new InvalidConfigValueException(
					"Enable SSL support before use ssl with TMCI"
				);
			}
			ssl = val;
			throw new InvalidConfigValueException(
	...
	static class TMCIBindtoCallback extends StringCallback {
		...
		public String get() {
			if (
				core.getTextModeClientInterface() != null
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			String[] failedAddresses = core
				.getTextModeClientInterface()
				.networkInterface.setBindTo(val, false);
		...
		while (true) {
			...
			while (isEnabled) {
				...
				try {
					...
					TextModeClientInterface tmci = new TextModeClientInterface(
						this,
						in,
						out
					);
					...
					n.executor.execute(
						tmci,
						"Text mode client interface handler for " + s.getPort()
					);
