...
public class NodeCryptoConfig {
	...
	NodeCryptoConfig(
		...
		config.register(
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					if (
						portNumber < -1 || portNumber == 0 || portNumber > 65535
					) {
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					if (val < 0) throw new InvalidConfigValueException(
						"testingDropPacketsEvery must not be negative"
					);
					synchronized (NodeCryptoConfig.this) {
		...
		alwaysAllowLocalAddresses = config.getBoolean(
			"alwaysAllowLocalAddresses"
		);
		...
		includeLocalAddressesInNoderefs = config.getBoolean(
			"includeLocalAddressesInNoderefs"
		);
	...
	synchronized void starting(NodeCrypto crypto2) {
		if (crypto != null) throw new IllegalStateException(
