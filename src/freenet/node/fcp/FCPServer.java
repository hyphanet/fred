...
public class FCPServer implements Runnable, DownloadCache {
    ...
    public FCPServer(String ipToBindTo, String allowedHosts, String allowedHostsFullAccess, int port, Node node,
                     NodeClientCore core, boolean isEnabled, boolean assumeDDADownloadAllowed,
    ...
    public static FCPServer maybeCreate(Node node, NodeClientCore core, Config config, ObjectContainer container)
            throws IOException, InvalidConfigValueException {
        ...
        fcpConfig.register("ssl", false, sortOrder++, true, true, "FcpServer.ssl", "FcpServer.sslLong",
                           new FCPSSLCallback());
        fcpConfig.register("port", FCPServer.DEFAULT_FCP_PORT /* anagram of 1984, and 1000 up from old number */, 2,
                           true, true, "FcpServer.portNumber", "FcpServer.portNumberLong",
                           new FCPPortNumberCallback(core), false);
        ...
        FCPServer fcp = new FCPServer(fcpConfig.getString("bindTo"), fcpConfig.getString("allowedHosts"),
                                      ...
                                      core, fcpConfig.getBoolean("enabled"),
                                      ...
                                      fcpConfig.getInt("maxMessageQueueLength"), container);
    ...
    public boolean removeGlobalRequestBlocking(final String identifier)
            throws MessageInvalidException, DatabaseDisabledException {
    ...
    public void makePersistentGlobalRequestBlocking(final FreenetURI fetchURI, final boolean filterData,
            ...
            throws NotAllowedException, IOException, DatabaseDisabledException {
        class OutputWrapper {
    ...
    public void makePersistentGlobalRequest(FreenetURI fetchURI, boolean filterData, String expectedMimeType,
            String persistenceTypeString, String returnTypeString, boolean realTimeFlag, ObjectContainer container)
        ...
        makePersistentGlobalRequest(fetchURI, filterData, expectedMimeType, persistenceTypeString, returnTypeString,
                                    realTimeFlag, container, core.getDownloadsDir());
    }
...
//                      File returnFilename, File returnTempFilename) throws IdentifierCollisionException {
        ...
        try {
            innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType,
                                             "FProxy:" + fetchURI.getPreferredFilename(), returnFilename,
                                             returnTempFilename, realTimeFlag, container);

            return;
        } catch (IdentifierCollisionException ee) {
            try {
                innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType,
                                                 "FProxy:" + fetchURI.getDocName(), returnFilename, returnTempFilename,
                                                 realTimeFlag, container);

                return;
            } catch (IdentifierCollisionException e) {
                try {
                    innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType,
                                                     "FProxy:" + fetchURI.toString(false, false), returnFilename,
                                                     returnTempFilename, realTimeFlag, container);
                    ...
                    try {
                        innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType,
                                                         "FProxy (" + System.currentTimeMillis() + ')', returnFilename,
                                                         returnTempFilename, realTimeFlag, container);
                        ...
                        while (true) {
                            ...
                            try {
                                ...
                                innerMakePersistentGlobalRequest(fetchURI, filterData, persistence, returnType, id,
                                                                 returnFilename, returnTempFilename, realTimeFlag,
                                                                 container);
    ...
    private void innerMakePersistentGlobalRequest(FreenetURI fetchURI, boolean filterData, boolean persistRebootOnly,
            short returnType, String id, File returnFilename, File returnTempFilename, boolean realTimeFlag,
            ObjectContainer container)
        ...
        final ClientGet cg = new ClientGet(persistRebootOnly ? globalRebootClient : globalForeverClient, fetchURI,
                                           defaultFetchContext.localRequestOnly, defaultFetchContext.ignoreStore,
                                           filterData, QUEUE_MAX_RETRIES, QUEUE_MAX_RETRIES, QUEUE_MAX_DATA_SIZE,
                                           returnType, persistRebootOnly, id, Integer.MAX_VALUE,
                                           RequestStarter.BULK_SPLITFILE_PRIORITY_CLASS, returnFilename,
                                           returnTempFilename, null, false, realTimeFlag, this, container);
     ...
     * password etc.
     */
    public void startBlocking(final ClientRequest req, ObjectContainer container, ClientContext context)
            throws IdentifierCollisionException, DatabaseDisabledException {
