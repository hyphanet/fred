...
public class ClientPut extends ClientPutBase {
    ...
    public ClientPut(FCPConnectionHandler handler, ClientPutMessage message, FCPServer server,
            ...
            throws IdentifierCollisionException, MessageInvalidException, MalformedURLException {
        super(checkEmptySSK(message.uri, message.targetFilename, server.core.clientContext), message.identifier,
              message.verbosity, null, handler, message.priorityClass, message.persistenceType, message.clientToken,
        ...
        if (message.uploadFromType == ClientPutMessage.UPLOAD_FROM_DISK) {
            if (!handler.server.core.allowUploadFrom(message.origFilename)) {
                throw new MessageInvalidException(ProtocolErrorMessage.ACCESS_DENIED,
                                                  "Not allowed to upload from " + message.origFilename, identifier,
                                                  global);
            }
            ...
            if (message.fileHash != null) {
                try {
                    ...
                    try {
                        ...
                        throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
                                                          "Can't base64 decode " + ClientPutBase.FILE_HASH, identifier,
                                                          global);
                    }
                }
                ...
                throw new MessageInvalidException(ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
                                                  ...
                                                  + ". Have you done a testDDA previously ?", identifier, global);
            }
        }
        ...
        if (binaryBlob) {
            if ((mimeType != null) &&!mimeType.equals(BinaryBlob.MIME_TYPE)) {
                throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
                                                  "No MIME type allowed when inserting a binary blob", identifier,
                                                  global);
            }
        }
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "data = " + tempData + ", uploadFrom = " + ClientPutMessage.uploadFromString(uploadFrom));
        }
        ...
        if (salt != null) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "FileHash result : we found " + Base64.encode(foundHash) + " and were given "
                             + Base64.encode(saltedHash) + '.');
            }

            if (!Arrays.equals(saltedHash, foundHash)) {
                throw new MessageInvalidException(ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
                                                  "The hash doesn't match! (salt used : \"" + salt + "\")", identifier,
                                                  global);
            }
        }
        ...
        putter = new ClientPutter(this, data, this.uri, cm, ctx, priorityClass, getCHKOnly, isMetadata, lowLevelClient,
                                  (this.uri.getDocName() == null) ? targetFilename : null, binaryBlob,
                                  server.core.clientContext, message.overrideSplitfileCryptoKey,
     ...
     */
    public ClientPut(FCPClient globalClient, FreenetURI uri, String identifier, int verbosity, String charset,
                     ...
                     Bucket data, FreenetURI redirectTarget, String targetFilename, boolean earlyEncode,
                     boolean canWriteClientCache, boolean forkOnCacheable, int extraInsertsSingleBlock,
                     int extraInsertsSplitfileHeaderBlock, boolean realTimeFlag,
                     InsertContext.CompatibilityMode compatMode, byte[] overrideSplitfileKey, FCPServer server,
                     ObjectContainer container)
            throws IdentifierCollisionException, NotAllowedException, FileNotFoundException, MalformedURLException,
                   MetadataUnresolvedException {
        super(uri = checkEmptySSK(uri, targetFilename, server.core.clientContext), identifier, verbosity, charset,
              null, globalClient, priorityClass, persistenceType, null, true, getCHKOnly, dontCompress, maxRetries,
        ...
        putter = new ClientPutter(this, data, this.uri, cm, ctx, priorityClass, getCHKOnly, isMetadata, lowLevelClient,
                                  (this.uri.getDocName() == null) ? targetFilename : null, binaryBlob,
                                  server.core.clientContext, overrideSplitfileKey, -1);
    }
    ...
    protected FCPMessage persistentTagMessage(ObjectContainer container) {
        ...
        return new PersistentPut(identifier, publicURI, uri, verbosity, priorityClass, uploadFrom, targetURI,
                                 ...
                                 getDataSize(container), clientToken, started, ctx.maxInsertRetries, targetFilename,
                                 binaryBlob, this.ctx.getCompatibilityMode(), this.ctx.dontCompress,
                                 this.ctx.compressorDescriptor, isRealTime(),
    ...
    RequestStatus getStatus(ObjectContainer container) {
        ...
        return new UploadFileRequestStatus(identifier, persistenceType, started, finished, succeeded, total, min,
                                           fetched, fatal, failed, totalFinalized, lastActivity, priorityClass,
                                           finalURI, origURI, failureCode, failureReasonShort, failureReasonLong,
                                           getDataSize(container), mimeType, fnam, isCompressing(container));
    }
}
