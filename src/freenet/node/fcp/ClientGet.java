...
public class ClientGet extends ClientRequest implements ClientGetCallback, ClientEventListener {
    ...
    public ClientGet(FCPConnectionHandler handler, ClientGetMessage message, FCPServer server,
        ...
        super(message.uri, message.identifier, message.verbosity, message.charset, handler, message.priorityClass,
              message.persistenceType, message.realTimeFlag, message.clientToken, message.global, container);
        ...
        if (returnType == ClientGetMessage.RETURN_TYPE_DISK) {
            ...
            if (!(server.core.allowDownloadTo(tempFile) && server.core.allowDownloadTo(targetFile))) {
                ...
                throw new MessageInvalidException(ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
                                                  "Not allowed to download to " + tempFile + " or " + targetFile
                                                  + ". You might need to do a " + TestDDARequestMessage.NAME
                                                  + " first.", identifier, global);
            }
        ...
        returnBucket = ret;
        getter = new ClientGetter(this, uri, fctx, priorityClass, lowLevelClient,
                                  binaryBlob ? new NullBucket() : returnBucket,
                                  binaryBlob ? new BinaryBlobWriter(returnBucket) : null, false,
                                  message.getInitialMetadata(), extensionCheck);
    }
     ...
     * @throws IOException
     */
    public ClientGet(FCPClient globalClient, FreenetURI uri, boolean dsOnly, boolean ignoreDS, boolean filterData,
                     int maxSplitfileRetries, int maxNonSplitfileRetries, long maxOutputLength, short returnType,
                     boolean persistRebootOnly, String identifier, int verbosity, short prioClass, File returnFilename,
                     File returnTempFilename, String charset, boolean writeToClientCache, boolean realTimeFlag,
        ...
        super(uri, identifier, verbosity, charset, null, globalClient, prioClass,
              (persistRebootOnly ? ClientRequest.PERSIST_REBOOT : ClientRequest.PERSIST_FOREVER), realTimeFlag, null,
              true, container);
        fctx = new FetchContext(server.defaultFetchContext, FetchContext.IDENTICAL_MASK, false, null);
        fctx.eventProducer.addEventListener(this);
        ...
        returnBucket = ret;
        getter = new ClientGetter(this, uri, fctx, priorityClass, lowLevelClient, returnBucket, null, false, null,
                                  extensionCheck);
    }
    ...
    public void onSuccess(FetchResult result, ClientGetter state, ObjectContainer container) {
        ...
        if (bucketChanged) {
            ...
            synchronized (this) {
                ...
                if (finished) {
                    Logger.error(this, "Already finished but onSuccess() for " + this + " data = " + data,
                                 new Exception("debug"));
                    data.free();
            ...
            if (bucketChanged && (persistenceType == PERSIST_FOREVER)) {
                if (container.ext().getID(returnBucket) == container.ext().getID(data)) {
                    Logger.error(this,
                                 ...
                                 + container.ext().getID(returnBucket) + " for " + returnBucket + " active="
                                 + container.ext().isActive(returnBucket) + " stored = "
                                 + container.ext().isStored(returnBucket) + " but UID(data)="
                                 + container.ext().getID(data) + " for " + data + " active = "
            ...
            synchronized (this) {
                if ((data instanceof FileBucket) && (returnBucket instanceof FileBucket)) {
                    ...
                    if (actualFile.toString().equals(expectedFile.toString())) {
                        Logger.warning(this,
                                       "Data was written to the correct file " + actualFile
                                       + " but the bucket changed: " + returnBucket + " -> " + data);
                        ...
                        Logger.error(this,
                                     "Data was written to " + actualFile + " but should be written to " + expectedFile);
                    ...
                    Logger.error(this,
                                 "Returned bucket " + data + " in onSuccess, expected " + returnBucket
                                 + ((data == returnBucket) ? " (equal)" : "(not equal)"), new Exception("error"));
        ...
        if (bucketChanged) {
            ...
            if ((persistenceType == PERSIST_FOREVER) && container.ext().isStored(this)) {
                ...
                Logger.error(this,
                             "Data returned to wrong bucket " + data + " expected " + returnBucket + " in " + this,
                             new Exception("error"));
                ...
                try {
                    ...
                    onFailure(new FetchException(FetchException.INTERNAL_ERROR,
                                                 "Data != returnBucket and then failed to copy", e), null, container);
        ...
        synchronized (this) {
            ...
            if (returnType == ClientGetMessage.RETURN_TYPE_DIRECT) {
                ...
                adm = new AllDataMessage(returnBucket, identifier, global, startupTime, completionTime,
                ...
                if (!FileUtil.renameTo(tempFile, targetFile)) {
                    postFetchProtocolErrorMessage =
                        new ProtocolErrorMessage(ProtocolErrorMessage.COULD_NOT_RENAME_FILE, false, null, identifier,
                                                 global);
    ...
    protected FCPMessage persistentTagMessage(ObjectContainer container) {
        ...
        return new PersistentGet(identifier, uri, verbosity, priorityClass, returnType, persistenceType, targetFile,
                                 tempFile, clientToken, client.isGlobalQueue, started, fctx.maxNonSplitfileRetries,
                                 binaryBlob, fctx.maxOutputLength, isRealTime());
    }
    ...
    public void receive(ClientEvent ce, ObjectContainer container, ClientContext context) {
        ...
        if (ce instanceof SplitfileProgressEvent) {
            ...
            progress = new SimpleProgressMessage(identifier, global, (SplitfileProgressEvent) ce);
            ...
            progress = new CompatibilityMode(identifier, global, event.minCompatibilityMode,
                                             event.maxCompatibilityMode, event.splitfileCryptoKey, event.dontCompress,
                                             event.bottomLayer);
    ...
    public InsertContext.CompatibilityMode[] getCompatibilityMode(ObjectContainer container) {
        ...
        if (compatMessage != null) {
            ...
            return new InsertContext.CompatibilityMode[] { InsertContext.CompatibilityMode.COMPAT_UNKNOWN,
        ...
        synchronized (this) {
            ...
            if ((persistenceType == PERSIST_FOREVER) && (getFailedMessage != null)) {
                ...
                if (getFailedMessage.redirectURI != null) {
                    ...
                    redirect = getFailedMessage.redirectURI.clone();
                }
    ...
    RequestStatus getStatus(ObjectContainer container) {
        ...
        return new DownloadRequestStatus(identifier, persistenceType, started, finished, succeeded, total, min,
                                         fetched, fatal, failed, totalFinalized, lastActivity, priorityClass,
                                         failureCode, mimeType, dataSize, target, getCompatibilityMode(container),
                                         getOverriddenSplitfileCryptoKey(container), getURI(container).clone(),
                                         failureReasonShort, failureReasonLong, overriddenDataType, shadow, filterData,
                                         getDontCompress(container));
    }
}
