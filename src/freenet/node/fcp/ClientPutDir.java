...
public class ClientPutDir extends ClientPutBase {
    ...
    public ClientPutDir(FCPConnectionHandler handler, ClientPutDirMessage message,
        ...
        super(checkEmptySSK(message.uri, (message.targetFilename != null) ? message.targetFilename : "site",
                            server.core.clientContext), message.identifier, message.verbosity, null, handler,
                                message.priorityClass, message.persistenceType, message.clientToken, message.global,
                                message.getCHKOnly, message.dontCompress, message.localRequestOnly, message.maxRetries,
                                message.earlyEncode, message.canWriteClientCache, message.forkOnCacheable,
                                message.compressorDescriptor, message.extraInsertsSingleBlock,
                                message.extraInsertsSplitfileHeaderBlock, message.realTimeFlag,
                                message.compatibilityMode, message.ignoreUSKDatehints, server, container);
        logMINOR = Logger.shouldLog(LogLevel.MINOR, this);
...
//      this.manifestElements.putAll(manifestElements);
    ...
    public ClientPutDir(FCPClient client, FreenetURI uri, String identifier, int verbosity, short priorityClass,
                        short persistenceType, String clientToken, boolean getCHKOnly, boolean dontCompress,
                        int maxRetries, HashMap<String, Object> elements, String defaultName, boolean global,
                        ...
                        byte[] overrideSplitfileCryptoKey, FCPServer server, ObjectContainer container)
            throws IdentifierCollisionException, MalformedURLException, TooManyFilesInsertException {
        super(checkEmptySSK(uri, "site", server.core.clientContext), identifier, verbosity, null, null, client,
              priorityClass, persistenceType, clientToken, global, getCHKOnly, dontCompress, maxRetries, earlyEncode,
              canWriteClientCache, forkOnCacheable, false, extraInsertsSingleBlock, extraInsertsSplitfileHeaderBlock,
              realTimeFlag, null, InsertContext.CompatibilityMode.COMPAT_CURRENT, false /* XXX ignoreUSKDatehints */,
              server, container);
        wasDiskPut = false;
    ...
    public ClientPutDir(FCPClient client, FreenetURI uri, String identifier, int verbosity, short priorityClass,
                        short persistenceType, String clientToken, boolean getCHKOnly, boolean dontCompress,
                        int maxRetries, File dir, String defaultName, boolean allowUnreadableFiles,
                        ...
                        boolean realTimeFlag, byte[] overrideSplitfileCryptoKey, FCPServer server,
                        ObjectContainer container)
            throws FileNotFoundException, IdentifierCollisionException, MalformedURLException,
        ...
        for (File f : files) {
            ...
            if (f.exists() && f.canRead()) {
                if (f.isFile()) {
                    ...
                    map.put(f.getName(),
                            new ManifestElement(f.getName(), prefix + f.getName(), bucket,
    ...
    private void makePutter(ObjectContainer container, ClientContext context) throws TooManyFilesInsertException {
        ...
        case ManifestPutter.MANIFEST_DEFAULTPUTTER :
            putter = new DefaultManifestPutter(this, manifestElements, priorityClass, uri, defaultName, ctx,
                                               getCHKOnly, lowLevelClient, earlyEncode,
                                               persistenceType == PERSIST_FOREVER, overrideSplitfileCryptoKey,
                                               container, context);

            break;

        default :
            putter = new SimpleManifestPutter(this, manifestElements, priorityClass, uri, defaultName, ctx, getCHKOnly,
                                              lowLevelClient, earlyEncode, persistenceType == PERSIST_FOREVER,
                                              overrideSplitfileCryptoKey, container, context);
        }
    }
    ...
    public void start(ObjectContainer container, ClientContext context) {
        ...
        try {
            ...
            if (logMINOR) {
                Logger.minor(this, "Started " + putter + " for " + this + " persistence=" + persistenceType);
            }
    ...
    protected void freeData(ObjectContainer container) {
        if (logMINOR) {
            Logger.minor(this, "freeData() on " + this + " persistence type = " + persistenceType);
        }
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 ...
                                 + ((persistenceType != PERSIST_FOREVER)
                                    ? "" : (" dir.active=" + container.ext().isActive(this))), new Exception("error"));
                }
        ...
        if (logMINOR) {
            Logger.minor(this, "freeData() more on " + this + " persistence type = " + persistenceType);
        }
    ...
    private void freeData(HashMap<String, Object> manifestElements, ObjectContainer container) {
        if (logMINOR) {
            Logger.minor(this,
                         "freeData() inner on " + this + " persistence type = " + persistenceType + " size = "
    ...
    protected FCPMessage persistentTagMessage(ObjectContainer container) {
        ...
        // FIXME end
        return new PersistentPutDir(identifier, publicURI, uri, verbosity, priorityClass, persistenceType, global,
                                    defaultName, manifestElements, clientToken, started, ctx.maxInsertRetries,
                                    ctx.dontCompress, ctx.compressorDescriptor, wasDiskPut, isRealTime(),
                                    (putter != null) ? putter.getSplitfileCryptoKey() : null, container);
    }
    ...
    RequestStatus getStatus(ObjectContainer container) {
        ...
        if (finalURI != null) {
            finalURI = getFinalURI(container).clone();
        }
        ...
        return new UploadDirRequestStatus(identifier, persistenceType, started, finished, succeeded, total, min,
