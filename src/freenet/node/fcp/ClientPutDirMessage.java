...
public abstract class ClientPutDirMessage extends BaseDataCarryingMessage {
    ...
    public ClientPutDirMessage(SimpleFieldSet fs) throws MessageInvalidException {
        ...
        if (s == null) {
            ...
            try {
                ...
                try {
                    ...
                    throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
                                                      "Invalid CompatibilityMode (not a valid number)", identifier,
                                                      global);
                }
            }
        }
        ...
        if ("simple".equalsIgnoreCase(manifestPutter)) {
            ...
            throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
                                              "Invalid ManifestPutter value: " + manifestPutter, identifier, global);
        }

        manifestPutterType = manifestType;

        if ((manifestPutterType != ManifestPutter.MANIFEST_SIMPLEPUTTER)
                && ((persistenceType != ClientRequest.PERSIST_CONNECTION)
                    && (persistenceType != ClientRequest.PERSIST_REBOOT))) {
            throw new MessageInvalidException(
                ProtocolErrorMessage.INVALID_FIELD,
                "Only Connection or Reboot persistenace can only be used with the default ManifestPutter", identifier,
                global);
        }
