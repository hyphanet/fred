    ...
    public ClientGetMessage(SimpleFieldSet fs) throws MessageInvalidException {
        ...
        if (verbosityString == null) {
            ...
            try {
                ...
                throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
                                                  "Error parsing Verbosity field: " + e.getMessage(), identifier,
                                                  global);
            }
        }
        ...
        if (returnType == RETURN_TYPE_DIRECT) {
            ...
            if (!diskFile.getAbsoluteFile().getParentFile().equals(tempFile.getAbsoluteFile().getParentFile())) {
                throw new MessageInvalidException(ProtocolErrorMessage.FILENAME_AND_TEMP_FILENAME_MUST_BE_IN_SAME_DIR,
                                                  null, identifier, global);
            }

            if (tempFile.exists()) {
                throw new MessageInvalidException(ProtocolErrorMessage.DISK_TARGET_EXISTS, "Temp file exists",
                                                  identifier, global);
            }
            ...
            try {
                ...
                if (!(tempFile.createNewFile() || (tempFile.exists() && tempFile.canRead() && tempFile.canWrite()))) {
                    throw new MessageInvalidException(ProtocolErrorMessage.COULD_NOT_CREATE_FILE,
                                                      "Could not create temp file " + tempFile, identifier, global);
                }
            ...
            try {
                ...
                if (!RequestStarter.isValidPriorityClass(priorityClass)) {
                    throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
                                                      ...
                                                      + RequestStarter.MINIMUM_PRIORITY_CLASS + " to "
                                                      + RequestStarter.MAXIMUM_PRIORITY_CLASS, identifier, global);
                }
            } catch (NumberFormatException e) {
                throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
                                                  "Error parsing PriorityClass field: " + e.getMessage(), identifier,
                                                  global);
            }
        }
        ...
        if ((persistenceString == null) || persistenceString.equalsIgnoreCase("connection")) {
            ...
            throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
                                              "Error parsing Persistence field: " + persistenceString, identifier,
                                              global);
        }
