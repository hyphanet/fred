...
public class FCPClient {
    ...
    public FCPClient(String name2, FCPConnectionHandler handler, boolean isGlobalQueue, RequestCompletionCallback cb,
                     short persistenceType, FCPPersistentRoot root, ObjectContainer container) {
        this.name = name2;
    ...
    public void finishedClientRequest(ClientRequest get, ObjectContainer container) {
        ...
        if (statusCache != null) {
            if (get instanceof ClientGet) {
                ...
                statusCache.finishedDownload(get.identifier, get.hasSucceeded(),
                                             ((ClientGet) get).getDataSize(container),
                                             ((ClientGet) get).getMIMEType(container), failureCode, longFailMessage,
                                             shortFailMessage, shadow, download.filterData(container));
                ...
                statusCache.finishedUpload(upload.getIdentifier(), upload.hasSucceeded(),
                                           upload.getGeneratedURI(container), failureCode, shortFailMessage,
                                           longFailMessage);
            } else {
                assert(false);
            }
    ...
    public int queuePendingMessagesOnConnectionRestart(FCPConnectionOutputHandler outputHandler,
            ObjectContainer container, int offset, int max) {
        assert((persistenceType == ClientRequest.PERSIST_FOREVER) == (container != null));
...
//                  container.activate(req, 1);
            ...
            if (req == null) {
                for (ClientRequest r : completedUnackedRequests) {
                    ...
                    if (r.getIdentifier().equals(identifier)) {
                        ...
                        Logger.error(this,
                                     "Found completed unacked request " + r + " for identifier " + r.getIdentifier()
                ...
                if (req == null) {
                    for (ClientRequest r : runningPersistentRequests) {
                        ...
                        if (r.getIdentifier().equals(identifier)) {
                            ...
                            Logger.error(this,
                                         "Found running request " + r + " for identifier " + r.getIdentifier()
                                         + " but not in clientRequestsByIdentifier!!");
            ...
            } else if (!((removedFromRunning = runningPersistentRequests.remove(req))
    ...
    public void addPersistentRequests(List<ClientRequest> v, boolean onlyForever, ObjectContainer container) {
        ...
        synchronized (this) {
            for (ClientRequest req : runningPersistentRequests) {
                ...
                if (req == null) {
                    Logger.error(this,
                                 "Request is null on runningPersistentRequests for " + this
                                 + " - database corruption??");
        ...
        if (isGlobalQueue) {
            ...
            if (clients != null) {
                for (FCPClient client : clients) {
                    ...
                    if (client.persistenceType != persistenceType) {
                        continue;
                    }
