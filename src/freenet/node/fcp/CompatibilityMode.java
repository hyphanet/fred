...
public class CompatibilityMode extends FCPMessage {
    ...
    CompatibilityMode(String id, boolean global, long min, long max, byte[] cryptoKey, boolean dontCompress,
                      boolean definitive) {
        this.identifier = id;
        this.global = global;
        this.min = min;
    ...
    void merge(long min, long max, byte[] cryptoKey, boolean dontCompress, boolean definitive) {
        ...
        if ((max < this.max) || (this.max == InsertContext.CompatibilityMode.COMPAT_UNKNOWN.ordinal())) {
            this.max = max;
        }
    ...
    public InsertContext.CompatibilityMode[] getModes() {
        return new InsertContext.CompatibilityMode[] { InsertContext.CompatibilityMode.values()[(int) min],
