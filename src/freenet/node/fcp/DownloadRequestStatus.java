...
public class DownloadRequestStatus extends RequestStatus {
    ...
    DownloadRequestStatus(String identifier, short persistence, boolean started, boolean finished, boolean success,
                          int total, int min, int fetched, int fatal, int failed, boolean totalFinalized, long last,
                          short prio,    // all these passed to parent
        ...
        if ((mimeType == null)
            ...
            Logger.error(this,
                         "MIME type is null but failure code is " + FetchException.getMessage(failureCode) + " for "
                         + identifier + " : " + uri, new Exception("error"));
        }
    ...
    synchronized void setFinished(boolean success, long dataSize, String mimeType, int failureCode,
                                  String failureReasonLong, String failureReasonShort, Bucket dataShadow,
                                  boolean filtered) {
        setFinished(success);
        ...
        if ((mimeType == null)
            ...
            Logger.error(this,
                         "MIME type is null but failure code is " + FetchException.getMessage(failureCode) + " for "
                         + getIdentifier() + " : " + uri, new Exception("error"));
        }
