...
public abstract class PeerNode
	implements USKRetrieverCallback, BasePeerNode, PeerNodeUnlocked {
	...
	public PeerNode(
		...
		this.backedOffPercentRT = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		this.backedOffPercentBulk = new TimeDecayingRunningAverage(
			0.0,
			180000,
			0.0,
			1.0,
		...
		if (negTypes == null || negTypes.length == 0) {
			if (fromAnonymousInitiator()) negTypes =
				outgoingMangler.supportedNegTypes(false); // Assume compatible. Anonymous initiator = short-lived, and we already connected so we know we are.
			else throw new FSParseException("No negTypes!");
		}

		if (
			fs.getBoolean("opennet", false) != isOpennetForNoderef()
		) throw new FSParseException(
		...
		if (identityString == null && isDarknet()) throw new PeerParseException(
			"No identity!"
		);
		try {
			if (identityString != null) {
				...
				identity = SHA256.digest(
					DSAPublicKey.create(sfs, Global.DSAgroupBigA).asBytes()
				);
			}
		...
		for (int i = 0; i < incomingSetupKey.length; i++) incomingSetupKey[i] =
			(byte) (nodeKey[i] ^ identityHashHash[i]);
		outgoingSetupKey = new byte[digestLength];
		for (int i = 0; i < outgoingSetupKey.length; i++) outgoingSetupKey[i] =
			(byte) (nodeKeyHash[i] ^ identityHash[i]);
		...
		swapRequestsInterval = new SimpleRunningAverage(
			50,
			Node.MIN_INTERVAL_BETWEEN_INCOMING_SWAP_REQUESTS
		);
		probeRequestsInterval = new SimpleRunningAverage(
			50,
		...
		pRejected = new TimeDecayingRunningAverage(
			0,
			MINUTES.toMillis(4),
			0.0,
			1.0,
		...
		if (fromLocal) {
			...
			if (metadata != null) {
				...
				hadRoutableConnectionCount = metadata.getLong(
					"hadRoutableConnectionCount",
					0
				);
				routableConnectionCheckCount = metadata.getLong(
		...
		if (fromLocal) innerCalcNextHandshake(false, false, now); // Let them connect so we can recognise we are NATed
	...
	private boolean parseARK(
		...
		try {
			...
			Logger.error(
				this,
				"Couldn't parse ARK info for " + this + ": " + e,
				e
			);
		} catch (NumberFormatException e) {
			Logger.error(
	 ...
	 */
	private Peer[] updateHandshakeIPs(
		Peer[] localHandshakeIPs,
		boolean ignoreHostnames
	) {
		for (Peer localHandshakeIP : localHandshakeIPs) {
			if (ignoreHostnames) {
	...
	public void maybeUpdateHandshakeIPs(boolean ignoreHostnames) {
		...
		synchronized (this) {
			...
			if (
				(now - lastAttemptedHandshakeIPUpdateTime) < MINUTES.toMillis(5)
			) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Updating handshake IPs for peer '" +
			shortToString() +
			"' (" +
			ignoreHostnames +
			')'
		...
		if (myNominalPeer.length == 0) {
			if (localDetectedPeer == null) {
				...
				if (logMINOR) Logger.minor(
					this,
					"1: maybeUpdateHandshakeIPs got a result of: " +
					handshakeIPsToString()
				);
				return;
			}
			localHandshakeIPs = new Peer[] { localDetectedPeer };
			localHandshakeIPs = updateHandshakeIPs(
				localHandshakeIPs,
				ignoreHostnames
			);
			synchronized (this) {
			...
			if (logMINOR) Logger.minor(
				this,
				"2: maybeUpdateHandshakeIPs got a result of: " +
				handshakeIPsToString()
			);
			return;
		}
		...
		for (Peer p : myNominalPeer) {
			...
			for (Peer nodePeer : nodePeers) {
				...
				if (myAddr.equals(addr)) {
					if (!addedLocalhost) localPeers.add(
						new Peer(localhost, p.getPort())
					);
					addedLocalhost = true;
				}
			}
		...
		if (logMINOR) {
			if (localDetectedPeer != null) Logger.minor(
				this,
				"3: detectedPeer = " +
				localDetectedPeer +
				" (" +
			...
			Logger.minor(
				this,
				"3: maybeUpdateHandshakeIPs got a result of: " +
				handshakeIPsToString()
			);
		}
	}
	...
	public boolean shouldBeExcludedFromPeerList() {
		...
		synchronized (this) {
			if (
				...
				BLACK_MAGIC_BACKOFF_PRUNING_TIME + now <
	...
	synchronized boolean isInMandatoryBackoff(long now, boolean realTime) {
		long mandatoryBackoffUntil = realTime
			? mandatoryBackoffUntilRT
			: mandatoryBackoffUntilBulk;
	...
	public MessageItem sendAsync(
		...
		MessageItem item = new MessageItem(
			msg,
			cb == null ? null : new AsyncMessageCallback[] { cb },
			ctr
		);
		long now = System.currentTimeMillis();
	...
	public boolean unqueueMessage(MessageItem message) {
		if (logMINOR) Logger.minor(
			this,
			"Unqueueing message on " + this + " : " + message
		);
	...
	public long getProbableSendQueueTime() {
		...
		if (shouldThrottle()) bandwidth = Math.min(
			bandwidth,
			node.getOutputBandwidthLimit() / 2
		);
	...
	public void maybeRekey() {
		...
		synchronized (this) {
			...
			shouldDisconnect = (timeWhenRekeyingShouldOccur +
					FNPPacketMangler.MAX_SESSION_KEY_REKEYING_DELAY <
				now) &&
			isRekeying;
	...
	public void startRekeying() {
		...
		Logger.normal(
			this,
			"We are asking for the key to be renewed (" +
			this.detectedPeer +
			')'
		);
	}
	...
	public boolean disconnected(
		...
		if (isRealConnection()) Logger.normal(
			this,
			"Disconnected " + this,
			new Exception("debug")
		);
		...
		if (messagesTellDisconnected != null) {
			if (logMINOR) Logger.minor(
				this,
				"Messages to dump: " + messagesTellDisconnected.length
			);
		...
		if (moreMessagesTellDisconnected != null) {
			if (logMINOR) Logger.minor(
				this,
				"Messages to dump: " + moreMessagesTellDisconnected.size()
			);
	...
	public long getNextUrgentTime(long now) {
		...
		if (pf != null) {
			...
			if (canSend) { // New messages are only sent on cur.
				...
				if (t >= now && l < now && logMINOR) Logger.minor(
					this,
					"Next urgent time from message queue less than now"
				);
				else if (logDEBUG) Logger.debug(
					this,
					"Next urgent time is " + (l - now) + "ms on " + this
				);
				t = l;
			}
			long l = pf.timeNextUrgent(canSend, now);
			if (l < now && logMINOR) Logger.minor(
				this,
				"Next urgent time from packet format less than now on " + this
			);
			t = Math.min(t, l);
		}
		return t;
	...
	public boolean shouldSendHandshake() {
		...
		synchronized (this) {
			...
			tempShouldSendHandshake = ((now > sendHandshakeTime) &&
				(handshakeIPs != null) &&
				(isRekeying || !isConnected()));
		}
		if (logMINOR) Logger.minor(
			this,
			"shouldSendHandshake(): initial = " + tempShouldSendHandshake
		);
		if (
			tempShouldSendHandshake && (hasLiveHandshake(now))
		...
		if (logMINOR) Logger.minor(
			this,
			"shouldSendHandshake(): final = " + tempShouldSendHandshake
		);
	...
	public boolean hasLiveHandshake(long now) {
		...
		if (c != null && logDEBUG) Logger.minor(
			this,
			"Last used (handshake): " + (now - c.lastUsedTime())
		);
		return !(
			(c == null) || (now - c.lastUsedTime() > Node.HANDSHAKE_TIMEOUT)
		);
	}
	...
	protected boolean innerCalcNextHandshake(
		boolean successfulHandshakeSend,
		...
		synchronized (this) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Next handshake in " + delay + " on " + this
			);
	...
	private synchronized boolean calcNextHandshakeBurstOnly(long now) {
		...
		if (listeningHandshakeBurstCount == 0) { // 0 only if we just reset it above
			...
			node.random.nextInt(
				Node.RANDOMIZED_TIME_BETWEEN_BURSTING_HANDSHAKE_BURSTS
			);
			listeningHandshakeBurstSize =
				Node.MIN_BURSTING_HANDSHAKE_BURST_SIZE +
				node.random.nextInt(
					Node.RANDOMIZED_BURSTING_HANDSHAKE_BURST_SIZE
				);
			isBursting = false;
		} else {
			delay = Node.MIN_TIME_BETWEEN_HANDSHAKE_SENDS +
	...
	protected void calcNextHandshake(
		...
		long now = System.currentTimeMillis();
		boolean fetchARKFlag = false;
		fetchARKFlag = innerCalcNextHandshake(
			successfulHandshakeSend,
			dontFetchARK,
			now
		);
		if (!notRegistered) setPeerNodeStatus(now); // Because of isBursting being set above and it can't hurt others
		...
		if (fetchARKFlag && !dontFetchARK) {
			...
			if (
				(arkFetcherStartTime2 - arkFetcherStartTime1) > 500
			) Logger.normal(
				this,
	...
	public boolean isBurstOnly() {
		...
		if (
			status == AddressTracker.Status.DEFINITELY_NATED ||
		...
		if (now - timeSetBurstNow > UPDATE_BURST_NOW_PERIOD) {
			burstNow = (node.random.nextInt(P_BURST_IF_DEFINITELY_FORWARDED) ==
				0);
			timeSetBurstNow = now;
		}
		return burstNow;
	...
	public boolean ping(int pingID) throws NotConnectedException {
		...
		try {
			...
			throw new NotConnectedException(
				"Disconnected while waiting for pong"
			);
		}
		return msg != null;
	}
	...
	public void sendSync(Message req, ByteCounter ctr, boolean realTime)
		...
		if (!cb.done) {
			Logger.warning(
				this,
				"Waited too long for a blocking send for " +
				req +
				" to " +
				PeerNode.this,
				new Exception("error")
	...
	private void setDetectedPeer(Peer newPeer) {
		...
		if (newPeer == null) {
			Logger.error(
				this,
				"Impossible: No address for detected peer! " + p + " on " + this
			);
			return;
		}
		synchronized (this) {
			...
			if (
				(newPeer != null) &&
				((oldPeer == null) || !oldPeer.equals(newPeer))
			) {
	...
	private void updateShortToString() {
		shortToString = super.toString() +
		'@' +
		detectedPeer +
		'@' +
		HexUtil.bytesToHex(peerECDSAPubKeyHash);
	}
		...
		synchronized (this) {
			if ((!isConnected()) && (!dontLog)) {
				...
				if (
					...
					this,
					"Received packet while disconnected!: " + this,
					new Exception("error")
				);
				else if (logMINOR) Logger.minor(
					this,
					"Received packet while disconnected on " +
					this +
	...
	public synchronized void setKeyAgreementSchemeContext(
		KeyAgreementSchemeContext ctx2
	) {
		this.ctx = ctx2;
		if (logMINOR) Logger.minor(
			this,
	...
	public long completedHandshake(
		...
		if (
			!(isSeed() && this instanceof SeedServerPeerNode)
		...
		try {
			...
			Logger.error(
				this,
				"Failed to parse new noderef for " + this + ": " + e1,
				e1
			);
			node.peers.disconnected(this);
			return -1;
		...
		if (isSeed()) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Not routing traffic to " + this + " it's for announcement."
			);
		} else if (bogusNoderef) {
			Logger.normal(
				this,
				"Not routing traffic to " + this + " - bogus noderef"
			);
			routable = false;
		...
		if (forwardInvalidVersion()) {
			...
			Logger.normal(
				this,
				"Not routing traffic to " + this + " - clock problems"
			);
			routable = false;
		} else older = false;
		changedIP(replyTo);
		...
		synchronized (this) {
			...
			if (currentTracker != null) {
				if (
					Arrays.equals(outgoingKey, currentTracker.outgoingKey) &&
					Arrays.equals(incommingKey, currentTracker.incommingKey)
				) {
					Logger.error(
						this,
						"completedHandshake() with identical key to current, maybe replayed JFK(4)?"
					);
					return -1;
				}
			}
			if (previousTracker != null) {
				if (
					Arrays.equals(outgoingKey, previousTracker.outgoingKey) &&
					Arrays.equals(incommingKey, previousTracker.incommingKey)
				) {
					Logger.error(
						this,
						"completedHandshake() with identical key to previous, maybe replayed JFK(4)?"
					);
					return -1;
				}
			}
			if (unverifiedTracker != null) {
				if (
					Arrays.equals(outgoingKey, unverifiedTracker.outgoingKey) &&
			...
			if (bootIDChanged && wasARekey) {
				...
				countSelectionsSinceConnected = 0;
				sentInitialMessages = false;
			} else if (bootIDChanged && logMINOR) Logger.minor(
				this,
				"Changed boot ID from " +
				oldBootID +
				" to " +
				thisBootID +
				" for " +
			...
			if (unverified) {
				if (unverifiedTracker != null) {
					...
					if (previousTracker == null) previousTracker =
						unverifiedTracker;
				}
			...
			if (
				...
				previousTracker != null &&
				Arrays.equals(
					currentTracker.outgoingKey,
					previousTracker.outgoingKey
				) &&
				Arrays.equals(
					currentTracker.incommingKey,
					previousTracker.incommingKey
				)
			) Logger.error(
				this,
			...
			if (
				...
				Arrays.equals(
					previousTracker.outgoingKey,
					unverifiedTracker.outgoingKey
				) &&
				Arrays.equals(
					previousTracker.incommingKey,
					unverifiedTracker.incommingKey
				)
			) Logger.error(
				this,
			...
			if (packetFormat == null) {
				packetFormat = new NewPacketFormat(
					this,
					ourInitialMsgID,
					theirInitialMsgID
				);
	...
	void startARKFetcher() {
		...
		synchronized (arkFetcherSync) {
			...
			if (arkFetcher == null) {
				Logger.minor(
					this,
					"Starting ARK fetcher for " + this + " : " + myARK
				);
				arkFetcher = node.clientCore.uskManager.subscribeContent(
					myARK,
					this,
					true,
	...
	protected void stopARKFetcher() {
		...
		synchronized (arkFetcherSync) {
			if (arkFetcher == null) {
				if (logMINOR) Logger.minor(
					this,
					"ARK fetcher not running for " + this
				);
				return;
			}
			ret = arkFetcher;
		...
		node.executor.execute(
			new Runnable() {
				@Override
				public void run() {
					node.clientCore.uskManager.unsubscribeContent(
						myARK,
						unsub,
						true
					);
	...
	protected void sendInitialMessages() {
		...
		Message locMsg = DMT.createFNPLocChangeNotificationNew(
			node.lm.getLocation(),
			node.peers.getPeerLocationDoubles(true)
		);
		Message ipMsg = DMT.createFNPDetectedIPAddress(detectedPeer);
		Message timeMsg = DMT.createFNPTime(System.currentTimeMillis());
		Message dRoutingMsg = DMT.createRoutingStatus(
			!disableRoutingHasBeenSetLocally
		);
		Message uptimeMsg = DMT.createFNPUptime(
			(byte) (int) (100 * node.uptime.getUptime())
		);

		try {
			if (isRealConnection()) sendAsync(
				locMsg,
				null,
	...
	private synchronized boolean invalidVersion() {
		return (
			bogusNoderef || forwardInvalidVersion() || reverseInvalidVersion()
		);
	}
	...
	private synchronized boolean reverseInvalidVersion() {
		...
		return !Version.checkArbitraryGoodVersion(
			Version.getVersionString(),
			lastGoodVersion
		);
	}
	...
	static SimpleFieldSet compressedNoderefToFieldSet(
		byte[] data,
		int offset,
		int length
	) throws FSParseException {
		...
		if ((firstByte & 1) == 1) {
			try {
				...
				if (logMINOR) Logger.minor(
					PeerNode.class,
					"We have decompressed a " + length + " bytes big reference."
				);
		...
		if (logMINOR) Logger.minor(
			PeerNode.class,
			"Reference: " +
			HexUtil.bytesToHex(data, offset, length) +
			'(' +
			length +
			')'
		);
	...
	protected synchronized boolean innerProcessNewNoderef(
		...
		if (forFullNodeRef) {
			// Check the signature.
			try {
				if (!verifyReferenceSignature(fs)) throw new FSParseException(
					"Invalid signature"
				);
		...
		if (s == null && forFullNodeRef) throw new FSParseException(
			"No opennet ref"
		);
		else if (s != null) {
			try {
				boolean b = Fields.stringToBool(s);
				if (b != isOpennetForNoderef()) throw new FSParseException(
		...
		if (identityString == null && forFullNodeRef) {
			if (isDarknet()) throw new FSParseException("No identity!");
				...
				this,
				"didn't send an identity;" + " let's assume it's pre-1471"
			);
		} else if (identityString != null) {
			try {
				byte[] id = Base64.decode(identityString);
				if (!Arrays.equals(id, identity)) throw new FSParseException(
		...
		if (newVersion == null) {
			...
			if (!forARK && !forDiffNodeRef) throw new FSParseException(
				"No version"
			);
		} else {
		...
		} else if (forFullNodeRef) throw new FSParseException(
			"No lastGoodVersion"
		);
		...
		if (locationString != null) {
			...
			if (!Location.isValid(newLoc)) {
				...
				if (!Location.equals(oldLoc, newLoc)) {
					if (!Location.isValid(oldLoc)) shouldUpdatePeerCounts =
						true;
		...
		try {
			...
			if (physical != null) {
				...
				Peer[] oldPeers = oldNominalPeer.toArray(
					new Peer[oldNominalPeer.size()]
				);
				...
				if (forFullNodeRef) throw new FSParseException(
					"ARK noderef has no physical.udp"
				);
			}
		} catch (Exception e1) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Parsed successfully; changedAnything = " + changedAnything
		);
		...
		if (sfs != null) {
			...
			if (
				...
				"ecdsa.P256.pub is not the right size!"
			);
			ECPublicKey key = ECDSA.getPublicKey(pub, ECDSA.Curves.P256);
			if (key == null) throw new FSParseException(
				"ecdsa.P256.pub is invalid!"
	...
	public synchronized SimpleFieldSet exportMetadataFieldSet(long now) {
		...
		if (detectedPeer != null) fs.putSingle(
			"detected.udp",
			detectedPeer.toStringPrefNumeric()
		);
		if (lastReceivedPacketTime() > 0) fs.put(
			"timeLastReceivedPacket",
			timeLastReceivedPacket
		);
		if (lastReceivedAckTime() > 0) fs.put(
			"timeLastReceivedAck",
			timeLastReceivedAck
		);
		long timeLastConnected = isConnected.getTimeLastTrue(now);
		if (timeLastConnected > 0) fs.put(
			"timeLastConnected",
			timeLastConnected
		);
		if (timeLastRoutable() > 0) fs.put(
			"timeLastRoutable",
			timeLastRoutable
		);
		if (getPeerAddedTime() > 0 && shouldExportPeerAddedTime()) fs.put(
			"peerAddedTime",
			peerAddedTime
		);
		if (neverConnected) fs.putSingle("neverConnected", "true");
		if (hadRoutableConnectionCount > 0) fs.put(
			"hadRoutableConnectionCount",
			hadRoutableConnectionCount
		);
		if (routableConnectionCheckCount > 0) fs.put(
			"routableConnectionCheckCount",
			routableConnectionCheckCount
		);
	...
	public SimpleFieldSet exportVolatileFieldSet() {
		...
		synchronized (this) {
			...
			if (
				idle > SECONDS.toMillis(60) && -1 != lastReceivedPacketTime()
			) fs.put("idle", idle);
			...
			fs.putSingle(
				"lastRoutingBackoffReasonBulk",
				lastRoutingBackoffReasonBulk
			);
			fs.put(
				"routingBackoffPercent",
				backedOffPercent.currentValue() * 100
			);
			fs.put(
				"routingBackoffRT",
				Math.max(
			...
			fs.put(
				...
				getPercentTimeRoutableConnection() * 100
			);
		}
	...
	public synchronized SimpleFieldSet exportFieldSet() {
		SimpleFieldSet fs = new SimpleFieldSet(true);
		if (getLastGoodVersion() != null) fs.putSingle(
			"lastGoodVersion",
			lastGoodVersion
		);
		for (int i = 0; i < nominalPeer.size(); i++) fs.putAppend(
			"physical.udp",
	...
	public boolean isRoutingBackedOff(
		...
		synchronized (this) {
			...
			if (now < routingBackedOffUntil) {
				if (
					routingBackedOffUntil - now >= ignoreBackoffUnder
				) return true;
			}
			...
			if (now < transferBackedOffUntil) {
				if (
					transferBackedOffUntil - now >= ignoreBackoffUnder
				) return true;
			}
	...
	public boolean isRoutingBackedOff(boolean realTime) {
		...
		synchronized (this) {
			...
			long transferBackedOffUntil = realTime
				...
				: transferBackedOffUntilBulk;
			if (
				now < routingBackedOffUntil || now < transferBackedOffUntil
			) return true;
			pingTime = averagePingTime();
		}
		if (pingTime > maxPeerPingTime()) return true;
	...
	public boolean isRoutingBackedOffEither() {
		...
		double pingTime;
		synchronized (this) {
			long routingBackedOffUntil = Math.max(
				routingBackedOffUntilRT,
				routingBackedOffUntilBulk
			);
			long transferBackedOffUntil = Math.max(
				transferBackedOffUntilRT,
				transferBackedOffUntilBulk
			);
			if (
	...
	static final int INITIAL_TRANSFER_BACKOFF_LENGTH = (int) SECONDS.toMillis(
		30
	); // 60 seconds, but it starts at twice this.
	...
	static final int INITIAL_MANDATORY_BACKOFF_LENGTH = (int) SECONDS.toMillis(
		1
	);
	...
	public void enterMandatoryBackoff(String reason, boolean realTime) {
		...
		synchronized (this) {
			long mandatoryBackoffUntil = realTime
				? mandatoryBackoffUntilRT
				: mandatoryBackoffUntilBulk;
			int mandatoryBackoffLength = realTime
				? mandatoryBackoffLengthRT
				: mandatoryBackoffLengthBulk;
			if (
				mandatoryBackoffUntil > -1 && mandatoryBackoffUntil > now
			) return;
			Logger.error(
				this,
				"Entering mandatory backoff for " +
				this +
				(realTime ? " (realtime)" : " (bulk)")
			);
			mandatoryBackoffUntil = now +
			...
			node.nodeStats.reportMandatoryBackoff(
				reason,
				mandatoryBackoffUntil - now,
				realTime
			);
	...
	public synchronized void resetMandatoryBackoff(boolean realTime) {
		if (realTime) mandatoryBackoffLengthRT =
			INITIAL_MANDATORY_BACKOFF_LENGTH;
				...
				if (now > routingBackedOffUntilRT) { // not backed off
					if (lastSampleTime > routingBackedOffUntilRT) { // last sample after last backoff
						...
						if (routingBackedOffUntilRT > 0) {
							report = (double) (routingBackedOffUntilRT -
								lastSampleTime) /
				...
				if (now > routingBackedOffUntilBulk) { // not backed off
					if (lastSampleTime > routingBackedOffUntilBulk) { // last sample after last backoff
						...
						if (routingBackedOffUntilBulk > 0) {
							double myReport =
								(double) (routingBackedOffUntilBulk -
									lastSampleTime) /
								(double) (now - lastSampleTime);
	...
	public void localRejectedOverload(String reason, boolean realTime) {
		...
		synchronized (this) {
			...
			long routingBackedOffUntil = realTime
				? routingBackedOffUntilRT
				: routingBackedOffUntilBulk;
			int routingBackoffLength = realTime
			...
			if (now > routingBackedOffUntil) {
				...
				if (logMINOR) {
					...
					if (0 < reason.length()) reasonWrapper = " because of '" +
					reason +
					'\'';
					Logger.minor(
						this,
	...
	public void transferFailed(String reason, boolean realTime) {
		...
		synchronized (this) {
			...
			long transferBackedOffUntil = realTime
				? transferBackedOffUntilRT
				: transferBackedOffUntilBulk;
			int transferBackoffLength = realTime
				? transferBackoffLengthRT
				: transferBackoffLengthBulk;
			if (now > transferBackedOffUntil) {
				transferBackoffLength = transferBackoffLength *
				TRANSFER_BACKOFF_MULTIPLIER;
				if (
	...
	public void transferSuccess(boolean realTime) {
		...
		synchronized (this) {
			...
			if (
				...
				if (realTime) transferBackoffLengthRT =
					INITIAL_TRANSFER_BACKOFF_LENGTH;
				else transferBackoffLengthBulk =
					INITIAL_TRANSFER_BACKOFF_LENGTH;
				if (logMINOR) Logger.minor(
					this,
					"Resetting transfer backoff on " + peer
				);
			} else {
				if (logMINOR) Logger.minor(
					this,
	...
	public synchronized String getPreviousBackoffReason(boolean realTime) {
		return realTime
			? previousRoutingBackoffReasonRT
			: previousRoutingBackoffReasonBulk;
	}
	...
	public void gotARK(SimpleFieldSet fs, long fetchedEdition) {
		try {
			synchronized (this) {
				...
				if (myARK.suggestedEdition < fetchedEdition + 1) myARK =
					myARK.copy(fetchedEdition + 1);
			}
	...
	public static String getPeerNodeStatusString(int status) {
		if (
			status == PeerManager.PEER_NODE_STATUS_CONNECTED
		) return "CONNECTED";
		if (
		...
		if (
			status == PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED
		...
		if (
			status == PeerManager.PEER_NODE_STATUS_LISTEN_ONLY
		) return "LISTEN ONLY";
		if (
			status == PeerManager.PEER_NODE_STATUS_LISTENING
		) return "LISTENING";
	...
	public static String getPeerNodeStatusCSSClassName(int status) {
		if (
			...
			status == PeerManager.PEER_NODE_STATUS_DISABLED
		) return "peer_disabled";
		else if (
			status == PeerManager.PEER_NODE_STATUS_ROUTING_DISABLED
		) return "peer_routing_disabled";
		else if (
			status == PeerManager.PEER_NODE_STATUS_BURSTING
	...
	protected synchronized int getPeerNodeStatus(
		...
		if (isRoutable()) { // Function use also updates timeLastConnected and timeLastRoutable
			if (noLoadStats) peerNodeStatus =
				...
				if (
					now < routingBackedOffUntilRT ||
					overPingTime ||
					isInMandatoryBackoff(now, true)
				) {
					peerNodeStatus =
						PeerManager.PEER_NODE_STATUS_ROUTING_BACKED_OFF;
					if (
						!lastRoutingBackoffReasonRT.equals(
							previousRoutingBackoffReasonRT
						) ||
						...
						peers.addPeerNodeRoutingBackoffReason(
							lastRoutingBackoffReasonRT,
							this,
							true
						);
				...
				if (
					overPingTime &&
					(lastRoutingBackoffReasonBulk == null ||
						now >= routingBackedOffUntilBulk)
				) {
				...
				if (
					now < routingBackedOffUntilBulk ||
					overPingTime ||
					isInMandatoryBackoff(now, false)
				) {
					peerNodeStatus =
						PeerManager.PEER_NODE_STATUS_ROUTING_BACKED_OFF;
					if (
						!lastRoutingBackoffReasonBulk.equals(
							previousRoutingBackoffReasonBulk
						) ||
		...
		} else if (isConnected && bogusNoderef) peerNodeStatus =
		...
		else if (isConnected && unroutableOlderVersion) peerNodeStatus =
			PeerManager.PEER_NODE_STATUS_TOO_OLD;
		else if (isConnected && disableRouting) peerNodeStatus =
			PeerManager.PEER_NODE_STATUS_ROUTING_DISABLED;
		else if (
			isConnected && Math.abs(clockDelta) > MAX_CLOCK_DELTA
		) peerNodeStatus = PeerManager.PEER_NODE_STATUS_CLOCK_PROBLEM;
		else if (neverConnected) peerNodeStatus =
			PeerManager.PEER_NODE_STATUS_NEVER_CONNECTED;
	...
	public int setPeerNodeStatus(long now, boolean noLog) {
		...
		synchronized (this) {
			...
			peerNodeStatus = getPeerNodeStatus(
				now,
		...
		if (peerNodeStatus == PeerManager.PEER_NODE_STATUS_ROUTING_BACKED_OFF) {
			long delta =
				...
				now +
				1;
			if (delta > 0) node.ticker.queueTimedJob(
				checkStatusAfterBackoff,
				"Update status for " + this,
				delta,
				true,
				true
			);
	...
	private boolean noLoadStats() {
		if (
			node.enableNewLoadManagement(false) ||
			node.enableNewLoadManagement(true)
		) {
			if (outputLoadTrackerRealTime.getLastIncomingLoadStats() == null) {
				if (isRoutable()) Logger.normal(
					this,
					"No realtime load stats on " + this
				);
				return true;
			}
	...
	public synchronized boolean noLongerRoutable() {
		if (
			unroutableNewerVersion || unroutableOlderVersion || disableRouting
		) return true;
		return false;
	}
	...
	public synchronized double getPercentTimeRoutableConnection() {
		...
		return (
			((double) hadRoutableConnectionCount) / routableConnectionCheckCount
		);
	}
	...
	private final PacketThrottle _lastThrottle = new PacketThrottle(
		Node.PACKET_SIZE
	);
	...
	public void offer(Key key) {
		...
		byte[] authenticator = HMAC.macWithSHA256(
	...
	public static PeerNode create(
		...
		throws FSParseException, PeerParseException, ReferenceSignatureVerificationException, PeerTooOldException {
		if (crypto.isOpennet) return new OpennetPeerNode(
			fs,
			node2,
			crypto,
			opennet,
	...
	public synchronized boolean shouldAcceptAnnounce(long uid) {
		long now = System.currentTimeMillis();
		if (
			runningAnnounceUIDs.length < MAX_SIMULTANEOUS_ANNOUNCEMENTS &&
			now - timeLastAcceptedAnnouncement > MAX_ANNOUNCE_DELAY
		) {
			long[] newList = new long[runningAnnounceUIDs.length + 1];
			if (runningAnnounceUIDs.length > 0) System.arraycopy(
				runningAnnounceUIDs,
				0,
				newList,
				0,
	...
	public synchronized boolean completedAnnounce(long uid) {
		...
		for (long l : runningAnnounceUIDs) {
			...
			if (x == newList.length) {
				Logger.warning(
					this,
					"UID not found in completedAnnounce, should not happen",
					new Exception("debug")
				);
		...
		if (x < newList.length) {
			Logger.error(
				this,
				"Duplicated UID, should not happen",
				new Exception("debug")
			);
	...
	public Peer getHandshakeIP() {
		...
		if ((secondTime - firstTime) > 1000) Logger.error(
			this,
		...
		if (localHandshakeIPs.length == 0) {
			...
			if ((thirdTime - secondTime) > 1000) Logger.error(
				this,
		...
		for (Peer peer : localHandshakeIPs) {
			...
			if (
				!peer.isRealInternetAddress(false, false, allowLocalAddresses)
			) {
				if (logMINOR) Logger.minor(
					this,
					"Not sending handshake to " +
			...
			if (!isConnected()) {
				...
				if (!outgoingMangler.allowConnection(this, addr)) {
					if (logMINOR) Logger.minor(
						this,
						"Not sending handshake packet to " +
						peer +
						" for " +
						this
					);
		...
		if ((loopTime2 - loopTime1) > 1000) Logger.normal(
			this,
	...
	public void sendNodeToNodeMessage(
		...
		if (isDarknet() && queueOnNotConnected) {
			...
			cb = new UnqueueMessageOnAckCallback(
	...
	public synchronized double selectionRate() {
		long timeSinceConnected =
			System.currentTimeMillis() - this.connectedTime;
	...
	public long getReusableTrackerID() {
		...
		if (logMINOR) Logger.minor(
			this,
			"getReusableTrackerID(): " + cur.trackerID + " on " + this
		);
	...
	public void registerPeerNodeStatusChangeListener(
		PeerManager.PeerStatusChangeListener listener
	) {
	...
	class LoadSender {
		...
		public void onSetPeerAllocation(
			...
			synchronized (this) {
				...
				if (now - timeLastSentAllocationNotice > 5000) {
					if (logMINOR) Logger.minor(
						this,
						"Last sent allocation " +
						TimeUtil.formatTime(now - timeLastSentAllocationNotice)
					);
					mustSend = true;
				} else {
		...
		public void onSetMaxOutputTransfers(int maxOutputTransfers) {
			synchronized (this) {
				...
				if (
					lastSentMaxOutputTransfers == Integer.MAX_VALUE ||
					lastSentMaxOutputTransfers == 0
				) {
					sendASAP = true;
				} else if (
					maxOutputTransfers > lastSentMaxOutputTransfers * 1.05 ||
		...
		public void onSetMaxOutputTransfersPeerLimit(
			int maxOutputTransfersPeerLimit
		) {
			synchronized (this) {
				if (
					maxOutputTransfersPeerLimit ==
					lastSentMaxOutputTransfersPeerLimit
				) return;
				if (
					lastSentMaxOutputTransfersPeerLimit == Integer.MAX_VALUE ||
					lastSentMaxOutputTransfersPeerLimit == 0
				) {
					sendASAP = true;
				} else if (
					maxOutputTransfersPeerLimit >
						lastSentMaxOutputTransfersPeerLimit * 1.05 ||
					maxOutputTransfersPeerLimit <
						lastSentMaxOutputTransfersPeerLimit * 0.9
				) {
					sendASAP = true;
				}
			}
		...
		Message makeLoadStats(
			long now,
			int transfersPerInsert,
			boolean noRemember
		) {
			PeerLoadStats stats = node.nodeStats.createPeerLoadStats(
				PeerNode.this,
				transfersPerInsert,
				realTimeFlag
			);
			synchronized (this) {
				...
				if (!noRemember) {
					if (
						lastFullStats != null && lastFullStats.equals(stats)
					) return null;
					lastFullStats = stats;
				}
	...
	public void onSetMaxOutputTransfers(
		boolean realTime,
		int maxOutputTransfers
	) {
		(realTime
				? loadSenderRealTime
				: loadSenderBulk).onSetMaxOutputTransfers(maxOutputTransfers);
	}
	...
	public void onSetMaxOutputTransfersPeerLimit(
		...
		(realTime
				? loadSenderRealTime
				: loadSenderBulk).onSetMaxOutputTransfersPeerLimit(
				maxOutputTransfers
			);
	}
	...
	public void onSetPeerAllocation(
		...
		(realTime ? loadSenderRealTime : loadSenderBulk).onSetPeerAllocation(
				input,
				thisAllocation,
				transfersPerInsert
			);
	...
	public static class SlotWaiter {
		 ...
		 * can be unlocked. */
		synchronized PeerNode[] innerOnWaited(
			PeerNode peer,
			RequestLikelyAcceptedState state
		) {
			if (logMINOR) Logger.minor(
				this,
				"Waking slot waiter " + this + " on " + peer
			);
			if (acceptedBy != null) {
			...
			if (!waitingFor.contains(peer)) {
				if (logMINOR) Logger.minor(
					this,
					"Not waiting for peer " + peer + " on " + this
				);
				if (acceptedBy != peer) {
					if (offeredKey) tag.removeFetchingOfferedKeyFrom(peer);
					else tag.removeRoutingTo(peer);
		...
		void unregister(PeerNode exclude, PeerNode[] all) {
			if (all == null) return;
			for (PeerNode p : all) if (p != exclude) p
				.outputLoadTracker(realTime)
				.unqueueSlotWaiter(this);
		}
		...
		void onFailed(PeerNode peer, boolean reallyFailed) {
			if (logMINOR) Logger.minor(
				this,
				"onFailed() on " + this + " reallyFailed=" + reallyFailed
			);
			synchronized (this) {
				if (acceptedBy != null) {
					if (logMINOR) Logger.minor(
						this,
						"Already matched on " + this
					);
					return;
				}
				// Always wake up.
		...
		public PeerNode waitForAny(long maxWait, boolean timeOutIsFatal)
			...
			for (PeerNode p : all) {
				if (
					(!p.isRoutable()) || p.isInMandatoryBackoff(now, realTime)
				) {
					if (logMINOR) Logger.minor(
						this,
						"Peer is not valid in waitForAny(): " + p
					);
					continue;
				}
				anyValid = true;
				RequestLikelyAcceptedState accept = p
					.outputLoadTracker(realTime)
					.tryRouteTo(
						tag,
				...
				if (accept != null) {
					if (logMINOR) Logger.minor(
						this,
						"tryRouteTo() pre-wait check returned " + accept
					);
					PeerNode[] unreg;
					PeerNode other = null;
					synchronized (this) {
						...
						if (other == null) {
							if (logMINOR) Logger.minor(
								this,
								"Trying the original tryRouteTo() on " + this
							);
			...
			if (!anyValid) {
				...
				if (logMINOR) Logger.minor(
					this,
					"None valid to wait for on " + this
				);
				unregister(ret, all);
				if (f != null && ret == null) throw f;
				tag.clearWaitingForSlot();
			...
			synchronized (this) {
				...
				while (
					acceptedBy == null && (!waitingFor.isEmpty()) && !failed
				) {
					try {
						if (maxWait == Long.MAX_VALUE) wait();
						else {
							int wait = (int) Math.min(
								Integer.MAX_VALUE,
							...
							if (logMINOR) Logger.minor(
								this,
								"Maximum wait time exceeded on " + this
							);
							if (shouldGrab()) {
				...
				if (!timedOut) {
					...
					if (waitEnd - waitStart > (realTime ? 6000 : 60000)) {
						Logger.warning(
							this,
							"Waited " + (waitEnd - waitStart) + "ms for " + this
						);
					} else if (
						waitEnd - waitStart > (realTime ? 1000 : 10000)
					) {
						Logger.normal(
							this,
							"Waited " + (waitEnd - waitStart) + "ms for " + this
			...
			if (timeOutIsFatal && all != null) {
				for (PeerNode pn : all) {
					pn
						.outputLoadTracker(realTime)
						.reportFatalTimeoutInWait(isLocal());
				}
			}
			unregister(ret, all);
			return ret;
	...
	static class SlotWaiterList {
		...
		public synchronized void remove(SlotWaiter waiter) {
			...
			if (map == null) {
				if (logMINOR) Logger.minor(
					this,
					"SlotWaiter " + waiter + " was not queued"
				);
				return;
			}
	...
	private static final RequestType[] RequestType_values =
		RequestType.values();
	...
	class OutputLoadTracker {
		...
		public synchronized double proportionTimingOutFatallyInWait() {
			...
			return (
				(double) totalFatalTimeouts /
				((double) (totalFatalTimeouts + totalAllocated))
			);
		}
		...
		public IncomingLoadSummaryStats getIncomingLoadStats() {
			...
			RunningRequestsSnapshot runningRequests =
				node.nodeStats.getRunningRequestsTo(
					PeerNode.this,
					loadStats.averageTransfersOutPerInsert,
					realTime
				);
			RunningRequestsSnapshot otherRunningRequests =
				loadStats.getOtherRunningRequests();
			...
			return new IncomingLoadSummaryStats(
				...
				otherRunningRequests.calculate(
					ignoreLocalVsRemoteBandwidthLiability,
					false
				),
				otherRunningRequests.calculate(
		...
		public RequestLikelyAcceptedState tryRouteTo(
			UIDTag tag,
			RequestLikelyAcceptedState worstAcceptable,
			boolean offeredKey
		) {
			PeerLoadStats loadStats;
			...
			if (
				isInMandatoryBackoff(System.currentTimeMillis(), realTime)
			) return null;
			synchronized (routedToLock) {
				loadStats = lastIncomingLoadStats;
				if (loadStats == null) {
				...
				if (dontSendUnlessGuaranteed) worstAcceptable =
				...
				if (
					acceptState.ordinal() > worstAcceptable.ordinal()
				) return null;
				if (
		...
		boolean queueSlotWaiter(SlotWaiter waiter) {
			...
			if (isInMandatoryBackoff(System.currentTimeMillis(), realTime)) {
				if (logMINOR) Logger.minor(
					this,
					"In mandatory backoff, so not queueing"
				);
				return false;
			}
			...
			synchronized (routedToLock) {
				...
				if (!noLoadStats) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Not waiting for " + this + " as no load stats"
					);
					all = waiter.innerOnWaited(
						PeerNode.this,
						RequestLikelyAcceptedState.UNKNOWN
					);
				}
			}
			if (all != null) {
				...
				if (
					...
					if (logMINOR) Logger.minor(
						this,
						"Queued but not routable or in mandatory backoff, failing"
					);
		...
		private void failSlotWaiters(boolean reallyFailed) {
			for (RequestType type : RequestType_values) {
				...
				for (SlotWaiter w : slots.values()) w.onFailed(
					PeerNode.this,
					reallyFailed
				);
			}
		...
		private void maybeNotifySlotWaiter() {
			...
			while (true) {
				...
				for (int i = 0; i < RequestType_values.length; i++) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Checking slot waiter list for " + type
					);
					SlotWaiter slot;
					RequestLikelyAcceptedState acceptState;
					...
					synchronized (routedToLock) {
						...
						if (list == null) {
							...
							typeNum++;
							if (typeNum == RequestType_values.length) typeNum =
								0;
							continue;
						}
						if (list.isEmpty()) {
						...
						if (logMINOR) Logger.minor(
							this,
							"Checking slot waiters for " + type
						);
						foundNone = false;
						// Requests already running to this node
						RunningRequestsSnapshot runningRequests =
						...
						if (
							acceptState == null ||
							acceptState == RequestLikelyAcceptedState.UNLIKELY
						) {
							if (logMINOR) Logger.minor(
								this,
								"Accept state is " +
								acceptState +
								" - not waking up - type is " +
								type
							);
							return;
						}
						if (
							dontSendUnlessGuaranteed &&
							acceptState != RequestLikelyAcceptedState.GUARANTEED
						) {
							if (logMINOR) Logger.minor(
								this,
						...
						peersForSuccessfulSlot = slot.innerOnWaited(
							PeerNode.this,
							acceptState
						);
		...
		private RequestLikelyAcceptedState getRequestLikelyAcceptedStateBandwidth(
			...
			double ourUsage = runningRequests.calculate(
				ignoreLocalVsRemote,
				input
			);
			if (logMINOR) Logger.minor(
		...
		private RequestLikelyAcceptedState getRequestLikelyAcceptedStateTransfers(
			...
			int maxTransfersOutPeerLimit = Math.min(
				stats.maxTransfersOutPeerLimit,
				stats.maxTransfersOut
			);
			if (logMINOR) Logger.minor(
				this,
				"Our usage is " +
			...
			if (
				ourUsage < maxTransfersOutPeerLimit
			) return RequestLikelyAcceptedState.GUARANTEED;
			otherRunningRequests.log(PeerNode.this);
			int theirUsage = otherRunningRequests.totalOutTransfers();
			if (logMINOR) Logger.minor(this, "Their usage is " + theirUsage);
			if (
				ourUsage + theirUsage < stats.maxTransfersOutLowerLimit
			) return RequestLikelyAcceptedState.LIKELY;
			else return RequestLikelyAcceptedState.UNLIKELY;
		}
	...
	public void noLongerRoutingTo(UIDTag tag, boolean offeredKey) {
		if (
			offeredKey && !(tag instanceof RequestTag)
		) throw new IllegalArgumentException(
			"Only requests can have offeredKey=true"
		);
		synchronized (routedToLock) {
	...
	public boolean handleReceivedPacket(
		byte[] buf,
		int offset,
		int length,
		long now,
	...
	public synchronized boolean matchesIP(
	...
	class MyDecodingMessageGroup implements DecodingMessageGroup {
		...
		public void processDecryptedMessage(
			...
			Message m = node.usm.decodeSingleMessage(
				data,
				offset,
				length,
				PeerNode.this,
	...
	public boolean isLowCapacity(boolean isRealtime) {
		PeerLoadStats stats = outputLoadTracker(
			isRealtime
		).getLastIncomingLoadStats();
		if (stats == null) return false;
		NodePinger pinger = node.nodeStats.nodePinger;
		if (pinger == null) return false; // FIXME possible?
		if (
			pinger.capacityThreshold(isRealtime, true) > stats.peerLimit(true)
		) return true;
		if (
			pinger.capacityThreshold(isRealtime, false) > stats.peerLimit(false)
		) return true;
		return false;
	}
	...
	public void reportRoutedTo(
		...
		(isLocal
				? node.nodeStats.routingMissDistanceLocal
				: node.nodeStats.routingMissDistanceRemote).report(distance);
		(realTime
				? node.nodeStats.routingMissDistanceRT
				: node.nodeStats.routingMissDistanceBulk).report(distance);
	...
	public synchronized void finishedSendingUOMJar(boolean isExt) {
		if (isExt) {
			sendingUOMLegacyExtJar = false;
			if (!(sendingUOMMainJar || uomCount > 0)) lastSentUOM =
				System.currentTimeMillis();
		} else {
			sendingUOMMainJar = false;
			if (!(sendingUOMLegacyExtJar || uomCount > 0)) lastSentUOM =
				System.currentTimeMillis();
		}
	}
	...
	public synchronized void decrementUOMSends() {
		uomCount--;
		if (
			uomCount == 0 && (!sendingUOMMainJar) && (!sendingUOMLegacyExtJar)
	...
	public synchronized boolean throttleRekey() {
		...
		if (now - lastIncomingRekey < THROTTLE_REKEY) {
			Logger.error(
				this,
				"Two rekeys initiated by other side within " +
				THROTTLE_REKEY +
				"ms"
			);
			return true;
	...
	public int calculateMaxTransfersOut(
		...
		return (int) Math.max(
			1,
			Math.min(packetsPerSecond * timeout, Integer.MAX_VALUE)
		);
	}
	...
	public void rejectedGuaranteed(boolean realTimeFlag) {
		synchronized (this) {
			if (realTimeFlag) {
				...
				if (
					consecutiveGuaranteedRejectsRT !=
					CONSECUTIVE_REJECTS_MANDATORY_BACKOFF
				) {
					return;
				}
	...
	public int getThrottleWindowSize() {
		...
		if (throttle != null) return (int) (Math.min(
	...
	private boolean verifyReferenceSignature(SimpleFieldSet fs)
		...
		try {
			...
			byte[] toVerifyECDSA = fs
				.toOrderedString()
				.getBytes(StandardCharsets.UTF_8);

			boolean isECDSAsigPresent =
				(signatureP256 != null && peerECDSAPubKey != null);
			boolean verifyECDSA = false; // assume it failed.

			// Is there a new ECDSA sig?
			if (isECDSAsigPresent) {
				fs.putSingle("sigP256", signatureP256);
				verifyECDSA = ECDSA.verify(
					Curves.P256,
					peerECDSAPubKey,
					Base64.decode(signatureP256),
					toVerifyECDSA
				);
			}
