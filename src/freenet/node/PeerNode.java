...
public abstract class PeerNode implements USKRetrieverCallback, BasePeerNode, PeerNodeUnlocked {
    ...
    public PeerNode(SimpleFieldSet fs, Node node2, NodeCrypto crypto, PeerManager peers, boolean fromLocal,
                    boolean fromAnonymousInitiator, OutgoingPacketMangler mangler, boolean isOpennet)
        ...
        if (fs.getBoolean("opennet", false) != isOpennet) {
            throw new FSParseException(
                "Trying to parse a darknet peer as opennet or an opennet peer as darknet isOpennet=" + isOpennet
                + " boolean = " + fs.getBoolean("opennet", false) + " string = \"" + fs.get("opennet") + "\"");
        }
        ...
        if (nominalPeer.isEmpty()) {
            Logger.normal(this,
                          "No IP addresses found for identity '" + identityAsBase64String + "', possibly at location '"
                          + location + ": " + userToString());
            detectedPeer = null;
        } else {
        ...
        if (fromLocal) {
            ...
            if (metadata != null) {
                ...
                peerAddedTime = metadata.getLong("peerAddedTime", 0    // missing peerAddedTime is normal: Not only do exported refs not include it, opennet peers don't either.
                    );
    ...
    private synchronized boolean parseECDSA(SimpleFieldSet fs, boolean fromAnonymousInitiator, boolean startup)
        ...
        if (sfs == null) {
            ...
            if (peerECDSAPubKey == null) {
                ...
                Logger.error(this,
                             "Tried to change ECDSA key on " + userToString()
    ...
    private boolean parseARK(SimpleFieldSet fs, boolean onStartup, boolean forDiffNodeRef) {
        ...
        try {
            ...
            if ((arkPubKey == null) && (arkNo <= -1)) {
                ...
                Logger.error(this,
                             "Got a differential node reference from " + this
    ...
    private Peer[] updateHandshakeIPs(Peer[] localHandshakeIPs, boolean ignoreHostnames) {
        for (Peer localHandshakeIP : localHandshakeIPs) {
            if (ignoreHostnames) {
                ...
                if (logMINOR) {
                    Logger.debug(this,
                                 "updateHandshakeIPs: calling getAddress(false) on Peer '" + localHandshakeIP
                ...
                if (logMINOR) {
                    Logger.debug(this,
                                 "updateHandshakeIPs: calling getHandshakeAddress() on Peer '" + localHandshakeIP
                                 + "' for " + shortToString() + " (" + ignoreHostnames + ')');
                }
    ...
    public MessageItem sendAsync(Message msg, AsyncMessageCallback cb, ByteCounter ctr) throws NotConnectedException {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Sending async: " + msg + " : " + cb + " on " + this + " for " + node.getDarknetPortNumber()
    ...
    public void requeueMessageItems(MessageItem[] messages, int offset, int length, boolean dontLog, String reason) {
        ...
        if (!dontLog) {
            ...
            if (!rateLimitLogging) {
                ...
                Logger.normal(this,
                              "Requeueing " + messages.length + " messages" + reasonWrapper + " on " + this
                              + rateLimitWrapper);
            }
        }
    ...
    public boolean ping(int pingID) throws NotConnectedException {
        ...
        try {
            msg = node.usm.waitFor(
                MessageFilter.create().setTimeout(2000).setType(DMT.FNPPong).setField(DMT.PING_SEQNO, pingID), null);
    ...
    public void sendSync(Message req, ByteCounter ctr, boolean realTime)
        ...
        if (!cb.done) {
            ...
            if (!messageQueue.removeMessage(item)) {
                ...
                if (!cb.done) {
                    Logger.error(this,
                                 "Waited too long for blocking send and then could not unqueue for " + req + " to "
    ...
    public long completedHandshake(long thisBootID, byte[] data, int offset, int length, BlockCipher outgoingCipher,
                                   byte[] outgoingKey, BlockCipher incommingCipher, byte[] incommingKey, Peer replyTo,
                                   boolean unverified, int negType, long trackerID, boolean isJFK4,
                                   boolean jfk4SameAsOld, byte[] hmacKey, BlockCipher ivCipher, byte[] ivNonce,
                                   int ourInitialSeqNum, int theirInitialSeqNum, int ourInitialMsgID,
        ...
        if (isSeed()) {
            ...
            Logger.normal(this,
                          "Not routing traffic to " + this + " - reverse invalid version " + Version.getVersionString()
                          + " for peer's lastGoodversion: " + getLastGoodVersion());
            newer = true;
        } else {
            newer = false;
        }
        ...
        synchronized (this) {
            ...
            if (bootIDChanged && wasARekey) {
                ...
                Logger.normal(this,
                              "Changed boot ID while rekeying! from " + oldBootID + " to " + thisBootID + " for "
                              + getPeer());
                wasARekey = false;
                connectedTime = now;
                countSelectionsSinceConnected = 0;
            ...
            newTracker = new SessionKey(this, outgoingCipher, outgoingKey, incommingCipher, incommingKey, ivCipher,
                                        ivNonce, hmacKey,
                                        new NewPacketFormatKeyContext(ourInitialSeqNum, theirInitialSeqNum), trackerID);

            if (logMINOR) {
                Logger.minor(this,
                             "New key tracker in completedHandshake: " + newTracker + " for " + shortToString()
                             + " neg type " + negType);
            }
            ...
            if ((currentTracker != null) && (previousTracker != null)
                ...
                Logger.error(this,
                             "currentTracker key equals previousTracker key: cur " + currentTracker + " prev "
                             + previousTracker);
            }
            ...
            if ((previousTracker != null) && (unverifiedTracker != null)
                ...
                Logger.error(this,
                             "previousTracker key equals unverifiedTracker key: prev " + previousTracker + " unv "
                             + unverifiedTracker);
            }
        ...
        Logger.normal(this,
                      "Completed handshake with " + this + " on " + replyTo + " - current: " + currentTracker
    ...
    protected void sendInitialMessages() {
        ...
        try {
            ...
            Logger.error(this,
                         "Completed handshake with " + getPeer() + " but disconnected (" + isConnected + ':'
                         + currentTracker + "!!!: " + e, e);
        }
        ...
        if ((firstByte & 0x2) == 2) {
            ...
            if (logMINOR) {
                Logger.minor(PeerNode.class,
                             "DSAGroup set to " + group.fingerprintToString() + " using the group-index " + groupIndex);
            }
        }
    ...
    protected synchronized boolean innerProcessNewNoderef(SimpleFieldSet fs, boolean forARK, boolean forDiffNodeRef,
        ...
        if ((s == null) && forFullNodeRef) {
            ...
            try {
                ...
                if (b != (isOpennet() || isSeed())) {
                    throw new FSParseException("Changed opennet status?!?!?!? expected=" + isOpennet() + " but got "
                                               + b + " (" + s + ") on " + this);
                }
            } catch (NumberFormatException e) {
        ...
        try {
            ...
            if (physical != null) {
                ...
                Logger.error(this,
                             "ARK noderef has no physical.udp for " + this + " : forDiffNodeRef=" + forDiffNodeRef
                             + " forARK=" + forARK);
    ...
    public void enterMandatoryBackoff(String reason, boolean realTime) {
        ...
        synchronized (this) {
            ...
            mandatoryBackoffUntil = now + (mandatoryBackoffLength / 2)
                                    + node.fastWeakRandom.nextInt((int) (mandatoryBackoffLength / 2));
    ...
    public void localRejectedOverload(String reason, boolean realTime) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Local rejected overload (" + reason + ") on " + this + " : pRejected="
        ...
        synchronized (this) {
            ...
            if (now > routingBackedOffUntil) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Ignoring localRejectedOverload: " + (routingBackedOffUntil - now)
    ...
    public void successNotOverload(boolean realTime) {
        ...
        synchronized (this) {
            ...
            if (now > (until = realTime ? routingBackedOffUntilRT : routingBackedOffUntilBulk)) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Ignoring successNotOverload: " + (until - now)
                                 + "ms remaining on routing backoff on " + peer);
                }
        ...
        synchronized (this) {
            ...
            if (now > transferBackedOffUntil) {
                ...
                if (logMINOR) {
                    ...
                    Logger.minor(this,
                                 "Backing off (transfer)" + reasonWrapper + ": transferBackoffLength="
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Ignoring transfer failure: " + (transferBackedOffUntil - now)
    ...
    public void transferSuccess(boolean realTime) {
        ...
        synchronized (this) {
            ...
            if (now > (until = realTime ? transferBackedOffUntilRT : transferBackedOffUntilBulk)) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Ignoring transfer success: " + (until - now) + "ms remaining on transfer backoff on "
                                 + peer);
                }
    ...
    public void reportThrottledPacketSendTime(long timeDiff, boolean realTime) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Reporting throttled packet send time: " + timeDiff + " to " + getPeer() + " ("
    ...
    public void onFound(USK origUSK, long edition, FetchResult result) {
        ...
        try {
            ...
            // Corrupt ref.
            Logger.error(this,
                         "Corrupt ARK reference? Fetched " + myARK.copy(edition) + " got while parsing: " + e
                         + " from:\n" + ref, e);
        }
    ...
    public static PeerNode create(SimpleFieldSet fs, Node node2, NodeCrypto crypto, OpennetManager opennet,
                                  PeerManager manager, OutgoingPacketMangler mangler)
            throws FSParseException, PeerParseException, ReferenceSignatureVerificationException {
        if (crypto.isOpennet) {
    ...
    public Peer getHandshakeIP() {
        ...
        if (!shouldSendHandshake()) {
            if (logMINOR) {
                Logger.minor(this,
                             "Not sending handshake to " + getPeer()
        ...
        if ((secondTime - firstTime) > 1000) {
            Logger.error(this,
                         "getHandshakeIPs() took more than a second to execute (" + (secondTime - firstTime)
        ...
        for (Peer peer : localHandshakeIPs) {
            ...
            if (peer.getAddress(false) == null) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not sending handshake to " + peer + " for " + getPeer()
        ...
        if ((loopTime2 - loopTime1) > 1000) {
            Logger.normal(this,
                          "loopTime2 is more than a second after loopTime1 (" + (loopTime2 - loopTime1)
                          + ") working on " + userToString());
        }
        ...
        synchronized (this) {
            ...
            if (!reportedRTT) {
                ...
                if (oldRTO > RTO) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Received first packet after backing off on resend. RTO is " + RTO + " but was "
                                     + oldRTO);
                    }
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Reported ping " + t + " avg is now " + pingAverage.currentValue() + " RTO is " + RTO
    ...
    public synchronized void backoffOnResend() {
        ...
        if (consecutiveRTOBackoffs > MAX_CONSECUTIVE_RTO_BACKOFFS) {
            Logger.warning(this,
                           "Resetting RTO for " + this + " after " + consecutiveRTOBackoffs
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Backed off on resend, RTO is now " + RTO + " for " + shortToString()
...
//      return new MessageItem(msg, null, node.nodeStats.allocationNoticesCounter, boostPriority ? DMT.PRIORITY_NOW : (short)-1);
    ...
    public void reportRoutedTo(double target, boolean isLocal, boolean realTime, PeerNode prev,
        ...
        if ((peersLocation != null) && (shallWeRouteAccordingToOurPeersLocation())) {
            for (double l : peersLocation) {
                ...
                if ((Math.abs(l - myLoc) < Double.MIN_VALUE * 2) || (Math.abs(l - prevLoc) < Double.MIN_VALUE * 2)) {
                    ignoreLoc = true;
                } else {
    ...
    private boolean verifyReferenceSignature(SimpleFieldSet fs) throws ReferenceSignatureVerificationException {
        ...
        try {
            ...
            if (isDSAsigPresent) {
                ...
                verifyDSA = DSA.verify(peerPubKey, new DSASignature(signature),
                                       new BigInteger(1, SHA256.digest(toVerifyDSA)), false);
            }
            ...
            if (failed) {
                ...
                Logger.error(this,
                             "The integrity of the reference has been compromised!" + errCause + " fs was\n"
                             + fs.toOrderedString());
            ...
            throw new ReferenceSignatureVerificationException(
                "The node reference you added is invalid: It does not have a valid DSA signature.");
        } catch (IllegalBase64Exception e) {
            Logger.error(this, "Invalid reference: " + e, e);
    ...
    public class IncomingLoadSummaryStats {
        ...
        public IncomingLoadSummaryStats(int totalRequests, double outputBandwidthPeerLimit,
                                        double inputBandwidthPeerLimit, double outputBandwidthTotalLimit,
                                        double inputBandwidthTotalLimit, double usedOutput, double usedInput,
                                        double othersUsedOutput, double othersUsedInput) {
    ...
    class LoadSender {
        ...
        Message makeLoadStats(long now, int transfersPerInsert, boolean noRemember) {
            ...
            synchronized (this) {
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Sending allocation notice to " + this + " allocation is " + lastSentAllocationInput
    ...
    class MyDecodingMessageGroup implements DecodingMessageGroup {
        ...
        public void processDecryptedMessage(byte[] data, int offset, int length, int overhead) {
            ...
            if (m == null) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Message not decoded from " + PeerNode.this + " (" + PeerNode.this.getVersionNumber()
                                 + ")");
                }
    ...
    class OutputLoadTracker {
        ...
        private final EnumMap<RequestType, SlotWaiterList> slotWaiters = new EnumMap<RequestType,
                                                                             SlotWaiterList>(RequestType.class);
        ...
        public RequestLikelyAcceptedState tryRouteTo(UIDTag tag, RequestLikelyAcceptedState worstAcceptable,
            ...
            synchronized (routedToLock) {
                ...
                if (loadStats == null) {
                    Logger.error(this,
                                 "Accepting because no load stats from " + PeerNode.this.shortToString() + " ("
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Predicted acceptance state for request: " + acceptState + " must beat "
                                 + worstAcceptable);
                }
        ...
        boolean queueSlotWaiter(SlotWaiter waiter) {
            ...
            synchronized (routedToLock) {
                ...
                if (!noLoadStats) {
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Queued slot " + waiter + " waiter for " + waiter.requestType + " on " + list
        ...
        private void maybeNotifySlotWaiter() {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Maybe waking up slot waiters for " + this + " realtime=" + realTime + " for "
                ...
                for (int i = 0; i < RequestType_values.length; i++) {
                    ...
                    synchronized (routedToLock) {
                        ...
                        if (dontSendUnlessGuaranteed && (acceptState != RequestLikelyAcceptedState.GUARANTEED)) {
                            if (logMINOR) {
                                Logger.minor(this,
                                             "Not accepting until guaranteed for " + PeerNode.this + " realtime="
                                             + realTime);
                            }
         ...
         */
        private RequestLikelyAcceptedState getRequestLikelyAcceptedState(RunningRequestsSnapshot runningRequests,
                RunningRequestsSnapshot otherRunningRequests, boolean ignoreLocalVsRemote, PeerLoadStats stats) {
        ...
        private RequestLikelyAcceptedState getRequestLikelyAcceptedStateBandwidth(boolean input,
                RunningRequestsSnapshot runningRequests, RunningRequestsSnapshot otherRunningRequests,
                boolean ignoreLocalVsRemote, PeerLoadStats stats) {
            double ourUsage = runningRequests.calculate(ignoreLocalVsRemote, input);

            if (logMINOR) {
                Logger.minor(this,
                             "Our usage is " + ourUsage + " peer limit is " + stats.peerLimit(input)
                             + " lower limit is " + stats.lowerLimit(input) + " realtime " + realTime + " input "
                             + input);
            }
        ...
        private RequestLikelyAcceptedState getRequestLikelyAcceptedStateTransfers(
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Our usage is " + ourUsage + " peer limit is " + maxTransfersOutPeerLimit
        ...
        public void clearDontSendUnlessGuaranteed() {
            synchronized (routedToLock) {
                if (dontSendUnlessGuaranteed) {
                    Logger.error(this,
                                 "Clearing don't-send-unless-guaranteed for " + PeerNode.this + " realtime="
                                 + realTime);
    ...
    public static class SlotWaiter {
        ...
        public PeerNode waitForAny(long maxWait, boolean timeOutIsFatal) throws SlotWaiterFailedException {
            ...
            for (PeerNode p : all) {
                ...
                if (accept != null) {
                    ...
                    synchronized (this) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "tryRouteTo() succeeded to " + p + " on " + this + " with " + accept
                    ...
                    if (other != null) {
                        Logger.normal(this,
                                      "Race condition: tryRouteTo() succeeded on " + p.shortToString()
            ...
            synchronized (this) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Waiting for any node to wake up " + this + " : "
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Returning after waiting: accepted by " + acceptedBy + " waiting for "
        ...
        private synchronized PeerNode grab() {
            if (logMINOR) {
                Logger.minor(this,
                             "Returning in first check: accepted by " + acceptedBy + " waiting for "
    ...
    static class SlotWaiterList {
        private final LinkedHashMap<PeerNode, TreeMap<Long, SlotWaiter>> lru = new LinkedHashMap<PeerNode,
                                                                                   TreeMap<Long, SlotWaiter>>();
            ...
            synchronized (this) {
                if (!done) {
                    if (!sent) {
                        ...
                        Logger.normal(this,
                                      "Acknowledged but not sent?! on " + this + " for " + PeerNode.this
                                      + " - lag ???");
                    }
                } else {
                    return;
