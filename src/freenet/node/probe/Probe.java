...
public class Probe implements ByteCounter {
    ...
    private void request(final Message message, final PeerNode source, final Listener listener) {
        ...
        if (htl < 1) {
            if (logWARNING) {
                Logger.warning(Probe.class,
                               "Received out-of-bounds HTL of " + htl + " from " + source.getIdentityString() + " ("
                               + source.userToString() + "); discarding.");
            }

            return;
        } else if (htl > MAX_HTL) {
            if (logMINOR) {
                Logger.minor(Probe.class,
                             "Received out-of-bounds HTL of " + htl + " from " + source.getIdentityString() + " ("
    ...
    private static MessageFilter createResponseFilter(final Type type, final PeerNode candidate, final long uid,
        ...
        filter.or(createFilter(candidate, uid,
                               timeout).setType(DMT.ProbeRefused).or(createFilter(candidate, uid,
                                   timeout).setType(DMT.ProbeError)));
