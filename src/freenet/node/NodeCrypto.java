...
public class NodeCrypto {
    ...
    SimpleFieldSet exportPublicFieldSet(boolean forSetup, boolean forAnonInitiator, boolean forARK) {
        ...
        if (!forAnonInitiator) {
            ...
            synchronized (referenceSync) {
                if ((myReferenceSignature == null) || (myReferenceECDSASignature == null)
    ...
    byte[] sign(byte[] hash) {
        ...
        System.arraycopy(s.getSBytes(Node.SIGNATURE_PARAMETER_LENGTH), 0, sig, Node.SIGNATURE_PARAMETER_LENGTH,
    ...
    public boolean allowConnection(PeerNode pn, FreenetInetAddress addr) {
        if (config.oneConnectionPerAddress()) {
            ...
            if (node.peers.anyConnectedPeerHasAddress(addr, pn) &&!detector.includes(addr)
                ...
                Logger.normal(this,
                              "Not sending handshake packets to " + addr + " for " + pn
            ...
            if (pn.crypto.config.oneConnectionPerAddress()) {
                if (pn instanceof DarknetPeerNode) {
                    ...
                    System.out.println("Disconnecting permanently from your friend \""
                                       + ((DarknetPeerNode) pn).getName() + "\" because your friend \""
                                       + ((DarknetPeerNode) peerNode).getName() + "\" is using the same IP address "
                                       + address + "!");
                }
    ...
    public long getNodeHandle(ObjectContainer setupContainer) {
        ...
        if (result.hasNext()) {
            ...
            while (true) {
                ...
                System.err.println("Generated and stored database handle for node on port " + portNumber + ": "
                                   + handle);
