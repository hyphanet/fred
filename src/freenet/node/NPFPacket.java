...
class NPFPacket {
        ...
        while (offset < plaintext.length) {
            ...
            if ((offset + fragmentLength) > plaintext.length) {
                Logger.error(NPFPacket.class,
                             "Fragment doesn't fit in the received packet: offset is " + offset
                             + " fragment length is " + fragmentLength + " plaintext length is " + plaintext.length
                             + " message length " + messageLength + " message ID " + messageID
    ...
    public boolean addAck(int ack, int maxPacketSize) {
        ...
        if (useCumulativeAcks) {
            ...
            if (acks.size() == 0) {
                ...
                if ((acks.first() - ack) > 255) {
                    return false;
                }
            } else if (ack > acks.last()) {
                if ((ack - acks.last()) > 255) {
                    return false;
                }
            }
    ...
    public void onSent(int totalPacketLength, BasePeerNode pn) {
        ...
        if (logDEBUG) {
            Logger.debug(this,
                         "Total packet overhead: " + overhead + " for " + size + " messages total message length "
                         + totalMessageData + " total packet length " + totalPacketLength + " biggest message "
                         + biggest);
        }
