...
class NPFPacket {
	...
	public static NPFPacket create(byte[] plaintext, BasePeerNode pn) {
		...
		if (pn == null) throw new IllegalArgumentException(
			"Can't estimate an ack type of received packet"
		);
		int offset = 0;
		...
		while (offset < plaintext.length) {
			...
			if ((plaintext[offset] & 0x10) != 0) {
				...
				if (prevFragmentID == -1) {
					Logger.warning(
						NPFPacket.class,
						"First fragment doesn't have full message id"
					);
					packet.error = true;
					return packet;
				}
				messageID = prevFragmentID +
				(((plaintext[offset] & 0x0F) << 8) |
	...
	private static int tryParseLossyMessages(
		NPFPacket packet,
		byte[] plaintext,
		int offset
	) {
	...
	public int toBytes(byte[] buf, int offset, Random paddingGen) {
		...
		for (MessageFragment fragment : fragments) {
			if (fragment.shortMessage) buf[offset] = (byte) ((buf[offset] &
					0xFF) |
				0x80);
			if (fragment.isFragmented) buf[offset] = (byte) ((buf[offset] &
					0xFF) |
				0x40);
			if (fragment.firstFragment) buf[offset] = (byte) ((buf[offset] &
					0xFF) |
				0x20);

			if (
				prevFragmentID == -1 ||
				(fragment.messageID - prevFragmentID >= 4096)
			) {
				buf[offset] = (byte) ((buf[offset] & 0xFF) | 0x10);
				buf[offset] = (byte) ((buf[offset] & 0xFF) |
					((fragment.messageID >>> 24) & 0x0F));
			...
			if (fragment.isFragmented) {
				...
				int value = fragment.firstFragment
	...
	public boolean addAck(int ack, int maxPacketSize) {
		if (ack < 0) throw new IllegalArgumentException(
			"Got negative ack: " + ack
		);
	...
	public int addMessageFragment(MessageFragment frag) {
		...
		for (MessageFragment fragment : fragments) {
			if (
				(prevMessageID == -1) ||
				(fragment.messageID - prevMessageID >= 4096)
			) {
				msgIDLength += 2;
			}
