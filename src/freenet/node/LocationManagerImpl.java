 ...
 */
class LocationManagerImpl implements ByteCounter, LocationManager, ProtectedLocationManager {
    ...
    public class MyCallback extends SendMessageOnErrorCallback {
        ...
        public MyCallback(Message message, PeerNode pn, RecentlyForwardedItem item) {
            super(message, pn, LocationManagerImpl.this);
            this.item = item;
        }
    ...
    public class IncomingSwapRequestHandler implements Runnable {
        ...
        public void run() {
            ...
            double myLoc = getLocation();
            double[] friendLocs = node.getPeerManager().getPeerLocationDoubles(false);
    ...
    public class OutgoingSwapRequestHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                PeerNode pn = node.getPeerManager().getRandomPeer();
                if(pn == null) {
                    // Nowhere to send
                    return;
                }
    ...
    private void announceLocChange(boolean log, boolean randomReset, boolean fromDupLocation) {
        Message msg = DMT.createFNPLocChangeNotificationNew(getLocation(), node.getPeerManager().getPeerLocationDoubles(true));
        node.getPeerManager().localBroadcast(msg, false, true, this);
	if(log)
    ...
    public boolean handleSwapRequest(Message m, PeerNode pn) {
        ...
        if(htl <= 0) {
            ...
            while(true) {
                // Forward
                PeerNode randomPeer = node.getPeerManager().getRandomPeer(pn);
                if(randomPeer == null) {
                ...
                try {
                    ...
                    // receiver thread.
                    randomPeer.sendAsync(m.cloneAndDropSubMessages(), new MyCallback(DMT.createFNPSwapRejected(oldID), pn, item), this);
                } catch (NotConnectedException e) {
    ...
    static double[] extractLocs(PeerNode[] peers, boolean indicateBackoff) {
    ...
    static long[] extractUIDs(PeerNode[] peers) {
