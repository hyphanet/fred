...
public class RequestTracker {
	...
	public boolean lockUID(
		...
		if (offerReply) {
			...
			HashMap<Long, OfferReplyTag> map = getOfferTracker(
				ssk,
				realTimeFlag
			);
			return innerLock(
			...
			HashMap<Long, InsertTag> overallMap = getInsertTracker(
				ssk,
				false,
				realTimeFlag
			);
			HashMap<Long, InsertTag> localMap = local
				? getInsertTracker(ssk, local, realTimeFlag)
				: null;
			return innerLock(
				overallMap,
				localMap,
			...
			HashMap<Long, RequestTag> overallMap = getRequestTracker(
				ssk,
				false,
				realTimeFlag
			);
			HashMap<Long, RequestTag> localMap = local
				? getRequestTracker(ssk, local, realTimeFlag)
				: null;
			return innerLock(
				overallMap,
				localMap,
			...
			if (local) {
				...
				if (oldTag != null) {
					if (oldTag == tag) {
						Logger.error(
							this,
							"Tag already registered (local): " + tag,
							new Exception("debug")
						);
					} else {
						...
						Logger.error(
							this,
							"Different tag already registered (local) EVEN THOUGH NOT ON MAIN MAP: " +
							tag,
							new Exception("debug")
						);
	...
	public void countRequests(
		...
		HashMap<Long, ? extends UIDTag> map = getTracker(
			local,
			ssk,
			insert,
			offer,
		...
		synchronized (mapLock) {
			...
			for (Map.Entry<Long, ? extends UIDTag> entry : map.entrySet()) {
				...
				int out = tag.expectedTransfersOut(
					ignoreLocalVsRemote,
					transfersPerInsert,
					true
				);
				int in = tag.expectedTransfersIn(
					ignoreLocalVsRemote,
					transfersPerInsert,
					true
				);
	...
	private void countRequestsWaitingForSlots(
		HashMap<Long, ? extends UIDTag> runningUIDs,
	...
	private HashMap<Long, RequestTag> getRequestTracker(
		...
		if (realTimeFlag) {
			...
			if (ssk) {
				return local
					? runningLocalSSKGetUIDsBulk
					: runningSSKGetUIDsBulk;
			} else {
				return local
					? runningLocalCHKGetUIDsBulk
	...
	private HashMap<Long, OfferReplyTag> getOfferTracker(
		boolean ssk,
		boolean realTimeFlag
	) {
		if (realTimeFlag) return ssk
	...
	private void onRestartOrDisconnect(
		PeerNode pn,
	...
	public int getNumSSKOfferReplies(boolean realTimeFlag) {
		return realTimeFlag
			? runningSSKOfferReplyUIDsRT.size()
			: runningSSKOfferReplyUIDsBulk.size();
	}

	public int getNumCHKOfferReplies(boolean realTimeFlag) {
		return realTimeFlag
			? runningCHKOfferReplyUIDsRT.size()
			: runningCHKOfferReplyUIDsBulk.size();
	}
	...
	public RequestSender getTransferringRequestSenderByKey(
		NodeCHK key,
		boolean realTimeFlag
	) {
		HashMap<NodeCHK, RequestSender> transferringRequestSenders =
			realTimeFlag
				? transferringRequestSendersRT
				: transferringRequestSendersBulk;
	...
	public void removeTransferringSender(NodeCHK key, RequestSender sender) {
		...
		synchronized (transferringRequestSenders) {
			...
			if (
				transferringRequestSenders.get(key) == sender
