...
public class RequestTracker {
    ...
    private <T extends UIDTag> boolean innerLock(HashMap<Long, T> overallMap, HashMap<Long, T> localMap, T tag,
        ...
        synchronized (overallMap) {
            if (logMINOR) {
                Logger.minor(this,
                             "Locking " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply=" + offerReply
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Locked " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply=" + offerReply
            ...
            if (local) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Locking (local) " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply="
                                 + offerReply + " local=" + local + " size=" + localMap.size(), new Exception("debug"));
                }
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Locked (local) " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply="
     ...
     * @param canFail
     */
    private <T extends UIDTag> void innerUnlock(HashMap<Long, T> overallMap, HashMap<Long, T> localMap, T tag,
            Long uid, boolean ssk, boolean insert, boolean offerReply, boolean local, boolean canFail) {
        synchronized (overallMap) {
            if (logMINOR) {
                Logger.minor(this,
                             "Unlocking " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply=" + offerReply
                             + " local=" + local + " size=" + overallMap.size(), new Exception("debug"));
            }
            ...
            if (overallMap.get(uid) != tag) {
                if (canFail) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Can fail and did fail: removing " + tag + " got " + overallMap.get(uid) + " for "
                                     + uid);
                    }
                } else {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Unlocked " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply=" + offerReply
                             + " local=" + local + " size=" + overallMap.size());
            }
            ...
            if (local) {
                if (localMap.get(uid) != tag) {
                    if (canFail) {
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Can fail and did fail (local): removing " + tag + " got " + localMap.get(uid)
                                         + " for " + uid);
                        }
                    } else {
                        Logger.error(this,
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Unlocked (local) " + uid + " ssk=" + ssk + " insert=" + insert + " offerReply="
                                 + offerReply + " local=" + local + " size=" + localMap.size());
                }
            } else {
            ...
            if (!requestsToNode) {
                ...
                for (Map.Entry<Long, ? extends UIDTag> entry : map.entrySet()) {
                    ...
                    if (tag.getSource() == source) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Counting " + tag + " from " + entry.getKey() + " from " + source
                                         + " count now " + count + " out now " + transfersOut + " in now "
                                         + transfersIn);
                        }
                    } else if (logDEBUG) {
    ...
    public RequestSender getTransferringRequestSenderByKey(NodeCHK key, boolean realTimeFlag) {
        HashMap<NodeCHK, RequestSender> transferringRequestSenders = realTimeFlag
    ...
    public void addTransferringSender(NodeCHK key, RequestSender sender) {
        HashMap<NodeCHK, RequestSender> transferringRequestSenders = sender.realTimeFlag
                                                                     ? transferringRequestSendersRT
                                                                     : transferringRequestSendersBulk;
