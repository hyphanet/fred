...
public class IPDetectorPluginManager implements ForwardPortCallback {
    ...
    IPDetectorPluginManager(Node node, NodeIPDetector detector) {
        ...
        noConnectionAlert = new MyUserAlert(l10n("noConnectivityTitle"), l10n("noConnectivity"), true, UserAlert.ERROR);
        symmetricAlert = new MyUserAlert(l10n("symmetricTitle"), l10n("symmetric"), true, UserAlert.ERROR);
        portRestrictedAlert = new MyUserAlert(l10n("portRestrictedTitle"), l10n("portRestricted"), true,
                UserAlert.WARNING);
        restrictedAlert = new MyUserAlert(l10n("restrictedTitle"), l10n("restricted"), false, UserAlert.MINOR);
    ...
    private boolean shouldDetectWithPeers(long now, PeerNode[] peers, PeerNode[] conns,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Checking whether should detect with " + peers.length + " peers and " + conns.length
    ...
    public void portForwardStatus(Map<ForwardPort, ForwardPortStatus> statuses) {
        ...
        for (ForwardPort p : currentPorts) {
            ...
            if (status.status == ForwardPortStatus.DEFINITE_SUCCESS) {
                Logger.normal(this,
                              "Succeeded forwarding " + p.name + " port " + p.portNumber + " for " + p.protocol
                              + " - port forward definitely succeeded " + status.reasonString);
            } else if (status.status == ForwardPortStatus.PROBABLE_SUCCESS) {
                Logger.normal(this,
                              "Probably succeeded forwarding " + p.name + " port " + p.portNumber + " for "
                ...
                Logger.error(this,
                             "Failed forwarding " + p.name + " port " + p.portNumber + " for " + p.protocol
                             + " - port forward definitely failed " + status.reasonString);
            } else if (status.status == ForwardPortStatus.PROBABLE_FAILURE) {
                Logger.error(this,
                             "Probably failed forwarding " + p.name + " port " + p.portNumber + " for " + p.protocol
...
//              DetectedIP[] list = (DetectedIP[]) map.values().toArray(new DetectedIP[map.size()]);
                    ...
                    synchronized (this) {
                        if (noConnectivityAlert == null) {
                            noConnectivityAlert = toRegister = new SimpleUserAlert(false, l10n("noConnectivityTitle"),
                                    l10n("noConnectivity"), l10n("noConnectivityShort"), UserAlert.ERROR);
                        }
                    }
            ...
            if (suggestPortForward) {
                if (portsNotForwarded.length == 1) {
                    ...
                    NodeL10n.getBase().addL10nSubstitution(div,
                            "IPDetectorPluginManager.suggestForwardTwoPortsWithLink", new String[] { "link",
                            "port1", "port2" }, new HTMLNode[] {
                                HTMLNode.link(
                                    ExternalLinkToadlet.escape(
                                        "http://wiki.freenetproject.org/FirewallAndRouterIssues")),
    ...
    public class PortForwardAlert extends AbstractUserAlert {
        ...
        public HTMLNode getHTMLText() {
            ...
            if (portsNotForwarded.length == 1) {
                NodeL10n.getBase().addL10nSubstitution(div, "IPDetectorPluginManager.forwardPort" + keySuffix,
                        new String[] { "port",
                                       "link" }, new HTMLNode[] { HTMLNode.text(Math.abs(portsNotForwarded[0])),
                                       HTMLNode.link(url) });
            } else if (portsNotForwarded.length == 2) {
                NodeL10n.getBase().addL10nSubstitution(div, "IPDetectorPluginManager.forwardTwoPorts" + keySuffix,
                        new String[] { "port1",
                                       "port2", "link", "connectivity" }, new HTMLNode[] {
        ...
        public String getShortText() {
            ...
            if (portsNotForwarded.length == 1) {
                return prefix
                       + l10n("forwardPortShort" + keySuffix, "port", Integer.toString(Math.abs(portsNotForwarded[0])));
            } else if (portsNotForwarded.length == 2) {
                return prefix
