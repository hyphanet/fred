...
public class LoggingConfigHandler {
			...
			new StringCallback() {
				...
				public void set(String val) throws InvalidConfigValueException {
					...
					if (fileLoggerHook == null) {
						...
						// Discard old data
						fileLoggerHook.switchBaseFilename(
							f.getPath() + File.separator + LOG_PREFIX
						);
						logDir = f;
		...
		config.register(
			...
			new LongCallback() {
				...
				public void set(Long val) throws InvalidConfigValueException {
					...
					if (fileLoggerHook != null) fileLoggerHook.setMaxListBytes(
						val
					);
				}
			},
	...
	private void enableLogger() {
		...
		synchronized (enableLoggerLock) {
			...
			try {
				...
				System.err.println(
					"INVALID LOGGING INTERVAL: " + e.getMessage()
				);
				...
				try {
					...
					System.err.println(
						"CANNOT START LOGGER: IMPOSSIBLE: " + e1.getMessage()
					);
					return;
				} catch (IOException e1) {
					System.err.println(
						"CANNOT START LOGGER: " + e1.getMessage()
					);
					return;
				}
			}
	...
	protected void preSetLogDir(File f) throws InvalidConfigValueException {
		...
		if (exists && !f.isDirectory()) throw new InvalidConfigValueException(
			"Cannot overwrite a file with a log directory"
		);
		if (!exists) {
			f.mkdir();
			exists = f.exists();
