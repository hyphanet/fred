...
public class Node implements TimeSkewDetectorCallback {
	...
	public class MigrateOldStoreData implements Runnable {
		...
		public void run() {
			System.err.println(
				"Migrating old " + (clientCache ? "client cache" : "datastore")
			);
			if (clientCache) {
	...
	public <T extends StorableBlock> void closeOldStore(StoreCallback<T> old) {
		...
		if (store instanceof SaltedHashFreenetStore) {
			SaltedHashFreenetStore<T> saltstore = (SaltedHashFreenetStore<
					T
				>) store;
			saltstore.close();
			saltstore.destruct();
	...
	public class NodeNameCallback extends StringCallback {
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			else if (val.length() > 128) throw new InvalidConfigValueException(
				"The given node name is too long (" + val + ')'
			);
	...
	private class StoreTypeCallback
		...
		public void set(String val)
			...
			if (!found) throw new InvalidConfigValueException(
				"Invalid store type"
			);
			...
			if (type.equals("ram")) {
				...
				throw new NodeNeedRestartException(
					"Store type cannot be changed on the fly"
				);
			}
		}
	...
	private static class L10nCallback
		...
		public void set(String val) throws InvalidConfigValueException {
			...
			try {
				NodeL10n.getBase()
					.setLanguage(BaseL10n.LANGUAGE.mapToLanguage(val));
	...
	public static final int HANDSHAKE_TIMEOUT = (int) MILLISECONDS.toMillis(
		4800
	); // Keep the below within the 30 second assumed timeout.
	...
	public boolean isUsingWrapper() {
		if (
			nodeStarter != null && WrapperManager.isControlledByNativeWrapper()
		) return true;
		else return false;
	}
	...
	Node(
		...
		this.pluginDir = setupProgramDir(
			installConfig,
			"pluginDir",
		...
		try {
			new NodeL10n(
				BaseL10n.LANGUAGE.mapToLanguage(nodeConfig.getString("l10n")),
				getCfgDir()
			);
		} catch (MissingResourceException e) {
			try {
				new NodeL10n(
					BaseL10n.LANGUAGE.mapToLanguage(
						nodeConfig.getOption("l10n").getDefault()
					),
					getCfgDir()
				);
				...
				new NodeL10n(
					BaseL10n.LANGUAGE.mapToLanguage(
						BaseL10n.LANGUAGE.getDefault().shortCode
					),
					getCfgDir()
				);
			}
		...
		try {
			toadlets = new SimpleToadletServer(
				fproxyConfig,
			...
			System.err.println(
				"Invalid config value, cannot start web interface: " + e4
			);
			e4.printStackTrace();
			throw new NodeInitException(
				NodeInitException.EXIT_COULD_NOT_START_FPROXY,
		...
		final NativeThread entropyGatheringThread = new NativeThread(
			new Runnable() {
				long tLastAdded = -1;
				...
				public void run() {
					...
					if (File.separatorChar == '/') {
						if (new File("/dev/hwrng").exists()) System.out.println(
							"/dev/hwrng exists - have you installed rng-tools?"
						);
		...
		showFriendsVisibilityAlert = nodeConfig.getBoolean(
			"showFriendsVisibilityAlert"
		);
		...
		for (int i = 0; i < 2; i++) {
			try {
				...
				if (
					securityLevels.getPhysicalThreatLevel() ==
					...
					securityLevels.resetPhysicalThreatLevel(
		...
		try {
			...
			if (buf.length != BOOT_FILE_LENGTH) System.err.println(
				"Not 16 bytes for boot ID " + bootID + " - WTF??"
			);
			raf.write(buf);
		} catch (IOException e) {
			oldBootID = -1;
		...
		disableProbabilisticHTLs = nodeConfig.getBoolean(
			"disableProbabilisticHTLs"
		);
		...
		class TrafficClassCallback
			...
			public void set(String tcName)
				...
				throw new NodeNeedRestartException(
					"TrafficClass cannot change on the fly"
				);
			}
			...
			public String[] getPossibleValues() {
				...
				for (TrafficClass tc : TrafficClass.values()) array.add(
					tc.name()
				);
		...
		enablePerNodeFailureTables = nodeConfig.getBoolean(
			"enablePerNodeFailureTables"
		);
		...
		enableULPRDataPropagation = nodeConfig.getBoolean(
			"enableULPRDataPropagation"
		);
		...
		publishOurPeersLocation = nodeConfig.getBoolean(
			"publishOurPeersLocation"
		);
		...
		routeAccordingToOurPeersLocation = nodeConfig.getBoolean(
			"routeAccordingToOurPeersLocation"
		);
		...
		enablePacketCoalescing = nodeConfig.getBoolean(
			"enablePacketCoalescing"
		);
		...
		// @see #191
		if (
			oldConfig != null && "-1".equals(oldConfig.get("node.listenPort"))
		) throw new NodeInitException(
			NodeInitException.EXIT_COULD_NOT_BIND_USM,
			"Your freenet.ini file is corrupted! 'listenPort=-1'"
		);
		NodeCryptoConfig darknetConfig = new NodeCryptoConfig(
			nodeConfig,
			sortOrder++,
			false,
			securityLevels
		...
		if (
			executor instanceof PooledExecutor
		...
		nodeConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer obwLimit)
					...
					try {
						outputThrottle.changeNanosAndBucketSize(
							SECONDS.toNanos(1) / obwLimit,
							obwLimit / 2
						);
					} catch (IllegalArgumentException e) {
						throw new InvalidConfigValueException(e);
		...
		try {
			...
			throw new NodeInitException(
				NodeInitException.EXIT_BAD_BWLIMIT,
				e.getMessage()
			);
		}
		...
		try {
			outputThrottle = new TokenBucket(
				bucketSize,
				SECONDS.toNanos(1) / obwLimit,
				obwLimit / 2
			);
			...
			new LongCallback() {
				...
				public void set(Long amountOfDataToCheckCompressionRatio) {
					synchronized (Node.this) {
						...
						Node.this.amountOfDataToCheckCompressionRatio =
							amountOfDataToCheckCompressionRatio;
					}
				}
			},
		...
		amountOfDataToCheckCompressionRatio = nodeConfig.getLong(
			"amountOfDataToCheckCompressionRatio"
		);
				...
				public void set(Integer minimumCompressionPercentage) {
					synchronized (Node.this) {
						if (
							...
							minimumCompressionPercentage > 100
						) {
							Logger.normal(
								Node.class,
								"Wrong minimum compression percentage" +
								minimumCompressionPercentage
							);
							return;
						}

						Node.this.minimumCompressionPercentage =
							minimumCompressionPercentage;
					}
				}
			},
		...
		minimumCompressionPercentage = nodeConfig.getInt(
			"minimumCompressionPercentage"
		);
		...
		nodeConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer maxTimeForSingleCompressor) {
					synchronized (Node.this) {
						Node.this.maxTimeForSingleCompressor =
							maxTimeForSingleCompressor;
					}
				}
			},
		...
		maxTimeForSingleCompressor = nodeConfig.getInt(
			"maxTimeForSingleCompressor"
		);
		...
		nodeConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean connectionSpeedDetection) {
					synchronized (Node.this) {
						Node.this.connectionSpeedDetection =
							connectionSpeedDetection;
					}
				}
			}
		...
		connectionSpeedDetection = nodeConfig.getBoolean(
			"connectionSpeedDetection"
		);
		...
		File nodeFileBackup = nodeDir.file(
		...
		try {
			...
			try {
				...
				if (nodeFile.exists() || nodeFileBackup.exists()) {
					System.err.println(
						"No node file or cannot read, (re)initialising crypto etc"
					);
		...
		nodeStats = new NodeStats(
			this,
			sortOrder,
			config.createSubConfig("node.load"),
			obwLimit,
			ibwLimit,
			lastVersion
		);
				...
				public void set(Boolean val)
					...
					synchronized (Node.this) {
						...
						if (opennet != null) throw new NodeNeedRestartException(
							l10n(
								"connectToSeednodesCannotBeChangedMustDisableOpennetOrReboot"
							)
						);
					}
				}
		...
		isAllowedToConnectToSeednodes = opennetConfig.getBoolean(
			"connectToSeednodes"
		);
		...
		opennetConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean val)
					...
					ipDetector.ipDetectorManager.notifyPortChange(
		...
		opennetConfig.register(
			...
			new IntCallback() {
				...
				@Override
				public void set(Integer inputMaxOpennetPeers)
					throws InvalidConfigValueException {
					if (
						inputMaxOpennetPeers < 0
					) throw new InvalidConfigValueException(
						l10n("mustBePositive")
					);
					if (
		...
		if (maxOpennetPeers > OpennetManager.MAX_PEERS_FOR_SCALING) {
			Logger.error(
				this,
				"maxOpennetPeers may not be over " +
				OpennetManager.MAX_PEERS_FOR_SCALING
			);
			maxOpennetPeers = OpennetManager.MAX_PEERS_FOR_SCALING;
		}

		opennetCryptoConfig = new NodeCryptoConfig(
			opennetConfig,
			2/* 0 = enabled */,
			true,
			securityLevels
		...
		securityLevels.addNetworkThreatLevelListener(
			new SecurityLevelListener<NETWORK_THREAT_LEVEL>() {
				...
				public void onChange(
					...
					if (
						newLevel == NETWORK_THREAT_LEVEL.HIGH ||
						...
						newLevel == NETWORK_THREAT_LEVEL.NORMAL ||
		...
		acceptSeedConnections = opennetConfig.getBoolean(
			"acceptSeedConnections"
		);

		if (acceptSeedConnections && opennet != null) opennet.crypto.socket
			.getAddressTracker()
			.setHugeTracker();
		...
		passOpennetRefsThroughDarknet = nodeConfig.getBoolean(
			"passOpennetPeersThroughDarknet"
		);

		this.extraPeerDataDir = userDir.file(
			"extra-peer-data-" + getDarknetPortNumber()
		);
		if (
			!((extraPeerDataDir.exists() && extraPeerDataDir.isDirectory()) ||
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				@Override
				public void set(Long storeSize)
					throws InvalidConfigValueException {
					long maxDatastoreSize;
					if (storeSize < MIN_STORE_SIZE) {
						throw new InvalidConfigValueException(
							l10n("invalidMinStoreSize")
						);
					}
					if (
						storeSize >
						(maxDatastoreSize = DatastoreUtil.maxDatastoreSize())
					) {
						throw new InvalidConfigValueException(
							l10n(
								"invalidMaxStoreSize",
								Long.toString(maxDatastoreSize / oneGiB)
							)
						);
					}
					...
					try {
						chkDatastore.setMaxKeys(
							maxStoreKeys,
							storeForceBigShrinks
						);
						chkDatacache.setMaxKeys(
							maxCacheKeys,
							storeForceBigShrinks
						);
						pubKeyDatastore.setMaxKeys(
							maxStoreKeys,
							storeForceBigShrinks
						);
						pubKeyDatacache.setMaxKeys(
							maxCacheKeys,
							storeForceBigShrinks
						);
						sskDatastore.setMaxKeys(
							maxStoreKeys,
							storeForceBigShrinks
						);
						sskDatacache.setMaxKeys(
							maxCacheKeys,
							storeForceBigShrinks
						);
					} catch (IOException e) {
						...
						Logger.error(
							this,
							"Caught " + e + " resizing the datastore",
							e
						);
						System.err.println(
							"Caught " + e + " resizing the datastore"
					...
					nodeStats.avgStoreCHKLocation.changeMaxReports(
						(int) maxStoreKeys
					);
					nodeStats.avgCacheCHKLocation.changeMaxReports(
						(int) maxCacheKeys
					);
					nodeStats.avgSlashdotCacheCHKLocation.changeMaxReports(
						(int) maxCacheKeys
					);
					nodeStats.avgClientCacheCHKLocation.changeMaxReports(
						(int) maxCacheKeys
					);

					nodeStats.avgStoreSSKLocation.changeMaxReports(
						(int) maxStoreKeys
					);
					nodeStats.avgCacheSSKLocation.changeMaxReports(
						(int) maxCacheKeys
					);
					nodeStats.avgSlashdotCacheSSKLocation.changeMaxReports(
						(int) maxCacheKeys
					);
					nodeStats.avgClientCacheSSKLocation.changeMaxReports(
						(int) maxCacheKeys
					);
				}
			},
		...
		if (
			maxTotalDatastoreSize < MIN_STORE_SIZE && !storeType.equals("ram")
		) { // totally arbitrary minimum!
			throw new NodeInitException(
				NodeInitException.EXIT_INVALID_STORE_SIZE,
		...
		nodeConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean val)
					...
					// FIXME l10n
					throw new NodeNeedRestartException(
						"Need to restart to change storeUseSlotFilters"
					);
				}
			}
		);
		...
		nodeConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					} else throw new InvalidConfigValueException(
		...
		storeSaltHashSlotFilterPersistenceTime = nodeConfig.getInt(
			"storeSaltHashSlotFilterPersistenceTime"
		);
		...
		storeSaltHashResizeOnStart = nodeConfig.getBoolean(
			"storeSaltHashResizeOnStart"
		);

		this.storeDir = setupProgramDir(
			installConfig,
			"storeDir",
		...
		nodeConfig.register(
			...
			new BooleanCallback() {
				...
				private void setPreallocate(
					StoreCallback<?> datastore,
					boolean val
				) {
					// Avoid race conditions by checking first.
					FreenetStore<?> store = datastore.getStore();
					if (
						store instanceof SaltedHashFreenetStore
			...
			securityLevels.addPhysicalThreatLevelListener(
				...
				>() {
					...
					public void onChange(
						...
						try {
							if (
								newLevel == PHYSICAL_THREAT_LEVEL.LOW
				...
				public void onChange(
					...
					if (newLevel == PHYSICAL_THREAT_LEVEL.MAXIMUM) {
						...
						try {
							...
							Logger.error(
								this,
								"Unable to securely delete " + masterKeysFile
							);
							System.err.println(
								NodeL10n.getBase()
									.getString(
							...
							clientCore.alerts.register(
								new SimpleUserAlert(
									...
									NodeL10n.getBase()
										.getString(
											"SecurityLevels.cantDeletePasswordFileTitle"
										),
									NodeL10n.getBase()
										.getString(
											"SecurityLevels.cantDeletePasswordFile"
					...
					if (
						oldLevel == PHYSICAL_THREAT_LEVEL.MAXIMUM &&
		...
		if (
			securityLevels.physicalThreatLevel == PHYSICAL_THREAT_LEVEL.MAXIMUM
		) {
			try {
				killMasterKeysFile();
			} catch (IOException e) {
		...
		// Total alchemy, FIXME!
		if (memoryLimit == Long.MAX_VALUE || memoryLimit < 0) defaultCacheSize =
			1024 * 1024;
		else if (memoryLimit <= 128 * 1024 * 1024) defaultCacheSize = 0; // Turn off completely for very small memory.
		else {
			// 9 stores, total should be 5% of memory, up to maximum of 1MB per store at 308MB+
			defaultCacheSize = Math.min(
				1024 * 1024,
				(memoryLimit - 128 * 1024 * 1024) / (20 * 9)
			);
		}
		...
		cachingFreenetStoreMaxSize = nodeConfig.getLong(
			"cachingFreenetStoreMaxSize"
		);
		if (cachingFreenetStoreMaxSize < 0) throw new NodeInitException(
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long val)
					...
					throw new NodeNeedRestartException(
						"Caching Period cannot be changed on the fly"
					);
				}
			},
			true
		...
		cachingFreenetStorePeriod = nodeConfig.getLong(
			"cachingFreenetStorePeriod"
		);

		if (cachingFreenetStoreMaxSize > 0 && cachingFreenetStorePeriod > 0) {
			cachingFreenetStoreTracker = new CachingFreenetStoreTracker(
				cachingFreenetStoreMaxSize,
				cachingFreenetStorePeriod,
				ticker
			);
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long storeSize)
					throws InvalidConfigValueException {
					if (
						storeSize < MIN_CLIENT_CACHE_SIZE
					) throw new InvalidConfigValueException(
					...
					try {
						chkClientcache.setMaxKeys(
							maxClientCacheKeys,
							storeForceBigShrinks
						);
						pubKeyClientcache.setMaxKeys(
							maxClientCacheKeys,
							storeForceBigShrinks
						);
						sskClientcache.setMaxKeys(
							maxClientCacheKeys,
							storeForceBigShrinks
						);
					} catch (IOException e) {
						...
						Logger.error(
							this,
							"Caught " + e + " resizing the clientcache",
							e
						);
						System.err.println(
							"Caught " + e + " resizing the clientcache"
		...
		if (clientCacheType.equals("salt-hash")) {
			if (clientCacheKey == null) {
				System.err.println(
					"Cannot open client-cache, it is passworded"
				);
				setClientCacheAwaitingPassword();
			} else {
		...
		long slashdotCacheLifetime = nodeConfig.getLong(
			"slashdotCacheLifetime"
		);
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long storeSize)
					throws InvalidConfigValueException {
					if (
						storeSize < MIN_SLASHDOT_CACHE_SIZE
					) throw new InvalidConfigValueException(
						l10n("invalidStoreSize")
					);
					int newMaxStoreKeys = (int) Math.min(
					...
					try {
						chkSlashdotcache.setMaxKeys(
							maxSlashdotCacheKeys,
							storeForceBigShrinks
						);
						pubKeySlashdotcache.setMaxKeys(
							maxSlashdotCacheKeys,
							storeForceBigShrinks
						);
						sskSlashdotcache.setMaxKeys(
							maxSlashdotCacheKeys,
							storeForceBigShrinks
						);
					} catch (IOException e) {
						...
						Logger.error(
							this,
							"Caught " + e + " resizing the slashdotcache",
							e
						);
						System.err.println(
							"Caught " + e + " resizing the slashdotcache"
		...
		maxSlashdotCacheKeys = (int) Math.min(
			maxSlashdotCacheSize / sizePerKey,
			Integer.MAX_VALUE
		);

		chkSlashdotcache = new CHKStore();
		chkSlashdotcacheStore = new SlashdotStore<CHKBlock>(
			chkSlashdotcache,
			maxSlashdotCacheKeys,
			slashdotCacheLifetime,
			PURGE_INTERVAL,
		...
		pubKeySlashdotcacheStore = new SlashdotStore<DSAPublicKey>(
			pubKeySlashdotcache,
			maxSlashdotCacheKeys,
			slashdotCacheLifetime,
			PURGE_INTERVAL,
		...
		sskSlashdotcacheStore = new SlashdotStore<SSKBlock>(
			sskSlashdotcache,
			maxSlashdotCacheKeys,
			slashdotCacheLifetime,
			PURGE_INTERVAL,
			...
			new IntCallback() {
				...
				public void set(Integer val)
					...
					synchronized (Node.this) {
						...
						if (
							val < UdpSocketHandler.MIN_MTU
						) throw new InvalidConfigValueException(
							"Must be over 576"
						);
						if (val > 1492) throw new InvalidConfigValueException(
		...
		datastoreTooSmallDismissed = nodeConfig.getInt(
			"datastoreTooSmallDismissed"
		);
		...
		if (
			!peersOffersDismissed && checkPeersOffersFrefFiles()
		...
		nodeDiagnostics = new DefaultNodeDiagnostics(
			this.nodeStats,
			this.ticker
		);
	}

	private void peersOffersFrefFilesConfiguration(
		SubConfig nodeConfig,
		int configOptionSortOrder
	) {
		...
		nodeConfig.register(
			...
			new BooleanCallback() {
				...
				public void set(Boolean val) {
					if (val) {
						for (UserAlert alert : clientCore.alerts.getAlerts()) if (
							alert instanceof PeersOffersUserAlert
		...
		for (File f : list) {
			...
			if (
				...
				try {
					...
					System.err.println(
						"Failed to delete old datastore file \"" +
						f +
						"\": " +
						e
					);
	...
	public ProgramDirectory setupProgramDir(
		...
		try {
			...
			throw new NodeInitException(
				NodeInitException.EXIT_BAD_DIR,
	...
	public void lateSetupDatabase(DatabaseKey databaseKey)
		...
		try {
			if (
				!clientCore.lateInitDatabase(databaseKey)
	...
	private void failLateInitDatabase() {
		System.err.println(
			"Failed late initialisation of database, closing..."
		);
	}
	...
	private final UserAlert masterPasswordUserAlert = new UserAlert() {
		...
		public HTMLNode getHTMLText() {
			...
			SecurityLevelsToadlet.generatePasswordFormPage(
				false,
				clientCore.getToadletContainer(),
				content,
				false,
				false,
				false,
	...
	private void initSaltHashFS(
		...
		try {
			...
			final FreenetStore<CHKBlock> chkDataFS = makeStore(
				"CHK",
				true,
				chkDatastore,
				dontResizeOnStart,
			...
			final FreenetStore<CHKBlock> chkCacheFS = makeStore(
				"CHK",
				false,
				chkDatacache,
				dontResizeOnStart,
			...
			((SaltedHashFreenetStore<
						CHKBlock
					>) chkCacheFS.getUnderlyingStore()).setAltStore(
					((SaltedHashFreenetStore<
								CHKBlock
							>) chkDataFS.getUnderlyingStore())
				);
			final PubkeyStore pubKeyDatastore = new PubkeyStore();
			final FreenetStore<DSAPublicKey> pubkeyDataFS = makeStore(
				"PUBKEY",
				true,
				pubKeyDatastore,
				dontResizeOnStart,
			...
			final FreenetStore<DSAPublicKey> pubkeyCacheFS = makeStore(
				"PUBKEY",
				false,
				pubKeyDatacache,
				dontResizeOnStart,
			...
			((SaltedHashFreenetStore<
						DSAPublicKey
					>) pubkeyCacheFS.getUnderlyingStore()).setAltStore(
					((SaltedHashFreenetStore<
								DSAPublicKey
							>) pubkeyDataFS.getUnderlyingStore())
				);
			final SSKStore sskDatastore = new SSKStore(getPubKey);
			final FreenetStore<SSKBlock> sskDataFS = makeStore(
				"SSK",
				true,
				sskDatastore,
				dontResizeOnStart,
			...
			final FreenetStore<SSKBlock> sskCacheFS = makeStore(
				"SSK",
				false,
				sskDatacache,
				dontResizeOnStart,
			...
			((SaltedHashFreenetStore<
						SSKBlock
					>) sskCacheFS.getUnderlyingStore()).setAltStore(
					((SaltedHashFreenetStore<
								SSKBlock
							>) sskDataFS.getUnderlyingStore())
				);
				...
				this.getTicker()
					.queueTimedJob(
						new Runnable() {
							...
							public void run() {
								System.err.println(
									"Starting delayed init of datastore"
								);
								try {
								...
								System.err.println(
									"Finishing delayed init of datastore"
								);
								migrate.run();
							}
						},
						...
						true,
						false
					); // Use Ticker to guarantee that this runs *after* constructors have completed.
			} else {
				Node.this.chkDatastore = chkDatastore;
				Node.this.chkDatacache = chkDatacache;
				Node.this.pubKeyDatastore = pubKeyDatastore;
			...
			e.printStackTrace();
			throw new NodeInitException(
				NodeInitException.EXIT_STORE_OTHER,
				e.getMessage()
			);
		}
	}
	...
	private void initSaltHashClientCacheFS(
		...
		try {
			...
			final FreenetStore<CHKBlock> chkDataFS = makeClientcache(
				"CHK",
				true,
				chkClientcache,
				dontResizeOnStart,
			...
			final FreenetStore<DSAPublicKey> pubkeyDataFS = makeClientcache(
				"PUBKEY",
				true,
				pubKeyClientcache,
				dontResizeOnStart,
			...
			final FreenetStore<SSKBlock> sskDataFS = makeClientcache(
				"SSK",
				true,
				sskClientcache,
				dontResizeOnStart,
				...
				getTicker()
					.queueTimedJob(
						new Runnable() {
							...
							public void run() {
								System.err.println(
									"Starting delayed init of client-cache"
								);
								try {
									chkDataFS.start(ticker, true);
									pubkeyDataFS.start(ticker, true);
									...
									Logger.error(
										this,
										"Failed to start client-cache: " + e,
										e
									);
									System.err.println(
										"Failed to start client-cache: " + e
								...
								System.err.println(
									"Finishing delayed init of client-cache"
								);
								migrate.run();
							}
						},
	...
	private <T extends StorableBlock> FreenetStore<T> makeClientcache(
		...
		FreenetStore<T> store = makeStore(
			type,
			"clientcache",
			maxClientCacheKeys,
			cb,
	...
	private <T extends StorableBlock> FreenetStore<T> makeStore(
		...
		SaltedHashFreenetStore<T> fs = SaltedHashFreenetStore.<T>construct(
			getStoreDir(),
			type + "-" + store,
			cb,
			random,
		...
		cb.setStore(fs);
		if (cachingFreenetStoreMaxSize > 0) return new CachingFreenetStore<T>(
			cb,
			fs,
	...
	public void start(boolean noSwaps) throws NodeInitException {
		...
		if (!NativeThread.HAS_ENOUGH_NICE_LEVELS) clientCore.alerts.register(
			new NotEnoughNiceLevelsUserAlert()
		);
	...
	private void scheduleVersionTransition() {
		...
		if (now < transition) ticker.queueTimedJob(
			new Runnable() {
				@Override
				public void run() {
	...
	private void checkForEvilJVMBugs() {
		...
		String jvmSpecVendor = System.getProperty(
		...
		if (
			((!isOpenJDK) &&
				(jvmVendor.startsWith("Sun ") ||
					jvmVendor.startsWith("Oracle "))) ||
			(jvmVendor.startsWith("The FreeBSD Foundation") &&
				(jvmSpecVendor.startsWith("Sun ") ||
					jvmSpecVendor.startsWith("Oracle "))) ||
			...
			jvmVendor.startsWith("Apple ") || jvmVendor.startsWith("\"Apple ")
		) {
			...
			if (jvmVendor.startsWith("Free Software Foundation")) {
				...
				try {
					...
					if (
						jvmVersionInt <= 422 && jvmVersionInt >= 100
					) jvmHasGCJCharConversionBug = true; // make sure that no bogus values cause true
				} catch (Throwable t) {
	...
	public int routedPing(double loc2, byte[] pubKeyHash) {
		...
		Message m = DMT.createFNPRoutedPing(
			uid,
			loc2,
			maxHTL,
			initialX,
		...
		MessageFilter mf1 = MessageFilter.create()
			.setField(DMT.UID, uid)
			.setType(DMT.FNPRoutedPong)
			.setTimeout(5000);
		try {
	...
	private KeyBlock makeRequestLocal(
		...
		if (key instanceof NodeCHK) {
			...
			if (pubKey == null) {
				pubKey = getPubKey.getKey(
					sskKey.getPubKeyHash(),
					canReadClientCache,
					offersOnly,
					null
				);
			...
			if (pubKey != null) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Not found because no pubkey: " + uid
				);
			}
		} else throw new IllegalStateException(
			"Unknown key type: " + key.getClass()
		);
		...
		if (kb != null) {
			...
			if (clientCore != null && clientCore.requestStarters != null) {
				if (kb instanceof CHKBlock) {
					clientCore.requestStarters.chkFetchSchedulerBulk.tripPendingKey(
						kb
					);
					clientCore.requestStarters.chkFetchSchedulerRT.tripPendingKey(
						kb
					);
				} else {
					clientCore.requestStarters.sskFetchSchedulerBulk.tripPendingKey(
						kb
					);
					clientCore.requestStarters.sskFetchSchedulerRT.tripPendingKey(
						kb
					);
				}
			}
	...
	public Object makeRequestSender(
		...
		RequestSender sender = key instanceof NodeCHK
			? tracker.getTransferringRequestSenderByKey(
				(NodeCHK) key,
				realTimeFlag
			)
			: null;
		if (sender != null) {
			if (logMINOR) Logger.minor(
				this,
				"Data already being transferred: " + sender
			);
	...
	public KeyBlock fetch(
		...
		if (key instanceof NodeSSK) return fetch(
			(NodeSSK) key,
			false,
			canReadClientCache,
			canWriteClientCache,
		...
		else if (key instanceof NodeCHK) return fetch(
			(NodeCHK) key,
			false,
			canReadClientCache,
			canWriteClientCache,
	...
	public SSKBlock fetch(
		...
		if (canReadClientCache) {
			try {
				SSKBlock block = sskClientcache.fetch(
					key,
					dontPromote || !canWriteClientCache,
					canReadClientCache,
					forULPR,
					false,
					meta
				...
				if (block != null) {
					...
					if (
						dist > nodeStats.furthestClientCacheSSKSuccess
					) nodeStats.furthestClientCacheSSKSuccess = dist;
					if (logDEBUG) Logger.debug(
						this,
						"Found key " + key + " in client-cache"
					);
					return block;
				}
			} catch (IOException e) {
		...
		if (forULPR || useSlashdotCache || canReadClientCache) {
			try {
				SSKBlock block = sskSlashdotcache.fetch(
					key,
					dontPromote,
					canReadClientCache,
					forULPR,
				...
				if (block != null) {
					...
					if (
						dist > nodeStats.furthestSlashdotCacheSSKSuccess
					) nodeStats.furthestSlashdotCacheSSKSuccess = dist;
					if (logDEBUG) Logger.debug(
						this,
						"Found key " + key + " in slashdot-cache"
					);
					return block;
				}
			} catch (IOException e) {
				Logger.error(
					this,
					"Could not read from slashdot/ULPR cache: " + e,
					e
				);
			}
		}
		...
		try {
			...
			SSKBlock block = sskDatastore.fetch(
				key,
			...
			if (block == null) {
				...
				if (store != null) block = store.fetch(
					key,
					dontPromote || !canWriteDatastore,
					canReadClientCache,
					forULPR,
			...
			if (block != null) {
				...
				if (
					dist > nodeStats.furthestStoreSSKSuccess
				) nodeStats.furthestStoreSSKSuccess = dist;
				if (logDEBUG) Logger.debug(
					this,
					"Found key " + key + " in store"
				);
				return block;
			}
			block = sskDatacache.fetch(
				key,
				dontPromote || !canWriteDatastore,
				canReadClientCache,
				forULPR,
			...
			if (block != null) {
				...
				if (
					dist > nodeStats.furthestCacheSSKSuccess
				) nodeStats.furthestCacheSSKSuccess = dist;
				if (logDEBUG) Logger.debug(
					this,
					"Found key " + key + " in cache"
				);
			}
			return block;
		} catch (IOException e) {
	...
	public CHKBlock fetch(
		...
		if (canReadClientCache) {
			try {
				CHKBlock block = chkClientcache.fetch(
					key,
					dontPromote || !canWriteClientCache,
					false,
					meta
				...
				if (block != null) {
					...
					if (
						dist > nodeStats.furthestClientCacheCHKSuccess
		...
		if (forULPR || useSlashdotCache || canReadClientCache) {
			try {
				CHKBlock block = chkSlashdotcache.fetch(
					key,
					dontPromote,
					false,
					meta
				...
				if (block != null) {
					...
					if (
						dist > nodeStats.furthestSlashdotCacheCHKSuccess
		...
		try {
			nodeStats.avgRequestLocation.report(loc);
			CHKBlock block = chkDatastore.fetch(
				key,
				dontPromote || !canWriteDatastore,
				ignoreOldBlocks,
				meta
			...
			if (block != null) {
				...
				if (
					dist > nodeStats.furthestStoreCHKSuccess
				) nodeStats.furthestStoreCHKSuccess = dist;
				return block;
			}
			block = chkDatacache.fetch(
				key,
				dontPromote || !canWriteDatastore,
				ignoreOldBlocks,
				meta
			...
			if (block != null) {
				...
				if (
					dist > nodeStats.furthestCacheCHKSuccess
	...
	public void store(
		...
		if (block instanceof CHKBlock) store(
			(CHKBlock) block,
			deep,
			canWriteClientCache,
			canWriteDatastore,
		...
		else if (block instanceof SSKBlock) store(
			(SSKBlock) block,
			deep,
			false,
			canWriteClientCache,
	...
	private void store(
		...
		try {
			...
			if (
				canWriteDatastore || forULPR || useSlashdotCache
	...
	public ClientKeyBlock fetchKey(
		...
		if (key instanceof ClientCHK) return fetch(
			(ClientCHK) key,
			canReadClientCache,
			canWriteClientCache,
			canWriteDatastore
		);
		else if (key instanceof ClientSSK) return fetch(
			(ClientSSK) key,
			canReadClientCache,
			canWriteClientCache,
			canWriteDatastore
			...
			"Don't know what to do with " + key
		);
	}
	...
	public ClientKeyBlock fetch(
		...
		if (block == null) {
			if (logMINOR) Logger.minor(
				this,
				"Could not find key for " + clientSSK
			);
			return null;
		}
	...
	public void park() {
		...
		try {
			...
			try {
				...
				Logger.error(
					this,
					"Failed to tell peers we are going down: " + t,
					t
				);
			} catch (Throwable t1) {
	...
	public void receivedNodeToNodeMessage(Message m, PeerNode src) {
		int type =
			((Integer) m.getObject(DMT.NODE_TO_NODE_MESSAGE_TYPE)).intValue();
		...
		new NodeToNodeMessageListener() {
			...
			public void handleMessage(
				...
				try {
					...
					Logger.error(
						this,
						"IOException while parsing node to node message data",
						e
					);
					return;
				}
				...
				try {
					...
					Logger.error(
						this,
						"FSParseException while parsing node to node message data",
						e
					);
					return;
				}
		...
		new NodeToNodeMessageListener() {
			...
			public void handleMessage(
				...
				if (!fromDarknet) {
					Logger.error(
						this,
						"Got N2NTM from non-darknet node ?!?!?!: from " + src
					);
					return;
				}
				...
				Logger.normal(
					this,
					"Received N2NTM from '" + darkSource.getPeer() + "'"
				);
				SimpleFieldSet fs = null;
				try {
					fs = new SimpleFieldSet(
				...
				int fileNumber = darkSource.writeNewExtraPeerDataFile(
					fs,
				...
				try {
					handleNodeToNodeTextMessageSimpleFieldSet(
						fs,
						darkSource,
						fileNumber
					);
	...
	public void connectToSeednode(SeedServerTestPeerNode node)
		throws OpennetDisabledException, FSParseException, PeerParseException, ReferenceSignatureVerificationException {
	...
	public OpennetPeerNode createNewOpennetNode(SimpleFieldSet fs)
		throws FSParseException, OpennetDisabledException, PeerParseException, ReferenceSignatureVerificationException, PeerTooOldException {
		if (opennet == null) throw new OpennetDisabledException(
			"Opennet is not currently enabled"
		);
		return new OpennetPeerNode(fs, this, opennet.crypto, opennet, false);
	}

	public SeedServerTestPeerNode createNewSeedServerTestPeerNode(
		SimpleFieldSet fs
	)
		throws FSParseException, OpennetDisabledException, PeerParseException, ReferenceSignatureVerificationException, PeerTooOldException {
		if (opennet == null) throw new OpennetDisabledException(
	...
	public synchronized boolean dontDetect() {
		...
		if (
			!darknetCrypto.getBindTo().isRealInternetAddress(false, true, false)
		) return false;
		if (opennet != null) {
			if (
				opennet.crypto
					.getBindTo()
					.isRealInternetAddress(false, true, false)
			) return false;
		}
		return true;
	}
	...
	public boolean hasKey(
		...
		// FIXME optimise!
		if (key instanceof NodeCHK) return (
			fetch(
				(NodeCHK) key,
				true,
				canReadClientCache,
	...
	public void setMasterPassword(String password, boolean inFirstTimeWizard)
		throws AlreadySetPasswordException, MasterKeysWrongPasswordException, MasterKeysFileSizeException, IOException {
		MasterKeys k;
		synchronized (this) {
			if (keys == null) {
				// Decrypting.
	...
	private void activatePasswordedClientCache(MasterKeys keys) {
		synchronized (this) {
			...
			if (!clientCacheType.equals("salt-hash")) {
				System.err.println(
					"Unknown client cache type, cannot activate passworded store: " +
					clientCacheType
				);
				return;
			}
		...
		try {
			...
			System.err.println(
				"Unable to activate passworded client cache: " + e
			);
			e.printStackTrace();
			return;
		}
	...
	public void changeMasterPassword(
		...
		throws MasterKeysWrongPasswordException, MasterKeysFileSizeException, IOException, AlreadySetPasswordException {
		if (
			securityLevels.getPhysicalThreatLevel() ==
			PHYSICAL_THREAT_LEVEL.MAXIMUM
		) Logger.error(
			this,
			"Changing password while physical threat level is at MAXIMUM???"
		);
	...
	public boolean wantEncryptedDatabase() {
		return (
			this.securityLevels.getPhysicalThreatLevel() !=
	 ...
	 * @return
	 */
	public boolean shouldStoreDeep(
		Key key,
		PeerNode source,
		PeerNode[] routedTo
	) {
		...
		if (source != null && !source.isLowUptime()) {
			if (Location.distance(source, target) < myDist) {
				if (logMINOR) Logger.minor(
					this,
					"Not storing because source is closer to target for " +
					key +
					" : " +
					source
				);
	...
	private UserAlert visibilityAlert = new SimpleUserAlert(
		true,
	...
	public boolean updateIsUrgent() {
		...
		if (om != null) {
			if (
				om.announcer != null && om.announcer.isWaitingForUpdater()
			) return true;
		}
		if (
			peers.getPeerNodeStatusSize(
