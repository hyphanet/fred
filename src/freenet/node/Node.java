...
public class Node implements TimeSkewDetectorCallback {
    ...
    private UserAlert visibilityAlert = new SimpleUserAlert(true, l10n("pleaseSetPeersVisibilityAlertTitle"),
    ...
    Node(PersistentConfig config, RandomSource r, RandomSource weakRandom, LoggingConfigHandler lc, NodeStarter ns,
        ...
        String tmp = "Initializing Node using Freenet Build #" + Version.buildNumber() + " r" + Version.cvsRevision()
                     ...
                     + " with " + System.getProperty("java.vendor") + " JVM version "
        ...
        nodeConfig.register("autoChangeDatabaseEncryption", true, sortOrder++, true, false,
                            "Node.autoChangeDatabaseEncryption", "Node.autoChangeDatabaseEncryptionLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
                synchronized (Node.this) {
        ...
        nodeConfig.register("defragDatabaseOnStartup", false, sortOrder++, false, true, "Node.defragDatabaseOnStartup",
                            "Node.defragDatabaseOnStartupLong", new BooleanCallback() {
            @Override
            public Boolean get() {
                synchronized (Node.this) {
        ...
        nodeConfig.register("defragOnce", false, sortOrder++, false, true, "Node.defragOnce", "Node.defragOnceLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
                synchronized (Node.this) {
        ...
        nodeConfig.register("showFriendsVisibilityAlert", false, sortOrder++, true, false,
                            "Node.showFriendsVisibilityAlert", "Node.showFriendsVisibilityAlertLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
                synchronized (Node.this) {
        ...
        nodeConfig.register("enablePerNodeFailureTables", true, sortOrder++, true, false,
                            "Node.enablePerNodeFailureTables", "Node.enablePerNodeFailureTablesLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        nodeConfig.register("routeAccordingToOurPeersLocation", true, sortOrder++, true, false,
                            "Node.routeAccordingToOurPeersLocation", "Node.routeAccordingToOurPeersLocationLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        clientCore = new NodeClientCore(this, config, nodeConfig, installConfig, getDarknetPortNumber(), sortOrder,
                                        oldConfig, fproxyConfig, toadlets, nodeDBHandle, db);
        ...
        nodeConfig.register("storeSaltHashResizeOnStart", false, sortOrder++, true, false,
                            "Node.storeSaltHashResizeOnStart", "Node.storeSaltHashResizeOnStartLong",
                            new BooleanCallback() {
            @Override
            public Boolean get() {
        ...
        if (storeType.equals("salt-hash")) {
            ...
            if ((lastVersion > 0) && (lastVersion <= lastVersionWithBloom)) {
                ...
                if (extraMemoryMB >= 10) {
                    ...
                    UpdateDeployContext.CHANGED changed = UpdateDeployContext.tryIncreaseMemoryLimit(extraMemoryMB,
                                                              " Increased because of slot filters in "
                                                              + (lastVersionWithBloom + 1));
        ...
        nodeConfig.register("clientCacheSize", "10M", sortOrder++, false, true, "Node.clientCacheSize",
                            "Node.clientCacheSizeLong", new LongCallback() {
            @Override
            public Long get() {
        ...
        nodeConfig.register("slashdotCacheSize", "10M", sortOrder++, false, true, "Node.slashdotCacheSize",
                            "Node.slashdotCacheSizeLong", new LongCallback() {
            @Override
            public Long get() {
        ...
        pubKeySlashdotcacheStore = new SlashdotStore<DSAPublicKey>(pubKeySlashdotcache, maxSlashdotCacheKeys,
                slashdotCacheLifetime, PURGE_INTERVAL, ticker, this.clientCore.tempBucketFactory);
    ...
    private InvalidConfigValueException lowBandwidthLimit(int limit) {
        return new InvalidConfigValueException(l10n("bandwidthMinimum", new String[] { "limit", "minimum" },
    ...
    public ProgramDirectory setupProgramDir(SubConfig installConfig, String cfgKey, String defaultValue,
            String shortdesc, String longdesc, String moveErrMsg, SubConfig oldConfig)
            throws NodeInitException {
    ...
    protected ProgramDirectory setupProgramDir(SubConfig installConfig, String cfgKey, String defaultValue,
            String shortdesc, String longdesc, SubConfig oldConfig)
            throws NodeInitException {
    ...
    private Configuration getNewDatabaseConfiguration(DatabaseKey databaseKey) {
        ...
        dbConfig.objectClass(freenet.client.async.PersistentCooldownQueueItem.class).objectField("keyAsBytes").indexed(
            true);
...
//      dbConfig.queries().evaluationMode(QueryEvaluationMode.LAZY);
    ...
    private void setupDatabase(DatabaseKey databaseKey)
        ...
        if (dbFileBackup.exists() &&!dbFile.exists()) {
            if (!dbFileBackup.renameTo(dbFile)) {
                throw new IOException("Database backup file " + dbFileBackup + " exists but cannot be renamed to "
                                      + dbFile + ". Not loading database, please fix permissions problems!");
            }
        }

        if (dbFileCryptBackup.exists() &&!dbFileCrypt.exists()) {
            if (!dbFileCryptBackup.renameTo(dbFileCrypt)) {
                throw new IOException("Database backup file " + dbFileCryptBackup + " exists but cannot be renamed to "
                                      + dbFileCrypt + ". Not loading database, please fix permissions problems!");
            }
        }
        ...
        try {
            if (securityLevels.getPhysicalThreatLevel() == PHYSICAL_THREAT_LEVEL.MAXIMUM) {
            ...
            } else if (dbFileCrypt.exists() && (securityLevels.getPhysicalThreatLevel() == PHYSICAL_THREAT_LEVEL.LOW)
                       && autoChangeDatabaseEncryption && enoughSpaceForAutoChangeEncryption(dbFileCrypt, false)) {
                ...
                EncryptingIoAdapter readAdapter = (EncryptingIoAdapter) adapter.open(dbFileCrypt.toString(), false, 0,
                                                      true);
                ...
                if (FileUtil.renameTo(tempFile, dbFile)) {
                    ...
                    database = openCryptDatabase(databaseKey);
                       ...
                       && autoChangeDatabaseEncryption && enoughSpaceForAutoChangeEncryption(dbFile, true)) {
                ...
                EncryptingIoAdapter readAdapter = (EncryptingIoAdapter) adapter.open(tempFile.getPath(), false, 0,
                                                      false);
            ...
            try {
                ...
                for (SplitFileInserterSegment seg : segments) {
                    try {
                        ...
                        System.out.println("Segment " + seg + " finished=" + finished + " cancelled=" + cancelled
                                           + " empty=" + empty + " encoded=" + encoded + " size="
                                           + seg.countDataBlocks() + " data " + seg.countCheckBlocks()
    ...
    private boolean enoughSpaceForAutoChangeEncryption(File file, boolean isCrypt) {
        ...
        if (freeSpace < minSpace) {
            System.err.println(l10n(isCrypt ? "notEnoughSpaceToAutoEncrypt" : "notEnoughSpaceToAutoDecrypt",
                                    new String[] { "size",
                    "file" }, new String[] { SizeUtil.formatSize(minSpace), dbFile.getAbsolutePath() }));
    ...
    private void maybeDefragmentDatabase(File databaseFile, final DatabaseKey databaseKey) throws IOException {
        ...
        DefragmentConfig config = new DefragmentConfig(databaseFile.getPath(), backupFile.getPath(),
                                      new BTreeIDMapping(tmpFile.getPath()));
        ...
        if (newSize <= 0) {
            ...
            FileUtil.secureDelete(tmpFile);
            FileUtil.secureDelete(backupFile);
            System.err.println("Defragment completed. " + SizeUtil.formatSize(oldSize) + " (" + oldSize + ") -> "
    ...
    private void createAutoCryptFailedUserAlert() {
        ...
        if (notEnoughSpaceRenameFailed) {
            ...
            text = l10n((isCrypt ? "failedToRenameEncrypting" : "failedToRenameDecrypting"), new String[] { "fromfile",
                    "tofile" }, new String[] { renameFailedFrom.getAbsolutePath(), renameFailedTo.getAbsolutePath() });
        } else {
            ...
            text = l10n((isCrypt ? "notEnoughSpaceToAutoEncrypt" : "notEnoughSpaceToAutoDecrypt"),
                        new String[] { "size",
                                       "file" }, new String[] { SizeUtil.formatSize(notEnoughSpaceMinimumSpace),
    ...
    private <T extends StorableBlock> FreenetStore<T> makeStore(String type, boolean isStore, StoreCallback<T> cb,
            boolean dontResizeOnStart, byte[] clientCacheMasterKey)
            throws IOException {
    ...
    private <T extends StorableBlock> FreenetStore<T> makeStore(String type, String store, long maxKeys,
            StoreCallback<T> cb, boolean lateStart, byte[] clientCacheMasterKey)
            throws IOException {
        ...
        SaltedHashFreenetStore<T> fs = SaltedHashFreenetStore.<T>construct(getStoreDir(), type + "-" + store, cb,
                                           ...
                                           storeSaltHashResizeOnStart &&!lateStart, lateStart ? ticker : null,
                                           clientCacheMasterKey);
...
//      pluginManager3 = new freenet.plugin_new.PluginManager(pluginManagerConfig);
    ...
    private void checkForEvilJVMBugs() {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "JVM vendor: " + jvmVendor + ", JVM name: " + jvmName + ", JVM version: " + javaVersion
...
//      }
    ...
    public Object makeRequestSender(Key key, short htl, long uid, RequestTag tag, PeerNode source, boolean localOnly,
                                    boolean ignoreStore, boolean offersOnly, boolean canReadClientCache,
                                    boolean canWriteClientCache, boolean realTimeFlag) {
        boolean canWriteDatastore = canWriteDatastoreRequest(htl);

        if (logMINOR) {
            Logger.minor(this,
                         "makeRequestSender(" + key + ',' + htl + ',' + uid + ',' + source + ") on "
    ...
    public void store(SSKBlock block, boolean deep, boolean overwrite, boolean canWriteClientCache,
        ...
        try {
            ...
            getPubKey.cacheKey((block.getKey()).getPubKeyHash(), (block.getKey()).getPubKey(), deep,
                               canWriteClientCache, canWriteDatastore, forULPR || useSlashdotCache,
    ...
    private void handleFproxyNodeToNodeTextMessageSimpleFieldSet(SimpleFieldSet fs, DarknetPeerNode source,
        ...
        if (type == Node.N2N_TEXT_MESSAGE_TYPE_USERALERT) {
            ...
            Logger.error(this,
                         "Received unknown fproxy node to node message sub-type '" + type + "' from "
                         + source.getPeer());
        }
    }
    ...
    public DarknetPeerNode createNewDarknetNode(SimpleFieldSet fs, FRIEND_TRUST trust, FRIEND_VISIBILITY visibility)
            throws FSParseException, PeerParseException, ReferenceSignatureVerificationException {
    ...
    public void setMasterPassword(String password, boolean inFirstTimeWizard)
            throws AlreadySetPasswordException, MasterKeysWrongPasswordException, MasterKeysFileSizeException,
                   IOException {
        synchronized (this) {
            if (enteredPassword) {
    ...
    public boolean shouldStoreDeep(Key key, PeerNode source, PeerNode[] routedTo) {
        ...
        for (PeerNode pn : routedTo) {
            if ((Location.distance(pn, target) < myDist) &&!pn.isLowUptime()) {
                if (logMINOR) {
                    Logger.minor(this,
                                 "Not storing because peer " + pn + " is closer to target for " + key + " his loc "
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Should store maybe, peer " + pn + " loc = " + pn.getLocation() + " my loc is "
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Should store returning true for " + key + " target=" + target + " myLoc=" + myLoc
                ...
                if (val.equals("salt-hash")) {
                    ...
                    if (key == null) {
                        ...
                        try {
                            ...
                            throw new InvalidConfigValueException("Master keys file corrupted (too "
                                    + e1.sizeToString() + ")");
                        } catch (IOException e1) {
    ...
    private static class DB4ODiagnosticListener implements DiagnosticListener {
        ...
        public void onDiagnostic(Diagnostic arg0) {
            ...
            if (arg0 instanceof DiagnosticBase) {
                ...
                Logger.debug(this,
                             "Diagnostic: " + d.getClass() + " : " + d.problem() + " : " + d.solution() + " : "
                             + d.reason(), new Exception("debug"));
            } else {
