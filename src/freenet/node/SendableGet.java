...
public abstract class SendableGet extends BaseSendableGet {
	...
	static final SendableGetRequestSender sender =
		new SendableGetRequestSender();
	 ...
	 * @return
	 */
	public abstract long getCooldownWakeup(
		SendableRequestItem token,
		ClientContext context
	);
	...
	public static FetchException translateException(LowLevelGetException e) {
		switch (e.code) {
			...
			default:
				Logger.error(
					SimpleSingleFileFetcher.class,
					"Unknown LowLevelGetException code: " + e.code
				);
				return new FetchException(
					FetchExceptionMode.INTERNAL_ERROR,
					"Unknown error code: " + e.code
				);
		}
	}

	@Override
	public boolean reduceWakeupTime(
				...
				.queueNormalOrDrop(
					new PersistentJob() {
						...
						public boolean run(ClientContext context) {
							((WantsCooldownCallback) parent).enterCooldown(
									getClientGetState(),
									wakeupTime,
									context
								);
			...
			context
				...
				.queueNormalOrDrop(
					new PersistentJob() {
						@Override
						public boolean run(ClientContext context) {
							((WantsCooldownCallback) parent).clearCooldown(
									getClientGetState()
								);
							return false;
						}
