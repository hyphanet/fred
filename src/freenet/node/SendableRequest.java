...
public abstract class SendableRequest implements RandomGrabArrayItem {
     ...
     */
    public abstract SendableRequestItem chooseKey(KeysFetchingLocally keys, ObjectContainer container,
    ...
    public abstract void internalError(Throwable t, RequestScheduler sched, ObjectContainer container,
                                       ClientContext context, boolean persistent);

    /**
     * Construct a full set of ChosenBlock's for a persistent request. These are transient, so we will need to clone keys
     * etc. 
     */
    public abstract List<PersistentChosenBlock> makeBlocks(PersistentChosenRequest request, RequestScheduler sched,
