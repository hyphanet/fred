...
public class AnnounceSender implements PrioRunnable, ByteCounter {
	...
	private boolean validateForwardReply(Message msg, PeerNode next) {
		...
		noderefBuf = om.innerWaitForOpennetNoderef(xferUID, paddedLength, noderefLength, next, false, uid, true, this);
		if(noderefBuf == null) {
	...
	private void complete() {
		msg = DMT.createFNPOpennetAnnounceCompleted(uid);
		if(source != null) {
			try {
				source.sendAsync(msg, null, this);
			} catch (NotConnectedException e) {
	...
	private void sendNotWanted() throws NotConnectedException {
		Message newMsg = DMT.createFNPOpennetAnnounceNodeNotWanted(uid);
		source.sendAsync(newMsg, null, this);
	}
