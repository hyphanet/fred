...
public class AnnounceSender implements PrioRunnable, ByteCounter {
	...
	private short htl;
	private Location.Valid target;
	private final AnnouncementCallback cb;
	private final PeerNode onlyNode;
	private int forwardedRefs;

	public AnnounceSender(Location.Valid target, short htl, long uid, PeerNode source, OpennetManager om, Node node, long xferUID, int noderefLength, int paddedLength, AnnouncementCallback cb) {
		this.source = source;
		this.uid = uid;
		this.om = om;
		this.node = node;
	...
	public AnnounceSender(Location.Valid target, OpennetManager om, Node node, AnnouncementCallback cb, PeerNode onlyNode) {
		source = null;
