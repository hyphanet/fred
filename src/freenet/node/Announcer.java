...
public class Announcer {
    ...
    private void connectSomeSeednodes() {
        ...
        synchronized (this) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "count = " + count + " announced = " + announcedToIdentities.size() + " running = "
    ...
    private synchronized int connectSomeNodesInner(List<SimpleFieldSet> seeds) {
        ...
        while (count < CONNECT_AT_ONCE) {
            ...
            try {
                SeedServerPeerNode seed = new SeedServerPeerNode(fs, node, om.crypto, node.peers, false,
                                              om.crypto.packetMangler);
    ...
    boolean enoughPeers() {
        ...
        if (opennetCount >= target) {
            if (logMINOR) {
                Logger.minor(this,
                             "We have enough opennet peers: " + opennetCount + " > " + target + " since "
        ...
        AnnounceSender sender = new AnnounceSender(node.getLocation(), om, node, new AnnouncementCallback() {
            ...
            public void completed() {
                ...
                synchronized (Announcer.this) {
                    runningAnnouncements--;
                    Logger.normal(this,
                                  "Announcement to " + seed.userToString() + " completed, now running "
            ...
            public void nodeNotAdded() {
                Logger.normal(this,
                              "Announcement to " + seed.userToString()
    ...
    class AnnouncementUserEvent extends AbstractUserEvent {
        ...
        public String getText() {
            ...
            if (node.clientCore.isAdvancedModeEnabled()) {
                ...
                sb.append(l10n("announceDetails", new String[] {
                    "addedNodes", "refusedNodes", "recentSentAnnouncements", "runningAnnouncements",
                    "connectedSeednodes", "disconnectedSeednodes"
                }, new String[] {
                    Integer.toString(addedNodes), Integer.toString(refusedNodes),
