...
public class NodeARKInserter implements ClientPutCallback, RequestClient {
	...
	private void innerUpdate() {
		...
		if (entries != null) {
			...
			if (logMINOR) Logger.minor(
				this,
				darknetOpennetString + " ref's physical.udp is null"
			);
		}
		...
		if (logMINOR) Logger.minor(
			...
			"Inserting " +
			darknetOpennetString +
			" ARK because peers list changed"
		);
	...
	private boolean checkIPUpdated() {
		...
		synchronized (this) {
			if (lastInsertedPeers != null) {
				...
				for (int i = 0; i < p.length; i++) if (
					!p[i].strictEquals(lastInsertedPeers[i])
				) return true;
			} else {
	...
	private void startInserter() {
		if (!canStart) {
			if (logMINOR) Logger.minor(
				this,
				darknetOpennetString + " ARK inserter can't start yet"
			);
			return;
		}
		...
		if (logMINOR) Logger.minor(
			this,
			"starting " + darknetOpennetString + " ARK inserter"
		);
		...
		InsertContext ctx = node.clientCore
			.makeClient((short) 0, true, false)
			.getInsertContext(true);
		inserter = new ClientPutter(
			this,
			b,
			uri,
			...
			synchronized (this) {
				if (fs.get("physical.udp") == null) lastInsertedPeers = null;
					...
					try {
						...
						for (int i = 0; i < all.length; i++) peers[i] =
							new Peer(all[i], false);
	...
	public void onSuccess(BaseClientPutter state) {
		...
		if (logMINOR) Logger.minor(
			this,
			darknetOpennetString + " ARK insert succeeded: " + uri
		);
		synchronized (this) {
			inserter = null;
	...
	public void onFailure(InsertException e, BaseClientPutter state) {
		if (logMINOR) Logger.minor(
			this,
			darknetOpennetString + " ARK insert failed: " + e
		);
		synchronized (this) {
	...
	public void onGeneratedURI(FreenetURI uri, BaseClientPutter state) {
		if (logMINOR) Logger.minor(
			this,
			"Generated URI for " + darknetOpennetString + " ARK: " + uri
		);
	...
	public void onGeneratedMetadata(Bucket metadata, BaseClientPutter state) {
		Logger.error(
			this,
			"Bogus onGeneratedMetadata() on " + this + " from " + state,
			new Exception("error")
		);
		metadata.free();
	}
