...
public abstract class BaseSender implements ByteCounter {
    ...
    protected static enum DO { FINISHED, WAIT, NEXT_PEER }
...
loadWaiterLoop:
        while (true) {
            ...
            expectedAcceptState = next.outputLoadTracker(realTimeFlag).tryRouteTo(origTag,
                    RequestLikelyAcceptedState.LIKELY, false);
            ...
            if (expectedAcceptState == RequestLikelyAcceptedState.UNKNOWN) {
                ...
                if (expectedAcceptState != null) {
                    if (logMINOR) {
                        Logger.minor(this,
                                     "Predicted accept state for " + this + " : " + expectedAcceptState + " realtime="
                                     + realTimeFlag);
                    }
                    ...
                    if ((lastNext == next) && (lastExpectedAcceptState == RequestLikelyAcceptedState.GUARANTEED)
                        ...
                        Logger.warning(this,
                                       ...
                                       + lastExpectedAcceptState + " is now " + expectedAcceptState + " from "
                                       + next.shortToString() + " (" + next.getVersionNumber() + ")");
                    ...
                    if (next.isLowCapacity(realTimeFlag)) {
                        if ((waiter.waitingForCount() == 1    // if not, already accepted
                            ...
                            if (alsoWaitFor != null) {
                                ...
                                if (logMINOR) {
                                    Logger.minor(this,
                                                 "Waiting for " + next + " and " + alsoWaitFor + " on " + waiter
                                                 + " because realtime");
                                }
                ...
                if (expectedAcceptState == null) {
                    ...
                    if (waited == null) {
                        ...
                        if (logMINOR) {
                            Logger.minor(this,
                                         "Sending to " + next + " after waited for "
                ...
                if (logMINOR) {
                    Logger.minor(this,
                                 "Leaving new load management big block: Predicted accept state for " + this + " : "
    ...
    private PeerNode closerPeer(HashSet<PeerNode> exclude, long now, boolean newLoadManagement) {
        return node.peers.closerPeer(sourceForRouting(), exclude, target, true, node.isAdvancedModeEnabled(), -1, null,
                                     2.0, isInsert() ? null : key, htl, ignoreLowBackoff(), source == null,
                                     realTimeFlag, null, false, now, newLoadManagement);
    }
    ...
    private void logDelta(long delta, int tryCount, boolean waitedForLoadManagement, boolean retriedForLoadManagement) {
        ...
        if ((delta > longTimeout) || (tryCount > 3)) {
            Logger.error(this,
                         "Took " + tryCount + " tries in " + TimeUtil.formatTime(delta, 2, true) + " waited="
