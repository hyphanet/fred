...
public abstract class BaseSender implements ByteCounter {
	...
	public static int calculateTimeout(
		...
		double timeout = realTimeFlag
			? SEARCH_TIMEOUT_REALTIME
			: SEARCH_TIMEOUT_BULK;
		timeout = ((timeout * ((double) htl + EXTRA_HOPS_AT_BOTTOM)) /
	...
	protected void innerRouteRequestsOld(PeerNode next, UIDTag origTag) {
		...
		try {
			...
			node.peers.incrementSelectionSamples(
	...
	protected void innerRouteRequestsNew(PeerNode next, UIDTag origTag) {
		NodeStats.RequestType type = isSSK
			? NodeStats.RequestType.SSK_REQUEST
			: NodeStats.RequestType.CHK_REQUEST;
		...
		loadWaiterLoop:while (true) {
			...
			synchronized (this) {
				if (rejectedLoops > MAX_REJECTED_LOOPS) canRerouteWhileWaiting =
					false;
			}
			...
			if (expectedAcceptState == RequestLikelyAcceptedState.UNKNOWN) {
				...
				if (expectedAcceptState == null) {
					if (logMINOR) Logger.minor(
						this,
						"Cannot send to " + next + " realtime=" + realTimeFlag
					);
					waitedForLoadManagement = true;
					if (waiter == null) waiter = PeerNode.createSlotWaiter(
						origTag,
						type,
						false,
						realTimeFlag,
					...
					if (next.isLowCapacity(realTimeFlag)) {
						if (
							waiter.waitingForCount() == 1 && // if not, already accepted
							...
							PeerNode alsoWaitFor = closerPeer(
								exclude,
								now,
								true
							);
							if (alsoWaitFor != null) {
								...
								try {
									...
									if (logMINOR) Logger.minor(
										this,
										"Rerouting as slot waiter failed..."
									);
									continue;
								}
								if (matched != null) {
									expectedAcceptState =
										waiter.getAcceptedState();
									next = matched;
									if (logMINOR) Logger.minor(
										this,
										"Matched " +
				...
				if (expectedAcceptState == null) {
					...
					if (waited == null) {
						...
						if (logMINOR) Logger.minor(
							this,
							"Timed out waiting for a peer to accept " +
							this +
							" on " +
							waiter
						);
			...
			if (logMINOR) Logger.minor(
				this,
				"Routing request to " + next + " realtime=" + realTimeFlag
			);
			nodesRoutedTo.add(next);
	...
	private double getLoad(HashSet<PeerNode> waitedFor) {
		...
		for (PeerNode pn : waitedFor) {
			...
			pn
				.outputLoadTracker(realTimeFlag)
				.proportionTimingOutFatallyInWait();
		}
	...
	protected long getLongSlotWaiterTimeout() {
		return (
			(realTimeFlag ? SEARCH_TIMEOUT_REALTIME : SEARCH_TIMEOUT_BULK) / 5
		);
	}

	protected long getShortSlotWaiterTimeout() {
		return (
			(realTimeFlag ? SEARCH_TIMEOUT_REALTIME : SEARCH_TIMEOUT_BULK) / 20
		);
	}
	...
	private void logDelta(
		...
		if ((delta > longTimeout) || tryCount > 3) Logger.error(
			this,
			"Took " +
			tryCount +
			" tries in " +
		...
		else if ((delta > longTimeout / 5) || tryCount > 1) Logger.warning(
			this,
			"Took " +
			tryCount +
			" tries in " +
			...
			logMINOR && (waitedForLoadManagement || retriedForLoadManagement)
		) Logger.minor(
			this,
			"Took " +
			tryCount +
	...
	private DO waitForAccepted(
		...
		while (true) {
			...
			if (msg == null) {
				if (logMINOR) Logger.minor(
					this,
					"Timeout waiting for Accepted for " + this
				);
			...
			if (msg.getSpec() == DMT.FNPRejectedOverload) {
				...
				if (msg.getBoolean(DMT.IS_LOCAL)) {
					...
					if (
						msg.getSubMessage(DMT.FNPRejectIsSoft) != null &&
						expectedAcceptState != null
					) {
						if (logMINOR) Logger.minor(
							this,
							"Soft rejection, waiting to resend"
						);
						if (
							expectedAcceptState ==
							RequestLikelyAcceptedState.GUARANTEED
						...
						next.noLongerRoutingTo(origTag, false);
						if (softRejectCount == null) softRejectCount =
							new HashMap<PeerNode, Integer>();
			...
			next
				.outputLoadTracker(realTimeFlag)
				.clearDontSendUnlessGuaranteed();
			return DO.FINISHED;
		}
	}

	protected abstract void handleAcceptedRejectedTimeout(
		final PeerNode next,
