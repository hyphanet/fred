...
public class HourlyStatsRecord {
	...
	public synchronized void remoteRequest(
		...
		if (location < 0 || location > 1) throw new IllegalArgumentException(
			"Invalid location."
		);
	...
	private static final DecimalFormat fix3p3pct = new DecimalFormat(
		"##0.000%"
	);
	...
	public synchronized String toString() {
		...
		s
			...
			.append("\n");
		s
			.append("HourlyStats: Node uptime (ms):\t")
			.append(node.getUptime())
			.append("\n");
		s
			.append("HourlyStats: build:\t")
	...
	public void fillRemoteRequestHTLsBox(HTMLNode html) {
		...
		synchronized (this) {
			...
			if (totalCHKT > 0) totalCHKRate = ((double) (totalCHKLS +
					totalCHKRS)) /
			totalCHKT;
			if (totalSSKT > 0) totalSSKRate = ((double) (totalSSKLS +
					totalSSKRS)) /
			totalSSKT;
	...
	private class StatsLine {
		...
		public String toString() {
			...
			sb
				.append(fix4p.format(fixNaN(chkLocalSuccess.currentValue())))
				.append("\t");
			sb
				.append(fix4p.format(fixNaN(chkRemoteSuccess.currentValue())))
				.append("\t");
			sb
				.append(fix4p.format(fixNaN(chkFailure.currentValue())))
				.append("\t");
			sb
				.append(fix4p.format(fixNaN(sskLocalSuccess.currentValue())))
				.append("\t");
			sb
				.append(fix4p.format(fixNaN(sskRemoteSuccess.currentValue())))
				.append("\t");
			sb
				.append(fix4p.format(fixNaN(sskFailure.currentValue())))
				.append("\t");
			return sb.toString();
		}
	}
