...
public class NodeClientCore implements Persistable, DBJobRunner, OOMHook, ExecutorIdleCallback {
    ...
    NodeClientCore(Node node, Config config, SubConfig nodeConfig, SubConfig installConfig, int portNumber,
                   ...
                   long nodeDBHandle, ObjectContainer container)
        ...
        clientDatabaseExecutor = new PrioritizedSerialExecutor(NativeThread.NORM_PRIORITY,
                NativeThread.MAX_PRIORITY + 1, NativeThread.NORM_PRIORITY, true, SECONDS.toMillis(30), this,
                node.nodeStats);
            ...
            this.getTicker().queueTimedJob(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        getTicker().queueTimedJob(this, "BandwidthStatsPutter", PersistentStatsPutter.OFFSET, false,
                                                  true);
                    } catch (DatabaseDisabledException e) {
        ...
        nodeConfig.register("maxRAMBucketSize", SizeUtil.formatSizeWithoutSpace(maxBucketSize), sortOrder++, true,
                            false, "NodeClientCore.maxRAMBucketSize", "NodeClientCore.maxRAMBucketSizeLong",
                            new LongCallback() {
            @Override
            public Long get() {
        ...
        nodeConfig.register("RAMBucketPoolSize", defaultRamBucketPoolSize + "MiB", sortOrder++, true, false,
                            "NodeClientCore.ramBucketPoolSize", "NodeClientCore.ramBucketPoolSizeLong",
                            new LongCallback() {
            @Override
            public Long get() {
        ...
        tempBucketFactory = new TempBucketFactory(node.executor, tempFilenameGenerator,
                nodeConfig.getLong("maxRAMBucketSize"), nodeConfig.getLong("RAMBucketPoolSize"), random,
                node.fastWeakRandom, nodeConfig.getBoolean("encryptTempBuckets"));
        ...
        healingQueue = new SimpleHealingQueue(new InsertContext(0, 2, 0, 0, new SimpleEventProducer(), false,
                Node.FORK_ON_CACHEABLE_DEFAULT, false, Compressor.DEFAULT_COMPRESSORDESCRIPTOR, 0, 0,
                InsertContext.CompatibilityMode.COMPAT_CURRENT), RequestStarter.PREFETCH_PRIORITY_CLASS,
                    512 /* FIXME make configurable */);
        clientContext = new ClientContext(node.bootID, nodeDBHandle, this, fecQueue, node.executor,
                                          backgroundBlockEncoder, archiveManager, persistentTempBucketFactory,
                                          tempBucketFactory, persistentTempBucketFactory, healingQueue, uskManager,
        ...
        try {
            requestStarters = new RequestStarterGroup(node, this, portNumber, random, config, throttleFS,
                    clientContext, nodeDBHandle, container);
        ...
        // Downloads directory
        this.downloadsDir = node.setupProgramDir(nodeConfig, "downloadsDir",
                node.userDir().file("downloads").getPath(), "NodeClientCore.downloadsDir",
                "NodeClientCore.downloadsDirLong", l10n("couldNotFindOrCreateDir"), (SubConfig) null);
        ...
        nodeConfig.register("uploadAllowedDirs", new String[] { "all" }, sortOrder++, true, true,
                            "NodeClientCore.uploadAllowedDirs", "NodeClientCore.uploadAllowedDirsLong",
                            new StringArrCallback() {
            @Override
            public String[] get() {
                synchronized (NodeClientCore.this) {
        ...
        nodeConfig.register("maxBackgroundUSKFetchers", "64", sortOrder++, true, false,
                            "NodeClientCore.maxUSKFetchers", "NodeClientCore.maxUSKFetchersLong", new IntCallback() {
            @Override
            public Integer get() {
        ...
        this.alerts.register(new SimpleUserAlert(true,
                ...
                NodeL10n.getBase().getString("QueueToadlet.persistenceBroken", new String[] { "TEMPDIR",
                "DBFILE" }, new String[] {
                    new File(FileUtil.getCanonicalFile(getPersistentTempDir()), File.separator).toString(),
                    new File(
                        FileUtil.getCanonicalFile(node.getUserDir()),
                        "node.db4o").toString() }), NodeL10n.getBase().getString(
                            "QueueToadlet.persistenceBrokenShortAlert"), UserAlert.CRITICAL_ERROR) {
            @Override
        ...
        fecQueue.init(RequestStarter.NUMBER_OF_PRIORITY_CLASSES, FEC_QUEUE_CACHE_SIZE, clientContext.jobRunner,
                      node.executor, clientContext);
    ...
    private void initPTBF(ObjectContainer container, SubConfig nodeConfig) throws NodeInitException {
        ...
        try {
            ...
            if (!killedDatabase) {
                ptbf = PersistentTempBucketFactory.load(getPersistentTempDir(), prefix, random, node.fastWeakRandom,
                        container, node.nodeDBHandle, nodeConfig.getBoolean("encryptPersistentTempBuckets"), this,
                        node.getTicker());
    ...
    private void initRestartJobs(long nodeDBHandle, ObjectContainer container) {
        ...
        if ((startupDatabaseJobs != null) && (startupDatabaseJobs.length > 0)) {
            try {
    ...
    private FECQueue initFECQueue(long nodeDBHandle, ObjectContainer container, FECQueue oldQueue) {
        ...
        try {
            ...
            if (!killedDatabase) {
                q = FECQueue.create(node.nodeDBHandle, container, oldQueue);
            } else {
    ...
    private void migratePluginStores(ObjectContainer container) {
        try {
            ...
            System.err.println("Failed to migrate plugin stores due to database error: " + e
                               + " - assuming node.db4o[.crypt] is corrupt.");
            Logger.error(this,
                         "Failed to migrate plugin stores due to database error: " + e
                         + " - assuming node.db4o[.crypt] is corrupt.", e);
            killedDatabase = true;
        } catch (Throwable e) {
        ...
        asyncGet(key, offersOnly, uid, new RequestSenderListener() {
            ...
            public void onRequestSenderFinished(int status, boolean fromOfferedKey, RequestSender rs) {
                ...
                if ((status != RequestSender.TIMED_OUT) && (status != RequestSender.GENERATED_REJECTED_OVERLOAD)
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     (isSSK ? "SSK" : "CHK") + " fetch cost " + rs.getTotalSentBytes() + '/'
                ...
                if ((status == RequestSender.TIMED_OUT) || (status == RequestSender.GENERATED_REJECTED_OVERLOAD)) {
                    if (!rejectedOverload) {
                        ...
                        if (isSSK) {
                           ...
                           && ((status == RequestSender.DATA_NOT_FOUND) || (status == RequestSender.RECENTLY_FAILED)
                               || (status == RequestSender.SUCCESS) || (status == RequestSender.ROUTE_NOT_FOUND)
                ...
                if (status == RequestSender.SUCCESS) {
                    ...
                    default :
                        Logger.error(this,
                                     "Unknown RequestSender code in get" + (isSSK ? "SSK" : "CHK") + ": " + status
                                     + " on " + rs);
                        listener.onFailed(new LowLevelGetException(LowLevelGetException.INTERNAL_ERROR));
        ...
        try {
            ...
            while (true) {
                ...
                if ((status != RequestSender.TIMED_OUT) && (status != RequestSender.GENERATED_REJECTED_OVERLOAD)
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "CHK fetch cost " + rs.getTotalSentBytes() + '/' + rs.getTotalReceivedBytes()
                                     + " bytes (" + status + ')');
                    }
        ...
        try {
            ...
            while (true) {
                ...
                if ((status != RequestSender.TIMED_OUT) && (status != RequestSender.GENERATED_REJECTED_OVERLOAD)
                    ...
                    if (logMINOR) {
                        Logger.minor(this,
                                     "SSK fetch cost " + rs.getTotalSentBytes() + '/' + rs.getTotalReceivedBytes()
                                     + " bytes (" + status + ')');
                    }
    ...
    public void realPutCHK(CHKBlock block, boolean canWriteClientCache, boolean forkOnCacheable, boolean preferInsert,
        ...
        try {
            ...
            is = node.makeInsertSender(block.getKey(), node.maxHTL(), uid, tag, null, headers, prb, false,
                                       canWriteClientCache, forkOnCacheable, preferInsert, ignoreLowBackoff,
                                       realTimeFlag);
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Completed " + uid + " overload=" + hasReceivedRejectedOverload + ' '
                             + is.getStatusString());
            }
