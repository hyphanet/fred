	...
	public static void main(String[] args)
		...
		try {
			...
			RandomAccessBucket data =
				node.clientCore.tempBucketFactory.makeBucket(TEST_SIZE);
			OutputStream os = data.getOutputStream();
			try {
				byte[] buf = new byte[4096];
				for (long written = 0; written < TEST_SIZE;) {
					node.fastWeakRandom.nextBytes(buf);
					int toWrite = (int) Math.min(
						TEST_SIZE - written,
						buf.length
					);
			...
			File secondInnerDir = new File(
				dir,
				Integer.toString(DARKNET_PORT2)
			);
			...
			secondNode = NodeStarter.createTestNode(
				DARKNET_PORT2,
				OPENNET_PORT2,
				dir.getPath(),
				false,
