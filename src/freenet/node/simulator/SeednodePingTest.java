...
public class SeednodePingTest extends RealNodeTest {
	...
	static File STATUS_DIR = new File(
		"/var/www/freenet/tests/seednodes/status/"
	);
	...
	public static void main(String[] args)
		throws FSParseException, IOException, OpennetDisabledException, PeerParseException, InterruptedException, ReferenceSignatureVerificationException, NodeInitException, InvalidThresholdException {
		Node node = null;
		try {
			...
			>();
			List<SimpleFieldSet> seedNodesAsSFS = Announcer.readSeednodes(
				new File("/tmp/", NodeFile.Seednodes.getFilename())
			);
			...
			if (seedNodes.size() != numberOfNodesInTheFile) System.out.println(
				"ERROR ADDING SOME OF THE SEEDNODES!!"
			);
			System.out.println(
				"Let some time for the " +
				seedNodes.size() +
				" nodes to connect..."
			);
			...
			while (System.currentTimeMillis() < deadline) {
				...
				for (SeedServerPeerNode seednode : node.peers.getConnectedSeedServerPeersVector(
					null
				)) {
					try {
						...
						// sanity check
						if (seednode.isRoutable()) System.out.println(
							seednode + " is routable!"
						);
				...
				Map<FATE, Integer> totals = new EnumMap<FATE, Integer>(
					SeedServerTestPeerNode.FATE.class
				);
				...
				for (Entry<FATE, Integer> fateEntry : totals.entrySet()) {
					System.out.println(
						fateEntry.getKey() + " : " + fateEntry.getValue()
					);
				}
				System.out.println(
					"################## (" +
			...
			for (SeedServerTestPeerNode peer : seedNodes) {
				...
				br.close();
				if (
					firstSample < countSince && successes == 0
				) System.err.println(
					"RESULT:" +
					peer.getIdentityString() +
					" NOT CONNECTED IN LAST WEEK! LAST CONNECTED: " +
					(lastSuccess > 0
							? TimeUtil.formatTime(writeTime - lastSuccess)
							: "NEVER")
				);
				System.out.println(
					peer.getIdentityString() +
