...
public class RealNodePitchBlackMitigationTest extends RealNodeTest {
	...
	public static final long PITCH_BLACK_MITIGATION_STARTUP_DELAY =
		MINUTES.toMillis(1);
	public static final long PITCH_BLACK_MITIGATION_FREQUENCY_ONE_DAY =
		MINUTES.toMillis(30);
	public static final int PINGS_PER_ITERATION = 10;

	public static int DARKNET_PORT_BASE =
		RealNodeRequestInsertTest.DARKNET_PORT_END;
	public static final int DARKNET_PORT_END =
		DARKNET_PORT_BASE + NUMBER_OF_NODES;
	...
	public static void main(String[] args) throws Exception {
		...
		nodes[0].lm.PITCH_BLACK_MITIGATION_FREQUENCY_ONE_DAY =
			PITCH_BLACK_MITIGATION_FREQUENCY_ONE_DAY;
		nodes[0].lm.PITCH_BLACK_MITIGATION_STARTUP_DELAY =
		...
		waitForPingAverage(
			0.98,
			nodes,
			new DummyRandomSource(3143),
			MAX_PINGS,
			BETWEEN_PING_SLEEP_TIME
		);
		System.exit(0);
	}
	...
	static void waitForPingAverage(
		...
		RunningAverage avg2 = new BootstrappingDecayingRunningAverage(
			0.0,
			0.0,
			1.0,
			100,
		...
		for (int total = 0; total < maxTests; total++) {
			...
			System.err.println(
				"This cycle ratio: " +
				((double) (noSwaps - lastNoSwaps)) /
					((double) (newSwaps - lastSwaps))
			);
			lastNoSwaps = noSwaps;
			System.err.println(
			...
			System.err.println(
				"Swaps rejected (nowhere to go): " +
				LocationManager.swapsRejectedNowhereToGo
			);
			System.err.println(
				"Swaps rejected (rate limit): " +
				LocationManager.swapsRejectedRateLimit
			);
			System.err.println(
				"Swaps rejected (recognized ID):" +
			...
			System.err.println(
				"Average swap time: " + (totalSwapTime / nodes.length)
			);
			System.err.println(
				"Average swap sender interval: " +
			...
			for (int i = 0; i < PINGS_PER_ITERATION; i++) {
				...
				try {
					...
					int hopsTaken = randomNode.routedPing(
						loc2,
						randomNode2.getDarknetPubKeyHash()
					);
					pings++;
					if (hopsTaken < 0) {
						failures++;
			...
			if (
				...
				(pings > MIN_PINGS &&
					...
					((double) successes / ((double) (failures + successes)) >
						accuracy))
			) {
				System.err.println();
				System.err.println(
					"Reached " + (accuracy * 100) + "% accuracy."
				);
				System.err.println();
				...
				System.err.println(
					"Total started swaps: " + LocationManager.startedSwaps
				);
				System.err.println(
				...
				System.err.println(
					"Total swaps rejected (rate limit): " +
					LocationManager.swapsRejectedRateLimit
				);
				System.err.println(
				...
				System.err.println(
					"Total swaps failed:" + LocationManager.noSwaps
				);
				System.err.println(
					"Total swaps succeeded:" + LocationManager.swaps
				);
				return;
			}
		}
