...
public class RealNodeULPRTest extends RealNodeTest {
	...
	public static final int DARKNET_PORT_BASE =
		RealNodePingTest.DARKNET_PORT_END;
	public static final int DARKNET_PORT_END =
		DARKNET_PORT_BASE + NUMBER_OF_NODES;

	public static void main(String[] args)
		throws FSParseException, PeerParseException, CHKEncodeException, InvalidThresholdException, NodeInitException, ReferenceSignatureVerificationException, KeyCollisionException, SSKEncodeException, IOException, InterruptedException, SSKVerifyException, InvalidCompressionCodecException, PeerTooOldException {
		...
		for (int i = 0; i < NUMBER_OF_NODES; i++) refs[i] =
		...
		for (int i = 0; i < NUMBER_OF_NODES * 5; i++) {
			if (i % NUMBER_OF_NODES == 0) Logger.normal(
				RealNodeRoutingTest.class,
				String.valueOf(i)
			);
		...
		for (int totalCount = 0; totalCount < NUMBER_OF_TESTS; totalCount++) {
			...
			if (isSSK) {
				...
				block = ((InsertableClientSSK) insertKey).encode(
						new ArrayBucket(buf),
						false,
						false,
						(short) -1,
			...
			System.err.println();
			System.err.println(
				"Created random test key " + testKey + " = " + nodeKey
			);
			System.err.println();
			Logger.error(
			...
			NodeDispatcherCallback cb = new NodeDispatcherCallback() {
				...
				public void snoop(Message m, Node n) {
					if (
						((!isSSK) && m.getSpec() == DMT.FNPCHKDataRequest) ||
						(isSSK && m.getSpec() == DMT.FNPSSKDataRequest)
					) {
						Key key = (Key) m.getObject(DMT.FREENET_ROUTING_KEY);
						if (key.equals(nodeKey)) {
							visited[n.getDarknetPortNumber() -
								DARKNET_PORT_BASE] = true;
						}
					}
				}
			};
				...
				try {
					...
					switch (e.code) {
						...
						default:
							...
							System.exit(
								EXIT_UNKNOWN_ERROR_CHECKING_KEY_NOT_EXIST
							);
					}
				}
			}
			...
			Logger.normal(
				RealNodeULPRTest.class,
				"Inserting to node " + (nodes.length - 1)
			);
			...
			Logger.normal(
				RealNodeULPRTest.class,
				"Inserted to node " + (nodes.length - 1)
			);
			...
			while (true) {
				...
				for (Node node : nodes) {
					if (
						node.hasKey(fetchKey.getNodeKey(false), true, true)
					) count++;
				}
				System.err.println(
					"T=" +
				...
				if (count == nodes.length) {
					...
					System.err.println(
						...
						(totalPropagationTime / successfulTests) +
						"ms"
					);
					System.err.println();
					break;
				...
				if (x % nodes.length == 0) {
					...
					for (int i = 0; i < nodes.length; i++) if (
						nodes[i].hasKey(fetchKey.getNodeKey(false), true, true)
					) {
						System.err.print(i + " ");
					}
					System.err.println();
		...
		System.err.println(
			"Overall average propagation time: " +
			(totalPropagationTime / successfulTests) +
			"ms"
		);
		System.exit(0);
