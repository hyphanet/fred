...
public class RealNodeULPRTest extends RealNodeTest {
    ...
    public static void main(String[] args)
        ...
        for (int i = 0; i < NUMBER_OF_NODES; i++) {
            nodes[i] = NodeStarter.createTestNode(DARKNET_PORT_BASE + i, 0, testName, true, MAX_HTL, 20 /* 5% */,
                    random, executor, 500 * NUMBER_OF_NODES, 1024 * 1024, true, ENABLE_SWAPPING, false, ENABLE_ULPRS,
        ...
        for (int totalCount = 0; totalCount < NUMBER_OF_TESTS; totalCount++) {
            ...
            if (isSSK) {
                ...
                block = ((InsertableClientSSK) insertKey).encode(new ArrayBucket(buf), false, false, (short) -1,
                        buf.length, random, COMPRESSOR_TYPE.DEFAULT_COMPRESSORDESCRIPTOR, false);
            } else {
                block = ClientCHKBlock.encode(buf, false, false, (short) -1, buf.length,
                                              COMPRESSOR_TYPE.DEFAULT_COMPRESSORDESCRIPTOR, false);
