...
public class RealNodeBusyNetworkTest extends RealNodeRoutingTest {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(
			"Busy network test (inserts/retrieves in quantity/stress test)"
		);
		System.out.println();
		...
		HighLevelSimpleClient[] clients =
			new HighLevelSimpleClient[nodes.length];
		for (int i = 0; i < clients.length; i++) {
			clients[i] = nodes[i].clientCore.makeClient(
		...
		for (int i = 0; i < INSERT_KEYS; i++) {
			...
			ClientCHKBlock newBlock = new ClientCHKBlock(
				encData,
				encHeaders,
				chk,
				true
			...
			Logger.minor(
				...
				"Decoded: " +
				new String(newBlock.memoryDecode(), StandardCharsets.UTF_8)
			);
			Logger.normal(
				RealNodeRequestInsertTest.class,
				"CHK: " + chk.getURI()
			...
			Logger.minor(
				RealNodeRequestInsertTest.class,
				"Headers: " + HexUtil.bytesToHex(block.getHeaders())
			);
			// Insert it.
			try {
		...
		for (int i = 0; i < INSERT_KEYS; i++) {
			...
			System.err.println(
				"Queueing requests for " + i + " of " + INSERT_KEYS
			);
