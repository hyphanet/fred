	...
	static void waitForAllConnected(Node[] nodes) throws InterruptedException {
		...
		while (true) {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				int countAlmostConnected =
					nodes[i].peers.countAlmostConnectedDarknetPeers();
				int countTotal = nodes[i].peers.countValidPeers();
				int countBackedOff = nodes[i].peers.countBackedOffPeers(false);
				int countCompatible =
					nodes[i].peers.countCompatibleDarknetPeers();
				...
				if (countBackedOff > 0) {
					if (logMINOR) Logger.minor(
						RealNodeTest.class,
						"Backed off: " + nodes[i] + " : " + countBackedOff
					);
				}
			}
			double avgPingTime = totalPingTime / nodes.length;
			if (
				countFullyConnected == nodes.length &&
