    ...
    static void waitForAllConnected(Node[] nodes) throws InterruptedException {
        ...
        while (true) {
                ...
                if (countConnected == countTotal) {
                    ...
                    if (logMINOR) {
                        Logger.minor(RealNodeTest.class,
                                     "Connection count for " + nodes[i] + " : " + countConnected + " partial "
            ...
            if ((countFullyConnected == nodes.length) && (countReallyConnected == nodes.length)
                    && (totalBackedOff == 0) && (minPingTime < NodeStats.DEFAULT_SUB_MAX_PING_TIME)
                ...
                System.err.println("Waiting for nodes to be fully connected: " + countFullyConnected + " / "
                                   + nodes.length + " (" + totalConnections + " / " + totalPeers
                                   + " connections total partial " + totalPartialConnections + " compatible "
                                   + totalCompatibleConnections + ") - backed off " + totalBackedOff
                                   + " ping min/avg/max " + (int) minPingTime + "/" + (int) avgPingTime + "/"
