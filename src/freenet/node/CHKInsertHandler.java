...
public class CHKInsertHandler implements PrioRunnable, ByteCounter {
	...
	public void run() {
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Exiting CHKInsertHandler.run() for " + uid
			);
			tag.unlockHandler();
		}
	}
	...
	private void realRun() {
		...
		try {
			...
			Logger.normal(
				this,
				"Disconnected while waiting for DataInsert on " + uid
			);
			return;
		}
		...
		if (htl > 0) sender = node.makeInsertSender(
			key,
			htl,
			uid,
			tag,
		...
		node.executor.execute(
			dataReceiver,
			"CHKInsertHandler$DataReceiver for UID " + uid
		);
		...
		while (true) {
			synchronized (sender) {
				try {
					if (
						sender.getStatus() == CHKInsertSender.NOT_FINISHED
					) sender.wait(5000);
			...
			if (
				(!receivedRejectedOverload) && sender.receivedRejectedOverload()
			) {
				receivedRejectedOverload = true;
				// Forward it
				// Does not need to be sent synchronously since is non-terminal.
				Message m = DMT.createFNPRejectedOverload(
					uid,
					false,
					true,
					realTimeFlag
			...
			if (
				...
				try {
					...
					Logger.error(
						this,
						"Took too long to send " + msg + " to " + source
					);
					return;
				}
			...
			if (
				(status == CHKInsertSender.ROUTE_NOT_FOUND) ||
			...
			Logger.error(
				this,
				"Unknown status code: " + sender.getStatusString()
			);
	...
	private MessageFilter makeDataInsertFilter(long timeout) {
		MessageFilter mfDataInsert = MessageFilter.create()
	...
	private void handleNoDataInsert() {
		try {
			...
			if (
				source.isConnected() &&
				(startTime >
					(source.timeLastConnectionCompleted() +
						Node.HANDSHAKE_TIMEOUT * 4))
			) Logger.warning(
				this,
				"Did not receive DataInsert on " +
				uid +
				" from " +
				source +
				" !"
			...
			source.localRejectedOverload(
				"TimedOutAwaitingDataInsert",
				realTimeFlag
			);
	...
	private void finish(int code) {
		...
		if ((sender != null) && (!sentCompletionWasSet)) {
			...
			if (routingTookTooLong) {
				...
				try {
					source.sendAsync(
						DMT.createFNPInsertTransfersCompleted(uid, true),
						null,
						this
					);
				} catch (NotConnectedException e) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Completed after telling downstream on " + this
				);
			}
			boolean failed = sender.anyTransfersFailed();
			if (!sentCompletionWasSet) m =
				DMT.createFNPInsertTransfersCompleted(uid, failed);
		}
		...
		if ((sender == null) && (!sentCompletionWasSet) && (canCommit)) {
			...
			m = DMT.createFNPInsertTransfersCompleted(
				uid,
				false
				/* no timeouts */
			);
		...
		if (m != null) {
			try {
				...
				if (logMINOR) Logger.minor(
					this,
					"Sent completion: " + m + " for " + this
				);
			} catch (NotConnectedException e1) {
				if (logMINOR) Logger.minor(
					this,
					"Not connected: " + source + " for " + this
				);
				// May need to commit anyway...
			} catch (SyncSendWaitedTooLongException e) {
				Logger.error(
					this,
					"Took too long to send " + m + " to " + source
				);
				// May need to commit anyway...
			}
		}
		...
		if (
			code != CHKInsertSender.TIMED_OUT &&
			...
			node.nodeStats.remoteChkInsertBytesReceivedAverage.report(
				totalReceived
			);
			if (code == CHKInsertSender.SUCCESS) {
				// Report both sent and received because we have both a Handler and a Sender
				if (
					sender != null && sender.startedSendingData()
				) node.nodeStats.successfulChkInsertBytesSentAverage.report(
					totalSent
				);
				node.nodeStats.successfulChkInsertBytesReceivedAverage.report(
					totalReceived
				);
			}
		}
	...
	private CHKBlock verify() {
		...
		synchronized (this) {
			...
			try {
				...
				toSend = DMT.createFNPDataInsertRejected(
					uid,
		...
		try {
			node.store(
				...
				node.shouldStoreDeep(
					key,
					source,
					sender == null ? new PeerNode[0] : sender.getRoutedTo()
				),
				false,
				canWriteDatastore,
				false
	...
	public class DataReceiver implements PrioRunnable {
		...
		public void run() {
			...
			if (logMINOR) Logger.minor(
				this,
				"Receiving data for " + CHKInsertHandler.this
			);
			...
			br.receive(
				new BlockReceiverCompletion() {
					...
					public void blockReceived(byte[] buf) {
						if (logMINOR) Logger.minor(
							this,
							"Received data for " + CHKInsertHandler.this
						);
					...
					public void blockReceiveFailed(RetrievalException e) {
						...
						// Cancel the sender
						if (sender != null) sender.onReceiveFailed(); // tell it to stop if it hasn't already failed... unless it's sending from store
						runThread.interrupt();
						...
						Message msg = DMT.createFNPDataInsertRejected(
							uid,
	...
	private BlockReceiverTimeoutHandler myTimeoutHandler =
		new BlockReceiverTimeoutHandler() {
			...
			public void onFatalTimeout(PeerContext receivingFrom) {
				Logger.error(
					this,
					"Fatal timeout receiving insert " +
					CHKInsertHandler.this +
					" from " +
					receivingFrom
				);
