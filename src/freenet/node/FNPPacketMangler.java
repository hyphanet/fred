...
public class FNPPacketMangler implements OutgoingPacketMangler {
	...
	*/
	...
	public FNPPacketMangler(ProtectedNode node, NodeCrypto crypt, PacketSocketHandler sock) {
		this.node = node;
		this.crypto = (ProtectedNodeCrypto) crypt;
		this.sock = sock;
	...
	private void processJFKMessage3(byte[] payload, int inputOffset, PeerNode pn,Peer replyTo, boolean oldOpennetPeer, boolean unknownInitiator, int setupType, int negType)
	{
		...
		if(newTrackerID > 0) {
			...
			if(dontWant) {
				node.getPeerManager().disconnectAndRemove(pn, true, true, true); // Let it connect then tell it to remove it.
			} else {
	...
	private PeerNode getPeerNodeFromUnknownInitiator(byte[] hisRef, int setupType, PeerNode pn, Peer from) {
		if(setupType == SETUP_OPENNET_SEEDNODE) {
			...
			node.getPeerManager().addPeer(seed);
			return seed;
		} else {
	...
	private void sendAuthPacket(byte[] output, BlockCipher cipher, PeerNode pn, Peer replyTo, boolean anonAuth) {
		...
		if(prePaddingLength < maxPacketSize) {
			paddingLength = node.getWeakRNG().nextInt(Math.min(100, maxPacketSize - prePaddingLength));
		} else {
		...
		Util.randomBytes(node.getWeakRNG(), data, hash.length+iv.length+2+output.length, paddingLength);
		try {
	...
	public void sendHandshake(PeerNode pn, boolean notRegistered) {
		...
		if(negType == -1) {
			...
			negType = negTypes[node.getRNG().nextInt(negTypes.length)];
	...
	private boolean maybeResetTransientKey() {
		...
		synchronized (authenticatorCache) {
			...
			node.getRNG().nextBytes(transientKey);
