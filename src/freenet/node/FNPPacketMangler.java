...
public class FNPPacketMangler implements OutgoingPacketMangler {
    ...
    private static final int HEADERS_LENGTH_MINIMUM = 4 +            // sequence number
    ...
    static public final int HEADERS_LENGTH_ONE_MESSAGE = HEADERS_LENGTH_MINIMUM + 2;    // 2 bytes = length of message. rest is the same.
    ...
    private boolean tryProcessAuth(byte[] buf, int offset, int length, PeerNode pn, Peer peer, boolean oldOpennetPeer,
        ...
        if (logDEBUG) {
            Logger.debug(this,
                         "Decrypt key: " + HexUtil.bytesToHex(pn.incomingSetupKey) + " for " + peer + " : " + pn
                         + " in tryProcessAuth");
        }
        ...
        if (dataLength > length - (ivLength + hash.length + 2)) {
            if (logDEBUG) {
                Logger.debug(this,
                             "Invalid data length " + dataLength + " (" + (length - (ivLength + hash.length + 2))
                             + ") in tryProcessAuth");
            }
        ...
        if (MessageDigest.isEqual(realHash, hash)) {
            ...
            if (logDEBUG) {
                Logger.debug(this,
                             "Incorrect hash in tryProcessAuth for " + peer + " (length=" + dataLength
    ...
    private boolean tryProcessAuthAnon(byte[] buf, int offset, int length, Peer peer) {
        ...
        if (MessageDigest.isEqual(realHash, hash)) {
            ...
            if (logMINOR) {
                Logger.minor(this,
                             "Incorrect hash in tryProcessAuthAnon for " + peer + " (length=" + dataLength
                             + "): \nreal hash=" + HexUtil.bytesToHex(realHash) + "\n bad hash="
                             + HexUtil.bytesToHex(hash));
            }
    ...
    private void processDecryptedAuthAnon(final byte[] payload, final Peer replyTo) {
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Received anonymous auth packet (phase=" + packetType + ", v=" + version + ", nt=" + negType
                         + ", setup type=" + setupType + ") from " + replyTo + "");
        }
        ...
        if ((packetType == 0) || (packetType == 2)) {
            ...
            Logger.error(this,
                         "Invalid phase " + packetType + " for anonymous-initiator (we are the responder) from "
                         + replyTo);
        }
    }
    ...
    private void processDecryptedAuthAnonReply(final byte[] payload, final Peer replyTo, final PeerNode pn) {
        if (logMINOR) {
            Logger.minor(this,
                         "Processing decrypted auth packet from " + replyTo + " for " + pn + " length "
                         + payload.length);
        }
        ...
        if ((packetType == 1) || (packetType == 3)) {
            ...
            Logger.error(this,
                         "Invalid phase " + packetType + " for anonymous-initiator (we are the initiator) from "
                         + replyTo);
        }
    }
    ...
    private void processDecryptedAuth(final byte[] payload, final PeerNode pn, final Peer replyTo,
        ...
        if (logMINOR) {
            ...
            Logger.minor(this,
                         "Received auth packet for " + pn.getPeer() + " (phase=" + packetType + ", v=" + version
        ...
        if ((negType >= 0) && (negType < 6)) {
            ...
            Logger.error(this,
                         "Decrypted auth packet but unknown negotiation type " + negType + " from " + replyTo
                         + " possibly from " + pn);
    ...
    private void processJFKMessage1(byte[] payload, int offset, PeerNode pn, Peer replyTo, boolean unknownInitiator,
        ...
        if (payload.length < nonceSize + modulusLength + 3 + (unknownInitiator ? NodeCrypto.IDENTITY_LENGTH : 0)) {
            Logger.error(this,
                         "Packet too short from " + pn + ": " + payload.length
                         + " after decryption in JFK(1), should be " + (nonceSize + modulusLength));
        ...
        if ((negType >= 8)
                || DiffieHellman.checkDHExponentialValidity(this.getClass(), new NativeBigInteger(1, hisExponential))) {
            ...
            Logger.error(this,
                         "We can't accept the exponential " + pn
    ...
    private void sendJFKMessage1(PeerNode pn, Peer replyTo, boolean unknownInitiator, int setupType, int negType)
        ...
        if (negType < 8) {    // Legacy DH
            if ((ctx == null) ||!(ctx instanceof DiffieHellmanLightContext)
        ...
        byte[] message1 =
            new byte[nonceSizeHashed + modulusLength + (unknownInitiator ? NodeCrypto.IDENTITY_LENGTH : 0)];
    ...
    private void sendJFKMessage2(byte[] nonceInitator, byte[] hisExponential, PeerNode pn, Peer replyTo,
        ...
        byte[] authenticator = HMAC.macWithSHA256(getTransientKey(),
                                   assembleJFKAuthenticator(myExponential, hisExponential, myNonce, nonceInitator,
                                       replyTo.getAddress().getAddress()), HASH_LENGTH);
    ...
    private void processJFKMessage2(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo,
        ...
        if (payload.length < expectedLength + 3) {
            Logger.error(this,
                         "Packet too short from " + pn.getPeer() + ": " + payload.length
        ...
        if (negType < 8) {    // legacy DH
            ...
            if (!DiffieHellman.checkDHExponentialValidity(this.getClass(), _hisExponential)) {
                Logger.error(this,
                             "We can't accept the exponential " + pn.getPeer()
                             + " sent us!! REDFLAG: IT CAN'T HAPPEN UNLESS AGAINST AN ACTIVE ATTACKER!!");
        ...
        if (negType < 9) {
            ...
            if (!DSA.verify(pn.peerPubKey, remoteSignature,
                            new NativeBigInteger(1, SHA256.digest(locallyExpectedExponentials)), false)) {
            ...
            if (!ECDSA.verify(Curves.P256, pn.peerECDSAPubKey(), sig, hisExponential)) {
                if (pn.peerECDSAPubKeyHash() == null) {
                    ...
                    Logger.error(this,
                                 "Peer attempting negType " + negType + " with ECDSA but no ECDSA key known: "
                                 + pn.userToString());
                ...
                if (logDEBUG) {
                    Logger.debug(this,
                                 "Expected signature on " + HexUtil.bytesToHex(hisExponential) + " with "
                                 + HexUtil.bytesToHex(pn.peerECDSAPubKeyHash()) + " signature "
    ...
    private void processJFKMessage3(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo, boolean oldOpennetPeer,
        ...
        if (!HMAC.verifyWithSHA256(getTransientKey(),
                                   assembleJFKAuthenticator(responderExponential, initiatorExponential, nonceResponder,
                                       nonceInitiatorHashed, replyTo.getAddress().getAddress()), authenticator)) {
            if (shouldLogErrorInHandshake(t1)) {
                if (logDEBUG) {
        ...
        if (message4 != null) {
            ...
            if (logDEBUG) {
                Logger.debug(this,
                             "No message4 found for " + HexUtil.bytesToHex(authenticator) + " responderExponential "
                             ...
                             + Fields.hashCode(nonceInitiatorHashed) + " address "
        ...
        if (negType >= 7) {
            ...
            theirInitialMsgID = ((sharedData[8] & 0xFF) << 24) | ((sharedData[9] & 0xFF) << 16)
                                | ((sharedData[10] & 0xFF) << 8) | (sharedData[11] & 0xFF);
            ourInitialMsgID = ((sharedData[12] & 0xFF) << 24) | ((sharedData[13] & 0xFF) << 16)
                              | ((sharedData[14] & 0xFF) << 8) | (sharedData[15] & 0xFF);
        }
        ...
        byte[] toVerify = assembleDHParams(nonceInitiatorHashed, nonceResponder, initiatorExponential,
                                           responderExponential, crypto.getIdentity(negType, false), data);
        ...
        long newTrackerID = pn.completedHandshake(bootID, hisRef, 0, hisRef.length, outgoingCipher, outgoingKey,
                                incommingCipher, incommingKey, replyTo, true, negType, trackerID, false, false,
                                hmacKey, ivCipher, ivNonce, ourInitialSeqNum, theirInitialSeqNum, ourInitialMsgID,
                                theirInitialMsgID);
        ...
        if (newTrackerID > 0) {
            ...
            sendJFKMessage4(1, negType, 3, nonceInitiatorHashed, nonceResponder, initiatorExponential,
                            responderExponential, c, Ke, Ka, authenticator, hisRef, pn, replyTo, unknownInitiator,
        ...
        if ((t2 - t1) > 500) {
            Logger.error(this,
                         "Message3 Processing packet for " + pn.getPeer() + " took "
    ...
    private boolean processJFKMessage4(byte[] payload, int inputOffset, PeerNode pn, Peer replyTo,
        ...
        byte[] locallyGeneratedText =
            new byte[nonceSizeHashed + nonceSize + modulusLength * 2 + identity.length + dataLen + pn.jfkMyRef.length];
        ...
        if (negType < 9) {    // DSA sig
            ...
            if (!DSA.verify(pn.peerPubKey, remoteSignature, new NativeBigInteger(1, messageHash), false)) {
                String error = "The signature verification has failed!! JFK(4) -" + pn.getPeer() + " message hash "
            ...
            if (!ECDSA.verify(Curves.P256, pn.peerECDSAPubKey(), sig, locallyGeneratedText)) {
                Logger.error(this,
                             "The ECDSA signature verification has failed!! JFK(4) - " + pn.getPeer() + " length "
                             + locallyGeneratedText.length + " hisRef " + hisRef.length + " hash "
        ...
        long newTrackerID = pn.completedHandshake(bootID, hisRef, 0, hisRef.length, outgoingCipher, pn.outgoingKey,
                                incommingCipher, pn.incommingKey, replyTo, false, negType, trackerID, true,
    ...
    private void sendJFKMessage3(int version, final int negType, int phase, byte[] nonceInitiator,
        ...
        if (negType >= 7) {
            ...
            pn.ourInitialMsgID = ((sharedData[8] & 0xFF) << 24) | ((sharedData[9] & 0xFF) << 16)
                                 | ((sharedData[10] & 0xFF) << 8) | (sharedData[11] & 0xFF);
            pn.theirInitialMsgID = ((sharedData[12] & 0xFF) << 24) | ((sharedData[13] & 0xFF) << 16)
                                   | ((sharedData[14] & 0xFF) << 8) | (sharedData[15] & 0xFF);
        }
    ...
    private void sendJFKMessage4(int version, int negType, int phase, byte[] nonceInitiatorHashed,
                                 byte[] nonceResponder, byte[] initiatorExponential, byte[] responderExponential,
                                 BlockCipher c, byte[] Ke, byte[] Ka, byte[] authenticator, byte[] hisRef, PeerNode pn,
                                 Peer replyTo, boolean unknownInitiator, int setupType, long newTrackerID,
        ...
        byte[] params = assembleDHParams(nonceInitiatorHashed, nonceResponder, initiatorExponential,
                                         responderExponential, pn.getIdentity(negType), data);

        if (logMINOR) {
            Logger.minor(this,
                         "Message length " + params.length + " myRef: " + myRef.length + " hash "
                         + Fields.hashCode(myRef) + " hisRef: " + hisRef.length + " hash " + Fields.hashCode(hisRef)
                         + " boot ID " + node.bootID);
        }
    ...
    private void sendAuthPacket(int version, int negType, int phase, byte[] data, PeerNode pn, Peer replyTo) {
        ...
        if (logMINOR) {
            ...
            delta = TimeUtil.formatTime(now - last, 2, true) + " ago";
            Logger.minor(this,
                         "Sending auth packet for " + String.valueOf(pn.getPeer()) + " (phase=" + phase + ", ver="
                         + version + ", nt=" + negType + ") (last packet sent " + delta + ") to " + replyTo
    ...
    private void sendAnonAuthPacket(int version, int negType, int phase, int setupType, byte[] data, PeerNode pn,
        ...
        if (logMINOR) {
            Logger.minor(this,
                         "Sending anon auth packet (phase=" + phase + ", ver=" + version + ", nt=" + negType
    ...
    private void sendAuthPacket(byte[] output, BlockCipher cipher, PeerNode pn, Peer replyTo, boolean anonAuth) {
        ...
        if (prePaddingLength < maxPacketSize) {
            ...
            Logger.error(this,
                         "Warning: sending oversize auth packet (anonAuth=" + anonAuth + ") of " + prePaddingLength
                         + " bytes!");
        }
        ...
        try {
            ...
            Logger.warning(this,
                           "Tried to send auth packet to local address: " + replyTo + " for " + pn
    ...
    public void sendHandshake(PeerNode pn, boolean notRegistered) {
        ...
        if (negType == -1) {
            ...
            Logger.normal(this,
                          "Cannot send handshake to " + pn + " because no common negTypes, choosing random negType of "
                          + negType);
        }
    ...
    private DiffieHellmanLightContext _genLightDiffieHellmanContext() {
        ...
        if (logDEBUG) {
            Logger.debug(this,
                         "Signing for DiffieHellman: "
                         + HexUtil.bytesToHex(SHA256.digest(assembleDHParams(ctx.getPublicKeyNetworkFormat(),
                             crypto.getCryptoGroup()))) + " for "
                                 + HexUtil.bytesToHex(assembleDHParams(ctx.getPublicKeyNetworkFormat(),
                                     crypto.getCryptoGroup())));
        }

        ctx.setDSASignature(crypto.sign(SHA256.digest(assembleDHParams(ctx.getPublicKeyNetworkFormat(),
                crypto.getCryptoGroup()))));
    ...
    private ECDHLightContext _genECDHLightContext() {
        ...
        if (logDEBUG) {
            Logger.debug(this,
                         "ECDSA Signature: " + HexUtil.bytesToHex(ctx.ecdsaSig) + " for "
    ...
    private byte[] assembleDHParams(byte[] nonceInitiator, byte[] nonceResponder, byte[] initiatorExponential,
        ...
        byte[] result =
            new byte[nonceInitiator.length + nonceResponder.length + initiatorExponential.length + responderExponential.length + id.length + sa.length];
        int offset = 0;
    ...
    private boolean maybeResetTransientKey() {
        ...
        Logger.normal(this,
                      ...
                      + (isCacheTooBig
                         ? ("the cache is oversized (" + authenticatorCacheSize + ')') : "it's time to rekey") + " on "
                             + this);
    ...
    private static class NoContextsException extends Exception {
        private enum CONTEXT { SENDING, REPLYING }
