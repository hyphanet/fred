...
public class NodeIPDetector {
	...
	private void onAddedValidIP() {
		node.getClientCore().alerts.unregister(primaryIPUndetectedAlert);
		node.onAddedValidIP();
	}
	
	private void onNotAddedValidIP() {
		node.getClientCore().alerts.register(primaryIPUndetectedAlert);
	}
	...
	private boolean innerDetect(List<FreenetInetAddress> addresses) {
		...
		if(node.getPeerManager() != null) {
			PeerNode[] peerList = ((ProtectedPeerManager) node.getPeerManager()).myPeers();
	...
	public void setMaybeSymmetric() {
		if(ipDetectorManager != null && ipDetectorManager.isEmpty()) {
			...
			if(node.getClientCore() != null && node.getClientCore().alerts != null)
				node.getClientCore().alerts.register(maybeSymmetricAlert);
		} else {
			if(maybeSymmetricAlert != null)
				node.getClientCore().alerts.unregister(maybeSymmetricAlert);
		}
	}
	...
	private void onGetValidAddressOverride() {
		node.getClientCore().alerts.unregister(invalidAddressOverrideAlert);
	}
	
	private void onNotGetValidAddressOverride() {
		node.getClientCore().alerts.register(invalidAddressOverrideAlert);
	}
