...
public class NodeIPDetector {
	...
	FreenetInetAddress[] detectPrimaryIPAddress(boolean dumpLocalAddresses) {
		...
		if (overrideIPAddress != null) {
			...
			if (
				overrideIPAddress.isRealInternetAddress(false, true, false)
			) addedValidIP = true;
		}
		...
		lastIPAddress = addresses.toArray(
			new FreenetInetAddress[addresses.size()]
		);
		if (dumpLocalAddresses) {
			ArrayList<FreenetInetAddress> filtered = new ArrayList<
				FreenetInetAddress
			>(lastIPAddress.length);
			for (FreenetInetAddress addr : lastIPAddress) {
				if (addr == null) continue;
				if (
					addr == overrideIPAddress && addr.hasHostnameNoIP()
				) filtered.add(addr);
	...
	private boolean innerDetect(List<FreenetInetAddress> addresses) {
		...
		if ((pluginDetectedIPs != null) && (pluginDetectedIPs.length > 0)) {
			for (DetectedIP pluginDetectedIP : pluginDetectedIPs) {
				...
				if (!addresses.contains(a)) {
					...
					addresses.add(a);
					if (
						a.isRealInternetAddress(false, false, false)
					) addedValidIP = true;
				}
			}
		}
		...
		if (node.peers != null) {
			...
			for (PeerNode pn : peerList) {
				...
				if (!pn.isRealConnection()) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Not a real connection and not a seed node: " + pn
					);
				}
				if (logMINOR) Logger.minor(
					this,
					"Maybe a usable connection for IP: " + pn
				);
				...
				if (logMINOR) Logger.minor(
					this,
					"Peer " + pn.getPeer() + " thinks we are " + addr
				);
			...
			if (countsByPeer.size() == 1) {
				Entry<FreenetInetAddress, Integer> countByPeer = countsByPeer
					.entrySet()
					.iterator()
					.next();
				...
				for (Map.Entry<
					FreenetInetAddress,
					Integer
				> entry : countsByPeer.entrySet()) {
					FreenetInetAddress cur = entry.getKey();
					int curPop = entry.getValue();
					Logger.minor(
						this,
						"Detected peer: " + cur + " popularity " + curPop
					);
				...
				if (best != null) {
					...
					for (InetAddress detectedAddr : detectedAddrs) {
						if (
							IPUtil.isValidAddress(detectedAddr, false)
					...
					if ((bestPopularity > 1) || !hasRealDetectedAddress) {
						if (!addresses.contains(best)) {
							...
							addresses.add(best);
							if (
								best.isRealInternetAddress(false, false, false)
							) addedValidIP = true;
						}
						confidence = bestPopularity;
						if (
							(secondBest != null) && (secondBestPopularity > 1)
						) {
							if (!addresses.contains(secondBest)) {
								Logger.minor(
									this,
									"Adding second best peer " +
									secondBest +
									" (" +
									secondBest +
									')'
								...
								if (
									secondBest.isRealInternetAddress(
										false,
										false,
										false
									)
	...
	private String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("NodeIPDetector." + key, pattern, value);
	}
	...
	public boolean hasDirectlyDetectedIP() {
		...
		for (InetAddress addr : addrs) {
			if (IPUtil.isValidAddress(addr, false)) {
				if (logMINOR) Logger.minor(
					this,
					"Has a directly detected IP: " + addr
				);
				return true;
			}
		}
	...
	public void processDetectedIPs(DetectedIP[] list) {
		...
		for (DetectedIP pluginDetectedIP : pluginDetectedIPs) reportMTU(
			pluginDetectedIP.mtu,
			pluginDetectedIP.publicAddress instanceof Inet6Address
		);
		redetectAddress();
	}
	...
	public int registerConfigs(SubConfig nodeConfig, int sortOrder) {
		...
		if (overrideIPAddressString.length() == 0) overrideIPAddress = null;
			...
			try {
				overrideIPAddress = new FreenetInetAddress(
				...
				System.err.println(
					msg +
					" but starting up anyway, ignoring the configured IP override"
				);
				overrideIPAddress = null;
			} catch (UnknownHostException e) {
				...
				Logger.error(this, msg);
				System.err.println(
					msg + " but starting up anyway with no IP override"
				);
				overrideIPAddress = null;
			}
		}
	...
	public void setMaybeSymmetric() {
		if (ipDetectorManager != null && ipDetectorManager.isEmpty()) {
			if (maybeSymmetricAlert == null) {
				maybeSymmetricAlert = new SimpleUserAlert(
					true,
			...
			if (
				node.clientCore != null && node.clientCore.alerts != null
			) node.clientCore.alerts.register(maybeSymmetricAlert);
		} else {
			if (maybeSymmetricAlert != null) node.clientCore.alerts.unregister(
				maybeSymmetricAlert
			);
		}
	}
