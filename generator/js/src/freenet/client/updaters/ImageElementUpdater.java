...
public class ImageElementUpdater extends ReplacerUpdater {
        ...
        if (lastMessage != null) {
          MessageManager.get()
              .replaceMessageAtPosition(
                  MessageManager.get().getMessagePosition(lastMessage), newMsg);
        } else {
          MessageManager.get().addMessage(newMsg);
        }
        lastMessage = newMsg;
   ...
   *
   * @param fetched - The number of the fetched blocks
   * @param total - The number of the total blocks
   * @return The message to be shown in the messages panel
   */
  private Message makeProgressMsg(int fetched, int total) {
    return new Message(
        L10n.get("imageprogress") + fetched + "/" + total, Priority.MINOR, null, true);
  }
    ...
    for (int j = 0; j < inputs.getLength(); j++) {
      ...
      if (input.getAttribute("name").compareTo("fetchedBlocks") == 0) {
        try {
          ...
          FreenetJs.log(
              "fetchedBlocks value \"" + input.getAttribute("value") + "\" is invalid: " + e);
        }
        ...
        try {
          ...
          FreenetJs.log(
              "requiredBlocks value \"" + input.getAttribute("value") + "\" is invalid: " + e);
        }
      }
    }
