...
public class SharedConnectionManager
	implements IConnectionManager, IUpdateManager {
	...
	private LongPollingConnectionManager longPollingManager =
		new LongPollingConnectionManager(this);
		...
		followerTakeOverTimer = new Timer() {
			...
			public void run() {
				...
				if (
					...
					FreenetJs.log(
						"Getting leadership lastKeepalive:" +
						Cookies.getCookie(LEADER_KEEPALIVE)
					);
	...
	public void openConnection() {
		...
		if (
			Cookies.getCookie(LEADER_NAME) == null ||
			...
			followerTakeOverTimer.scheduleRepeating(
				sharedConnectionKeepaliveIntervalInMs * 3
			);
	...
	public void updated(String message) {
		...
		FreenetJs.log(
			"SharedConnectionManagaer updated:requestId:" + requestId
		);
		...
		if (requestId.compareTo(FreenetJs.requestId) == 0) {
			...
			Cookies.setCookie(
				MESSAGE_PREFIX + (messageCounter % MAX_MESSAGES),
				message,
				null,
				null,
				"/",
	...
	private void processMessages() {
		...
		while (
			...
			String message = Cookies.getCookie(
				MESSAGE_PREFIX + (messageCounter % MAX_MESSAGES)
			);
			FreenetJs.log("Got messsage:" + message);
			String requestId = Base64.decode(
				message.substring(
					0,
					message.indexOf(UpdaterConstants.SEPARATOR)
				)
			);
			FreenetJs.log("Got requestId:" + requestId);
			String msg = message.substring(
				message.indexOf(UpdaterConstants.SEPARATOR) + 1
			);
			FreenetJs.log(
				"Processing message:requestId:" + requestId + " msg:" + msg
			);
			if (requestId.compareTo(FreenetJs.requestId) == 0) {
