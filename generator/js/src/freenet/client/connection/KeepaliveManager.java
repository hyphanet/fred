...
/**
 * This ConnectionManager sends a keepalive message periodically to notify the server that the page
 * is still open. If a keepalive fails, it tells the client that the server already cleaned up this
 * request.
 */
public class KeepaliveManager implements IConnectionManager {
  ...
  public void closeConnection() {
    timer.cancel();
    // If it wasn't cancelled, then we show a message about pushing cancelled. It makes sure that
    // this message shows only once
    if (cancelled == false) {
      if (FreenetJs.isPushingCancelledExpected == false) {
        MessageManager.get()
            .addMessage(new Message(L10n.get("pushingCancelled"), Priority.ERROR, null, true));
      }
      cancelled = true;
    }
  }
