...
public class MessageManager {
   ...
   *
   * @param position - The position of the message that is removed
   */
   ...
   *
   * @param msg - The message which position will be returned
   ...
   * @param msg - The message that will replace the original
   */
                  ...
                  FreenetRequest.sendRequest(
                      ...
                      new RequestCallback() {
                        ...
                        public void onResponseReceived(Request request, Response response) {
                          // When a response is got, the server is already removed the message. We
                          // can remove it too safely
                          removeMessage(m);
                        }

                        @Override
                        public void onError(Request request, Throwable exception) {
                          // Don't do anything. If the server removed the message, it will push the
                          // change, if not, the user will try again
                        }
                      });
                } else {
  ...
  public void updateMessages() {
    ...
    if (RootPanel.get("alerts") != null) {
      ...
      for (int i = 0;
          i < RootPanel.get("alerts").getElement().getElementsByTagName("alert").getLength();
          i++) {
        Element alert =
            RootPanel.get("alerts").getElement().getElementsByTagName("alert").getItem(i);
        String anchor = alert.getElementsByTagName("anchor").getItem(0).getInnerText();
        Priority priority = null;
        switch (Integer.parseInt(
            alert.getElementsByTagName("priority").getItem(0).getInnerText())) {
          case 0:
        ...
        addMessage(
            new Message(
                ...
                Boolean.parseBoolean(
                    alert.getElementsByTagName("canDismiss").getItem(0).getInnerText())));
      }
    }
  }
}
