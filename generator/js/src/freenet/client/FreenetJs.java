...
public class FreenetJs implements EntryPoint {
        ...
        new ClosingHandler() {
          ...
          public void onWindowClosing(ClosingEvent event) {
            ...
            FreenetRequest.sendRequest(
                UpdaterConstants.leavingPath, new QueryParameter("requestId", requestId));
            cm.closeConnection();
          }
        });
      ...
      if (isDebug) {
        ...
        try {
          FreenetRequest.sendRequest(
              ...
              new QueryParameter("msg", requestId + ":" + (logCounter++) + ":" + urlEncode(msg)));
        } catch (Exception e) {
   ...
   * Java's URLEncoder isn't available, and Freenet's URLEncoder doesn't compile: getBytes() doesn't
   * work. So hack together a pathetic feature incomplete encoder that doesn't use getBytes().
